...
여덟 섹션(5.4): 컨셉 / 연구 목표 / 가설과 판정 상태 / 관련 세부영역 / 단계 진행 현황 / 살아있는 산출물 링크 / 최근 실행 / 참고 자료. 제목·순서 고정.
...
> 트랙 상태: {{track_status}} · 현재 단계: {{current_stage_no}}. {{current_stage_name}} · 마지막 트랙 실행: {{last_run_or_없음}}
<!-- 트랙 상태는 config/tracks/<slug>.yaml 의 status(active | paused | done). 현재 단계는 번호와 이름을 함께. 퍼블리셔가 5절과 함께 이 줄도 최신 값으로 맞춘다는 보장은 없으므로, 스토리텔러가 트랙 실행마다 이 줄을 갱신한다. -->
...
<!-- 네 산출물(온톨로지 초안, 비교표, 매트릭스, 평가 절차)은 필수 링크. 각 줄 끝에 현재 상태 한 줄(예: "빈 틀", "v0 시드", "단계 1 실행 2026-09-26-01 에서 초안 작성"). 숫자는 백로그 페이지와 맞춘다. -->
...
{{footnotes}}
<!-- 각주 정의만 둔다. 형식: "[^ref-003]: 기관, 제목, 발행일, URL, 접근일". 트랙 정의 문서(사양서 8장)는 출처가 아니라 설정이므로 각주로 달지 않는다. 구축 시 각주가 없으면 "없음". -->
