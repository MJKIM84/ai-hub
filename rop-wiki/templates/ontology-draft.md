---
title: "능력 온톨로지 초안"                   # 트랙의 살아있는 산출물 이름. 첫 트랙은 "능력 온톨로지 초안"
type: ontology-draft
...
일곱 섹션(5.4): 목적과 범위 / 개념 목록 / 관계 목록 / 다이어그램 / 적용 예시 / 미해결 모델링 질문 / 버전 이력. 제목·순서 고정. 7절은 퍼블리셔가 자동 갱신한다.
...
-->
[홈](../../index.md) › 중점 연구 트랙 › [{{track_name}}](index.md) › 능력 온톨로지 초안

# 능력 온톨로지 초안 (v{{ontology_version}})
...
{{footnotes}}
<!-- 각주 정의만 둔다. 형식: "[^ref-012]: 기관, 제목, 발행일, URL, 접근일". 5.4 의 온톨로지 초안 일곱 절에는 출처 절이 없으므로 2·5절에서 쓴 [^ref-NNN] 의 정의는 7절 auto 마커 아래(페이지 끝)에 둔다 [가정]. 정의가 없는 참조는 check_links 가 반려한다. 프런트매터 sources 와 일치시킨다. v0 시드처럼 각주가 없으면 이 줄을 지운다. -->
