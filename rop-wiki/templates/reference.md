...
published: "{{published}}"                  # 발행일 YYYY-MM-DD 또는 YYYY-MM 또는 YYYY. 모르면 "미확인"(시드와 같은 필드 값. 각주 문자열에서는 공통 규칙 9대로 "발행일 미확인")
url: "{{url}}"                              # 출처 URL 그대로
...
이 페이지의 "각주 형식" 절에 있는 한 줄("[^ref-NNN]: 기관, 제목, 발행일, URL, 접근일")은 다른 페이지가 그대로 복사해 쓰는 정본이다. 절 이름은 시드 페이지(docs/references/ref-001.md ~ ref-010.md)와 같게 "각주 형식"으로 쓴다. 정본 줄의 발행일·접근일 표기는 공통 규칙 9 를 따른다: 발행일을 모르면 "발행일 미확인", 접근일은 날짜만(YYYY-MM-DD), 원문 미열람이면 접근일 뒤에 " (원문 미열람)". 시드 참고문헌 페이지와 시드 페이지의 각주 정의는 "미확인"·"접근일 YYYY-MM-DD" 로 적고 있어 이 정본과 다르며, 같은 형식으로 맞추도록 시드 담당에게 요청했다(templates/README.md) [가정].
"인용된 페이지" 절의 목록은 auto:reference-cited-pages 마커 안에 있고 퍼블리셔가 채운다. 이 key 는 공통 규약의 auto key 목록에 없지만 시드 참고문헌 페이지와 퍼블리셔(pipeline/lib/autoregion.py AUTO_KEYS, pipeline/lib/render.py render_reference_cited_pages)가 이미 쓰는 값이다 [가정].
...
| 신뢰도 | {{reliability}} |
| 접근일 | {{accessed}} |
<!-- 프런트매터와 같은 값. 제목은 원문 표기(영문이면 영문) 그대로. 발행일을 확인하지 못하면 이 표와 프런트매터에서는 "미확인"(각주 문자열에서는 "발행일 미확인"). URL 은 꺾쇠로 감싸 자동 링크한다. -->
...
```
[^{{ref_id}}]: {{org}}, {{source_title}}, {{published_or_발행일미확인}}, {{url}}, {{accessed}}
```
<!-- 다른 페이지가 복사해 쓰는 한 줄. 시드 페이지처럼 코드 펜스 안에 두어 이 페이지 자체의 각주로 렌더링되지 않게 한다(check_links 는 코드 펜스 안의 정의를 세지 않는다). 발행일 자리에는 프런트매터 published 값을 쓰되 "미확인"이면 "발행일 미확인"으로 쓴다. 접근일은 날짜만(YYYY-MM-DD) 쓰고 앞에 "접근일 "을 붙이지 않는다. 원문 미열람이면 접근일 뒤에 " (원문 미열람)" 을 붙인다. 공통 규칙 9·agents/shared-rules.md·agents/storyteller.md 의 각주 규칙과 같은 형식이며, 시드 참고문헌 페이지의 "미확인"·"접근일 YYYY-MM-DD" 표기와는 다르다(README 요청 참고). -->
