...
{{intro}}
<!-- 한두 문장. 이 로그가 트랙 실행마다 무엇을 기록하는지와 [트랙 개요](index.md)·[질문 백로그](question-backlog.md)·온톨로지 초안(첫 트랙은 [능력 온톨로지 초안](ontology-draft.md)) 링크. 구축 시 작성하고 이후 바꾸지 않는다. -->
...
항목은 최신순으로 쌓인다(가장 최근 실행이 맨 위). "실행 기록" 절의 본문은 자동 갱신 영역(auto 마커, 키 `track-log`) 안에 있으며, 퍼블리셔가 트랙 실행마다 원천 데이터 `data/tracks/{{track_slug}}/log.json`에 항목을 추가하고 그 파일에서 이 영역을 최신순으로 다시 만든다. 구축 시드 항목(build-{{created}})도 그 파일에 들어 있어 다시 만들어도 사라지지 않는다. 이 페이지에는 사실 태그를 붙이지 않는다(운영 기록). [가정]
...
<!-- auto:track-log:end -->
<!--
퍼블리셔가 실행마다 아래 블록(### 소제목 + 표)을 마커 안에 최신순으로 넣는다. 구축 시 첫 항목(실행 id build-<구축일>)을 넣을 때는 마커 안의 안내 문구 대신 이 블록을 두고, 같은 항목을 data/tracks/<트랙 slug>/log.json 의 items 에도 넣는다(퍼블리셔가 그 파일에서 영역을 다시 만들므로 파일에 없는 블록은 다음 갱신에서 사라진다. 시드와 같다). 마커 밖에는 실행 블록을 두지 않는다.
