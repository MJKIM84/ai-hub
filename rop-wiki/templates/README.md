...
ROP 연구 위키의 페이지 템플릿 14종이다(2026-09-25에 track-draft.md·idea.md 추가). 사양서 5.1(프런트매터)·5.2(상태·신뢰도)·5.3(사실·출처 표기)·5.4(섹션 구성)와 4.1·4.2·4.3·4.6·4.7의 페이지 정의를 파일로 옮긴 것이다. 각 템플릿은 스토리텔러 에이전트의 프롬프트에 그대로 들어가므로 자기완결적으로 쓰여 있다. 즉, 템플릿 하나만 읽어도 그 페이지를 쓰는 데 필요한 규칙(태그·각주·분량·시나리오 여섯 항목·번호+이름 표기·경로 규약)이 모두 들어 있다.
...
| area.md | area | docs/categories/<slug>/NN-<slug>.md | 5.4 세부 연구영역 13개 절(괄호 설명 포함) | area-tracks [가정], page-status [가정], area-recent | 구축자(시드) → 스토리텔러 → 퍼블리셔(12) | 세부 연구영역 28페이지. H1 아래 소속 대분류·핵심 질문은 시드와 같은 세 줄 admonition 블록(`!!! info "소속 대분류"` / 공백 4칸 + `[<대분류>](index.md) — 핵심 질문:` / 공백 4칸 + `<핵심 질문 원문> [분류원문]`) |
| topic.md | topic | docs/topics/YYYY/YYYY-MM-DD-<slug>.md | 5.4 주제 페이지 10개 절 | page-status [가정] | 스토리텔러 | 일일 주제 페이지(2주기 이후), 트랙 주제 페이지(track 필드) |
...
| ontology-draft.md | ontology-draft | docs/tracks/<slug>/ontology-draft.md | 5.4 온톨로지 초안 7개 절(2·3절은 `2. 개념 목록 표`·`3. 관계 목록 표`). title·H1·이동 경로의 산출물 이름은 자리 표시 `{{ontology_title}}`(첫 트랙 "능력 온톨로지 초안") | page-status [가정], ontology-version-history | 구축자(v0) → 스토리텔러(검증 승인 변경만) → 퍼블리셔(7) | 살아있는 온톨로지 초안 |
...
| track-draft.md | ontology-draft | docs/tracks/<slug>/<draft_page> (트랙 정의의 draft_page) | ontology-draft.md 와 같은 7개 절 | page-status [가정], ontology-version-history | 구축자(v0) → 스토리텔러(검증 승인 변경만) → 퍼블리셔(7절·상태 줄) | 두 번째 이후 트랙의 살아있는 초안 문서(업무 분해·배정 설계 초안, 공간 그래프 스키마 초안). 트랙 정의의 `draft_template: track-draft.md` 로 지정한다 [가정] |
| idea.md | idea | docs/ideas/<slug>.md (트랙 정의의 idea_page) | 1. 문제 정의 / 2. 관련 세부 연구영역 / 3. 선행 연구·제품 사례 / 4. 필요한 데이터와 표준 / 5. 구현 가설 / 6. 검증 방법 / 7. 미해결 질문 백로그 | page-status, idea-areas, idea-backlog [가정] | 구축자(1·2절) → 스토리텔러(3~6절, 트랙 실행) → 퍼블리셔(2·7절) | 확장 아이디어 페이지. 색인 docs/ideas/index.md(`subtype: index`, auto: idea-area-map)는 템플릿 없이 구축자가 쓴다 [가정] |
...
4. **H1 제목**. 페이지 상태(프런트매터 status)는 본문에 손으로 쓰지 않는다. area·topic·ontology-draft·track-draft·idea 는 H1 아래(세부영역은 소속 대분류 블록과 `auto:area-tracks` 영역 아래)에 `auto:page-status` 마커만 두고, 퍼블리셔가 프런트매터에서 `> 페이지 상태: … · 신뢰도: … · 페이지 버전: … · 마지막 갱신: … · 마지막 실행: …` 한 줄(초안은 앞에 초안 버전)을 만든다. 마커 밖의 "페이지 상태:" 줄과 "상태:"·"> 상태:"로 시작하고 값이 페이지 상태 값인 줄은 `check_frontmatter` 가 반려한다(`pipeline/lib/render.py` 의 `status_line_violations`). 페이지 상태가 아닌 트랙·단계 상태 줄은 손으로 쓴다(2026-09-25 변경, 그 전의 area·topic `> 상태: … · 신뢰도: … · 갱신일: …` 줄은 폐지). 손으로 쓰는 상태 줄: track-overview `> 트랙 상태: … · 현재 단계: 단계 n. <단계 이름> · 마지막 트랙 실행: …`(시드와 같이 "단계 " 접두어를 둔다), track-stage `> 단계 상태: … · 열린 질문: …건 · 답한 질문: …건 · 완료 조건: <충족 | 미충족> · 마지막 실행: …`, ontology-draft (온톨로지 초안 줄은 `auto:page-status` 로 바뀌었다). home·about·category·glossary·reference·daily-log·track-log 에는 상태 줄이 없다.
...
- **상태 줄과 현재 상태 열.** 세부영역·주제·초안·아이디어 페이지의 페이지 상태 줄은 `auto:page-status` 영역이며 퍼블리셔가 프런트매터에서 다시 만든다(단일 원천). 스토리텔러는 프런트매터 값만 맞추고 마커 사이를 쓰지 않는다. 트랙 개요·단계 페이지의 트랙 상태·단계 상태 줄(`> 트랙 상태: …`, `> 단계 상태: …`)은 페이지 상태가 아니므로 auto 마커 밖에 두고 스토리텔러가 맞춘다. 대분류 페이지의 "현재 상태" 열은 위와 같이 퍼블리셔가 채운다. 세부영역 머리의 `auto:area-tracks`(관련 연구 트랙 안내)도 퍼블리셔가 채운다.
