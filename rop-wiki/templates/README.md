...
| area.md | area | docs/categories/<slug>/NN-<slug>.md | 5.4 세부 연구영역 13개 절(괄호 설명 포함) | area-recent | 구축자(시드) → 스토리텔러 → 퍼블리셔(12) | 세부 연구영역 28페이지. H1 아래 소속 대분류·핵심 질문은 시드와 같은 `!!! info "소속 대분류"` admonition 블록 |
...
| track-overview.md | track | docs/tracks/<slug>/index.md | 5.4 트랙 개요 8개 절 | track-progress, track-recent-runs | 구축자 → 스토리텔러 → 퍼블리셔(5, 7) | 트랙 개요 |
| track-stage.md | track-stage | docs/tracks/<slug>/stage-N-<slug>.md | 5.4 트랙 단계 9개 절 | 없음 | 구축자(시드) → 스토리텔러 | 트랙 단계 7페이지 |
| ontology-draft.md | ontology-draft | docs/tracks/<slug>/ontology-draft.md | 5.4 온톨로지 초안 7개 절 | ontology-version-history | 구축자(v0) → 스토리텔러(검증 승인 변경만) → 퍼블리셔(7) | 살아있는 온톨로지 초안 |
| track-log.md | track-log | docs/tracks/<slug>/log.md | 머리말 / 항목 형식 [가정] / 실행 기록(auto 마커 안, 실행마다 8항목 표) | track-log [가정] | 구축자(머리말·항목 형식) → 퍼블리셔(실행 기록) | 트랙 로그. 시드 docs/tracks/manual-capability-ontology/log.md 와 같은 구성 |
...
3. **이동 경로 줄** (`홈 › … › 현재 페이지`). 페이지 위치 기준 상대 링크. 홈은 "홈" 한 단어. 단계는 4.8 의 사이드바 구조(홈 → 소개 → 대분류 → 세부영역 → 중점 연구 트랙 → 주제 → 용어집 → 참고문헌 → …)를 따르되 시드와 같게 쓴다 [가정]: 세부영역 `[홈](../../index.md) › [<대분류>](index.md) › <세부영역>`, 대분류 `[홈](../../index.md) › <대분류>`, 소개 `[홈](../index.md) › <제목>`("소개" 단계 없음, 시드 7페이지와 같음), 주제·용어집·참고문헌·로그 `[홈](…) › [<색인 이름>](index.md 또는 ../index.md) › <제목>`, 트랙 개요 `[홈](../../index.md) › 중점 연구 트랙 › <트랙 이름>`, 트랙 하위 페이지(단계·온톨로지 초안·트랙 로그) `[홈](../../index.md) › 중점 연구 트랙 › [<트랙 이름>](index.md) › <페이지>`. "중점 연구 트랙"에는 링크가 없다(색인 페이지가 없다). 시드 가운데 `docs/tracks/manual-capability-ontology/{ontology-draft,log,question-backlog,experiments}.md` 와 `agents/storyteller.md` 4.2 의 트랙 하위 페이지 예시는 "중점 연구 트랙" 단계가 없어 이 규칙·트랙 개요 시드·단계 시드 7페이지와 다르다. 트랙 시드 담당·agents 담당에게 통일을 요청한다(요청).
4. **H1 제목**. 상태 줄은 다음 다섯 템플릿에만 있고 형식이 서로 다르다: area·topic `> 상태: … · 신뢰도: … · 갱신일: …`, track-overview `> 트랙 상태: … · 현재 단계: … · 마지막 트랙 실행: …`, track-stage `> 단계 상태: … · 열린 질문: …건 · 답한 질문: …건 · 완료 조건: … · 마지막 실행: …`, ontology-draft `> 온톨로지 버전: v… · 페이지 상태: … · 신뢰도: … · 마지막 변경 실행: …`. home·about·category·glossary·reference·daily-log·track-log 에는 상태 줄이 없다.
...
`pipeline/checks/protect_source.py` 의 `AREA_SECTIONS`·`CATEGORY_SECTIONS` 가 세부영역·대분류 페이지의 H2 문자열을 글자 단위로 검사한다. 아래 area·category 정본은 그 값 및 시드 페이지와 같다(사양서 5.4·4.3 문구 그대로: 괄호 설명 포함, 대분류는 번호 없음). 나머지 유형은 검사 스크립트가 H2 를 보지 않으며 2차 검증(사양서 6.2 "템플릿 섹션 순서 준수")이 템플릿의 H2 를 기준으로 본다.

해석 규칙 [가정]: 세부영역·대분류의 H2 는 검사 스크립트 문자열을 그대로 따른다(세부영역은 5.4 의 괄호 설명까지 제목에 포함). 나머지 유형(주제·트랙 개요·트랙 단계·온톨로지 초안)은 5.4 의 괄호·줄표(—) 뒤 설명구를 제목에서 뺐다(예: 5.4 "5. ROP 관점의 시사점 (직접 범위 / 연계 범위 구분)" → `5. ROP 관점의 시사점`, "9. 검증 노트 — 판정, …" → `9. 검증 노트`, "5. 단계 진행 현황 표 (…)" → `5. 단계 진행 현황`, "7. 최근 실행 (자동)" → `7. 최근 실행`, "2. 질문 목록 (상태: …)" → `2. 질문 목록`, "2. 개념 목록 표 (…)" → `2. 개념 목록`). 같은 5.4 문구를 유형에 따라 다르게 적용한 것이며, 시드 페이지·`agents/storyteller.md` 부록 B·`agents/verifier.md` 부록 C 가 같은 값을 쓴다. 설명구는 각 템플릿의 안내 주석에 옮겨 두었다.
...
- **track-overview**: `1. 컨셉` `2. 연구 목표` `3. 가설과 판정 상태` `4. 관련 세부영역` `5. 단계 진행 현황` `6. 살아있는 산출물 링크` `7. 최근 실행` `8. 참고 자료`
- **track-stage**: `1. 이 단계에서 밝힐 것` `2. 질문 목록` `3. 조사 결과` `4. 결론과 남은 불확실성` `5. 이 단계가 낳은 후속 질문` `6. 완료 조건 충족 현황` `7. 관련 세부영역` `8. 출처` `9. 이력`
- **ontology-draft**: `1. 목적과 범위` `2. 개념 목록` `3. 관계 목록` `4. 다이어그램` `5. 적용 예시` `6. 미해결 모델링 질문` `7. 버전 이력`
...
**agents 파일과의 대조(2026-09-24 확인)**: `agents/storyteller.md` 부록 B(1.1)와 `agents/verifier.md` 부록 C(1.1)는 위 정본과 일치한다(세부영역 괄호 포함, 대분류 번호 없음, 대분류 "현재 상태" 열은 퍼블리셔 갱신). 이전에 남겼던 정본 불일치 요청은 해소됐다. 남아 있던 불일치는 세부영역 머리 형식이었다: 시드 28페이지와 두 agents 파일은 `!!! info "소속 대분류"` admonition 블록(공백 4칸 들여쓴 `[<대분류>](index.md) — 핵심 질문: <원문> [분류원문]`)을 정본으로 보고 한 줄 `**소속 대분류:** …` 형식을 [분류원문] 훼손(불통과)으로 규정하는데, 이 템플릿(area.md)만 한 줄 형식이었다. 처리 방침: area.md 를 admonition 블록으로 고쳐 시드·agents 와 일치시켰다(2026-09-24). **요청(agents 담당)**: storyteller.md 4.2(2항)와 verifier.md 부록 C 의 "[가정: 시드(admonition)와 템플릿(한 줄)이 다르며 … 템플릿 담당에게 통일을 요청한다]" 문구는 더 이상 사실이 아니므로 "템플릿도 admonition 형식"으로 고치기를 요청한다.
...
- **세부영역 페이지(`check_area`).** 프런트매터 title·category·area_no·type, H2 13개(위 정본과 글자 단위 일치), H1, H2 앞 부분에 소속 대분류 이름과 핵심 질문 + ` [분류원문]`, 1절 첫 내용 줄 = 정의 + ` [분류원문]`, 2절 첫 내용 줄 = 질문 + ` [분류원문]`, 2절 안의 `> 원문 주석: <문단> [분류원문]` 인용 블록 목록 = 원문에서 그 영역을 "N번"으로 언급하는 문단 목록(5. 로봇 능력·작업 온톨로지, 6. 지도·공간·위치 모델, 7. 화물·재고·자산 식별과 추적, 8. 실시간 세계 상태·데이터 일관성, 13. 작업 배정 — MRTA, 17. 로봇 간 협업·물리적 인계, 19. 모니터링·이상 탐지·원인 분석, 21. 온보딩·설정·현장 시운전, 22. 시뮬레이션·예측용 디지털 트윈, 27. AI·학습·적응과 모델 운영만 해당. 6. 지도·공간·위치 모델은 두 문단). 원문 주석을 1절에 두거나 굵은 "**원문 주석:**" 단락으로 두면 반려된다. 시드 세부영역 페이지와 템플릿은 소속 대분류·핵심 질문을 `!!! info "소속 대분류"` admonition 블록(공백 4칸 들여쓴 `[<대분류>](index.md) — 핵심 질문: <원문> [분류원문]`)으로 둔다. 검사(`check_area`)는 H1 뒤·1절 앞에 핵심 질문 원문 + ` [분류원문]` 과 대분류 이름이 있으면 블록 형식에 관계없이 통과하지만, 2차 검증(`agents/verifier.md` 항목 3·부록 C)은 시드의 admonition 형식만 인정하고 한 줄 `**소속 대분류:**` 형식은 [분류원문] 훼손(불통과)으로 본다. 따라서 이 형식만 쓴다.
- **대분류 페이지(`check_category`).** H2 앞 여섯 개(번호 없음), "핵심 질문"·"개요" 첫 줄, "세부 연구영역" 표 각 행의 앞 3칸(첫 칸은 `**5. …**` 굵은 원문 셀, 링크 없음)과 표 아래 `[분류원문]` 줄, "이 대분류의 핵심 포인트"에서 ` [분류원문]` 으로 끝나는 줄 목록 = 원문 문단 목록. 표는 원문 3열 + "페이지" + "현재 상태" 5열이며 `auto:category-area-table` 마커 안에 있고 퍼블리셔(`pipeline/lib/render.py` `render_category_area_table`)가 현재 상태 열까지 다시 쓴다. 이 key 는 공통 컨텍스트의 auto key 목록에 없지만 시드 페이지·퍼블리셔가 이미 쓰는 값이다 [가정]. key 목록 추가를 요청한다.
...
- **태그 줄 원문 대조(`check_tagged_lines`).** 홈·소개 4페이지·대분류 7페이지에서 ` [분류원문]` 으로 끝나는 모든 줄(표 아래 단독 태그 줄 제외)은 태그를 뗀 문자열이 원문의 한 줄 전체 또는 표 셀 하나(머리말의 범위 문장 포함)와 글자 단위로 같아야 한다. 두 셀을 이어 붙인 줄, 문장 일부, 인용 부호 `>` 가 붙은 줄에는 태그를 붙일 수 없다.
...
- **각주 정의 형식**은 `[^ref-NNN]: 기관, 제목, 발행일, URL, 접근일`, 원문 미열람은 접근일 뒤 ` (원문 미열람)`. reference.md 의 "각주 형식" 절이 이 한 줄의 원본이다. 표기 결정(템플릿 쪽, 2026-09-24): 발행일을 모르면 각주 문자열에서는 `발행일 미확인`(프런트매터·서지 정보 표의 `published` 는 `미확인`), 접근일은 날짜만 `2026-09-24`(앞에 "접근일 "을 붙이지 않는다). 이는 템플릿 공통 규칙 9항, `agents/shared-rules.md` 공통 표기, `agents/storyteller.md` 4.4 의 각주 규칙과 같은 값이며, reference.md 의 정본 줄(`{{published_or_발행일미확인}}`)·필드 주석·규칙 9를 이 값으로 일치시켰다. 시드 참고문헌 페이지(ref-001~ref-010)의 "각주 형식" 줄과 시드 페이지(세부영역·대분류·소개·용어집·표준)의 각주 정의는 `미확인` / `접근일 2026-09-24` 로 적고 있어 이 정본과 다르다. 시드 참고문헌·시드 페이지 담당에게 같은 형식(`발행일 미확인`, 접근일 날짜만)으로 맞추기를 요청한다(요청). 참고문헌 시드의 `status` 는 `seed` 이며 템플릿 status 주석에 반영했다. 시드 참고문헌 페이지는 "서지 정보" H2 없이 표를 H1 바로 아래에 두고 "비고" 절이 없다. 템플릿은 5절 구성을 유지하되 제목 형식(`ref-003 — 제목`)과 "각주 형식"·"인용된 페이지" 절(auto:reference-cited-pages 마커와 마커 위 안내 문장)은 시드에 맞췄다.
- **상태 줄과 현재 상태 열.** 세부영역·주제·트랙 페이지의 상태 줄(`> 상태: …` 등)은 auto 마커 밖에 있으므로 스토리텔러가 해당 페이지를 갱신할 때 맞춘다. 대분류 페이지의 "현재 상태" 열은 위와 같이 퍼블리셔가 채운다.
- **트랙 상태 값(구축자 정의, 사양서에 없음 [가정]).** 단계 상태: `대기 | 진행 중 | 완료 | 재개` (퍼블리셔 `render_track_progress` 와 `config/tracks/<slug>.yaml` 의 `stage_status` 가 같은 값) / 완료 조건: `충족 | 미충족` (퍼블리셔 출력), 단계 페이지 표에서는 `부분 충족` 도 허용 / 검증 판정 표시: `stage_complete` true → 충족, false → 미충족, `stage_transition_approved` true → 전환 승인, false → 미승인, 판정 전 `없음(판정 전)` / 질문 표 제기 근거: `시드 | <finding id> | 사용자 | 단계 n 에서 되돌아옴` (`backlog.json` 의 `origin` 값 `seed` 를 표에서는 `시드` 로 표시) / 온톨로지 개념 상태: `초안 | 제안 | 확정 | 폐기` / 온톨로지 버전: v0 시드는 `"0"`, 이후 `"0.1"`, `"0.2"` … . 백로그 JSON 담당·pipeline 담당과 값을 맞춘다(요청).
- 일일 로그는 퍼블리셔가 페이지 전체를 렌더링한다(마커 없음). 실행이 하루에 둘 이상이면 같은 날짜 페이지에 "실행 <id>" 블록을 최신순으로 반복한다. 트랙 로그는 머리말과 "항목 형식" 절을 구축자가 쓰고(마커 밖), "실행 기록" 절 본문만 `auto:track-log` 마커 안에서 퍼블리셔가 원천 `data/tracks/<slug>/log.json` 으로 다시 쓴다(시드와 같은 구성). `track-log` 키는 공통 규약의 auto key 목록에 없고 `pipeline/lib/autoregion.py` 의 `AUTO_KEYS` 와 `pipeline/lib/render.py` 의 `render_for` 에도 아직 등록되어 있지 않아, 등록 전까지는 `refresh_all_auto_regions` 가 이 영역을 그대로 둔다 [가정]. **요청(pipeline 담당)**: `AUTO_KEYS` 와 `render_for` 에 `track-log` 를 등록하고 `data/tracks/<slug>/log.json` 에서 8항목 블록을 최신순으로 렌더링하는 렌더러를 추가하기를 요청한다.
...
- `auto:category-area-table` key 는 공통 key 목록에 없는 시드·퍼블리셔 기존 값이다.
- `auto:reference-cited-pages` key 는 공통 key 목록에 없는 시드·퍼블리셔 기존 값이다(`autoregion.AUTO_KEYS`, `render.render_reference_cited_pages`). reference.md 의 "인용된 페이지" 절이 이 마커를 쓴다.
- `auto:track-log` key 는 공통 key 목록에 없는 시드 값이며 퍼블리셔에는 아직 등록되지 않았다. track-log.md 의 "실행 기록" 절이 이 마커를 쓰고, 시드와 같은 "항목 형식" 절(5.4 의 8항목 밖의 구축자 추가 절)을 둔다.
- 섹션 제목의 해석 규칙(세부영역·대분류는 검사 스크립트 문자열, 나머지 유형은 5.4 의 괄호·줄표 설명구를 제목에서 뺌)은 위 "섹션 제목 정본"의 [가정]이다.
...
- 세부영역 페이지 머리의 admonition 블록 형식은 시드 28페이지와 agents 파일을 정본으로 본 구축자 결정이다(퍼블리셔 검사는 형식을 가리지 않는다).
- glossary.md 의 `term_ko, term_en, definition`, reference.md 의 `ref_title` 은 5.1 에 없는 필드이며 퍼블리셔 색인과 시드 페이지가 쓰는 값이다.
- 트랙 페이지의 단계 상태·완료 조건·제기 근거·온톨로지 개념 상태·온톨로지 버전 표기 값은 위 "트랙 상태 값" 항목대로 구축자가 정했다.
