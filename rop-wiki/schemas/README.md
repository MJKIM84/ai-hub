...
20. 페이지의 특정 절을 가리키는 링크 필드 세 곳(`track_updates.backlog_updates[].answer_link`, `open_question_updates[].link`, `flow_matrix_updates[].link`)은 같은 규칙으로 `docs/….md#앵커` 를 허용한다(사이트는 유니코드 slugify 를 쓰므로 한글 제목 앵커도 유효하다. 답 위치는 명시 id 를 붙인 질문 소제목 앵커 `#q1-01` 을 쓴다). 해결된 열린 질문(4.7)과 시나리오 칸(B.3)도 답이 실린 절을 가리키는 일이 많기 때문이다. 파일을 가리키는 경로 필드는 앵커를 받지 않는다.
