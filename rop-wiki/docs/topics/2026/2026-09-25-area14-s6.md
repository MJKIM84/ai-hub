---
title: "14. 작업 순서·스케줄링 — 대표 접근법과 기술"
type: topic
category: "D. 계획·최적화"
primary_area_no: 14
related_areas: [13, 15, 16, 1, 2, 3, 4, 9, 18]
tags: [분리 페이지]
status: published
confidence: medium
created: 2026-09-25
updated: 2026-09-25
sources: [ref-006, ref-110, ref-133, ref-134, ref-376, ref-377, ref-378, ref-379, ref-381, ref-383, ref-384, ref-385, ref-386, ref-389]
last_run: 2026-09-25
version: 1
split_from: docs/categories/d-planning-and-optimization/14-task-sequencing-and-scheduling.md#6
---

[홈](../../index.md) › [주제](../index.md) › 14. 작업 순서·스케줄링 — 대표 접근법과 기술

# 14. 작업 순서·스케줄링 — 대표 접근법과 기술

<!-- auto:page-status:start -->
> 페이지 상태: published · 신뢰도: medium · 페이지 버전: 1 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->

## 1. 세 줄 요약

- 앞의 시나리오에서 순서를 정하는 방법은 크게 여섯 갈래로 연구되어 있다.
- 이 페이지는 [14. 작업 순서·스케줄링](../../categories/d-planning-and-optimization/14-task-sequencing-and-scheduling.md) 페이지의 "대표 접근법과 기술" 절이 분량 기준을 넘어 옮겨 온 것이다. 원 페이지의 검증을 거친 내용이며 새 주장은 없다.

## 2. 배경

[14. 작업 순서·스케줄링](../../categories/d-planning-and-optimization/14-task-sequencing-and-scheduling.md) 페이지를 쓰는 과정에서 "대표 접근법과 기술" 절의 분량이 세부영역 페이지 기준(4,000자)을 넘어, 내용을 줄이지 않고 이 주제 페이지로 분리했다.

## 3. 본문

앞의 시나리오에서 순서를 정하는 방법은 크게 여섯 갈래로 연구되어 있다.

### 작업대의 배치·순서 동시 최적화

랙 이동 로봇 창고에서 작업대의 주문 배치·순서와 그에 맞물린 랙 도착 순서를 함께 정하는 문제가 연구되었다(2017). [사실][^ref-381] 작업대가 여럿인 KIVA 방식 창고에서는 주문을 작업대에 할당하고 작업대별 주문·랙 순서를 함께 정하는 모델이 제안되었고, arXiv 프리프린트(2021-08) 기준 저자 실험(원문 미열람, 독립 재현 미확인)에서 랙 방문 수를 탐욕 정책보다 3분의 1 넘게, 작업대별 독립 스케줄링보다 5분의 1 넘게 줄였다. [사실][^ref-384] 이후 저널 게재판의 게재지·판은 미확인이다.

### 뒤 공정 기준 순서 결정

풋월 수동 통합에서 자동창고의 빈 방출 순서를 최적화해 주문 완료 시간과 포장 작업자 유휴를 줄이는 문제가 단일 기계 스케줄링과 가까운 문제로 정식화되었다(2019, 원문 미열람 기준 요약). [사실][^ref-385] 로봇 전방-예비 창고에서는 배송 요구를 조건으로 수동 피킹과 로봇 분류를 동기화하는 문제가 제안되었고, 저자 실험(원문 미열람, 독립 재현 미확인)에서 작업 완료 시간(makespan)과 전방 구역 면적이 줄었다(2022). [사실][^ref-386]

### 시간·순서 제약과 배정·경로의 결합

시간·순서 제약이 있는 다중 로봇 작업 배정의 분류가 제안되었고, 이 문제군은 차량 경로 문제·스케줄링 문제와 관련된다(2017). [사실][^ref-383] 선후 제약 다중 에이전트 경로 찾기(PC-MAPF)는 작업 사이 선후를 둔 확장이며 PC-CBS 가 작업 완료 시간 최적 해를 찾는다고 제안된다(프리프린트, 2022-02, 저자 주장). [사실][^ref-389] 선후 의존이 있으면 배정·시간 순서·충돌 없는 경로가 서로 얽혀, 이 영역을 배정·경로와 분리해 풀기 어려운 경우가 생길 것으로 보인다(이 위키의 추론). [추정][^ref-389][^ref-383]

### 플릿 안 일정 계획 (Open-RMF)

Open-RMF 에서 플릿 선정은 디스패처의 입찰로 하고 기본 평가기는 QuickestFinishEvaluator 다(배정 설명은 13. 작업 배정 — MRTA 참고). [사실][^ref-376][^ref-378] 순서 관점에서는 그 뒤가 중요하다. 플릿 어댑터는 rmf_task 의 TaskPlanner 로 새 요청을 로봇 일정의 어디에 넣을지 정하고 배정마다 가장 이른 시작 시각을 기록하며, 배터리가 모자라면 충전 작업을 일정에 끼워 넣는다. 탐욕 방식(빠르나 최적 보장 없음)과 A* 기반 방식(최적 보장, 더 오래 걸림) 가운데 고를 수 있다. [사실][^ref-376][^ref-377] 작업은 단계(phase)의 연쇄·조합으로 구성된다. [사실][^ref-110] 이 자료들은 같은 기관 자료라 서로 독립된 교차 확인은 아니다(확인일 2026-09-25).

### 제약 프로그래밍

OR-Tools CP-SAT 은 작업을 시작·길이·끝이 묶인 구간 변수로 두고, 공용 자원의 겹침 금지(NoOverlap), 실행 여부를 정하는 선택 구간, 시작·끝 사이 선형 부등식으로 쓰는 선후 관계로 스케줄링 문제를 표현한다(확인일 2026-09-25). [사실][^ref-379]

### 온라인 재계획

작업이 계속 들어오는 조건에서 배정과 충돌 없는 경로를 함께 계획하는 다중 에이전트 픽업·배송(MAPD)은 순서 결정이 한 번의 계획이 아니라 연속 재계획이 되는 상황을 다룬다(2017). [사실][^ref-006] 동적으로 도착하는 주문의 피킹에서 재최적화의 효과를 다룬 연구(2025)와 웨이브·웨이브리스 출고 정책 비교 연구(2010)도 있다. [사실][^ref-133][^ref-134]

## 4. 현장 시나리오

현장 시나리오는 원 페이지의 [5. 현장 시나리오 (물류 흐름의 어느 단계인지 명시)](../../categories/d-planning-and-optimization/14-task-sequencing-and-scheduling.md) 절에 있다.

## 5. ROP 관점의 시사점

ROP가 직접 맡는 것과 외부와 연계하는 것은 원 페이지의 [9. ROP가 직접 맡는 것과 외부와 연계하는 것 (부록 A 9장 기준)](../../categories/d-planning-and-optimization/14-task-sequencing-and-scheduling.md) 절에 있다.

## 6. 연결되는 연구영역

- 주 연구영역: [14. 작업 순서·스케줄링](../../categories/d-planning-and-optimization/14-task-sequencing-and-scheduling.md)
- 관련 영역: [13. 작업 배정 — MRTA](../../categories/d-planning-and-optimization/13-task-allocation-mrta.md), [15. 다중 로봇 경로·교통 관리 — MAPF](../../categories/d-planning-and-optimization/15-multi-robot-path-and-traffic-management-mapf.md), [16. 공용 자원·충전·에너지 최적화](../../categories/d-planning-and-optimization/16-shared-resource-charging-and-energy-optimization.md), [1. 주문·업무 시스템 연계](../../categories/a-business-supply-chain-design/01-order-and-business-system-integration.md), [2. 공정·워크플로 모델링](../../categories/a-business-supply-chain-design/02-process-and-workflow-modeling.md), [3. 처리능력·거점·설비 계획](../../categories/a-business-supply-chain-design/03-capacity-site-and-facility-planning.md), [4. 성과·경제성·프로세스 개선](../../categories/a-business-supply-chain-design/04-performance-economics-and-process-improvement.md), [9. 로봇·제조사 관제 연동](../../categories/c-connectivity-and-execution-foundation/09-robot-and-vendor-fleet-manager-integration.md), [18. 사람–로봇 협업·운영 인터페이스](../../categories/e-collaboration-and-field-operations/18-human-robot-collaboration-and-operator-interface.md)

## 7. 열린 질문

열린 질문은 원 페이지의 [11. 열린 질문](../../categories/d-planning-and-optimization/14-task-sequencing-and-scheduling.md) 절과 [열린 질문](../../open-questions.md) 페이지에 모은다.

## 8. 출처

[^ref-006]: Ma, H., Li, J., Kumar, T. K. S., & Koenig, S., Lifelong Multi-Agent Path Finding for Online Pickup and Delivery Tasks, 2017, https://arxiv.org/abs/1705.10868, 접근일 2026-09-25 (원문 미열람)
[^ref-110]: Open Robotics, Tasks in RMF (task_new) - Programming Multiple Robots with ROS 2, 미확인, https://osrf.github.io/ros2multirobotbook/task_new.html, 접근일 2026-09-25
[^ref-133]: Lorenz, Otto, & Gendreau (Networks, Wiley), Picking Operations in Warehouses With Dynamically Arriving Orders: How Good is Reoptimization?, 2025, https://onlinelibrary.wiley.com/doi/full/10.1002/net.22281, 접근일 2026-09-25 (원문 미열람)
[^ref-134]: Gallien, J., & Weber, T. G., To Wave or Not to Wave? Order Release Policies for Warehouses with an Automated Sorter, 2010, https://pubsonline.informs.org/doi/abs/10.1287/msom.1100.0291, 접근일 2026-09-25 (원문 미열람)
[^ref-376]: Open Robotics, Tasks in RMF (task) - Programming Multiple Robots with ROS 2, 미확인, https://osrf.github.io/ros2multirobotbook/task.html, 접근일 2026-09-25
[^ref-377]: Open Robotics (open-rmf), rmf_task — rmf_task/include/rmf_task/TaskPlanner.hpp, 미확인, https://github.com/open-rmf/rmf_task/blob/main/rmf_task/include/rmf_task/TaskPlanner.hpp, 접근일 2026-09-25
[^ref-378]: Open Robotics (open-rmf), rmf_ros2 — rmf_task_ros2/include/rmf_task_ros2/Dispatcher.hpp, 미확인, https://github.com/open-rmf/rmf_ros2/blob/main/rmf_task_ros2/include/rmf_task_ros2/Dispatcher.hpp, 접근일 2026-09-25
[^ref-379]: Google (google/or-tools GitHub), OR-Tools — ortools/sat/docs/scheduling.md (Scheduling recipes for the CP-SAT solver), 미확인, https://github.com/google/or-tools/blob/stable/ortools/sat/docs/scheduling.md, 접근일 2026-09-25
[^ref-381]: Boysen, N., Briskorn, D., & Emde, S., Parts-to-picker based order processing in a rack-moving mobile robots environment, 2017, https://www.sciencedirect.com/science/article/abs/pii/S0377221717302758, 접근일 2026-09-25 (원문 미열람)
[^ref-383]: Nunes, E., Manner, M., Mitiche, H., & Gini, M., A taxonomy for task allocation problems with temporal and ordering constraints, 2017, https://www.sciencedirect.com/science/article/abs/pii/S0921889016306157, 접근일 2026-09-25 (원문 미열람)
[^ref-384]: Yang, X., Hua, G., Zhang, L., Cheng, T. C. E., & Choi, T. M., Joint order assignment and picking station scheduling in KIVA warehouses with multiple stations, 2021-08, https://arxiv.org/abs/2108.09056, 접근일 2026-09-25 (원문 미열람)
[^ref-385]: Boysen, N., Stephan, K., & Weidinger, F., Manual order consolidation with put walls: the batched order bin sequencing problem, 2019, https://www.sciencedirect.com/science/article/pii/S2192437620300315, 접근일 2026-09-25 (원문 미열람)
[^ref-386]: Jiang, M., & Huang, G. Q., Intralogistics synchronization in robotic forward-reserve warehouses for e-commerce last-mile delivery, 2022, https://www.sciencedirect.com/science/article/abs/pii/S1366554522000175, 접근일 2026-09-25 (원문 미열람)
[^ref-389]: Kedia, K., Jenamani, R. K., Hazra, A., & Chakrabarti, P. P., Optimal Multi-Agent Path Finding for Precedence Constrained Planning Tasks, 2022-02, https://arxiv.org/abs/2202.10449, 접근일 2026-09-25 (원문 미열람)

## 9. 검증 노트

원 페이지와 함께 실행 2026-09-25-34 의 1차·2차 검증을 거쳤다. 분리는 코드가 했고 내용은 바꾸지 않았다.

## 10. 이력

| 날짜 | 실행 id | 변경 |
|---|---|---|
| 2026-09-25 | 2026-09-25-34 | 14. 작업 순서·스케줄링 의 "대표 접근법과 기술" 절에서 분리 |
