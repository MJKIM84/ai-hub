...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-228, ref-230, ref-364, ref-403, ref-553, ref-556, ref-557]
last_run: 2026-09-25
version: 1
...
Lei 외(2025)의 검토 논문은 산업용 로봇의 고장 모드와 근본 원인, 데이터 수집 전략과 센서, 모델 기반·데이터 기반 상태 감시·고장 진단 기술을 상태 기반 정비 구현 관점에서 정리했다. [사실][^ref-553] 부품 진단 자체는 로봇 제조사 쪽 연계 대상으로 보인다. [추정][^ref-553] ROP 가 그 결과를 받는 방식은 원 페이지의 [9. ROP가 직접 맡는 것과 외부와 연계하는 것 (부록 A 9장 기준)](../../categories/f-deployment-verification-and-maintenance/24-asset-and-software-lifecycle-management.md) 절에서 다룬다.
...
ROS 2 관리형 노드는 실행 전 초기화 확인과 실행 중 노드 교체·재시작을 위한 상태 전이를 제공한다. [사실][^ref-364] aws-samples 저장소의 시연용 샘플인 ROS 2 플릿 무선 업데이트(Over-the-Air, OTA) 구현은 IoT Jobs·Greengrass v2·Docker 로 배포를 지시·추적하고 플릿 색인으로 기기별 펌웨어 버전을 조회한다. [추정] 벤더 주장[^ref-556] README 는 실패한 업데이트를 이전의 검증된 버전으로 자동 복귀시키는 것을 이점으로 나열할 뿐, 자동 롤백의 구현 절차는 보이지 않는다. [추정] 벤더 주장[^ref-556]

국내에서는 2026-07-22 판교에서 열린 SDR(Software Defined Robot) 차세대 로봇 공통 플랫폼 기술개발 3차년도 착수 워크숍에서 KIST 휴머노이드연구단이 클라우드 기반 SDR 공통 서비스 프레임워크를 소개했고, 이 플랫폼이 무선 업데이트로 로봇 소프트웨어를 갱신하고 기능을 추가하는 것을 목표로 한다고 보도됐다(원 매체 미확인). [추정][^ref-557]
