...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-117, ref-125, ref-134, ref-380, ref-383, ref-385, ref-389]
last_run: 2026-09-25
version: 1
...
- **주문 배치(Order Batching)** — 여러 고객 주문을 한 번의 피킹 작업으로 묶는 결정으로, 피킹의 주요 통제 결정 문제 가운데 하나다. [사실][^ref-380]
- **선후 제약(Precedence Constraint)** — "작업 i 가 끝나야 작업 j 가 시작한다"처럼 작업 사이 실행 순서를 제한하는 조건이다. 여러 로봇의 협업 픽업이나 입력 자원이 먼저 도착해야 하는 창고 조립 작업에서 생긴다. [사실][^ref-389]
- **시간창(Time Window)** — 작업이 실행되어야 하는 시간 구간이다. 시간·순서 제약이 있는 다중 로봇 작업 배정의 분류(2017)에서 시간 제약은 시간창으로 표현된다. [사실][^ref-383]
...
- **풋월(Put Wall)** — 배치·구역 피킹 뒤 고객 주문별로 물품을 모으는 수동 통합 설비다. [사실][^ref-385]
