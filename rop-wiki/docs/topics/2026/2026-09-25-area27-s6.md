...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-056, ref-092, ref-359, ref-417, ref-088, ref-351, ref-622, ref-623, ref-625, ref-626]
last_run: 2026-09-25
version: 1
...
- LLM이 만든 계획은 실행 가능성 확인, 형식 검증, 되묻기, 안전 판정을 거쳐야 하고, 학습 모델은 운영 중 버전·시험 관리가 따로 필요하다. [추정][^ref-088][^ref-626] 아래 도식은 3절의 겹 구조를 이 위키가 그린 것이다.
...
RTAW는 창고 다중 로봇 작업 배정을 마르코프 결정 과정으로 정식화하고 주의 기반 정책을 PPO로 학습해 총 이동 지연을 줄이며, 500개 작업에서 탐욕·후회 기반 기준 대비 최대 10% 개선과 로봇·작업 수에 독립적인 정책 크기를 보고한다(저자 보고, 시뮬레이션 창고 조건, 단일 출처). [사실][^ref-623]

### 모델 운영 관리

ML Test Score는 28개 시험·감시 항목으로 ML 시스템의 운영 준비도를 점수화하는 기준표다. [사실][^ref-625] MLflow 모델 레지스트리는 운영 대상 버전에 별칭(예: champion)을 붙이고 별칭을 다른 버전으로 옮겨 운영 모델을 교체하게 한다. [사실][^ref-626]
