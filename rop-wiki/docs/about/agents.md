...
스토리텔러는 도구를 쓰지 않는다. 문체는 평서체, 짧은 단락이며 마케팅 표현과 근거 없는 단정을 쓰지 않는다. 출력하는 각 페이지는 프런트매터를 포함한 전체 마크다운(`content`)으로 `pages.json`에 담기고, 스크립트가 이를 `runs/<실행 id>/pages/`에 파일로 푼다.
...
**단계 전환 규칙이 있다.** 완료 조건을 모두 채우고 막힌 질문이 없으면 다음 단계로 간다. 뒤 단계에서 앞 단계의 질문이 생기면 그 질문은 앞 단계 태그로 백로그에 들어가고 다음 트랙 실행에서 우선 처리한다. 마지막 단계가 끝나면 트랙 상태를 `done`으로 바꾸고 후속 트랙을 사용자에게 제안한다.
