---
title: "단계 3. 비정형 문서에서 온톨로지를 추출하는 방법 조사"
type: track-stage
...
[홈](../../index.md) › 중점 연구 트랙 › [매뉴얼 기반 로봇 기능 온톨로지](index.md) › 단계 3. 비정형 문서에서 온톨로지를 추출하는 방법 조사

# 단계 3. 비정형 문서에서 온톨로지를 추출하는 방법 조사
...
|---|---|---|---|---|---|
| q3-01 | PDF·HTML의 레이아웃·표·그림을 보존해 파싱하는 방법과 도구는 무엇이고, 로봇 문서에서 어떤 결과를 내는가? | 열림 | 사용자 | | |
| q3-02 | 텍스트에서 온톨로지를 만드는 연구(온톨로지 학습, 정보 추출, 지식그래프 구축)의 현재 수준과 한계는? | 열림 | 사용자 | | |
| q3-03 | LLM 기반 추출(스키마 유도, 온톨로지 기반 추출, 그래프 기반 검색 증강)은 정확도·재현성·환각 측면에서 어떤 결과를 보이는가? | 열림 | 사용자 | | |
| q3-04 | 추출 결과에 출처(문서·페이지·문장)를 붙여 추적하는 방법은? | 열림 | 사용자 | | |
| q3-05 | 사람 검토 루프를 어디에 두어야 비용 대비 효과가 큰가? | 열림 | 사용자 | | |
| q3-06 | 기존 온톨로지(단계 1)에 맞추어 추출하는 방식과 문서에서 새 개념을 유도하는 방식은 어떻게 다르고 무엇을 섞어야 하는가? | 열림 | 사용자 | | |
| q3-07 | 매뉴얼에서 능력과 제약을 추출할 때 가장 자주 틀리는 유형은 무엇인가? | 열림 | 사용자 | | |
| q3-08 | 신규 로봇의 매뉴얼·SDK 문서에서 능력 정의 초안을 자동 생성할 때, 초안을 어떤 형식(능력 목록·제약·장착 장비·근거 문서 위치)으로 내고 사람이 검토할 항목(추출 신뢰도가 낮은 값, 문서에 없는 값)을 어떻게 표시하는가? (범위 능력: 이동·계단·적재·도어 조작·충전) | 열림 | 사용자 | | |

q3-07·q3-08은 확장 아이디어 1([아이디어 1. 로봇 기능 온톨로지](../../ideas/robot-capability-ontology.md)) 편입 때 사용자 요청으로 더한 질문이다(2026-09-25). q3-07은 요청 문구 그대로이고, q3-08은 새 연구 목표 5(신규 로봇 온보딩 시 능력 정의 초안 자동 생성)의 질문이다.

표의 질문 문장은 트랙 정의 그대로 두었다. 다음은 구축자 보충이다. 질문 문장 속 약어는 다음과 같다. PDF는 이동 문서 형식(Portable Document Format, PDF), HTML은 하이퍼텍스트 마크업 언어(HyperText Markup Language, HTML)이며, LLM은 1절에서 풀어 쓴 대규모 언어 모델이다.

## 3. 조사 결과

아직 없음. 이 단계를 다룬 트랙 실행이 아직 없다.

트랙 실행에서 답한 질문마다 질문 id로 시작하는 소제목(예: `### q3-01 …`)을 두고 그 아래에 답(2~5단락)을 쓴다. 주장마다 `[사실]`/`[추정]`/`[의견]` 태그와 각주를 붙이고, 표준 이름에는 발행 기관과 현재 버전·기준일을 밝히며, 원문을 열지 못한 표준은 "원문 미열람"을 표시한다. 근거 finding id는 문장에 쓰지 않고 각주와 [트랙 로그](log.md)에만 남긴다.

완료 조건인 추출 방법 비교표와 추출 파이프라인 후보안 2~3개·비교 기준은 고정된 산출물 페이지가 없으므로 이 절에 둔다. 분량이 커지면 질문 단위로 주제 페이지(프런트매터에 `track` 포함)로 분리하고 여기서 링크한다. [가정] 추출 방식을 실제 공개 문서에 적용해 보는 실험은 에이전트가 수행하지 않고 [실험](experiments.md)에 계획으로 제안한다.
...
> 완료 조건: 추출 방법 비교표, 추출 파이프라인 후보안 2~3개와 비교 기준.

| 완료 조건 | 충족 여부 | 근거 | 검증 판정 |
|---|---|---|---|
| 추출 방법 비교표 | 미충족 | 이 페이지의 3절에 비교표가 없다 | 없음(구축 시점, 판정 전) |
| 추출 파이프라인 후보안 2~3개와 비교 기준 | 미충족 | 이 페이지의 3절에 후보안과 비교 기준이 없다 | 없음(구축 시점, 판정 전) |
...
|---|---|---|---|---|---|
| 2026-09-24 | build-2026-09-24(구축 시드, 파이프라인 실행 아님) | 없음 | 시드 q3-01~q3-06(6건, 구축 시 [질문 백로그](question-backlog.md)에 등록) | 없음(v0 시드는 [능력 온톨로지 초안](ontology-draft.md)에서 생성) | 1 |
| 2026-09-25 | build-2026-09-25(확장 아이디어 1 편입, 파이프라인 실행 아님) | 없음 | q3-07·q3-08 추가(사용자 요청, [질문 백로그](question-backlog.md)에 등록) | 없음 | 2 |
