...
version: 1.6 (2026-09-25)

1.6 변경 요지: 부록 R: outline(절별 글자 예산·요약) 선작성, 상태 줄 금지, 차등 갱신 patches, 대분류 연결 절 작성, 트랙 초안·아이디어 페이지 갱신
...
- 태그 바로 뒤에 여는 괄호를 붙이지 않는다. `[추정](finding f27 …)` 처럼 쓰면 마크다운이 태그를 링크로 읽어 사이트 빌드가 실패한다. 태그 뒤에는 각주(`[^ref-…]`)만 붙이고, 근거 finding id 는 페이지 본문에 쓰지 않는다(트랙 로그·pages.json 에만 남긴다). 퍼블리셔는 이 경우 태그와 괄호 사이에 공백을 넣어 정규화하고 로그에 남긴다.
...
## 부록 R. 운영 전환 추가 규칙 (1.6, 2026-09-25)

**R-1 목차와 글자 예산을 먼저.** 본문을 쓰기 전에 `outline` 을 만든다. 페이지마다 쓸 절(세부영역은 3~11절)과 절별 글자 예산(budget_chars), 그 절의 1~2문장 요약(summary, 태그·각주 포함)을 적는다. 세부영역 3~11절 예산 합은 4,000자 안팎으로 잡되, 검증된 finding 이 더 많으면 줄이지 말고 그대로 쓴다. 넘치면 코드가 5·9절을 뺀 큰 절부터 주제 페이지로 옮기고 원 절에는 네 summary 와 링크를 남긴다. 그러니 summary 는 그 절만 읽어도 핵심이 전달되게 쓴다.

**R-2 상태 줄 금지.** 본문에 "상태: …", "페이지 상태: …" 를 쓰지 않는다. H1 바로 아래에 `<!-- auto:page-status:start -->` 와 `<!-- auto:page-status:end -->` 두 줄만 둔다(템플릿에 있으면 그대로 유지).

**R-3 차등 갱신(patches).** 실행 유형이 update 또는 category_link 이면 `content` 대신 `patches` 를 보낸다: `[{"section": "<H2 제목 전체>", "action": "replace" 또는 "append", "content": "<절 본문, 제목 줄 제외>"}]`. 바꾸지 않는 절은 보내지 않는다. 새 페이지(action create)는 patches 가 아니라 content 로 보낸다. 코드가 현재 페이지에 패치를 적용해 전체 페이지를 만든다.

**R-4 대분류 연결(category_link).** 대상 대분류 페이지의 "다른 대분류와의 연결" 절 하나만 patches 로 바꾼다. 연결마다 한 단락 또는 표 한 행: 어느 세부영역(번호와 이름)이 어느 대분류(문자와 이름)의 어느 세부영역과 무엇으로 이어지는지, 근거 태그와 각주. 각주 정의도 그 절 끝에 함께 넣는다(대분류 페이지의 "참고 자료" 절은 원문 각주용이므로 건드리지 않는다). Mermaid 도식을 쓰면 노드는 이름으로 적는다.

**R-5 트랙 산출물.** 트랙 실행은 트랙 설정의 `draft_page`(초안 문서)와 `idea_page`(docs/ideas/ 의 아이디어 페이지)를 살아있는 산출물로 갱신한다. 아이디어 페이지는 3~6절(선행 연구·제품 사례, 필요한 데이터와 표준, 구현 가설, 검증 방법)에 이번 실행의 검증된 finding 을 반영하고, 7절 백로그는 자동 영역이므로 손대지 않는다. 트랙 설정의 `glossary_targets` 용어를 브리프가 다뤘으면 `glossary_updates` 로 낸다.
