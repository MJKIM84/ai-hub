...
**R-1 목차와 글자 예산을 먼저.** 본문을 쓰기 전에 `outline` 을 만든다. 페이지마다 쓸 절(세부영역은 3~11절)과 절별 글자 예산(budget_chars), 그 절의 1~2문장 요약(summary, 태그·각주 포함)을 적는다. 세부영역 3~11절 예산 합은 4,000자 안팎으로 잡되, 검증된 finding 이 더 많으면 줄이지 말고 그대로 쓴다. 넘치면 코드가 5·9절을 뺀 큰 절부터 주제 페이지로 옮기고 원 절에는 그 절 첫 문단의 앞 문장(최대 두 문장)과 링크를 남긴다. 그러니 절마다 첫 문장은 그 절만 읽어도 핵심이 전달되는 독자용 요약 문장(태그·각주 포함)으로 쓴다. summary 는 예산 계획용이며 페이지에 실리지 않는다.

**R-2 상태 줄 금지.** 본문에 "상태: …", "페이지 상태: …" 를 쓰지 않는다. H1 바로 아래에 `<!-- auto:page-status:start -->` 와 `<!-- auto:page-status:end -->` 두 줄만 둔다(템플릿에 있으면 그대로 유지).

**R-3 차등 갱신(patches).** 실행 유형과 관계없이 이미 있는 페이지를 고칠 때(action update)는 `content` 대신 `patches` 를 보낸다(주제 조사·트랙·주간 정리·월간 재검증·갱신·대분류 연결 모두). 예외 하나: 영역 심화(area_deep_dive)에서 대상 세부영역 페이지가 seed 상태면 3~11절을 처음 채우므로 `content` 로 보낸다. 형식: `[{"section": "<H2 제목 전체>", "action": "replace" 또는 "append", "content": "<절 본문, 제목 줄 제외>"}]`. 바꾸지 않는 절은 보내지 않는다. 새 페이지(action create)는 patches 가 아니라 content 로 보낸다. 코드가 현재 페이지에 패치를 적용해 전체 페이지를 만든다. 패치한 페이지의 프런트매터 `version`(기존 +1)·`updated`(실행 날짜)·`status`(draft)와, 패치 본문에 새로 쓴 각주 참조의 정의 줄(참고문헌 페이지나 이번 브리프 출처에서 만든다)은 코드가 채우므로 따로 보내지 않아도 된다. 월간 재검증의 needs_update·deprecated 처럼 다른 상태가 필요할 때만 패치의 `frontmatter` 에 적는다.
