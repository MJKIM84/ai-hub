...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(ref-112 OMG 페이지 열람 차단). 검색 결과로 ISO/IEC 19510:2013(iso.org/standard/62652)이 OMG BPMN 2.0.1을 PAS 절차로 채택한 것과 '모든 업무 사용자가 쉽게 이해할 수 있는 표기법' 목표는 확인했다. 브리프 발췌의 '2.0.2 가 2013년판으로 발행됨'은 틀렸다. ISO/IEC 19510:2013은 BPMN 2.0.1과 같고, OMG BPMN 2.0.2는 2014-01에 나왔다. 기준일 2013."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "원문 미열람. ref-121(Blondin·Mazowiecki·Offtermatt, LICS 2022)는 검색으로 실재를 확인했고, 'workflow nets … one of the standard ways to model and analyze workflows' 부분을 뒷받침한다. ref-120(Formal Aspects of Computing 2011)은 검증 검색 예산(회당 30회)이 바닥나 실재를 확인하지 못했다(원문 미열람(검증 예산)). 따라서 '건전성이 교착·라이브락 부재를 보장한다'는 부분은 [추정]으로 강등한다."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "삭제",
      "note": "ref-115(장크트갈렌 대학 Alexandria 저장소, 저자·발행일 미확인)은 검증 검색 예산이 바닥나 실재를 확인하지 못했다(원문 미열람(검증 예산)). 이번 페이지에서 빼고, 다음 실행의 additional_research_requests로 넘긴다."
    },
    {
      "finding_id": "f21",
      "source_exists": true,
  ...
  "duplication": {
    ...
    "overlaps": [
      ...
      "참고문헌 id 충돌 가능성: 이전 브리프 2026-09-25-04·05·06이 ref-110~ref-061을 다른 출처에 이미 부여했다. 참고문헌 목록(ref-052까지)에는 없어 이번 브리프는 next_ref_id를 따랐다. 퍼블리셔 확인이 필요하다."
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "required_fixes": [
    ...
    "f16: '워크플로 넷은 워크플로를 모델링·분석하는 표준적 방법 가운데 하나다'는 [사실]로 두고 ref-064만 각주로 단다. '건전성이 교착·라이브락 부재를 보장한다'는 [추정]으로 강등하고 ref-120 각주에 '원문 미열람' 표시를 유지한다. 이유: ref-063은 검증 예산 부족으로 실재 확인을 못 했다.",
    ...
    "f21: '전문가 검증으로 결과를 확인했다'는 빼고, 네 형식을 임무 수준에서 비교했다는 부분만 쓴다. ref-116 기관 칸은 'Filippone, G., Pettinari, S., & Pelliccione, P.'로 등록한다. 이유: 검증 검색에서 저자명은 확인됐고 전문가 검증은 확인되지 않았다.",
    "ref-121: reference_updates 기관 칸을 'Blondin, M., Mazowiecki, F., & Offtermatt, P. (LICS 2022)'로 등록한다. 이유: 검증 검색으로 저자를 확인했다.",
    ...
    "각주: 원문을 열지 못한 출처 ref-112·ref-114·ref-116·ref-119·ref-120·ref-121·ref-122·ref-123·ref-067은 각주 정의의 접근일 뒤에 ' (원문 미열람)'을 붙이고 reference_updates에 source_unopened: true를 넣는다. ref-023·ref-031은 입력 data/source_texts 원문으로 확인했으므로 ' (원문 미열람)'을 붙이지 않는다.",
  ...
  "confidence": "medium",
  "verification_note": "판정: 1차 조건부 승인. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(fetch_mode mirror_only; raw.githubusercontent.com 원문과 입력 source_texts만 열람). 확인 21건, 미확인 3건(f15·f16·f20), 교차 확인 1건(f23). 강등: f15 사실 → 확인된 SCOR 단계(F1.3·F1.11)로 축소, f16 건전성 부분 사실 → 추정. 삭제: f20(ref-115 실재 미확인, 검증 예산 소진). 원문 미열람 출처: ref-112, ref-114, ref-115, ref-116, ref-119, ref-120, ref-121, ref-122, ref-123, ref-124. 원문 확인 출처: ref-044·ref-049·ref-110·ref-111·ref-113·ref-117·ref-118(GitHub 원본), ref-023·ref-031(입력 원문 텍스트). 주의: 핵심 정의가 모두 발행 기관 한 곳의 자료라 교차 확인이 거의 없다. '운반 완료'와 '인수 확인·재고 반영 완료'를 별도 단계로 두자는 결론(f3·f14·f24)은 표준 정의를 엮은 추정이며, 적용 표준이나 사례는 확인되지 않았다(oq-001과 연결). Camunda 동작(f2)은 벤더 주장이다. 브리프 기록 불일치: ref-023·ref-031은 입력 원문 텍스트로 확인됐으므로 fetched_via는 inbox여야 하고, 출처 요약의 '원문 미열람.' 머리말은 fetched: true와 모순된다. 참고문헌 id ref-110~ref-061은 이전 브리프(2026-09-25-04·05·06)와 충돌할 수 있어 퍼블리셔 확인이 필요하다. 검증 검색은 8회로, 리서치 22회와 합쳐 회당 상한 30회에 도달했다. 정정 요청 없음.",
  "retry_reason": null
}
```
...
```json
{
  ...
  "outline": [
    {
      ...
      "section": "3. 왜 중요한가",
      "budget_chars": 650,
      "summary": "로봇 완료 신호는 물리적 인도만 나타내고 재고 반영·점유 변경은 다른 규격이 정의하므로, 공정 모델은 '운반 완료'와 '인수 확인·재고 반영 완료'를 다른 단계로 두어야 할 것으로 보인다. [추정][^ref-044] SCOR 이행도 배송 증빙·고객 인수 단계로 끝난다. [사실][^ref-123]",
      "planned_findings": [
        "f14",
        "f15",
        "f23"
    ...
    },
    {
      ...
      "section": "4. 핵심 개념과 용어",
      "budget_chars": 750,
      "summary": "BPMN, ISA-95 세그먼트 의존, Open-RMF 작업·단계, CBV의 arriving·receiving·accepting, 워크플로 넷, OCEL 2.0 이 이 영역의 기본 용어다. [사실][^ref-112]",
      "planned_findings": [
        "f1",
        "f6",
        "f9",
    ...
    },
    {
      ...
      "budget_chars": 900,
      "summary": "이 위키는 관련 접근법을 업무 프로세스 표기, 세그먼트 의존, 로봇 작업 단계 구성, 형식적 설계 점검의 네 갈래로 정리한다. [의견]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 600,
      "summary": "BPMN(ISO/IEC 19510), IEC 62264-3, B2MML, Open-RMF, VDA 5050, GS1 CBV, SCOR, OCEL 2.0 이 관련 표준이다. [사실][^ref-117]",
      "planned_findings": [
        "f1",
        "f4",
        "f5",
    ...
    },
    {
      ...
      "section": "8. 대표 연구와 자료",
      "budget_chars": 450,
      "summary": "BPMN 기반 다중 로봇 실행 프레임워크 FaMe, 로봇 임무 기술 형식 비교, 워크플로 넷 건전성 복잡도, OCEL 2.0 명세가 대표 자료다. [사실][^ref-114]",
      "planned_findings": [
        "f16",
        "f17",
        "f18",
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "섹션 3~11 신규 작성(BPMN·ISA-95/B2MML·Open-RMF 작업 단계·CBV 업무 단계·워크플로 넷·OCEL 2.0, 입고 → 적치·출하 시나리오), 페이지 상태 자동 영역 추가. 2차 수정: 5절 태그 강등 2건·예외 칸 태그 추가, 6절 요약 [의견], 프런트매터 sources 정리"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area02-s4.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 2. 공정·워크플로 모델링 의 \"4. 핵심 개념과 용어\" 절을 옮겼다. 2차 수정: ref-120 각주와 건전성의 교착·라이브락 문장을 빼고 ref-121 근거 문장으로 줄였다"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area02-s6.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 2. 공정·워크플로 모델링 의 \"6. 대표 접근법과 기술\" 절을 옮겼다. 2차 수정: 첫 문장을 [의견] 정리로 바꾸고 ref-120 문장을 ref-121 근거 문장으로 줄였다"
    },
    {
    ...
    },
    {
      ...
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 2. 공정·워크플로 모델링 의 \"10. 다른 연구영역과의 연결 (번호와 이름을 함께 표기)\" 절을 옮겼다(링크는 주제 페이지 위치 기준). 2차 수정: 7번 항목 [추정] 강등, 12번 항목 두 주장 분리, 1번 항목 ref-119 추가"
    }
  ],
  "changelog_entry": "2026-09-25 | 2. 공정·워크플로 모델링 | 영역 심화 3~11절 초안 작성(BPMN·ISA-95/B2MML·Open-RMF 작업 단계·CBV 업무 단계·워크플로 넷·OCEL 2.0, 입고 → 적치·출하 시나리오, 1차 조건부 승인 수정 13건과 2차 수정 지시 9건 반영) | run 2026-09-25-09",
  "index_updates": {
  ...
  "glossary_updates": [
    ...
    },
    {
      ...
      "description": "워크플로 넷의 건전성(soundness) 판정은 그 계산 복잡도까지 다뤄지는 연구 대상이다.",
      "related_areas": [
        2,
        23
      ],
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "cited_by": [
        "docs/categories/a-business-supply-chain-design/02-process-and-workflow-modeling.md",
        "docs/topics/2026/2026-09-25-area02-s7.md",
    ...
    },
    {
      ...
      "cited_by": [
        "docs/categories/a-business-supply-chain-design/02-process-and-workflow-modeling.md",
        "docs/topics/2026/2026-09-25-area02-s4.md",
        "docs/topics/2026/2026-09-25-area02-s6.md",
        "docs/topics/2026/2026-09-25-area02-s7.md",
        "docs/topics/2026/2026-09-25-area02-s10.md"
      ]
    },
    ...
    },
    {
      ...
      "cited_by": [
        ...
        "docs/topics/2026/2026-09-25-area02-s6.md",
        "docs/topics/2026/2026-09-25-area02-s7.md",
        "docs/topics/2026/2026-09-25-area02-s8.md",
        "docs/topics/2026/2026-09-25-area02-s10.md"
      ]
    },
    {
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-044, ref-110, ref-112, ref-117, ref-118, ref-121, ref-122]
last_run: 2026-09-25
version: 1
...
- **BPMN(Business Process Model and Notation, 비즈니스 프로세스 모델 및 표기법)** — OMG(Object Management Group)가 정한 프로세스 표기법으로, 업무 분석가부터 구현 개발자·운영 관리자까지 이해할 수 있는 표기를 목표로 한다. ISO/IEC 19510:2013은 OMG BPMN 2.0.1을 공개 규격(PAS) 절차로 국제표준화한 것이다. [사실][^ref-112]
- **운영 세그먼트 의존(Segment Dependency)** — [ISA-95](../../glossary/isa-95.md)의 XML 구현 B2MML에서 운영 세그먼트는 SegmentDependency 요소와 DependentOperationsSegmentID 요소를 두고, 공통 스키마의 의존 유형은 NotFollow, PossibleParallel, NotInParallel, AtStart, AfterStart, AfterEnd, NoLaterAfterStart, NoEarlierAfterStart, NoLaterAfterEnd, NoEarlierAfterEnd, Other 값을 둔다(2023년 판 0701 기준). [사실][^ref-117][^ref-118]
- **작업과 단계(task, phase)** — [Open-RMF](../../glossary/open-rmf.md)는 작업을 단계를 만들어 내는 객체로 본다(2026-09-25 확인). [사실][^ref-110]
...
- **워크플로 넷(Workflow Net)과 건전성(soundness)** — 워크플로 넷은 워크플로를 모델링·분석하는 표준적 방법 가운데 하나로 쓰이는 페트리 넷의 한 부류다(2022년 기준). [사실][^ref-121] 워크플로 넷의 건전성(soundness) 판정은 그 계산 복잡도까지 다뤄지는 연구 대상이다. [사실][^ref-121]
- **OCEL(Object-Centric Event Log, 객체 중심 이벤트 로그) 2.0** — 이벤트와 여러 객체(예: 주문·품목·출하) 사이 관계를 명시적으로 기록하는 이벤트 로그 교환 표준으로, 객체 간 관계·관계의 한정자(qualifier)·시간에 따라 바뀌는 객체 속성을 담고 SQLite·XML·JSON 세 교환 형식을 둔다(2024-03 발행). [사실][^ref-122]
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-044, ref-110, ref-111, ref-112, ref-113, ref-117, ref-118, ref-121]
last_run: 2026-09-25
version: 1
...
BPMN 엔진 Camunda 8 문서는 메시지 대기 지점(수신 작업·메시지 중간 이벤트)이 활성화되면 메시지 이름과 상관 키(correlation key)로 구독을 만들고, 들어온 메시지를 구독에 맞춰 공정 인스턴스에 연결하며, 유지 시간(TTL) 동안 메시지를 보관하고, 같은 이름·상관 키·메시지 ID의 메시지가 아직 버퍼에 있는 동안에는 중복 메시지를 거부한다고 설명한다(2026-09-25 확인). [추정] 벤더 주장[^ref-113] 이 구조를 쓰면 로봇 운반 뒤 인수 확인을 기다리는 단계를 둘 수 있을 것으로 보이며, 아래 도식은 이 추정 구성을 그린 것이다(적용 사례 미확인). [추정][^ref-112][^ref-113][^ref-044]
...
B2MML 운영 정의 스키마는 운영 세그먼트에 SegmentDependency 요소와 DependentOperationsSegmentID 요소를 두어 선후관계를 표현한다. [사실][^ref-117][^ref-118] 공통 스키마의 자재 사용 유형(Consumed, Produced, Consumable, By-product Produced, Co-product Produced, Inventoried 등)은 세그먼트가 자재를 소비하는지 생산하는지 재고로 두는지를 구분한다. [사실][^ref-117] 이 의존 유형을 창고 작업에 쓰면 선후·병행·시간 제약을 세밀하게 표현할 수 있을 것으로 보이나, 창고 물류에 적용한 사례는 확인하지 못했다. [추정][^ref-117][^ref-118]

### 로봇 작업의 단계 구성(Open-RMF)

Open-RMF는 배송 작업을 픽업 지점 이동·화물 수령·하역 지점 이동·화물 인도·복귀 단계로 나누고, Compose 유형으로 단계·활동의 순서를 직접 조합하게 하며, 여러 층 배송의 승강기 요청 같은 필수 단계는 필요할 때 자동으로 더한다(2026-09-25 확인). [사실][^ref-110] 작업 상태 스키마는 작업 상태를 uninitialized, blocked, error, failed, queued, standby, underway, delayed, skipped, canceled, killed, completed 12개 값으로 두고, 단계를 완료·진행·대기로 나눠 보고한다. [사실][^ref-111]

### 설계 점검(워크플로 넷)

선후관계·병렬·대기를 담은 제어 흐름은 워크플로 넷으로 모델링·분석할 수 있다. [사실][^ref-121] 워크플로 넷의 건전성(soundness) 판정은 그 계산 복잡도까지 다뤄지는 연구 대상이다(2022년 기준). [사실][^ref-121]
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-044, ref-110, ref-111, ref-112, ref-117, ref-118, ref-119, ref-122, ref-123]
last_run: 2026-09-25
version: 1
...
| 이름 | 유형 | 이 영역과의 관계 | 출처 |
|---|---|---|---|
| BPMN 2.0 / ISO/IEC 19510:2013 | 표준 | 업무 단계·순서·대기를 그리는 표기법. ISO/IEC 19510:2013은 OMG BPMN 2.0.1을 PAS 절차로 채택한 것이다. [사실][^ref-112] | ref-112 (원문 미열람) |
| IEC 62264-3:2016 (ISA-95 Part 3) | 표준 | 수준 4(업무 계획·물류)와 수준 2(공정 제어) 사이 제조 운영 관리를 생산·유지보수·품질·재고 운영 관리 네 활동 모델로 정의하며, 재고 운영 관리는 재고와 자재 이동을 조정·지시·관리·추적하는 활동이다(2016년 판). [사실][^ref-119] | ref-119 (원문 미열람) |
| B2MML (판 0701) | 표준 | MESA International이 ISA-95 데이터 모델을 XML 스키마로 구현한 것으로, ANSI/ISA-95.00.02-2018과 ANSI/ISA-95.00.05-2018을 기반으로 한다(2023). 세그먼트 의존 유형을 제공한다. [사실][^ref-117][^ref-118] | ref-117, ref-118 |
| Open-RMF 작업·작업 상태 스키마 | 오픈소스 | 로봇 작업을 단계로 구성하고 작업·단계 상태를 보고한다. [사실][^ref-110][^ref-111] | ref-110, ref-111 |
...
| [SCOR](../../glossary/scor.md) Fulfill | 표준 | B2C 이행 단계 F1.3 Pick Product와 마지막 단계 F1.11 Obtain Proof of Delivery or Customer Acceptance를 둔다. [사실][^ref-123] | ref-123 (원문 미열람) |
| OCEL 2.0 | 표준 | 여러 객체에 걸친 실행 이벤트를 기록하는 이벤트 로그 교환 표준이다(2024-03). [사실][^ref-122] | ref-122 (원문 미열람) |
...
### runs/2026-09-25-09/verification2.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "2차 검증에서 원문은 다시 열지 않았다(1차 판정 기준). f14는 1차에서 [추정]·low로 유지하기로 한 추론이다. 그런데 스토리텔러가 세부영역 페이지 5절의 '이 시점은 CBV로 보면 arriving에 가깝다. [사실][^ref-044]'와 분리 페이지 2026-09-25-area02-s10.md의 7. 화물·재고·자산 식별과 추적 항목 'CBV의 arriving·receiving·accepting 구분이 두 완료를 잇는 기준이 된다. [사실][^ref-044]'에서 이 추론을 [사실]로 올렸다. 로봇 완료 신호를 CBV 단계에 대응시키는 판단은 ref-044에 없으므로 두 곳 모두 [추정]으로 되돌린다."
    },
    {
      "finding_id": "f16",
      "source_exists": false,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "원문 미열람. ref-064가 뒷받침하는 '워크플로 넷은 워크플로를 모델링·분석하는 표준적 방법 가운데 하나다'는 [사실]로 그대로 둔다. '건전성이 교착·라이브락 부재를 보장한다'는 부분만 문제다. 이 부분의 근거인 ref-063은 1차에서 실재를 확인하지 못했다(source_exists false, 검증 예산 소진). 공통 규칙 2절은 존재를 확인하지 않은 출처를 어떤 출력에도 넣지 못하게 하고, 2차 필드 검사도 reference_updates에 source_exists false 출처가 있으면 안 된다고 정한다. 따라서 1차 지시 가운데 'ref-120 각주 유지' 부분을 정정한다. ref-120 부분은 페이지에서 빼고, 이미 올라가 있는 additional_research_requests로 넘긴다."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "분리 페이지 2026-09-25-area02-s11.md는 새 질문 3건을 oq-001·oq-002와 함께 싣고, 셋째 새 질문을 oq-001과 연결했다. 1차가 지시한 첫째 새 질문과 oq-002의 연결 문구는 없지만 두 질문이 같은 목록에 있어 수정 지시 대상으로 보지 않는다.",
      "참고문헌 id 충돌 가능성(ref-110~ref-118, 이전 브리프 2026-09-25-04·05·06)은 1차에서 넘긴 대로 퍼블리셔 확인 사항으로 남는다."
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "세부영역 페이지 5절 시나리오 1 표 아래 문장 '로봇이 팔레트를 내려놓으면 … 이 시점은 CBV로 보면 arriving에 가깝다. [사실][^ref-044]'의 태그를 [추정]으로 바꾸고 각주를 [^ref-031][^ref-049][^ref-044]로 단다. 이유: 로봇 완료를 CBV arriving에 대응시킨 것은 f14의 추론([추정])이고 ref-044에는 없다.",
    "분리 페이지 docs/topics/2026/2026-09-25-area02-s10.md의 7. 화물·재고·자산 식별과 추적 항목에서 'CBV의 arriving·receiving·accepting 구분이 두 완료를 잇는 기준이 된다. [사실][^ref-044]'를 [추정]으로 바꾼다. 이유: f14 추론의 태그가 올라갔다.",
    "세부영역 페이지 6절의 요약 문장과 분리 페이지 2026-09-25-area02-s6.md의 1. 세 줄 요약 첫 줄·3. 본문 첫 문장 '두 완료를 나눠 표현하는 방법은 업무 프로세스 표기, … 형식적 설계 점검으로 나뉜다. [사실]'을 고친다. 문장을 이 위키의 정리로 다시 쓰고(예: '이 위키는 관련 접근법을 … 네 갈래로 정리한다') 태그를 [의견]으로 바꾼다. 이유: 이 분류는 브리프 finding에 없는 스토리텔러의 종합이다. 특히 워크플로 넷·Open-RMF 단계가 '두 완료를 나누는 방법'이라는 근거는 없다. 태그 바로 뒤에 괄호를 붙이지 않는다.",
    "세부영역 페이지 5절 시나리오 2 아래 문장 '출하에서도 로봇 작업 완료와 고객 인수 사이에 업무 단계가 남는다. [사실][^ref-123]'을 [추정]으로 바꾸고 각주를 [^ref-031][^ref-123]으로 단다. 이유: f11(drop 완료)과 f15(F1.11)를 엮은 추론이며, 어느 출처도 이 결론을 직접 말하지 않는다.",
    "ref-063을 모든 출력에서 뺀다. 분리 페이지 s4·s6의 [^ref-120] 각주와 각주 정의, 프런트매터 sources의 ref-120, reference_updates의 ref-120 항목을 지운다. 이에 따라 s4의 '건전성은 … 교착·라이브락 … 이상이 없음을 보장하는 속성으로 설명된다. [추정][^ref-120]'과 s6의 '건전성 검사는 교착·라이브락 … 보장하는 데 쓰이는 것으로 보인다. [추정][^ref-120]' 문장을 지우거나, ref-064로 뒷받침되는 '워크플로 넷의 건전성(soundness) 판정이 연구 대상이다' 수준으로 줄인다. 교착·라이브락 언급은 넣지 않는다. 용어집 workflow-net의 description에서도 교착·라이브락 문구를 뺀다. 이유: ref-063은 1차에서 실재를 확인하지 못했고(f16 source_exists false), 공통 규칙은 존재를 확인하지 않은 출처를 출력에 넣지 못하게 한다. 1차의 'ref-120 각주 유지' 지시를 이 지시로 정정한다. 확인 요청은 additional_research_requests에 이미 있다.",
    "세부영역 페이지 프런트매터 sources에서 ref-114·ref-120·ref-065를 뺀다. 이유: 자동 분리 뒤 이 페이지 본문과 13절 각주에 세 출처가 없다. reference_updates의 cited_by도 이 페이지를 가리키지 않아 서로 맞지 않는다.",
    "분리 페이지 2026-09-25-area02-s10.md의 12. 명령·작업 실행의 신뢰성 항목을 두 주장으로 나눈다. '메시지 상관·중복 거부'는 [추정] 벤더 주장[^ref-113]으로, '작업 상태 12개 값'은 [사실][^ref-111]로 적는다. 이유: 작업 상태 값은 ref-111(f10)의 사실인데 벤더 각주 하나에 묶여 태그와 출처가 어긋났다.",
    "분리 페이지 2026-09-25-area02-s10.md의 1. 주문·업무 시스템 연계 항목에서 '재고 운영 관리' 부분의 각주에 [^ref-119]를 더하고, 프런트매터 sources와 8. 출처에 ref-062를 넣는다. 이유: f24의 근거 출처는 ref-044와 ref-062다.",
    "세부영역 페이지 5절 시나리오 1 예외·성과 칸의 '인수 확인이 오지 않거나 하역이 실패하면 공정은 대기하거나 예외로 분기해야 한다.'에 태그와 각주가 없다. [추정][^ref-044][^ref-119]를 붙인다. 이유: f24 추론에 기댄 당위 서술이다."
  ],
  "confidence": "medium",
  "verification_note": "판정: 1차 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(fetch_mode mirror_only). 열람한 것은 raw.githubusercontent.com 원문과 입력 source_texts뿐이다. 확인 21건, 미확인 3건(f15·f16·f20), 교차 확인 1건(f23). 강등: f15는 확인된 SCOR 단계(F1.3·F1.11)만 남기고, f16의 건전성 부분은 사실 → 추정으로 낮췄다. 2차에서 f16의 ref-120 부분을 삭제로 바꿨다(출처 실재 미확인). 삭제: f20. 원문 미열람 출처: ref-112, ref-114, ref-116, ref-119, ref-121, ref-122, ref-123, ref-124. 주의: 핵심 정의는 모두 각각 한 발행 기관의 자료에만 기대며 교차 확인이 거의 없다. '운반 완료'와 '인수 확인·재고 반영 완료'를 별도 단계로 두자는 결론(f3·f14·f24)은 표준 정의를 엮은 추정이다(oq-001과 연결). Camunda 동작(f2)은 벤더 주장이다. 정정 요청은 없다. / 2차 수정 후 재검증. 드리프트 4건을 적발했다: f14 추론을 [사실]로 올린 2곳, 6절 접근법 분류를 [사실]로 쓴 곳, 출하 시나리오의 결합 추론을 [사실]로 쓴 곳. 출처 문제 1건: 실재를 확인하지 못한 ref-063이 각주와 reference_updates에 남아 있다(1차 지시 정정). 각주·태그 누락 3건, 프런트매터 sources 불일치 1건도 있다. [분류원문] 보존, 섹션 순서 준수, 링크 유효. 1차 수정 지시 13건은 모두 이행됐다. 범위 경계는 지켰다(9절 연계 대상 표기). 검증 검색은 회당 상한 30회에 이미 도달해 2차에서는 도구를 쓰지 않았다.",
  "retry_reason": null
}
```
