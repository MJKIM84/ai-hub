...
```json
{
  ...
  "self_check": {
    ...
    "unverified": [
      ...
      "ref-031·ref-096~ref-102·ref-103 발행일 미확인"
    ],
    "scope_violations": [
    ...
    "limits": "재실행 1회차(스키마 불일치 반려). 반려 사유 1(f20: 벤더 문서만 근거로 한 사실 태그에 vendor_claim 없음): 직전 반환값이 입력에 포함되지 않아 형식만 고칠 수 없었으므로 브리프를 다시 작성했고, 벤더 문서(ref-103)만 근거로 한 주장은 f14 하나로 두어 vendor_claim true·태그 추정·신뢰도 low·evidence_excerpt 첫머리 '벤더 주장: '으로 표시했다. 이번 브리프의 finding 번호는 직전 반환값과 대응하지 않는다. fetch_mode mirror_only: VDA 5050 명세, Open-RMF rmf_api_msgs 스키마 3건과 task_new 원본, B2MML README·거래 프로파일 스키마는 raw.githubusercontent.com 으로 열었다(fetched true). OPC 10031-4, arXiv 논문, 벤더 블로그는 원문 미열람(신뢰도 상한 medium). 검색 7회/30, 신규 출처 9건/15(ref-096~ref-103), 재사용 1건(ref-031). ref-001 SCOR·ref-002 ISA-95 는 이번 주장의 근거로 쓰지 않아 넣지 않았다. 한국 자료는 벤더 블로그 1건뿐이며 공공기관·학술 자료는 찾지 못했다. ERP·TMS·MES 연계 사례와 SCOR 관점 서술은 조사하지 못해 섹션 3·8 근거가 얇다. 27. AI·학습·적응과 모델 운영 관련 finding 없음."
  }
}
```
...
```json
{
  ...
  "claim_checks": [
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추론 finding. 근거 f1·f5·f7·f11 은 확인됐고, 매핑 부재는 검색 범위 기준이며 부재의 확인이 아님을 유지해야 한다. ref-102 는 원문 미열람인데 브리프가 source_unopened false 로 적었다(브리프 기록 누락)."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추론 finding. 근거 f10·f12(원문 미열람)·f1·f2·f6 확인. cancelOrder 가 정지와 동작 상태만 규정하고 화물 원위치 복귀를 규정하지 않는다는 점은 원문 6.1.3절과 맞다. ref-102 원문 미열람 표기 누락(브리프)."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "ref-031 은 기존 참고문헌 재사용(정상). 새 id ref-096~ref-103 은 참고문헌 목록(ref-052 까지, ref-046 결번)과는 겹치지 않으나, 이전 브리프 2026-09-25-04·05·06 이 같은 id 를 다른 출처에 부여했으므로 퍼블리셔가 등록 시 충돌 여부를 확인해야 한다.",
  ...
  "required_fixes": [
    ...
    "각주: ref-102·ref-060·ref-103 의 각주 정의 접근일 뒤에 ' (원문 미열람)'을 붙이고 reference_updates 의 세 항목에 source_unopened: true 를 넣는다. ref-031·ref-096~ref-101 은 공식 저장소 원문(github_raw·inbox)을 열었으므로 붙이지 않는다.",
    "5절 현장 시나리오: 출하 우선순위 변경(f14·f16)과 피킹 중 취소(f17)를 흐름 단계와 여섯 항목(시작 조건·예외·성과 등) 문자열로 명시하고, 수치 없는 설명용 가상 시나리오임을 밝힌다.",
    "용어집: '작업 지시(Job Order)'·'작업 응답(Job Response)' 정의는 ISA-95 용어집 항목과 연결하고 근거 각주를 ref-102(원문 미열람)로 둔다."
  ],
  "confidence": "medium",
  "verification_note": "판정: 조건부 승인. 이번 실행은 원문 열람이 차단된 환경(fetch_mode mirror_only)에서 검증됐으며, 공식 GitHub 저장소 원문(VDA 5050 3.0.0 명세 입력 원문, Open-RMF rmf_api_msgs 스키마 3건·task_new 원본, B2MML README·거래 프로파일 스키마)은 직접 열어 대조했다. 확인 18건, 미확인 0건, 교차 확인 0건. 강등: 없음(f14 는 추정·벤더 주장 유지, 확인되지 않은 재정렬 기준 세부 삭제 지시). 원문 미열람 출처: ref-102, ref-060, ref-103. 브리프 출처 원문 미열람 표시 누락: f15·f17·f18 이 ref-102·ref-060 에 기대는데 source_unopened false 로 적힘. 주의: 모든 사실 주장이 규격별 발행 주체 한 곳의 자료라 독립 교차 확인이 없고, 섹션 3·8 근거가 얇으며(ERP·TMS·MES 연계 사례, SCOR 관점 미조사), 한국 자료는 벤더 블로그 1건이다. ISA-95 계열 작업 지시와 VDA 5050·Open-RMF 작업을 잇는 표준 매핑은 검색 범위에서 확인되지 않았을 뿐 부재가 확인된 것은 아니다. 참고문헌 id ref-096~ref-103 은 이전 트랙 브리프들이 같은 id 를 다른 출처에 쓴 적이 있어 퍼블리셔의 충돌 확인이 필요하다. 정정 요청 없음.",
  "retry_reason": null
}
```
...
```json
{
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "영역 심화: 3~11절 신규 작성(상위 지시의 변경·취소를 로봇 작업으로 옮기는 ISA-95·B2MML·OPC UA Job Control·VDA 5050·Open-RMF 정리), 13절 각주 11건, 페이지 상태 자동 영역 추가. 2차 수정: 5절 시작 조건·9절 마지막 문장을 사실/추정으로 나눔"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area01-s6.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 1. 주문·업무 시스템 연계 의 \"6. 대표 접근법과 기술\" 절을 옮겼다(2차 수정 대상 아님, 변경 없음)"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area01-s4.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 1. 주문·업무 시스템 연계 의 \"4. 핵심 개념과 용어\" 절을 옮겼다(2차 수정 대상 아님, 변경 없음)"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area01-s3.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 1. 주문·업무 시스템 연계 의 \"3. 왜 중요한가\" 절을 옮겼다. 2차 수정: [의견]에 '구축자 의견' 병기"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area01-s8.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 1. 주문·업무 시스템 연계 의 \"8. 대표 연구와 자료\" 절을 옮겼다. 2차 수정: [의견]에 '구축자 의견' 병기"
    }
  ],
  "outline": [
    {
    ...
    },
    {
      ...
      "section": "4. 핵심 개념과 용어",
      "budget_chars": 750,
      "summary": "상위 쪽 작업 지시·작업 응답·B2MML 거래 동사와 로봇 쪽 작업 상태를 나란히 정의한다. [사실][^ref-102]",
      "planned_findings": [
        "f11",
        "f9",
        "f10",
    ...
    },
    {
      ...
      "budget_chars": 400,
      "summary": "B2MML, OPC UA for ISA-95 Job Control, VDA 5050 3.0.0, Open-RMF 작업 API 네 가지를 비교한다. [사실][^ref-100]",
      "planned_findings": [
        "f9",
        "f10",
        "f12",
    ...
    },
    {
      ...
      "budget_chars": 700,
      "summary": "ROP는 작업 지시를 로봇 작업으로 바꾸고 결과를 작업 응답으로 되돌리는 경계에 서는 것으로 보이며, 경계는 제품 전략에 따라 이동할 수 있다. [추정][^ref-102]",
      "planned_findings": [
        "f18",
        "f4"
      ]
    },
    {
      ...
      "budget_chars": 550,
      "summary": "2. 공정·워크플로 모델링, 9. 로봇·제조사 관제 연동, 12. 명령·작업 실행의 신뢰성, 13. 작업 배정 — MRTA, 14. 작업 순서·스케줄링, 20. 예외 복구·재계획·업무 연속성, 28. 표준·상호운용성·다사업자 거버넌스와 연결한다.",
      "planned_findings": [
        "f11",
        "f1",
        "f8",
  ...
  "changelog_entry": "2026-09-25 | 1. 주문·업무 시스템 연계 | 영역 심화: 3~11절 초안 작성(ISA-95·B2MML·OPC UA Job Control·VDA 5050·Open-RMF의 변경·취소 처리와 번역 경계), 2차 수정 반영 | run 2026-09-25-08",
  "index_updates": {
  ...
  "additional_research_requests": [
    ...
    "참고문헌 id ref-096~ref-103 은 이전 브리프가 다른 출처에 쓴 적이 있어 퍼블리셔의 등록 시 충돌 확인 필요(1차 검증 지적).",
  ...
  "fixes_applied": [
    ...
    "각주 — ref-102·ref-060·ref-103 각주 정의 접근일 뒤에 ' (원문 미열람)'을 붙이고 reference_updates 세 항목에 source_unopened: true 를 넣었으며, ref-031·ref-096~ref-101 에는 붙이지 않았다.",
    "5절 — 흐름 단계 '피킹 → 출하'를 명시하고 출하 우선순위 변경(f14·f16)과 피킹 중 취소(f17)를 시작 조건·제약·완료·인계·예외·성과 칸에 넣었으며, 첫 서술 문장에서 수치 없는 설명용 가상 시나리오임을 밝혔다.",
    "용어집 — '작업 지시(Job Order)'·'작업 응답(Job Response)'을 glossary_updates 에 내며 설명에 ISA-95 용어집 항목과의 연결을 적고 sources 를 ref-102 로 두었다(원문 미열람 명시).",
    "2차: 5절 시작 조건 칸 — 'B2MML로는 CHANGE·CANCEL 동사에 해당한다. [사실]'을 'B2MML 거래 동사에는 CHANGE·CANCEL이 있다. [사실][^ref-101] 이 변경·취소는 그 동사에 대응하는 것으로 보인다. [추정][^ref-101]'로 나눴다.",
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-098, ref-100, ref-101, ref-102, ref-103]
last_run: 2026-09-25
version: 1
...
- **작업 지시(Job Order)** — [ISA-95](../../glossary/isa-95.md) 운영 관리 활동 모델에서 실행할 작업 단위의 요청이다. 작업 일정(work schedule)은 하나 이상의 작업 요청(work request)으로, 작업 요청은 하나 이상의 작업 지시로 이루어진다. [사실][^ref-102]
- **작업 응답(Job Response)** — 작업 지시에 대해 실행된 작업을 보고하는 정보로, 작업 지시와 함께 요청–응답 순환을 이룬다. [사실][^ref-102]
- **B2MML(Business To Manufacturing Markup Language)** — MESA International이 공개한 ISA-95(IEC/ISO 62264) 데이터 모델의 XML 스키마(XSD) 구현으로, ERP·공급망 시스템과 제조 실행·제어 시스템의 통합에 쓰인다. [사실][^ref-100]
- **거래 동사(Transaction Verb)** — B2MML 거래 프로파일 스키마(Version 0701, 저작권 표기 2023 기준)는 NOTIFY, GET, PROCESS, CHANGE, CANCEL, CONFIRM, SYNC ADD, SYNC CHANGE, SYNC DELETE와 확장용 Other를 정의한다. [사실][^ref-101]
- **작업 상태(task state)** — Open-RMF 작업 상태 스키마는 queued, underway, delayed, canceled, killed, completed 등 12개 상태 값과 배정 로봇(assigned_to)·예상 소요 시간·중단·취소 정보를 담는다. [사실][^ref-098]
- **[WMS·WES·WCS](../../glossary/wes-wcs-wms-mes-tms.md)** — 국내 로봇 기업 블로그는 WMS가 창고 업무 관리를, 창고 실행 시스템(Warehouse Execution System, WES)이 WMS 작업 지시를 바탕으로 장비·작업 순서 조정을, 창고 제어 시스템(Warehouse Control System, WCS)이 장비 실시간 제어를 맡는다고 설명한다. [추정] 벤더 주장[^ref-103]
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-096, ref-097, ref-098, ref-102]
last_run: 2026-09-25
version: 1
...
상위 업무 쪽의 작업 지시–작업 응답, VDA 5050의 주문–상태, Open-RMF의 작업 요청–작업 상태는 모두 요청–응답 구조다. 그러나 이들을 서로 옮기는 표준 매핑은 이번 검색 범위에서 확인되지 않았고, 이는 부재가 확인된 것은 아니다. [추정][^ref-102][^ref-031][^ref-096][^ref-098]

2절의 질문처럼 진행 중인 작업의 우선순위를 바꾸려 할 때 로봇 인터페이스가 주는 수단은 주문 갱신, 취소 후 재지시, 요청 시점의 우선순위 지정 정도로 보인다. 따라서 우선순위 변경 규칙 자체는 ROP 쪽 작업 대기열에서 정해야 할 것으로 보인다. [추정][^ref-031][^ref-096][^ref-097] 이 번역이 어긋나면 로봇이 빨리 움직여도 업무 시스템의 주문 상태와 현장 작업이 서로 다른 이야기를 하게 된다(구축자 의견). [의견]
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "2차: 세부영역 페이지 5절 표의 시작 조건 칸에 적힌 'B2MML로는 CHANGE·CANCEL 동사에 해당한다. [사실][^ref-101]'은 출하 우선순위 변경 시나리오를 B2MML 동사에 대응시킨 것으로, f17(추정)의 추론 내용이다. f10(사실)은 동사가 정의돼 있다는 것만 말한다. 1차 처분(추정 유지)보다 태그가 올라갔으므로 [추정]으로 되돌리거나, 사실 부분('B2MML 거래 동사에 CHANGE·CANCEL이 있다')만 [사실]로 남기도록 문장을 나눈다."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "참고문헌 id ref-096~ref-103 은 이전 트랙 브리프가 같은 id 를 다른 출처에 쓴 적이 있어(1차 지적) 퍼블리셔가 등록할 때 충돌 여부를 확인해야 한다. additional_research_requests 에 기록돼 있다."
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "세부영역 페이지 5절 표 '시작 조건' 칸: 'B2MML로는 CHANGE·CANCEL 동사에 해당한다. [사실][^ref-101]'을 'B2MML 거래 동사에는 CHANGE·CANCEL이 있다. [사실][^ref-101] 이 변경·취소는 그 동사에 대응하는 것으로 보인다. [추정][^ref-101]'처럼 둘로 나누거나 전체를 [추정]으로 고친다 — 시나리오를 동사에 대응시킨 것은 f17(추정)의 추론이며, f10은 동사가 정의돼 있다는 사실만 뒷받침한다.",
  ...
  "confidence": "medium",
  "verification_note": "판정: 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경(fetch_mode mirror_only)에서 검증됐다. 공식 GitHub 저장소 원문(VDA 5050 3.0.0 명세 입력 원문, Open-RMF rmf_api_msgs 스키마 3건·task_new 원본, B2MML README·거래 프로파일 스키마)은 직접 열어 대조했다. 확인 18건, 미확인 0건, 교차 확인 0건. 강등: 없음(f14 는 추정·벤더 주장 유지, 확인되지 않은 재정렬 기준 세부는 삭제). 원문 미열람 출처: ref-102, ref-060, ref-103. 주의: 모든 사실 주장은 규격마다 발행 주체 한 곳의 자료여서 독립 교차 확인이 없다. 3절과 8절의 근거가 얇고(ERP·TMS·MES 연계 사례와 SCOR 관점은 조사하지 않음), 한국 자료는 벤더 블로그 1건뿐이다. ISA-95 계열 작업 지시와 VDA 5050·Open-RMF 작업을 잇는 표준 매핑은 검색 범위에서 확인되지 않았을 뿐, 없다는 것이 확인된 것은 아니다. 참고문헌 id ref-096~ref-103 은 퍼블리셔가 충돌 여부를 확인해야 한다. 정정 요청 없음. / 2차 수정 후 재검증. 드리프트 2건: 5절 시작 조건 칸의 B2MML 동사 대응 문장과 9절 마지막 문장에서 추론(f17·f18 계열)이 [사실]로 올라갔다. 이 밖에 두 주제 페이지(3절·8절 분리분)의 [의견] 2건에 누구의 의견인지 표시가 빠졌다. [분류원문]은 보존됐다(소속 대분류 블록, 1·2절, 9절 원문 두 문단 글자 단위 일치). 1차 수정 지시 11건은 모두 이행됐다. 범위 경계는 준수됐고 링크는 유효하다(docs_tree 와 이번 실행 생성 페이지 기준). 참고: 흐름 매트릭스 갱신은 7칸인데, 5절 본문은 이 영역이 맡는 칸을 시작 조건·예외·성과·완료·인계로 적어 서술 범위가 조금 다르다. 수정 지시 대상은 아니다. 자동 분리 주제 페이지의 9. 검증 노트는 코드가 만든 한 줄 문구라 1차 판정·건수 형식과 다르므로 퍼블리셔 담당에게 알린다.",
  "retry_reason": null
}
```
