...
- 대상: 트랙 nl-task-chatbot (자연어 업무 지시 챗봇) · 현재 단계: 단계 3. 구현 가설 설계 · 이번에 다룰 백로그 질문 id: q3-02 · 중심 세부영역: 13. 작업 배정 — MRTA (D. 계획·최적화)
- 예산:
...
### runs/2026-09-25-71/target.json
...
```json
{
  ...
  "selection_rationale": "CLI 지정 run_type=track, area=13; 트랙 실행일(Fri, track_days 앞 3개) → 트랙 nl-task-chatbot 단계 3, 질문 q3-02 (CLI 지정 질문 id)"
}
```

### runs/2026-09-25-71/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q3-02 지시 해석 → 작업 분해 → 능력 질의 → 배정 → 스케줄링 → 진행 관리의 흐름에서 단계마다 입력·출력은 무엇이고, 규칙·최적화처럼 결과가 정해진(결정적) 구성 요소는 어디에 두는가?",
    "LLM 과 기호·결정적 검증기를 결합한 로봇 계획 구조(신경-기호 구조)는 어느 단계에 LLM 을, 어느 단계에 계획기·검증기·실행기를 두며, 결정적 검증기를 LLM 비평자로 바꾸면 결과가 어떻게 달라지는가? (단계 3 페이지 3절, 27. AI·학습·적응과 모델 운영 겨냥)",
    "능력 질의(온톨로지 추론)의 출력은 배정 알고리즘에 어떤 형식으로 넘기며, Open-RMF 입찰·rmf_task 같은 오케스트레이션 도구는 배정·일정·진행 상태를 어떤 입력·출력으로 다루는가? (13. 작업 배정 — MRTA 섹션 6, 5. 로봇 능력·작업 온톨로지 연결)",
    "LLM 제안을 진행 상태·실행에 반영하기 전에 검증·승인하는 게이트(안전 게이트, 원자적 반영)는 어디에 두는가? (20. 예외 복구·재계획·업무 연속성, 25. 안전·위험 관리 연결)",
    "LLM 에게 로봇을 도구(MCP, LangChain 도구)로 노출하는 방식은 결정적 구성 요소를 우회할 위험이 있는가? (26. 사이버보안·접근권한·개인정보 연결)",
    "국내에 LLM 해석과 결정적 배정·검증을 잇는 로봇 관제 처리 흐름 연구·사례가 있는가? (한국 자료 우선 규칙)"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "Open-RMF 에서는 사용자가 작업 요청을 내면 디스패처가 모든 플릿 어댑터에 입찰 공고(BidNotice)를 보내고, 처리할 수 있는 플릿 어댑터가 rmf_task 작업 계획기로 비용을 계산해 입찰(BidProposal)하며, 디스패처가 가장 빨리 끝나는 것·가장 낮은 비용 같은 설정 기준으로 비교해 이긴 플릿에 배치 요청(DispatchRequest)을 보내고, 필요하면 충전 작업이 자동으로 끼워진다.",
      "tag": "사실",
      "source_ids": [
        "ref-376"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "작업 수명주기: 요청 제출 → rmf_dispatcher_node 의 BidNotice → 플릿 어댑터의 BidProposal(비용 추정, TaskPlanner 사용) → 설정 기준으로 평가 → DispatchRequest. 짧은 인용: \"tasks are awarded to robot fleets based on the outcome of a bidding process\" (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": "피킹",
      "flow_item": "수행 자원"
    },
    {
      "id": "f2",
      "claim": "Open-RMF rmf_task 의 작업 계획기는 플릿 안의 작업과 로봇을 받아 요청된 시작 시각을 지키며 작업이 가장 빨리 끝나도록 로봇별 작업 순서를 정하는 결정적 구성 요소로, 탐욕 방식과 A* 기반 방식 가운데 하나로 풀고 배터리 제약에 따라 충전 작업을 끼워 넣는다.",
      "tag": "사실",
      "source_ids": [
        "ref-377",
        "ref-404"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "TaskPlanner.hpp 는 greedy 와 A* 기반 방식 선택, 기본 비용 계산기 BinaryPriorityCostCalculator 를 둔다. README 는 자원 제약을 고려한 충전 작업 자동 삽입을 설명한다. (재인용: 2026-09-25-66)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f3",
      "claim": "Open-RMF 작업 상태 스키마는 배정 결과를 assigned_to(그룹·이름)로, 배정 과정을 dispatch 상태(queued·selected·dispatched·failed_to_assign·canceled_in_flight)로, 진행을 status 값과 단계별 상태·예상 소요 시간으로 표현해, 진행 관리 단계가 받을 결정적 상태 기록의 형식이 된다.",
      "tag": "사실",
      "source_ids": [
        "ref-111"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "task_state.json: booking 필수, assigned_to, dispatch 상태 값, status 값(queued·underway·delayed·completed·canceled·failed 등), estimate_millis. (재인용: 2026-09-25-51)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "완료·인계",
      "source_unopened": true
    ...
    {
      "id": "f4",
      "claim": "Electronics(2026) 게재 논문은 로봇·작업·장소의 의미 모델에 선언적 추론과 절차적 평가를 결합해 여러 축의 능력 조건과 적재 상태에서의 장소 도달 가능성을 판정하고, 그 결과를 특정 배정기에 묶이지 않는 ReasonerOutput 으로 정형화해 여러 배정 알고리즘의 공통 입력으로 쓴다고 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-236"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: hybrid reasoning(declarative + procedural)으로 multi-axis capability conditions·loaded-state place reachability 판정, 결과를 allocator-independent ReasonerOutput 으로 정형화. 기존 연구는 실행 가능성을 특정 최적화기·계획기 안에서 다뤘다고 지적.",
      "as_of": "2026-08-11",
      "flow_step": null,
      "flow_item": "수행 자원",
      "source_unopened": true
    ...
    {
      "id": "f5",
      "claim": "LiP-LLM 은 작업 계획을 기술 목록 생성, 의존 그래프 생성, 작업 배정의 세 단계로 나누고, 앞 두 단계는 LLM 이, 배정은 선형계획이 맡는다.",
      "tag": "사실",
      "source_ids": [
        "ref-166"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: LiP-LLM divides task planning into three steps: skill list generation, dependency graph generation, and task allocation(선형계획).",
      "as_of": "2024-10",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f6",
      "claim": "Liu 외(KTH, arXiv 2606.08214)는 산업용 로봇에서 언어 이해·맥락 추론만 LLM 에 맡기고 검증·순서 결정·실행은 모두 결정적으로 두는 Specifier–Designer–Inspector 구조를 제안했으며, Inspector 는 LLM 비평자 대신 기호 제약 검증기이고 LangGraph 기반 동적 경로 설정으로 실패 복구를, Unity3D 디지털 트윈으로 사람 검토를 하며, 5개 난이도의 자연어 명령 70개에서 100% 성공을 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-674"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: LLMs are used for tasks that require language understanding or contextual reasoning, while all verification, sequencing, and execution remain deterministic. 70개 명령·5개 난이도 100% 성공(저자 보고).",
      "as_of": "2026-06",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f7",
      "claim": "같은 연구(Liu 외)의 절제 실험에서 기호 검증기(Inspector)를 같은 방식으로 프롬프트한 LLM 으로 바꾸면 전체 성공률이 98.1% 에서 3.8% 로 떨어졌다고 저자들이 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-674"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: Replacing the symbolic Inspector with an equivalently-prompted LLM collapses overall success from 98.1% to 3.8%, with 0% on Groups C and D(저자 보고값, 산업용 로봇 명령 조건, 독립 재현 미확인).",
      "as_of": "2026-06",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f8",
      "claim": "Pesjak·Žabkar(Machine Learning and Knowledge Extraction 8권, 2026)의 Sense–Plan–Code–Act 틀은 LLM 이 세계 기술을 PDDL 로 바꾸고, 휴리스틱 계획기가 유효한 계획을 만들며, 두 번째 LLM 이 계획을 코드로 바꾼 뒤 그 코드를 컴파일로 구문 검증하고 시뮬레이션으로 의미 검증하는 구조다.",
      "tag": "사실",
      "source_ids": [
        "ref-675",
        "ref-676"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "논문 요약(원문 미열람): LLM→PDDL, heuristic planner→valid plan, second LLM→code, compilation 과 simulation 으로 검증. 공식 저장소 README 는 Plan 단계를 LLM+기호 계획기 혼합, Act 를 실행·평가 뒤 재계획으로 적는다(README 는 Sense 를 결정적 인식으로 적어 논문 요약과 표현이 다름).",
      "as_of": "2026",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f9",
      "claim": "Tang 외(arXiv 2606.31339)는 산업용 다중 로봇에서 작업 위계를 담는 작업 숲(task forest)과 실행 상태·로봇 기록·자원 잠금·세계 믿음·제안·검증 기록을 담는 관리형 블랙보드를 동기화해 두고, 에이전트(LLM)의 제안은 결정적 검증과 원자적 반영(atomic commit)을 거쳐야만 받아들이는 구조를 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-677"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: admits agentic proposals only through deterministic verification and atomic commit; 제안 생성만으로는 작업 의존·자원 소유·안전 보류·복구 경계가 보장되지 않는다는 동기.",
      "as_of": "2026-06",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f10",
      "claim": "CoMuRoS(arXiv 2511.22354)는 작업 관리자 LLM 이 자연어 목표를 해석·분류하고 정적 규칙과 동적 맥락(작업·이력·로봇 상태·사건)으로 하위 작업을 배정하며, 로봇마다 로컬 LLM 이 기본 기술로 실행 코드를 구성하고, 작업 실패나 사용자 의도 변경이 재계획을 촉발하는 구조로, 약 20대 로봇의 22개 텍스트 시나리오 벤치마크에서 정확도 최대 0.91을 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-678"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: Task Manager LLM interprets natural-language goals, classifies tasks, and allocates subtasks using static rules plus dynamic contexts; 재계획 세트 5개 시나리오 정확도 1.0(저자 보고, 실험실·텍스트 벤치마크 조건).",
      "as_of": "2025-11",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f11",
      "claim": "SafeGate(arXiv 2604.05427)는 자연어 명령에서 안전 관련 속성을 구조화해 뽑고 ISO 13482 기반의 결정적 판정으로 실행을 승인·거부한 뒤, 통과한 명령을 불변 조건·가드·중단 조건으로 된 작업 안전 계약으로 분해하는 실행 전 게이트다.",
      "tag": "사실",
      "source_ids": [
        "ref-417"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: deterministic decision gates to approve or reject execution based on ISO 13482. (재인용: 2026-09-25-37)",
      "as_of": "2026-04",
      "flow_step": null,
      "flow_item": "제약",
      "source_unopened": true
    ...
    {
      "id": "f12",
      "claim": "오픈소스 ROS-MCP-Server 는 rosbridge 를 통해 로봇 코드 수정 없이 LLM 에게 ROS·ROS 2 의 토픽 발행·구독, 서비스 호출, 액션, 파라미터를 도구로 노출하며, README 에는 권한 제한이 기여 안내의 계획 항목으로만 언급되고 현재의 권한·제한 장치 설명은 없다.",
      "tag": "사실",
      "source_ids": [
        "ref-679"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: LLM 이 topics, services, actions 와 그 타입(사용자 정의 포함)을 설정 없이 발견; rosbridge 사용; Apache 2.0. 권한(permissions)은 계획 기능으로만 언급. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f13",
      "claim": "한국전자기술연구원 연구진은 LangChain 에이전트의 도구를 ROS 2 토픽·서비스 인터페이스로 정의해 자연어 명령을 로봇 제어 명령으로 바꾸는 다중 로봇 관제 시스템을 발표해, 국내 사례에서도 LLM 이 닿는 범위가 도구 정의로 정해지는 구조가 쓰였다.",
      "tag": "사실",
      "source_ids": [
        "ref-180"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "LangChain 에이전트 도구를 ROS 2 토픽·서비스로 정의, 로봇별 위치·상태 모니터링(학술대회 이름·일자 미확인). (재인용: 2026-09-25-21)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f14",
      "claim": "Rasa 폼은 필수 슬롯을 정해 두고 비어 있는 다음 필수 슬롯을 사용자에게 묻고, 추출한 값을 사용자 정의 검증 동작으로 검사한 뒤 필수 슬롯이 모두 채워지면 비활성화되어, 지시 해석 단계의 출력(채워진 슬롯)을 규칙으로 검사하는 결정적 구성 요소의 예가 된다.",
      "tag": "사실",
      "source_ids": [
        "ref-356"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "forms.mdx: required_slots, utter_ask_<슬롯>, 검증 동작(validate), 모두 채워지면 비활성화. (재인용: 2026-09-25-30)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "시작 조건",
      "source_unopened": true
    ...
    {
      "id": "f15",
      "claim": "STRAP-LLM(Park·Kim, Intelligent Service Robotics)은 구조화 프롬프트로 LLM 이 이종 로봇의 작업 배정과 기술 계획을 로봇 실행 언어로 직접 생성하게 하는 틀로, 저자들은 새 로봇을 추가해도 실행 정확도가 높게 유지된다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-680"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "검색 요약: structured prompting framework that generates task allocation and planning in the robot's execution language; consistently high execution accuracy(저자 보고, 수치·비교 대상·저자 소속 미확인, 게재 연도는 요약마다 2025·2026로 다름). (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f16",
      "claim": "Kambhampati 외는 LLM 을 근사적 아이디어 생성기로 두고 외부 모델 기반 검증기·비평자와 양방향으로 결합하는 LLM-모듈로 틀을 제안하며, 자기회귀 LLM 이 혼자서는 계획이나 자기 검증을 하지 못한다고 본다.",
      "tag": "의견",
      "source_ids": [
        "ref-586"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "ICML 2024 입장 논문 요약: LLM-Modulo, external model-based verifiers. (재인용: 2026-09-25-66)",
      "as_of": "2024-02",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f17",
      "claim": "q3-02 에 대해 확인한 자료를 이 위키가 묶으면, 처리 흐름은 (1) 지시 해석: 입력 채팅·대화 맥락 → 출력 의도·슬롯(규칙 검사), (2) 작업 분해: 입력 슬롯 → 출력 작업 목록·의존 그래프 또는 형식 명세(계획기·검증기 검사), (3) 능력 질의: 입력 작업 요구 → 출력 배정기 독립 실행 가능성 판정(온톨로지 추론), (4) 배정: 입력 판정·비용 → 출력 로봇 또는 플릿(최적화·입찰 비교), (5) 스케줄링: 입력 배정·시각 제약 → 출력 로봇별 순서·충전 삽입(작업 계획기), (6) 진행 관리: 입력 로봇·플릿 상태 보고 → 출력 진행 상태 기록과 재계획 요청으로 나눌 수 있고, LLM 은 (1)·(2)의 제안과 결과 설명에, 결정적 구성 요소는 (2)의 검사와 (3)~(6)에 두는 배치가 근거가 가장 많은 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-356",
        "ref-166",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합: 슬롯 검사(Rasa), 기술 목록·의존 그래프 뒤 선형계획 배정(LiP-LLM), LLM→PDDL→계획기(SPCA), 배정기 독립 판정 출력(ReasonerOutput), 입찰 비교(Open-RMF), 작업 계획기(rmf_task), 작업 상태 스키마, 검증·순서·실행의 결정적 처리(SDI). 이 여섯 단계를 한 흐름으로 제시한 단일 출처는 확인하지 못함.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f18",
      "claim": "확인한 신경-기호 구조들은 LLM 출력이 상태나 실행에 반영되기 직전마다 결정적 검사를 두므로(해석 뒤 슬롯 검사, 분해 뒤 계획기·컴파일·시뮬레이션 검사, 배치 전 안전 게이트·사람 검토, 진행 상태 반영 시 검증 뒤 원자적 반영), ROP 에서도 검증 게이트를 단계 사이 경계에 두는 것이 선택지로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-674",
        "ref-675",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "SDI 의 기호 Inspector, SPCA 의 컴파일·시뮬레이션 검증, 관리형 블랙보드의 결정적 검증·원자적 반영, SafeGate 의 실행 전 판정, Rasa 검증 동작을 이 위키가 대응시킨 추론.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f19",
      "claim": "반례로 CoMuRoS 와 STRAP-LLM 은 LLM 이 배정과 재계획까지 맡으면서 높은 정확도를 보고해 LLM 배정이 배제되는 것은 아니지만, 평가가 실험실·텍스트 벤치마크 조건이고 결정적 배정기와 같은 조건에서 비교한 결과는 확인되지 않아, 흐름 배치의 반박 근거로는 약한 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-678",
        "ref-680",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "CoMuRoS 정확도 최대 0.91(텍스트 벤치마크), STRAP-LLM 실행 정확도 높음(수치 미확인); SDI 절제 실험은 검증기 대체 시 급락을 보고. 결정적 배정기와의 동조건 비교 미확인.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f20",
      "claim": "LLM 에게 로봇 토픽·서비스·액션을 도구로 직접 노출하는 방식(ROS-MCP-Server, LangChain 도구 정의)은 LLM 이 닿는 범위를 도구 목록이 정하므로, 채팅 LLM 에 저수준 로봇 도구를 열면 위 흐름의 능력 질의·배정·검증 게이트를 우회할 수 있어, ROP 는 검증 파이프라인으로 들어가는 상위 도구(작업 요청 제출 등)만 노출해야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-679",
        "ref-180",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "ROS-MCP-Server 는 토픽·서비스·액션을 설정 없이 노출하고 권한 제한은 계획 단계, KETI 는 도구를 ROS 2 인터페이스로 정의. 우회 위험과 상위 도구만 노출하자는 판단은 이 위키의 추론.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f21",
      "claim": "분류 원문 질문(가장 가까운 로봇에 맡기는 것이 전체적으로도 유리한가)과 관련해, 이 흐름에서 '누구에게 맡길지'는 능력 판정으로 거른 후보 가운데 입찰 비교·최적화의 목적 기준(가장 빨리 끝남, 가장 낮은 비용)으로 결정되고 LLM 은 그 목적 가중치를 지시에서 뽑아 넘기는 데 그치므로, 최근접 배정이 전체적으로 유리한지는 배정 단계에 둔 목적 기준에 따라 달라지는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-376",
        "ref-236",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "Open-RMF 디스패처의 설정 기준 비교, ReasonerOutput 의 배정기 독립 입력, 작업 계획기의 완료 시각 최소화를 이 위키가 이은 추론.",
      "as_of": "2026-09-25",
      "flow_step": "피킹",
      "flow_item": "수행 자원"
    },
    {
      "id": "f22",
      "claim": "피킹 단계에서 관리자가 채팅으로 '피킹 끝난 토트를 10시 전까지 포장대로'라고 지시하면, 해석 단계는 장소·대상·기한 슬롯을 채워 검사하고 빠진 값을 되묻고, 능력 질의가 토트 운반이 가능한 로봇을 판정하며, 배정·스케줄링은 입찰·작업 계획기가 정하고, 진행 관리는 작업 상태 기록으로 지연을 알리는 흐름이 가능해 보인다(설명용 가정 사례).",
      "tag": "추정",
      "source_ids": [
        "ref-356",
        "ref-236",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "설명용 가정 사례. 각 단계 구성 요소는 f14·f4·f1·f2·f3 의 출처에 기댄 이 위키의 구성.",
      "as_of": "2026-09-25",
      "flow_step": "피킹",
      "flow_item": "시작 조건"
    },
    {
      "id": "f23",
      "claim": "이번에 확인한 처리 흐름 연구의 평가 환경은 산업용 로봇 셀(SDI), 조작·격자 시뮬레이션(SPCA), 산업용 다중 로봇 임무(관리형 블랙보드), 실험실 이종 로봇 팀(CoMuRoS)이었고, 이종 제조사 창고 플릿에서 LLM 해석부터 진행 관리까지의 흐름 전체를 평가한 자료와 국내 연구는 검색 범위에서 찾지 못했다(부재의 확인은 아님).",
      "tag": "추정",
      "source_ids": [
        "ref-674",
        "ref-675",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "검색 범위의 관찰. 한국어 검색 2회는 해외 논문의 번역 페이지만 나왔다.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
  ...
  "sources": [
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "RMF 작업 수명주기: 디스패처 입찰 공고, 플릿 어댑터 비용 입찰, 설정 기준 비교 뒤 배치 요청, 충전 작업 자동 삽입. 이번 실행에서 mdBook 원본을 열었다.",
      "fetched": true,
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. rmf_task 작업 계획기 헤더(탐욕·A* 방식, 기본 비용 계산기). 이번 실행에서는 다시 열지 않았다.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. rmf_task 저장소 README(작업 순서 계획, 충전 작업 자동 삽입). 이번 실행에서는 다시 열지 않았다.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. Open-RMF 작업 상태 스키마(배정 결과·배정 과정·진행 상태). 이번 실행에서는 다시 열지 않았다.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 온톨로지 기반 의미적 실행 가능성 추론으로 능력 조건·적재 상태 도달 가능성을 판정하고 배정기 독립 출력으로 넘기는 방법.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 이 기술 목록·의존 그래프를 만들고 선형계획이 배정하는 다중 로봇 작업 계획.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자연어 명령의 안전 속성을 뽑아 결정적 게이트로 승인·거부하고 작업 안전 계약으로 분해하는 SafeGate.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LangChain 에이전트 도구를 ROS 2 인터페이스로 정의한 다중 로봇 관제 시스템(국내 발표).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 필수 슬롯 되묻기와 검증 동작을 두는 Rasa 폼 문서. 이번 실행에서는 다시 열지 않았다.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 을 생성기로, 외부 검증기를 비평자로 두는 LLM-모듈로 입장 논문.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 은 언어 이해·맥락 추론만, 검증·순서·실행은 결정적으로 두는 Specifier–Designer–Inspector 구조와 기호 검증기 절제 실험(98.1%→3.8%).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. Machine Learning and Knowledge Extraction 8(1) 22. LLM→PDDL, 계획기, LLM 코드 생성, 컴파일·시뮬레이션 검증의 Sense–Plan–Code–Act 틀.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "title": "minigrid-crewai — Sense–Plan–Code–Act (SPCA) framework (GitHub README)",
      "published": null,
      "url": "https://github.com/DrejcPesjak/minigrid-crewai",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "SPCA 네 단계의 입력·출력과 LLM·결정적 구분, 논문 서지(MAKE 2026, doi 10.3390/make8010022)를 적은 저장소 README.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 작업 숲과 관리형 블랙보드를 두고 에이전트 제안을 결정적 검증·원자적 반영으로만 받아들이는 산업용 다중 로봇 임무 상태 관리 구조.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 작업 관리자 LLM 이 해석·분류·배정, 로봇별 로컬 LLM 이 코드 구성, 사건 기반 재계획을 하는 CoMuRoS 구조와 텍스트 벤치마크 결과.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "rosbridge 로 ROS·ROS 2 의 토픽·서비스·액션·파라미터를 MCP 도구로 LLM 에 노출하는 오픈소스 서버. 권한 기능은 계획 항목으로만 언급.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-680",
      "org": "Park, J., & Kim, J. S.(소속 미확인)",
      "title": "STRAP-LLM: structured task allocation and planning for heterogeneous robots using large language models",
      "published": null,
      "url": "https://link.springer.com/article/10.1007/s11370-025-00676-0",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. Intelligent Service Robotics 게재. 구조화 프롬프트로 LLM 이 이종 로봇의 배정·기술 계획을 실행 언어로 생성하는 틀(게재 연도 미확인).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
  ...
  "page_proposals": [
    {
      ...
      "rationale": "q3-02 답: f1·f2·f3·f4·f5·f6·f7·f8·f9·f10·f11·f12·f13·f14·f15·f16·f17·f18·f19·f20·f21·f22·f23 (신뢰도 low) — 2절 q3-02 상태 답함, 3절 q3-02 소제목 신설({#q3-02}): 오케스트레이션 도구의 흐름(Open-RMF 입찰 f1, 작업 계획기 f2, 작업 상태 f3), 능력 질의의 출력 형식(f4), LLM 분해+해법 배정(f5), 신경-기호 구조(SDI f6·f7, SPCA f8, 관리형 블랙보드 f9, LLM-모듈로 f16 의견), 해석 단계의 규칙 검사(f14), 실행 전 게이트(f11), 도구 노출(f12·f13, 국내 f13), 반례(f10·f15·f19), 종합: 여섯 단계 입력·출력과 결정적 구성 요소 위치(f17, mermaid 도식 권장)·검증 게이트 경계(f18)·도구 노출 경계(f20)·SCM 질문 연결(f21)·피킹 시나리오(f22)·근거 공백(f23) / 4절 결론·불확실성 / 5절 후속 질문 / 6절 완료 조건 현황 / 8절 출처 / 9절 이력"
    },
    {
      "action": "update",
      ...
      "rationale": "아이디어 페이지 5절(트랙 산출물): '처리 흐름과 핵심 구성 요소' 소절 신설 — 단계별 입력·출력과 LLM·결정적 구성 요소 배치 가설 f17·f18(추정), 근거 f1·f2·f3·f4·f6·f7·f8·f9·f14, 도구 노출 경계 f20, 반례 f10·f19. 다른 아이디어와의 연결: 능력 질의(아이디어 1, f4), 장소 슬롯의 공간 노드 해석(아이디어 3)은 구조만 언급하고 근거는 기존 4절 링크"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신: track.ontology_changes(배정 산출 방식에 '입찰 비교' 값, 개념 '실행 가능성 판정' 추가, 개념 '검증 기록' 추가)가 승인되면 2·3절 반영과 초안 버전 인상(f1·f4·f6·f9·f18). 미승인 부분과 '검증 기록'과 배정의 '확인 여부'·사용자 확인 개념(q4-01·q4-04) 관계는 6절 질문으로"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 3 반영 제안 (f1, f4, f5, f17, f21): 능력 판정 출력을 배정기 독립 입력으로 넘기는 방법, 입찰 비교의 위치, LLM 해석–결정적 배정 흐름과 분류 원문 질문 연결. 27. AI·학습·적응과 모델 운영과 양쪽 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 3 반영 제안 (f6, f7, f8, f9, f16, f18): 신경-기호 구조(SDI·SPCA·관리형 블랙보드)와 결정적 검증기 절제 실험, 검증 게이트 배치. 적용 대상 13. 작업 배정 — MRTA 와 함께 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 3 반영 제안 (f12, f13, f20): LLM 에 로봇 토픽·서비스를 도구로 직접 노출하는 MCP·LangChain 방식과 권한 장치 부재, 상위 도구만 노출하는 경계(추정)"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 3 반영 제안 (f4): 온톨로지 기반 실행 가능성 판정 결과를 배정기 독립 출력으로 13. 작업 배정 — MRTA 에 넘기는 연결"
    }
  ],
  "glossary_candidates": [
    {
    ...
    "ontology_changes": [
      {
        ...
        "description": "속성 '배정 산출 방식'의 값 후보에 '입찰 비교(플릿이 낸 비용 입찰을 설정 기준—가장 빨리 끝남·가장 낮은 비용—으로 비교, Open-RMF 디스패처)'를 더한다. 기존 값(LLM 직접 추론 / 최적화 해법)과 충돌하지 않으며, 6절의 '규칙' 값 질문과는 별개다."
      },
      {
        "op": "add",
        "kind": "concept",
        "name": "실행 가능성 판정 (Feasibility Result)",
        "evidence_finding_ids": [
          "f4",
          "f17"
        ],
        "description": "작업 요구를 로봇 능력·장소 모델과 대조한 온톨로지 질의의 출력으로, 로봇(또는 플릿)별 수행 가능 여부와 근거를 담으며 특정 배정 알고리즘에 묶이지 않는 입력(ReasonerOutput 에 해당)이다. 기존 관계 '작업 요구 / 온톨로지 질의로 후보 로봇을 찾는다 / 로봇'의 결과물을 개념으로 드러내는 제안이며, 관계 재구성(작업 요구 → 실행 가능성 판정 → 배정)은 검증 판단에 맡긴다."
      },
      {
        "op": "add",
        "kind": "concept",
        "name": "검증 기록 (Verification Record)",
        "evidence_finding_ids": [
          "f9",
          "f6",
          "f18"
        ],
        "description": "LLM 이 낸 해석·분해·재계획 제안이 결정적 검사(슬롯 검사, 계획기·기호 검증, 안전 게이트)를 통과했는지와 반영 시각을 남기는 기록. 관리형 블랙보드의 검증 기록(f9)에 해당한다. 배정 속성 '확인 여부'와 6절 질문 '사용자 확인을 별도 개념으로 둘지'(q4-01·q4-04)와 겹칠 수 있어, 사람 확인까지 포함할지는 검증이 판단한다."
      }
    ],
...
### runs/2026-09-25-71/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-376 mdBook 원본(raw.githubusercontent.com/osrf/ros2multirobotbook/master/src/task.md)을 열람해 BidNotice→TaskPlanner 기반 BidProposal→설정 기준(가장 빨리 끝남·가장 낮은 비용) 비교→DispatchRequest, ChargeBattery 자동 삽입을 확인했다. 인용 구절도 원문과 같다. 단일 출처. flow_step '피킹'은 출처에 없는 이 위키의 배치다."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 실행 2026-09-25-66 에서 검증한 ref-377·ref-404 의 재인용이며 이번 실행에서는 다시 열지 않았다(원문 미열람). 같은 발행 주체(open-rmf) 문서 쌍이라 독립 교차가 아니다."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 실행 2026-09-25-51 에서 검증한 ref-111 task_state.json 의 재인용(이번 실행 원문 미열람). '진행 관리 단계가 받을 결정적 상태 기록의 형식이 된다'는 뒷부분은 이 위키의 해석이다."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): Electronics 2026, 15(16), 3562, 2026-08-11 게재. 의미 모델·선언적+절차적 혼합 추론·다축 능력 조건·적재 상태 장소 도달 가능성·배정기 독립 ReasonerOutput 이 검색 요약에 나타난다. ReasonerOutput 필드 구성과 저자는 미확인."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 기존 페이지(13. 작업 배정 — MRTA, 아이디어 2)에 검증된 ref-166 재인용. 원문 미열람."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): arXiv 2606.08214(2026-06-06), LLM 은 언어 이해·맥락 추론만, 검증·순서·실행은 결정적, SDI 구조와 기호 검증기 Inspector, 5개 난이도 70개 명령 100% 성공(저자 보고). LangGraph 기반 동적 경로 설정·Unity3D 디지털 트윈 세부는 이번 검색 요약에서 재확인되지 않았다."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): 98.1%→3.8%, 그룹 C·D 0% 는 저자 보고값이다. 검색 요약은 이 절제 실험이 그룹 A–D 의 52개 명령 부분집합에서 수행됐다고 적는다 — 조건 병기 필요. 단일 출처 핵심 수치이므로 '저자 보고값·독립 재현 미확인' 표기를 유지한다."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): arXiv 2606.31339(2026-06-30), 작업 숲·관리형 블랙보드(실행 상태·로봇 기록·자원 잠금·세계 믿음·제안·검증 기록), 결정적 검증과 원자적 상태 갱신 뒤에만 반영. 다만 원문 요약은 제안 주체를 '에이전트·휴리스틱·최적화 모듈'로 적어, 제안 주체를 LLM 으로만 좁힌 표현은 고쳐야 한다. 평가는 실내 공장 시나리오·원격 건설 벤치마크."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): 22개 시나리오·54개 작업·약 20대 로봇 벤치마크에서 정답률(correctness) 최대 0.91±0.053(저자 보고). 지표 이름은 '정확도'가 아니라 정답률(correctness)이다. evidence_excerpt 의 '재계획 세트 5개 시나리오 1.0' 은 검색 요약(20개 시나리오 재계획 벤치마크 0.948)과 맞지 않아 쓰지 않는다. Frontiers in Robotics and AI 2026 게재판이 있다."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 실행 2026-09-25-37 에서 검증한 SafeGate(ref-417) 재인용, 원문 미열람. ISO 13482 는 개인 돌봄 로봇 안전 표준이므로 물류 이동로봇 적용은 미확인으로 둔다."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-679 README 를 raw.githubusercontent.com 으로 열람했다. rosbridge 사용, 로봇 소스 코드 변경 불필요, 토픽 발행·구독·서비스·액션 호출·파라미터 설정, Apache 2.0, 권한(permissions)은 기여 안내의 새 기능 예시로만 언급됨을 확인. 발행일 미확인(확인일 2026-09-25 기준)."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 기존 아이디어 2 페이지의 검증된 문장과 같은 ref-180 재인용, 원문 미열람. 학술대회 이름·일자 미확인."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 기존 검증된 ref-356(Rasa forms.mdx) 재인용, 이번 실행 원문 미열람. '결정적 구성 요소의 예가 된다'는 뒷부분은 이 위키의 해석이다."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): Springer Intelligent Service Robotics 게재, 구조화 프롬프트로 실행 언어의 배정·계획 생성, 새 로봇 추가 시에도 높은 실행 정확도(저자 보고, 수치 미확인). 검색 결과는 게재 연도를 2025 로 적으나 브리프가 요약마다 다르다고 해 발행일은 미확인으로 둔다. 저자 소속 미확인."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 기존 페이지에 검증된 ref-586 재인용(원문 미열람). 저자 입장이므로 [의견] 유지."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 이 위키의 종합 [추정]. 여섯 단계 흐름을 한 번에 제시한 단일 출처가 없음을 브리프가 밝혔다. (2) 작업 분해 근거 중 SPCA(f8)는 강등됐으므로 f8 을 인용할 때 추정 근거임을 유지한다."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 이 위키의 대응 추론 [추정]. f8 강등과 f9 의 제안 주체 정정을 반영해 서술해야 한다."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: [추정]. CoMuRoS 수치는 정답률(correctness) 최대 0.91 로 표기한다."
    },
    {
      "finding_id": "f20",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ROS-MCP-Server 원문(열람)과 KETI(미열람)에 기댄 이 위키의 경계 판단 [추정]. 로봇 토픽·액션 직접 제어는 분류 원문 9장 '로봇 자체 지능·제어' 연계 영역이므로 ROP 직접 범위처럼 쓰지 않는다."
    },
    {
      "finding_id": "f21",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 분류 원문 SCM 질문과의 연결 추론 [추정]. f1(열람 확인)·f4 에 기댄다."
    },
    {
      "finding_id": "f22",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 수치 없는 설명용 가정 사례 [추정]. 가상임을 본문에 밝히면 드리프트가 아니다."
    },
    {
      "finding_id": "f23",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 검색 범위 관찰 [추정]. f9 의 평가는 실내 공장·원격 건설, f10 은 실험실·하드웨어 시연이 확인되어 서술과 일치한다. 부재의 확인이 아님을 유지한다."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "scope_boundary": {
    "ok": true,
    "issues": [
      "f12·f20: ROS 토픽·서비스·액션 직접 제어는 분류 원문 9장 '로봇 자체 지능·제어' 연계 영역이다. 26. 사이버보안·접근권한·개인정보 반영 제안과 단계 3 페이지에서 ROP 가 저수준 로봇 도구를 운영하는 것처럼 쓰지 않고, ROP 는 상위 도구 노출 경계만 판단한다는 [추정]으로 둔다.",
      "f6·f8·f9: 산업용 로봇 셀·조작·공장/원격 건설 대상 연구이므로 물류 적용은 미확인으로 명시한다(원격 건설 벤치마크는 업종별 조건 연계 대상).",
      "f11: SafeGate 의 ISO 13482 는 개인 돌봄 로봇 안전 표준이므로 물류 이동로봇 적용은 미확인이며, 안전 판정 자체를 ROP 직접 범위로 단정하지 않는다."
    ]
  },
  "duplication": {
    "ok": true,
    "overlaps": [
      "f1 은 13. 작업 배정 — MRTA 5절·7절과 단계 3 페이지 q3-01 의 기존 검증 문장(Open-RMF 입찰, ref-376)과 같은 내용이다 — 새 각주를 만들지 말고 ref-376 을 재사용하며 반복 서술은 링크로 줄인다.",
      "f2(rmf_task 작업 계획기)는 단계 3 q3-01 절과 업무 분해·배정 설계 초안의 일정 개념 문장과 같다 — ref-377·ref-404 재사용.",
      "f3(작업 상태 스키마)는 단계 2 q2-02 와 초안 진행 상태·배정 개념의 기존 문장과 같다 — ref-111 재사용.",
      "f5·f13·f14·f16 은 아이디어 2 페이지·초안의 기존 검증 문장과 같다 — 기존 각주 재사용.",
      "새 id ref-674~ref-680 은 실행 2026-09-25-66 브리프가 다른 출처(Kambhampati, ConstraintBench 등, 이후 ref-586·ref-592 등으로 합쳐짐)에 쓴 번호와 같다. 퍼블리셔가 URL 기준으로 합치므로 모순은 아니지만, 참고문헌 전체 목록(637건)에서 ref-674~ref-680 이 이미 다른 URL 에 배정돼 있는지 퍼블리셔 확인이 필요하다.",
      "새 질문 'ROP 가 온톨로지 기반 판정으로 후보를 거른 뒤 플릿 단위 입찰…'은 열린 질문 oq-053(두 수준 배정의 최적성 손실)과 관련된다 — 중복은 아니나 백로그 항목에 '(관련: oq-053)'을 병기한다.",
      "새 질문 '채팅 LLM 에 노출할 도구를 상위 도구로 한정…'은 백로그 q4-03(채팅 사용자별 명령 권한)과 관련된다 — 중복은 아니나 '(관련: q4-03)'을 병기한다."
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "required_fixes": [
    ...
    "모든 각주 정의 가운데 이번 실행에서 원문을 열지 못한 출처(ref-377, ref-404, ref-111, ref-236, ref-166, ref-417, ref-180, ref-356, ref-586, ref-674, ref-675, ref-677, ref-678, ref-680)의 접근일 뒤에 ' (원문 미열람)'을 붙이고, reference_updates 의 해당 신규 항목(ref-674, ref-675, ref-677, ref-678, ref-680)에 source_unopened: true 를 넣는다. 원문을 연 ref-376·ref-676·ref-679 는 표기하지 않는다.",
...
### docs/references/index.md (요약: 이번 대상 페이지가 인용한 127건 / 전체 650건. 목록에 없는 출처는 새 id 로 적는다 — 같은 URL 이 이미 있으면 퍼블리셔가 기존 id 로 합친다)
...
### docs/open-questions.md (요약: 대상 영역 [1, 2, 5, 6, 8, 12, 13, 14, 16, 18, 19, 20, 23, 25, 26, 27] 에 걸린 88건 / 전체 112건)
...
### runs/2026-09-25-71/docs_tree.txt
...
| [단계 4. 오해석 방지와 확인 절차](stage-4-misinterpretation-safeguards.md) | 대기 | 8 | 미충족 |
...
<!-- auto:page-status:start -->
> 초안 버전: v0.6 · 페이지 상태: published · 신뢰도: low · 페이지 버전: 7 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
원천: [질문 백로그](../tracks/nl-task-chatbot/question-backlog.md)([자연어 업무 지시 챗봇](../tracks/nl-task-chatbot/index.md) 트랙) · 열림 30건 · 답함 8건
...
````json
{
  ...
  "outline": [
    {
      ...
      "section": "2. 질문 목록",
      "budget_chars": 900,
      "summary": "q3-02 를 답함(실행 2026-09-25-71, #q3-02)으로 바꾸고 이 단계 태그의 새 질문 q3-12 를 더한다."
    },
    {
      "path": "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
      "section": "3. 조사 결과",
      "budget_chars": 6500,
      "summary": "q3-02: 처리 흐름을 지시 해석·작업 분해·능력 질의·배정·스케줄링·진행 관리로 나누고, LLM 은 해석·분해 제안과 설명을, 결정적 구성 요소는 분해 검사와 나머지 단계를 맡는 배치가 근거가 가장 많은 것으로 보인다. [추정][^ref-674]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 2200,
      "summary": "q3-02 결론(신뢰도 low)과 검증 게이트·도구 노출 경계, 초안 v0.7 반영 내역, 남은 불확실성을 정리한다."
    },
    {
      "path": "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
      "section": "5. 이 단계가 낳은 후속 질문",
      "budget_chars": 1200,
      "summary": "q3-12(f4, 관련 oq-053), q4-09(f20, 관련 q4-03), q5-07(f7) 를 더한다."
    },
    {
      "path": "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
      "section": "6. 완료 조건 충족 현황",
      "budget_chars": 700,
      "summary": "처리 흐름은 실렸으나 다른 아이디어와의 연결·실험 계획이 없어 미충족, 전환 아니오."
    },
    {
      "path": "docs/ideas/nl-task-chatbot.md",
      "section": "5. 구현 가설",
      "budget_chars": 3500,
      "summary": "처리 흐름과 핵심 구성 요소 소절을 신설한다. [추정][^ref-674]",
      "planned_findings": [
        "f1",
        "f3",
        "f4",
    ...
    },
    {
      ...
      "section": "2. 개념 목록 표",
      "budget_chars": 400,
      "summary": "배정 개념의 배정 산출 방식 값 후보에 '입찰 비교'를 더한다(v0.7). [사실][^ref-376]",
      "planned_findings": [
        "f1"
      ]
    },
    {
      ...
      "budget_chars": 900,
      "summary": "반영되지 않은 실행 가능성 판정·검증 기록 개념 제안을 질문으로 둔다."
    }
  ],
  "pages": [
    {
    ...
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "5절에 '처리 흐름과 핵심 구성 요소' 소절 신설(q3-02, 실행 2026-09-25-71), 절 안내 문장 갱신, 5절 각주 정의에 새 출처 추가·원문 미열람 표기 보완",
      "patches": [
        {
          "section": "5. 구현 가설",
          "action": "replace",
    ...
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "초안 v0.6 → v0.7: 배정 개념의 배정 산출 방식에 값 후보 '입찰 비교' 추가(f1, 상태 확정 유지), 1절 버전 설명 추가, 6절에 거부된 제안(실행 가능성 판정·검증 기록) 질문 2건 추가. 참고: H1 '(v0.6)' 은 절 밖이라 패치하지 못함 — '(v0.7)' 로 맞춰야 함",
      "patches": [
        {
          "section": "1. 목적과 범위",
          "action": "replace",
          ...
          "content": "이 페이지는 중점 연구 트랙 [자연어 업무 지시 챗봇](index.md)의 살아있는 산출물이다. 사용자가 채팅으로 준 지시가 어떤 단위로 파악·분해되고, 어떤 작업 요구를 거쳐 로봇에 배정·배치되며, 진행과 일정이 어떻게 관리되는지를 하나의 작업 모델로 표현하는 것이 목적이다. 이 작업 모델은 챗봇(LLM)이 내놓는 해석 결과의 형식이자, 온톨로지 질의와 최적화 엔진이 받는 입력의 형식이 된다. [가정]\n\nv0은 확장 아이디어 2의 정의 문구(아래 인용)에 나오는 요소만으로 시드했다. 개념과 관계의 정의 문장은 구축자가 그 문구에서 도출한 것이므로 모두 \"아이디어 정의 기반 [가정]\"으로 표기했다. 출처 finding이 없는 개념·관계는 더 넣지 않는다. 이후 트랙 실행에서 리서치 에이전트가 근거 finding id와 함께 변경을 제안하고, 내용 검증 에이전트가 승인한 변경만 스토리텔러 에이전트가 반영하며 그때 초안 버전을 올린다. v0.1(실행 2026-09-25-04)에서는 검증이 승인한 개념 1개(로봇 팀)를 더했고, 승인되지 않은 제안 3건은 6절의 질문으로 두었다. v0.2(실행 2026-09-25-21)에서는 검증이 승인한 배정 개념의 수정(속성 '배정 산출 방식' 추가, 상태 초안 → 확정)을 반영했고, 같은 제안 가운데 값 후보 '규칙'은 근거 finding이 없어 6절의 질문으로 두었다. v0.3(실행 2026-09-25-30)에서는 검증이 승인한 상황 개념의 수정(속성 '값 출처' 추가, 상태 초안 → 확정)을 반영했고, 상황의 시간 조건에 모호한 시간 표현을 담는 제안은 일정 개념과 겹쳐 6절의 질문으로 두었다. v0.4(실행 2026-09-25-37)에서는 검증이 승인한 상황 개념의 수정(장소 표현에 해석 결과 '공간 노드 참조'를 짝으로 추가, 상태 확정 유지)과 업무 개념의 수정(기한·우선순위의 값 원천 후보와 로봇 인터페이스의 기한 필드 부재 메모, 상태 초안 → 확정)을 반영했다. 작업 요구에 적재물 식별과 적재물 유형·치수·중량을 더하는 제안은 능력 온톨로지 초안의 작업 요구와 충돌하는지 확인하지 못해 검증이 거부했고, 6절의 질문으로 두었다. v0.5(실행 2026-09-25-51)에서는 검증이 승인한 진행 상태 개념의 수정(상태 값·갱신 시각의 외부 표현 원천 메모, 상태 초안 → 확정)과 배정 개념의 수정(배정 결과의 외부 표현 대응 메모, 상태 확정 유지)을 반영했다. 외부 형식에 선택 근거·배정 산출 방식·확인 여부 필드가 확인되지 않았다는 관찰은 속성 정의가 아니라 추정 메모로만 적었고, 진행 상태 값의 대응 규칙, 플릿 사이 작업 선행 의존, IEEE 1872.1-2024 작업 개념과의 대응은 6절의 질문으로 두었다. v0.6(실행 2026-09-25-66)에서는 검증이 승인한 일정 개념의 수정(속성 '일정 산출 방식' 추가, 상태 초안 → 확정)을 반영했고, 같은 제안 가운데 값 후보 'LLM 직접 생성'은 근거 finding 이 지정되지 않아 6절의 질문으로 두었다. v0.7(실행 2026-09-25-71)에서는 검증이 승인한 배정 개념의 수정(속성 '배정 산출 방식'에 값 후보 '입찰 비교' 추가, 상태 확정 유지)을 반영했고, 같은 실행에서 제안된 개념 '실행 가능성 판정'과 '검증 기록'의 추가는 검증이 반영하지 않아 6절의 질문으로 두었다.\n\n> 사용자가 채팅으로 상황과 처리할 일을 입력하면 AI가 업무를 파악·분해하고, 온톨로지로 적합한 로봇을 찾아 배정·배치한 뒤 작업 진행과 스케줄링을 자동으로 관리\n\n초안 버전(프런트매터 `ontology_version`)은 페이지 버전(`version`)과 별개다. 키 이름은 첫 트랙의 온톨로지 초안과 같게 두어 파이프라인이 같은 방식으로 버전을 대조한다. [가정] 이 초안과 다른 아이디어의 공통 데이터 모델(공간 노드, 공용 자원, 로봇 능력, 작업)의 관계는 [확장 아이디어 연결 구조](../../ideas/index.md)에 있다."
        },
        {
          "section": "2. 개념 목록 표",
          "action": "replace",
    ...
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "6절 살아있는 산출물: 초안 현재 버전 v0.7(v0.6·v0.7 내역), 아이디어 2 5절 작성 현황 갱신. 참고: H1 아래 트랙 상태 줄('현재 단계: 단계 1')은 절 밖이라 패치하지 못함 — '현재 단계: 단계 3. 구현 가설 설계 · 마지막 트랙 실행: 2026-09-25' 로 맞춰야 함",
      "patches": [
        {
          "section": "6. 살아있는 산출물 링크",
          "action": "replace",
          "content": "- [업무 분해·배정 설계 초안](task-model-draft.md) — 현재 버전 v0.7. 아이디어 정의에서 도출한 v0 개념 10개·관계 9개에 검증을 거친 개념 로봇 팀을 더했고(v0.1, 실행 2026-09-25-04), 배정 개념에 속성 '배정 산출 방식'을 더해 확정했으며(v0.2, 실행 2026-09-25-21), 상황 개념에 속성 '값 출처'를 더해 확정했고(v0.3, 실행 2026-09-25-30), 상황의 장소 표현에 해석 결과 '공간 노드 참조'를 짝으로 더하고 업무 개념의 기한·우선순위 값 원천을 정리해 확정했으며(v0.4, 실행 2026-09-25-37), 진행 상태 개념에 외부 표현 원천 메모를 더해 확정하고 배정 개념에 외부 표현 대응 메모를 더했다(v0.5, 실행 2026-09-25-51). 일정 개념에 속성 '일정 산출 방식'을 더해 확정했고(v0.6, 실행 2026-09-25-66), 배정 개념의 배정 산출 방식에 값 후보 '입찰 비교'를 더했다(v0.7, 실행 2026-09-25-71). 실행 2026-09-25-26 과 2026-09-25-62 에서는 변경이 없었다. 트랙 실행이 근거 finding과 함께 갱신한다.\n- [아이디어 2. 자연어 업무 지시 챗봇](../../ideas/nl-task-chatbot.md) — 확장 아이디어 페이지. 3~6절(선행 연구·제품 사례, 필요한 데이터와 표준, 구현 가설, 검증 방법)을 이 트랙의 단계 1·2·3·4·5 실행이 채운다. 3절은 선행 연구와 제품 사례(벤더 주장 수준), 채팅·음성 지시 제품의 확인·승인 방식 비교(q1-03, 실행 2026-09-25-26)에 더해 상황 정보 추출과 되묻기(q1-04, 실행 2026-09-25-30)가 작성되었다. 로봇에 자연어로 일을 지시하는 제품의 해석 결과 확인·승인 절차와 물류 지시를 대상으로 한 추출·되묻기 연구는 공개 자료·검색 범위에서 확인되지 않았다(부재의 확인은 아님). 4절은 필요한 데이터 항목과 원천(q2-01, 실행 2026-09-25-37), 작업·배정 결과를 표현하는 표준·형식 비교(q2-02, 실행 2026-09-25-51), 해석·분해 평가 데이터(q2-03, 실행 2026-09-25-62)가 작성되었다. 물류 창고 지시를 정답과 짝지은 공개 데이터셋은 검색 범위에서 찾지 못했다(부재의 확인은 아님). 5절은 스케줄링 결정의 분담(q3-01, 실행 2026-09-25-66)과 처리 흐름·핵심 구성 요소(q3-02, 실행 2026-09-25-71)가 작성되었다.\n- [질문 백로그](question-backlog.md) — 최신 수치는 백로그 페이지의 자동 표를 따른다.\n- [트랙 로그](log.md) — 실행별 기록\n- [실험](experiments.md) — 제안된 실험 계획과 사용자 실험 결과 요약(선택). 현재 제안된 실험은 없다."
        }
      ]
    }
  ],
  "changelog_entry": "2026-09-25 | 자연어 업무 지시 챗봇 단계 3 | q3-02 답함(처리 흐름 여섯 단계의 입력·출력과 결정적 구성 요소·검증 게이트 배치, 신뢰도 low), 업무 분해·배정 설계 초안 v0.6 → v0.7(배정 산출 방식 '입찰 비교'), 후속 질문 3건, 1차 수정 지시 15건 이행 | run 2026-09-25-71",
  "index_updates": {
    "home_recent": "2026-09-25 — 자연어 업무 지시 챗봇 단계 3: 지시 해석부터 진행 관리까지 여섯 단계의 입력·출력과, LLM 제안 뒤마다 결정적 검증을 두는 배치를 정리(q3-02, 추정·신뢰도 low)",
    "category_recent": "2026-09-25 — 13. 작업 배정 — MRTA(자연어 업무 지시 챗봇 트랙 단계 3): 능력 판정을 배정기 독립 입력으로 넘기고 입찰 비교·최적화가 배정하는 처리 흐름 가설과 반영 제안"
  },
  "glossary_updates": [
    {
      "action": "new",
      ...
      "term_en": "Model Context Protocol (MCP)",
      "definition": "LLM 이 외부 시스템의 상태를 조회하고 정해진 도구로 동작을 수행하도록 도구·자원을 구조화해 노출하는 개방형 연결 규약이다.",
      "description": "로봇 분야에서는 ROS-MCP-Server 처럼 ROS·ROS 2 의 토픽·서비스·액션을 LLM 도구로 노출하는 구현이 있으며, 노출한 도구 목록이 LLM 이 닿는 범위를 정한다.",
      "related_areas": [
        27,
        26,
        18
    ...
    },
    {
      ...
      "term_en": "Neuro-symbolic AI",
      "definition": "LLM 같은 신경망 모델의 유연한 해석·생성과 계획기·검증기 같은 기호적(규칙·논리 기반) 구성 요소의 결정적 검사를 결합하는 AI 구성 방식이다.",
      "description": "로봇 계획에서는 LLM 이 언어 이해·제안을 맡고 기호 검증기·계획기가 검증·순서·실행을 맡는 구조(예: Specifier–Designer–Inspector)로 쓰인다.",
      "related_areas": [
        27,
        13
      ],
  ...
  "reference_updates": [
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 은 언어 이해·맥락 추론만, 검증·순서·실행은 결정적으로 두는 Specifier–Designer–Inspector 구조와 기호 검증기 절제 실험(98.1%→3.8%, 52개 명령 부분집합, 저자 보고).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/ideas/nl-task-chatbot.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. Machine Learning and Knowledge Extraction 8(1) 22. Sense–Plan–Code–Act 틀의 하이브리드 구성(LLM→PDDL→계획기→LLM 코드 생성, 컴파일·시뮬레이션 검증)을 논문 요약 기준으로 서술.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "SPCA 네 단계의 입력·출력과 Plan 단계의 PDDL·LLM·하이브리드 구성, 컴파일·시뮬레이션 검증, 논문 서지(MAKE 2026, doi 10.3390/make8010022)를 적은 저장소 README.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 작업 숲과 관리형 블랙보드를 두고 에이전트·휴리스틱·최적화 모듈의 제안을 결정적 검증·원자적 반영으로만 받아들이는 산업용 다중 로봇 임무 상태 관리 구조(평가: 실내 공장, 원격 건설).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/ideas/nl-task-chatbot.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 작업 관리자 LLM 이 해석·분류·배정, 로봇별 로컬 LLM 이 코드 구성, 사건 기반 재계획을 하는 CoMuRoS 구조와 정답률 최대 0.91(22개 시나리오·54개 작업·약 20대 로봇, 저자 보고).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. Intelligent Service Robotics 게재. 구조화 프롬프트로 LLM 이 이종 로봇의 배정·기술 계획을 실행 언어로 생성하는 틀(게재 연도·수치 미확인, 저자 보고).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/ideas/nl-task-chatbot.md"
  ...
  "flow_matrix_updates": [
    {
      "step": "피킹",
      "item": "시작 조건",
      "link": "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md#q3-02",
      "title": "단계 3. 구현 가설 설계"
    },
    {
      "step": "피킹",
  ...
  "area_reflection_proposals": [
    ...
    },
    {
      "area_no": 27,
      "section": "6. 대표 접근법과 기술",
      "summary": "트랙 단계 3(실행 2026-09-25-71): 신경-기호 구조 — SDI(LLM 은 언어 이해만, 검증·순서·실행은 결정적, ref-674), SPCA(Plan 단계 PDDL·LLM·하이브리드 수용, 하이브리드 파이프라인은 [추정], ref-675·ref-676), 관리형 블랙보드(에이전트·휴리스틱·최적화 모듈 제안을 결정적 검증·원자적 반영으로만 수용, ref-677), LLM 출력 반영 직전 단계 경계마다 검증 게이트를 두는 배치([추정]). 적용 대상 13. 작업 배정 — MRTA 와 함께 연결한다."
    },
    {
      "area_no": 27,
      "section": "8. 대표 연구와 자료",
      "summary": "SDI 기호 검증기 절제 실험: 검증기를 같은 방식으로 프롬프트한 LLM 으로 바꾸면 성공률 98.1%→3.8%(그룹 A–D 52개 명령 부분집합, 저자 보고값, 독립 재현 미확인, 원문 미열람, ref-674)."
    },
    {
      "area_no": 26,
      "section": "6. 대표 접근법과 기술",
      "summary": "LLM 에 로봇을 도구로 노출하는 방식: ROS-MCP-Server 는 rosbridge 로 토픽·서비스·액션·파라미터를 MCP 도구로 노출하고 README 에 현재 권한 장치 설명이 없다(ref-679, 확인일 2026-09-25), 국내 KETI 연구는 LangChain 도구를 ROS 2 인터페이스로 정의했다(ref-180). 로봇 토픽·액션 직접 제어는 분류 원문 9장 '로봇 자체 지능·제어' 경계의 연계 대상이며, ROP 는 검증 파이프라인으로 들어가는 상위 도구만 노출해야 할 것으로 보인다는 판단은 [추정]으로만 둔다."
    },
    {
      "area_no": 5,
      "section": "10. 다른 연구영역과의 연결 (번호와 이름을 함께 표기)",
      "summary": "온톨로지 기반 실행 가능성 판정(다축 능력 조건·적재 상태 도달 가능성) 결과를 배정기 독립 출력(ReasonerOutput)으로 13. 작업 배정 — MRTA 에 넘기는 연결(Electronics 2026, ref-236, 원문 미열람)."
    }
  ],
  "track_updates": {
...
### runs/2026-09-25-71/pages/ideas/nl-task-chatbot.md
...
updated: 2026-09-25
version: 10
sources: [ref-054, ref-055, ref-057, ref-058, ref-059, ref-061, ref-089, ref-090, ref-091, ref-093, ref-094, ref-095, ref-087, ref-164, ref-166, ref-167, ref-168, ref-169, ref-170, ref-171, ref-172, ref-174, ref-175, ref-176, ref-177, ref-178, ref-179, ref-180, ref-181, ref-242, ref-272, ref-275, ref-276, ref-277, ref-278, ref-279, ref-280, ref-350, ref-351, ref-352, ref-353, ref-354, ref-355, ref-356, ref-357, ref-358, ref-359, ref-360, ref-362, ref-015, ref-031, ref-125, ref-130, ref-228, ref-411, ref-413, ref-418, ref-111, ref-495, ref-230, ref-496, ref-500, ref-501, ref-502, ref-116, ref-504, ref-539, ref-540, ref-541, ref-542, ref-543, ref-544, ref-545, ref-546, ref-547, ref-548, ref-056, ref-404, ref-377, ref-592, ref-593, ref-594, ref-595, ref-596, ref-598, ref-611, ref-612, ref-615, ref-616, ref-376, ref-236, ref-417, ref-586, ref-674, ref-675, ref-676, ref-677, ref-678, ref-679, ref-680]
confidence: low
last_run: 2026-09-25
---
...
### runs/2026-09-25-71/pages/tracks/nl-task-chatbot/task-model-draft.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-054, ref-055, ref-059, ref-089, ref-090, ref-091, ref-093, ref-125, ref-130, ref-166, ref-167, ref-169, ref-181, ref-242, ref-350, ref-352, ref-356, ref-357, ref-358, ref-359, ref-361, ref-411, ref-412, ref-413, ref-414, ref-111, ref-495, ref-230, ref-496, ref-501, ref-502, ref-504, ref-404, ref-377, ref-596, ref-597, ref-611, ref-612, ref-592, ref-594, ref-595, ref-598, ref-615, ref-616, ref-376, ref-236, ref-674, ref-677]
last_run: 2026-09-25
version: 8
---
...
### runs/2026-09-25-71/pages/tracks/nl-task-chatbot/index.md
...
### runs/2026-09-25-71/verification2.json
...
```json
{
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "1차에서 퍼블리셔 확인을 요청한 신규 id ref-674~ref-680 은 docs_tree.txt 의 참고문헌 목록(ref-001~ref-616, ref-640~ref-643)에 없어 기존 배정과 겹치지 않는다."
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md H1 아래 상태 줄: '답한 질문: 1건'을 '답한 질문: 2건'으로 고친다. 이유: 2절 표에서 q3-01·q3-02 가 답함이다. 열린 질문 10건(q3-03~q3-12)은 맞으므로 그대로 둔다. H2 절 밖의 줄이라 patches 로 고칠 수 없다면 이 줄을 담은 페이지 전문을 보내거나, 파이프라인 담당에게 머리 줄 패치 경로를 요청한다.",
    "docs/tracks/nl-task-chatbot/task-model-draft.md H1 '업무 분해·배정 설계 초안 (v0.6)'을 '(v0.7)'로 고친다. 이유: 프런트매터 ontology_version '0.7'과 track_updates.ontology_draft_version '0.7'과 같아야 한다. 절 밖이라 patches 로 고칠 수 없다면 위와 같은 방식으로 보낸다.",
    "stage-3-implementation-hypothesis.md 6절 표 아래 전환 줄을 '다음 단계로 전환: 아니오(아이디어 2 5절 다른 아이디어와의 연결 없음, 실험 계획 없음, 열린 질문 q3-03~q3-12)'로 고친다. 이유: 이번 실행에서 q3-12 가 열림으로 추가됐다. 첫 행 완료 조건도 미충족이다. log_entry 의 완료 조건 평가와도 맞춰야 한다."
  ],
  "confidence": "low",
  "verification_note": "판정: 1차 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(raw.githubusercontent.com 미러만 열림). 확인 22건, 미확인 1건, 교차 확인 0건. 강등: f8 사실 → 추정. SPCA 는 여러 구성을 수용하는 틀이며, 제시한 파이프라인은 원문 미열람 논문 요약에 기댄 것이다. 원문 미열람 출처: ref-377, ref-404, ref-111, ref-236, ref-166, ref-417, ref-180, ref-356, ref-586, ref-674, ref-675, ref-677, ref-678, ref-680. 원문 열람 확인: ref-376, ref-676, ref-679. 주의: q3-02 의 여섯 단계 입력·출력과 결정적 구성 요소 배치는 이 위키의 종합(추정)이다. 근거는 산업용 로봇 셀·조작 시뮬레이션·공장·실험실 조건이다. 기호 검증기 절제 실험(98.1%→3.8%)은 52개 명령 부분집합 조건의 저자 보고값이다. 온톨로지 변경 승인: 배정 (Assignment) 속성 '배정 산출 방식'에 값 후보 '입찰 비교' 추가(f1). 거부: 실행 가능성 판정(f4·f17), 검증 기록(f9·f6·f18) — 두 개념 모두 초안 6절 질문으로 반영됐다. 단계 완료 조건: 미충족. 부족한 것은 아이디어 2 5절의 다른 아이디어와의 연결과 실험 계획이다. 단계 전환: 미승인. 막힌 질문 q3-03~q3-12 가 열려 있다. 백로그 중복 정리 필요: q3-09·q3-10, q5-05·q5-06, q1-05·q1-06. / 2차 수정 후 재검증. 드리프트 없음. [분류원문] 보존, 섹션 순서 준수, 링크 유효. 1차 수정 지시 15건은 이행됐다. 남은 문제는 절 밖 머리 줄 두 곳(단계 3 상태 줄의 답한 질문 수, 초안 H1 버전)과 6절 전환 줄의 질문 범위다. 참고 1: 스토리텔러는 트랙 개요 상태 줄을 '현재 단계: 단계 3'으로 바꾸자고 제안했다. 그러나 config/tracks/nl-task-chatbot.yaml 의 current_stage 가 1이고, 같은 페이지의 자동 진행 표도 단계 1 진행 중·단계 3 대기로 표시한다. 그래서 수정 지시에 넣지 않았다. 이번 실행은 CLI 로 단계 3 을 지정해 강제 실행했으므로, 공식 현재 단계를 어떻게 둘지는 사용자가 결정할 사항이다. 참고 2: 1차 지시에 따라 ref-377·ref-404·ref-111·ref-356 에 '(원문 미열람)'을 붙였다. 이는 이번 실행에서 원문을 다시 열지 않았다는 뜻이다. 참고문헌 목록에 있는 이전 실행의 원문 열람 기록(예)을 퍼블리셔가 아니오로 덮어쓰지 않아야 한다. 이번에 패치하지 않은 절의 기존 각주 정의는 차등 갱신 규칙(R-4)에 따라 검사 대상에서 뺐다.",
  "retry_reason": null,
  "track_checks": {
    "standard_sources_ok": true,
    "vendor_claims_tagged": true,
...
- 수정 지시(required_fixes):
    ...
    - docs/tracks/nl-task-chatbot/task-model-draft.md H1 '업무 분해·배정 설계 초안 (v0.6)'을 '(v0.7)'로 고친다. 이유: 프런트매터 ontology_version '0.7'과 track_updates.ontology_draft_version '0.7'과 같아야 한다. 절 밖이라 patches 로 고칠 수 없다면 위와 같은 방식으로 보낸다.
    - stage-3-implementation-hypothesis.md 6절 표 아래 전환 줄을 '다음 단계로 전환: 아니오(아이디어 2 5절 다른 아이디어와의 연결 없음, 실험 계획 없음, 열린 질문 q3-03~q3-12)'로 고친다. 이유: 이번 실행에서 q3-12 가 열림으로 추가됐다. 첫 행 완료 조건도 미충족이다. log_entry 의 완료 조건 평가와도 맞춰야 한다.
- 검증 노트: 판정: 1차 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(raw.githubusercontent.com 미러만 열림). 확인 22건, 미확인 1건, 교차 확인 0건. 강등: f8 사실 → 추정. SPCA 는 여러 구성을 수용하는 틀이며, 제시한 파이프라인은 원문 미열람 논문 요약에 기댄 것이다. 원문 미열람 출처: ref-377, ref-404, ref-111, ref-236, ref-166, ref-417, ref-180, ref-356, ref-586, ref-674, ref-675, ref-677, ref-678, ref-680. 원문 열람 확인: ref-376, ref-676, ref-679. 주의: q3-02 의 여섯 단계 입력·출력과 결정적 구성 요소 배치는 이 위키의 종합(추정)이다. 근거는 산업용 로봇 셀·조작 시뮬레이션·공장·실험실 조건이다. 기호 검증기 절제 실험(98.1%→3.8%)은 52개 명령 부분집합 조건의 저자 보고값이다. 온톨로지 변경 승인: 배정 (Assignment) 속성 '배정 산출 방식'에 값 후보 '입찰 비교' 추가(f1). 거부: 실행 가능성 판정(f4·f17), 검증 기록(f9·f6·f18) — 두 개념 모두 초안 6절 질문으로 반영됐다. 단계 완료 조건: 미충족. 부족한 것은 아이디어 2 5절의 다른 아이디어와의 연결과 실험 계획이다. 단계 전환: 미승인. 막힌 질문 q3-03~q3-12 가 열려 있다. 백로그 중복 정리 필요: q3-09·q3-10, q5-05·q5-06, q1-05·q1-06. / 2차 수정 후 재검증. 드리프트 없음. [분류원문] 보존, 섹션 순서 준수, 링크 유효. 1차 수정 지시 15건은 이행됐다. 남은 문제는 절 밖 머리 줄 두 곳(단계 3 상태 줄의 답한 질문 수, 초안 H1 버전)과 6절 전환 줄의 질문 범위다. 참고 1: 스토리텔러는 트랙 개요 상태 줄을 '현재 단계: 단계 3'으로 바꾸자고 제안했다. 그러나 config/tracks/nl-task-chatbot.yaml 의 current_stage 가 1이고, 같은 페이지의 자동 진행 표도 단계 1 진행 중·단계 3 대기로 표시한다. 그래서 수정 지시에 넣지 않았다. 이번 실행은 CLI 로 단계 3 을 지정해 강제 실행했으므로, 공식 현재 단계를 어떻게 둘지는 사용자가 결정할 사항이다. 참고 2: 1차 지시에 따라 ref-377·ref-404·ref-111·ref-356 에 '(원문 미열람)'을 붙였다. 이는 이번 실행에서 원문을 다시 열지 않았다는 뜻이다. 참고문헌 목록에 있는 이전 실행의 원문 열람 기록(예)을 퍼블리셔가 아니오로 덮어쓰지 않아야 한다. 이번에 패치하지 않은 절의 기존 각주 정의는 차등 갱신 규칙(R-4)에 따라 검사 대상에서 뺐다.
