...
- run_id: 2026-09-25-72
- date: 2026-09-25
- run_type: track (트랙 실행)
- 대상: 트랙 floorplan-recognition (건축 도면 자동 인식) · 현재 단계: 단계 4. 지도 변환 보정과 현장 정합 · 이번에 다룰 백로그 질문 id: q4-01 · 중심 세부영역: 6. 지도·공간·위치 모델 (B. 공통 정보·환경 모델)
- 예산:
...
### runs/2026-09-25-72/target.json
...
```json
{
  ...
  "selection_rationale": "CLI 지정 run_type=track, area=6; 트랙 실행일(Fri, track_days 앞 3개) → 트랙 floorplan-recognition 단계 4, 질문 q4-01 (CLI 지정 질문 id)"
}
```

### runs/2026-09-25-72/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q4-01 인식 결과를 로봇 내비게이션 지도로 바꿀 때 필요한 보정은 무엇인가?",
    "로봇 내비게이션 스택과 관제(Nav2 지도 서버, Open-RMF traffic-editor·플릿 어댑터, VDA 5050 3.0.0)는 지도의 좌표·축척·층·좌표계 변환을 어떤 값과 절차로 요구하는가? (단계 4 페이지 3절, 스키마 초안 2절 겨냥)",
    "BIM·CAD 도면에서 만든 점유 격자 지도를 위치추정·주행에 쓴 연구는 어떤 보정(장애물 채움, 센서가 못 보는 요소 제외, 도면–현장 편차)을 요구하고 결과는 어떠했는가? (국내 연구 포함, 한국 자료 우선 규칙)",
    "금지 구역·속도 제한·로봇 크기 여유(인플레이션)처럼 도면에 없는 운영 규칙은 내비게이션 지도에 어떤 층으로 더해지는가? (16. 공용 자원·충전·에너지 최적화, 15. 다중 로봇 경로·교통 관리 — MAPF 연결)",
    "도면 기반 지도를 로봇 쪽 SLAM 지도와 합치거나 계속 갱신하는 도구·연구는 무엇이며, 보정 가운데 무엇이 ROP 직접 범위이고 무엇이 로봇 쪽 연계 대상인가? (9절 경계 겨냥)"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "ROS 2 Nav2 지도 서버는 점유 격자 지도를 이미지와 YAML 메타데이터 한 쌍으로 읽으며, 메타데이터는 이미지 파일, 해상도(셀당 미터), 원점 좌표, 색 반전 여부, 점유·빈 공간 판정 임계값을 담는다.",
      "tag": "사실",
      "source_ids": [
        "ref-440"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README 예시: image, resolution: 0.1, origin: [2.0, 3.0, 1.0], negate: 0, occupied_thresh: 0.65, free_thresh: 0.196. Nav1 의 YAML 지도 형식을 유지하고 nav_msgs/msg/OccupancyGrid 를 지원한다.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f2",
      "claim": "Open-RMF traffic-editor 는 평면도 배경 이미지의 픽셀 좌표(왼쪽 위 원점, +Y 아래 방향)로 편집하고 건물 지도 생성 단계에서 세로축을 뒤집어 직교 좌표로 바꾸며, 두 점 사이 실제 거리(미터)를 넣은 측정선으로 층의 축척을 정하고 층마다 고도(미터)를 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-079"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "측정선의 distance 를 실제 거리(m)로 두면 레벨 Scale 이 갱신된다. GUI 는 base 평면도 이미지의 픽셀 좌표를 쓰고, 지도 생성 단계에서 세로축을 뒤집어 데카르트 평면으로 만든다. 레벨마다 elevation(m). (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f3",
      "claim": "Open-RMF traffic-editor 는 여러 층에서 수직으로 겹칠 것으로 기대되는 기준점(fiducial) 쌍으로 두 층 사이의 이동·회전·축척 변환을 구한다.",
      "tag": "사실",
      "source_ids": [
        "ref-079"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "fiducial 은 두 개 이상 층에서 수직으로 정렬될 것으로 기대되는 위치의 기준 표식이며, 짝을 맞추면 두 층 사이 기하 변환(translation, rotation and scale)을 유도할 수 있다. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f4",
      "claim": "VDA 5050 3.0.0 은 로봇 위치를 관제와 로봇 사이에 정한 프로젝트 고유 좌표계로 주고, 층·구역마다 고유한 mapId 를 쓰며, 지도 좌표계는 z 축이 위를 향하는 오른손 좌표계, 좌표는 미터, 방향은 −π~+π 라디안으로 정한다.",
      "tag": "사실",
      "source_ids": [
        "ref-031"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "6.3 Maps: 위치는 항상 project-specific coordinate system 기준, 층 구분은 고유 mapId, 오른손 좌표계·z 축 위, X·Y·Z 는 미터, 방향은 라디안(−Pi~+Pi). (명세 main 브랜치 3.0.0, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f5",
      "claim": "Open-RMF 플릿 어댑터 튜토리얼은 로봇이 RMF 와 같은 좌표계를 쓰지 않을 때 층마다 대응 경유점 쌍으로 회전·축척·이동 변환을 추정하고(최소 4쌍 권장) 변환 오차 추정값을 계산해 정확도를 확인하게 한다.",
      "tag": "사실",
      "source_ids": [
        "ref-153"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "로봇이 RMF 와 같은 좌표계에서 동작하지 않을 때 대응 경유점으로 변환(rotation, scale, translation)을 추정하며, 'a minimum of 4 matching waypoints is recommended'. nudged 라이브러리로 추정하고 transformation error estimate 를 계산, 레벨별로 적용. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f6",
      "claim": "Open-RMF 주행 지도 통합 문서는 경유점 위치를 층 이름과 그 층 안의 미터 좌표로 요구하고, 좌표계와 건물 구조의 정렬을 확인하는 데 화면 캡처 비교가 도움이 된다고 적는다.",
      "tag": "사실",
      "source_ids": [
        "ref-080"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "경유점마다 level name(B1, L1, L2 등)과 레벨 안의 (x, y) 미터 좌표. 스크린샷은 좌표계와 건물 구조와의 정렬을 sanity-check 하는 데 도움이 된다. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f7",
      "claim": "Vega-Torres 외는 BIM 에서 만든 점유 격자 지도로 AMCL 위치추정을 하는 연구들이 BIM 이 현실을 정확히 나타낸다고 가정하지만 가구·잡동사니와 설계–시공 편차로 인한 차이가 위치추정 정확도를 크게 떨어뜨린다고 보고, 건물 요소 유형의 의미 정보로 라이다가 잘 못 보는 창문이나 현장에서 위치가 바뀌기 쉬운 문·가구를 지도에서 제외했다.",
      "tag": "사실",
      "source_ids": [
        "ref-081"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "Scan-BIM 편차는 가구·잡동사니뿐 아니라 설계 단계 모델의 as-planned/as-built 편차에서도 오며 AMCL 정확도에 크게 영향. 요소 유형 의미 지식으로 창문(라이다 검출 어려움)·문·가구(변동) 제외. (검색 요약 기준)",
      "as_of": "2023-08",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f8",
      "claim": "Ogm2Pgbm 공식 저장소 README 는 BIM·CAD 기반 점유 격자 지도를 변환하기 전에 장애물 내부에 흰 영역이 남지 않도록 장애물을 완전히 검게 채우라고 요구하고, 골격화·커버리지 경로 경유점·광선 추적으로 모의 센서 데이터를 만들어 포즈 그래프 지도를 생성한다.",
      "tag": "사실",
      "source_ids": [
        "ref-082"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "'completely black out the obstacles, ensuring there are no white areas within any obstacles!' 처리: skeletonization → coverage path planning·waypoint 추출 → raytracing 으로 모의 스캔. 저자는 실시간 위치추정 정확도 78% 개선을 보고(저자 보고). (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f9",
      "claim": "연계 대상: Lee·Woo·Shin(IJPEM, 2026)은 2D 건축 CAD 도면으로 3D 가상 환경을 만든 뒤 2D 점유 격자 지도를 자동 생성해 센서 주행 없이 AMCL 위치추정에 쓰고, 가상 지도의 평균 이동 RMSE 0.17±0.06 m, 회전 RMSE 3.59°±1.78°, 궤적 일관성 오차 0.10±0.08 m 로 SLAM 기반 지도와 비슷했다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-628"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "가상 지도 TCE 0.10±0.08 m, 비교: Cartographer 0.16±0.15 m, SLAM Toolbox 0.07±0.03 m, RTAB-Map 0.08±0.04 m. 저자 보고 단일 출처, 시험 환경 규모 미확인. (재인용: 2026-09-25-70)",
      "as_of": "2026",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f10",
      "claim": "IFC 파일에서 자율 로봇용 지도를 만드는 연구(Construction Robotics, 2023)는 IFC 에서 의미별 색을 입힌 장애물 지도를 자동 생성해 사전 지도 작성 주행을 없애고, 배치 시에는 장애물과 진입 불가 공간을 검게, 나머지를 희게 한 흑백 점유 지도로 불러오며, 정지·주행 임무용 경유점을 제안한다.",
      "tag": "사실",
      "source_ids": [
        "ref-647"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "IFC → 내비게이션 지도·시뮬레이션 환경·의미 정보 JSON·stop-and-go 경유점. 2D 지도는 obstacles and inaccessible spaces in black 인 흑백 점유 지도로 로드. (검색 요약 기준, 저자 미확인)",
      "as_of": "2023",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f11",
      "claim": "BIM-to-Robot Mapping(IEEE 학술대회 논문)은 BIM 의 기하 위상과 기능 구획을 뽑아 점유 격자 지도와 의미 대응 사전을 만들고, 방화 구획·금지 구역 같은 BIM 의미 제약을 경로 비용으로 부호화해 경로가 건물 규정을 따르게 하는 틀을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-646"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "OGM 과 semantic mapping dictionary 를 BIM 의 geometric topology·functional partitions 에서 구성, fire zoning·prohibited areas 를 path costs 로 부호화. (검색 요약 기준, 저자·발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f12",
      "claim": "Nav2 비용 지도(costmap_2d)는 지도 위에 공간별 래스터 특성인 필터 마스크를 적용하는 비용 지도 필터로 로봇이 들어가지 않는 금지 구역과 속도 제한 구역을 표현하고, 비용 지도는 로봇 외형(footprint)에 따른 인플레이션 반경으로 부풀려진다.",
      "tag": "사실",
      "source_ids": [
        "ref-644"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "Costmap Filters 는 'filter-masks' 라는 공간 의존 래스터 특성을 지도에 적용하는 도구. 예: keep-out/safety zones, speed restriction areas. 비용 지도는 로봇 footprint 기반 inflation radius 로 부풀려진다. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f13",
      "claim": "Nav2 문서는 필터 마스크를 일반 Nav2 2D 지도와 같은 PGM·PNG·BMP 래스터 파일과 YAML 메타데이터로 배포하며, 금지 구역 필터는 로봇이 금지 구역을 피하거나 선호 차선에 머물게 한다고 설명한다.",
      "tag": "사실",
      "source_ids": [
        "ref-645"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "filter mask 는 PGM, PNG, BMP 래스터와 YAML 메타데이터로 배포되는 일반 Nav2 2D 지도. Keepout Filter 는 keepout areas 회피 또는 preferred lanes 유지. (검색 요약 기준, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약",
      "source_unopened": true
    ...
    {
      "id": "f14",
      "claim": "연계 대상: 유리문·유리벽·창문과 거울·광택 금속 면은 대부분의 각도에서 라이다에 보이지 않거나 반사 잡음을 만들어 점유 격자 지도 작성과 주행에 문제를 일으킨다고 보고된다.",
      "tag": "사실",
      "source_ids": [
        "ref-648"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "Glass paned doors, windows, glass walls, mirrors 등은 실내 로봇 주행에 큰 문제이며 대부분 각도에서 LIDAR 에 보이지 않는다. 연구는 강도 기반 다층 격자로 유리를 검출해 점유 격자를 개선. (검색 요약 기준)",
      "as_of": "2021",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f15",
      "claim": "연계 대상: slam_toolbox 공식 README 는 저장(직렬화)된 포즈 그래프 지도를 불러와 계속 정제·재작성·확장하는 평생 지도 작성, 기존 지도 위 위치추정 모드, 여러 부분 지도를 대화형 표식으로 맞춰 하나의 전역 지도로 합치는 기능, 도킹 위치·노드·지정 자세에서 시작하는 초기화를 제공한다.",
      "tag": "사실",
      "source_ids": [
        "ref-270"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "'continuing to refine, remap, or continue mapping a saved (serialized) pose-graph at any time', localization mode 는 기존 직렬화 지도를 로드, RVIZ 에서 submap 을 배치해 'merge the submaps into a global map'. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f16",
      "claim": "Shaheer 외는 건축 도면에서 만든 그래프와 라이다로 추정한 상황 그래프를 결합해 도면(as-planned)과 현장(as-built)의 전역 정렬과 구조 편차를 실시간으로 추정하는 방법을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-224"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "architectural plan 기반 그래프와 lidar 기반 S-Graph 를 긴밀 결합해 도면–현장 전역 정렬·구조 편차 추정(arXiv 2024-08 제출, 2025-06 개정). (재인용: 2026-09-25-11)",
      "as_of": "2024-08",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f17",
      "claim": "q4-01 에 대해 확인한 자료를 이 위키가 묶으면, 인식 결과를 로봇 내비게이션 지도로 바꿀 때의 보정은 (1) 좌표·축척 보정(픽셀→미터, 세로축 반전, 원점·해상도), (2) 층 정렬과 층 고도, (3) 제조사·플릿별 로봇 지도 좌표계와의 변환과 변환 오차 확인, (4) 표현 보정(장애물 내부 채움, 점유 임계값, 센서가 못 보는 창·유리 요소 처리), (5) 도면에 없는 가구·랙과 설계–시공 편차 반영, (6) 금지 구역·속도 제한 같은 운영 규칙 층 추가의 여섯 묶음으로 나뉘는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-440",
        "ref-079",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "종합: Nav2 YAML 메타데이터(f1), traffic-editor 축척·축 반전·고도·기준점(f2·f3), 플릿 어댑터 대응점 변환(f5), VDA 5050 좌표 규약(f4), 장애물 채움(f8), 의미 기반 요소 제외·편차(f7·f16), 유리(f14), 비용 지도 필터(f12). 이 묶음을 제시한 단일 출처는 없음.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f18",
      "claim": "확인한 도구 구조를 보면 도면 한 장에서 경로 계획용 지도, 위치추정용 지도, 운영 규칙 마스크를 따로 만들어야 할 것으로 보이며, 유리벽처럼 도면에는 벽이지만 라이다가 못 보는 요소는 경로 계획용에서는 장애물로 두고 위치추정용에서는 빼는 식으로 용도별로 다르게 다뤄야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-081",
        "ref-648",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "근거: 의미 기반 창문 제외(f7), 유리의 라이다 비가시성(f14), 필터 마스크가 지도와 같은 형식의 별도 래스터(f12·f13), 진입 불가 공간을 장애물로 칠한 계획용 지도(f10). 용도별 분리는 이 위키의 추정.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f19",
      "claim": "이종 제조사를 연결하는 ROP 가 직접 맡을 보정은 도면 좌표·축척·층 정렬, 제조사 지도 좌표계와의 변환과 오차 확인, 금지 구역·속도 제한 규칙의 공통 정의와 판 관리 쪽이고, 장애물 채움·인플레이션·SLAM 재정합·위치추정 지도 갱신은 로봇·제조사 쪽 연계 대상이 되는 경계로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-153",
        "ref-031",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "플릿 어댑터가 층별 좌표 변환을 맡고(f5), VDA 5050 은 좌표 규약과 지도 배포만 정하며(f4), 인플레이션은 로봇 쪽 비용 지도(f12), 지도 갱신·병합은 로봇 SLAM 도구(f15). 분류 원문 9장 '로봇 자체 지능·제어' 경계 적용.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f20",
      "claim": "분류 원문 질문의 ‘3층 출하 대기장’을 제조사가 다른 로봇이 같은 장소로 인식하려면, 도면 좌표로 정한 대기장 경유점을 층별 변환으로 각 제조사 지도에 옮긴 뒤 그 변환 오차가 해당 노드의 허용 편차 안에 드는지 확인하는 단계가 도착 판정 전에 필요할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-153",
        "ref-031",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "근거: 대응 경유점 기반 변환과 변환 오차 추정(f5), VDA 5050 노드 통과 조건은 allowedDeviationXY·allowedDeviationTheta 안의 위치(6.6.2), traffic-editor 층 고도·기준점(f2·f3). 오차–허용 편차 연결은 이 위키의 추정.",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "완료·인계"
    }
  ...
  "sources": [
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "Nav2 지도 서버가 읽는 점유 격자 지도 YAML 메타데이터(image, resolution, origin, negate, occupied_thresh, free_thresh)와 지도 입출력.",
      "fetched": true,
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "평면도 배경 위 주석 편집기. 측정선으로 축척 설정, 층 고도, 층간 기준점으로 이동·회전·축척 변환, 픽셀 좌표에서 직교 좌표로의 축 반전.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "플릿 어댑터에서 RMF 좌표와 로봇 좌표 사이 회전·축척·이동 변환을 대응 경유점(최소 4쌍 권장)으로 추정하고 변환 오차를 계산하는 방법.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "RMF 주행 지도 요건: 경유점의 층 이름과 층 안 미터 좌표, 좌표계·건물 구조 정렬 확인.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "VDA 5050 3.0.0 명세. 6.3 지도 절의 좌표계 규약(프로젝트 고유 좌표계, mapId, 오른손 좌표계, 미터·라디안)과 노드 허용 편차.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": null,
      "source_unopened": false
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. BIM 기반 점유 격자 지도의 편차 문제와 의미 기반 요소 제외, 포즈 그래프 지도 생성.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "BIM·CAD 기반 격자 지도를 포즈 그래프 지도로 바꾸는 도구. 장애물 내부 채움 요건과 골격화·커버리지 경로·광선 추적 처리.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 2D 건축 CAD 도면에서 3D 가상 환경과 2D 점유 격자 지도를 자동 생성해 AMCL 위치추정 성능을 SLAM 지도와 비교한 국내 저자 연구.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 건축 도면 그래프와 라이다 그래프를 결합해 도면–현장 정렬과 구조 편차를 추정하는 SLAM.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "직렬화 포즈 그래프의 계속 지도 작성·위치추정 모드·부분 지도 병합·시작 자세 초기화를 제공하는 ROS SLAM 도구.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "Nav2 비용 지도 패키지. 필터 마스크를 적용하는 비용 지도 필터(금지 구역·속도 제한)와 로봇 외형 기반 인플레이션.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-645",
      "org": "Open Navigation (Nav2 documentation)",
      "title": "Navigating with Keepout Zones — Nav2 documentation",
      "published": null,
      "url": "https://docs.nav2.org/tutorials/docs/navigation2_with_keepout_filter.html",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 필터 마스크를 일반 Nav2 2D 지도와 같은 래스터+YAML 로 배포하고 금지 구역·선호 차선을 표시하는 튜토리얼.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    ...
    {
      "id": "ref-646",
      "org": "IEEE 학술대회 논문 저자(미확인)",
      "title": "BIM-to-Robot Mapping: Constructing IFC-Referenced Occupancy Grids and Semantic Metadata for Rule-Based Navigation",
      "published": null,
      "url": "https://ieeexplore.ieee.org/document/11019519/",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. BIM 에서 점유 격자 지도와 의미 대응 사전을 만들고 방화 구획·금지 구역을 경로 비용으로 부호화하는 규칙 기반 내비게이션 틀.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    ...
    {
      "id": "ref-647",
      "org": "Construction Robotics(Springer) 게재 논문 저자(미확인)",
      "title": "Improving autonomous robotic navigation using IFC files",
      "published": "2023",
      "url": "https://link.springer.com/article/10.1007/s41693-023-00112-8",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. IFC 에서 의미별 장애물 지도·시뮬레이션 환경·경유점을 자동 생성해 사전 지도 작성 주행 없이 쓰는 방법.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 유리·반사면이 라이다에 잘 보이지 않는 문제와 강도 기반 유리 검출로 점유 격자 지도를 개선하는 방법.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
  ...
  "page_proposals": [
    {
      ...
      "rationale": "q4-01 답: f1·f2·f3·f4·f5·f6·f7·f8·f9·f10·f11·f12·f13·f14·f15·f16·f17·f18·f19·f20 (신뢰도 low) — 2절 q4-01 상태 답함, 3절 q4-01 소제목 신설({#q4-01}): 내비게이션 지도 형식과 메타데이터(f1), 좌표·축척·축 반전·층 고도(f2), 층 정렬(f3), 관제 좌표 규약(f4), 제조사 좌표계 변환과 오차(f5·f6), BIM·CAD 기반 지도의 보정 사례(f7·f8·f9 연계 대상·f10·f11), 운영 규칙 층(f12·f13), 센서가 못 보는 요소(f14 연계 대상), 지도 갱신·병합(f15 연계 대상), 도면–현장 편차 추정(f16), 종합: 보정 여섯 묶음(f17)·용도별 지도 분리(f18)·ROP 경계(f19)·분류 원문 질문(f20) / 4절 결론·불확실성 / 5절 후속 질문 / 6절 완료 조건 현황 / 8절 출처 / 9절 이력"
    },
    {
      "action": "update",
      ...
      "rationale": "아이디어 페이지 5절(트랙 산출물): '핵심 구성 요소'에 '내비게이션 지도 변환 보정' 소절 신설 — 근거 f1·f2·f3·f5·f7·f8·f12, 구현 가설 f17·f18·f19(추정). '아직 조사되지 않은 구성 요소'에 도면–현장 정합 절차(q4-02·q4-03)가 남음을 명시"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신: track.ontology_changes 가 검증 승인되면 층별 지도에 '로봇 지도 좌표계 변환(플릿별, 대응점·변환 오차)'(f5)과 '내비게이션 지도 메타데이터(해상도·원점·점유 임계값)'(f1) 속성 반영. 6절에 보정 항목 목록(f17 추정)과 용도별 지도 분리(f18 추정), 운영 규칙 마스크를 스키마 개념으로 둘지(f12·f13, VDA 5050 구역 집합 질문과 연결) 질문 추가"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 4 반영 제안 (f2, f3, f5, f7, f17, f19): 6절(주제 페이지 area06-s6)에 도면 기반 지도를 로봇 내비게이션 지도로 쓰기 전 보정 항목(추정)과 대응점 기반 좌표 변환·오차 확인, 9절에 장애물 채움·인플레이션·SLAM 재정합은 연계 대상이고 좌표 변환·층 정렬·규칙 층 판 관리는 ROP 쪽이라는 경계(추정)"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 4 반영 제안 (f5, f9, f10, f20): 시운전에서 도면 기반 지도의 좌표 변환 오차를 대응점으로 확인하는 절차와 사전 지도 작성 주행을 줄인 연구 사례(연계 대상 포함)"
    }
  ],
  "glossary_candidates": [
    {
      "term_ko": "비용 지도",
      "term_en": "Costmap",
      "definition": "로봇 경로 계획을 위해 점유 격자 지도에 장애물·로봇 크기 여유(인플레이션)·금지 구역 같은 비용을 겹쳐 칸마다 통과 비용을 매긴 격자 지도다."
    },
    {
      "term_ko": "필터 마스크",
      "term_en": "Filter Mask (Nav2 costmap filter)",
      "definition": "Nav2 에서 금지 구역·속도 제한처럼 공간별 동작 규칙을 표시하는 별도 래스터 지도로, 일반 지도와 같은 이미지+YAML 형식으로 배포되어 비용 지도에 적용된다."
    }
  ],
  "open_questions_new": [],
    ...
    "ontology_changes": [
      {
        ...
        "description": "속성 '로봇 지도 좌표계 변환(제조사·플릿별 회전·축척·이동, 대응 경유점 최소 4쌍 권장, 변환 오차 추정값; Open-RMF 플릿 어댑터 근거)'과 '내비게이션 지도 메타데이터(해상도·원점·점유/빈 공간 임계값; Nav2 지도 YAML 근거)'를 더한다. 기존 속성 '도면 대비 변환(이동·회전)'은 도면→층별 지도이고 이번 제안은 층별 지도→로봇 지도라 충돌하지 않으나, 6절의 '정렬 정보를 층별 지도 속성으로 둘지 별도 개념으로 둘지' 질문(q4-03)과 겹치므로 검증이 판단한다."
      }
    ],
...
### runs/2026-09-25-72/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: raw.githubusercontent.com 의 nav2_map_server README 를 열어 YAML 필드(image, resolution 0.1, origin [2.0,3.0,1.0], negate 0, occupied_thresh 0.65, free_thresh 0.196), Nav1 YAML 형식 유지, nav_msgs/msg/OccupancyGrid 지원을 확인했다. 발행일 미확인, 확인일 2026-09-25. 단일 출처."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: traffic-editor.md 원본(raw)에서 측정선 distance(m)로 레벨 Scale 갱신, 기준 평면도 픽셀 좌표(+X 오른쪽, +Y 아래, 왼쪽 위 원점), 지도 생성 단계의 세로축 반전, 레벨 elevation(m)을 확인했다. 발행일 미확인."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 같은 원본에서 수직으로 정렬되는 기준점 쌍 두 개 이상으로 층 사이 이동·회전·축척 변환을 유도한다는 구절을 확인했다. f2 와 같은 문서라 독립 교차 확인은 아니다."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 입력 원문 텍스트(data/source_texts/ref-031.txt) 6.3 Maps 에서 프로젝트 고유 좌표계, 층 구분용 고유 mapId, z 축이 위를 향하는 오른손 좌표계, 미터 단위, −Pi~+Pi 라디안을 확인했다. 명세 머리 표기 Version 3.0.0, 발행일은 원문에 없음(oq-005). 브리프 sources 의 ref-031 은 fetched_via github_raw·fetch_url null 로 적혀 있으나 실제 근거는 inbox 원문 텍스트다(verification_note 참조)."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: integration_fleets_adapter_tutorial.md 원본에서 'A minimum of 4 matching waypoints is recommended', nudged.estimate 로 회전·축척·이동 추정, nudged.estimate_error 로 오차(MSE) 계산, reference_coordinates 레벨별 적용을 확인했다. 06 페이지 6절의 기존 ref-153 주장과 같은 내용이므로 기존 각주를 재사용한다."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: integration_nav-maps.md 원본에서 경유점의 level name 과 레벨 안 (x, y) 미터 좌표 요구, 스크린샷이 좌표계·건물 구조 정렬 sanity-check 에 도움이 된다는 구절을 확인했다."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "부분 확인(원문 미열람, 검색 결과 일치): arXiv 2308.05443 초록 스니펫은 BIM 추출 OGM 에서 AMCL 을 쓰는 연구들과, 가구·잡동사니뿐 아니라 as-planned/as-built 편차가 AMCL 정확도에 크게 영향을 준다는 부분까지만 담는다(저자 Vega Torres·Braun·Borrmann, 2023-08-10 제출, ECPPM 2022 판). 창문·문·가구를 요소 유형 의미로 지도에서 제외했다는 부분은 스니펫에 없어 그 부분만 [추정]으로 강등한다. 위치추정 자체는 로봇 쪽 연계 대상."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: Ogm2Pgbm README 원본에서 장애물을 완전히 검게 채우라는 지시, 골격화·보로노이 경유점, 커버리지 경로 계획(wavefront), 경유점별 광선 추적으로 /laserscan 발행을 확인했다. README 머리의 78% 위치추정 정확도 개선은 저자 주장이며 claim 에 없으므로 본문에 쓰면 '저자 보고 [추정]'으로만 쓴다. 포즈 그래프 위치추정 지도는 로봇 쪽 연계 대상."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이전 실행 2026-09-25-70 의 1차 검증에서 같은 주장(당시 f11)이 확인된 것을 재사용했다. 이번에는 다시 열지 않았다(원문 미열람). 수치는 저자 보고 단일 출처이며 '연계 대상' 표시 유지. 참고문헌은 기존 ref-628 각주를 그대로 쓴다."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "부분 확인(원문 미열람, 검색 결과 일치): Springer 스니펫은 IFC 에서 내비게이션 지도·시뮬레이션 환경·의미 정보 JSON·stop-and-go 경유점을 만들고, 의미별 색 장애물 지도를 자동 생성해 사전 지도 작성 필요를 없앤다는 부분까지 담는다. '장애물과 진입 불가 공간을 검게, 나머지를 희게 한 흑백 점유 지도로 불러온다'는 스니펫에 없어 그 부분을 [추정]으로 강등하거나 뺀다. 저자 미확인, 발행 2023."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): IEEE Xplore 11019519 제목 일치, 스니펫이 BIM 기하 위상·기능 구획에서 OGM 과 의미 대응 사전을 구성하고 방화 구획·금지 구역을 경로 비용으로 부호화한다는 부분을 담는다. 저자·발행일 미확인. 논문의 위치추정 전략 부분은 연계 대상이라 쓰지 않는다."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: nav2_costmap_2d README 원본에서 filter-masks 를 적용하는 Costmap Filters(keep-out/safety zones, speed restriction areas)와 로봇 footprint 기반 inflation radius 를 확인했다. 비용 지도·인플레이션은 로봇 쪽 내비게이션 스택 기능(연계 대상)."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): docs.nav2.org 튜토리얼 스니펫이 필터 마스크를 PGM·PNG·BMP 래스터와 YAML 메타데이터로 배포되는 일반 Nav2 2D 지도로 설명하고 Keepout Filter 가 금지 구역과 선호 차선을 다룬다고 적는다. ref-644 와 같은 Nav2 프로젝트라 독립 교차 확인은 아니다."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "부분 확인(원문 미열람, 검색 결과 일치): Sensors 21(7) 2263(2021-04, 저자 Tibebu·Roche·De Silva·Kondoz, Loughborough University London) 스니펫은 유리가 라이다에 보이지 않아 지도 작성이 어렵고 유리 검출로 점유 격자 오류를 줄인다는 부분만 담는다. '거울·광택 금속 면', '대부분의 각도', '반사 잡음'은 스니펫에 없어 [추정]으로 강등하거나 빼고 유리에 한정한다. 연계 대상 표시 유지."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: slam_toolbox README(ros2 브랜치) 원본에서 직렬화 포즈 그래프의 계속 정제·재작성·지도 작성, 위치추정 모드, RVIZ 대화형 표식으로 여러 포즈 그래프를 옮겨 전역 지도로 병합, 도크·노드·지정 영역에서 시작하는 초기화를 확인했다. 연계 대상 표시 유지."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "기존 페이지(06 페이지·공간 그래프 스키마 초안)에 같은 주장이 ref-224 로 이미 실려 있어 기존 각주를 재사용한다. 이번에 다시 열지 않았다(원문 미열람). 도면 기반 SLAM 은 로봇 쪽 연계 대상으로 표시해야 한다."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 종합이며 [추정] 유지. 근거 finding 은 대부분 확인됐으나 (4)의 '창·유리 요소 처리'는 강등된 f7·f14 부분에 기대므로 '것으로 보인다' 수준을 유지한다. 여섯 묶음을 제시한 단일 출처 없음."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 추정으로 [추정] 유지. 근거 가운데 f7 의 창문 제외·f10 의 흑백 지도 세부는 강등됐으므로 본문에서 근거 문구를 '의미 정보로 요소를 거른 연구가 있다는 검색 요약' 수준으로 낮춘다."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 추정으로 [추정] 유지. 분류 원문 9장 '로봇 자체 지능·제어' 경계와 맞는다. f5·f4·f12·f15·f8 은 확인됐다."
    },
    {
      "finding_id": "f20",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 추정으로 [추정] 유지. VDA 5050 6.6.2 의 allowedDeviationXY·allowedDeviationTheta 조건은 입력 원문 텍스트에서 확인했다. 흐름 단계 출하 / 완료·인계."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "scope_boundary": {
    "ok": false,
    "issues": [
      "f7: BIM 기반 점유 격자 지도로 하는 AMCL 위치추정은 분류 원문 9장 '로봇 자체 지능·제어' 연계 영역인데 '연계 대상' 표시가 없다",
      "f8: Ogm2Pgbm 의 포즈 그래프 위치추정 지도 생성은 로봇 쪽 위치추정 기술(연계 대상)인데 표시가 없다",
      "f16: 도면–라이다 결합 SLAM 은 기존 06 페이지 9절이 연계 대상으로 분류한 기술인데 브리프에 표시가 없다"
    ]
  },
  "duplication": {
    "ok": true,
    "overlaps": [
      "f5 는 06 페이지 6절의 기존 [사실] 주장(대응 좌표 쌍으로 회전·축척·이동 변환 추정, ref-153)과 같다 — 기존 각주 재사용",
      "f9 는 실행 2026-09-25-70 f11(ref-628, 당시 브리프 id ref-644)과 같은 주장 — 기존 ref-628 각주 재사용",
      "f7 은 실행 2026-09-25-70 f13·스키마 초안 6절의 ref-081 주장과 겹친다 — 기존 ref-081 각주 재사용",
      "f16 은 06 페이지·스키마 초안 6절의 ref-224 주장과 같다 — 기존 각주 재사용",
      "f2·f3 은 스키마 초안의 층별 지도(축척·도면 대비 변환)·층간 정렬 기준점 근거(ref-079)와 겹친다",
      "온톨로지 변경 '로봇 지도 좌표계 변환'은 스키마 초안 6절의 미해결 질문(정렬 정보를 층별 지도 속성으로 둘지 별도 개념으로 둘지, q4-03)을 근거 없이 결정한다"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "required_fixes": [
    "f7: 앞부분(BIM 추출 점유 격자 지도로 AMCL 을 쓰는 연구들이 BIM 을 현실로 가정하지만 가구·잡동사니와 설계–시공 편차가 AMCL 정확도에 크게 영향을 준다)만 [사실]로 쓰고, '창문·문·가구를 요소 유형 의미로 지도에서 제외했다'는 부분은 [추정]으로 강등해 '검색 요약 기준'을 밝힌다 — 검증 검색 스니펫에 제외 부분이 나타나지 않았다.",
    "f10: '장애물과 진입 불가 공간을 검게, 나머지를 희게 한 흑백 점유 지도로 불러온다'는 부분을 빼거나 [추정]으로 강등한다 — 검색 스니펫은 IFC→내비게이션 지도·시뮬레이션 환경·JSON·경유점 생성과 사전 지도 작성 불필요까지만 뒷받침한다.",
    "f14: 주장을 '유리는 라이다에 보이지 않아 점유 격자 지도 작성을 어렵게 한다'로 좁혀 [사실]로 쓰고, 거울·광택 금속 면·반사 잡음 부분은 빼거나 [추정]으로 둔다. ref-648 의 기관을 'Tibebu, H., Roche, J., De Silva, V., & Kondoz, A. (Loughborough University London, Sensors 21(7), 2263)', 발행일을 2021-04 로 고쳐 reference_updates 와 각주에 반영한다 — 검색 결과로 저자·발행월을 확인했다.",
    "f7·f8·f16: 본문에서 문장 앞에 '연계 대상:'을 붙이고, 위치추정·SLAM 자체는 분류 원문 9장 '로봇 자체 지능·제어' 쪽이며 ROP 는 지도 형식·좌표·판 관리 관점으로만 다룬다는 점을 유지한다 — 범위 경계.",
    "f8: README 머리의 '위치추정 정확도 78% 개선'은 본문에 쓰지 않거나, 쓸 경우 '저자 보고' [추정]으로만 쓴다 — 저자 측 단일 주장이다.",
    "f12: 비용 지도 인플레이션·필터 적용은 로봇 쪽 내비게이션 스택 기능으로 '연계 대상'임을 밝히고, ROP 쪽은 규칙(금지 구역·속도 제한)의 공통 정의와 배포까지로 한정하는 f19 의 경계를 [추정]으로 유지한다.",
    "f17·f18: 근거 요약에서 강등된 f7(창문 제외)·f10(흑백 지도)·f14(거울) 세부를 사실처럼 인용하지 않고, 두 종합은 [추정] 태그와 '이 위키의 종합, 단일 출처 없음'을 유지한다.",
    "온톨로지 변경 '층별 지도' 수정 가운데 '내비게이션 지도 메타데이터(해상도·원점·점유/빈 공간 임계값; Nav2 지도 YAML 근거, f1)'만 승인한다: 기존 속성 '교환 형식(후보)'의 Nav2 격자 지도 값에 딸린 속성으로 추가하고 행 상태는 '확정'을 유지한다. 근거 칸에 'finding f1 (실행 2026-09-25-72)[^ref-440]'을 적는다.",
    "온톨로지 변경 '층별 지도' 수정 가운데 '로봇 지도 좌표계 변환(제조사·플릿별 회전·축척·이동, 대응 경유점 최소 4쌍 권장, 변환 오차 추정값)'은 반영하지 않고, 스키마 초안 6절의 '도면 좌표계와 로봇별 지도 좌표계의 정렬 정보 … 층별 지도의 속성으로 둘지 별도 개념으로 둘지' 항목에 근거 보강으로 f5([사실][^ref-153])를 적는다 — 이 속성은 미해결 질문 q4-03 을 근거 없이 결정한다(v0.6 의 '지도 판' 거부와 같은 이유).",
    "공간 그래프 스키마 초안: 승인한 변경이 있으므로 ontology_version 을 '0.9'에서 한 단계(+0.1) 올린 '1.0'으로 하고 H1·auto 상태 줄 이외 표기·프런트매터·track_updates.ontology_draft_version 네 곳을 같게 맞추며, 2절 변경 설명 문단에 v1.0(실행 2026-09-25-72)의 승인·거부 내용을 한 문장씩 더한다.",
    "단계 4 페이지 6절: 완료 조건 '충족 여부'와 '검증 판정' 칸을 '미충족'으로, 표 아래 줄을 '다음 단계로 전환: 아니오(도면–현장 정합 절차 초안 없음; 막힌 질문 q4-02·q4-03·q4-04·q4-05·q4-07·q4-08)'로 쓰고, 상태 줄의 단계 상태는 '진행 중'으로 둔다 — 1차 판정 단계 전환 미승인.",
    "단계 4 페이지 2절: q4-01 을 '답함'(답한 실행 id 2026-09-25-72, 답 위치 #q4-01)으로 바꾸고 3절에 '### q4-01 … {#q4-01}' 소제목을 둔다. 핵심 결론(f17~f20)은 [추정]이며 종합 신뢰도 low 임을 4절에 밝힌다.",
    "각주: 원문을 열지 못한 출처 ref-081·ref-628·ref-224·ref-645·ref-646·ref-647·ref-648 의 각주 정의 접근일 뒤에 ' (원문 미열람)'을 붙이고 reference_updates 의 해당 항목에 source_unopened: true 를 넣는다. 원문을 연 ref-440·ref-079·ref-153·ref-080·ref-082·ref-270·ref-644·ref-031 에는 붙이지 않는다.",
    "ref-646·ref-647: 기관 칸에 저자 미확인, ref-646 발행일은 '미확인'(JSON 은 null)으로 둔다.",
    "세부영역 반영 제안(06. 지도·공간·위치 모델 6·9절, 21. 온보딩·설정·현장 시운전 6절)은 세부영역 페이지를 직접 고치지 않고 트랙 로그의 '세부영역 반영 제안'으로만 남기며, 6. 지도·공간·위치 모델 쪽 제안에서 f5 는 기존 6절 문장(ref-153)과 같으므로 새 문장이 아닌 기존 각주 재사용으로 적는다.",
    "용어집: '필터 마스크' 정의에 근거 각주 ref-644(필요 시 ref-645)를 달고, '비용 지도' 정의의 금지 구역 겹침은 필터 마스크 적용으로 표현한다 — 용어집 기존 '점유 격자 지도'와 충돌하지 않도록 비용 지도를 그 위에 비용을 매긴 격자로 설명한다.",
    "새 질문 2건은 단계 4 태그로 백로그에 등록하고 origin 은 각각 f18·f12 로 적는다."
  ],
  "confidence": "low",
  "verification_note": "판정: 조건부 승인. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(mirror_only: raw.githubusercontent.com 원본 7건과 입력 원문 텍스트 1건은 직접 확인). 확인 17건, 미확인 3건(f7·f10·f14 부분 불일치), 교차 확인 0건. 강등: f7 사실 → 일부 추정(창문·문·가구 제외 부분), f10 사실 → 일부 추정(흑백 지도 세부), f14 사실 → 일부 추정(거울·금속 반사 부분). 원문 미열람 출처: ref-081, ref-628, ref-224, ref-645, ref-646, ref-647, ref-648. 주의: q4-01 의 답(보정 여섯 묶음, 용도별 지도 분리, ROP 경계, 분류 원문 질문 연결)은 이 위키의 종합 [추정]이며 이를 제시한 단일 출처가 없다. 위치추정·SLAM·비용 지도 인플레이션은 로봇 쪽 연계 대상이다. 브리프의 ref-031 은 fetched_via github_raw·fetch_url null 로 기록됐으나 실제 근거는 입력 원문 텍스트(inbox)다. f9·f16 은 이전 실행 확인분을 재사용했다. 온톨로지 변경 승인: 층별 지도 속성 '내비게이션 지도 메타데이터(Nav2 YAML: 해상도·원점·점유/빈 임계값)'(f1) / 거부: 층별 지도 속성 '로봇 지도 좌표계 변환'(f5 — 6절 정렬 정보 질문 q4-03 을 근거 없이 결정하므로 6절 근거 보강으로 둠). 단계 완료 조건: 미충족(부족: 도면–현장 정합 절차 초안 없음, 보정 항목 목록은 이번 반영 전; 막힌 질문 q4-02·q4-03·q4-04·q4-05·q4-07·q4-08). 단계 전환: 미승인(완료 조건 미충족·막힌 질문 있음).",
  "retry_reason": null,
  "track_checks": {
...
### docs/references/index.md (요약: 이번 대상 페이지가 인용한 149건 / 전체 650건. 목록에 없는 출처는 새 id 로 적는다 — 같은 URL 이 이미 있으면 퍼블리셔가 기존 id 로 합친다)
...
### docs/open-questions.md (요약: 대상 영역 [3, 5, 6, 8, 10, 15, 16, 21, 22, 23, 24, 27, 28] 에 걸린 72건 / 전체 112건)
...
### runs/2026-09-25-72/docs_tree.txt
...
- [공간 그래프 스키마 초안](space-graph-schema-draft.md) — 현재 버전 v0.9. 실행 2026-09-25-05에서 창문·난간 개념과 문 없는 인접 관계를 더하고 공간 노드에 방 유형 속성을 넣었으며 벽·문·계단·공간 노드를 확정했다. 실행 2026-09-25-11에서 층간 정렬 기준점을 더하고 층별 지도(축척·도면 대비 변환)와 평면도(형식 값 후보)를 확정했으며, 문 속성 추가 제안은 미해결 질문으로 두었다. 실행 2026-09-25-19에서 작업 스테이션 개념을 더하고 충전 위치(접근 지점 속성)·엘리베이터(BIM 대응 클래스 속성)를 확정했으며, 정보 출처 속성과 작업 스테이션의 공용 자원 포함 여부는 미해결 질문으로 두었다. 실행 2026-09-25-22에서는 변경이 없었다. 실행 2026-09-25-28에서 공간 노드·층에 표준 대응 클래스(후보) 속성을 더하고 층을 확정했으며 주제 레이어 개념을 더했고, 문의 표준 대응 클래스와 주제 레이어의 로봇·사람 레이어 구분은 미해결 질문으로 두었다. 실행 2026-09-25-36에서 문·계단에 BIM 대응 클래스(IfcDoor·IfcStair) 속성을, 평면도에 길이 단위·축척 정보 속성을 더하고 엘리베이터 유형 값 ELEVATOR를 확정했으며, 계단이 잇는 층의 도출과 래스터 축척 복원 방식은 미해결 질문으로 두었다. 실행 2026-09-25-44에서 층별 지도에 교환 형식(후보) 속성을 더했으며, 지도 판 속성과 VDA 5050 구역 집합의 개념화는 미해결 질문으로 두었다. 실행 2026-09-25-54(단계 3, q3-01)에서는 변경이 없었다. 실행 2026-09-25-58(단계 3, q3-02)에서 경유점·주행 차선 개념을 더하고 공용 자원에 상호 배제 여부 속성을 넣어 확정했으며, 공용 자원의 점유 요소 속성과 경유점·주행 차선과 공간 노드의 관계는 미해결 질문으로 두었다. 실행 2026-09-25-65(단계 3, q3-03)에서 문에 자동 구동 여부·장애인 접근 가능 속성을, 계단에 단 높이·디딤판 길이·단 수 속성을 더했으며, 개념 '통과 요구 조건'은 미해결 질문으로 두었다. 실행 2026-09-25-70(단계 3, q3-04)에서 층의 높이 기준에 층 고도 값 후보를 병기하고, 문의 여닫는 방식에 Open-RMF 문 유형 값 후보와 동작 범위 속성을, 엘리베이터에 칸 치수 속성을 더했으며, 로봇 스폰 위치·주문 흐름·초기 재고를 스키마 안에 둘지는 미해결 질문으로 두었다.
- [아이디어 3. 건축 도면 자동 인식](../../ideas/floorplan-recognition.md) — 확장 아이디어 페이지. 3절(선행 연구·제품 사례)에 공개 데이터셋 비교(실행 2026-09-25-05), 도면→로봇 지도 연구·오픈소스 도구 비교와 제품 사례(벤더 주장 병기, 실행 2026-09-25-11), 운영 시설(충전소·작업 스테이션) 보완 사례(실행 2026-09-25-19), 현장 모델링 부담의 근거(실행 2026-09-25-22)가 실렸다. 4절에는 IFC 4.3·VDMA LIF·VDA 5050의 시설 표현(실행 2026-09-25-19), 공간 그래프를 표현하는 표준 비교(q2-01, 실행 2026-09-25-28), 입력 형식(BIM·벡터 CAD·래스터 스캔)별 정보 항목(q2-02, 실행 2026-09-25-36), 관제·ROP 수용 형식(q2-03, 실행 2026-09-25-44)이 실렸다. 5절(구현 가설)에는 처리 흐름의 단계별 입력·출력과 사람 검토 지점(q3-01, 실행 2026-09-25-54), 핵심 구성 요소 가운데 공간 그래프의 두 층위·자원 예약 단위·통과 조건 분리(q3-02, 실행 2026-09-25-58, 추정), 능력 대조와 다른 아이디어와의 연결(q3-03, 실행 2026-09-25-65, 추정), 시뮬레이션 초기값과 설계용·운영 예측용 초기화 구분(q3-04, 실행 2026-09-25-70, 추정)이 실렸고, 나머지 핵심 구성 요소는 단계 3·4 실행이 채운다. 6절에는 단계 5 조사 전 선행 근거로 가설 3 판정의 비교 기준 후보와 측정 대상 반복 작업이 실렸다(실행 2026-09-25-22).
- [질문 백로그](question-backlog.md) — 시작 질문 18건과 후속 질문 21건(실행 2026-09-25-05의 3건, 실행 2026-09-25-11의 1건, 실행 2026-09-25-19의 1건, 실행 2026-09-25-22의 2건, 실행 2026-09-25-28의 2건, 실행 2026-09-25-36의 2건, 실행 2026-09-25-44의 2건, 실행 2026-09-25-54의 2건, 실행 2026-09-25-58의 2건, 실행 2026-09-25-65의 2건, 실행 2026-09-25-70의 2건). 실행 2026-09-25-11에서 중복 등록된 q1-07·q2-05·q4-06은 실행 2026-09-25-19에서 폐기했다. q1-01·q1-02·q1-03·q1-04·q2-01·q2-02·q2-03·q3-01·q3-02·q3-03·q3-04 답함. 최신 수치는 백로그 페이지의 자동 표를 따른다.
- [트랙 로그](log.md) — 실행별 기록
...
<!-- auto:page-status:start -->
> 초안 버전: v0.9 · 페이지 상태: published · 신뢰도: low · 페이지 버전: 10 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
원천: [질문 백로그](../tracks/floorplan-recognition/question-backlog.md)([건축 도면 자동 인식](../tracks/floorplan-recognition/index.md) 트랙) · 열림 28건 · 답함 11건 · 폐기 3건
...
```json
{
  ...
  "outline": [
    {
      ...
      "section": "2. 질문 목록",
      "budget_chars": 900,
      "summary": "q4-01 을 답함(실행 2026-09-25-72)으로 바꾸고 다른 실행이 이 단계로 보낸 q4-05·q4-07·q4-08 과 새 후속 질문 q4-09·q4-10 을 표에 더한다."
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
      "section": "3. 조사 결과",
      "budget_chars": 5200,
      "summary": "인식 결과를 로봇 내비게이션 지도로 바꿀 때의 보정은 좌표·축척, 층 정렬, 제조사 좌표계 변환과 오차 확인, 표현 보정, 도면–현장 편차, 운영 규칙 층의 여섯 묶음으로 나뉘는 것으로 보인다. [추정][^ref-440][^ref-079][^ref-153]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 1300,
      "summary": "q4-01 의 결론(여섯 묶음, 용도별 지도 분리, ROP 경계)은 모두 이 위키의 종합 [추정]이며 종합 신뢰도는 low 이다. [추정][^ref-153][^ref-644]"
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
      "section": "5. 이 단계가 낳은 후속 질문",
      "budget_chars": 500,
      "summary": "용도별 지도 분리 규칙(q4-09, f18)과 운영 규칙의 한 벌 정의·내보내기(q4-10, f12) 두 질문을 단계 4 로 올린다."
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
      "section": "6. 완료 조건 충족 현황",
      "budget_chars": 600,
      "summary": "완료 조건은 미충족이며 단계 전환은 승인되지 않았다(도면–현장 정합 절차 초안 없음)."
    },
    {
      "path": "docs/tracks/floorplan-recognition/space-graph-schema-draft.md",
      "section": "2. 개념 목록 표 / 6. 미해결 모델링 질문",
      "budget_chars": 1200,
      "summary": "층별 지도에 내비게이션 지도 메타데이터 속성을 더해 v1.0 으로 올리고, 좌표계 변환 속성은 6절 근거 보강으로 둔다. [사실][^ref-440][^ref-153]"
    },
    {
      "path": "docs/ideas/floorplan-recognition.md",
      "section": "5. 구현 가설",
      "budget_chars": 1500,
      "summary": "내비게이션 지도 변환 보정 소절을 더하고 도면–현장 정합 절차가 아직 조사되지 않았음을 밝힌다. [추정][^ref-079][^ref-153][^ref-644]"
    }
  ],
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "q4-01 답함(보정 여섯 묶음·용도별 지도·ROP 경계, 추정), 후속 질문 2건(q4-09·q4-10), 완료 조건 미충족, 상태 줄 진행 중"
    },
    {
      "path": "docs/tracks/floorplan-recognition/space-graph-schema-draft.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "v0.9 → v1.0: 층별 지도 교환 형식(후보)의 Nav2 격자 지도 값에 딸린 속성 '내비게이션 지도 메타데이터' 추가(f1); 거부된 '로봇 지도 좌표계 변환'은 6절 정렬 정보 질문 근거 보강(f5); 6절에 보정 항목·용도별 지도·운영 규칙 마스크 질문 추가"
    },
    {
      "path": "docs/tracks/floorplan-recognition/index.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "상태 줄 갱신(현재 단계 단계 4, 마지막 트랙 실행 2026-09-25), 6절 산출물 링크에 스키마 초안 v1.0·q4-01 답·후속 질문 2건 반영"
    },
    {
      "path": "docs/ideas/floorplan-recognition.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "5절 끝에 '내비게이션 지도 변환 보정' 소절 추가(q4-01, 추정), 도면–현장 정합 절차 미조사 명시, 새 각주 5건 정의",
      "patches": [
        {
          "section": "5. 구현 가설",
          "action": "append",
  ...
  "changelog_entry": "2026-09-25 | 건축 도면 자동 인식 단계 4 | q4-01 답함(내비게이션 지도 변환 보정 여섯 묶음·용도별 지도·ROP 경계, 추정), 공간 그래프 스키마 초안 v1.0, 후속 질문 2건 | run 2026-09-25-72",
  "index_updates": {
    "home_recent": "2026-09-25 — 건축 도면 자동 인식 단계 4: q4-01(도면 인식 결과를 로봇 내비게이션 지도로 바꿀 때의 보정) 답함, 공간 그래프 스키마 초안 v1.0 (실행 2026-09-25-72)",
    "category_recent": "2026-09-25 — 6. 지도·공간·위치 모델: 트랙 건축 도면 자동 인식 단계 4에서 내비게이션 지도 변환 보정 여섯 묶음과 ROP·로봇 쪽 경계를 추정으로 정리, 세부영역 반영 제안 3건 (실행 2026-09-25-72)",
    "area_recent": "2026-09-25 — 6. 지도·공간·위치 모델: 트랙 건축 도면 자동 인식 단계 4 q4-01 답(내비게이션 지도 변환 보정), 6절·9절 반영 제안 (실행 2026-09-25-72)"
  },
  "glossary_updates": [
    {
      "action": "new",
      ...
      "description": "Nav2 의 costmap_2d 패키지가 대표 구현이다. 점유 격자 지도가 칸의 점유 여부를 나타낸다면 비용 지도는 그 위에 통과 비용을 매긴다. 비용 지도 생성과 인플레이션은 로봇 쪽 내비게이션 스택 기능이며 ROP 에서는 연계 대상이다.",
      "related_areas": [
        6,
        15
      ],
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "VDA 5050 3.0.0 명세. 6.3 지도 절의 좌표계 규약(프로젝트 고유 좌표계, mapId, 오른손 좌표계, 미터·라디안)과 노드 허용 편차. 이번 실행의 근거는 입력 원문 텍스트다.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
        "docs/ideas/floorplan-recognition.md"
    ...
    {
      "id": "ref-648",
      "org": "Tibebu, H., Roche, J., De Silva, V., & Kondoz, A. (Loughborough University London, Sensors 21(7), 2263)",
      "title": "LiDAR-Based Glass Detection for Improved Occupancy Grid Mapping",
      "published": "2021-04",
      "url": "https://www.mdpi.com/1424-8220/21/7/2263",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 유리가 라이다에 잘 보이지 않는 문제와 유리 검출로 점유 격자 지도를 개선하는 방법.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
        "docs/tracks/floorplan-recognition/space-graph-schema-draft.md",
  ...
  "flow_matrix_updates": [
    {
      "step": "출하",
      "item": "수행 자원",
      "link": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md#q4-01",
      "title": "단계 4. 지도 변환 보정과 현장 정합"
    },
    {
      "step": "출하",
  ...
  "standards_updates": [
    {
      "name": "Nav2 costmap_2d (비용 지도·비용 지도 필터: 금지 구역·속도 제한)",
      "kind": "오픈소스",
      ...
      "summary": "Nav2 의 비용 지도 패키지로, 필터 마스크를 적용하는 비용 지도 필터로 금지 구역·속도 제한 구역을 표현하고 로봇 외형에 따른 인플레이션을 적용한다. 로봇 쪽 내비게이션 스택 기능이며 ROP 에서는 연계 대상이다.",
      "ref_id": "ref-644"
    }
  ],
  ...
  "track_updates": {
    ...
    "log_entry": "답한 질문: q4-01(보정 여섯 묶음·용도별 지도·ROP 경계, 핵심 종합은 추정, 신뢰도 low) / 새 질문: q4-09(f18), q4-10(f12) / 온톨로지 변경: v0.9 → v1.0: 층별 지도 교환 형식(후보)의 Nav2 격자 지도 값에 딸린 속성 '내비게이션 지도 메타데이터(해상도·원점·점유/빈 공간 임계값)' 추가(f1); 거부: 층별 지도 속성 '로봇 지도 좌표계 변환'(f5 → 6절 정렬 정보 질문 근거 보강, q4-03). 버전 이력 행: 1.0 / 2026-09-25 / v0.9 → v1.0: 층별 지도 '내비게이션 지도 메타데이터' 추가(f1); 거부: '로봇 지도 좌표계 변환'(f5 → 6절 질문); H1 버전 표기 v1.0 / 2026-09-25-72 / 완료 조건 평가: 미충족(부족: 도면–현장 정합 절차 초안 없음, 보정 항목 목록은 이번 반영의 검증 판정 전; 막힌 질문 q4-02·q4-03·q4-04·q4-05·q4-07·q4-08) / 세부영역 반영 제안: 6. 지도·공간·위치 모델 2건(6. 대표 접근법과 기술, 9. ROP가 직접 맡는 것과 외부와 연계하는 것), 21. 온보딩·설정·현장 시운전 1건(6. 대표 접근법과 기술) / 다음 실행 제안: q4-03, q4-02",
    "overview_progress": "단계 4 진행 중 — 열린 질문 8, 답함 1, 완료 조건 미충족"
  },
  "area_reflection_proposals": [
    {
      "area_no": 6,
      "section": "6. 대표 접근법과 기술",
      "summary": "도면 기반 지도를 로봇 내비게이션 지도로 쓰기 전 보정 여섯 묶음(좌표·축척, 층 정렬, 제조사 좌표계 변환과 오차, 표현 보정, 도면–현장 편차, 운영 규칙 층, [추정], f17)과 traffic-editor 의 측정선 축척·세로축 반전·층 기준점 정렬([사실], f2·f3, ref-079). 대응점 기반 좌표 변환·오차 확인(f5)은 기존 6절 문장과 같으므로 새 문장 없이 기존 ref-153 각주를 재사용한다."
    },
    {
      "area_no": 6,
      "section": "9. ROP가 직접 맡는 것과 외부와 연계하는 것 (부록 A 9장 기준)",
      "summary": "보정 가운데 도면 좌표·축척·층 정렬, 제조사 지도 좌표계 변환과 오차 확인, 금지 구역·속도 제한 규칙의 공통 정의와 판 관리는 ROP 쪽, 장애물 채움·비용 지도 인플레이션·SLAM 재정합·위치추정 지도 갱신은 로봇·제조사 쪽 연계 대상이라는 경계([추정], f19, ref-153·ref-031·ref-644·ref-270·ref-082)."
    },
    {
      "area_no": 21,
      "section": "6. 대표 접근법과 기술",
      "summary": "시운전에서 도면 기반 지도의 좌표 변환 오차를 대응 경유점(최소 4쌍 권장)으로 확인하는 절차([사실], f5, ref-153)와 이를 노드 허용 편차와 비교해 도착 판정 전에 확인하는 단계([추정], f20), 사전 지도 작성 주행을 줄인 연구(IFC 기반 지도 생성 [사실] f10 ref-647, 연계 대상: CAD 기반 가상 지도 위치추정 f9 ref-628)."
    }
  ]
}
```

### runs/2026-09-25-72/pages/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md
...
### runs/2026-09-25-72/pages/tracks/floorplan-recognition/space-graph-schema-draft.md
...
### runs/2026-09-25-72/pages/tracks/floorplan-recognition/index.md
...
### runs/2026-09-25-72/pages/ideas/floorplan-recognition.md
...
### runs/2026-09-25-72/verification2.json
...
- 수정 지시(required_fixes):
    - 단계 4 페이지 3절 '현장 시나리오: 3층 출하 대기장 도착 판정' 표의 제약 칸: '금지 구역·속도 제한은 로봇 쪽 비용 지도 필터로 적용된다. [사실][^ref-644]'를 'Nav2 를 쓰는 로봇에서는 금지 구역·속도 제한이 로봇 쪽 비용 지도 필터로 적용된다. [사실][^ref-644]'처럼 Nav2 로 한정하거나 [추정]으로 낮춘다 — ref-644(f12)는 Nav2 비용 지도의 기능만 뒷받침하며, 시나리오의 VDA 5050 로봇·Open-RMF 로봇 모두에 적용된다는 일반화는 브리프에 근거가 없다(태그 드리프트).
    - 단계 4 페이지 6절 완료 조건 표 첫 행(보정 항목 목록의 반영): 이번 실행의 페이지에서 보정 항목 목록(f17, [추정])이 공간 그래프 스키마 초안 6절과 아이디어 3. 건축 도면 자동 인식 5절에 실제로 실렸음을 2차에서 확인했으므로, 충족 여부를 '충족', 검증 판정을 '충족(2차 확인)'으로, 근거 칸의 '검증 판정 전이다'를 '두 페이지에 반영됨(q4-01 답, 종합은 추정)'으로 고친다. 둘째 행(도면–현장 정합 절차 초안)은 '미충족 · 미승인'을 유지하고, 표 아래 '다음 단계로 전환: 아니오(…)' 줄과 상태 줄의 '완료 조건: 미충족'·'진행 중'은 그대로 둔다 — 단계 전체 완료 조건은 여전히 미충족이며 전환은 미승인이다.
- 검증 노트: 판정: 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(mirror_only: raw.githubusercontent.com 원본 7건과 입력 원문 텍스트 1건은 직접 확인). 확인 17건, 미확인 3건(f7·f10·f14 부분 불일치), 교차 확인 0건. 강등: f7 사실 → 일부 추정(창문·문·가구 제외 부분), f10 사실 → 일부 추정(흑백 지도 세부), f14 사실 → 일부 추정(거울·금속 반사 부분). 원문 미열람 출처: ref-081, ref-628, ref-224, ref-645, ref-646, ref-647, ref-648. 주의: q4-01 의 답(보정 여섯 묶음, 용도별 지도 분리, ROP 경계, 분류 원문 질문 연결)은 이 위키의 종합 [추정]이며 이를 제시한 단일 출처가 없다. 위치추정·SLAM·비용 지도 인플레이션은 로봇 쪽 연계 대상이다. 브리프의 ref-031 은 fetched_via github_raw·fetch_url null 로 기록됐으나 실제 근거는 입력 원문 텍스트(inbox)다. f9·f16 은 이전 실행 확인분을 재사용했다. 온톨로지 변경 승인: 층별 지도 속성 '내비게이션 지도 메타데이터(Nav2 YAML: 해상도·원점·점유/빈 임계값)'(f1) / 거부: 층별 지도 속성 '로봇 지도 좌표계 변환'(f5 — 6절 정렬 정보 질문 q4-03 을 근거 없이 결정하므로 6절 근거 보강으로 둠). 단계 완료 조건: 미충족(부족: 도면–현장 정합 절차 초안 없음; 보정 항목 목록은 이번 실행에서 스키마 초안 6절·아이디어 5절에 반영됨을 2차에서 확인; 막힌 질문 q4-02·q4-03·q4-04·q4-05·q4-07·q4-08·q4-09·q4-10). 단계 전환: 미승인(완료 조건 미충족·막힌 질문 있음). / 2차 수정 후 재검증. 드리프트 1건(단계 4 페이지 시나리오 제약 칸의 Nav2 비용 지도 필터 일반화) 수정 지시, 1차 수정 지시 17항목은 모두 이행 확인(f7 분리, f10·f14 축소, ref-648 정정, 연계 대상 표시, 78% 미기재, 온톨로지 v1.0 승인·거부 반영, 원문 미열람 각주, 새 질문 q4-09·q4-10 등록). [분류원문] 보존, 섹션 순서 준수, 링크 유효. 참고: 3절 종합 표의 '맡는 쪽(추정)' 열은 표현 보정 전체를 로봇 쪽으로 두어 f19(장애물 채움·인플레이션만 명시)보다 넓게 읽힐 수 있다 — [추정] 표이므로 수정 지시하지 않는다.
