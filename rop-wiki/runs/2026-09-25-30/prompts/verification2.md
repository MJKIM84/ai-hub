...
- 대상: 트랙 nl-task-chatbot (자연어 업무 지시 챗봇) · 현재 단계: 단계 1. 선행 연구·제품 사례 조사 · 이번에 다룰 백로그 질문 id: q1-04 · 중심 세부영역: 13. 작업 배정 — MRTA (D. 계획·최적화)
- 예산:
...
### runs/2026-09-25-30/target.json
...
```json
{
  ...
  "selection_rationale": "CLI 지정 run_type=track, area=13; 트랙 실행일(Fri, track_days 앞 3개) → 트랙 nl-task-chatbot 단계 1, 질문 q1-04 (CLI 지정 질문 id)"
}
```

### runs/2026-09-25-30/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q1-04 자연어 지시에서 장소·대상 화물·긴급도·기한 같은 상황 정보를 뽑아내는 기존 방법은 무엇이고, 빠진 정보는 어떻게 되묻는가?",
    "과업 지향 대화 시스템의 의도 인식·슬롯 채우기와 필수 슬롯 되묻기(폼) 방식은 어떻게 동작하는가? (단계 1 페이지 3절, 18. 사람–로봇 협업·운영 인터페이스 겨냥)",
    "로봇 지시에서 장소 표현·기한 같은 시간 조건을 추출해 구조화하는 연구는 무엇이 있는가? (업무 분해·배정 설계 초안 '상황' 속성 겨냥)",
    "LLM 기반 로봇·에이전트가 지시의 모호성이나 빠진 인자를 탐지하고 되묻는 방법(불확실성 추정, 등각 예측, 되묻기 프롬프트)과 그 평가 결과는 무엇인가? (27. AI·학습·적응과 모델 운영 겨냥)",
    "LLM 출력을 정해진 스키마(필수 필드)에 맞추는 구조화 출력 기능은 빠진 값을 어떻게 다루는가? (단계 4 확인 절차와 연결)",
    "국내 연구기관·대학의 지시 모호성 판별·되묻기 연구나 물류 현장 사례가 있는가? (한국 자료 우선 규칙)"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "과업 지향 대화 시스템의 자연어 이해는 발화의 의도를 분류하는 의도 인식(intent detection)과 발화의 각 토큰에 의미 유형을 붙여 인자 값을 뽑는 슬롯 채우기(slot filling)의 두 하위 과제로 이루어지며, 두 과제를 함께 학습하는 결합 모델이 연구되어 왔다.",
      "tag": "사실",
      "source_ids": [
        "ref-565"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: SLU 는 intent detection 과 slot filling 두 하위 과제로 구성, 전통적으로 따로 풀다가 결합 모델로 발전. ATIS·SNIPS 가 대표 벤치마크. ACM Computing Surveys(DOI 10.1145/3547138). 원문 미열람.",
      "as_of": "2022",
      "flow_step": null,
      "flow_item": "시작 조건",
    ...
    {
      "id": "f2",
      "claim": "오픈소스 대화 프레임워크 Rasa 의 폼(form)은 도메인 파일에 필수 슬롯(required_slots)을 정해 두고, 활성화되면 비어 있는 다음 필수 슬롯을 utter_ask_<슬롯 이름> 응답으로 사용자에게 묻고, 추출한 값을 사용자 정의 검증 동작으로 검사하며, 필수 슬롯이 모두 채워지면 비활성화된다.",
      "tag": "사실",
      "source_ids": [
        "ref-564"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "forms.mdx 원문: 'the form gets activated and will prompt the user for the next required slot value.' 응답 이름 utter_ask_<form_name>_<slot_name> 또는 utter_ask_<slot_name>. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "시작 조건"
    },
    {
      "id": "f3",
      "claim": "Lang2LTL 은 명령에서 공간·시간 지칭 표현을 인식하고, 그 표현을 랜드마크의 텍스트·시각 설명으로 실제 환경의 장소에 접지한 뒤, 명령을 선형 시간 논리(LTL) 식으로 옮기는 모듈 구조를 쓰므로, 장소 표현 추출과 장소 접지를 별도 단계로 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-055"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README 원문(master): 'uses both textual and visual descriptions of landmarks to ground spatiotemporal navigation commands in novel indoor and outdoor environments'. 번역은 T5 미세조정 또는 GPT-3/4. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "작업 대상"
    },
    {
      "id": "f4",
      "claim": "DELIVER(2025)는 말이나 글로 된 지시를 경량 LLaMA3 가 해석해 픽업·배송 위치를 뽑고, 보로노이 분할로 로봇별 구역을 나눈 뒤 경계의 중계 지점에서 화물을 넘기는 다중 로봇 픽업·배송 체계를 ROS 2 Gazebo 와 TurtleBot3 실물로 검증했다.",
      "tag": "사실",
      "source_ids": [
        "ref-568"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 'a lightweight instance of LLaMA3 interprets the command to extract pickup and delivery locations'; 유한 상태 기계로 실행, MultiTRAIL 플랫폼. 원문 미열람.",
      "as_of": "2025-08",
      "flow_step": null,
      "flow_item": "작업 대상",
      "source_unopened": true
    ...
    {
      "id": "f5",
      "claim": "Sucker·Neubauer·Henrich(IEEE IRC 2024)는 '몇 분 뒤에 시작' 같은 모호한 시간 요구를 담은 지시를 시작 시각별 사용자 만족도를 나타내는 만족도 함수를 가진 퍼지 기술(fuzzy skill)로 표현하고, 사용자 연구에서 사다리꼴 함수가 종 모양보다 잘 맞으며 그 모양이 전치사에 따라 체계적으로 달라진다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-569"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: satisfaction functions 'representing vague execution time requirements'; 여러 퍼지 기술의 시간 허용 창으로 만족도 기반 스케줄링. 저자 보고, 원문 미열람.",
      "as_of": "2024-11",
      "flow_step": null,
      "flow_item": "제약",
      "source_unopened": true
    ...
    {
      "id": "f6",
      "claim": "Chen 외의 언어 모델 기반 상식 추론(LMCR, ICRA 2020)은 자연어 지시를 동사 프레임으로 파싱한 뒤, 빠진 정보를 사용자에게 묻지 않고 주변에서 관찰한 객체와 대규모 텍스트로 학습한 언어 모델의 상식 추론으로 채운다.",
      "tag": "사실",
      "source_ids": [
        "ref-566"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 'automatically fill in information missing from the instruction using environmental context and a new commonsense reasoning approach'. 원문 미열람.",
      "as_of": "2019-04",
      "flow_step": null,
      "flow_item": "작업 대상",
      "source_unopened": true
    ...
    {
      "id": "f7",
      "claim": "고려대 등의 CLARA(IEEE RA-L 2024)는 LLM 불확실성 추정으로 로봇 명령을 명확한 명령과 불확실한 명령으로 나누고, 불확실한 명령을 상황 맥락으로 모호한 명령과 수행 불가능한 명령으로 다시 구분해, 모호한 명령에는 LLM 이 만든 질문으로 사용자와 대화해 모호성을 풀고 수행 불가능한 명령은 거절한다.",
      "tag": "사실",
      "source_ids": [
        "ref-560",
        "ref-561"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "프로젝트 페이지 원문: SaGC 데이터셋 15개 장면(요리·청소·마사지 로봇), 탁상 집기·놓기 시뮬레이션, 실물 건네기 실험. 저자 Park, J. 외(고려대·연세대·Google Research). 논문과 같은 저자라 독립 교차 아님.",
      "as_of": "2024",
      "flow_step": null,
      "flow_item": "예외·성과"
    },
    {
      "id": "f8",
      "claim": "KnowNo(CoRL 2023)는 LLM 계획기가 단계마다 여러 선택지를 만들게 하고 등각 예측(conformal prediction)으로 정한 문턱을 넘는 선택지가 둘 이상이면 사람에게 도움을 요청하며, 사용자가 정한 수준의 작업 성공을 통계적으로 보장하면서 사람 개입을 줄인다고 밝힌다.",
      "tag": "사실",
      "source_ids": [
        "ref-558",
        "ref-559"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "프로젝트 페이지 원문: 'KnowNo builds upon conformal prediction (CP) to formally quantify the LLM uncertainty'. 공간·수치 모호성, 사람 선호 등 과업으로 평가. Google DeepMind·Princeton. 논문과 같은 저자.",
      "as_of": "2023-07",
      "flow_step": null,
      "flow_item": "예외·성과"
    },
    {
      "id": "f9",
      "claim": "Wang 외(EMNLP 2025)는 실제 지시를 분석해 LLM 에이전트가 불명확한 지시에서 빠진 도구 호출 인자를 임의로 지어내는 경향이 있다고 보고하고, 불명확한 지시 데이터셋 NoisyToolBench 와, 장애를 만나면 사용자에게 묻게 하는 Ask-when-Needed(AwN) 프롬프트 틀을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-567"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 'LLM agents tend to arbitrarily generate missed arguments, which may lead to hallucinations and risks'; AwN 으로 도구 호출·최종 응답 정확도 개선(저자 보고). 원문 미열람.",
      "as_of": "2025",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f10",
      "claim": "AmbiK 는 주방 환경의 모호한 로봇 지시와 그 모호하지 않은 짝 1000쌍(2000개 과업)을 모은 텍스트 데이터셋으로, 모호성 유형(사람 선호·상식·안전), 환경 설명, 명확화 질문과 답, 사용자 의도, 과업 계획을 담고 KnowNo·LofreeCP·LAP·Binary·NoHelp 다섯 방법의 실험 틀을 제공한다.",
      "tag": "사실",
      "source_ids": [
        "ref-562",
        "ref-563"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README 원문: '1000 pairs of ambiguous tasks and their unambiguous counterparts'; 과업마다 직접·간접(바꿔 말한)·모호 세 표현. 논문(ACL 2025)과 같은 저자 계열.",
      "as_of": "2025",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f11",
      "claim": "AmbiK 논문 저자들은 KnowNo·LAP·LofreeCP·Binary 같은 기존 모호성 탐지 방법이 모호한 과업과 모호하지 않은 과업을 대부분 구분하지 못했고, 모호성 구분 점수가 대부분 10% 미만(가장 높은 것은 Llama-3-8B 에서 LofreeCP 44%, KnowNo 40%)이었다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-563"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: AmbDif 대부분 10% 미만, 세트 크기 정확도(SSC)는 어느 방법도 20% 초과 못함. 저자 보고값, 주방 텍스트 과업 조건, 독립 재현 미확인. 원문 미열람.",
      "as_of": "2025",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f12",
      "claim": "OpenAI 는 구조화 출력(Structured Outputs) 기능이 모델 출력을 개발자가 준 JSON 스키마에 맞추도록 보장해 필수 키 누락이나 잘못된 열거 값을 막는다고 설명한다.",
      "tag": "추정",
      "source_ids": [
        "ref-570"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "벤더 주장: 발표문 검색 요약 'model will always generate responses that adhere to your supplied JSON Schema'. 엄격 모드에서는 모든 필드를 필수로 두고 선택 값은 null 허용 유형으로 표현한다는 설명은 제3자 요약에만 있음. 원문 미열람.",
      "as_of": "2024-08",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true,
    ...
    {
      "id": "f13",
      "claim": "q1-04 에 대해 확인한 기존 방법은 (1) 의도·슬롯을 미리 정의하고 비어 있는 필수 슬롯을 차례로 묻는 슬롯 기반 방식(의도 인식·슬롯 채우기, Rasa 폼), (2) 빠진 정보를 환경 관찰과 상식 추론으로 스스로 채우는 방식(LMCR), (3) LLM 의 불확실성·빠진 인자를 탐지해 필요할 때만 되묻는 방식(KnowNo, CLARA, AwN)으로 나뉘는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-565",
        "ref-564",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f1·f2·f6·f7·f8·f9 를 빠진 정보 처리 방식으로 묶은 이 위키의 정리. 이 3분류를 제시한 단일 출처는 확인하지 못함. 장소·시간 추출(f3·f4·f5)은 세 방식 모두의 앞단에 해당.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "시작 조건",
    ...
    {
      "id": "f14",
      "claim": "이번에 확인한 상황 정보 추출·되묻기 연구의 평가 환경은 주방·가정(AmbiK, CLARA), 도구 호출 API(NoisyToolBench), 실내·도시 내비게이션(Lang2LTL)이었고, 물류에 가까운 예는 픽업·배송 위치만 뽑는 DELIVER 뿐이어서 화물 식별자·긴급도·기한을 필수 항목으로 둔 물류 지시 추출·되묻기 연구나 데이터셋은 이번 검색 범위에서 찾지 못했다.",
      "tag": "추정",
      "source_ids": [
        "ref-562",
        "ref-560",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f3·f4·f7·f9·f10 의 평가 환경 정리. 한국어 물류 지시 검색 2회에서도 해당 연구를 찾지 못함. 검색 범위의 결과이며 부재의 확인이 아님(q1-05·q1-06·q2-03 연결).",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": false
    ...
    {
      "id": "f15",
      "claim": "업무 분해·배정 설계 초안의 '상황' 속성(장소 표현, 대상 표현, 시간 조건)을 필수 슬롯으로 두면 값마다 지시 원문에서 읽었는지, 환경·상식으로 추론했는지, 사용자에게 되물어 얻었는지를 구분해 기록할 수 있고, 추론으로 채운 값(LMCR 방식)은 AwN 이 지적한 인자 지어내기와 구분되지 않아 확인 대상이 될 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-564",
        "ref-566",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f2(필수 슬롯 되묻기)·f6(추론으로 채움)·f9(빠진 인자 지어내기)를 초안 2절 '상황' 개념에 대응시킨 설계 추론. 이를 적용한 로봇 관제 사례는 확인하지 못함.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "시작 조건",
      "source_unopened": false
    ...
    {
      "id": "f16",
      "claim": "분류 원문 질문(가장 가까운 로봇에 맡기는 것이 전체적으로도 유리한가)과 관련해, 지시에서 뽑은 긴급도·기한은 배정의 목적(가까움 대 기한 준수)을 바꾸는 입력이므로, 퍼지 시간 요구를 만족도 함수로 표현한 연구처럼 기한을 엄격 제약이 아닌 허용 창으로 스케줄러·배정기에 넘기는 구조가 13. 작업 배정 — MRTA 와 14. 작업 순서·스케줄링 사이의 연결 근거가 될 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-569"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f5 를 13. 작업 배정 — MRTA 의 SCM 질문에 대응시킨 추론. 물류 배정에서 긴급도 추출 결과를 목적함수에 넣은 연구는 찾지 못함.",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "제약",
      "source_unopened": true
  ...
  "sources": [
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "Lang2LTL 공식 코드 README. 지칭 표현 인식, 랜드마크 접지, LTL 로의 lifted 번역 모듈을 설명(재사용, 이번 실행에서 다시 열어 확인).",
      "fetched": true,
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "KnowNo 프로젝트 페이지 원본. LLM 선택지 생성, 등각 예측 문턱, 예측 집합이 둘 이상이면 도움 요청, 작업 성공의 통계적 보장, CoRL 2023 최우수 학생 논문상을 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 등각 예측으로 LLM 계획기의 불확실성을 정량화해 필요할 때만 사람 도움을 요청하는 KnowNo 논문(CoRL 2023, PMLR 229).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "CLARA 프로젝트 페이지 원본. 명령을 명확·모호·수행 불가로 분류하고 모호한 명령에 질문을 생성하는 방법, SaGC 데이터셋(15개 장면), 시뮬레이션·실물 실험을 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 불확실성 추정과 상황 맥락으로 명령의 명확·모호·수행 불가를 판별하고 모호한 명령을 질문 생성으로 해소하는 논문(IEEE RA-L 9(2), 1059-1066).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "AmbiK 데이터셋 README 원본. 모호·비모호 과업 1000쌍, 모호성 유형 3종, 명확화 질문·답·계획 필드, 다섯 탐지 방법의 실험 틀을 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-563",
      "org": "AmbiK 저자(ACL 2025, 저자 목록 미확인)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 주방 환경 모호 지시 데이터셋과 기존 모호성 탐지 방법(KnowNo, LAP, LofreeCP 등)의 비교 결과를 보고한 ACL 2025 논문.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "Rasa 폼 문서 원본. 필수 슬롯 정의, 빈 슬롯을 묻는 응답 이름 규칙, 사용자 정의 검증, 모든 슬롯이 채워지면 비활성화되는 흐름을 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-565",
      "org": "ACM Computing Surveys 게재 서베이 저자(미확인)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자연어 이해의 의도 인식·슬롯 채우기 결합 모델을 정리한 서베이(ATIS·SNIPS 등 벤치마크 포함).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 지시를 동사 프레임으로 파싱하고 빠진 정보를 환경 관찰과 언어 모델 기반 상식 추론(LMCR)으로 채우는 논문(ICRA 2020 발표).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 불명확한 지시에서 LLM 에이전트가 빠진 인자를 지어내는 문제를 분석하고 NoisyToolBench 와 Ask-when-Needed 프롬프트를 제안한 논문(EMNLP 2025).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLaMA3 로 지시에서 픽업·배송 위치를 뽑고 보로노이 분할·중계 지점으로 다중 로봇 픽업·배송을 조율하는 체계 논문.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 모호한 시간 요구를 만족도 함수를 가진 퍼지 기술로 표현하고 사용자 연구로 함수 모양을 정한 논문(IEEE IRC 2024).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "low",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 모델 출력을 개발자가 준 JSON 스키마에 맞추는 구조화 출력 기능 발표문.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
  ...
  "page_proposals": [
    {
      ...
      "rationale": "q1-04 답: f1·f2·f3·f4·f5·f6·f7·f8·f9·f10·f11·f12·f13·f14·f15·f16 (신뢰도 medium) — 2절 q1-04 상태 답함, 3절 q1-04 소제목 신설({#q1-04}): 상황 정보 추출(의도·슬롯 f1, 장소 표현 f3·f4, 모호한 시간 요구 f5), 빠진 정보 처리 세 방식(f13: 필수 슬롯 되묻기 f2, 추론으로 채움 f6, 불확실성 기반 되묻기 f7·f8·f9), 되묻기 판단의 한계(f10·f11, 저자 보고값), 구조화 출력(f12 벤더 주장), 물류 적용 공백(f14), 초안 '상황'과의 연결(f15), SCM 질문 연결(f16) / 4절 결론·불확실성 / 5절 후속 질문 / 6절 완료 조건 현황 / 8절 출처 / 9절 이력"
    },
    {
      "action": "update",
      ...
      "rationale": "아이디어 페이지 3절(트랙 산출물): '상황 정보 추출과 되묻기' 소절 신설 — 세 방식 정리 f13, 근거 f2·f6·f7·f8·f9, 되묻기 판단 한계 f11, 물류 적용 공백 f14. 구조화 출력 f12 는 [추정] 벤더 주장 병기"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신: track.ontology_changes(상황 개념에 '값 출처' 속성, 시간 조건의 모호 시간 표현)가 승인되면 2절 반영과 초안 버전 인상(f2·f5·f6·f7·f8·f15). 미승인 부분은 6절 '상황의 항목' 질문(q1-04·q2-01)에 근거로 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 1 반영 제안 (f2, f7, f8, f13): 지시를 받는 운영 인터페이스의 되묻기 방식(필수 슬롯 폼, 불확실성 기반 도움 요청, 명령의 명확·모호·수행 불가 판별)과 CLARA(국내 연구)·KnowNo"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 1 반영 제안 (f8, f9, f10, f11, f12): LLM 불확실성 정량화(등각 예측)와 빠진 인자 지어내기, 모호성 탐지 벤치마크 AmbiK 의 한계 보고, 구조화 출력(벤더 주장). 적용 대상 13. 작업 배정 — MRTA·18. 사람–로봇 협업·운영 인터페이스와 함께 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 1 반영 제안 (f4, f5, f16): 지시에서 뽑은 픽업·배송 위치와 긴급도·기한이 배정의 입력이 되는 구조, 모호한 시간 요구를 만족도 함수로 넘기는 연구와 분류 원문 질문 연결. 27. AI·학습·적응과 모델 운영과 양쪽 연결"
    }
  ],
  "glossary_candidates": [
    {
    ...
    },
    {
      ...
      "definition": "LLM 의 응답을 JSON 스키마 같은 정해진 형식의 필드와 값으로 내도록 제약하는 방식이다."
    },
    {
      "term_ko": "환각",
      "term_en": "Hallucination",
      "definition": "LLM 이 근거 없이 그럴듯한 내용을 만들어 내는 현상으로, 지시에서 빠진 인자를 임의로 채워 넣는 것도 여기에 든다."
    }
  ],
  "open_questions_new": [],
    ...
    "ontology_changes": [
      {
        ...
        "description": "주요 속성에 '값 출처'를 더한다. 값 후보: 지시 원문에서 추출(슬롯 채우기 f2) / 환경·상식으로 추론(LMCR f6) / 사용자 되묻기 응답(Rasa 폼 f2, CLARA f7, KnowNo f8). 추론으로 채운 값을 확인 대상으로 표시하는 근거(f15)가 된다. 사용자 확인을 별도 개념으로 둘지(6절 질문, q4-01·q4-04)와는 충돌하지 않는 속성 수준 제안이다."
      },
      {
        "op": "modify",
        "kind": "concept",
        ...
        "description": "속성 '시간 조건'의 값에 엄격 기한 외에 '몇 분 뒤' 같은 모호한 시간 표현을 만족도 함수(허용 창)로 담을 수 있게 한다(f5). 일정 개념의 속성과 겹칠 수 있어 일정 계산 주체(q3-01) 결정 전에는 6절 질문으로 둘지 검증 판단."
      }
    ],
...
### runs/2026-09-25-30/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): ACM Computing Surveys 55(8) Article 156, 2022-12, DOI 10.1145/3547138. 저자는 Weld, H., Huang, X., Long, S., Poon, J., & Han, S. C.로 확인되었다(arXiv 2101.08091). 스니펫이 의도 분류, 토큰별 의미 유형 표지(슬롯 채우기), 따로 풀던 두 과제가 결합 모델로 발전한 흐름을 모두 담고 있다. ref-565 기관 칸의 '저자(미확인)'와 발행일을 고쳐야 한다."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: forms.mdx를 github_raw로 다시 열었다. required_slots, 'will prompt the user for the next required slot value', utter_ask_<form_name>_<slot_name>/utter_ask_<slot_name>, validate_<form_name> 검증 동작, 필수 슬롯이 모두 채워지면 자동 비활성화된다는 설명을 확인했다. 문서에 'Changed in 3.0' 표기가 있어 Rasa 3.x 문서(main 브랜치)에 해당한다. 발행일은 미확인이므로 확인일 2026-09-25를 기준으로 한다. 오픈소스 문서의 동작 구조에 관한 진술이라 [사실]로 둘 수 있다."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "부분 불일치: README를 github_raw로 다시 열었다. 'textual and visual descriptions of landmarks' 문장의 주어는 후속판 Lang2LTL-2이고 원래의 Lang2LTL이 아니다. Lang2LTL이 지칭 표현을 인식하고 접지한 뒤 LTL로 번역하는 모듈 구조는 기존 q1-01 문장(ref-055·ref-056)으로 이미 검증되어 있다. 시각 설명을 이용한 접지는 Lang2LTL-2에 관한 [추정]으로만 쓴다."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): arXiv 2508.19114이며 IEEE Xplore 게재 기록(SII 2026)이 있다. 경량 LLaMA3로 픽업·배송 위치 추출, 보로노이 분할, 경계의 중계 지점, 유한 상태 기계, MultiTRAIL, ROS 2 Gazebo와 TurtleBot3 실물 검증을 스니펫으로 확인했다. 저자 목록은 여전히 미확인이다."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): IEEE IRC 2024(Tokyo), arXiv 2024-11-14. 만족도 함수, 사다리꼴 함수가 사용자 만족도를 가장 잘 근사한다는 결과, 전치사(in/before/after)에 따라 모양이 달라진다는 결과를 확인했다. '종 모양보다'라는 비교 대상은 이번 스니펫에서 확인하지 못했으므로 뺀다는 조건으로 유지한다. 저자 보고 결과다."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): 저자 Chen, H., Tan, H., Kuntz, A., Bansal, M., & Alterovitz, R., ICRA 2020(IEEE Xplore 9197315). 동사 프레임 파싱, 주변 객체 관찰, 텍스트 말뭉치로 학습한 언어 모델의 상식 추론으로 빠진 정보를 자동으로 채운다는 내용을 확인했다. '사용자에게 묻지 않고'는 스니펫에 없는 해석이므로 삭제하는 조건으로 유지한다."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: CLARA 프로젝트 페이지를 github_raw로 다시 열었다. 불확실성 추정으로 명확·불확실을 판별하고, 상황 맥락으로 모호·수행 불가를 구분하며, 모호한 명령에는 질문을 생성한다는 흐름을 확인했다. SaGC 15개 장면(요리·청소·마사지), RA-L 게재와 ICRA 2024 발표, 고려대·연세대·Google Research 소속도 확인했다. 논문(ref-561)과 저자가 같아 독립 교차 확인은 아니다. 수행 불가 명령의 '거절'은 브리프 요약 기준이다."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: KnowNo 프로젝트 페이지를 github_raw로 다시 열었다. 'KnowNo builds upon conformal prediction', 예측 집합에 선택지가 둘 이상이면 도움을 요청하는 규칙, 작업 성공의 통계적 보장과 개입 최소화, Google DeepMind·Princeton, CoRL 2023 최우수 학생 논문상을 확인했다. 논문(ref-559)과 저자가 같다."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): EMNLP 2025 main(ACL Anthology 2025.emnlp-main.1104) 게재를 확인했다. 실제 지시 분석, 빠진 인자를 임의로 생성하는 경향('may lead to hallucinations and risks'), NoisyToolBench, AwN, ToolEvaluator를 확인했다. AwN의 성능 개선은 저자 보고다."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: README를 github_raw로 다시 열었다. 1000쌍(2000개 작업), 모호성 유형 3종(사람 선호·상식·안전), 환경 설명, 명확화 질문과 답, 사용자 의도, 계획, KnowNo·LofreeCP·LAP·Binary·NoHelp 실험 틀을 확인했다. 게재처(ACL 2025, 2025.acl-long.1593, arXiv 2506.04089)는 검색 결과로 확인했다."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): 검증 검색 스니펫에서 AmbDif 대부분 10% 미만, Llama-3-8B에서 LofreeCP 44%·KnowNo 40%를 확인했다. 비교 모델은 GPT-3.5, GPT-4, Llama-2-7B, Llama-3-8B다. 단일 출처의 저자 보고값이므로 '저자 보고값, 독립 재현 미확인, 주방 텍스트 작업 조건'을 같은 문장에 적는 조건으로 [사실]을 유지한다. dblp 기록으로 보아 ref-563의 제1저자는 Ivanova다."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 결과 일치): 발표문 스니펫 'always generate responses that adhere to your supplied JSON Schema ... omitting a required key, or hallucinating an invalid enum value'를 확인했다. OpenAI 개발자 가이드도 같은 발행 주체라 독립 확인이 아니다. [추정] 벤더 주장을 유지한다."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 이 위키의 종합 분류이며 단일 출처가 없다는 점이 명시되어 있어 [추정]을 유지한다. 근거 가운데 ref-565·ref-566·ref-567은 원문 미열람인데 브리프의 source_unopened: false 표시는 이와 맞지 않는다. q1-01 여섯 유형(분해 결과 형태), q1-02 세 방식(LLM 담당 범위)과는 축이 다르다(빠진 정보 처리 방식)."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 평가 환경을 정리한 것이며 부재의 확인이 아님이 명시되어 있다. 기존 q1-01·q1-02의 평가 환경 정리와 결론이 같고 모순이 없다(q1-05·q1-06·q2-03 연결)."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 설계 추론이며 적용 사례가 없다는 점이 명시되어 있어 [추정]을 유지한다. f6의 '묻지 않고' 구절을 빼도 추론은 성립한다."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: f5를 13. 작업 배정 — MRTA의 분류 원문 질문에 대응시킨 추론이라 [추정]을 유지한다. 근거가 단일 원문 미열람 출처다. flow_step '출하'는 근거 없이 붙은 값이므로 페이지 시나리오에 쓰지 않는다."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "f3(Lang2LTL)은 단계 1 페이지 q1-01의 Lang2LTL 문장과 겹친다. 모듈 구조는 기존 문장과 각주 ref-055·ref-056을 재사용하고 새 각주를 만들지 않는다.",
      "f4 DELIVER는 실행 2026-09-25-21 브리프에서 '검색 목록에만 나와 제외'로 기록된 연구다. 이번에 출처로 들어온 것이며 모순은 없다.",
      "f13(빠진 정보 처리 세 방식)은 q1-01 여섯 유형(분해 결과 형태), q1-02 세 방식(LLM 담당 범위)과 축이 다른 세 번째 분류다. 본문에서 축의 차이를 밝힌다.",
      "f14는 q1-01·q1-02의 평가 환경 한계 서술(물류 지시 연구 미확인)과 결론이 같다. 백로그 q1-05·q1-06·q2-03과 이어진다.",
      "f8(KnowNo의 통계적 보장)과 f11(AmbiK에서 KnowNo의 낮은 구분 점수)은 평가 조건과 지표가 달라 모순이 아니다. 함께 쓸 때 조건을 명시한다."
    ]
  },
  "terminology": {
    "ok": false,
    "conflicts": [
      "f5의 '퍼지 기술(fuzzy skill)'은 용어집 등록어 '스킬(Skill)'과 다르다. 본문에서는 '퍼지 스킬(fuzzy skill)'로 쓴다.",
      "f1의 '과업 지향 대화 시스템'과 f10의 '2000개 과업'은 이 트랙이 통일한 '작업' 표기와 다르다(실행 2026-09-25-12·21 검증 지시). '작업 지향 대화 시스템(task-oriented dialogue system)', '2000개 작업'으로 쓴다.",
      "용어 후보 '환각'의 정의 가운데 '빠진 인자를 임의로 채워 넣는 것도 여기에 든다'는 f9 출처가 '환각으로 이어질 수 있다'고 말한 것을 정의에 포함한 과장이다. 정의에서 빼고 관련 설명으로만 둔다."
    ]
  },
  "quotation_check": {
    "ok": true
  ...
  "required_fixes": [
    "f3: [사실] → 부분 강등. Lang2LTL 문장은 '지칭 표현 인식 → 랜드마크 접지 → LTL 번역' 모듈 구조만 [사실]로 쓰고, 단계 1 페이지 q1-01의 기존 문장과 각주 ref-055·ref-056을 재사용한다. '텍스트·시각 설명으로 접지'는 후속판 Lang2LTL-2의 설명이므로 Lang2LTL에 붙이지 않는다. 쓰려면 'Lang2LTL-2는 … 쓴다고 README가 소개한다'로 [추정][^ref-055]에 둔다. 이유: README 원문에서 그 문장의 주어가 Lang2LTL-2다.",
    "f5: '사다리꼴 함수가 종 모양보다 잘 맞으며'에서 '종 모양보다'를 삭제하고, '사다리꼴 함수가 사용자 만족도를 가장 잘 근사했다(저자 보고, 사용자 연구 조건)'로 쓴다. '퍼지 기술'은 '퍼지 스킬(fuzzy skill)'로 표기한다. 이유: 비교 대상이 검증 스니펫에 없고, 용어집 등록어가 '스킬'이다.",
    "f6: '빠진 정보를 사용자에게 묻지 않고'를 삭제하고 '빠진 정보를 주변 관찰 객체와 언어 모델의 상식 추론으로 자동으로 채운다'로 쓴다. 되묻기 방식과의 대비는 f13의 [추정] 문장(이 위키의 정리)에서만 한다. 이유: '묻지 않는다'는 출처 스니펫에 없는 해석이다.",
    "f11: 수치(대부분 10% 미만, Llama-3-8B에서 LofreeCP 44%·KnowNo 40%)를 쓸 때 같은 문장에 '저자 보고값, 독립 재현 미확인, 주방 텍스트 작업(AmbiK) 조건'을 적는다. f8(KnowNo의 통계적 보장)과 같은 절에 쓸 때는 두 결과의 평가 조건·지표가 다르다는 점을 밝히고, 한쪽이 다른 쪽을 반박한다고 쓰지 않는다.",
    "f12: 모든 페이지(단계 1 페이지 3절, 아이디어 2 페이지 3절, 27. AI·학습·적응과 모델 운영 반영 제안)에서 [추정]과 '벤더 주장'을 병기한다. 엄격 모드(모든 필드 필수, null 허용 유형) 설명은 브리프가 claim에서 뺐으므로 본문에 넣지 않는다.",
    "용어: 본문에서 '과업 지향 대화 시스템'은 '작업 지향 대화 시스템(task-oriented dialogue system)'으로, '과업'은 '작업'으로 통일한다(예: AmbiK '2000개 작업'). 의도 인식(intent detection)·슬롯 채우기(slot filling)·구조화 출력(structured output)·등각 예측(conformal prediction)은 첫 등장 때 영문을 병기한다. LLM 에이전트는 기존 용어집 링크를 쓴다.",
    "glossary_updates: 의도 인식·슬롯 채우기·구조화 출력·환각 4건을 신규 등록한다. '환각' 정의에서 '빠진 인자를 임의로 채워 넣는 것도 여기에 든다'를 빼고 'LLM이 근거 없이 그럴듯한 내용을 만들어 내는 현상'까지만 둔다. 빠진 인자 생성과의 관계는 'Wang 외(EMNLP 2025)는 빠진 인자를 임의로 생성하는 경향이 환각과 위험으로 이어질 수 있다고 본다'는 설명으로 용어 페이지 본문에 둔다.",
    "reference_updates: ref-565의 기관을 'Weld, H., Huang, X., Long, S., Poon, J., & Han, S. C.'로, 발행일을 '2022-12'로 고친다. ref-563의 기관은 'Ivanova, A. 외(AmbiK 저자, dblp 기록 기준)'로 고친다. ref-561 요약에 'ICRA 2024 발표'를, ref-568 요약에 'SII 2026 게재(IEEE Xplore)'를 더한다.",
    "원문 미열람 표시: ref-559·ref-561·ref-563·ref-565·ref-566·ref-567·ref-568·ref-569·ref-570의 각주 정의 접근일 뒤에 ' (원문 미열람)'을 붙이고, reference_updates에 source_unopened: true를 넣는다. github_raw로 원문을 연 ref-055·ref-558·ref-560·ref-562·ref-564에는 붙이지 않는다.",
    "f2: Rasa 폼 서술에 'Rasa 3.x 문서(main 브랜치, 확인일 2026-09-25 기준)'를 적는다.",
    "f16: 흐름 단계 '출하'는 근거가 없으므로 현장 시나리오·흐름 매트릭스 칸을 채우는 데 쓰지 않는다(flow_matrix_updates에 넣지 않는다).",
    "온톨로지 변경 1(상황 (Situation) 수정)은 승인한다. 2절 상황 행의 주요 속성에 '값 출처'를 더하고 값 후보를 '지시 원문에서 추출(f1·f2)', '환경·상식으로 추론(f6)', '사용자 되묻기 응답(f2·f7·f8)'으로 둔다. 근거 출처 칸에 finding id와 각주(ref-564·ref-565·ref-566·ref-560·ref-558)를 적고, 상황 행 상태를 초안 → 확정으로 바꾼다. 초안 버전은 '0.2' → '0.3'으로 올리고 H1·auto 상태 줄 원천·프런트매터 ontology_version·track_updates.ontology_draft_version 네 곳을 맞춘다. '추론으로 채운 값을 확인 대상으로 표시한다'(f15)는 설계 추론이므로 속성 정의에 넣지 않고, 6절 '상황의 항목' 질문에 [추정] 근거로 덧붙인다.",
    "온톨로지 변경 2(상황의 시간 조건에 모호 시간 표현·만족도 함수 허용)는 반영하지 않는다. 6절 미해결 모델링 질문에 '모호한 시간 표현(예: 몇 분 뒤)을 상황의 시간 조건과 일정 개념 가운데 어디에 만족도 함수(허용 창)로 둘 것인가 — 관련: q3-01, q2-01'로 두고 f5 근거를 [사실][^ref-569]로 단다. 이유: 일정 개념의 속성과 겹치고 일정 계산 주체(q3-01)가 정해지지 않았으며, 근거가 원문 미열람 단일 출처다.",
    "새 질문 2건은 backlog_updates에 등록한다(단계 4의 f11 파생 질문, 단계 3의 f6 파생 질문). origin은 각각 f11·f6이다. 단계 3 질문은 q3-03(온톨로지 질의 결과에 따른 되묻기)·q2-01(정보 출처)과 이어지되 '추론 허용 항목과 되묻기 필수 항목의 기준'을 묻는 점이 다르다고 단계 1 페이지 5절에 적는다.",
    "단계 1 페이지: 2절 q1-04를 '답함'(답한 실행 2026-09-25-30, 답 위치 #q1-04)으로 바꾸고, 3절에 '### q1-04 … {#q1-04}' 소제목을 둔다. 상태 줄은 '열린 질문: 2건 · 답한 질문: 4건'으로 한다. 6절 완료 조건 표는 첫째 항목 '충족(1차 예비, 2차에서 확정)', 둘째 항목 '미충족 · 미승인'으로 두고 아래 줄은 '다음 단계로 전환: 아니오(지시 분해 접근 유형 목록이 업무 분해·배정 설계 초안에 미반영, 열린 질문 q1-05·q1-06)'로 쓴다. q1-05·q1-06 중복 정리가 필요하다는 기존 문장은 유지한다.",
    "아이디어 2 페이지 3절: '상황 정보 추출과 되묻기' 소절에 f13(이 위키의 정리, [추정]), f2·f6·f7·f8·f9의 근거, f11(저자 보고값 조건), f14(물류 적용 공백, 부재의 확인 아님), f12([추정] 벤더 주장)만 쓴다. f3은 위 수정 지시를 따른다.",
    "세부영역 13. 작업 배정 — MRTA, 18. 사람–로봇 협업·운영 인터페이스, 27. AI·학습·적응과 모델 운영 페이지는 직접 고치지 않는다. area_reflection_proposals로만 내고, 13·18과 27에는 교차 규칙에 따라 서로 연결을 표시한다."
  ],
  "confidence": "low",
  "verification_note": "판정: 조건부 승인. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(fetch_mode mirror_only). GitHub 공식 원문 5건(ref-055·ref-558·ref-560·ref-562·ref-564)은 다시 열어 확인했다. 확인 15건, 미확인 1건(f3), 교차 확인 0건. 강등: f3 사실 → 부분 강등. '텍스트·시각 설명으로 접지'는 후속판 Lang2LTL-2의 설명이며, 모듈 구조는 기존 q1-01 문장으로 유지한다. f5('종 모양보다')와 f6('사용자에게 묻지 않고')은 출처에 없는 구절을 빼는 조건으로 유지했다. 원문 미열람 출처: ref-559, ref-561, ref-563, ref-565, ref-566, ref-567, ref-568, ref-569, ref-570. 주의: q1-04의 결론(빠진 정보 처리 세 방식, f13)은 이 위키의 정리이며 단일 출처가 없다. AmbiK 수치(모호성 구분 점수 대부분 10% 미만, 최고 44%·40%)는 주방 텍스트 작업 조건의 저자 보고값이다. 확인한 연구의 평가 환경은 가정·도구 호출·내비게이션이며, 화물·긴급도·기한을 다룬 물류 지시 추출·되묻기 연구는 이번 검색 범위에서 찾지 못했다(부재의 확인 아님). 구조화 출력은 벤더 주장이다. f13·f14·f15의 source_unopened: false 표시는 원문 미열람 출처에 기대는 실제와 맞지 않는다. 검증 검색 7회(리서치 22회 포함 29/40). 백로그의 q1-05와 q1-06은 여전히 중복 등록 상태라 정리가 필요하다. 온톨로지 변경 승인: 상황 (Situation) 수정 — 속성 '값 출처'(지시 원문 추출 f1·f2 / 환경·상식 추론 f6 / 사용자 되묻기 응답 f2·f7·f8), 초안 → 확정, v0.2 → v0.3. 거부: 상황의 시간 조건에 모호 시간 표현을 담는 수정(f5·f16) — 일정 개념과 겹치고 q3-01이 미결정이라 6절 질문으로 옮긴다. 새 질문 2건(단계 4 f11, 단계 3 f6)은 등록하며 중복이 없다. q1-04 답함 인정(f1·f2·f7·f8·f9 등). 단계 완료 조건: 미충족(부족: 지시 분해 접근의 유형 목록이 업무 분해·배정 설계 초안에 미반영). 단계 전환: 미승인(완료 조건 미충족, 막힌 질문 q1-05·q1-06).",
  "retry_reason": null,
  "track_checks": {
...
```json
{
  ...
  "outline": [
    {
      "path": "docs/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md",
      "section": "3. 조사 결과 (q1-04 소제목 신설)",
      "budget_chars": 4200,
      "summary": "자연어 지시에서 상황 정보를 뽑고 빠진 정보를 다루는 기존 방법은 필수 슬롯 되묻기, 추론으로 채움, 불확실성 기반 되묻기의 세 방식으로 정리된다(이 위키의 정리). [추정][^ref-564][^ref-566][^ref-558]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 900,
      "summary": "q1-04 결론 3항목과 불확실성(원문 미열람, 저자 보고값, 물류 지시 연구 부재 미확인)을 더한다. [추정][^ref-563]",
      "planned_findings": [
        "f7",
        "f8",
        "f11",
    ...
    },
    {
      ...
      "budget_chars": 500,
      "summary": "q3-07(추론 허용 항목과 되묻기 필수 항목의 기준)과 q4-07(물류 지시에서의 모호성 탐지 성능)을 등록한다.",
      "planned_findings": [
        "f6",
        "f11"
      ]
    },
    {
      ...
      "section": "2. 개념 목록 표",
      "budget_chars": 700,
      "summary": "상황 개념에 속성 '값 출처'를 더하고 확정으로 바꾼다(v0.3). [사실][^ref-564][^ref-566]",
      "planned_findings": [
        "f1",
        "f2",
        "f6",
    ...
    },
    {
      ...
      "budget_chars": 1800,
      "summary": "'상황 정보 추출과 되묻기' 소절을 더한다. [추정][^ref-564][^ref-566][^ref-558]",
      "planned_findings": [
        "f2",
        "f6",
        "f7",
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "q1-04 답함(3절 소제목 신설), 상태 줄·2절·4절·5절(q3-07, q4-07)·6절·7절·8절·9절 갱신"
    },
    {
      "path": "docs/tracks/nl-task-chatbot/task-model-draft.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "v0.2 → v0.3: 상황 개념에 속성 '값 출처' 추가·확정, 6절에 추론 값 확인 근거와 모호 시간 표현 질문 추가"
    },
    {
      "path": "docs/ideas/nl-task-chatbot.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "3절에 '상황 정보 추출과 되묻기' 소절 추가(q1-04, 실행 2026-09-25-30)",
      "patches": [
        {
          "section": "3. 선행 연구·제품 사례",
          "action": "append",
          ...
          "content": "(절 본문 생략 — runs/2026-09-25-30/pages/ideas/nl-task-chatbot.md 의 해당 절을 본다)"
        }
      ]
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "6. 살아있는 산출물 링크: 초안 v0.3, 아이디어 3절 q1-04 반영 현황 갱신",
      "patches": [
        {
          "section": "6. 살아있는 산출물 링크",
          "action": "replace",
          "content": "(절 본문 생략 — runs/2026-09-25-30/pages/tracks/nl-task-chatbot/index.md 의 해당 절을 본다)"
        }
      ]
    }
  ],
  "changelog_entry": "2026-09-25 | 자연어 업무 지시 챗봇 단계 1 | q1-04 답함(상황 정보 추출과 빠진 정보 처리 세 방식), 업무 분해·배정 설계 초안 v0.2 → v0.3(상황 개념에 값 출처), 새 질문 q3-07·q4-07 | run 2026-09-25-30",
  "index_updates": {
    "home_recent": "2026-09-25 — 자연어 업무 지시 챗봇 단계 1: q1-04(지시의 상황 정보 추출과 되묻기) 답함, 업무 분해·배정 설계 초안 v0.3",
    "category_recent": "2026-09-25 — 자연어 업무 지시 챗봇 단계 1: 지시에서 뽑은 위치·긴급도·기한이 13. 작업 배정 — MRTA의 입력이 되는 연구와 되묻기 방식 정리(q1-04)",
    "area_recent": "2026-09-25 — 자연어 업무 지시 챗봇 트랙 단계 1(q1-04): 픽업·배송 위치 추출(DELIVER)과 모호한 기한을 만족도 함수로 넘기는 연구를 6. 대표 접근법과 기술 반영 제안으로 냄"
  },
  "glossary_updates": [
    {
      "action": "new",
      ...
      "term_en": "Intent Recognition (Intent Detection)",
      "definition": "사용자 발화가 어떤 요청(의도)인지 미리 정한 의도 유형 가운데 하나로 분류하는 자연어 이해 과제이다.",
      "description": "작업 지향 대화 시스템의 자연어 이해는 의도 인식과 슬롯 채우기의 두 하위 과제로 이루어지며, 두 과제를 함께 학습하는 결합 모델이 연구되어 왔다(Weld 외, 2022-12).",
      "related_areas": [
        18,
        27,
        13
    ...
    },
    {
      ...
      "term_en": "Slot Filling",
      "definition": "발화에서 요청 처리에 필요한 인자 값(장소·대상·시간 등)을 찾아 미리 정한 항목(슬롯)에 채우는 자연어 이해 과제로, 비어 있는 필수 슬롯은 사용자에게 되묻는 데 쓰인다.",
      "description": "Rasa의 폼은 필수 슬롯을 정해 두고 비어 있는 다음 필수 슬롯을 사용자에게 묻고, 모든 필수 슬롯이 채워지면 비활성화된다(Rasa 3.x 문서, 확인일 2026-09-25 기준).",
      "related_areas": [
        18,
        27,
        13
    ...
    },
    {
      ...
      "term_en": "Structured Output",
      "definition": "LLM 의 응답을 JSON 스키마 같은 정해진 형식의 필드와 값으로 내도록 제약하는 방식이다.",
      "description": "OpenAI는 구조화 출력 기능이 모델 출력을 개발자가 준 JSON 스키마에 맞추도록 보장해 필수 키 누락을 막는다고 설명한다(2024-08 발표, 벤더 주장).",
      "related_areas": [
        27,
        18
      ],
    ...
    },
    {
      ...
      "description": "Wang 외(EMNLP 2025)는 LLM 에이전트가 불명확한 지시에서 빠진 인자를 임의로 생성하는 경향이 환각과 위험으로 이어질 수 있다고 본다.",
      "related_areas": [
        27,
        18,
        13
  ...
  "reference_updates": [
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "Lang2LTL 공식 코드 README. 지칭 표현 인식, 랜드마크 접지, LTL 로의 번역 모듈을 설명하고 후속판 Lang2LTL-2 를 소개한다.",
      "cited_by": [
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 불확실성 추정과 상황 맥락으로 명령의 명확·모호·수행 불가를 판별하고 모호한 명령을 질문 생성으로 해소하는 논문(IEEE RA-L 9(2), 1059-1066, ICRA 2024 발표).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "AmbiK 데이터셋 README 원본. 모호·비모호 작업 1000쌍, 모호성 유형 3종, 명확화 질문·답·계획 필드, 다섯 탐지 방법의 실험 틀을 설명한다.",
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md",
        "docs/ideas/nl-task-chatbot.md"
      ]
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "Rasa 폼 문서 원본(Rasa 3.x, main 브랜치). 필수 슬롯 정의, 빈 슬롯을 묻는 응답 이름 규칙, 사용자 정의 검증, 모든 슬롯이 채워지면 비활성화되는 흐름을 설명한다.",
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md",
        "docs/ideas/nl-task-chatbot.md",
        "docs/tracks/nl-task-chatbot/task-model-draft.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자연어 이해의 의도 인식·슬롯 채우기 결합 모델을 정리한 서베이(ACM Computing Surveys 55(8), ATIS·SNIPS 등 벤치마크 포함).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md",
        "docs/ideas/nl-task-chatbot.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLaMA3 로 지시에서 픽업·배송 위치를 뽑고 보로노이 분할·중계 지점으로 다중 로봇 픽업·배송을 조율하는 체계 논문(SII 2026 게재, IEEE Xplore).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 모호한 시간 요구를 만족도 함수를 가진 퍼지 스킬로 표현하고 사용자 연구로 함수 모양을 정한 논문(IEEE IRC 2024).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md",
        "docs/tracks/nl-task-chatbot/task-model-draft.md"
    ...
    },
    {
      ...
      "reliability": "low",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 모델 출력을 개발자가 준 JSON 스키마에 맞추는 구조화 출력 기능 발표문(벤더 주장).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md",
        "docs/ideas/nl-task-chatbot.md"
  ...
  "standards_updates": [
    {
      ...
      "summary": "오픈소스 대화 프레임워크 Rasa 의 폼 기능. 필수 슬롯을 정해 두고 비어 있는 슬롯을 사용자에게 차례로 묻고 검증한다(확인일 2026-09-25 기준).",
      "ref_id": "ref-564"
    }
  ],
  ...
  "area_reflection_proposals": [
    ...
    },
    {
      "area_no": 18,
      "section": "8. 대표 연구와 자료",
      "summary": "Rasa 폼 문서(ref-564), CLARA(고려대 등, IEEE RA-L 2024, ref-560·ref-561), KnowNo(CoRL 2023, ref-558·ref-559), 작업 지향 대화의 의도 인식·슬롯 채우기 서베이(Weld 외 2022, ref-565)"
    },
    {
      "area_no": 27,
      "section": "6. 대표 접근법과 기술",
    ...
    },
    {
      "area_no": 27,
      "section": "8. 대표 연구와 자료",
      "summary": "KnowNo(ref-558·ref-559), AmbiK 데이터셋·논문(ref-562·ref-563), Wang 외 Learning to Ask(EMNLP 2025, ref-567), LMCR(ICRA 2020, ref-566), OpenAI 구조화 출력 발표문(ref-570, [추정] 벤더 주장)"
    }
  ]
}
```

### runs/2026-09-25-30/format_check.md
...
### runs/2026-09-25-30/pages/tracks/nl-task-chatbot/stage-1-prior-work-and-products.md
...
tags: [선행 연구, 제품 사례, 작업 분해, LLM, 확인·승인, 되묻기]
status: draft
confidence: low
created: 2026-09-25
updated: 2026-09-25
...
> 단계 상태: 진행 중 · 열린 질문: 2건 · 답한 질문: 4건 · 완료 조건: 미충족 · 마지막 실행: 2026-09-25
...
이 단계의 시작 질문 4개와 실행 2026-09-25-04에서 생긴 후속 질문 2개다. q1-01은 사용자 요청의 시작 질문 문구 그대로이고, q1-02~q1-04는 구축자가 이 단계의 밝힐 것에서 정한 시작 질문이다. [가정] 상태와 답 위치는 [질문 백로그](question-backlog.md)와 일치시키며, 백로그의 "조사 중"은 이 표에서 "열림"으로 표시하고 "폐기"는 표에서 빼고 백로그에만 남긴다. 답은 3절의 질문 id로 시작하는 소제목에 실리고, 답 위치 칸에는 그 앵커 또는 주제 페이지 링크를 적는다. 제기 근거 칸에는 finding id 또는 "사용자"만 쓴다. q1-05와 q1-06은 같은 질문이 중복 등록된 것으로 보여 백로그 정리가 필요하다(실행 2026-09-25-21·2026-09-25-26·2026-09-25-30 검증 지적).
...
| q1-04 | 자연어 지시에서 장소·대상 화물·긴급도·기한 같은 상황 정보를 뽑아내는 기존 방법은 무엇이고, 빠진 정보는 어떻게 되묻는가? | 답함 | 사용자 | 2026-09-25-30 | [#q1-04](#q1-04) |
...
- 음성 피킹(voice-directed picking)에서는 시스템이 작업자에게 갈 위치와 할 일을 음성으로 지시하고, 작업자는 위치 라벨의 체크 디지트나 수량 같은 짧은 음성 응답으로 각 동작을 확인한다(확인일 2026-09-25 기준). [사실][^ref-272][^ref-275]
- Lucas Systems는 자사 음성 비서 Jennifer가 작업자에게 갈 곳과 할 일을 말하고 작업자가 짧은 응답으로 확인한다고 설명한다. [추정] 벤더 주장[^ref-272]
- 위치 체크 디지트의 생성·갱신에 관한 미국 특허 공보 US 8868519(양수인 VOCOLLECT, INC., 출원 2011-05-27, 검색 요약 기준)는 작업자가 말하거나 입력한 체크 디지트가 그 위치에 저장된 확인 값과 맞지 않으면 사용자에게 경고하는 방식을 기술한다. 특허 공보의 기술 내용이며 특정 제품의 동작을 확인한 것은 아니다. [사실][^ref-275]

#### 작업자 대상 화면 지시: 협업 피킹 로봇

- Locus Robotics는 협업 피킹 로봇에 달린 화면이 작업자에게 피킹할 품목·위치·수량을 보여 주고, 선택 기능인 피킹 검증에서는 위치나 용기 바코드를 스캔하게 한 뒤 작업자가 확인하면 로봇이 다음 목적지로 이동한다고 소개한다(확인일 2026-09-25 기준). Locus와 스캐너 협력사 Aila의 자료이며 두 회사가 협력 관계라 독립 교차 확인이 아니다. [추정] 벤더 주장[^ref-279][^ref-280]

#### 로봇 대상 자연어 지시 제품

- Amazon은 2026-06-04(발표일, 검색 요약 기준) 차세대 Proteus를 직원이 일상 언어로 할 일을 말하면 로봇이 우선순위·경로·시점을 스스로 정하는 자율이동로봇으로 발표했으며, 발표 시점에는 실험실 파일럿 단계이고 유럽 배치는 2027년 상반기로 계획했다. 기사는 같은 발표를 옮긴 것이라 독립 확인이 아니다. [추정] 벤더 주장[^ref-276][^ref-277]
- 이번에 확인한 차세대 Proteus 발표와 보도의 검색 요약에는 자연어 지시의 해석 결과를 실행 전에 작업자나 관리자에게 보여 주고 확인·승인받는 절차 설명이 없었다. 이는 검색 요약 범위의 관찰이며 부재의 확인이 아니다. [추정][^ref-276][^ref-277]
- InOrbit은 RobOps Copilot 제품 페이지에서 대화형으로 자율 주행 사건·미션 성과·로봇 상태를 개별 로봇과 플릿 단위로 탐색하게 한다고 설명한다(제품 페이지 요약 기준, 확인일 2026-09-25). 같은 제품 페이지 요약 기준으로 InOrbit Connect에서 WMS·다제조사 AMR과 연계한 미션을 정의·실행·분석한다고 밝히지만, 미션 실행 전 확인·승인 방식은 요약에 없었다(검색 요약 범위의 관찰이며 부재의 확인이 아니다). [추정] 벤더 주장[^ref-278]

#### 국내 사례

- 연계 대상: 현대로템은 2026년 5월 산업통상부·국방과학연구소 발주 국방 무인로봇 과제 '자연어 명령 기반 이종·다중 로봇 통합 관제 시스템'의 사업자로 선정되었고, 여러 무인로봇을 언어·문자로 통합 제어하는 소프트웨어를 국방 무인차량·다족보행로봇에 적용할 계획이라고 보도되었다(2026-05-26 보도). 물류 현장 제품이 아니며 확인·승인 방식은 보도 요약에 없다. [추정][^ref-281]

#### 두 확인 방식의 차이

확인한 제품에서 작업자 대상 음성·화면 지시는 체크 디지트·바코드 스캔처럼 물리 동작 하나하나를 현장에서 확인받는 방식을 쓰는 반면, 로봇 대상 자연어 지시 제품은 공개 자료에서 해석 결과(무엇을 어느 로봇이 할지)를 실행 전에 확인받는 방식이 드러나지 않아, 두 확인은 대상과 시점이 다른 것으로 보인다. 이는 여러 제품 자료를 대응시킨 이 위키의 정리(추론)이며, 이 구분을 제시한 단일 출처는 확인하지 못했다. [추정][^ref-272][^ref-275][^ref-279][^ref-276][^ref-177][^ref-178] 이 구분은 [단계 4. 오해석 방지와 확인 절차](stage-4-misinterpretation-safeguards.md)의 확인 절차 질문으로 이어진다(5절 q4-06).
...
세 방식은 여러 연구를 빠진 정보 처리 방식으로 묶은 이 위키의 정리이며, 이 분류를 제시한 단일 출처는 확인하지 못했다. [q1-01](#q1-01)의 여섯 유형(분해 결과의 형태), [q1-02](#q1-02)의 세 방식(LLM이 맡는 범위)과는 기준 축(빠진 정보 처리 방식)이 다르다. 장소·시간 표현의 추출은 세 방식 모두의 앞단에 해당한다.

| 방식 | 빠진 정보를 다루는 방법 | 조사한 사례 |
|---|---|---|
| 방식 1. 필수 슬롯 되묻기 | 의도·슬롯을 미리 정하고 비어 있는 필수 슬롯을 차례로 묻는다 | 의도 인식·슬롯 채우기, Rasa 폼 |
| 방식 2. 추론으로 채움 | 환경 관찰과 상식 추론으로 스스로 채운다 | LMCR |
| 방식 3. 불확실성 기반 되묻기 | LLM의 불확실성이나 빠진 인자를 탐지해 필요할 때만 묻는다 | KnowNo, CLARA, Ask-when-Needed |
...
- 연구 수치(LaMMA-P 105%·36%, Peng 외 82%·90%, LTAA 77%, AmbiK 10% 미만·44%·40%)는 모두 저자 보고값이며 독립 재현이 확인되지 않았다.
- LTAA의 비교 우위는 초록 요약과 다른 2차 요약이 충돌해 확정하지 못했다(열린 질문으로 올림).
- 운영 제품의 미션이 미리 정의된 미션 호출인지 새 분해인지와 실행 전 확인·권한 장치는 미확인(f22, 실행 2026-09-25-21). 이번 실행(2026-09-25-26)에서도 확인되지 않음: InOrbit RobOps Copilot(2026 발표·제품 페이지)과 Formant F3의 공개 자료에서 실행 전 확인·승인이나 명령 권한 제한 장치 설명을 찾지 못했다. 이는 검색 요약 범위의 관찰이며 부재의 확인이 아니다. [추정][^ref-177][^ref-178][^ref-278]
...
- 물류·창고 지시를 다룬 LLM 분해 연구는 확인되지 않았다(q1-05·q1-06으로 이어짐). 국내 물류 현장에서 로봇에 자연어로 일을 지시하는 제품도 이번 검색에서 찾지 못했다. q1-04 답도 물류 지시(화물 식별자·긴급도·기한)를 대상으로 한 연구 없이 가정·도구 호출·내비게이션 연구에 기댄다.
- q1-04 답은 신뢰도 low다(실행 2026-09-25-30). Weld 외 서베이, LMCR, KnowNo·CLARA·AmbiK 논문, AwN, DELIVER, 퍼지 시간 요구 연구, 구조화 출력 발표문은 원문 미열람이다. DELIVER의 저자 목록은 미확인이고, CLARA에서 수행 불가능한 명령을 '거절'한다는 부분은 브리프 요약 기준이다. 구조화 출력은 벤더 주장이다.
- 업무 분해·배정 설계 초안은 v0 → v0.1(개념 로봇 팀 추가, 실행 2026-09-25-04), v0.1 → v0.2(배정 개념에 속성 '배정 산출 방식' 추가, 실행 2026-09-25-21), v0.2 → v0.3(상황 개념에 속성 '값 출처' 추가, 초안 → 확정, 실행 2026-09-25-30)으로 올렸다. 실행 2026-09-25-26에서는 초안 변경이 없었다. 상황의 시간 조건에 모호한 시간 표현(만족도 함수)을 담는 제안은 일정 개념과 겹치고 일정 계산 주체(q3-01)가 정해지지 않아 반영하지 않고 [미해결 모델링 질문](task-model-draft.md#6-미해결-모델링-질문)으로 두었다. 확인(승인)을 별도 개념으로 둘지는 근거가 벤더 주장 수준이라 제안하지 않았다. 배정 산출 방식의 값 '규칙'은 근거 finding이 없어 넣지 않았다. 허용 동작 목록, 형식 작업 명세, 작업 사이 선행 의존 관계도 같은 절의 질문으로 남아 있다.
...
| q4-07 | 필수 슬롯 누락은 규칙(스키마)으로 검사하고 지시의 모호성은 KnowNo·CLARA 같은 불확실성 추정으로 판단하는 식으로 두 방식을 나눠 쓸 때, 가정용 벤치마크(AmbiK)에서 보고된 모호성 탐지의 낮은 구분 성능이 물류 지시(화물·장소·기한)에서도 나타나는가, 되묻기 횟수와 오배정은 어떻게 달라지는가? (q1-04 에서 파생) | 단계 4. 오해석 방지와 확인 절차 | f11(실행 2026-09-25-30) | 열림 |
| q3-07 | LMCR 처럼 환경 관찰·상식으로 빠진 정보를 스스로 채워도 되는 상황 항목(예: 가장 가까운 출하 도크)과 반드시 사용자에게 되물어야 하는 항목(예: 기한·대상 화물)을 어떤 기준으로 나누는가? (q1-04 에서 파생) | 단계 3. 구현 가설 설계 | f6(실행 2026-09-25-30) | 열림 |

실행 2026-09-25-04에서 함께 제기된 두 질문(분해 결과의 중간 표현을 작업 모델·관제 인터페이스로 옮기는 문제, 허용 동작 밖의 동작을 걸러내는 장치)은 각각 기존 백로그 질문 q2-02, q4-02와 겹쳐 새로 등록하지 않았다. 실행 2026-09-25-21에서 운영 제품이 미션 실행 전에 확인·승인을 받는지에 관한 질문은 기존 q1-03과 겹쳐 새로 등록하지 않았다. q4-06은 기존 q4-01(확인 절차)·q4-02(실행 전 검증 방법과 각각이 잡는 오류)와 이어지지만, 두 확인 방식의 조합과 확인 부담을 묻는 점이 달라 따로 등록했다(실행 2026-09-25-26 검증 판정). q3-07은 기존 q3-03(온톨로지 질의 결과에 따른 되묻기)·q2-01(정보를 어디서 가져오는가)과 이어지지만, 추론으로 채워도 되는 항목과 반드시 되물어야 하는 항목의 기준을 묻는 점이 달라 따로 등록했다(실행 2026-09-25-30 검증 판정). q4-07은 규칙 검사와 불확실성 추정을 나눠 쓸 때 물류 지시에서의 모호성 탐지 성능을 묻는다.
...
|---|---|---|---|---|---|
| 2026-09-25 | 2026-09-25-30 | q1-04 | q3-07, q4-07 | v0.2 → v0.3(상황 개념에 속성 '값 출처' 추가, 초안 → 확정) | 5 |
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-053, ref-054, ref-055, ref-056, ref-057, ref-058, ref-059, ref-061, ref-089, ref-090, ref-091, ref-092, ref-093, ref-094, ref-095, ref-087, ref-088, ref-164, ref-165, ref-166, ref-167, ref-168, ref-169, ref-170, ref-171, ref-172, ref-173, ref-174, ref-175, ref-176, ref-177, ref-178, ref-179, ref-180, ref-181, ref-242, ref-272, ref-275, ref-276, ref-277, ref-278, ref-279, ref-280, ref-281]
last_run: 2026-09-25
version: 4
---
...
### runs/2026-09-25-30/pages/tracks/nl-task-chatbot/task-model-draft.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-054, ref-055, ref-059, ref-089, ref-090, ref-091, ref-093, ref-166, ref-167, ref-169, ref-181, ref-242, ref-558, ref-560, ref-564, ref-565, ref-566, ref-567, ref-569]
last_run: 2026-09-25
version: 4
---
...
| 상황(Situation) | 지시가 전제하는 현장 조건. 장소·대상·시간 조건 같은 맥락이다. 아이디어 정의 기반 [가정] 상황의 값은 얻는 경로가 다를 수 있다. 작업 지향 대화 시스템은 발화에서 인자 값을 뽑는 슬롯 채우기(slot filling)를 쓰고 [사실][^ref-565] Rasa 폼은 비어 있는 필수 슬롯을 사용자에게 묻는다. [사실][^ref-564] LMCR은 빠진 정보를 주변 관찰 객체와 언어 모델의 상식 추론으로 채운다. [사실][^ref-566] CLARA는 모호한 명령에 질문을 만들어 사용자와 대화하고, KnowNo는 불확실할 때 사람에게 도움을 요청한다. [사실][^ref-560][^ref-558] | 장소 표현, 대상 표현, 시간 조건(단계 2에서 확정), 값 출처(값 후보: 지시 원문에서 추출 / 환경·상식으로 추론 / 사용자 되묻기 응답) | 확장 아이디어 2의 정의 문구; 값 출처: finding f1·f2 (실행 2026-09-25-30)[^ref-565][^ref-564], finding f6 (실행 2026-09-25-30)[^ref-566], finding f2·f7·f8 (실행 2026-09-25-30)[^ref-564][^ref-560][^ref-558] | 확정 |
...
- 상황의 항목(장소·대상·긴급도·기한)과, 그 가운데 무엇을 지시에서 읽고 무엇을 업무 시스템·공간 그래프·온톨로지에서 가져오는지 정해지지 않았다. — 관련: q1-04, q2-01, q3-07 [가정] 실행 2026-09-25-30에서 상황에 속성 '값 출처'를 두었다(v0.3). 상황 속성을 필수 슬롯으로 두면 값마다 지시 원문에서 읽었는지, 환경·상식으로 추론했는지, 사용자에게 되물어 얻었는지를 구분해 기록할 수 있고, 추론으로 채운 값(LMCR 방식)은 Wang 외가 지적한 빠진 인자 지어내기와 구분되지 않아 확인 대상으로 표시해야 할 것으로 보인다. 이는 설계 추론이라 속성 정의에는 넣지 않았다([단계 1 조사 결과](stage-1-prior-work-and-products.md#q1-04)). [추정][^ref-564][^ref-566][^ref-567]
- 모호한 시간 표현(예: 몇 분 뒤)을 상황의 시간 조건과 일정 개념 가운데 어디에 만족도 함수(허용 창)로 둘 것인가. Sucker 외(IEEE IRC 2024)는 모호한 시간 요구를 시작 시각별 사용자 만족도를 나타내는 만족도 함수를 가진 퍼지 스킬(fuzzy skill)로 표현했다. [사실][^ref-569] 일정 개념의 속성과 겹치고 일정 계산 주체(q3-01)가 정해지지 않았으며 근거가 원문 미열람 단일 출처라 실행 2026-09-25-30 검증에서 반영하지 않았다. — 관련: q3-01, q2-01
...
### runs/2026-09-25-30/pages/ideas/nl-task-chatbot.md
...
<!-- auto:page-status:start -->
> 페이지 상태: published · 신뢰도: low · 페이지 버전: 4 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
지시에서 장소·대상·시간 같은 상황 정보를 뽑은 뒤 빠진 정보를 다루는 기존 방법은 (1) 의도·슬롯을 미리 정하고 비어 있는 필수 슬롯을 차례로 묻는 방식, (2) 빠진 정보를 환경 관찰과 상식 추론으로 스스로 채우는 방식, (3) LLM의 불확실성이나 빠진 인자를 탐지해 필요할 때만 되묻는 방식으로 나뉘는 것으로 보인다는 것이 이 위키의 정리이며, 이 분류를 제시한 단일 출처는 확인하지 못했다. [추정][^ref-565][^ref-564][^ref-566][^ref-558][^ref-560][^ref-567] 위의 여섯 유형(분해 결과의 형태), 세 방식(LLM이 맡는 범위)과는 기준 축(빠진 정보 처리 방식)이 다르다. 이 소절은 가정·주방, 도구 호출, 내비게이션 조건의 연구에 기대므로 신뢰도가 낮다. 자세한 내용과 출처는 [단계 1. 선행 연구·제품 사례 조사](../tracks/nl-task-chatbot/stage-1-prior-work-and-products.md#q1-04)의 q1-04에 있다.

- **필수 슬롯 되묻기**: 작업 지향 대화 시스템(task-oriented dialogue system)의 자연어 이해는 의도 인식(intent detection)과 슬롯 채우기(slot filling)의 두 하위 과제로 이루어지며, 두 과제를 함께 학습하는 결합 모델이 연구되어 왔다. [사실][^ref-565] Rasa의 폼은 필수 슬롯을 정해 두고 비어 있는 다음 필수 슬롯을 사용자에게 묻고, 추출한 값을 검증 동작으로 검사하며, 필수 슬롯이 모두 채워지면 비활성화된다(Rasa 3.x 문서, main 브랜치, 확인일 2026-09-25 기준). [사실][^ref-564]
- **추론으로 채움**: LMCR(ICRA 2020)은 지시를 동사 프레임으로 파싱한 뒤 빠진 정보를 주변 관찰 객체와 언어 모델의 상식 추론으로 자동으로 채운다. [사실][^ref-566]
- **불확실성 기반 되묻기**: KnowNo(CoRL 2023)는 등각 예측(conformal prediction)으로 정한 문턱을 넘는 선택지가 둘 이상이면 사람에게 도움을 요청한다. [사실][^ref-558][^ref-559] 국내 연구인 고려대 등의 CLARA(IEEE RA-L 2024)는 LLM 불확실성과 상황 맥락으로 명령을 명확·모호·수행 불가로 나누고, 모호한 명령에는 질문을 만들어 사용자와 대화한다. [사실][^ref-560][^ref-561] Wang 외(EMNLP 2025)는 [LLM 에이전트](../glossary/llm-agent.md)가 불명확한 지시에서 빠진 도구 호출 인자를 임의로 지어내는 경향을 보고하고, 필요할 때 사용자에게 묻게 하는 Ask-when-Needed 프롬프트 틀을 제안했다. [사실][^ref-567]
- **되묻기 판단의 한계**: AmbiK 논문 저자들은 기존 모호성 탐지 방법이 모호한 작업과 모호하지 않은 작업을 대부분 구분하지 못해 구분 점수가 대부분 10% 미만이고 가장 높은 값도 Llama-3-8B에서 LofreeCP 44%, KnowNo 40%였다고 보고했으며, 이는 저자 보고값이고 독립 재현 미확인이며 주방 텍스트 작업(AmbiK) 조건의 결과다. [사실][^ref-563] KnowNo의 통계적 보장(작업 성공 수준)과 이 점수(모호성 구분)는 평가 조건과 지표가 달라 서로를 반박하는 결과로 읽지 않는다. [추정][^ref-558][^ref-563]
- **구조화 출력**: OpenAI는 구조화 출력(structured output) 기능이 모델 출력을 개발자가 준 JSON 스키마에 맞추도록 보장해 필수 키 누락을 막는다고 설명한다(2024-08 발표). [추정] 벤더 주장[^ref-570]
- **물류 적용 공백**: 이번에 확인한 연구의 평가 환경은 주방·가정, 도구 호출 API, 실내·도시 내비게이션이었고, 물류에 가까운 예는 픽업·배송 위치만 뽑는 DELIVER뿐이어서 화물 식별자·긴급도·기한을 필수 항목으로 둔 물류 지시 추출·되묻기 연구나 데이터셋은 이번 검색 범위에서 찾지 못했다. 부재의 확인은 아니다. [추정][^ref-562][^ref-560][^ref-567][^ref-055][^ref-568]
...
원천: [질문 백로그](../tracks/nl-task-chatbot/question-backlog.md)([자연어 업무 지시 챗봇](../tracks/nl-task-chatbot/index.md) 트랙) · 열림 22건 · 답함 3건
...
updated: 2026-09-25
version: 4
sources: [ref-054, ref-055, ref-057, ref-058, ref-059, ref-061, ref-089, ref-090, ref-091, ref-093, ref-094, ref-095, ref-087, ref-164, ref-166, ref-167, ref-168, ref-169, ref-170, ref-171, ref-172, ref-174, ref-175, ref-176, ref-177, ref-178, ref-179, ref-180, ref-181, ref-242, ref-272, ref-275, ref-276, ref-277, ref-278, ref-279, ref-280]
confidence: low
last_run: 2026-09-25
---
...
### runs/2026-09-25-30/pages/tracks/nl-task-chatbot/index.md
...
| [단계 4. 오해석 방지와 확인 절차](stage-4-misinterpretation-safeguards.md) | 대기 | 6 | 미충족 |
...
### runs/2026-09-25-30/docs_tree.txt
