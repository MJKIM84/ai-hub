...
- 언어: ko
- next_ref_id: ref-648
- 새 출처 id 구간: ref-648 ~ ref-677 — 이 실행 전용으로 예약한 번호다(동시에 도는 다른 실행과 겹치지 않는다). 새 출처는 ref-648 부터 순서대로 쓰고 ref-677 를 넘기지 않는다. 기존 출처는 참고문헌 목록의 id 를 그대로 쓴다
...
### data/area_reflection_proposals.json (대상 영역 13. 작업 배정 — MRTA 에 대한 트랙 반영 제안 6건, status 제안 — 반영은 이 실행에서: 사양서 6.3 절차 9·공통 규칙 11)
