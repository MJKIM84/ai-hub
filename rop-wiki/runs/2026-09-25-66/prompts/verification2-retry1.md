...
### runs/2026-09-25-66/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q3-01 스케줄링 결정은 LLM과 최적화 엔진 중 어디에 맡기는가?",
    "LLM 이 스케줄을 직접 생성할 때 제약 만족(실행 가능성)과 최적성은 해법기 대비 어느 수준이며, 어떤 조건에서 무너지는가? (단계 3 페이지 3절, 14. 작업 순서·스케줄링 겨냥)",
    "LLM 이 문제를 정식화·인스턴스화하고 해법기·계획기가 푸는 결합 구조(LLM+P, OptiMUS, LAPPI 등)는 역할을 어떻게 나누는가? (27. AI·학습·적응과 모델 운영 겨냥)",
    "실시간 재스케줄링에서 LLM 추론 지연을 어떻게 다루는가(규칙·휴리스틱을 LLM 이 오프라인으로 만들고 결정적 실행기가 쓰는 구조)? (20. 예외 복구·재계획·업무 연속성 연결)",
    "기존 로봇 오케스트레이션 도구(Open-RMF rmf_task, VDA 5050 관제 기능)는 스케줄링·충전 삽입을 어떤 구성 요소에 두는가? (9. 로봇·제조사 관제 연동, 16. 공용 자원·충전·에너지 최적화 연결)",
    "국내에 LLM 과 최적화 엔진의 스케줄링 역할 분담을 다룬 연구·사례가 있는가? (한국 자료 우선 규칙)"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "Open-RMF rmf_task 의 TaskPlanner 는 플릿 안의 작업과 로봇을 받아 요청된 시작 시각을 지키며 작업이 가장 짧게 끝나도록 로봇별 작업 순서를 정하고, 배터리 같은 자원 제약을 고려해 필요하면 충전 작업을 일정에 자동으로 끼워 넣는다.",
      "tag": "사실",
      "source_ids": [
        "ref-404",
        "ref-377"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: planner \"solves the problem of optimal allocation of tasks among available robots\", 충전 작업 자동 삽입(\"automatically injects recharging tasks\"). 같은 저장소의 README·헤더라 독립 교차 아님 (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "수행 자원"
    },
    {
      "id": "f2",
      "claim": "rmf_task TaskPlanner 는 최적성을 보장하지 않지만 빠른 탐욕(greedy) 방식과 최적성을 보장하지만 오래 걸릴 수 있는 A* 기반 방식 가운데 하나로 배정을 풀고, 비용 계산기를 지정하지 않으면 BinaryPriorityCostCalculator 를 쓰며, 각 로봇의 배정 끝에 수행할 마무리 작업(예: 충전)을 만드는 요청 생성기를 옵션으로 받는다.",
      "tag": "사실",
      "source_ids": [
        "ref-377"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "TaskPlanner.hpp Options 주석: greedy \"Optimality is not guaranteed but the solution time may be faster\" / A* \"guarantees optimality but may take longer to solve\" (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f3",
      "claim": "Open-RMF 에서는 디스패처가 입찰 공고를 보내면 각 플릿 어댑터가 TaskPlanner 로 비용을 계산해 입찰하고, 디스패처가 가장 빨리 끝나는 것·가장 낮은 비용 같은 설정 기준으로 비교해 작업을 줄 플릿을 정한다.",
      "tag": "사실",
      "source_ids": [
        "ref-376"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "BidNotice → BidProposal(비용) → DispatchRequest, 평가 기준은 fastest to finish, lowest cost 등으로 설정 가능 (재인용: 2026-09-25-33)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "수행 자원",
    ...
    {
      "id": "f4",
      "claim": "VDA 5050 3.0.0 은 관제의 최소 기능으로 주문 배정, 에너지 관리(충전 주문이 운반 주문을 중단할 수 있음), 교통 제어를 두면서도 경로·우선순위·혼잡 처리 같은 교통 관리 전략·알고리즘은 범위에서 제외해, 배정·일정 결정 로직을 관제 구현에 맡긴다.",
      "tag": "사실",
      "source_ids": [
        "ref-031"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "5.3 절: \"Energy management: Charging orders can interrupt transfer orders\". 2절 범위 제외: Traffic Management Logic(routing, prioritization, congestion handling …) (VDA 5050 3.0.0, main 브랜치, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f5",
      "claim": "LLM+P 는 LLM 이 자연어 계획 문제를 PDDL 문제 파일로 바꾸고 고전 계획기 Fast Downward 가 계획을 구하는 구조이며, LLM 이 계획을 직접 내는 방식(LLM-as-Planner)과 문맥 예시 유무를 바꾼 기준선을 barman·blocksworld 등 7개 도메인에서 비교한다.",
      "tag": "사실",
      "source_ids": [
        "ref-091"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: 계획기 Fast-Downward, 기준선 \"llm -> plan does not generate pddl\" 대 제안 \"llm + context -> p.pddl\", 도메인 7개 (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f6",
      "claim": "LLM+P 논문 저자들은 GPT-4 기반 실험에서 LLM+P 가 LLM-as-Planner 보다 훨씬 많은 계획 문제를 풀었고, LLM 이 계획을 직접 내는 방식은 공간 관계가 복잡한 문제에서 완전히 실패했으며, 문맥 예시가 없으면 LLM+P 도 실패했다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-092"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 보고: GPT-4 가 만든 PDDL 을 Fast Downward(SEQ-OPT-FDSS-1, LAMA, 탐색 한도 200초)에 넘김. 검색 요약 기준, 원문 미열람",
      "as_of": "2023-04",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f7",
      "claim": "Kambhampati 외(ICML 2024 입장 논문)는 자기회귀 LLM 이 혼자서는 계획이나 자기 검증을 하지 못한다고 보고, LLM 을 근사적 아이디어 생성기로 두고 외부 모델 기반 검증기·비평자와 양방향으로 결합하는 LLM-Modulo 틀을 제안한다.",
      "tag": "의견",
      "source_ids": [
        "ref-674"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "입장 논문(position paper)의 주장. LLM 은 계획기 자체가 아니라 외부 검증기와 결합된 생성기 역할이라는 평가. 검색 요약 기준, 원문 미열람",
      "as_of": "2024-02",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f8",
      "claim": "ConstraintBench 저자들은 10개 운영과학 영역에서 LLM 이 제약 최적화 문제를 직접 풀게 했을 때 가장 좋은 모델의 실행 가능 해 비율이 65.0%였고, 실행 가능한 해는 Gurobi 최적값의 평균 89~96% 수준이었지만 실행 가능성과 최적성을 함께 만족한 비율은 어느 모델도 30.5%를 넘지 못했으며 영역별 실행 가능 비율은 0.8~85.0%였다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-675"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 보고값, 정답은 Gurobi 로 산출. 실패 유형: 소요 시간 제약 오해, 존재하지 않는 개체 생성(entity hallucination). 검색 요약 기준, 원문 미열람",
      "as_of": "2026-02",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f9",
      "claim": "R-ConstraintBench 저자들은 자원 제약 프로젝트 스케줄링 문제(RCPSP)에서 강한 LLM 도 선후 제약만 있을 때는 실행 가능성이 천장에 가깝지만 정지 시간·시간창·배타(disjunctive) 제약이 함께 걸리면 실행 가능성이 급락하며, 병목은 그래프 깊이가 아니라 제약 사이의 상호작용이라고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-676"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 보고, 데이터센터 이전 시나리오로 예시. 합성 난이도 증가 결과가 현장형 시나리오로 옮겨지지 않는다고도 밝힘. 검색 요약 기준, 원문 미열람",
      "as_of": "2025-08",
      "flow_step": null,
      "flow_item": "제약",
      "source_unopened": true
    ...
    {
      "id": "f10",
      "claim": "SCHEDBench 저자들은 작업장·자원 제약 프로젝트·간호사 근무·시간표 스케줄링 1,132개 사례로 13개 LLM 을 평가해, 같은 문제를 다른 문장 표현으로 주면 실행 가능 비율이 떨어지고 제약 위반이 달라지며 제약 순서 바꾸기에 가장 민감했다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-677"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 보고: LLM 이 완성 스케줄을 직접 내게 하고 영역별 검사기로 제약을 검사. 의미가 같은 표현에 불변이지 않음. 검색 요약 기준, 원문 미열람",
      "as_of": "2026-08",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f11",
      "claim": "Starjob 저자들은 작업장 스케줄링 문제(JSSP) 13만 개 사례를 자연어로 기술한 지도 학습 데이터셋으로 Llama 8B 를 미세 조정하면 실행 가능한 스케줄을 생성하고 우선순위 디스패치 규칙과 초기 신경망 방법(L2D)보다 DMU 평균 15.36%, Taillard 평균 7.85% 개선된다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-678"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 보고값, 비교 대상은 우선순위 디스패치 규칙·L2D 이며 정확 해법기(OR-Tools 등)와의 비교는 검색 요약에서 확인되지 않음. 원문 미열람",
      "as_of": "2025-03",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f12",
      "claim": "DynaSchedBench 저자들은 동적 유연 작업장 스케줄링에서 LLM 스케줄러에 전체 구조 정보를 주면 간결한 통계 요약을 줄 때보다 성능이 나빠졌고(1.66% 대 0.65%), 도구를 쓰는 탐색은 토큰 비용이 약 3배인데 성능은 더 낮았으며, 현재 LLM 은 참된 최적화기보다 정교한 휴리스틱처럼 동작한다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-682"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 보고값(지표의 정의는 검색 요약에서 미확인). 'observability paradox'로 명명. 검색 요약 기준, 원문 미열람",
      "as_of": "2026-05",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f13",
      "claim": "RACE-Sched 는 LLM 추론 지연이 산업 제어의 밀리초 단위 결정 주기와 맞지 않는다고 보고, 실시간 디스패치는 저지연 기호 휴리스틱이 맡고 병렬 흐름에서 LLM 이 규칙을 합성·검증·진화시킨 뒤 샌드박스 시험을 거쳐 제어 루프를 막지 않는 원자적 갱신으로 배포하는 이중 흐름 구조를 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-683"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "Reactive Stream(휴리스틱 실시간 디스패치) / Deliberative Stream(LLM 규칙 합성). 동적 유연 작업장 벤치마크에서 DRL·LLM 기준선보다 낫다는 것은 저자 보고. 검색 요약 기준, 원문 미열람",
      "as_of": "2026-05",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f14",
      "claim": "Li·Li(칭화대)는 동적 생산·AGV 스케줄링의 이산 사건 시뮬레이션에서 LLM 관리 에이전트가 시뮬레이션 사건 기록으로 병목 가설을 세우고 편집 에이전트가 규칙 기반 정책 코드를 고치는 휴리스틱 설계 틀을 제안했으며, 결과 정책이 수리계획·휴리스틱·메타휴리스틱 기준선보다 나았다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-684"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 보고(2026-08 프리프린트). LLM 은 개별 스케줄이 아니라 정책(코드)을 설계하고, 평가는 반복 시뮬레이션으로 한다. 검색 요약 기준, 원문 미열람",
      "as_of": "2026-08",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f15",
      "claim": "OptiMUS 는 LLM 이 자연어 문제 기술에서 (혼합 정수) 선형 계획 모델을 정식화하고 Gurobi 파이썬 API 코드로 옮겨 MIP 해법기가 최적해를 구하게 하는 구조이며, 순차형(v1)에서 에이전트형(v2), 검색 증강·대규모 기법(v3)으로 발전했고 각 LLM 구성 요소에 오류 검사 모듈을 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-679",
        "ref-680"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: V1 \"Sequential work-flow implementation. Suitable for small and medium-sized problems.\" 논문(0.3)은 LaTeX 모델→gurobipy 코드→실행으로 최적해 출력. 같은 저자 계열이라 독립 교차 아님",
      "as_of": "2024-07",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f16",
      "claim": "LAPPI 는 LLM 이 자연어 대화로 사용자의 모호한 선호를 후보 항목·선호 점수·제약으로 바꿔 최적화 문제를 인스턴스화하고 풀이는 기존 최적화 해법기에 맡기는 대화형 최적화 방식이며, 여행 계획 사용자 연구에서 기존 방식과 프롬프트만 쓴 방식보다 나은 실행 가능 계획을 냈다고 저자가 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-681"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 보고, 여행 계획 조건. '맥락 이해'와 '풀이 논리'를 분리해 LLM 을 해법기가 아니라 문제 인스턴스화 인터페이스로 쓴다는 설계. 검색 요약 기준, 원문 미열람",
      "as_of": "2025-12",
      "flow_step": null,
      "flow_item": "시작 조건",
      "source_unopened": true
    ...
    {
      "id": "f17",
      "claim": "운영과학(OR)에서의 LLM 적용을 정리한 서베이(Wang·Li)는 기존 방법을 자동 모델링, 보조 최적화(휴리스틱·알고리즘 설계), 직접 풀이의 세 경로로 나누고, 의미–구조 대응의 불안정, 일반화·해석 가능성 한계, 평가 체계 부족, 산업 배치 장벽을 과제로 든다.",
      "tag": "사실",
      "source_ids": [
        "ref-686"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "서베이의 세 경로: automatic modeling / auxiliary optimization / direct solving. 검색 요약 기준, 원문 미열람",
      "as_of": "2025-09",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f18",
      "claim": "PortAgent 는 자동화 컨테이너 터미널의 차량 디스패칭 시스템을 새 터미널로 옮기는 작업을 LLM 가상 전문가 팀(지식 검색·모델러·코더·디버거)이 자동화하는 방식으로, LLM 이 개별 배차 결정을 내리기보다 디스패칭 모델과 코드를 만들고 디버거가 정적 분석·샌드박스 실행으로 오류를 검사·수정한다.",
      "tag": "사실",
      "source_ids": [
        "ref-685"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저자 주장 특징: 항만 전문가 불필요, 적은 데이터, 빠른 배치(2025-12 프리프린트). 성능 수치는 검색 요약에서 미확인. 원문 미열람",
      "as_of": "2025-12",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f19",
      "claim": "Powell 외(Journal of Intelligent Information Systems 63권, 2025)는 스케줄링 시스템이 낸 결과를 사람에게 설명하는 텍스트를 LLM 의 추론(사고 사슬 프롬프트)으로 생성하는 방법을 연구했다.",
      "tag": "사실",
      "source_ids": [
        "ref-687"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "LLM 이 스케줄을 만드는 것이 아니라 스케줄링 시스템 출력의 정당화 설명을 만드는 역할. 63권 1287–1337쪽. 검색 요약 기준, 원문 미열람",
      "as_of": "2025",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f20",
      "claim": "Saha 외(arXiv 2605.15486)는 건설 로봇 작업 스케줄링에서 LLM 에 에이전트 행동 능력과 목표를 주고 생성 LLM(GPT-4)과 감독 LLM(Gemma 3·Llama 4·Mistral 7B)이 함께 스케줄을 만드는, 해법기 없이 LLM 이 일정을 직접 산출하는 틀을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-689"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "시간 효율·자원 활용을 함께 고려한 배정 전략이라고 저자가 밝힘. 해법기 대비 정량 비교는 검색 요약에서 미확인. 원문 미열람",
      "as_of": "2026-05",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f21",
      "claim": "다중 로봇 LLM 연구 가운데 LiP-LLM(선형계획), PIP-LLM(정수계획), FLEET(makespan 최소화), Peng 외(MILP)는 LLM 이 의존 그래프·적합도·제약을 정식화하고 배정·일정은 결정적 해법이 푸는 분담을 쓴다.",
      "tag": "사실",
      "source_ids": [
        "ref-166",
        "ref-181",
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "각 논문의 방법 서술 요약(저자별 단일 출처, 원문 미열람) (재인용: 2026-09-25-21)",
      "as_of": "2025-10",
      "flow_step": null,
      "flow_item": "수행 자원",
      "source_unopened": true
    ...
    {
      "id": "f22",
      "claim": "q3-01 에 대해, 확인한 자료로는 LLM 이 스케줄을 직접 만들면 제약이 겹치거나 표현이 바뀔 때 실행 가능성이 흔들리므로(ConstraintBench, R-ConstraintBench, SCHEDBench, DynaSchedBench), ROP 에서는 순서·시각·충전 삽입 같은 스케줄링 결정은 rmf_task 같은 결정적 최적화·계획 해법이 맡고 LLM 은 지시에서 목적·제약·기한을 뽑아 문제를 인스턴스화하는 일(LLM+P, OptiMUS, LAPPI)과 결과 설명을 맡는 분담이 근거가 가장 많은 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-675",
        "ref-676",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. 반례로 좁은 문제(JSSP)에 미세 조정한 LLM 이 실행 가능 스케줄을 낸 보고(Starjob)가 있으나 정확 해법기와의 비교는 미확인",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f23",
      "claim": "직접 생성의 반례로, 미세 조정한 LLM 이 작업장 스케줄링에서 규칙·초기 신경망 방법을 앞섰다는 보고(Starjob)와 LLM 두 개가 건설 로봇 스케줄을 직접 만든 연구가 있어, LLM 직접 스케줄링이 배제되는 것은 아니지만 그 비교 대상이 정확 해법기가 아니거나 확인되지 않아 해법기 대체의 근거로는 약한 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-678",
        "ref-689",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. Starjob 비교 대상은 PDR·L2D, Saha 외는 해법기 비교 미확인, ConstraintBench 는 Gurobi 기준",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f24",
      "claim": "진행 중 고장·새 지시 같은 동적 사건에 대한 재스케줄링은 LLM 추론 지연 때문에 결정 루프 안에 LLM 을 두기 어렵고, RACE-Sched·Li·Li 처럼 LLM 은 규칙·정책을 루프 밖에서 만들어 시뮬레이션·샌드박스 검증을 거쳐 반영하며 실시간 재계산은 해법기(rmf_task 의 충전 삽입·재배정)가 맡는 구조가 ROP 의 선택지로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-683",
        "ref-684",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. 물류 로봇 플릿에서 허용 가능한 LLM 지연 한계를 잰 자료는 찾지 못함",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": false
    ...
    {
      "id": "f25",
      "claim": "분류 원문 질문(가장 가까운 로봇에 맡기는 것이 전체적으로도 유리한가)과 관련해, 전체 이익은 완료 시각·비용 같은 명시적 목적함수를 최적화하는 해법(rmf_task, 선형·정수계획)이 계산·비교할 수 있지만, LLM 직접 배정·스케줄은 실행 가능하더라도 최적성과 함께 만족하는 비율이 낮게 보고되어 전체 이익을 보장하는 수단으로 쓰기 어려운 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-377",
        "ref-166",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. ConstraintBench 결합 만족 30.5% 이하는 저자 보고값이며 운영과학 일반 문제 조건",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "수행 자원",
      "source_unopened": false
    ...
    {
      "id": "f26",
      "claim": "출하 마감 전에 채팅으로 긴급 출고 지시가 들어오면, LLM 은 지시에서 기한·우선순위를 뽑아 문제 인스턴스(제약·목적 가중치)로 바꾸고 해법기가 충전 삽입을 포함한 일정을 다시 계산한 뒤 LLM 이 바뀐 일정과 이유를 설명하는 흐름이 가능해 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-681",
        "ref-377",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "설명용 시나리오 구성(이 위키의 추론). LAPPI 인스턴스화, rmf_task 재계획·충전 삽입, 스케줄 설명 생성 연구를 대응시킴",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "제약",
      "source_unopened": false
    ...
    {
      "id": "f27",
      "claim": "이번에 확인한 LLM 스케줄링 근거의 평가 환경은 작업장·프로젝트·근무표 스케줄링, 운영과학 일반 문제, 건설 로봇, 컨테이너 터미널, 여행 계획이었고, 이종 제조사 창고 로봇 플릿에서 LLM 직접 스케줄과 해법기를 비교한 자료는 검색 범위에서 찾지 못했다(부재의 확인은 아님).",
      "tag": "추정",
      "source_ids": [
        "ref-675",
        "ref-676",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 관찰. 한국어 검색 3회에서도 해당 국내 자료를 찾지 못함",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
  ...
  "sources": [
    {
      "id": "ref-674",
      "org": "Kambhampati, S., Valmeekam, K., Guan, L., Verma, M., Stechly, K., Bhambri, S., Saldyt, L., & Murthy, A.",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. ICML 2024 입장 논문. LLM 단독 계획·자기 검증의 한계를 주장하고 외부 검증기와 결합하는 LLM-Modulo 틀을 제안.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 10개 운영과학 영역에서 LLM 직접 풀이의 실행 가능성·최적성을 Gurobi 정답과 비교한 벤치마크.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-676",
      "org": "Jain, R. 외(R-ConstraintBench 저자)",
      "title": "R-ConstraintBench: Evaluating LLMs on NP-Complete Scheduling",
      "published": "2025-08",
      "url": "https://arxiv.org/abs/2508.15204",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자원 제약 프로젝트 스케줄링에서 제약을 늘려 가며 LLM 실행 가능성이 무너지는 지점을 평가.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-677",
      "org": "SCHEDBench 저자(arXiv 2608.00991, 저자 미확인)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 1,132개 스케줄링 사례로 13개 LLM 의 직접 스케줄 생성이 문장 표현 변화에 얼마나 흔들리는지 평가.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-678",
      "org": "Starjob 저자(arXiv 2503.01877, 저자 미확인)",
      "title": "Starjob: Dataset for LLM-Driven Job Shop Scheduling",
      "published": "2025-03",
      "url": "https://arxiv.org/abs/2503.01877",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 작업장 스케줄링 13만 사례 자연어 데이터셋과 미세 조정 Llama 8B 의 직접 스케줄 생성 결과.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "title": "OptiMUS — Optimization Modeling Using mip Solvers and large language models (GitHub README)",
      "published": null,
      "url": "https://github.com/teshnizi/OptiMUS",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "LLM 이 자연어 문제를 MIP 모델로 정식화하고 해법기로 푸는 OptiMUS 의 판별 구성과 NLP4LP 데이터셋 안내.",
      "fetched": true,
    ...
    {
      "id": "ref-680",
      "org": "AhmadiTeshnizi, A. 외(OptiMUS 저자)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LaTeX 모델→gurobipy 코드→실행으로 최적해를 내는 OptiMUS-0.3 과 오류 검사 모듈.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-681",
      "org": "Nakagawa, M., Koyama, Y. 외(OMRON SINIC X, LAPPI 저자)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 이 대화로 최적화 문제를 인스턴스화하고 해법기가 푸는 대화형 최적화(IEEE 학술지 게재 기록 있음).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-682",
      "org": "DynaSchedBench 저자(arXiv 2605.27566, 저자 미확인)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 동적 유연 작업장 스케줄링에서 LLM 스케줄러의 관측 정보 수준별 성능과 도구 탐색 비용을 평가.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 실시간 휴리스틱 디스패치와 LLM 규칙 합성을 분리한 비동기 이중 흐름 스케줄링 틀.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 시뮬레이션 사건 기록으로 LLM 에이전트가 생산·AGV 스케줄링 휴리스틱 코드를 개선하는 틀.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-685",
      "org": "Hu, J., Li, J., Lin, W., Jia, P., Ji, Y., & Lai, J.",
      "title": "PortAgent: LLM-driven Vehicle Dispatching Agent for Port Terminals",
      "published": "2025-12",
      "url": "https://arxiv.org/abs/2512.14417",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 가상 전문가 팀이 컨테이너 터미널 차량 디스패칭 시스템의 이전(모델·코드 작성·디버깅)을 자동화.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 운영과학의 LLM 적용을 자동 모델링·보조 최적화·직접 풀이로 분류한 서베이.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. Journal of Intelligent Information Systems 63권. 스케줄링 시스템 출력의 설명문을 LLM 으로 생성.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "TaskPlanner API 헤더. 탐욕·A* 풀이 옵션, 비용 계산기 기본값, 마무리(충전) 요청 생성기, plan 함수를 정의.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-689",
      "org": "Saha, S., Das, S., Duan, H., & Liu, X.-Y.",
      "title": "Hybrid LLM-based Intelligent Framework for Robot Task Scheduling",
      "published": "2026-05",
      "url": "https://arxiv.org/abs/2605.15486",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 생성·감독 LLM 두 개로 건설 로봇 작업 스케줄을 직접 만드는 틀.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "rmf_task 의 작업 배정·순서 계획(TaskPlanner)과 충전 작업 자동 삽입을 소개하는 README.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않음. 디스패처–플릿 어댑터 입찰 기반 작업 배정 설명.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "VDA 5050 3.0.0 명세 원문(입력 원문 텍스트). 범위·관제 기능·주문·동작을 규정.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": null,
      "source_unopened": false
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "LLM+P 공식 저장소 README. Fast Downward 계획기, 7개 도메인, 비교 방법 구성.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 이 PDDL 을 만들고 고전 계획기가 푸는 LLM+P 와 LLM-as-Planner 비교.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 의존 그래프 + 선형계획 배정.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자연어 작업 기술을 MILP 모델·코드로 바꾸는 다중 로봇 배정·스케줄링.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 팀 수준 PDDL 계획 뒤 정수계획으로 로봇 배정.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 작업 그래프·적합도 행렬 + makespan 최소화 형식적 뒷단.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
  ...
  "page_proposals": [
    {
      ...
      "rationale": "q3-01 답: f1·f2·f3·f4·f5·f6·f7·f8·f9·f10·f11·f12·f13·f14·f15·f16·f17·f18·f19·f20·f21·f22·f23·f24·f25·f26·f27 (신뢰도 low) — 2절 q3-01 상태 답함, 3절 q3-01 소제목 신설({#q3-01}): 오케스트레이션 도구의 스케줄링 위치(rmf_task f1·f2, Open-RMF 입찰 f3, VDA 5050 관제 기능 f4), LLM 직접 스케줄 생성의 한계(f8·f9·f10·f12, 입장 f7 의견)와 반례(f11·f20, f23), LLM+해법기 결합(LLM+P f5·f6, OptiMUS f15, LAPPI f16, 다중 로봇 f21, 서베이 f17), 루프 밖 규칙·정책 설계(f13·f14·f18), 설명 생성(f19), 분담 종합(f22)·동적 재스케줄링(f24)·SCM 질문 연결(f25)·출하 시나리오(f26)·물류 근거 공백(f27) / 4절 결론·불확실성 / 5절 후속 질문 / 6절 완료 조건 현황 / 8절 출처 / 9절 이력"
    },
    {
      "action": "update",
      ...
      "rationale": "아이디어 페이지 5절(트랙 산출물): '스케줄링 결정의 분담' 소절 신설 — 분담 가설 f22(추정), 근거 f8·f9·f10·f12·f15·f16·f21·f1, 반례 f23, 동적 재스케줄링 f24, 설명 역할 f19. 처리 흐름 전체(q3-02)·되묻기(q3-03)·지시 변경(q3-04)은 미조사임을 명시"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신: track.ontology_changes(일정 개념에 '일정 산출 방식' 속성)가 승인되면 2절 반영과 초안 버전 인상(f1·f2·f13·f14·f15·f21). 6절 '일정을 누가 계산하는가' 질문에 q3-01 답(f22) 연결, 목적 가중치 전달 인터페이스는 질문으로 유지"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 3 반영 제안 (f1, f2, f8, f9, f10, f12, f13, f22, f24): rmf_task 의 순서 계획·충전 삽입, LLM 직접 스케줄 생성의 실행 가능성 한계 벤치마크, 루프 밖 LLM 규칙 합성 구조. 27. AI·학습·적응과 모델 운영과 양쪽 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 3 반영 제안 (f2, f3, f21, f25): TaskPlanner 탐욕·A* 선택과 비용 계산기, LLM 정식화+해법기 배정 분담, 분류 원문 질문과 목적함수 기반 전체 최적 비교"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 3 반영 제안 (f7, f8, f15, f16, f17, f19): LLM-Modulo(의견), 직접 풀이 한계 벤치마크, OptiMUS·LAPPI 의 정식화·인스턴스화 역할, 운영과학 LLM 서베이 세 경로, 스케줄 설명 생성. 적용 대상 13. 작업 배정 — MRTA·14. 작업 순서·스케줄링과 함께 연결"
    }
  ],
  "glossary_candidates": [
    {
    ...
    {
      "term_ko": "자원 제약 프로젝트 스케줄링 문제",
      "term_en": "Resource-Constrained Project Scheduling Problem (RCPSP)",
      "definition": "선후 관계가 있는 활동들을 한정된 자원 용량 안에서 시작 시각을 정해 배치하는 스케줄링 문제로, 실행 가능한 일정을 찾는 것 자체가 어려운 NP-난해 문제이다."
    },
    {
  ...
  "track": {
    ...
    "new_questions": [
      ...
      {
        "question": "ConstraintBench 처럼 해법기 최적해를 정답으로 두고 LLM 직접 스케줄의 실행 가능성·최적성 결합 비율을 재는 방식을 물류 창고 배정·스케줄링 시나리오로 옮기면, 제약 상호작용(충전·시간창·공용 자원)에 따라 결과가 어떻게 달라지는가? (q3-01 에서 파생)",
        "stage": 5,
...
### runs/2026-09-25-66/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: rmf_task README 원문(raw)에 최적 배정, 요청된 시작 시각 기준 최단 완료 순서, 배터리 자원 제약과 충전 작업 자동 삽입 문구가 있다. ref-404·ref-377 은 같은 저장소라 독립 교차 아님. 발행일 미확인, 확인일 2026-09-25."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: TaskPlanner.hpp 원문(raw)에 탐욕(최적성 미보장·빠를 수 있음)/A*(최적성 보장·오래 걸릴 수 있음) 옵션, nullptr 이면 BinaryPriorityCostCalculator, 배정 끝 마무리 작업 요청 생성기 주석이 있다. 다만 헤더에는 '충전' 예시가 없다 — '(예: 충전)'은 README(ref-404) 근거로 옮기거나 뺀다."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 검증에서 공식 미러(osrf/ros2multirobotbook src/task.md, raw)를 열어 BidNotice→BidProposal(TaskPlanner 로 비용 계산)→DispatchRequest, 'fastest to finish, lowest cost 등 설정 가능' 문구를 확인했다. 13. 작업 배정 — MRTA 페이지 5·7절의 같은 주장과 같은 각주(ref-376)를 재사용한다."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 입력 원문 텍스트(ref-031) 5.3절에 주문 배정·'Charging orders can interrupt transfer orders'·교통 제어, 2절 범위 제외에 Traffic Management Logic(routing, prioritization, congestion handling, deadlock resolution)이 있다. 단 범위 제외는 교통 관리 로직이며 배정·일정 알고리즘을 명시적으로 제외한 문장은 없다 — 마지막 구절 '배정·일정 결정 로직을 관제 구현에 맡긴다'는 [추정]으로 분리해야 한다."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: llm-pddl README 원문(raw)에 Fast Downward, 7개 도메인(barman·blocksworld·floortile·grippers·storage·termes·tyreworld), llm_planner·llm_pddl_planner·llm_ic_planner·llm_ic_pddl_planner 비교가 있다."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "사실 → 추정: ref-092 는 참고문헌 목록에 등록된 실재 출처이나 원문 미열람이고, 이번 검증에서 '공간 관계가 복잡한 문제에서 완전히 실패'·'문맥 예시 없으면 LLM+P 도 실패' 구절을 검색 결과로 재확인하지 못했다(검증 예산). 저자 보고로 표기."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(의견 유지): 검색 결과(ICML 2024 PMLR v235, arXiv 2402.01817)에서 저자 목록과 '자기회귀 LLM 은 스스로 계획·자기 검증을 못 한다', 외부 모델 기반 검증기와의 양방향 LLM-Modulo 틀이 일치. 원문 미열람. 의견 주체(Kambhampati 외)를 밝힌다."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "부분 확인(원문 미열람, 검색 요약): 10개 OR 영역·200개 과제·6개 모델, 최고 65.0% 제약 만족, 실행 가능성+최적성(솔버 기준 0.1% 이내) 결합 30.5% 초과 모델 없음, 소요 시간 제약 오해·개체 환각 확인. 불일치: 검증 검색 요약은 영역별 평균 실행 가능 비율을 0.8%(승무원 배정)~83.3%(생산 믹스)로 적어 브리프의 0.8~85.0% 와 다르다. 'Gurobi 최적값의 평균 89~96%'는 확인하지 못했다. 저자 목록 미확인."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "사실 → 추정: 출처 실재(Jain, R. 외, arXiv 2508.15204, 2025-08)와 설정(RCPSP, 선후 제약 증가 뒤 정지 시간·시간창·배타 제약 도입, 데이터센터 이전 시나리오)은 확인했으나, '선후 제약만일 때 천장 근처·함께 걸리면 급락·병목은 제약 상호작용' 결론은 검증 검색 요약에 나타나지 않았다. 원문 미열람."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "부분 확인(원문 미열람): 1,132개 사례, 작업장·단일/다중 모드 RCPSP·간호사 근무·교과 시간표, 의미가 같은 표현 변화에 대한 제약 충실성 평가는 확인. '13개 LLM'과 '제약 순서 바꾸기에 가장 민감'은 검증 검색 요약에서 확인하지 못했다 — 미확인 표시 또는 삭제."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람, 검색 요약): 13만 사례 JSSP 지도 학습 데이터셋, LLaMA 8B 4비트·LoRA 미세 조정, PDR 대비 우위와 L2D 대비 DMU 평균 15.36%·Taillard 평균 7.85% 개선(저자 보고값). 정확 해법기 비교는 미확인으로 유지."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "부분 확인(원문 미열람): DFJSP, 전체 구조 사전 정보(L3)가 간결 요약(L2)보다 나쁨(1.66% 대 0.65%), 도구 탐색 토큰 비용 약 3배·성능 저하, 'observability paradox' 확인. '참된 최적화기보다 정교한 휴리스틱처럼 동작' 구절은 검증 검색 요약에서 확인하지 못함 — 삭제하거나 미확인 표시. 수치 지표 정의 미확인 유지."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "부분 확인(원문 미열람): LLM 추론 지연과 밀리초 결정 주기 불일치, Reactive Stream(저지연 기호 휴리스틱 실시간 디스패치)·Deliberative Stream(LLM 이 규칙 합성·검증·진화), DRL·생성형 기준선 대비 우위(저자 보고) 확인. '샌드박스 시험·제어 루프를 막지 않는 원자적 갱신'은 검증 검색 요약에 없어 미확인 표시 필요."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람): 반복 시뮬레이션 선택·사건 기록 진단, 관리 에이전트의 병목 가설과 편집 에이전트의 코드 수정, 100개 시드에서 rolling-MILP·규칙·메타휴리스틱 정책보다 우위(저자 보고). 저자 소속(칭화대)은 검증 검색에서 확인하지 못함 — 미확인 표시."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "부분 확인: OptiMUS README 원문(raw)에서 V1 순차형(중소 규모)·V2 에이전트형·V3 RAG·대규모 기법과 MIP/LP 해법기 사용 확인. 'Gurobi 파이썬 API'·'각 LLM 구성 요소의 오류 검사 모듈'은 README 에 없고 ref-680(원문 미열람) 요약에만 기댄다. 두 출처 같은 저자 계열."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람): 대화로 후보 항목·선호 점수·제약을 정해 문제를 인스턴스화하고 기존 해법기에 넘김, 여행 계획 사용자 연구에서 기존·프롬프트 방식보다 나은 실행 가능 계획(저자 보고). 출처 기관 표기 불일치: 검색 결과 저자는 Kuroki, S., Nakagawa, M., Yoshida, S., Koyama, Y., Kozuno, T.(IEEE Access 2026)이다."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람): 세 경로(automatic modeling / auxiliary optimization / direct solving)와 과제(의미–구조 대응 불안정, 일반화·해석 가능성 한계, 평가 체계 부족, 산업 배치 장벽) 일치."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "부분 확인(원문 미열람): 컨테이너 터미널 VDS 이전 워크플로 자동화, 가상 전문가 팀(Knowledge Retriever·Modeler·Coder·Debugger), 세 특징 확인. '디버거가 정적 분석·샌드박스 실행으로 오류 검사'는 검증 검색 요약에 없어 미확인 표시. 항만 터미널은 분류 원문 9장 업종별 조건·거점 간 운송 연계 영역이므로 '연계 대상'으로만 쓴다."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람): JIIS 2025 게재, Strathclyde(strathprints 저장본), 최적으로 도출된 스케줄링 해의 설명문을 LLM 으로 생성. 63권 1287–1337쪽은 검증에서 확인하지 못함."
    },
    {
      "finding_id": "f20",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(원문 미열람): 저자 Saha·Das·Duan·Liu, 행동 능력·목표 입력, 생성(GPT-4)·감독(Gemma 3/Llama 4/Mistral 7b) LLM, 건설 로봇 스케줄. 건설 현장은 업종별 조건 연계 영역 — 방법 사례로만 쓴다."
    },
    {
      "finding_id": "f21",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재사용 출처(ref-166·ref-167·ref-181·ref-242, 원문 미열람). 같은 방법 서술이 실행 2026-09-25-21 검증을 거쳐 업무 분해·배정 설계 초안 배정 개념에 [사실]로 실려 있다 — 기존 각주 재사용. 연구마다 단일 출처."
    },
    {
      "finding_id": "f22",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: 이 위키의 종합. 근거 finding 가운데 f6 강등, f8·f10·f12 는 일부 구절 미확인이므로 본문에서 수치를 옮길 때 f8·f10·f12 수정 지시를 따른다. q3-01 의 답 결론이지만 물류 플릿 근거가 없어 신뢰도 low."
    },
    {
      "finding_id": "f23",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: Starjob 비교 대상(PDR·L2D), Saha 외 해법기 비교 미확인, ConstraintBench 솔버 기준이 검색 결과와 일치."
    },
    {
      "finding_id": "f24",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: RACE-Sched 지연 문제·루프 밖 규칙 합성, Li·Li 시뮬레이션 기반 정책 개선 확인. rmf_task 는 README 기준 충전 삽입만 확인되고 '재배정'은 문서에서 확인하지 않았다 — 추정 문장 안에서만 쓴다. 8. 실시간 세계 상태·데이터 일관성과 22. 시뮬레이션·예측용 디지털 트윈을 섞지 않음(시뮬레이션은 정책 검증 도구)."
    },
    {
      "finding_id": "f25",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: 분류 원문 SCM 질문을 다룬다. 30.5% 는 OR 일반 문제·솔버 기준 0.1% 이내 조건의 저자 보고값임을 함께 적는다."
    },
    {
      "finding_id": "f26",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: 설명용 시나리오(출하 / 제약). 수치 없이 가상 시나리오임을 밝힌다."
    },
    {
      "finding_id": "f27",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: 검색 범위의 관찰이며 부재 확인 아님을 병기. 한국어 검색 3회 기록 확인."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "scope_boundary": {
    "ok": false,
    "issues": [
      "f18: 컨테이너 터미널 차량 디스패칭은 분류 원문 9장 '거점 간 운송'·'업종별 조건' 연계 영역인데 claim 문구에 '연계 대상' 표시가 없다(브리프 self_check 는 인지) — 본문에서 '연계 대상:'으로 표시하고 LLM 이 디스패칭 시스템을 구성하는 방법 사례로만 쓴다",
      "f20: 건설 로봇 스케줄링은 업종별 조건 연계 영역 — 방법 사례로만 쓰고 ROP 직접 범위처럼 서술하지 않는다"
    ]
  },
  "duplication": {
    "ok": true,
    "overlaps": [
      "f3 은 13. 작업 배정 — MRTA 페이지 5·7·9절의 Open-RMF 입찰 배정 서술(ref-376)과 같다 — 기존 각주 재사용",
      "f4 는 13. 작업 배정 — MRTA 페이지 9절 'VDA 5050 은 주문 배정을 관제의 기능으로 두지만 배정 알고리즘 자체는 규정하지 않는다'(ref-031)와 겹친다 — 기존 표현과 충돌하지 않게 맞춘다",
      "f1 은 13. 작업 배정 — MRTA 페이지 10절 rmf_task 배정·순서 동시 결정(ref-404)과 겹친다",
      "f21 은 업무 분해·배정 설계 초안 배정 개념과 아이디어 2 페이지 3절의 LiP-LLM·PIP-LLM·FLEET·Peng 외 서술(ref-166·ref-181·ref-242·ref-167)과 같다 — 기존 각주 재사용",
      "새 질문 3(ConstraintBench 방식을 물류 시나리오로)은 백로그 q5-05·q3-05 와 겹친다"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "quotation_check": {
    "ok": false,
    "issues": [
      "ref-377: f2 evidence_excerpt 가 같은 헤더에서 두 구절(greedy·A* 주석)을 직접 인용한다 — 페이지에서는 한 구절 이하로 줄이고 나머지는 재서술",
      "ref-404: f1 evidence_excerpt 가 README 에서 두 구절을 직접 인용한다 — 페이지에서는 한 구절 이하"
    ]
  },
  "corrections_applied": [],
  "corrections_rejected": [],
  "required_fixes": [
    "f2: '(예: 충전)'을 TaskPlanner.hpp(ref-377) 근거 문장에서 빼고, 충전 작업 자동 삽입은 README(ref-404) 각주를 단 f1 문장으로만 쓴다 — 헤더 주석에는 충전 예시가 없다.",
    "f4: 앞부분(관제 최소 기능·충전 주문의 운반 주문 중단·교통 관리 로직 범위 제외)은 [사실][^ref-031]로 두고, '배정·일정 결정 로직을 관제 구현에 맡긴다'는 별도 문장 [추정][^ref-031]으로 나눈다 — 명세 2절은 교통 관리 로직만 명시적으로 제외한다. 13. 작업 배정 — MRTA 페이지 9절의 기존 표현('배정 알고리즘 자체는 규정하지 않는다')과 어긋나지 않게 쓴다.",
    "f6: [사실] → [추정]으로 강등하고 'LLM+P 논문 저자 보고, 원문 미열람'을 병기한다 — 검증에서 결과 구절을 재확인하지 못했다.",
    "f8: 영역별 실행 가능 비율 '0.8~85.0%'는 검색 요약끼리 다르므로(검증 요약: 평균 0.8% 승무원 배정~83.3% 생산 믹스) 수치를 쓰지 말고 '영역별 편차가 크다(수치는 요약마다 달라 미확인)'로 쓰며, 'Gurobi 최적값의 평균 89~96%'는 삭제한다. 30.5% 에는 '솔버 기준 0.1% 이내' 조건과 '6개 모델·200개 과제, 저자 보고값'을 병기한다.",
    "f9: [사실] → [추정]으로 강등하고 결론 부분(선후 제약만일 때 천장 근처, 복합 제약에서 급락, 병목은 제약 상호작용)에 '저자 보고, 원문 미열람·검증 미재확인'을 병기한다.",
    "f10: '13개 LLM'과 '제약 순서 바꾸기에 가장 민감'은 미확인으로 표시하거나 뺀다 — 검증 검색 요약에서 확인되지 않았다.",
    "f12: '현재 LLM 은 참된 최적화기보다 정교한 휴리스틱처럼 동작' 구절은 빼거나 미확인으로 표시한다 — 1.66% 대 0.65%, 약 3배 토큰 비용은 저자 보고값으로 유지하되 지표 정의 미확인을 병기한다.",
    "f13: '샌드박스 시험을 거쳐 제어 루프를 막지 않는 원자적 갱신으로 배포'는 미확인으로 표시하거나 '규칙을 합성·검증·진화시킨다'로 줄인다.",
    "f14: 저자 소속 '칭화대'를 미확인으로 표시한다.",
    "f15: 'Gurobi 파이썬 API 코드'와 '각 LLM 구성 요소의 오류 검사 모듈'은 ref-680(원문 미열람) 요약 기준임을 병기하고, README(ref-679)가 확인해 주는 것은 V1~V3 구성과 MIP/LP 해법기 사용뿐임을 구분한다.",
    "f16·ref-681: 참고문헌 항목과 각주 기관을 'Kuroki, S., Nakagawa, M., Yoshida, S., Koyama, Y., & Kozuno, T.(OMRON SINIC X 등, IEEE Access 2026)'로 고친다 — 검색 결과의 저자 목록과 다르다.",
    "f18: 문장 앞에 '연계 대상:'을 붙이고 '디버거가 정적 분석·샌드박스 실행으로 오류 검사'는 미확인으로 표시한다.",
    "f19: '63권 1287–1337쪽'은 미확인으로 표시하거나 뺀다.",
    "f20: 건설 로봇 사례임을 밝히고 방법 사례로만 쓴다(연계 대상 업종).",
    "f24: 'rmf_task 의 재배정'은 확인된 문서 근거가 없으므로 '충전 삽입' 로만 쓰거나 [추정] 문장 안에 둔다.",
    "인용: ref-377·ref-404 는 페이지에서 출처당 직접 인용 한 구절 이하로 하고 나머지는 재서술한다.",
    "각주: 원문을 열지 못한 출처 ref-674·ref-675·ref-676·ref-677·ref-678·ref-680·ref-681·ref-682·ref-683·ref-684·ref-685·ref-686·ref-687·ref-689·ref-092·ref-166·ref-167·ref-181·ref-242 의 각주 정의 접근일 뒤에 ' (원문 미열람)'을 붙이고 reference_updates[] 의 해당 항목에 source_unopened: true 를 넣는다. ref-679·ref-377·ref-404·ref-091·ref-031·ref-376 은 원문 열람 출처로 둔다.",
    "온톨로지: '일정 (Schedule)' 개념 수정을 승인한다 — 속성 '일정 산출 방식'(값 후보: 최적화·계획 해법(rmf_task 탐욕·A*, 선형·정수계획·MILP·makespan 최소화, MIP 해법기) / LLM 이 만든 규칙·휴리스틱을 결정적 실행기가 적용)을 더하고 상태를 초안 → 확정, 초안 버전 0.5 → 0.6 으로 올린다. 값 후보 'LLM 직접 생성'은 evidence_finding_ids 에 근거가 없으므로 넣지 않고 6절 미해결 모델링 질문으로 둔다(관련 f11·f20·f23).",
    "업무 분해·배정 설계 초안 6절 '일정을 누가 계산하는가' 질문에는 q3-01 답(f22, [추정], 신뢰도 low)을 연결하되 질문을 해결로 지우지 않는다 — 물류 플릿 근거가 없다.",
    "새 질문 3('ConstraintBench 처럼 해법기 최적해를 정답으로 … 물류 창고 … ')은 q5-05(및 q3-05)와 중복 — backlog_updates 에 넣지 않는다.",
    "q3-01 은 답함(답 위치 #q3-01, 단계 페이지 3절 '### q3-01 … {#q3-01}' 소제목)으로 처리한다. 단계 3 페이지 6절 완료 조건은 세 항목 모두 '미충족', 아래 줄은 '다음 단계로 전환: 아니오(아이디어 2 5절 처리 흐름·구성 요소 미작성, 실험 계획 없음, 열린 질문 q3-02~q3-10)'로 쓴다.",
    "트랙 개요 상태 줄의 '현재 단계'는 입력 값(단계 1. 선행 연구·제품 사례 조사)을 바꾸지 않는다 — 이번 실행은 CLI 지정 단계 3 실행이며 단계 전환 승인이 없다. '마지막 트랙 실행'만 2026-09-25 로 둔다."
  ],
  "confidence": "low",
  "verification_note": "판정: 1차 조건부 승인. 이번 실행은 원문 열람이 차단된 환경(mirror_only)에서 검증됐다. 확인 25건, 미확인 2건, 교차 확인 0건. 강등: f6 사실 → 추정, f9 사실 → 추정. 일부 구절 미확인으로 수정 지시: f8(영역별 비율 요약 간 불일치 0.8~85.0% 대 0.8~83.3%, 89~96% 미확인), f10, f12, f13, f14, f15, f18, f19. 원문 미열람 출처: ref-674, ref-675, ref-676, ref-677, ref-678, ref-680, ref-681, ref-682, ref-683, ref-684, ref-685, ref-686, ref-687, ref-689, ref-092, ref-166, ref-167, ref-181, ref-242. 검증에서 원문을 연 출처: ref-377·ref-404·ref-091·ref-679(github_raw), ref-376(공식 미러 src/task.md, 검증 열람), ref-031(입력 원문 텍스트 — 브리프는 fetched_via github_raw·fetch_url null 로 적었으나 한계 항목은 inbox 로 적어 불일치). 출처 기관 오기: ref-681(LAPPI 저자는 Kuroki 외). 주의: q3-01 의 결론(스케줄링 결정은 결정적 해법, LLM 은 인스턴스화·설명)은 이 위키의 종합([추정])이며 근거는 작업장·프로젝트·OR 일반·건설·항만·여행 계획 조건이고 이종 제조사 창고 플릿 비교 자료는 찾지 못했다. 트랙 개요의 현재 단계는 단계 1 인데 이번 실행은 CLI 로 단계 3 을 지정했다(단계 1 q1-05·q1-06, 단계 2 q2-04~q2-07 열림). 백로그 중복 q3-09/q3-10, q5-05/q5-06 정리 필요(기존 지적 유지). 검증 검색 13회(리서치 25회와 합쳐 38/40). 온톨로지 변경 승인: 일정(Schedule) 속성 '일정 산출 방식'(f1·f2·f13·f14·f15·f21), 상태 초안 → 확정, v0.5 → v0.6 / 거부: 값 후보 'LLM 직접 생성'(근거 finding 미지정, 6절 질문으로). 새 질문 중복: 'ConstraintBench 방식 물류 시나리오 측정'은 q5-05 와 중복. 단계 완료 조건: 미충족(부족: 아이디어 2 5절 처리 흐름·핵심 구성 요소·다른 아이디어와의 연결, 실험 페이지 실험 계획). 단계 전환: 미승인(막힌 질문 q3-02~q3-10 열림).",
  "retry_reason": null,
  "track_checks": {
    ...
    "backlog_duplicates": [
      "ConstraintBench 처럼 해법기 최적해를 정답으로 두고 LLM 직접 스케줄의 실행 가능성·최적성 결합 비율을 재는 방식을 물류 창고 배정·스케줄링 시나리오로 옮기면, 제약 상호작용(충전·시간창·공용 자원)에 따라 결과가 어떻게 달라지는가? (q3-01 에서 파생) — q5-05 와 중복"
    ],
    "stage_tag_issues": [],
...
```json
{
  ...
  "outline": [
    {
      ...
      "section": "2. 질문 목록",
      "budget_chars": 1400,
      "summary": "q3-01 을 답함으로 바꾸고, 앞 단계에서 이 단계로 올라온 q3-05~q3-10 과 이번 실행의 q3-11 을 표에 더한다.",
      "planned_findings": []
    },
    {
      "path": "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
      "section": "3. 조사 결과",
      "budget_chars": 6500,
      "summary": "확인한 자료로는 순서·시각·충전 삽입 같은 스케줄링 결정은 결정적 최적화·계획 해법이 맡고 LLM 은 문제 인스턴스화와 결과 설명을 맡는 분담이 근거가 가장 많은 것으로 보인다. [추정][^ref-675][^ref-377][^ref-681]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 1400,
      "summary": "분담 가설은 이 위키의 종합이며 근거가 물류 플릿이 아닌 조건이라 신뢰도가 낮다. [추정][^ref-675][^ref-377]",
      "planned_findings": [
        "f22",
        "f24",
        "f27"
    ...
    },
    {
      ...
      "budget_chars": 600,
      "summary": "후속 질문 2건(q3-11, q4-08)을 올리고 중복 1건은 제외한다.",
      "planned_findings": [
        "f16",
        "f13"
      ]
    },
    {
      ...
      "budget_chars": 700,
      "summary": "초안 갱신 조건만 충족, 나머지 두 조건 미충족, 다음 단계로 전환하지 않는다.",
      "planned_findings": []
    },
    {
      "path": "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
      "section": "7. 관련 세부영역",
      "budget_chars": 900,
      "summary": "13·14·27·20 세부영역에 반영 제안을 낸다.",
      "planned_findings": []
    }
  ],
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "q3-01 답함(3절 #q3-01 신설), 2절에 q3-05~q3-11 추가, 후속 질문 2건, 완료 조건(초안 갱신만 충족), 상태 줄 갱신. 2차 수정: 6절 초안 갱신 행 충족으로 정정, 전환 줄 q3-02~q3-11, 시나리오 예외·성과 칸에 ConstraintBench 조건 병기"
    },
    {
      "path": "docs/tracks/nl-task-chatbot/task-model-draft.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "초안 v0.5 → v0.6: 일정(Schedule) 개념에 속성 '일정 산출 방식' 추가·초안 → 확정(f1·f2·f13·f14·f15·f21), 6절 '일정을 누가 계산하는가'에 q3-01 답 연결(해결로 닫지 않음), 값 후보 'LLM 직접 생성'을 6절 질문으로 추가(2차 수정 대상 아님, 변경 없음)"
    },
    {
      "path": "docs/ideas/nl-task-chatbot.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "5절 구현 가설에 '스케줄링 결정의 분담' 소절 신설(q3-01, 실행 2026-09-25-66). 2차 수정: 새 각주 정의 12개를 5절 끝에 직접 두고 ref-681 기관을 Kuroki 외로, ref-684 소속을 '소속 미확인'으로 고침",
      "patches": [
        {
          "section": "5. 구현 가설",
          "action": "replace",
          "content": "(절 본문 생략 — runs/2026-09-25-66/pages/ideas/nl-task-chatbot.md 의 해당 절을 본다)",
          "frontmatter": {
            "sources": [
              "ref-054",
              "ref-055",
  ...
  "changelog_entry": "2026-09-25 | 자연어 업무 지시 챗봇 단계 3 | q3-01 답함(스케줄링은 결정적 해법, LLM 은 인스턴스화·설명 분담 가설, 신뢰도 low), 업무 분해·배정 설계 초안 v0.5 → v0.6(일정 산출 방식), 아이디어 2 5절에 분담 소절, 후속 질문 2건 | run 2026-09-25-66",
  "index_updates": {
    "home_recent": "2026-09-25 — 자연어 업무 지시 챗봇 단계 3. 구현 가설 설계: q3-01 답함 — 스케줄링 결정은 결정적 최적화·계획 해법, LLM 은 문제 인스턴스화와 결과 설명을 맡는 분담 가설(신뢰도 low), 업무 분해·배정 설계 초안 v0.6",
    "category_recent": "2026-09-25 — 13. 작업 배정 — MRTA: 자연어 업무 지시 챗봇 트랙 단계 3에서 rmf_task 작업 계획기와 LLM 직접 스케줄 생성의 한계를 조사하고 13. 작업 배정 — MRTA·14. 작업 순서·스케줄링 반영 제안",
    "area_recent": "2026-09-25 — 13. 작업 배정 — MRTA: 트랙 자연어 업무 지시 챗봇 단계 3(q3-01)에서 TaskPlanner 탐욕·A* 선택, LLM 정식화+해법기 배정 분담, 분류 원문 질문과의 연결을 6. 대표 접근법과 기술 반영 제안으로 냄"
  },
  "glossary_updates": [
    {
      "action": "new",
      "slug": "job-shop-scheduling-problem",
      "term_ko": "작업장 스케줄링 문제",
      "term_en": "Job Shop Scheduling Problem (JSSP)",
      "definition": "여러 작업이 정해진 순서로 여러 기계를 거칠 때 기계별 작업 순서를 정해 전체 완료 시간 같은 목표를 최소화하는 대표적 조합 최적화 스케줄링 문제이다.",
      "description": "LLM 이 스케줄을 직접 만드는 연구(Starjob 등)의 평가 대상으로 쓰인다.",
      "related_areas": [
        14,
        27
      ],
    ...
    {
      "action": "new",
      "slug": "resource-constrained-project-scheduling-problem",
      "term_ko": "자원 제약 프로젝트 스케줄링 문제",
      "term_en": "Resource-Constrained Project Scheduling Problem (RCPSP)",
      "definition": "선후 관계가 있는 활동들을 한정된 자원 용량 안에서 시작 시각을 정해 배치하는 스케줄링 문제로, 실행 가능한 일정을 찾는 것 자체가 어려운 NP-난해 문제이다.",
      "description": "R-ConstraintBench·SCHEDBench 가 LLM 의 제약 만족을 평가하는 문제 유형으로 쓴다.",
      "related_areas": [
        14,
        27
      ],
    ...
    {
      "action": "new",
      "slug": "llm-modulo-framework",
      "term_ko": "LLM-모듈로 프레임워크",
      "term_en": "LLM-Modulo Framework",
      "definition": "LLM 을 계획의 후보를 내는 생성기로 두고 외부 모델 기반 검증기·비평자가 후보를 검사해 되먹임하는 LLM–기호 시스템 결합 구조이다.",
      "description": "Kambhampati 외(ICML 2024 입장 논문)가 제안했다. 입장 논문의 주장이므로 [의견]으로 인용한다.",
      "related_areas": [
        27,
        14
      ],
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 10개 운영과학 영역에서 LLM 직접 풀이의 실행 가능성·최적성을 솔버 정답과 비교한 벤치마크.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자원 제약 프로젝트 스케줄링에서 제약을 늘려 가며 LLM 실행 가능성을 평가.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 1,132개 스케줄링 사례로 LLM 직접 스케줄 생성이 문장 표현 변화에 얼마나 흔들리는지 평가.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/tracks/nl-task-chatbot/task-model-draft.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "LLM 이 자연어 문제를 MIP 모델로 정식화하고 해법기로 푸는 OptiMUS 의 판별 구성(v1~v3)과 NLP4LP 데이터셋 안내.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/tracks/nl-task-chatbot/task-model-draft.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. LLM 이 대화로 최적화 문제를 인스턴스화하고 해법기가 푸는 대화형 최적화(IEEE Access 2026 게재 기록).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/tracks/nl-task-chatbot/task-model-draft.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. Journal of Intelligent Information Systems 게재. 스케줄링 시스템 출력의 설명문을 LLM 으로 생성.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/tracks/nl-task-chatbot/task-model-draft.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "TaskPlanner API 헤더. 탐욕·A* 풀이 옵션, 비용 계산기 기본값, 마무리 작업 요청 생성기, plan 함수를 정의.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md",
        "docs/tracks/nl-task-chatbot/task-model-draft.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "디스패처–플릿 어댑터 입찰 기반 작업 배정 설명. 이번 실행의 검증에서 공식 미러(src/task.md) 원문으로 확인.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md"
      ]
  ...
  "open_question_updates": [
    {
      "action": "new",
      "question": "창고 이동로봇 플릿의 재배정·재스케줄링 주기에서 LLM 추론 지연이 허용되는 한계를 측정했거나, LLM 을 결정 루프 밖에 둔 운영 사례가 있는가?",
      "areas": [
        14,
        27
      ],
  ...
  "flow_matrix_updates": [
    {
      "step": "출하",
      "item": "시작 조건",
      "link": "docs/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md#q3-01",
      "title": "단계 3. 구현 가설 설계"
    },
    {
      "step": "출하",
  ...
  "area_reflection_proposals": [
    ...
    },
    {
      "area_no": 27,
      "section": "6. 대표 접근법과 기술",
      "summary": "LLM-모듈로 틀(Kambhampati 외, [의견] f7), LLM 을 해법기가 아니라 정식화·인스턴스화 인터페이스로 쓰는 OptiMUS(f15)·LAPPI(f16, Kuroki 외), 스케줄링 결과의 설명문을 LLM 으로 생성하는 연구(f19). 적용 대상 13. 작업 배정 — MRTA·14. 작업 순서·스케줄링과 함께 연결한다."
    },
    {
      "area_no": 27,
      "section": "8. 대표 연구와 자료",
      "summary": "운영과학 LLM 적용 서베이(Wang·Li, 자동 모델링·보조 최적화·직접 풀이의 세 경로, f17)와 LLM 직접 풀이 벤치마크 ConstraintBench(f8)·SCHEDBench(f10)를 대표 자료 후보로 제안한다(원문 미열람)."
    },
    {
      "area_no": 20,
      "section": "10. 다른 연구영역과의 연결 (번호와 이름을 함께 표기)",
      "summary": "동적 재스케줄링에서 LLM 추론 지연 때문에 LLM 을 결정 루프 밖에 두고 규칙·정책을 만들어 시뮬레이션·검증 뒤 반영하는 구조(RACE-Sched f13, Li·Li f14, 종합 f24 [추정])가 14. 작업 순서·스케줄링·27. AI·학습·적응과 모델 운영과 이어진다. 시뮬레이션은 정책 검증 도구로만 다룬다."
    }
  ]
}
```

### runs/2026-09-25-66/format_check.md
...
### runs/2026-09-25-66/pages/tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-404, ref-377, ref-376, ref-031, ref-091, ref-092, ref-674, ref-675, ref-676, ref-677, ref-678, ref-679, ref-680, ref-681, ref-682, ref-683, ref-684, ref-685, ref-686, ref-687, ref-689, ref-166, ref-181, ref-242, ref-167]
last_run: 2026-09-25
version: 2
---
...
확인한 자료로는 LLM 이 스케줄을 직접 만들면 제약이 겹치거나 문장 표현이 바뀔 때 실행 가능성이 흔들리므로, ROP 에서는 순서·시각·충전 삽입 같은 스케줄링 결정은 rmf_task 같은 결정적 최적화·계획 해법이 맡고 LLM 은 지시에서 목적·제약·기한을 뽑아 문제를 인스턴스화하는 일과 결과 설명을 맡는 분담이 근거가 가장 많은 것으로 보인다. [추정][^ref-675][^ref-676][^ref-677][^ref-682][^ref-377][^ref-092][^ref-679][^ref-681][^ref-687] 이 결론은 이 위키의 종합이며, 근거가 작업장·프로젝트·운영과학 일반·건설·항만·여행 계획 조건이고 이종 제조사 창고 플릿에서 두 방식을 비교한 자료가 없어 신뢰도가 낮다. 아래에 근거를 나누어 적는다.
...
- ConstraintBench 저자들은 10개 운영과학 영역 200개 과제에서 6개 모델에게 제약 최적화 문제를 직접 풀게 했을 때, 가장 좋은 모델의 실행 가능 해 비율이 65.0%였고 실행 가능성과 최적성(솔버 최적값 기준 0.1% 이내)을 함께 만족한 비율은 어느 모델도 30.5%를 넘지 못했으며, 실패 유형으로 소요 시간 제약 오해와 존재하지 않는 개체 생성을 들었다(저자 보고값, 원문 미열람). [사실][^ref-675] 영역별 실행 가능 비율은 편차가 크다고 보고되었으나 수치는 검색 요약마다 달라 미확인이다.
- R-ConstraintBench 저자들은 자원 제약 프로젝트 스케줄링 문제(Resource-Constrained Project Scheduling Problem, RCPSP)에서 선후 제약을 늘린 뒤 정지 시간·시간창·배타 제약을 더해 LLM 을 평가했고, 선후 제약만 있을 때는 실행 가능성이 천장에 가깝지만 제약이 함께 걸리면 급락하며 병목은 그래프 깊이가 아니라 제약 사이 상호작용이라고 보고한 것으로 보인다(저자 보고, 원문 미열람·검증 미재확인). [추정][^ref-676]
- SCHEDBench 저자들은 작업장·자원 제약 프로젝트·간호사 근무·교과 시간표 스케줄링 1,132개 사례로 LLM 이 완성 스케줄을 직접 내게 하고, 같은 문제를 의미가 같은 다른 문장 표현으로 주면 실행 가능 비율이 떨어지고 제약 위반이 달라진다고 보고했다(저자 보고, 원문 미열람; 평가한 모델 수와 가장 민감한 변형 유형은 미확인). [사실][^ref-677]
- DynaSchedBench 저자들은 동적 유연 작업장 스케줄링에서 LLM 스케줄러에 전체 구조 정보를 주면 간결한 통계 요약을 줄 때보다 성능이 나빠졌고(1.66% 대 0.65%, 지표 정의 미확인), 도구를 쓰는 탐색은 토큰 비용이 약 3배인데 성능은 더 낮았다고 보고했다(저자 보고값, 원문 미열람). [사실][^ref-682]
- Kambhampati 외(ICML 2024 입장 논문)는 자기회귀 LLM 이 혼자서는 계획이나 자기 검증을 하지 못한다고 보고, LLM 을 근사적 아이디어 생성기로 두고 외부 모델 기반 검증기·비평자와 양방향으로 결합하는 LLM-모듈로(LLM-Modulo) 틀을 제안한다. [의견][^ref-674]

#### 반례: LLM 이 스케줄을 직접 만든 연구

- Starjob 저자들은 작업장 스케줄링 문제(Job Shop Scheduling Problem, JSSP) 13만 개 사례를 자연어로 기술한 데이터셋으로 Llama 8B 를 미세 조정하면 실행 가능한 스케줄을 생성하고, 우선순위 디스패치 규칙과 초기 신경망 방법(L2D)보다 DMU 평균 15.36%, Taillard 평균 7.85% 개선된다고 보고했다(저자 보고값, 원문 미열람; 정확 해법기와의 비교는 미확인). [사실][^ref-678]
- 건설 로봇 사례로, Saha 외는 LLM 에 에이전트의 행동 능력과 목표를 주고 생성 LLM(GPT-4)과 감독 LLM(Gemma 3·Llama 4·Mistral 7B)이 함께 스케줄을 만드는, 해법기 없이 LLM 이 일정을 직접 내는 틀을 제안했다(해법기 대비 정량 비교는 미확인, 건설 현장은 분류 원문 9장의 업종별 조건에 속하는 연계 대상이어서 방법 사례로만 다룸). [사실][^ref-689]
- 두 연구로 보면 LLM 직접 스케줄링이 배제되는 것은 아니지만, 비교 대상이 정확 해법기가 아니거나 확인되지 않아 해법기 대체의 근거로는 약한 것으로 보인다. [추정][^ref-678][^ref-689][^ref-675]
...
- OptiMUS 공식 README 는 순차형(v1, 중소 규모 문제), 에이전트형(v2), 검색 증강·대규모 기법(v3)의 구성과 MIP·LP 해법기 사용을 밝힌다(확인일 2026-09-25 기준). [사실][^ref-679] OptiMUS-0.3 논문은 LLM 이 정식화한 모델을 Gurobi 파이썬 API 코드로 옮겨 해법기로 풀고 각 LLM 구성 요소에 오류 검사 모듈을 둔다고 설명하며, 이 부분은 원문 미열람 논문의 요약 기준이고 README 와 같은 저자 계열이라 독립 교차가 아니다. [사실][^ref-680]
- LAPPI(Kuroki 외, IEEE Access 2026)는 LLM 이 대화로 사용자의 모호한 선호를 후보 항목·선호 점수·제약으로 바꿔 최적화 문제를 인스턴스화하고 풀이는 기존 해법기에 맡기는 대화형 최적화 방식이며, 여행 계획 사용자 연구에서 기존 방식과 프롬프트만 쓴 방식보다 나은 실행 가능 계획을 냈다고 저자가 보고했다(원문 미열람). [사실][^ref-681]
...
- 운영과학(Operations Research, OR)의 LLM 적용을 정리한 서베이(Wang·Li)는 기존 방법을 자동 모델링, 보조 최적화(휴리스틱·알고리즘 설계), 직접 풀이의 세 경로로 나누고, 의미–구조 대응의 불안정, 일반화·해석 가능성 한계, 평가 체계 부족, 산업 배치 장벽을 과제로 든다. [사실][^ref-686]

#### 결정 루프 밖에서 규칙·정책을 만드는 구조

- RACE-Sched 는 LLM 추론 지연이 산업 제어의 밀리초 단위 결정 주기와 맞지 않는다고 보고, 실시간 디스패치는 저지연 기호 휴리스틱이 맡고 병렬 흐름에서 LLM 이 규칙을 합성·검증·진화시키는 이중 흐름 구조를 제안했다(저자 보고, 원문 미열람; 규칙을 운영에 반영하는 방식의 세부는 미확인). [사실][^ref-683]
- Li·Li(소속 미확인)는 동적 생산·AGV 스케줄링의 이산 사건 시뮬레이션에서 LLM 관리 에이전트가 사건 기록으로 병목 가설을 세우고 편집 에이전트가 규칙 기반 정책 코드를 고치는 휴리스틱 설계 틀을 제안했으며, 결과 정책이 수리계획·규칙·메타휴리스틱 기준선보다 나았다고 보고했다(저자 보고, 원문 미열람). [사실][^ref-684] 여기서 시뮬레이션은 LLM 이 만든 정책을 검증하는 도구로만 쓰인다.
- 연계 대상: 컨테이너 터미널 차량 디스패칭은 분류 원문 9장의 거점 간 운송·업종별 조건에 가까운 영역이며, PortAgent 는 LLM 이 개별 배차를 결정하기보다 가상 전문가 팀(지식 검색·모델러·코더·디버거)이 디스패칭 모델과 코드를 만들고 디버거가 오류를 검사·수정하는 방법 사례다(검사 방식의 세부와 성능 수치는 미확인, 원문 미열람). [사실][^ref-685]
- Powell 외(Journal of Intelligent Information Systems, 2025)는 스케줄링 시스템이 낸 결과를 사람에게 설명하는 텍스트를 LLM 의 추론(사고 사슬 프롬프트)으로 생성하는 방법을 연구했다(원문 미열람). [사실][^ref-687]
...
- 진행 중 고장·새 지시 같은 동적 사건의 재스케줄링은 LLM 추론 지연 때문에 결정 루프 안에 LLM 을 두기 어렵고, RACE-Sched·Li·Li 처럼 LLM 은 규칙·정책을 루프 밖에서 만들어 시뮬레이션·검증을 거쳐 반영하며 실시간 재계산(rmf_task 의 충전 삽입 등)은 해법이 맡는 구조가 ROP 의 선택지로 보인다. rmf_task 의 재배정 기능은 문서에서 확인하지 않았다. [추정][^ref-683][^ref-684][^ref-404][^ref-674]
- 분류 원문 13. 작업 배정 — MRTA 의 SCM 관점 질문은 다음과 같다.

> 가장 가까운 로봇에 맡기는 것이 전체적으로도 유리한가? [분류원문]

- 이 질문과 관련해, 전체 이익은 완료 시각·비용 같은 명시적 목적함수를 최적화하는 해법(rmf_task, 선형·정수계획)이 계산·비교할 수 있지만, LLM 직접 배정·스케줄은 실행 가능하더라도 최적성까지 함께 만족하는 비율이 낮게 보고되어(운영과학 일반 문제, 솔버 기준 0.1% 이내 조건의 저자 보고값 30.5% 이하) 전체 이익을 보장하는 수단으로 쓰기 어려운 것으로 보인다. [추정][^ref-377][^ref-166][^ref-675]
- 이번에 확인한 LLM 스케줄링 근거의 평가 환경은 작업장·프로젝트·근무표 스케줄링, 운영과학 일반 문제, 건설 로봇, 컨테이너 터미널, 여행 계획이었고, 이종 제조사 창고 로봇 플릿에서 LLM 직접 스케줄과 해법기를 비교한 자료는 검색 범위에서 찾지 못했다(한국어 검색 포함, 부재의 확인은 아님). [추정][^ref-675][^ref-676][^ref-677][^ref-678][^ref-689][^ref-685][^ref-681]
...
| 수행 자원 | LLM 은 지시에서 기한·우선순위를 뽑아 문제 인스턴스로 바꾸고, 해법이 일정을 다시 계산하며, LLM 이 바뀐 일정과 이유를 설명하는 분담이 가능해 보인다. [추정][^ref-681][^ref-377][^ref-687] |
| 제약 | rmf_task 는 배터리 같은 자원 제약을 고려해 충전 작업을 일정에 끼워 넣고 [사실][^ref-404] VDA 5050 에서는 충전 주문이 운반 주문을 중단할 수 있다. [사실][^ref-031] |
| 완료·인계 | 해당 없음 |
| 예외·성과 | LLM 이 일정을 직접 만들면 실행 가능성과 최적성을 함께 만족하는 비율이 낮게 보고되었고(운영과학 일반 제약 최적화 문제, 솔버 기준 0.1% 이내 조건의 저자 보고값) [사실][^ref-675] LLM 추론 지연은 실시간 결정 주기와 맞지 않을 수 있다. [추정][^ref-683] |

다음은 설명을 위한 가상의 시나리오이다. 출하 마감 전에 채팅으로 긴급 출고 지시가 들어오면, LLM 은 지시에서 기한·우선순위를 뽑아 문제 인스턴스(제약·목적 가중치)로 바꾸고 해법이 충전 삽입을 포함한 일정을 다시 계산한 뒤, LLM 이 바뀐 일정과 이유를 설명하는 흐름이 가능해 보인다. [추정][^ref-681][^ref-377][^ref-687] 이 흐름에서 LLM 이 뽑은 값을 해법의 비용 계산기나 목적함수로 넘기는 형식은 아직 정하지 않았으며 후속 질문 q3-11 로 둔다.
...
- q3-01 의 답: 스케줄링 결정은 결정적 최적화·계획 해법이 맡고 LLM 은 문제 인스턴스화와 결과 설명을 맡는 분담이 근거가 가장 많은 것으로 보인다(신뢰도 low). [추정][^ref-675][^ref-677][^ref-377][^ref-679][^ref-681][^ref-687]
- 동적 재스케줄링에서는 LLM 을 결정 루프 밖에 두고 규칙·정책을 만들어 검증 뒤 반영하는 구조가 선택지로 보인다. [추정][^ref-683][^ref-684]
...
[^ref-674]: Kambhampati, S., Valmeekam, K., Guan, L., Verma, M., Stechly, K., Bhambri, S., Saldyt, L., & Murthy, A., LLMs Can't Plan, But Can Help Planning in LLM-Modulo Frameworks, 2024-02, https://arxiv.org/abs/2402.01817, 접근일 2026-09-25 (원문 미열람)
[^ref-675]: ConstraintBench 저자(arXiv 2602.22465, 저자 미확인), ConstraintBench: Benchmarking LLM Constraint Reasoning on Direct Optimization, 2026-02, https://arxiv.org/abs/2602.22465, 접근일 2026-09-25 (원문 미열람)
[^ref-676]: Jain, R. 외(R-ConstraintBench 저자), R-ConstraintBench: Evaluating LLMs on NP-Complete Scheduling, 2025-08, https://arxiv.org/abs/2508.15204, 접근일 2026-09-25 (원문 미열람)
[^ref-677]: SCHEDBench 저자(arXiv 2608.00991, 저자 미확인), SCHEDBench: A Benchmark for Evaluating LLM Constraint Faithfulness in Natural-Language Combinatorial Scheduling, 2026-08, https://arxiv.org/abs/2608.00991, 접근일 2026-09-25 (원문 미열람)
[^ref-678]: Starjob 저자(arXiv 2503.01877, 저자 미확인), Starjob: Dataset for LLM-Driven Job Shop Scheduling, 2025-03, https://arxiv.org/abs/2503.01877, 접근일 2026-09-25 (원문 미열람)
[^ref-679]: teshnizi (OptiMUS 공식 저장소), OptiMUS — Optimization Modeling Using mip Solvers and large language models (GitHub README), 미확인, https://github.com/teshnizi/OptiMUS, 접근일 2026-09-25
[^ref-680]: AhmadiTeshnizi, A. 외(OptiMUS 저자), OptiMUS-0.3: Using Large Language Models to Model and Solve Optimization Problems at Scale, 2024-07, https://arxiv.org/abs/2407.19633, 접근일 2026-09-25 (원문 미열람)
[^ref-681]: Kuroki, S., Nakagawa, M., Yoshida, S., Koyama, Y., & Kozuno, T.(OMRON SINIC X 등, IEEE Access 2026), LAPPI: Interactive Optimization with LLM-Assisted Preference-Based Problem Instantiation, 2025-12, https://arxiv.org/abs/2512.14138, 접근일 2026-09-25 (원문 미열람)
[^ref-682]: DynaSchedBench 저자(arXiv 2605.27566, 저자 미확인), DynaSchedBench: Calibrated Dynamic Scheduling Benchmarks and Observability Paradox in LLM-based Scheduling Agents, 2026-05, https://arxiv.org/abs/2605.27566, 접근일 2026-09-25 (원문 미열람)
[^ref-683]: RACE-Sched 저자(arXiv 2605.29262, 저자 미확인), Harmonizing Real-Time Constraints and Long-Horizon Reasoning: An Asynchronous Agentic Framework for Dynamic Scheduling, 2026-05, https://arxiv.org/abs/2605.29262, 접근일 2026-09-25 (원문 미열람)
[^ref-684]: Li, J., & Li, C.(소속 미확인), LLM-Guided Heuristic Design from Simulation Traces: A Case Study in Dynamic Production and AGV Scheduling, 2026-08, https://arxiv.org/abs/2608.09343, 접근일 2026-09-25 (원문 미열람)
[^ref-685]: Hu, J., Li, J., Lin, W., Jia, P., Ji, Y., & Lai, J., PortAgent: LLM-driven Vehicle Dispatching Agent for Port Terminals, 2025-12, https://arxiv.org/abs/2512.14417, 접근일 2026-09-25 (원문 미열람)
[^ref-686]: Wang, Y., & Li, K., Large Language Models in Operations Research: Methods, Applications, and Challenges, 2025-09, https://arxiv.org/abs/2509.18180, 접근일 2026-09-25 (원문 미열람)
[^ref-687]: Powell, C. 외(University of Strathclyde), Generating textual explanations for scheduling systems leveraging the reasoning capabilities of large language models, 2025, https://link.springer.com/article/10.1007/s10844-025-00940-w, 접근일 2026-09-25 (원문 미열람)
[^ref-689]: Saha, S., Das, S., Duan, H., & Liu, X.-Y., Hybrid LLM-based Intelligent Framework for Robot Task Scheduling, 2026-05, https://arxiv.org/abs/2605.15486, 접근일 2026-09-25 (원문 미열람)
...
### runs/2026-09-25-66/pages/tracks/nl-task-chatbot/task-model-draft.md
...
- 일정을 누가 계산하는가. 스케줄링 결정을 LLM과 최적화 엔진 가운데 어디에 맡기는지에 따라 일정 개념의 속성이 달라진다. — 관련: q3-01 [가정] 실행 2026-09-25-66 에서 일정 개념에 속성 '일정 산출 방식'을 두었다(v0.6). [단계 3 조사 결과](stage-3-implementation-hypothesis.md#q3-01)는 LLM 이 스케줄을 직접 만들면 제약이 겹치거나 문장 표현이 바뀔 때 실행 가능성이 흔들린다는 보고를 근거로, 순서·시각·충전 삽입은 결정적 최적화·계획 해법이 맡고 LLM 은 지시에서 목적·제약·기한을 뽑아 문제를 인스턴스화하고 결과를 설명하는 분담이 근거가 가장 많은 것으로 보인다고 정리했다(신뢰도 low). [추정][^ref-675][^ref-677][^ref-377][^ref-681][^ref-687] 근거가 작업장·프로젝트·운영과학 일반 문제 조건이고 물류 플릿 비교 자료가 없어 이 질문은 해결로 닫지 않는다.
- 일정 산출 방식에 'LLM 직접 생성' 값을 둘 것인가. 미세 조정한 LLM 이 작업장 스케줄링에서 규칙·초기 신경망 방법을 앞섰다는 보고(Starjob)와 LLM 두 개가 건설 로봇 스케줄을 직접 만든 연구(Saha 외)가 있으나, 비교 대상이 정확 해법기가 아니거나 확인되지 않아 해법기 대체의 근거로는 약한 것으로 보인다. [추정][^ref-678][^ref-689][^ref-675] 실행 2026-09-25-66 검증은 이 값을 뒷받침하는 근거 finding 이 지정되지 않아 넣지 않았다(관련 f11·f20·f23). — 관련: q3-01, q3-05
...
### runs/2026-09-25-66/pages/ideas/nl-task-chatbot.md
...
이 절은 [단계 3. 구현 가설 설계](../tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md)와 [단계 4. 오해석 방지와 확인 절차](../tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md)의 결과를 싣는다. 지금까지는 q3-01 의 답인 스케줄링 결정의 분담만 실었고(실행 2026-09-25-66), 처리 흐름 전체와 단계별 입력·출력(q3-02), 온톨로지 질의 결과에 따른 되묻기(q3-03), 지시 변경 반영(q3-04), 확인 절차(단계 4)는 아직 조사되지 않았다. 다른 아이디어와의 연결 구조(구축자 제안)는 [확장 아이디어 연결 구조](index.md)에 있다.

### 스케줄링 결정의 분담

확인한 자료로는 순서·시각·충전 삽입 같은 스케줄링 결정은 결정적 최적화·계획 해법이 맡고, LLM 은 지시에서 목적·제약·기한을 뽑아 문제를 인스턴스화하는 일과 결과 설명을 맡는 분담이 근거가 가장 많은 것으로 보인다. [추정][^ref-675][^ref-677][^ref-377][^ref-679][^ref-681][^ref-687] 이는 이 위키의 종합이며, 근거가 작업장·프로젝트·운영과학 일반·건설·항만·여행 계획 조건이고 이종 제조사 창고 플릿 비교 자료는 검색 범위에서 찾지 못해 신뢰도가 낮다. 자세한 근거는 [단계 3 조사 결과](../tracks/nl-task-chatbot/stage-3-implementation-hypothesis.md#q3-01)에 있다.

- **LLM 직접 생성의 한계**: ConstraintBench 저자들은 10개 운영과학 영역 200개 과제에서 6개 모델을 평가해 가장 좋은 모델의 실행 가능 해 비율이 65.0%였고, 실행 가능성과 최적성(솔버 기준 0.1% 이내)을 함께 만족한 비율은 어느 모델도 30.5%를 넘지 못했다고 보고했다(저자 보고값, 원문 미열람). [사실][^ref-675] SCHEDBench 저자들은 같은 스케줄링 문제를 의미가 같은 다른 문장 표현으로 주면 실행 가능 비율이 떨어지고 제약 위반이 달라진다고 보고했다. [사실][^ref-677] 자원 제약 프로젝트 스케줄링에서 여러 제약이 함께 걸리면 실행 가능성이 급락한다는 보고도 있는 것으로 보인다(저자 보고, 검증 미재확인). [추정][^ref-676]
- **정식화와 해법기의 결합**: OptiMUS 는 LLM 이 정식화한 모델을 MIP·LP 해법기로 푼다. [사실][^ref-679] LAPPI 는 LLM 이 대화로 선호를 후보·점수·제약으로 바꿔 최적화 문제를 인스턴스화하고 풀이는 해법기에 맡긴다. [사실][^ref-681] 다중 로봇 연구 LiP-LLM·PIP-LLM·FLEET·Peng 외도 LLM 이 정식화하고 결정적 해법이 배정·일정을 푼다. [사실][^ref-166][^ref-181][^ref-242][^ref-167]
...
- **반례**: 미세 조정한 LLM 이 작업장 스케줄링에서 규칙·초기 신경망 방법을 앞섰다는 보고와 LLM 두 개가 건설 로봇 스케줄을 직접 만든 연구가 있으나, 비교 대상이 정확 해법기가 아니거나 확인되지 않아 해법기 대체의 근거로는 약한 것으로 보인다. [추정][^ref-678][^ref-689][^ref-675]
- **동적 재스케줄링**: LLM 추론 지연 때문에 결정 루프 안에 LLM 을 두기 어렵고, LLM 은 규칙·정책을 루프 밖에서 만들어 시뮬레이션·검증을 거쳐 반영하며 실시간 재계산은 해법이 맡는 구조가 선택지로 보인다. [추정][^ref-683][^ref-684][^ref-404]
- **설명 역할**: 스케줄링 시스템이 낸 결과를 사람에게 설명하는 텍스트를 LLM 으로 생성하는 연구가 있다. [사실][^ref-687]
...
### runs/2026-09-25-66/docs_tree.txt
...
### docs/references/index.md (요약: 이번 대상 페이지가 인용한 111건 / 전체 605건. 목록에 없는 출처는 새 id 로 적는다 — 같은 URL 이 이미 있으면 퍼블리셔가 기존 id 로 합친다)
...
### docs/open-questions.md (요약: 대상 영역 [1, 2, 5, 6, 8, 12, 13, 14, 16, 18, 19, 20, 23, 25, 26, 27] 에 걸린 82건 / 전체 103건)
...
### runs/2026-09-25-66/verification2.json
...
```json
{
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "단계 3 페이지 3절의 Open-RMF 입찰(f3)·VDA 5050 관제 기능(f4)은 13. 작업 배정 — MRTA 페이지 5·7·9절과 같은 각주(ref-376·ref-031)를 재사용했고 기존 표현('배정 알고리즘 자체는 규정하지 않는다')과 충돌하지 않는다",
      "새 질문 'ConstraintBench 방식 물류 측정'은 q5-05 와 중복이라 backlog_updates 에서 제외됨(1차 지시 이행)"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "docs/ideas/nl-task-chatbot.md 끝에 새로 붙인 각주 [^ref-681] 정의의 기관을 'Kuroki, S., Nakagawa, M., Yoshida, S., Koyama, Y., & Kozuno, T.(OMRON SINIC X 등, IEEE Access 2026)'로 고친다 — 1차 수정 지시 f16·ref-681 이 단계 3 페이지·업무 분해·배정 설계 초안·reference_updates 에는 이행됐지만 아이디어 페이지 각주는 옛 표기('Nakagawa, M., Koyama, Y. 외(OMRON SINIC X, LAPPI 저자)')로 남아 fixes_applied 의 보고와 다르다.",
    "docs/ideas/nl-task-chatbot.md 끝에 새로 붙인 각주 [^ref-684] 정의의 '(칭화대학교 산업공학과)'를 '(소속 미확인)'으로 고친다 — 1차 수정 지시 f14 가 아이디어 페이지 각주에는 이행되지 않았다(다른 두 페이지와 reference_updates 는 이미 '소속 미확인').",
    "단계 3 페이지 6절 표 두 번째 행(업무 분해·배정 설계 초안이 근거 finding과 함께 v0.1 이상으로 갱신됨)의 충족 여부를 '충족', 근거를 '이번 실행에서 일정 개념에 속성 일정 산출 방식을 더해 v0.6 으로 갱신(f1·f2·f13·f14·f15·f21)', 검증 판정을 '충족'으로 고친다 — 2차 최종 판정에서 이 조건은 채워졌다(1차 지시의 '세 항목 모두 미충족'은 1차 검증 노트의 부족 목록과도 어긋난 것으로, 이번에 바로잡는다). 나머지 두 행은 '미충족 · 미승인'을 유지하고 상태 줄의 '완료 조건: 미충족'도 그대로 둔다.",
    "단계 3 페이지 6절 아래 줄을 '다음 단계로 전환: 아니오(아이디어 2 5절 처리 흐름·구성 요소 미작성, 실험 계획 없음, 열린 질문 q3-02~q3-11)'로 고친다 — 이번 실행이 q3-11 을 열림으로 추가했고 상태 줄의 '열린 질문: 10건'·log_entry 와 맞춘다.",
    "단계 3 페이지 3절 설명용 시나리오 표의 예외·성과 칸 'LLM 이 일정을 직접 만들면 실행 가능성과 최적성을 함께 만족하는 비율이 낮게 보고되었고 [사실][^ref-675]' 에 '(운영과학 일반 제약 최적화 문제, 솔버 기준 0.1% 이내 조건의 저자 보고값)'을 덧붙인다 — ConstraintBench(f8)는 스케줄 전용이 아닌 OR 일반 문제 조건이며 1차 지시가 30.5% 에 조건 병기를 요구했다."
  ],
  "confidence": "low",
  "verification_note": "판정: 1차 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경(mirror_only)에서 검증됐다. 확인 25건, 미확인 2건, 교차 확인 0건. 강등: f6 사실 → 추정, f9 사실 → 추정. 원문 미열람 출처: ref-674, ref-675, ref-676, ref-677, ref-678, ref-680, ref-681, ref-682, ref-683, ref-684, ref-685, ref-686, ref-687, ref-689, ref-092, ref-166, ref-167, ref-181, ref-242. 주의: q3-01 의 결론(스케줄링 결정은 결정적 해법, LLM 은 인스턴스화·설명)은 이 위키의 종합([추정])이며 근거는 작업장·프로젝트·OR 일반·건설·항만·여행 계획 조건이고 이종 제조사 창고 플릿 비교 자료는 찾지 못했다. 온톨로지 변경 승인: 일정(Schedule) 속성 '일정 산출 방식'(f1·f2·f13·f14·f15·f21), 상태 초안 → 확정, v0.5 → v0.6 / 거부: 값 후보 'LLM 직접 생성'(근거 finding 미지정, 6절 질문으로). 단계 완료 조건: 미충족(부족: 아이디어 2 5절 처리 흐름·핵심 구성 요소·다른 아이디어와의 연결, 실험 페이지 실험 계획; 업무 분해·배정 설계 초안 v0.6 갱신 조건은 충족). 단계 전환: 미승인(막힌 질문 q3-02~q3-11 열림). / 2차 수정 후 재검증. 드리프트 없음(세부영역 반영 제안에 20. 예외 복구·재계획·업무 연속성 1건이 추가됐으나 f13·f14·f24 근거의 제안에 그쳐 허용), [분류원문] 보존, 섹션 순서 준수, 링크 유효. 지적: 아이디어 페이지의 새 각주 ref-681·ref-684 가 1차 정정(저자 표기·소속 미확인)을 반영하지 않았고, 단계 3 페이지 6절 초안 갱신 조건 판정과 전환 줄의 열린 질문 범위, 시나리오 예외·성과 칸의 조건 병기를 고쳐야 한다. 트랙 개요는 내용 변경이 없어 pages 에서 빠졌으며 현재 단계 표시는 단계 1 유지(CLI 지정 단계 3 실행). 검증 검색 0회(2차, 도구 미사용).",
  "retry_reason": null,
  "track_checks": {
    "standard_sources_ok": true,
    "vendor_claims_tagged": true,
    "ontology_changes_grounded": true,
    "backlog_duplicates": [
      "ConstraintBench 처럼 해법기 최적해를 정답으로 두고 LLM 직접 스케줄의 실행 가능성·최적성 결합 비율을 재는 방식을 물류 창고 배정·스케줄링 시나리오로 옮기면, 제약 상호작용(충전·시간창·공용 자원)에 따라 결과가 어떻게 달라지는가? (q3-01 에서 파생) — q5-05 와 중복(제외 이행 확인)"
    ],
    "stage_tag_issues": [],
    "completeness_wording_ok": true,
    "stage_complete": false,
