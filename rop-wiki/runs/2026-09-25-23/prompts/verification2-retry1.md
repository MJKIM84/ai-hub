...
- 대상: 트랙 manual-capability-ontology (매뉴얼 기반 로봇 기능 온톨로지) · 현재 단계: 단계 1. 기존 능력 표현 모델과 표준 조사 · 이번에 다룰 백로그 질문 id: q1-03, q1-06, q1-07 · 중심 세부영역: 5. 로봇 능력·작업 온톨로지 (B. 공통 정보·환경 모델)
- 예산:
...
### runs/2026-09-25-23/target.json
...
```json
{
  ...
  "selection_rationale": "CLI 지정 run_type=track, area=5; 트랙 실행일(Fri, track_days 앞 3개) → 트랙 manual-capability-ontology 단계 1, 질문 q1-03, q1-06, q1-07 (사용자 지정 0건, 되돌아온 질문 0건, 현재 단계 열린 질문 6건 중 오래된 순)"
}
```

### runs/2026-09-25-23/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q1-03 이 모델들은 ROP가 배정·실행·검증에 필요로 하는 정보 — 전제조건, 파라미터 범위, 적재·환경 제약, 완료 확인 방법, 오류의 의미 — 를 얼마나 담는가? 빠진 것은 무엇인가? (이번 실행은 남은 학술 온톨로지 행 IEEE 1872 CORA·IEEE 1872.2·KnowRob·SOMA·SSN/SOSA 를 공식·공개 저장소 원문으로 대조)",
    "q1-06 부록 A 5번 정의의 요소(제조사별 기능·제약·장착 장비·실행 조건, 작업 요구)를 기준으로 ROP용 능력 개념에 무엇을 더 추가해야 하는가?",
    "q1-07 VDA 5050 3.0에서 팩트시트(유형 명세·적재 명세·지원 action)의 필드가 2.0 대비 어떻게 바뀌었으며, 적재 제약을 어떤 필드로 기술하는가?",
    "요구 능력과 제공 능력을 속성 단위로 비교(능력 매칭)하는 기존 연구는 ROP용 능력 개념 요구 목록에 무엇을 더하는가? (단계 1 완료 조건 둘째 항목, 온톨로지 초안 2절 겨냥)",
    "국내 자료에 로봇 능력 온톨로지나 능력 기반 작업 할당을 다룬 연구가 있는가? (한국 자료 우선 규칙)"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "VDA 5050 공식 저장소 2.1.0 태그의 팩트시트 JSON 스키마(제목 'AGV Factsheet')는 version·manufacturer·serialNumber 와 typeSpecification·physicalParameters·protocolLimits·protocolFeatures·agvGeometry·loadSpecification 블록을 필수로 두고, 버전·네트워크 정보를 담는 vehicleConfig 블록을 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-348"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "2.1.0 태그 factsheet.schema(github_raw 열람): 필수 블록 목록과 vehicleConfig(versions 키-값, network). 스키마 예시 판 표기 2.0.0. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f2",
      "claim": "VDA 5050 공식 저장소 main(3.0.0 판)의 팩트시트 JSON 스키마는 headerId·timestamp·version·manufacturer·serialNumber 와 typeSpecification·physicalParameters·protocolLimits·protocolFeatures·mobileRobotGeometry·loadSpecification 블록을 필수로, mobileRobotConfiguration 블록을 선택으로 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-228"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "main factsheet.schema(github_raw 열람): 최상위 required 11개와 선택 mobileRobotConfiguration(versions, network, batteryCharging). (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f3",
      "claim": "2.1.0 태그와 main(3.0.0) 팩트시트 스키마를 대조하면 agvGeometry→mobileRobotGeometry, vehicleConfig→mobileRobotConfiguration, agvActions→mobileRobotActions, resultDescription→actionResult, agvKinematic·agvClass·maxLoadMass→mobileRobotKinematics·mobileRobotClass·maximumLoadMass, speedMin·speedMax·accelerationMax·decelerationMax·heightMin·heightMax→minimumSpeed·maximumSpeed·maximumAcceleration·maximumDeceleration·minimumHeight·maximumHeight 로 이름이 바뀌었다.",
      "tag": "사실",
      "source_ids": [
        "ref-348",
        "ref-228"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "두 스키마 필드 목록을 열람 도구로 대조한 결과. 두 파일 모두 VDA5050 공식 저장소라 독립 교차 아님. 클래스 열거값(FORKLIFT·CONVEYOR·TUGGER·CARRIER)은 두 판이 같다. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f4",
      "claim": "main(3.0.0) 팩트시트 스키마는 2.1.0 태그 스키마에 없던 action 적용 범위 ZONE, action 필수 필드 pauseAllowed·cancelAllowed, 유형 명세의 supportedZones, 구성 블록의 batteryCharging(임계 저충전 수준·희망 최소·최대 충전 수준·최소 충전 시간)을 더했고, 지원·필수 선택 파라미터 목록(optionalParameters: parameter·support SUPPORTED/REQUIRED·description)은 두 판 모두 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-348",
        "ref-228"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "2.1.0 태그: actionScopes INSTANT·NODE·EDGE, pauseAllowed·cancelAllowed·supportedZones·배터리 충전 필드 없음. main: actionScopes에 ZONE, supportedZones 열거(BLOCKED, LINE_GUIDED, RELEASE 등 10종), batteryCharging 4필드. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "두 스키마의 loadSets 하위 필드(setName·loadType·loadPositions·boundingBoxReference·loadDimensions 는 공통, main 은 setName·loadType 만 필수). (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": "적치",
      "flow_item": "제약"
    },
    {
      "id": "f6",
      "claim": "2.x 와 3.0.0 팩트시트의 적재 제약은 적재 세트 단위(치수·최대 중량·취급 높이·깊이·기울기·적재 시 속도 한계·적재·하역 시간)라는 구조가 같고 필드 이름만 바뀐 것으로 보여, ROP 가 두 판을 함께 받으려면 필드 이름 대응표로 판 무관 속성에 정규화할 수 있을 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-348",
        "ref-228"
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f3·f5 의 대조에서 도출. 2.1.0 태그가 2.0.0 게시판과 필드 단위로 같은지는 확인하지 않음(2.0.0 태그에는 json_schemas/factsheet.schema 경로가 404).",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f7",
      "claim": "VDA 5050 공식 저장소 2.0.0 태그의 명세 마크다운(머리말 'RELEASE CANDIDATE' 표기)은 팩트시트 블록으로 localizationParameters 를 적지만, 2.1.0 태그와 main 스키마에서는 이 블록이 확인되지 않아 2.0.0 게시판의 블록 구성은 확정할 수 없는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-354",
        "ref-348",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "2.0.0 태그 VDA5050_EN_V1.md(github_raw 열람): 팩트시트 7개 객체 중 7번째로 localizationParameters 기술. main 스키마 열람 응답: 해당 블록 없음. 열람 도구 요약 기준.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f8",
      "claim": "W3C SSN 의 System Capabilities 모듈(ssn-system.ttl)은 조건 아래 시스템 성질을 기술하는 SystemCapability·OperatingRange·SurvivalRange·Condition 클래스와, 하위 성질 MeasurementRange·ActuationRange·Accuracy·Latency·ResponseTime 등, 운용 성질 MaintenanceSchedule·OperatingPowerRange, 생존 성질 SystemLifetime·BatteryLifetime 을 두고 hasSystemCapability·hasOperatingRange·inCondition 속성으로 잇는다.",
      "tag": "사실",
      "source_ids": [
        "ref-235"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "SystemCapability 주석: \"Describes normal measurement, actuation, sampling properties such as accuracy, range, precision, etc. of a System under some specified Conditions.\" (W3C sdw 저장소 편집본, /TR 판과 문구 차이 가능) (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f9",
      "claim": "SSN System Capabilities 모듈은 다섯 정보 항목 가운데 파라미터 범위(ActuationRange·조건 아래 성능)와 환경 제약(Condition·OperatingRange·SurvivalRange)을 담지만, 이번 열람 범위에서 전제조건·완료 확인 방법·오류 의미·적재 제약을 기술하는 클래스는 확인되지 않았다.",
      "tag": "추정",
      "source_ids": [
        "ref-235"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f8 클래스 목록에서 도출. SOSA 핵심 모듈(Actuation·Procedure)은 이번에 열지 않아 부재 확정 아님.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f10",
      "claim": "SOMA 공식 저장소의 SOMA-ACT 온톨로지는 작업 실행 상태 영역(ExecutionStateRegion)에 Active·Cancelled·Failed·Paused·Pending·Succeeded 여섯 상태를 두고, 전제 상황(hasRequiredInitialSituation)과 기대 종료 상황(hasExpectedTerminalSituation) 관계, 충족되지 않은 사후조건 같은 기대 불일치를 나타내는 NonmanifestedSituation 클래스를 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-349"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "ExecutionStateRegion 주석: 'A region containing labels that describe different states in the evolution/completion of a task execution.' PhysicalTask·GetTaskParameter 등 작업 클래스도 정의. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "완료·인계"
    },
    {
      "id": "f11",
      "claim": "SOMA 는 다섯 정보 항목 가운데 전제조건(요구 초기 상황), 완료 확인(기대 종료 상황과 Succeeded 상태), 오류 의미(Failed 상태와 기대 불일치 상황)를 담지만, 이번에 연 SOMA-ACT 파일에서 파라미터 허용 범위와 적재 제약은 확인되지 않았다.",
      "tag": "추정",
      "source_ids": [
        "ref-349"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f10 에서 도출. SOMA 의 다른 모듈(SOMA-OBJ 등)은 열지 않아 부재 확정 아님.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f12",
      "claim": "헬무트 슈미트 대학이 공개한 IEEE 1872.2 AuR 온톨로지 OWL 구현은 기능(Function)·기능 실행(FunctionExecution), 행동 분류(ArchitecturalBehavior·ManifestedBehavior·EmergentBehavior 등), 물리·정보 상호작용, 객체 중심 환경 기술 클래스를 두고 기능을 행동에 잇는 isPlayedBy 속성을 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-350"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "AuR_IEEE1872-2.ttl(github_raw 열람) 클래스 목록. 제3자 OWL 구현이며 IEEE 표준 문서 본문이 아님. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f13",
      "claim": "이 IEEE 1872.2 OWL 구현에서는 능력·전제조건·사후조건·파라미터 범위·실패·작업 완료 상태를 명시하는 클래스·속성이 확인되지 않아, 다섯 정보 항목을 구조로 담지 않는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-350"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "열람 도구 응답 기준의 부재 관찰(파일 전체를 글자 단위로 대조하지 않음). 표준 원문(유료)과 구현의 차이는 미확인.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f14",
      "claim": "IEEE 1872-2015 CORA 의 공개 OWL 번역(cora-bare.owl)은 Robot·RobotGroup·RobotInterface·RoboticEnvironment·RoboticSystem·SingleRoboticSystem·CollectiveRoboticSystem 클래스와 equippedWith·robotPart 및 자율성 수준(자율·반자율·원격 조종·원격 조작) 속성을 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-355"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "cora-bare.owl(github_raw 열람)은 스스로를 CORA 개념·관계만 분리한 'bare implementation'이라 설명. 저장소 README 기준 IEEE 1872 명세를 수작업으로 OWL 번역(제3자). (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "수행 자원"
    },
    {
      "id": "f15",
      "claim": "CORA 는 로봇·로봇 시스템의 분류와 부품·장착 관계를 표현할 뿐 다섯 정보 항목(전제조건·파라미터 범위·적재·환경 제약·완료 확인·오류 의미)과 능력·작업 개념은 담지 않는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-355"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f14 의 클래스·속성 목록에서 도출. POS·CORAX·RPARTS 파일은 열지 않음.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f16",
      "claim": "KnowRob 공식 저장소 README 는 KnowRob 을 RDF·RDFS·OWL 어휘의 맥락화된 트리플로 지식을 표현하고 여러 추론기의 결과를 결합하는 하이브리드 지식 베이스로 설명하며, 저장 백엔드로 Prolog 기반 저장소·MongoDB·Redland 를 들고, 로봇 제어 구조에 묻힌 암묵 지식을 온톨로지 기반 데이터 접근으로 명시화한다고 적는다.",
      "tag": "사실",
      "source_ids": [
        "ref-351"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README(dev 브랜치, github_raw 열람): 'ensemble of reasoners approach where inferences of different reasoners are combined'. 현재 C++ 구현, Prolog 선택 지원. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f17",
      "claim": "KnowRob README 는 전제조건·파라미터 범위·실패 같은 항목을 직접 다루지 않아, KnowRob 행의 다섯 정보 항목은 README 만으로 판정할 수 없고 KnowRob 이 쓰는 도메인 온톨로지(SOMA 등) 쪽 판정에 기대야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-351",
        "ref-349"
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "README 에 SOMA·DUL 명칭은 이번 열람 응답에 없었음. KnowRob–SOMA 관계는 이전 실행의 논문 요약(ref-027·ref-028) 수준.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f18",
      "claim": "원문을 연 학술 온톨로지 기준으로 전제조건·완료 확인·오류 의미는 SOMA 가, 파라미터 범위·환경 제약은 SSN System Capabilities 가 일부 담고, CORA·IEEE 1872.2 구현은 다섯 항목을 담지 않으며, 적재 제약은 어떤 학술 온톨로지에서도 확인되지 않고 VDA 5050 적재 세트 같은 산업 규격에만 있어, ROP 는 적재 제약과 오류의 조치 의미(등급·재시도)를 산업 규격 쪽에서 가져와야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-349",
        "ref-235",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f8~f15 와 f5 를 다섯 항목에 대응시킨 이 위키의 종합. 산업 규격 행 판정은 2026-09-25-16 실행 결과(재인용: 2026-09-25-16).",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f19",
      "claim": "IDTA 02020 능력 기술 1.0 README 는 이 서브모델이 공정·제품 쪽 요구 능력과 자원 쪽 제공 능력을 비교해 생산 계획·오케스트레이션을 돕는다고 설명한다.",
      "tag": "사실",
      "source_ids": [
        "ref-229"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README(github_raw 열람): \"This enables reliable comparison between required and provided capabilities and supports efficient planning and orchestration of production processes.\" (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "수행 자원",
    ...
    {
      "id": "f20",
      "claim": "Järvenpää 외(IJCIM 36(1), 2023)는 제품과 자원의 온톨로지 기술과 SPIN 규칙으로 결합 자원의 결합 능력 파라미터를 추론하고 제품 특성을 자원(조합)의 능력 파라미터와 비교해 가능한 자원 조합을 찾는 능력 매칭 소프트웨어를 제시했다.",
      "tag": "사실",
      "source_ids": [
        "ref-352"
      ],
      ...
      "confidence": "medium",
      "source_unopened": true,
      "evidence_excerpt": "검색 요약: 규칙은 1) 결합 자원의 결합 능력 파라미터 추론·단언, 2) 제품 특성과 자원 능력 파라미터 비교. 제조 시스템 설계·재구성 대상. 원문 미열람.",
      "as_of": "2023",
      "flow_step": null,
      "flow_item": "수행 자원"
    },
    {
      "id": "f21",
      "claim": "Köcher·Vieira da Silva·Fay(arXiv 2312.08801, AAAI 2024 CAIPI 워크숍)는 의미 능력 모델과 SMT 로 제품 생산이나 자율 로봇 임무 수행에 필요한 개별 능력의 순서를 자동으로 찾는 공정 계획 방법을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-353"
      ],
      ...
      "confidence": "medium",
      "source_unopened": true,
      "evidence_excerpt": "검색 요약: 'finding a sequence of individual capabilities required to manufacture a certain product or to accomplish a mission using autonomous robots'. v2 2024-02. 원문 미열람.",
      "as_of": "2023-12",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f22",
      "claim": "VDA 5050 main(3.0.0) 팩트시트는 적재 취급 장치 식별자 목록(loadPositions), 위치추정·주행 방식(localizationTypes·navigationTypes), 지원 구역 유형(supportedZones), 하드웨어·소프트웨어 버전 키-값(mobileRobotConfiguration.versions), 충전 설정(batteryCharging)을 기술하게 해, 장착 장비·운용 구역·버전·충전 조건을 로봇 선언의 일부로 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-228"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "main factsheet.schema 열람: loadPositions 는 유효한 적재 취급 장치 식별자 배열, localizationTypes 열거 NATURAL·REFLECTOR·RFID·DMC·SPOT·GRID. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "수행 자원"
    },
    {
      "id": "f23",
      "claim": "IDTA 02047 무인운반차 기술 데이터 1.0 템플릿은 부착 장비 인터페이스(InterfacesForAttachments)와 속도·가동 시간의 명세값(AsSpecified)·운용값(AsOperated) 쌍을 속성으로 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-245"
      ],
      ...
      "confidence": "medium",
      "source_unopened": true,
      "evidence_excerpt": "템플릿 속성 InterfacesForAttachments, SpeedMaxWithMaxLoadAsSpecified/AsOperated, MaxRunTimeAsSpecified/AsOperated. 이번 실행 재열람 안 함 (재인용: 2026-09-25-16)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "수행 자원"
    },
    {
      "id": "f24",
      "claim": "분류 원문 5. 로봇 능력·작업 온톨로지 정의 요소를 기준으로 온톨로지 초안 v0.2 에 더할 ROP용 능력 개념 요구 후보는 (1) 기능의 요구·제공 구분과 속성 단위 비교, (2) 기능 속성의 값·범위, (3) 전제·사후조건, (4) 실행 상태, (5) 로봇의 하드웨어·소프트웨어 구성 버전, (6) 장착 장비의 부착 인터페이스·적재 취급 장치 위치, (7) 운용 구역·환경 조건, (8) 충전 조건, (9) 결합 자원의 결합 능력인 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-229",
        "ref-352",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f19~f23 과 f8·f10·f14 를 원문 정의 요소(기능·제약·장착 장비·실행 조건·작업 요구)에 대응시킨 이 위키의 요구 목록 초안. 이 목록을 제시한 단일 출처는 없음.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f25",
      "claim": "분류 원문의 질문(같은 운반 로봇 중 누가 이 화물을 실제로 취급할 수 있는가)에 답하려면 작업 요구를 화물 치수·중량 같은 요구 능력 속성으로 표현하고 로봇의 적재 세트 같은 제공 능력 속성과 비교하는 규칙이 필요할 것으로 보이며, 제조 분야의 능력 매칭 연구가 이 비교를 의미 규칙으로 구현한 선례가 된다.",
      "tag": "추정",
      "source_ids": [
        "ref-229",
        "ref-352",
      ...
      "evidence_excerpt": "f5·f19·f20 과 적재 상태별 도달 가능성 판정 연구(ref-236, 재인용: 2026-09-25-16)에서 도출. 물류 이동로봇 대상의 능력 매칭 규칙은 찾지 못함.",
      "as_of": "2026-09-25",
      "flow_step": "적치",
      "flow_item": "수행 자원"
    }
  ...
  "sources": [
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "VDA 5050 팩트시트 JSON 스키마(main, 3.0.0 판). 이번 실행은 최상위 블록, 적재 세트 필드, action 정의, 선택 파라미터, 구성 블록(versions·batteryCharging), supportedZones 를 확인했다.",
      "fetched": true,
    ...
    },
    {
      "id": "ref-348",
      "org": "VDA / VDMA (VDA5050 GitHub)",
      "title": "VDA5050/VDA5050 (tag 2.1.0) — json_schemas/factsheet.schema",
      "published": null,
      "url": "https://github.com/VDA5050/VDA5050/blob/2.1.0/json_schemas/factsheet.schema",
      "type": "표준",
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "VDA 5050 공식 저장소 2.1.0 태그의 팩트시트 JSON 스키마('AGV Factsheet'). agvGeometry·agvActions·vehicleConfig 등 2.x 필드 이름과 적재 세트 필드를 정의한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      "id": "ref-354",
      "org": "VDA / VDMA (VDA5050 GitHub)",
      "title": "VDA5050/VDA5050 (tag 2.0.0) — VDA5050_EN_V1.md",
      "published": null,
      "url": "https://github.com/VDA5050/VDA5050/blob/2.0.0/VDA5050_EN_V1.md",
      "type": "표준",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "공식 저장소 2.0.0 태그의 명세 마크다운. 머리말에 RELEASE CANDIDATE 표기가 있어 VDA 게시 PDF 와 같다고 확인하지 못했다. 팩트시트 블록 구성 설명을 확인했다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "SSN 시스템 능력 모듈의 온톨로지 파일(작업반 편집본). SystemCapability·OperatingRange·SurvivalRange·Condition 과 하위 성질 클래스를 정의한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "SOMA 공식 저장소의 활동 모듈 OWL 파일. 작업·행동 클래스, 실행 상태 영역 6상태, 요구 초기 상황·기대 종료 상황 관계, 기대 불일치 상황 클래스를 정의한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "title": "IndustrialStandard-ODP-IEEE1872-2 — AuR_IEEE1872-2.ttl",
      "published": null,
      "url": "https://github.com/hsu-aut/IndustrialStandard-ODP-IEEE1872-2/blob/main/AuR_IEEE1872-2.ttl",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "IEEE 1872.2 AuR 온톨로지의 제3자 OWL 구현 파일. 기능·기능 실행·행동·상호작용·환경 기술 클래스를 둔다. IEEE 표준 본문이 아니다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "title": "IEEE1872-owl — cora-bare.owl (OWL specification of CORA)",
      "published": null,
      "url": "https://github.com/srfiorini/IEEE1872-owl/blob/master/cora-bare.owl",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "IEEE 1872-2015 CORA 의 개념·관계만 분리한 수작업 OWL 번역(제3자). 로봇·로봇 시스템 분류와 equippedWith·robotPart·자율성 수준 속성을 둔다. IEEE 표준 본문이 아니다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "title": "knowrob — README (dev branch)",
      "published": null,
      "url": "https://github.com/knowrob/knowrob",
      "type": "오픈소스 문서",
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "KnowRob 공식 저장소 README. RDF·OWL 기반 하이브리드 지식 베이스, 추론기 앙상블, 저장 백엔드, 온톨로지 기반 데이터 접근을 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "IDTA 능력 기술 서브모델 1.0 README. 요구 능력과 제공 능력의 비교, 속성, 속성 제약·전이 제약, 스킬에 의한 구현을 설명한다.",
      "fetched": false,
      "fetched_via": null,
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서는 다시 열지 않았다(2026-09-25-16 실행에서 원문 확인). 무인운반차 기술 데이터 서브모델 템플릿으로 적재 질량·명세값·운용값·부착 인터페이스 속성을 정의한다.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 로봇·작업·장소의 의미 모델과 혼합 추론으로 이종 로봇의 수행 가능성과 적재 상태별 장소 도달 가능성을 판정하는 방법을 제안한 논문.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "title": "Semantic rules for capability matchmaking in the context of manufacturing system design and reconfiguration",
      "published": "2023",
      "url": "https://www.tandfonline.com/doi/full/10.1080/0951192X.2022.2081361",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 제품·자원 온톨로지와 SPIN 규칙으로 결합 능력 파라미터를 추론하고 제품 요구와 자원 능력을 비교하는 능력 매칭 방법(IJCIM 36(1) 128-154).",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    ...
    {
      "id": "ref-353",
      "org": "Köcher, A., Vieira da Silva, L. M., & Fay, A.",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 의미 능력 모델과 SMT 로 제품 생산·자율 로봇 임무에 필요한 능력 순서를 찾는 공정 계획 프리프린트(AAAI 2024 CAIPI 워크숍).",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
  ...
  "page_proposals": [
    ...
    },
    {
      ...
      "rationale": "트랙 산출물 갱신: IEEE 1872 CORA 행(f14·f15, 제3자 OWL 번역 원문), IEEE 1872.2 행(f12·f13, 제3자 OWL 구현), KnowRob·SOMA 행(f10·f11·f16·f17), W3C SSN/SOSA 행(f8·f9)의 다섯 정보 항목 열 채움. VDA 5050 행 종류 칸에 2.x→3.0.0 필드 이름 변화와 추가 필드(f3·f4·f5) 반영. 5절 빠진 정보 요약을 f18 로 갱신(적재 제약은 산업 규격에만 있음)"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신: track.ontology_changes 가 검증 승인되면 ROP용 능력 개념 요구 목록 초안(f24)을 반영 — 기능 한정자(요구·제공, f19·f20), 개념 실행 상태(f10), 로봇 구성 버전(f22), 장착 장비 부착 인터페이스·적재 취급 장치 위치(f14·f22·f23), 제약 종류 값(운용 구역·환경 조건·충전 조건, f8·f22). 제약 값 추가는 6절 '실행 조건과 제약의 경계' 질문과 충돌할 수 있어 미승인 시 6절 질문으로"
    },
    {
      "action": "update",
      ...
      "rationale": "아이디어 페이지 3절: 능력 매칭 선행 연구(f20·f21)와 요구·제공 능력 비교(f19), 분류 원문 질문 연결(f25) / 아이디어 페이지 4절: 범위 능력 '충전'에 팩트시트 batteryCharging(f4·f22), '적재'에 적재 세트 판별 필드 이름(f5), 장착 장비 표현(f14·f22·f23)"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 manual-capability-ontology 단계 1 반영 제안 (f14, f18, f19, f20, f24, f25): 섹션 4 요구 능력·제공 능력, 섹션 7 CORA·SSN·SOMA 가 다섯 정보 항목을 담는 정도, 섹션 8 능력 매칭 연구(Järvenpää 외, Köcher 외). 분류 원문 SCM 질문과 요구–제공 능력 비교 규칙 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 manual-capability-ontology 단계 1 반영 제안 (f3, f4, f5, f6): VDA 5050 2.x 와 3.0.0 팩트시트의 필드 이름 변화와 추가 필드(ZONE 범위, pauseAllowed·cancelAllowed, supportedZones, batteryCharging) — 어댑터가 두 판을 함께 받을 때의 정규화 필요"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 manual-capability-ontology 단계 1 반영 제안 (f4, f22): VDA 5050 3.0.0 팩트시트의 충전 설정(임계 저충전 수준·희망 최소·최대 충전 수준·최소 충전 시간)이 충전 시점 계획의 입력이 될 수 있음"
    }
  ],
  "glossary_candidates": [
    {
    ...
    },
    {
      ...
      "definition": "공정·작업 쪽이 필요로 하는 능력과 자원 쪽이 내놓는 능력을 구분한 표현으로, 둘을 비교해 작업을 맡을 자원을 정한다."
    },
    {
      "term_ko": "능력 기반 작업 배정",
    ...
    "ontology_changes": [
      {
        ...
        "description": "속성 '한정자(요구 능력 / 제공 능력)'를 더한다(IDTA 02020 의 요구·제공 비교, 능력 매칭 연구의 제품 요구 대 자원 능력 비교). 작업 요구는 요구 능력 속성 값으로 표현되어 제공 능력 속성과 비교된다는 메모를 둔다. 기존 관계 '작업 요구 / 기능과 대응된다 / 기능'과 충돌하지 않고 그 판정 방식을 구체화한다."
      },
      {
        "op": "add",
        "kind": "concept",
        ...
        "description": "기능·스킬 실행의 진행 단계(대기·진행·일시정지·취소·실패·성공 등). SOMA 실행 상태 영역 6상태에 대응하며 완료 확인 방법의 판정 대상이다. 스킬 인터페이스·오류와의 관계는 근거가 더 필요해 이번에는 개념만 제안한다."
      },
      {
        "op": "modify",
        "kind": "concept",
        ...
        "description": "v0 의 '펌웨어 버전(단계 6에서 확정)' 속성을 '구성 버전(하드웨어·소프트웨어 버전 키-값)'으로 구체화한다(VDA 5050 3.0.0 mobileRobotConfiguration.versions)."
      },
      {
        "op": "modify",
        "kind": "concept",
        "name": "장착 장비 (Mounted Equipment)",
        "evidence_finding_ids": [
          "f14",
          "f22",
          "f23"
        ],
        "description": "속성 '부착 인터페이스'(IDTA 02047 InterfacesForAttachments)와 '적재 취급 장치 위치'(VDA 5050 loadPositions)를 더한다. 관계 '로봇 / 장착 장비를 갖춘다'는 CORA equippedWith 속성과 대응한다는 근거 메모를 붙인다."
      },
      {
        "op": "modify",
        "kind": "concept",
        ...
        "description": "속성 '종류'의 값 후보에 '운용 구역(VDA 5050 supportedZones)', '환경 조건(SSN Condition·OperatingRange)', '충전 조건(VDA 5050 batteryCharging)'을 더한다. 충전 수준은 실행 시점에 확인하는 실행 조건일 수도 있어 초안 6절 '실행 조건과 제약의 경계' 질문과 충돌할 수 있다."
      }
    ],
...
### runs/2026-09-25-23/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 검증자가 raw 2.1.0 태그 factsheet.schema 를 열었다. 제목 'AGV Factsheet', 필수 9항목, 선택 vehicleConfig(versions·network), 스키마 예시 판 2.0.0 이 일치한다. 단일 발행 주체이며 발행일 미확인."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: main factsheet.schema 최상위 required 11개와 선택 mobileRobotConfiguration(versions·network·batteryCharging)이 일치한다. 발행일 미확인, 확인일 2026-09-25."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 두 스키마에서 나열된 이름 변경을 확인했다. 같은 저장소라 독립 교차가 아니다. 검증자는 운동학 열거값이 DIFF·OMNI·THREEWHEEL 에서 DIFFERENTIAL·OMNIDIRECTIONAL·THREE_WHEEL 로 바뀐 것도 확인했으므로, f3 목록은 완결 목록이 아니다."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 2.1.0 태그에는 ZONE·pauseAllowed·cancelAllowed·supportedZones·batteryCharging 이 없고 main 에는 있다. main 에서는 pauseAllowed·cancelAllowed 가 필수다. supportedZones 10종과 optionalParameters 는 main 에서 확인했다. 2.1.0 의 optionalParameters 는 검증자가 확인하지 않았다(브리프 기록 기준)."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 2.1.0 loadSets 필드, main loadSets(필수 setName·loadType, maximum* 필드), maxLoadMass→maximumLoadMass 가 일치한다."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "f3·f5 에서 도출한 추론이므로 [추정] low 를 유지한다. '2.x' 는 2.1.0 태그 기준임을 밝혀야 한다."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 2.0.0 태그 마크다운 머리말에 'RELEASE CANDIDATE, FOR REVIEW' 표기가 있다. 7.1.3 절은 7개 객체 가운데 7번째로 localizationParameters 를 적는다. 2.1.0·main 스키마에는 이 객체가 없다."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ssn-system.ttl 의 클래스·속성 목록과 SystemCapability 주석 인용이 일치한다. 작업반 편집본이라 /TR 판과 문구가 다를 수 있다."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검증자가 연 파일에도 전제조건·실패·완료·적재 클래스가 없다. SOSA 핵심 모듈은 열지 않았으므로 부재 확정은 아니다."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ExecutionStateRegion(주석 일치)과 6개 상태, hasRequiredInitialSituation·hasExpectedTerminalSituation, 사후조건 미충족을 예로 드는 NonmanifestedSituation, PhysicalTask·GetTaskParameter 가 있다."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "f10 에서 도출했다. 검증자가 연 파일에도 파라미터 범위·적재 제약 클래스가 없다. 다른 모듈은 열지 않았으므로 부재 확정은 아니다."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 기능·행동·상호작용·환경 기술 클래스와 isPlayedBy 가 있다. 제3자 OWL 구현이며 IEEE 표준 본문이 아니다."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검증자가 열람한 범위에서도 능력·전제조건·사후조건·범위·실패·완료 클래스가 없다. 유료 표준 원문과의 차이는 미확인이다."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 'bare implementation of CORA only' 문구, 7개 클래스, equippedWith·robotPart, 자율성 하위 속성이 있다. 다만 equippedWith 는 정의역이 RoboticEnvironment, 치역이 RoboticSystem 이다. 로봇–장착 장비 관계가 아니다."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "다섯 항목과 능력·작업 개념이 없다는 점은 검증자 확인과 맞는다. 그러나 '장착 관계'라는 표현은 원문과 맞지 않으므로 문구 수정을 지시한다."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 하이브리드 지식 베이스, RDF 트리플, 추론기 앙상블(인용 일치), Prolog·MongoDB·Redland, C++ 구현과 선택 Prolog, OBDA 가 확인된다. KnowRob 2.0 을 Prolog 구현으로 적은 게시 문장(ref-027)과는 판이 다르다."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "README 에 SOMA·DUL 언급이 없다는 점을 검증자도 확인했다."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "근거 finding 이 모두 확인됐다. 판정 범위는 원문을 연 학술 온톨로지 파일로 한정되며, PDDL·KnowRob 자체는 포함하지 않는다."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검증자가 raw README 에서 인용 구절을 확인했다. 브리프는 ref-229 를 fetched=false 로 기록하면서 excerpt 에는 github_raw 열람이라고 적어 표시가 어긋난다(R-1)."
    },
    {
      "finding_id": "f20",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검색으로 저자, IJCIM 36(1) 128-154(2023, 온라인 2022-06-07), DOI 를 확인했다. 이 논문의 기여는 매칭 '규칙'이다. 매칭 소프트웨어는 같은 연구진의 별도 논문이므로 문구를 수정한다."
    },
    {
      "finding_id": "f21",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검색으로 저자 3인, 2023-12-14 제출·2024-02-14 개정, AAAI 2024 CAIPI 워크숍을 확인했다. 요구·제공 능력을 SMT 계획 문제로 바꾼다는 요약이 주장과 맞는다."
    },
    {
      "finding_id": "f22",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: main 스키마에서 loadPositions('load positions / load handling devices'), localizationTypes 6종, navigationTypes, supportedZones, versions, batteryCharging 을 확인했다."
    },
    {
      "finding_id": "f23",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이번 실행에서 다시 열지 않았다. 실행 2026-09-25-16 검증에서 원문으로 확인된 속성이며, 여기서는 재인용이다."
    },
    {
      "finding_id": "f24",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 종합 요구 목록이며 단일 출처는 없다. 근거 finding 은 모두 확인됐다. 9개 후보 가운데 일부만 온톨로지 변경으로 승인했다."
    },
    {
      "finding_id": "f25",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추론이므로 [추정] low 를 유지한다. 원문 미열람 출처에 기대는데도 source_unopened: false 로 적혀 있어 표시가 누락됐다."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": false,
    "overlaps": [
      "f1~f5·f7 은 단계 1 페이지 q1-02 절의 기존 [추정] 팩트시트 블록 문장(판 미확인, ref-031)을 판별 원문 근거로 대체한다. 판 차이이며 충돌은 아니다",
      "f16(dev 브랜치 C++ 구현)은 게시된 'KnowRob 2.0은 Prolog로 구현'(ref-027) 문장과 판이 다르다",
      "f8 은 게시된 SSN System Capabilities 문장(ref-030)과 겹친다",
      "f12(ref-350)는 기존 ref-232(같은 저장소 README)와 같은 구현을 가리킨다",
      "새 질문(요구 능력–적재 세트 비교 규칙 형식, f25)은 q4-07 과 중복이다",
      "기존 백로그 중복 등록: q1-10=q1-09, q4-11=q4-09, q5-08=q5-07",
      "온톨로지 변경의 'CORA equippedWith 대응' 메모는 CORA 원문 정의역·치역과 충돌한다"
    ]
  },
  "terminology": {
    "ok": false,
    "conflicts": [
      "용어 후보 '요구 능력·제공 능력'은 온톨로지 초안이 capability 를 '기능(Capability)'으로 부르는 용어집 규약(능력·스킬·서비스 모델 항목)과 맞지 않는다. 초안에서는 속성 이름을 '한정자(요구 / 제공)'로 쓰고 용어집 정의에 그 대응 관계를 밝혀야 한다"
    ]
  },
  "quotation_check": {
    "ok": true
  ...
  "required_fixes": [
    "q1-03·q1-06·q1-07 은 답함으로 처리한다. 3절 q1-03 소제목에서 '(부분 답)'을 빼고 '### q1-03 … {#q1-03}'으로 바꾸며, q1-06·q1-07 소제목에 '{#q1-06}'·'{#q1-07}'을 붙인다. 4절 남은 불확실성에는 KnowRob 판정이 README 범위에 그친다는 점(f17)과 PDDL 행은 검색 요약 수준이라는 점을 남긴다.",
    "q1-07 답(f1~f7): '2.x'는 공식 저장소 2.1.0 태그 스키마 기준이고 2.0.0 게시판과 필드 단위로 대조하지 못했음(2.0.0 태그에는 팩트시트 스키마가 없고 명세는 RELEASE CANDIDATE 표기)을 명시한다. f3·f4 는 '확인된 변화'로 쓰고 완결 목록처럼 쓰지 않는다 — 검증자가 목록 밖 변화(운동학 열거값 변경)를 확인했다.",
    "단계 1 페이지 q1-02 절: '팩트시트는 … agvGeometry … 블록으로 구성되는 것으로 보인다(판 미확인 …) [추정][^ref-031]' 문장은 2.1.0 태그 구성(f1, [사실][^ref-348])과 3.0.0 main 구성(f2, [사실][^ref-228])으로 판을 나눠 바꿔 쓴다. 2.0.0 RC 문서의 localizationParameters 는 f7 에 따라 [추정]으로 병기한다.",
    "f15: '부품·장착 관계'를 '부품 관계(robotPart)와 로봇 환경이 로봇 시스템을 갖춘다는 관계(equippedWith)'로 고친다 — cora-bare.owl 의 equippedWith 는 정의역이 RoboticEnvironment, 치역이 RoboticSystem 이다. f14 를 쓸 때도 equippedWith 를 로봇–장착 장비 관계로 설명하지 않는다.",
    "f16: 'KnowRob 2.0(2018 논문)은 Prolog 구현'(ref-027)과 '현재 dev 브랜치는 C++ 구현이며 Prolog 는 선택 지원'(ref-351, 확인일 2026-09-25)을 판을 구분해 함께 두고, 기존 문장은 지우지 않는다.",
    "f20: '능력 매칭 소프트웨어를 제시했다'를 '능력 매칭을 위한 의미 규칙(SPIN)을 제시했다'로 고치고, 기준일은 2023(IJCIM 36(1), 온라인 2022-06-07)으로 적는다.",
    "ref-229: 각주는 단계 1 페이지에 이미 있는 줄을 그대로 쓰고 reference_updates 에서 표기를 바꾸지 않는다 — 검증자가 raw README 에서 인용 구절을 확인했다.",
    "ref-245·ref-236·ref-352·ref-353 의 각주에는 접근일 뒤에 ' (원문 미열람)'을 붙이고 source_unopened: true 로 둔다. ref-348·ref-354·ref-235·ref-349·ref-350·ref-355·ref-351·ref-228 은 이번에 원문을 열었으므로 그 표기를 붙이지 않는다.",
    "ref-350·ref-355 는 제3자 OWL 구현·번역이며 IEEE 표준 본문이 아님을, ref-354 는 RELEASE CANDIDATE 표기 문서임을 본문과 비교표 칸에 유지한다.",
    "온톨로지 초안: 승인한 변경 3건만 반영하고 버전을 '0.2' → '0.3'으로 올린다(H1, 프런트매터 ontology_version, track_updates.ontology_draft_version). (1) 기능 속성에 '한정자(요구 / 제공)'를 더한다(f19·f20). 작업 요구를 요구 쪽 속성 값으로 표현한다는 메모는 [추정]으로 둔다. (2) 새 개념 '실행 상태'를 '확정'으로 넣는다(f10, SOMA-ACT 6상태 대응). 다른 개념과의 관계는 넣지 않는다. (3) 장착 장비 속성에 '부착 인터페이스'(f23)와 '적재 취급 장치 위치'(f22)를 더하고 상태를 초안 → 확정으로 바꾼다. 4절 다이어그램에 '실행 상태' 노드를 더하고 도식 아래 문장을 고친다.",
    "온톨로지 초안 6절 미해결 질문으로 둘 것: (a) 로봇의 '펌웨어 버전' → '구성 버전' 수정(f22) — 단일 규격 필드에만 기대며, 버전 대응은 단계 6 q6-02 의 몫이다. (b) 제약 종류에 운용 구역·환경 조건·충전 조건 추가(f8·f22) — 기존 '실행 조건과 제약의 경계' 질문과 충돌한다. (c) '로봇 / 장착 장비를 갖춘다' 관계와 CORA equippedWith 의 대응(f14) — 원문 정의역·치역과 맞지 않는다.",
    "ROP용 능력 개념 요구 목록: f24 의 9개 후보를 온톨로지 초안 6절(또는 1절 끝)에 '[추정] ROP용 능력 개념 요구 목록 초안(실행 2026-09-25-23, f24)'으로 싣는다. 항목마다 'v0.3 반영'(요구·제공 한정자, 실행 상태, 부착 인터페이스·적재 취급 장치 위치), '미반영 — 6절 질문'(구성 버전, 운용 구역·환경 조건, 충전 조건), '변경 제안 없음'(속성 값·범위, 전제·사후조건, 결합 능력)을 표시한다.",
    "새 질문 '작업 요구를 … 요구 능력 속성으로 표현하고 … SPARQL·SHACL·SPIN 등 …'(f25)은 q4-07 과 중복이므로 backlog_updates 에 넣지 않는다. 새 질문 'VDA 5050 2.x·3.0.0 필드 이름 정규화와 판 변경 시 재검증'(단계 6, f6)은 등록한다.",
    "백로그 정리: q1-10(q1-09 중복), q4-11(q4-09 중복), q5-08(q5-07 중복)을 backlog_updates 에서 '폐기'로 바꾸고, 단계 페이지 2절 표에서 q1-10 을 뺀다.",
    "단계 페이지 6절·상태 줄: 비교표 조건은 모든 후보 행이 채워지면 '충족'으로 적을 수 있다(KnowRob 칸은 'README 범위에서 판정 불가 — SOMA 행 참조'). 요구 목록 조건은 '미충족(초안은 실었으나 일부만 개념에 반영)'으로 적는다. 전환은 '아니오(요구 목록 일부 미반영, 막힌 질문 q1-08·q1-09)'로 적는다. 답한 질문 7건과 열린 질문 수는 백로그와 맞춘다.",
    "모델·표준 비교표: IEEE 1872 CORA(f14·f15), IEEE 1872.2(f12·f13), KnowRob·SOMA(f10·f11·f16·f17), W3C SSN/SOSA(f8·f9) 행을 채운다. 상태는 '확인(제3자 구현/편집본 원문 열람)'으로 적고, 부재 관찰은 '[추정](열람 파일 기준, 부재 확정 아님)'으로 적는다. VDA 5050 행의 2.x 기준이 2.1.0 태그임을 밝힌다.",
    "용어집: '요구 능력·제공 능력' 정의 끝에 '이 위키의 온톨로지 초안에서는 capability 를 기능으로 부르므로 요구·제공 한정자에 해당한다'를 더한다."
  ],
  "confidence": "medium",
  "verification_note": "판정: 1차 조건부 승인. 이번 실행은 원문 열람이 차단된 환경(mirror_only)에서 검증됐다. 검증자가 raw.githubusercontent.com 원문 9건(VDA 5050 2.1.0·main 팩트시트 스키마, 2.0.0 명세, SSN 시스템 능력 모듈, SOMA-ACT, IEEE 1872.2 제3자 OWL, CORA 제3자 OWL, KnowRob README, IDTA 02020 README)을 직접 대조했고, 논문 2건은 검색으로 확인했다(검색 2회, 리서치와 합쳐 8/40). 확인 25건, 미확인 0건, 교차 확인 0건. 강등: 없음. 문구 수정: f15(CORA equippedWith 는 로봇 환경→로봇 시스템 관계), f20(매칭 소프트웨어가 아니라 매칭 규칙). 원문 미열람 출처: ref-245, ref-236, ref-352, ref-353, ref-229(브리프는 미열람으로 기록했으나 검증자가 README 구절을 확인해 표시가 불일치). 브리프 출처 원문 미열람 표시 누락: f25. 미사용 출처: 없음. 주의: q1-07 의 2.x 비교는 2.1.0 태그 기준이며 2.0.0 게시판 필드 대조는 미확인이다. 3.0.0 변경 목록은 완결 목록이 아니다. 학술 온톨로지 판정은 연 파일 기준의 부재 관찰이며, KnowRob 은 README 범위에서 판정할 수 없다. 한국 자료는 없다. 온톨로지 변경 승인: 기능 수정(한정자 요구/제공, f19·f20), 실행 상태 추가(f10), 장착 장비 수정(부착 인터페이스 f23·적재 취급 장치 위치 f22), 시드 개념 확정: 장착 장비(f22·f23) → v0.3. 거부: 로봇 수정(f22), 제약 종류 수정(f8·f22, 실행 조건–제약 경계 질문과 충돌), CORA equippedWith 대응 메모(f14, 원문 불일치) — 셋 다 6절 질문으로 둔다. 백로그 중복: f25 새 질문은 q4-07 과 중복이고, 기존 q1-10·q4-11·q5-08 은 폐기 지시. q1-03·q1-06·q1-07 답 인정. 단계 완료 조건: 미충족(부족: ROP용 능력 개념 요구 목록 초안의 일부만 온톨로지 초안에 반영. 비교표는 이번 실행으로 채워질 수 있다는 예비 판정). 단계 전환: 미승인(막힌 질문 q1-08·q1-09).",
  "track_checks": {
    "standard_sources_ok": true,
    "vendor_claims_tagged": true,
    "ontology_changes_grounded": false,
    "backlog_duplicates": [
      "작업 요구를 화물 치수·중량 같은 요구 능력 속성으로 표현하고 로봇 적재 세트와 비교하는 규칙을 어떤 질의·규칙 형식(SPARQL·SHACL·SPIN 등)으로 표현할 수 있는가 … (q4-07 과 중복)",
      "q1-10 (q1-09 와 중복)",
      "q4-11 (q4-09 와 중복)",
      "q5-08 (q5-07 과 중복)"
    ],
...
```json
{
  ...
  "outline": [
    {
      ...
      "section": "3. 조사 결과",
      "budget_chars": 16000,
      "summary": "q1-03 은 학술 온톨로지 행 원문 대조를 더해 답함으로, q1-06 은 요구·제공 능력 비교와 요구 목록 초안으로, q1-07 은 2.1.0 태그 대 main 팩트시트 스키마 대조로 답했다. [추정][^ref-349][^ref-235][^ref-228]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 3500,
      "summary": "적재 제약은 산업 규격에만 있고 전제·완료·오류는 SOMA, 범위·환경은 SSN 이 일부 담는다. [추정][^ref-349][^ref-235][^ref-228]",
      "planned_findings": [
        "f17",
        "f18",
        "f24"
    ...
    },
    {
      ...
      "budget_chars": 900,
      "summary": "비교표는 모든 행에 조사 결과가 있으나 일부 행에 미조사 칸이 남고, 요구 목록은 일부만 반영돼 단계 전환은 미승인이다.",
      "planned_findings": [
        "f24"
      ]
    },
    {
      ...
      "budget_chars": 4000,
      "summary": "v0.3: 기능 한정자(요구/제공), 실행 상태 개념, 장착 장비 속성 2건·확정. 거부 3건은 6절 질문. [추정][^ref-229][^ref-349][^ref-245]",
      "planned_findings": [
        "f10",
        "f19",
        "f20",
    ...
    {
      "path": "docs/tracks/manual-capability-ontology/model-standard-comparison.md",
      "section": "4. 비교표 / 5. 빠진 정보 요약",
      "budget_chars": 5000,
      "summary": "학술 온톨로지 행의 다섯 정보 항목을 채우고 VDA 5050 2.x 기준을 2.1.0 태그로 밝혔다. [추정][^ref-355][^ref-350][^ref-349][^ref-235]",
      "planned_findings": [
        "f1",
        "f3",
        "f4",
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "q1-03·q1-06·q1-07 답함(3절 소제목·앵커), q1-02 팩트시트 문장 판별 정리, KnowRob 판 구분 병기, 4·5·6·7·8·9절과 상태 줄 갱신, 후속 질문 q6-05. 2차 수정: 6절 비교표 조건 근거 칸의 '모두 채워졌다'를 미조사 칸이 남은 행을 밝히는 문장으로 고침"
    },
    {
      "path": "docs/tracks/manual-capability-ontology/ontology-draft.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "v0.2 → v0.3: 기능 한정자(요구/제공), 개념 실행 상태 추가, 장착 장비 속성 2건·확정, 다이어그램 갱신, 6절에 요구 목록 초안·거부된 변경 3건 질문(2차 재실행에서 변경 없음)"
    },
    {
      "path": "docs/tracks/manual-capability-ontology/model-standard-comparison.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "학술 온톨로지 행(IEEE 1872 CORA, IEEE 1872.2, KnowRob·SOMA, SSN/SOSA) 다섯 정보 항목 채움, VDA 5050 행에 2.1.0 태그 기준·3.0.0 필드 변화 반영, 빠진 정보 요약 갱신, 상태 줄 갱신(2차 재실행에서 변경 없음)"
    },
    {
      "path": "docs/tracks/manual-capability-ontology/index.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "6. 살아있는 산출물 링크: 온톨로지 초안 v0.3, 비교표 학술 온톨로지 행 채움, 백로그 수치 갱신(2차 재실행에서 변경 없음)"
    },
    {
      "path": "docs/ideas/robot-capability-ontology.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "3절에 능력 매칭 선행 연구, 4절에 충전·적재·장착 장비 표현(실행 2026-09-25-23) 추가. 2차 수정: 4절 ref-245 각주에 원문 미열람 표기"
    }
  ],
  "changelog_entry": "2026-09-25 | 매뉴얼 기반 로봇 기능 온톨로지 단계 1 | q1-03·q1-06·q1-07 답함, 비교표 학술 온톨로지 행 채움, 온톨로지 초안 v0.2 → v0.3, 후속 질문 q6-05 등록·중복 백로그 3건 폐기 | run 2026-09-25-23",
  "index_updates": {
    "home_recent": "2026-09-25 — 매뉴얼 기반 로봇 기능 온톨로지 단계 1: q1-03·q1-06·q1-07 답함, 모델·표준 비교표 학술 온톨로지 행 채움, 능력 온톨로지 초안 v0.3(요구·제공 한정자, 실행 상태, 장착 장비 속성)",
    "category_recent": "2026-09-25 — 매뉴얼 기반 로봇 기능 온톨로지 단계 1(중심 영역 5. 로봇 능력·작업 온톨로지): 학술 온톨로지가 다섯 정보 항목을 담는 정도, VDA 5050 팩트시트 2.x(2.1.0 태그)→3.0.0 필드 변화, ROP용 능력 개념 요구 목록 초안"
  },
  "glossary_updates": [
    {
      "action": "new",
      "slug": "capability-matchmaking",
      "term_ko": "능력 매칭",
      "term_en": "Capability Matchmaking",
      "definition": "제품·작업이 요구하는 특성을 자원(로봇·설비)이 제공하는 능력의 파라미터와 비교해 수행 가능한 자원이나 자원 조합을 찾는 일이다.",
      "description": "제조 분야에서는 제품·자원 온톨로지와 SPIN 규칙으로 결합 자원의 결합 능력을 추론하고 제품 요구와 비교하는 방법이 제시됐다. 물류 이동로봇 대상의 능력 매칭 규칙은 이 위키에서 아직 확인하지 못했다.",
      "related_areas": [
        5,
        13
      ],
    ...
    {
      "action": "new",
      "slug": "required-and-provided-capability",
      "term_ko": "요구 능력·제공 능력",
      "term_en": "Required Capability / Provided (Offered) Capability",
      ...
      "description": "IDTA 02020 능력 기술 1.0 README 는 요구 능력과 제공 능력의 비교가 생산 계획·오케스트레이션을 돕는다고 설명한다. 능력 온톨로지 초안 v0.3 은 기능에 한정자(요구 / 제공) 속성을 두었다.",
      "related_areas": [
        5,
        13,
        28
    ...
    {
      "action": "new",
      "slug": "capability-based-task-allocation",
      "term_ko": "능력 기반 작업 배정",
      "term_en": "Capability-based Task Allocation",
      "definition": "로봇이 선언하거나 관측된 능력·제약과 작업의 요구 조건을 대조해 수행 가능한 로봇에게 작업을 배정하는 방식이다.",
      "related_areas": [
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "IEEE 1872-2015 CORA 의 개념·관계만 분리한 수작업 OWL 번역(제3자). 로봇·로봇 시스템 분류와 robotPart·equippedWith(로봇 환경이 로봇 시스템을 갖춤)·자율성 수준 속성을 둔다. IEEE 표준 본문이 아니다.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/manual-capability-ontology/stage-1-existing-models-and-standards.md",
        "docs/tracks/manual-capability-ontology/model-standard-comparison.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 제품·자원 온톨로지와 SPIN 규칙으로 결합 능력 파라미터를 추론하고 제품 요구와 자원 능력을 비교하는 능력 매칭 의미 규칙(IJCIM 36(1) 128-154, 온라인 2022-06-07).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/manual-capability-ontology/stage-1-existing-models-and-standards.md",
        "docs/tracks/manual-capability-ontology/ontology-draft.md",
  ...
  "standards_updates": [
    {
      ...
      "summary": "RDF·RDFS·OWL 어휘의 맥락화된 트리플로 지식을 표현하고 여러 추론기의 결과를 결합하는 로봇용 하이브리드 지식 베이스. 현재 dev 브랜치는 C++ 구현이며 Prolog 는 선택 지원이다(확인일 2026-09-25).",
      "ref_id": "ref-351"
    },
    {
      "name": "IEEE1872-owl (CORA 공개 OWL 번역, 제3자)",
      "kind": "오픈소스",
      "org": "srfiorini (IEEE1872-owl GitHub)",
      "url": "https://github.com/srfiorini/IEEE1872-owl/blob/master/cora-bare.owl",
      "related_areas": [
      ...
      "summary": "IEEE 1872-2015 CORA 의 개념·관계만 분리한 제3자 수작업 OWL 번역. 로봇·로봇 시스템 분류와 부품·자율성 수준 속성을 둔다. IEEE 표준 본문이 아니다.",
      "ref_id": "ref-355"
    }
  ],
  "additional_research_requests": [
    "단계 1 페이지 3절 q1-03 과 비교표 KnowRob 칸: KnowRob 자체가 전제조건·파라미터 범위·실패를 어떻게 표현하는지는 README 만으로 판정할 수 없다. KnowRob 이 쓰는 온톨로지 파일(또는 SOMA·DUL 연결 문서)을 원문으로 확인할 필요가 있다.",
    "비교표 PDDL 행: 전제조건·효과 외 파라미터 범위·완료 확인·오류 표현은 검색 요약 수준이다. PDDL 원 명세 또는 공개 문서 원문 대조가 필요하다.",
    "비교표 SSN/SOSA·SOMA 행: SOSA 핵심 모듈(Actuation·Procedure)과 SOMA 의 다른 모듈(SOMA-OBJ 등)을 열지 않아 부재 관찰이 확정되지 않았다. 해당 파일 원문 대조가 필요하다.",
    "비교표 OPC UA Robotics·MassRobotics·AAS·Open-RMF·RCO·이종 자율 로봇 능력·스킬 모델 행: 다섯 정보 항목 가운데 미조사 칸이 남아 있다. 해당 명세·논문 원문으로 채울 조사가 필요하다.",
    "단계 1 페이지 q1-07: 2.x 비교가 2.1.0 태그 기준이다. VDA 5050 2.0.0 게시판(PDF)의 팩트시트 필드와 2.1.0 태그 스키마가 필드 단위로 같은지, localizationParameters 블록이 게시판에 있는지 확인이 필요하다.",
    "IEEE 1872-2015·1872.2-2021 표준 원문과 제3자 OWL 구현·번역의 차이는 미확인이다. 공식 요약·공개 초안으로 확인할 수 있는지 조사가 필요하다.",
    "능력 매칭 논문 2건(Järvenpää 외 2023, Köcher 외 2023)은 원문 미열람이다. 원문 열람으로 규칙 형식과 결합 능력 추론 방식을 확인하면 q4-07 답에 쓸 수 있다.",
    "다음 단계 1 실행: q1-08(MassRobotics 식별 보고 필드)은 실행 2026-09-25-16 의 원문 확인 finding 으로 답할 수 있다. q1-09(ECLASS·IEC CDD 범위 능력 항목) 조사가 단계 전환의 막힌 질문으로 남아 있다.",
    "파이프라인 담당 요청(조사 아님): 트랙 단계 페이지·온톨로지 초안·비교표의 상태 줄과 H1 버전 표기는 첫 H2 앞에 있어 patches 로 갱신할 수 없다. 이번 실행은 모든 페이지를 전체 content 로 보냈다. patches 가 H2 앞 머리 영역을 다룰 수 있게 하거나 상태 줄을 자동 영역으로 바꾸는 처리가 필요하다."
  ],
  "fixes_applied": [
    "q1-03·q1-06·q1-07 답함 처리 — 단계 페이지 2절 표 상태를 답함(실행 2026-09-25-23, 앵커)으로 바꾸고, 3절 q1-03 소제목에서 '(부분 답)'을 빼 '### q1-03 … {#q1-03}'으로, q1-06·q1-07 소제목에 {#q1-06}·{#q1-07}을 붙였으며, 4절 남은 불확실성에 KnowRob 판정이 README 범위에 그친다는 점과 PDDL 행이 검색 요약 수준이라는 점을 남겼다.",
    "q1-07 2.x 기준 명시 — q1-07 답 첫 단락에 '2.x'가 공식 저장소 2.1.0 태그 스키마 기준이고 2.0.0 태그에는 팩트시트 스키마가 없으며 명세는 RELEASE CANDIDATE 표기라 2.0.0 게시판과 대조하지 못했음을 적고, f3·f4 를 '확인된 이름 변경'·'확인된 추가 필드'로 쓰고 완결 목록이 아니라고 밝혔다(4절·비교표에도 같은 표기).",
    "q1-02 절 팩트시트 문장 교체 — '[추정][^ref-031]' 블록 구성 문장을 2.1.0 태그 구성([사실][^ref-348])과 3.0.0 main 구성([사실][^ref-228])으로 판을 나눠 바꿔 쓰고, 2.0.0 RC 문서의 localizationParameters 는 [추정]으로 병기했다.",
    "f15·f14 문구 수정 — 단계 페이지 q1-03 학술 온톨로지 단락, 비교표 CORA 행, 아이디어 페이지에서 '부품·장착 관계'를 '부품 관계(robotPart)와 로봇 환경이 로봇 시스템을 갖춘다는 관계(equippedWith)'로 쓰고, equippedWith 를 로봇–장착 장비 관계로 설명하지 않았다.",
    "f16 판 구분 — 단계 페이지 q1-01 절에 'KnowRob 2.0(2018 논문)은 Prolog 구현'(ref-027) 기존 문장을 그대로 두고 '현재 dev 브랜치는 C++ 구현, Prolog 선택 지원'(ref-351, 확인일 2026-09-25) 문장과 판 차이 설명을 더했다. 비교표 KnowRob·SOMA 행 종류 칸에도 두 판을 병기했다.",
    "f20 문구 수정 — 단계 페이지 q1-06 답과 아이디어 페이지 3절에서 '능력 매칭을 위한 의미 규칙(SPIN)을 제시했다'로 쓰고 기준일을 2023(IJCIM 36(1), 온라인 2022-06-07)으로 적었다.",
    "ref-229 각주 유지 — 단계 페이지·온톨로지 초안·비교표의 ref-229 각주는 기존 줄을 그대로 쓰고 reference_updates 에 ref-229 를 넣지 않았다.",
    "원문 미열람 표기 — ref-245·ref-236·ref-352·ref-353 각주 접근일 뒤에 ' (원문 미열람)'을 붙이고(ref-245 는 단계 페이지·온톨로지 초안·비교표에서 추가) reference_updates 에 source_unopened: true 로 두었으며, ref-348·ref-354·ref-235·ref-349·ref-350·ref-355·ref-351·ref-228 에는 붙이지 않았다.",
    "제3자 구현·RC 표시 유지 — ref-350·ref-355 는 본문과 비교표 칸에 '제3자 OWL 구현·번역이며 IEEE 표준 본문이 아님'을, ref-354 는 'RELEASE CANDIDATE 표기 문서'임을 적었다.",
    "온톨로지 초안 승인 변경 3건 반영·v0.3 — (1) 기능 속성에 한정자(요구 / 제공)(f19·f20)를 더하고 작업 요구 메모는 [추정]으로 두었고, (2) 새 개념 실행 상태를 확정으로 넣되 관계는 넣지 않았으며, (3) 장착 장비에 부착 인터페이스(f23)·적재 취급 장치 위치(f22)를 더해 초안 → 확정으로 바꿨다. H1·프런트매터 ontology_version·track_updates.ontology_draft_version 을 0.3 으로 맞추고 4절 다이어그램에 실행 상태 노드와 도식 아래 문장을 고쳤다.",
    "온톨로지 초안 6절 질문 3건 — (a) 로봇 펌웨어 버전 → 구성 버전(f22, q6-02 몫), (b) 제약 종류에 운용 구역·환경 조건·충전 조건(f8·f22, 실행 조건–제약 경계와 충돌), (c) '로봇 / 장착 장비를 갖춘다'와 CORA equippedWith 의 대응(f14, 정의역 RoboticEnvironment·치역 RoboticSystem 불일치)을 6절 질문으로 두었다.",
    "ROP용 능력 개념 요구 목록 수록 — 온톨로지 초안 6절에 '[추정] ROP용 능력 개념 요구 목록 초안(실행 2026-09-25-23, f24)'과 아홉 후보 표를 싣고 항목마다 v0.3 반영(요구·제공 한정자, 실행 상태, 부착 인터페이스·적재 취급 장치 위치)·미반영 — 6절 질문(구성 버전, 운용 구역·환경 조건, 충전 조건)·변경 제안 없음(속성 값·범위, 전제·사후조건, 결합 능력)을 표시했다.",
    "새 질문 처리 — f25 질문은 q4-07 과 중복이라 backlog_updates 에 넣지 않고 단계 페이지 5절 표 아래에 중복 사유를 적었으며, f6 질문은 q6-05(단계 6. 변경 관리·운영·거버넌스 조사, 근거 f6)로 등록했다.",
    "백로그 정리 — backlog_updates 에서 q1-10·q4-11·q5-08 을 폐기로 바꾸고, 단계 페이지 2절 표에 q1-10 을 두지 않고(표 아래에 폐기 사유 기재) 5절 아래에도 폐기 사실을 적었다.",
    "6절·상태 줄 — 비교표 조건은 충족(KnowRob 칸 'README 범위에서 판정 불가 — SOMA 행 참조'), 요구 목록 조건은 미충족(초안은 실었으나 일부만 개념에 반영)으로 적고 전환 줄을 '아니오(요구 목록 일부 미반영, 막힌 질문 q1-08·q1-09)'로 썼으며, 상태 줄을 열린 질문 2건·답한 질문 7건으로 백로그와 맞췄다.",
    "모델·표준 비교표 — IEEE 1872 CORA(f14·f15), IEEE 1872.2(f12·f13), KnowRob·SOMA(f10·f11·f16·f17), W3C SSN/SOSA(f8·f9) 행을 채우고 상태를 '확인(제3자 구현/편집본 원문 열람)'으로 적었으며, 부재 관찰은 태그 바로 뒤 괄호 금지 규칙(부록 R-4)에 따라 '없음(열람 파일 기준, 부재 확정 아님) [추정]' 형식으로 적고, VDA 5050 행에 2.x 기준이 2.1.0 태그임을 밝혔다.",
    "용어집 — '요구 능력·제공 능력' 정의 끝에 '이 위키의 온톨로지 초안에서는 capability 를 기능으로 부르므로 요구·제공 한정자에 해당한다'를 더했다.",
    "2차: 단계 1 페이지 6절 완료 조건 표 첫 행 근거 칸 — '후보 10행과 후보 밖 5행이 모두 채워졌다'를 '후보 10행과 후보 밖 5행 모두에 조사 결과가 있으나, PDDL·OPC UA Robotics·MassRobotics·AAS·Open-RMF 행과 후보 밖 RCO·이종 자율 로봇 능력·스킬 모델 행에는 \"미조사\" 칸이 남아 있다'로 고쳤고, 충족 여부 칸 '충족'과 검증 판정 칸 '미충족(단계 전체) · 미승인'은 그대로 두었다.",
    "2차: track_updates.log_entry — '비교표는 이번 실행으로 모든 후보 행이 채워짐'을 '비교표 학술 온톨로지 행의 다섯 정보 항목이 채워짐(일부 행에 미조사 칸 남음)'으로 고쳤다.",
    "2차: docs/ideas/robot-capability-ontology.md 4절 — 기존 각주 정의 [^ref-245] 끝(접근일 2026-09-25 뒤)에 ' (원문 미열람)'을 붙였다."
  ],
  "track_updates": {
    ...
    "log_entry": "답한 질문: q1-03(학술 온톨로지 행 원문 대조 f8~f18), q1-06(요구·제공 능력 비교와 요구 목록 초안 f19~f25), q1-07(VDA 5050 2.1.0 태그 대 main 팩트시트 필드 대조 f1~f7) / 새 질문: q6-05(f6, 단계 6. 변경 관리·운영·거버넌스 조사). f25 질문은 q4-07 과 중복이라 등록하지 않음. 중복 등록된 q1-10·q4-11·q5-08 폐기 / 온톨로지 변경: v0.2 → v0.3: 기능 속성 '한정자(요구 / 제공)' 추가(f19·f20), 개념 '실행 상태' 추가(f10), 장착 장비 속성 '부착 인터페이스'(f23)·'적재 취급 장치 위치'(f22) 추가와 장착 장비 초안 → 확정. 거부(6절 질문): 로봇 구성 버전(f22), 제약 종류 값 추가(f8·f22), CORA equippedWith 대응 메모(f14). ROP용 능력 개념 요구 목록 초안(f24) 6절 수록 / 완료 조건 평가: 미충족(부족: ROP용 능력 개념 요구 목록 초안의 일부만 온톨로지 초안에 반영. 비교표 학술 온톨로지 행의 다섯 정보 항목이 채워짐(일부 행에 미조사 칸 남음)) / 세부영역 반영 제안: 5. 로봇 능력·작업 온톨로지 3건(4·7·8절), 9. 로봇·제조사 관제 연동 1건(7절), 16. 공용 자원·충전·에너지 최적화 1건(7절) / 다음 실행 제안: q1-08(실행 2026-09-25-16 의 MassRobotics 식별 보고 확인 결과로 답 가능), q1-09",
    "overview_progress": "단계 1 진행 중 — 열린 질문 2, 답함 7, 완료 조건 미충족(ROP용 능력 개념 요구 목록 초안 일부만 온톨로지 초안 v0.3 에 반영, 막힌 질문 q1-08·q1-09)"
  },
  "area_reflection_proposals": [
    {
      "area_no": 5,
      "section": "4. 핵심 개념과 용어",
      "summary": "요구 능력·제공 능력의 구분과 능력 매칭(IDTA 02020 README 의 요구·제공 능력 비교, Järvenpää 외 2023 의 SPIN 능력 매칭 규칙). 이 위키 온톨로지 초안에서는 기능의 한정자(요구 / 제공)로 표현한다. 분류 원문 SCM 질문과 요구–제공 속성 비교 규칙의 연결은 [추정]으로 둔다(실행 2026-09-25-23, f19·f20·f25)."
    },
    {
      "area_no": 5,
      "section": "7. 관련 표준·프레임워크·오픈소스",
      "summary": "학술 온톨로지가 다섯 정보 항목을 담는 정도: SOMA-ACT 는 전제·완료·오류(실행 상태 6종, 요구 초기·기대 종료 상황), SSN System Capabilities 는 범위·환경 조건을 일부 담고, CORA 제3자 OWL 번역·IEEE 1872.2 제3자 구현은 담지 않으며, 적재 제약은 VDA 5050 적재 세트 같은 산업 규격에만 있다(열람 파일 기준, 실행 2026-09-25-23, f8~f15·f18)."
    },
    {
      "area_no": 5,
      "section": "8. 대표 연구와 자료",
      "summary": "능력 매칭 연구: Järvenpää 외(IJCIM 36(1), 2023)의 결합 능력 추론·능력 매칭 SPIN 규칙, Köcher·Vieira da Silva·Fay(arXiv 2312.08801)의 의미 능력 모델+SMT 공정 계획. 제조 공정 연구이므로 방법 선례로만 소개(원문 미열람, 실행 2026-09-25-23, f20·f21)."
    },
    {
      "area_no": 9,
      "section": "7. 관련 표준·프레임워크·오픈소스",
    ...
    },
    {
      "area_no": 16,
      "section": "7. 관련 표준·프레임워크·오픈소스",
      "summary": "VDA 5050 3.0.0(main) 팩트시트 구성 블록의 충전 설정 batteryCharging(임계 저충전 수준·희망 최소·최대 충전 수준·최소 충전 시간)이 로봇 선언의 일부로 추가돼 충전 시점 계획의 입력이 될 수 있다(실행 2026-09-25-23, f4·f22)."
    }
  ]
}
```

### runs/2026-09-25-23/format_check.md
...
### runs/2026-09-25-23/pages/tracks/manual-capability-ontology/stage-1-existing-models-and-standards.md
...
created: 2026-09-24
updated: 2026-09-25
sources: [ref-022, ref-025, ref-026, ref-027, ref-028, ref-029, ref-030, ref-031, ref-032, ref-033, ref-034, ref-035, ref-036, ref-037, ref-038, ref-039, ref-040, ref-041, ref-042, ref-043, ref-051, ref-228, ref-229, ref-243, ref-231, ref-230, ref-244, ref-245, ref-246, ref-236, ref-247, ref-248, ref-138, ref-249, ref-250, ref-348, ref-354, ref-235, ref-349, ref-350, ref-355, ref-351, ref-352, ref-353]
last_run: 2026-09-25
version: 4
---
...
독일자동차산업협회(Verband der Automobilindustrie, VDA)의 VDA 5050 2.0.0(2022년 1월판) 기준으로, 차량은 팩트시트(factsheet) 토픽으로 자신의 기능(차량 유형, 구동 방식 등)을 상위 관제(master control)에 미리 알린다. [추정][^ref-022] 팩트시트의 블록 구성은 판마다 나눠 적는다. 공식 저장소 2.1.0 태그의 팩트시트 스키마(제목 'AGV Factsheet')는 version·manufacturer·serialNumber 와 유형 명세(typeSpecification), 물리 파라미터(physicalParameters), 프로토콜 한계(protocolLimits), 지원 기능(protocolFeatures), 차량 기하(agvGeometry), 적재 명세(loadSpecification) 블록을 필수로 두고, 버전·네트워크 정보를 담는 vehicleConfig 블록을 둔다(확인일 2026-09-25). [사실][^ref-348] 공식 저장소 main(3.0.0 판)의 팩트시트 스키마는 headerId·timestamp·version·manufacturer·serialNumber 와 typeSpecification·physicalParameters·protocolLimits·protocolFeatures·mobileRobotGeometry·loadSpecification 블록을 필수로, mobileRobotConfiguration 블록을 선택으로 둔다(확인일 2026-09-25). [사실][^ref-228] 2.0.0 태그의 명세 마크다운(머리말 RELEASE CANDIDATE 표기)은 팩트시트 블록으로 localizationParameters 를 적지만 2.1.0 태그와 main 스키마에서는 이 블록이 확인되지 않아, 2.0.0 게시판의 블록 구성은 확정할 수 없는 것으로 보인다. [추정][^ref-354][^ref-348][^ref-228] 실행 2026-09-25-16에서는 공식 저장소 main 브랜치의 팩트시트 스키마를 원문으로 열어 적재 명세와 지원 action 정의 블록을 확인했다(내용은 q1-03 답). [사실][^ref-228] 2.0.0 기준 상태 메시지는 오류를 유형(errorType)·등급(errorLevel: WARNING 또는 FATAL)·설명·참조로 보고하고, action 완료는 actionStatus 가 finished 로 바뀐 상태 메시지로 알린다. [사실][^ref-022]
...
분류 원문 5. 로봇 능력·작업 온톨로지의 정의 요소(제조사별 기능·제약·장착 장비·실행 조건, 작업 요구)를 기준으로 보면, 기능을 요구 쪽과 제공 쪽으로 나눠 속성 단위로 비교하는 표현이 먼저 필요한 것으로 보인다. [추정][^ref-229][^ref-352][^ref-228] 아래는 근거와 요구 목록 초안이다.

**요구 능력과 제공 능력의 비교.** IDTA 02020 능력 기술 1.0 README 는 이 서브모델이 공정·제품 쪽 요구 능력과 자원 쪽 제공 능력을 비교해 생산 계획·오케스트레이션을 돕는다고 설명한다. [사실][^ref-229] Järvenpää 외(IJCIM 36(1), 2023, 온라인 2022-06-07)는 제품과 자원의 온톨로지 기술과 SPIN 규칙으로 결합 자원의 결합 능력 파라미터를 추론하고 제품 특성을 자원(조합)의 능력 파라미터와 비교해 가능한 자원 조합을 찾는, 능력 매칭을 위한 의미 규칙(SPIN)을 제시했다. [사실][^ref-352] Köcher·Vieira da Silva·Fay(arXiv 2312.08801, 2023-12, AAAI 2024 CAIPI 워크숍)는 의미 능력 모델과 SMT 로 제품 생산이나 자율 로봇 임무 수행에 필요한 개별 능력의 순서를 자동으로 찾는 공정 계획 방법을 제안했다. [사실][^ref-353] 두 논문은 제조 공정 계획 연구이므로 물류 현장 적용 사례가 아니라 방법의 선례로만 쓴다.
...
**블록 구성.** 2.1.0 태그 스키마는 version·manufacturer·serialNumber 와 typeSpecification·physicalParameters·protocolLimits·protocolFeatures·agvGeometry·loadSpecification 블록을 필수로 두고 vehicleConfig 블록을 둔다. [사실][^ref-348] main(3.0.0) 스키마는 headerId·timestamp·version·manufacturer·serialNumber 와 typeSpecification·physicalParameters·protocolLimits·protocolFeatures·mobileRobotGeometry·loadSpecification 블록을 필수로, mobileRobotConfiguration 블록을 선택으로 둔다. [사실][^ref-228] 2.0.0 태그 명세가 적는 localizationParameters 블록은 2.1.0 태그와 main 스키마에서 확인되지 않아, 2.0.0 게시판의 블록 구성은 확정할 수 없는 것으로 보인다. [추정][^ref-354][^ref-348][^ref-228]

**확인된 이름 변경.** 두 스키마를 대조하면 agvGeometry→mobileRobotGeometry, vehicleConfig→mobileRobotConfiguration, agvActions→mobileRobotActions, resultDescription→actionResult, agvKinematic·agvClass·maxLoadMass→mobileRobotKinematics·mobileRobotClass·maximumLoadMass, speedMin·speedMax·accelerationMax·decelerationMax·heightMin·heightMax→minimumSpeed·maximumSpeed·maximumAcceleration·maximumDeceleration·minimumHeight·maximumHeight 로 이름이 바뀌었다. [사실][^ref-348][^ref-228] 이 목록은 확인된 변화이며 완결 목록이 아니다.

**확인된 추가 필드.** main(3.0.0) 스키마는 2.1.0 태그 스키마에 없던 action 적용 범위 ZONE, action 필수 필드 pauseAllowed·cancelAllowed, 유형 명세의 supportedZones, 구성 블록의 batteryCharging(임계 저충전 수준·희망 최소·최대 충전 수준·최소 충전 시간)을 더했고, 지원·필수 선택 파라미터 목록(optionalParameters: parameter·support SUPPORTED/REQUIRED·description)은 두 판 모두 둔다. [사실][^ref-348][^ref-228]

**적재 제약 필드.** 적재 제약은 두 판 모두 적재 명세(loadSpecification)의 적재 세트(loadSets)에 기술되며, 2.1.0 태그는 maxWeight, min/maxLoadhandlingHeight·Depth·Tilt, agvSpeedLimit·agvAccelerationLimit·agvDecelerationLimit, pickTime·dropTime 을, main(3.0.0)은 maximumWeight, minimum/maximumLoadhandlingHeight·Depth·Tilt, maximumSpeed·maximumAcceleration·maximumDeceleration, pickTime·dropTime 을 두고, 로봇 전체 최대 적재 질량은 유형 명세(maxLoadMass→maximumLoadMass)에 둔다. [사실][^ref-348][^ref-228] 2.1.0 태그 기준의 2.x 와 3.0.0 의 적재 제약은 적재 세트 단위(치수·최대 중량·취급 높이·깊이·기울기·적재 시 속도 한계·적재·하역 시간)라는 구조가 같고 필드 이름만 바뀐 것으로 보여, ROP 가 두 판을 함께 받으려면 필드 이름 대응표로 판 무관 속성에 정규화할 수 있을 것으로 보인다. [추정][^ref-348][^ref-228] 판이 바뀔 때의 정규화와 재검증 절차는 후속 질문 q6-05 로 보냈다.
...
- 다섯 정보 항목은 여러 모델에 흩어져 담기며 한 모델이 모두 담는 경우는 확인되지 않았다. 원문을 연 학술 온톨로지 기준으로 전제조건·완료 확인·오류 의미는 SOMA 가, 파라미터 범위·환경 제약은 SSN System Capabilities 가 일부 담고, CORA·IEEE 1872.2 구현은 다섯 항목을 담지 않으며, 적재 제약은 산업 규격에만 있어 ROP 는 적재 제약과 오류의 조치 의미를 산업 규격 쪽에서 가져와야 할 것으로 보인다(q1-03). [추정][^ref-349][^ref-235][^ref-355][^ref-350][^ref-228]
...
- ROP용 능력 개념 요구 목록 초안은 아홉 후보로 정리되며, 그 첫 항목인 요구·제공 능력의 속성 단위 비교는 제조 분야 능력 매칭 연구가 의미 규칙으로 구현한 선례가 있다(q1-06). [추정][^ref-229][^ref-352][^ref-228]
- VDA 5050 팩트시트는 2.1.0 태그 기준의 2.x 와 3.0.0 사이에 필드 이름이 바뀌고 ZONE 범위·pauseAllowed·cancelAllowed·supportedZones·batteryCharging 이 더해졌으며, 적재 제약은 두 판 모두 적재 세트에 기술된다(q1-07). [사실][^ref-348][^ref-228]
...
- ref-031·ref-034·ref-039·ref-040·ref-228~ref-245·ref-138·ref-348~ref-351·ref-354·ref-355의 발행일과 ref-236의 저자는 미확인이다.
...
[^ref-348]: VDA / VDMA (VDA5050 GitHub), VDA5050/VDA5050 (tag 2.1.0) — json_schemas/factsheet.schema, 미확인, https://github.com/VDA5050/VDA5050/blob/2.1.0/json_schemas/factsheet.schema, 접근일 2026-09-25
[^ref-354]: VDA / VDMA (VDA5050 GitHub), VDA5050/VDA5050 (tag 2.0.0) — VDA5050_EN_V1.md, 미확인, https://github.com/VDA5050/VDA5050/blob/2.0.0/VDA5050_EN_V1.md, 접근일 2026-09-25
...
[^ref-349]: EASE CRC (ease-crc/soma), SOMA — owl/SOMA-ACT.owl, 미확인, https://github.com/ease-crc/soma/blob/master/owl/SOMA-ACT.owl, 접근일 2026-09-25
[^ref-350]: Helmut Schmidt University, Institute of Automation Technology (hsu-aut GitHub), IndustrialStandard-ODP-IEEE1872-2 — AuR_IEEE1872-2.ttl, 미확인, https://github.com/hsu-aut/IndustrialStandard-ODP-IEEE1872-2/blob/main/AuR_IEEE1872-2.ttl, 접근일 2026-09-25
[^ref-355]: srfiorini (IEEE1872-owl GitHub), IEEE1872-owl — cora-bare.owl (OWL specification of CORA), 미확인, https://github.com/srfiorini/IEEE1872-owl/blob/master/cora-bare.owl, 접근일 2026-09-25
[^ref-351]: KnowRob (knowrob GitHub), knowrob — README (dev branch), 미확인, https://github.com/knowrob/knowrob, 접근일 2026-09-25
[^ref-352]: Järvenpää, E., Siltala, N., Hylli, O., Nylund, H., & Lanz, M., Semantic rules for capability matchmaking in the context of manufacturing system design and reconfiguration, 2023, https://www.tandfonline.com/doi/full/10.1080/0951192X.2022.2081361, 접근일 2026-09-25 (원문 미열람)
[^ref-353]: Köcher, A., Vieira da Silva, L. M., & Fay, A., Automated Process Planning Based on a Semantic Capability Model and SMT, 2023-12, https://arxiv.org/abs/2312.08801, 접근일 2026-09-25 (원문 미열람)
...
created: 2026-09-24
updated: 2026-09-25
sources: [ref-022, ref-025, ref-026, ref-027, ref-028, ref-029, ref-030, ref-031, ref-032, ref-033, ref-034, ref-035, ref-036, ref-037, ref-038, ref-039, ref-040, ref-041, ref-042, ref-043, ref-051, ref-228, ref-229, ref-243, ref-231, ref-230, ref-244, ref-245, ref-246, ref-236, ref-247, ref-248, ref-138, ref-249, ref-250]
last_run: 2026-09-25
version: 3
---
...
표의 질문 문장은 트랙 정의(q1-01~q1-06)와 리서치 브리프(q1-07·q1-08·q1-09) 그대로 두었다. 다음은 구축자 보충이다. q1-06의 "부록 A 5번 정의"에서 5번은 [5. 로봇 능력·작업 온톨로지](../../categories/b-common-information-and-environment-model/05-robot-capability-and-task-ontology.md)를 가리킨다. "부록 A"는 이 위키의 분류 원문(`_source/ROP_SCM_연구분야_분류.md`)을 뜻한다. q1-02의 AMR은 자율이동로봇(Autonomous Mobile Robot, AMR)이다. q1-03은 실행 2026-09-25-16에서도 부분 답에 머물러 백로그에서 "조사 중"이며 이 표에서는 "열림"이다.

## 3. 조사 결과

실행 2026-09-25-02는 페이지 열람이 차단된 환경에서 이루어져, 그 실행의 출처는 검색 결과의 기관·제목·URL 일치로만 실재를 확인했다. 실행 2026-09-25-16은 일반 웹 열람이 막힌 대신 공식 GitHub 저장소 원문을 열 수 있어, VDA 5050 명세·상태 스키마·팩트시트 스키마, IDTA 02020·02047 템플릿과 README, MassRobotics 스키마, OPC UA Robotics 노드셋 문서, CaSkMan·SkiROS2 README, Open-RMF 튜토리얼 원본을 원문으로 대조했다. 논문·ISO·KS·IDTA PDF 자료는 원문 미열람이며, 두 실행 모두 핵심 주장을 독립 출처로 교차 확인한 것은 없다. 조사 결과를 모은 표는 [모델·표준 비교표](model-standard-comparison.md)에, 반영된 개념 변경은 [능력 온톨로지 초안](ontology-draft.md) v0.2에 있다.
...
독일자동차산업협회(Verband der Automobilindustrie, VDA)의 VDA 5050 2.0.0(2022년 1월판) 기준으로, 차량은 팩트시트(factsheet) 토픽으로 자신의 기능(차량 유형, 구동 방식 등)을 상위 관제(master control)에 미리 알린다. [추정][^ref-022] 팩트시트는 유형 명세(typeSpecification), 물리 파라미터(physicalParameters), 프로토콜 한계(protocolLimits), 지원 기능(protocolFeatures, 지원 action 목록과 action 범위·결과 설명 포함), 차량 기하(agvGeometry), 적재 명세(loadSpecification) 블록으로 구성되는 것으로 보인다(판 미확인 — GitHub main 브랜치, 구현 라이브러리 문서 혼재). [추정][^ref-031] 실행 2026-09-25-16에서는 공식 저장소 main 브랜치의 팩트시트 스키마를 원문으로 열어 적재 명세와 지원 action 정의 블록을 확인했다(내용은 q1-03 답). [사실][^ref-228] 2.0.0 기준 상태 메시지는 오류를 유형(errorType)·등급(errorLevel: WARNING 또는 FATAL)·설명·참조로 보고하고, action 완료는 actionStatus 가 finished 로 바뀐 상태 메시지로 알린다. [사실][^ref-022]

VDA 5050은 3.0.0판이 2026년에 발행되어(3.0.0 발행 2026-03, 보도자료 2026-04) 자율도 높은 이동로봇 통합을 위해 인터페이스를 확장했다. [사실][^ref-032] 확장 내용으로 구역(zone) 개념, 경로 공유, 새 오류 등급 CRITICAL·URGENT, 절전 모드 action 추가와 기존 궤적·회랑 방식 유지가 거론되지만, 이 목록은 검색 요약 기준이며 원문 미열람이다. [추정][^ref-032] 이 가운데 오류 등급은 공식 저장소 main 브랜치의 상태 스키마 원문으로 확인했다: 3.0.0의 오류 등급은 WARNING·URGENT·CRITICAL·FATAL 넷이다(확인일 2026-09-25). [사실][^ref-051] 오류 등급과 q1-03 답에 적은 팩트시트·pick·drop 확인 범위를 빼면, 2.0.0 기준 서술이 3.0.0에서 어떻게 바뀌었는지는 미확인이다(후속 질문 q1-07).
...
원문을 연 산업 규격·서브모델·오픈소스 기준으로 다섯 정보 항목은 여러 모델에 흩어져 담기며, 다섯을 한 모델이 모두 담는 경우는 확인되지 않았다. [추정][^ref-228][^ref-243][^ref-051] 실행 2026-09-25-16은 원문을 연 자료만 항목별로 대조했고, IEEE 1872 계열·KnowRob·SOMA·SSN/SOSA는 항목별 원문 대조를 하지 못해 이 질문은 부분 답으로 남는다.
...
**다른 후보.** OPC UA for Robotics 공식 노드셋 문서는 모션 장치 시스템·컨트롤러·모션 장치·축·동력 전달계·부하·안전 상태(비상정지·보호정지) 유형과, 프로그램을 이름·노드로 적재하고 시작·정지하는 작업 제어와 운영 상태 기계를 정의하며, 목록에서 능력·스킬 유형은 확인되지 않았다(노드셋 판 표기 v100, 명세 본문 아님). [사실][^ref-244] 이 규격의 모션 장치·축·안전 정지 유형은 로봇 제어 쪽이므로 능력 기술이 있는지를 판단하는 데만 썼다. Open-RMF 문서는 플릿이 수행할 수 있는 사용자 정의 동작을 설정 파일의 actions 목록으로 선언하고, 작업 요청의 category(동작 이름)와 description(동작별 내용)을 어댑터의 execute_action 이 받아 처리한 뒤 execution.finished() 로 완료를 알리게 하며, 파라미터 스키마·전제조건·구조화된 실패 보고는 설명하지 않는다(확인일 2026-09-25). [사실][^ref-040] 학술 모델 쪽은 이전 실행의 검색 요약 기준 추정이 그대로다: 전제조건·효과는 PDDL이, 파라미터와 제약은 SSN 운용 범위가 일부 담는 것으로 보인다. [추정][^ref-029][^ref-030] 오류의 의미는 SOMA의 실행 실패 하위 온톨로지가 일부 담는 것으로 보인다. [추정][^ref-028]

**종합.** 원문을 연 모델 기준으로 전제조건은 IDTA 02020 속성 제약·SkiROS2 스킬 조건이, 파라미터 범위는 IDTA 02020 Range 속성이(VDA 5050 팩트시트는 데이터형만), 적재·환경 제약은 VDA 5050 적재 세트·IDTA 02047·MassRobotics 화물 최대값이, 완료 확인은 VDA 5050 pick·drop 완료 정의·IDTA 02020 사후조건·SkiROS2 사후조건이, 오류 의미는 VDA 5050 오류 등급·힌트가 담으며, 다섯을 한 모델이 모두 담는 경우는 확인되지 않았다. [추정][^ref-228][^ref-031][^ref-051][^ref-243][^ref-245][^ref-230][^ref-250] IDTA 02047 이 속도·가동 시간을 명세값과 운용값으로 나눠 두는 것은 RCO가 구분한 광고 능력과 운용 능력에 대응하는 표현으로 보인다(IDTA 문서가 두 값의 정의를 어떻게 적는지는 미확인). [추정][^ref-245][^ref-041]
...
- 산업 규격 쪽 기능 기술은 VDA 5050 팩트시트(2.0.0 기준), Open-RMF 작업 유형·사용자 정의 동작 선언처럼 관제 연동 메시지·설정 안에 들어 있다. [추정][^ref-022][^ref-039][^ref-040] MassRobotics AMR 상호운용 표준 1.0은 setup·status 메시지로 제조사·모델, 위치·속도·방향, 상태, 작업·가용 상태를 공유하게 한다. [사실][^ref-033]
- VDA 5050 3.0.0(공식 저장소 main)의 오류 등급은 WARNING·URGENT·CRITICAL·FATAL 넷이고, 사전 정의 action 은 29종이며 dock·lift 라는 이름은 없다. [사실][^ref-051][^ref-031]
- 원문을 연 모델 기준으로 다섯 정보 항목은 여러 모델에 흩어져 담기며, 다섯을 한 모델이 모두 담는 경우는 확인되지 않았다. 완료 확인 방법은 VDA 5050 pick·drop 완료 정의와 사후조건 같은 항목으로 담기는 것으로 보인다(q1-03 부분 답). [추정][^ref-228][^ref-031][^ref-243][^ref-250]
- 능력 기술과 실행 인터페이스의 연결은 같은 프로토콜 안의 이름 맞물림, 능력–스킬–스킬 인터페이스 모델, 연결 없음의 세 방식으로 나뉘는 것으로 보이며, ROP는 둘을 잇는 매핑 계층이 따로 필요할 것으로 보인다(q1-04, 이 위키의 분류). [추정][^ref-228][^ref-036][^ref-230]
- 같은 이름 기능의 의미 차이는 공통 어휘, 사전 정의 동작, 속성 단위 외부 사전 식별자, 분류·일반화 관계로 다뤄지는 것으로 보이나, 동작 의미의 동일성을 판정하는 방법은 확인되지 않았다(q1-05, 이 위키의 분류). [추정][^ref-025][^ref-031][^ref-247][^ref-243]
- [능력 온톨로지 초안](ontology-draft.md)은 v0.1 → v0.2로 올랐다: 개념 오류 수정(3.0.0 등급·해결 힌트·재시도 가능 여부), 개념 스킬 인터페이스, 관계 "스킬 / 노출된다 / 스킬 인터페이스"·"기능 / 일반화된다 / 기능"이 반영됐다. 제약의 적용 시점 구분과 기능의 의미 식별자 속성 제안은 근거가 부족해 반영하지 않고 초안 6절 질문으로 두었다. v0.1(실행 2026-09-25-02)의 변경은 효과·스킬·오류 개념, "기능 / 구현된다 / 스킬" 관계, 기능의 능력 출처 구분 속성이다.

**남은 불확실성**

- 실행 2026-09-25-02의 출처는 모두 원문 미열람이다. 실행 2026-09-25-16은 공식 GitHub 저장소 원문 11건을 열었으나 논문·ISO·KS·IDTA PDF 자료는 원문 미열람이며, 두 실행 모두 교차 확인은 0건이다. 표준·모델마다 발행 주체 한 곳의 자료에만 기댄다.
- q1-03 부분 답: IEEE 1872 계열·KnowRob·SOMA·SSN/SOSA의 다섯 정보 항목별 충족 정도는 원문으로 대조하지 못했다.
- IDTA 02020 요소 이름이 충돌한다: 공식 1.0 템플릿은 ConstraintSet·CapabilityRealizedBy 를 두지만, 이전 실행이 인용한 제3자 논문은 ConditionContainer·realizedBy 로 적었다. 제3자 논문 표기는 1.0 템플릿에서 확인되지 않으며, 판 차이 여부는 미확인이다.
- VDA 5050 팩트시트는 main 브랜치 스키마로 적재 명세와 지원 action 정의를 확인했으나, 2.0.0 대비 필드 변화는 미확인이다(q1-07). 3.0.0의 오류 등급과 사전 정의 action 목록 외의 확장(구역·경로 공유 등)은 원문 대조 전이다.
- VDA 5050 3.0.0의 정확한 발행일이 미확인이다. 검색 요약은 3.0 발행을 2026-03-19, 보도자료를 2026-04-20로 전하지만 보도자료 URL은 260421 계열이다.
- MassRobotics 공식 JSON 스키마의 식별·상태 보고 필드는 원문으로 확인했으나 q1-08의 답 처리는 다음 실행으로 미뤘고, 표준 2.0의 현재 상태는 미확인이다.
- f3·f17·f34 의 부재 관찰(팩트시트의 범위·전제조건 블록, 관제의 사전 검증 의무, Open-RMF 공통 어휘 장치)은 열람 범위 기준이며 부재의 확정이 아니다.
- CSS 계열 스킬의 FeasibilityCheck·PreconditionCheck 설명은 출처 미확정이다.
- OPC UA Robotics는 노드셋 문서(판 표기 v100)만 열었고 명세 본문과 Part 2 이후 부의 범위는 미확인이다.
- KS B 7321-2 와 ISO 22166-202 의 부합화 여부·제정일은 미확인이다([열린 질문](../../open-questions.md)의 oq-004 와 연결된 새 질문으로 올렸다).
- 국내 논문(신민종·한영석·정재윤, 2024)은 게재 사실만 확인했고 본문 내용은 미확인이다.
- ref-031·ref-034·ref-039·ref-040·ref-228~ref-245·ref-138의 발행일과 ref-236의 저자는 미확인이다.
...
같은 질문은 [질문 백로그](question-backlog.md)에 등록된다(백로그 반영은 퍼블리셔가 한다). 실행 2026-09-25-16에서 함께 제안된 "VDA 5050 팩트시트의 action 파라미터 범위를 IDTA 02020 범위 속성으로 보완할 수 있는가" 질문은 q4-06 과 중복이라 등록하지 않았다.
...
| 완료 조건 | 충족 여부 | 근거 | 검증 판정 |
|---|---|---|---|
| 모델·표준 비교표([model-standard-comparison.md](model-standard-comparison.md)) 작성 | 미충족 | [모델·표준 비교표](model-standard-comparison.md)의 산업 규격·서브모델·오픈소스 행(VDA 5050, MassRobotics, OPC UA Robotics, AAS, Open-RMF)과 후보 밖 행(IDTA 02047, SkiROS2, CaSkMan)은 원문 근거로 보강했으나, IEEE 1872 계열·KnowRob·SOMA·SSN/SOSA 행의 다섯 정보 항목은 미조사다 | 미충족 · 미승인 |
| ROP용 능력 개념 요구 목록 초안이 온톨로지 초안에 반영됨 | 미충족 | [능력 온톨로지 초안](ontology-draft.md) v0.2에 오류 수정·스킬 인터페이스 개념과 관계 2건이 반영됐으나, q1-06을 조사하지 않아 요구 목록 초안은 없다 | 미충족 · 미승인 |

다음 단계로 전환: 아니오(비교표 학술 온톨로지 행의 다섯 정보 항목 미조사, ROP용 능력 개념 요구 목록 초안 미반영, 막힌 질문 q1-03·q1-06·q1-07·q1-08)
...
- [27. AI·학습·적응과 모델 운영](../../categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md) — 매뉴얼 해석은 이 영역의 방법이 5. 로봇 능력·작업 온톨로지와 21. 온보딩·설정·현장 시운전에 적용되는 것으로 다룬다. 단계 1에서는 기존 모델·표준이 AI가 해석한 기능 정보(신뢰도, 근거 위치)를 담을 자리를 갖는지의 관점으로 연결한다. 실행 2026-09-25-02와 2026-09-25-16에서는 이 영역에 해당하는 발견 사항이 없어 반영 제안을 내지 않았다.

**이 단계의 질문이 언급하는 영역(트랙 개요의 배정에 따른 추가 연결, [가정])**

- [9. 로봇·제조사 관제 연동](../../categories/c-connectivity-and-execution-foundation/09-robot-and-vendor-fleet-manager-integration.md) — q1-04(능력 기술과 실행 인터페이스의 연결)와 q1-02의 Open-RMF Fleet Adapter 후보가 이 영역의 어댑터 문제에 닿는다. 실행 2026-09-25-02에서 "7. 관련 표준·프레임워크·오픈소스" 절에 VDA 5050 팩트시트·오류 보고, MassRobotics 메시지, Open-RMF 작업 능력·사용자 정의 동작의 반영을 제안했다. 실행 2026-09-25-16에서 "6. 대표 접근법과 기술"·"7. 관련 표준·프레임워크·오픈소스" 절에 어댑터가 능력 선언을 명령으로 옮기는 방식, VDA 5050 3.0.0 오류 등급·action 상태, MassRobotics 보고 전용 구조의 반영을 제안했다.
- [28. 표준·상호운용성·다사업자 거버넌스](../../categories/g-safety-security-intelligence-and-governance/28-standards-interoperability-and-multi-vendor-governance.md) — q1-02의 산업 상호운용 규격 조사가 이 영역의 공통 규격·적합성 시험에 닿는다. 실행 2026-09-25-02에서 "7. 관련 표준·프레임워크·오픈소스" 절에 능력 기술 관련 표준·규격의 발행 기관과 현재 판의 반영을 제안했다. 실행 2026-09-25-16에서 같은 절에 공통 어휘(IEEE 1872), 의미 식별자(AAS IEC 61360·ECLASS), 서비스 로봇 모듈 정보 모델(ISO 22166-202, KS B 7321-2)의 반영을 제안했다.
...
### runs/2026-09-25-23/pages/tracks/manual-capability-ontology/ontology-draft.md
...
created: 2026-09-24
updated: 2026-09-25
sources: [ref-022, ref-028, ref-029, ref-035, ref-036, ref-037, ref-041, ref-051, ref-229, ref-243, ref-231, ref-245, ref-246, ref-247, ref-250, ref-352, ref-349, ref-355, ref-228, ref-235]
last_run: 2026-09-25
version: 4
---
...
> 온톨로지 버전: v0.2 · 페이지 상태: published · 신뢰도: medium · 페이지 버전: 3 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
...
| 기능(Capability) | 로봇이 수행할 수 있는 동작 또는 작업의 단위. 실행 조건을 요구하고 제약을 받으며, 작업 요구와 대응된다. [분류원문] 기반 [가정] | 이름, 제조사별 명칭, 파라미터(단계 1·4에서 확정), 단위 크기(단계 4 질문), 능력 출처 구분(광고 능력 / 운용 능력), 한정자(요구 / 제공) | 분류 원문 5. 로봇 능력·작업 온톨로지 정의. 속성 "능력 출처 구분"은 finding f8 (실행 2026-09-25-02)[^ref-041]. 매뉴얼 등 근거 문서에서 온 값은 제조사가 명시한 광고 능력에 해당한다는 해석은 [추정] (finding f27, 실행 2026-09-25-02)[^ref-041]. 속성 "한정자(요구 / 제공)"는 finding f19·f20 (실행 2026-09-25-23)[^ref-229][^ref-352]. 작업 요구를 요구 쪽 기능의 속성 값으로 표현해 제공 쪽 속성과 비교한다는 메모는 [추정] (finding f19·f20, 실행 2026-09-25-23)[^ref-229][^ref-352] | 확정 |
...
| 실행 상태(Execution State) | 기능·스킬 실행의 진행 단계(대기·진행·일시정지·취소·실패·성공 등)로, 완료 확인 방법의 판정 대상이다. SOMA 실행 상태 영역의 여섯 상태(Pending·Active·Paused·Cancelled·Failed·Succeeded)에 대응한다. | 상태 값 | finding f10 (실행 2026-09-25-23)[^ref-349] | 확정 |
...
[추정] ROP용 능력 개념 요구 목록 초안(실행 2026-09-25-23, f24). 분류 원문 5. 로봇 능력·작업 온톨로지 정의 요소(기능·제약·장착 장비·실행 조건·작업 요구)를 기준으로 이 온톨로지에 더할 요구 후보는 아래 아홉 가지인 것으로 보인다. [추정][^ref-229][^ref-352][^ref-349][^ref-235][^ref-228][^ref-245][^ref-355] 이 목록은 이 위키가 여러 출처를 원문 정의 요소에 대응시킨 종합이며, 이 목록을 제시한 단일 출처는 없다. 근거 설명은 [단계 1의 q1-06 답](stage-1-existing-models-and-standards.md#q1-06)에 있다.
...
- "로봇 / 장착 장비를 갖춘다" 관계를 CORA 의 equippedWith 속성과 대응시킬 수 있는가. CORA 공개 OWL 번역(제3자)에서 equippedWith 는 정의역이 로봇 환경(RoboticEnvironment), 치역이 로봇 시스템(RoboticSystem)인 관계라 로봇–장착 장비 관계와 맞지 않는다(f14, 실행 2026-09-25-23)[^ref-355]. 장착 장비에 대응하는 표준 관계는 아직 확인되지 않았다. — 관련: q1-06(단계 1. 기존 능력 표현 모델과 표준 조사) [가정]
- 실행 상태를 기능·스킬·스킬 인터페이스·오류 가운데 무엇과 어떤 관계로 이을지 정해지지 않았다. v0.3에서는 SOMA 실행 상태 영역에 대응하는 개념만 더했다[^ref-349]. — 관련: q4-09(단계 4) [가정]
...
created: 2026-09-24
updated: 2026-09-25
sources: [ref-022, ref-028, ref-029, ref-035, ref-036, ref-037, ref-041, ref-051, ref-229, ref-243, ref-231, ref-245, ref-246, ref-247, ref-250]
last_run: 2026-09-25
version: 3
---
...
v0.2는 [단계 1. 기존 능력 표현 모델과 표준 조사](stage-1-existing-models-and-standards.md)의 트랙 실행 2026-09-25-16에서 내용 검증 에이전트가 승인한 변경(개념 오류 수정, 개념 스킬 인터페이스 추가, 관계 "스킬 / 노출된다 / 스킬 인터페이스"·"기능 / 일반화된다 / 기능" 추가)을 반영한 버전이다. 함께 제안된 제약의 적용 시점 구분과 기능의 의미 식별자 속성은 근거가 부족해 반영하지 않고 6절 질문으로 두었다. v0.1(실행 2026-09-25-02)은 개념 효과·스킬·오류, 관계 "기능 / 구현된다 / 스킬", 기능의 능력 출처 구분 속성을 더했고 전제조건 개념 제안은 6절 질문으로 두었다.
...
개념은 번호나 코드로 부르지 않고 이름으로 부른다. 상태 값은 초안(시드) / 제안(검증 승인 전) / 확정(검증 승인) / 폐기(이유 병기)이며, 폐기한 개념은 표에서 지우지 않고 상태만 바꾼다. 이 표의 기능(Capability)은 능력·스킬·서비스(Capabilities, Skills and Services, CSS) 모델의 능력(capability), 곧 구현과 무관한 기능 명세에 대응시켜 부른다. 스킬 인터페이스의 프로토콜 값은 근거가 확인된 OPC UA·REST로 한정했고, VDA 5050 action·Open-RMF 동작을 여기에 넣을지는 6절 질문으로 둔다.
...
v0의 여섯 관계는 트랙 정의(빌드 사양서 8.1)의 v0 관계를 그대로 옮긴 것이고, v0.1에서 "기능 / 구현된다 / 스킬", v0.2에서 "스킬 / 노출된다 / 스킬 인터페이스"와 "기능 / 일반화된다 / 기능" 관계가 더해졌다. 관계의 방향은 주어에서 목적어로 읽는다. v0 관계의 카디널리티(한 로봇이 몇 개의 기능을 가지는지 등)는 정하지 않았으며 6절의 미해결 질문으로 둔다. 제조사는 아직 관계에 등장하지 않고, 효과와 오류도 다른 개념과의 관계가 승인되지 않았다(6절 참고).
...
도식은 2절의 개념 12개와 3절의 관계 9개만 그렸다. "모든 개념은 근거 문서를 가리킨다"는 개념마다 선을 긋는 대신 개념 묶음에서 근거 문서로 가는 점선 하나로 나타냈고, "기능 / 일반화된다 / 기능"은 기능에서 기능으로 돌아오는 선으로 나타냈다. 제조사·효과·오류는 승인된 관계가 없어 선 없이 놓여 있다.
...
### runs/2026-09-25-23/pages/tracks/manual-capability-ontology/model-standard-comparison.md
...
created: 2026-09-24
updated: 2026-09-25
sources: [ref-022, ref-025, ref-026, ref-027, ref-028, ref-029, ref-030, ref-031, ref-032, ref-033, ref-034, ref-035, ref-036, ref-037, ref-038, ref-039, ref-040, ref-041, ref-051, ref-228, ref-229, ref-243, ref-231, ref-230, ref-244, ref-245, ref-250, ref-348, ref-354, ref-235, ref-349, ref-350, ref-355, ref-351]
last_run: 2026-09-25
version: 4
---
...
- **전제조건**: 형식으로 담는 것은 IDTA 02020 속성 제약(전제·불변·사후조건), SkiROS2 스킬 조건, SOMA 요구 초기 상황, PDDL 행동 전제조건이다. VDA 5050 팩트시트의 action 정의에서는 전제조건 블록이 확인되지 않았다. [추정][^ref-229][^ref-250][^ref-349][^ref-029][^ref-228]
...
- **완료 확인 방법**: VDA 5050 3.0.0 pick·drop 완료 정의, IDTA 02020 사후조건, SkiROS2 사후조건, SOMA 기대 종료 상황·Succeeded 상태가 담는 것으로 보인다. [추정][^ref-031][^ref-229][^ref-250][^ref-349]
- **오류의 의미**: VDA 5050 오류 등급·해결 힌트와 SOMA 의 Failed 상태·기대 불일치 상황이 담으며, 오류 복구 절차를 구조화한 모델은 RETRIABLE 상태와 자유 서술 힌트 외에 확인하지 못했다. [추정][^ref-051][^ref-349]
- **학술 온톨로지 종합**: 원문을 연 학술 온톨로지 기준으로 전제조건·완료 확인·오류 의미는 SOMA 가, 파라미터 범위·환경 제약은 SSN System Capabilities 가 일부 담고, CORA·IEEE 1872.2 구현은 다섯 항목을 담지 않으며, 적재 제약은 산업 규격에만 있어 ROP 는 적재 제약과 오류의 조치 의미(등급·재시도)를 산업 규격 쪽에서 가져와야 할 것으로 보인다. [추정][^ref-349][^ref-235][^ref-355][^ref-350][^ref-228]
- **종합**: 다섯 정보 항목을 한 모델이 모두 담는 경우는 확인되지 않았다. [추정][^ref-228][^ref-031][^ref-051][^ref-243][^ref-245][^ref-230][^ref-250][^ref-349][^ref-235]
...
created: 2026-09-24
updated: 2026-09-25
sources: [ref-022, ref-025, ref-026, ref-027, ref-028, ref-029, ref-030, ref-031, ref-032, ref-033, ref-034, ref-035, ref-036, ref-037, ref-038, ref-039, ref-040, ref-041, ref-051, ref-228, ref-229, ref-243, ref-231, ref-230, ref-244, ref-245, ref-250]
last_run: 2026-09-25
version: 3
---
...
| Robotic Capability Ontology(RCO) (후보 밖) | 후보 밖 — finding f8, 실행 2026-09-25-02 | Naqvi 외(Scientific Reports, 2025-10-02) | 온톨로지(광고 능력·운용 능력 구분) [사실] | 미조사 | 미조사 | 미조사 | 미조사 | 미조사 | 미조사 | [^ref-041] | 원문 미열람 |
| 이종 자율 로봇 능력·스킬 모델 (후보 밖) | 후보 밖 — finding f18, 실행 2026-09-25-02 | Vieira da Silva·Köcher·Fay(2022-09) | 능력·스킬 모델, AAS 서브모델–온톨로지 양방향 매핑 개념 [사실] | 미조사 | 미조사 | 미조사 | 미조사 | 미조사 | 미조사 | [^ref-038] | 원문 미열람 |
| IDTA 02047 무인운반차 기술 데이터 (후보 밖) | 후보 밖 — finding f9, 실행 2026-09-25-16 | IDTA | AAS 서브모델 템플릿 1.0 [사실] | 미조사 | 부분: 최대 속도·가동 시간의 명세값(AsSpecified)·운용값(AsOperated), 위치추정·정위치 정확도 [사실] | 담음(형식): 최대 적재 질량, 적재·무적재 등판·측경사 각, 실외 사용 적합 여부와 요구 환경 조건 [사실] | 미조사 | 미조사 | 미조사 | [^ref-245] | 확인(템플릿 원문 열람) |
| SkiROS2 (후보 밖) | 후보 밖 — finding f10, 실행 2026-09-25-16 | RVMI lab, Aalborg University | 스킬을 행동 트리로 조합하는 ROS 기반 플랫폼 [사실]. 로봇 내부 실행 플랫폼(연계 대상)이며 스킬 조건 표현 사례로만 비교 | 담음(형식): 스킬마다 전제조건·유지조건·사후조건 [사실] | 부분: 의미 데이터베이스 세계 모델로 스킬 파라미터 자동 추론 [사실] | 미조사 | 부분: 사후조건이 완료 확인에 쓰일 수 있음 [추정] | 미조사 | 미조사 | [^ref-250] | 확인(README 원문 열람) |
| CaSkMan (후보 밖) | 후보 밖 — finding f20, 실행 2026-09-25-16 | CaSkade-Automation(Köcher 외) | 제조 기계의 능력·스킬 OWL 온톨로지(이동로봇 사례 없음) [사실] | 미조사 | 미조사 | 미조사 | 미조사 | 미조사 | 기계가 능력을 제공하고 능력이 스킬로 실현되며 스킬이 ISA 88 상태 기계와 REST·OPC UA 스킬 인터페이스로 실행됨 [사실] | [^ref-231] | 확인(README 원문 열람) |
...
- **전제조건**: 형식으로 담는 것은 IDTA 02020 속성 제약(전제·불변·사후조건), SkiROS2 스킬 조건, PDDL 행동 전제조건이다. VDA 5050 팩트시트의 action 정의에서는 전제조건 블록이 확인되지 않았다. [추정][^ref-229][^ref-250][^ref-029][^ref-228]
- **파라미터 범위**: IDTA 02020 범위(Range) 속성이 담고, VDA 5050 팩트시트는 데이터형만 둔다. [추정][^ref-243][^ref-228]
- **적재·환경 제약**: VDA 5050 적재 세트, IDTA 02047, MassRobotics 화물 최대값이 담는다. [추정][^ref-228][^ref-245][^ref-230]
- **완료 확인 방법**: VDA 5050 3.0.0 pick·drop 완료 정의, IDTA 02020 사후조건, SkiROS2 사후조건이 담는 것으로 보인다. 이전 실행의 "완료 확인 방법을 명시하는 항목 미확인" 판단은 검색 범위의 추정이었고 이번 원문 대조로 고쳤다. [추정][^ref-031][^ref-229][^ref-250]
- **오류의 의미**: VDA 5050 오류 등급·해결 힌트가 담으며, 오류 복구 절차를 구조화한 모델은 RETRIABLE 상태와 자유 서술 힌트 외에 확인하지 못했다. [추정][^ref-051]
- **종합**: 다섯 정보 항목을 한 모델이 모두 담는 경우는 확인되지 않았다. [추정][^ref-228][^ref-031][^ref-051][^ref-243][^ref-245][^ref-230][^ref-250]
...
### runs/2026-09-25-23/pages/tracks/manual-capability-ontology/index.md
...
| [단계 5. 완전성과 정확성을 검증하는 방법 조사](stage-5-completeness-verification.md) | 대기 | 8 | 미충족 |
...
- [능력 온톨로지 초안](ontology-draft.md) — 현재 버전 v0.2. v0 시드(개념 8개·관계 6개)에 단계 1 실행 2026-09-25-02(v0.1: 개념 효과·스킬·오류, 관계 기능 / 구현된다 / 스킬, 기능의 능력 출처 구분 속성)와 실행 2026-09-25-16(v0.2: 오류 수정, 개념 스킬 인터페이스, 관계 스킬 / 노출된다 / 스킬 인터페이스·기능 / 일반화된다 / 기능)에서 검증 승인된 변경을 반영했다.
- [모델·표준 비교표](model-standard-comparison.md) — 단계 1 산출물. 실행 2026-09-25-02에서 초안 작성, 실행 2026-09-25-16에서 산업 규격·서브모델·오픈소스 행을 공식 저장소 원문으로 보강하고 후보 밖 3행을 더했다(학술 온톨로지 행의 다섯 정보 항목은 미조사).
- [문서 유형 매트릭스](document-type-matrix.md) — 단계 2 산출물. 빈 틀(단계 2 실행에서 채운다).
- [평가 지표와 검증 절차](evaluation-and-verification.md) — 단계 5 산출물. 빈 틀(단계 5 실행에서 채운다).
- [질문 백로그](question-backlog.md) — 열림 39건 · 조사 중 1건 · 답한 질문 4건(실행 2026-09-25-16 반영 기준. 최신 수치는 백로그 페이지의 자동 표를 따른다)
- [트랙 로그](log.md) — 실행별 기록
- [실험](experiments.md) — 제안된 실험 계획과 사용자 실험 결과 요약(선택). 현재 제안된 실험은 없다.
- [아이디어 1. 로봇 기능 온톨로지](../../ideas/robot-capability-ontology.md) — 확장 아이디어 페이지(문제 정의, 관련 세부 연구영역, 선행 연구·제품 사례, 필요한 데이터와 표준, 구현 가설, 검증 방법, 미해결 질문 백로그). 3~6절은 이 트랙의 실행이 채우며, 실행 2026-09-25-16에서 3·4절을 채웠다.
...
### runs/2026-09-25-23/pages/ideas/robot-capability-ontology.md
...
<!-- auto:page-status:start -->
> 페이지 상태: published · 신뢰도: 미부여 · 페이지 버전: 2 · 마지막 갱신: 2026-09-25 · 마지막 실행: 없음
<!-- auto:page-status:end -->
...
**능력 매칭 연구(실행 2026-09-25-23).** 근거 문장 전체는 [단계 1의 q1-06 답](../tracks/manual-capability-ontology/stage-1-existing-models-and-standards.md#q1-06)에 있다. IDTA 02020 능력 기술 1.0 README 는 이 서브모델이 공정·제품 쪽 요구 능력과 자원 쪽 제공 능력을 비교해 생산 계획·오케스트레이션을 돕는다고 설명한다. [사실][^ref-229] Järvenpää 외(IJCIM 36(1), 2023, 온라인 2022-06-07)는 제품과 자원의 온톨로지 기술과 SPIN 규칙으로 결합 자원의 결합 능력 파라미터를 추론하고 제품 특성을 자원(조합)의 능력 파라미터와 비교해 가능한 자원 조합을 찾는, 능력 매칭을 위한 의미 규칙(SPIN)을 제시했다. [사실][^ref-352] Köcher·Vieira da Silva·Fay(arXiv 2312.08801, 2023-12, AAAI 2024 CAIPI 워크숍)는 의미 능력 모델과 SMT 로 제품 생산이나 자율 로봇 임무 수행에 필요한 개별 능력의 순서를 자동으로 찾는 공정 계획 방법을 제안했다. [사실][^ref-353] 두 연구는 제조 공정 계획 연구이므로 물류 현장 적용 사례가 아니라 방법의 선례로만 참고한다.

작업 할당 질의가 분류 원문 질문(같은 운반 로봇 중 누가 이 화물을 실제로 취급할 수 있는가)에 답하려면 작업 요구를 화물 치수·중량 같은 요구 능력 속성으로 표현하고 로봇의 적재 세트 같은 제공 능력 속성과 비교하는 규칙이 필요할 것으로 보이며, 제조 분야의 능력 매칭 연구가 이 비교를 의미 규칙으로 구현한 선례가 된다. [추정][^ref-229][^ref-352][^ref-228][^ref-236] 물류 이동로봇 대상의 능력 매칭 규칙은 찾지 못했고, 질의 형식은 트랙 질문 q4-07 로 남아 있다.
...
원천: [질문 백로그](../tracks/manual-capability-ontology/question-backlog.md)([매뉴얼 기반 로봇 기능 온톨로지](../tracks/manual-capability-ontology/index.md) 트랙) · 열림 43건 · 조사 중 1건 · 답함 4건
...
| 열림 | q1-10 | ECLASS·IEC CDD 에 이동로봇의 범위 능력(이동·계단·적재·도어 조작·충전)과 그 속성을 기술하는 항목이 있는가, 있으면 능력 온톨로지의 의미 식별자로 쓸 수 있는가? (q1-05 에서 파생) | [단계 1. 기존 능력 표현 모델과 표준 조사](../tracks/manual-capability-ontology/stage-1-existing-models-and-standards.md) | f30 | — |
...
| 열림 | q4-11 | CSS 참조 모델의 스킬 상태 기계(SkillType·SkillStateMachine)와 VDA 5050 action 상태(WAITING~FAILED·RETRIABLE), Open-RMF execute_action 완료 신호를 하나의 스킬 실행 상태 모델로 대응시킬 수 있는가? (q1-04 에서 파생) | [단계 4. 온톨로지를 실행에 연결하는 방법 조사](../tracks/manual-capability-ontology/stage-4-execution-grounding.md) | f26 | — |
...
| 열림 | q5-08 | 서로 다른 제조사가 정의한 사용자 정의 action(예: 도킹·리프트)이 같은 동작인지를 파라미터·완료 정의·효과 비교로 판정하는 시험 절차를 어떻게 둘 것인가? (q1-05 에서 파생) | [단계 5. 완전성과 정확성을 검증하는 방법 조사](../tracks/manual-capability-ontology/stage-5-completeness-verification.md) | f35 | — |
...
### runs/2026-09-25-23/docs_tree.txt
...
### runs/2026-09-25-23/verification2.json
