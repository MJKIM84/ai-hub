...
- 대상: 트랙 nl-task-chatbot (자연어 업무 지시 챗봇) · 현재 단계: 단계 4. 오해석 방지와 확인 절차 · 이번에 다룰 백로그 질문 id: q4-01 · 중심 세부영역: 13. 작업 배정 — MRTA (D. 계획·최적화)
- 예산:
...
### runs/2026-09-25-79/target.json
...
```json
{
  ...
  "selection_rationale": "CLI 지정 run_type=track, area=13; 트랙 실행일(Fri, track_days 앞 3개) → 트랙 nl-task-chatbot 단계 4, 질문 q4-01 (CLI 지정 질문 id)"
}
```

### runs/2026-09-25-79/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q4-01 LLM의 잘못된 해석이 로봇 배정으로 이어지지 않게 하는 확인 절차는 어떻게 두는가?",
    "LLM 로봇 계획·배정 출력을 실행 전에 거르는 가드레일(형식 논리 기반 안전 모듈, 규칙 추론기, 결정적 게이트)은 어느 단계에서 무엇을 검사하는가? (단계 4 페이지 3절, 27. AI·학습·적응과 모델 운영 겨냥)",
    "LLM 에이전트 설계 지침·도구 규격(OWASP LLM Top 10, 모델 컨텍스트 프로토콜, LangChain 사람 참여 미들웨어)은 사람 확인을 어떤 행동에, 어떤 형태(승인·수정·거부)로 요구하는가? (18. 사람–로봇 협업·운영 인터페이스, 26. 사이버보안·접근권한·개인정보 연결)",
    "로봇 관제 인터페이스(VDA 5050, Open-RMF)에서 확인이 어느 시점 전에 끝나야 하며, 로봇·디스패처 쪽의 마지막 거절 장치는 무엇인가? (12. 명령·작업 실행의 신뢰성 연결)",
    "사람 승인은 실제로 오류를 걸러내는가 — 계획 승인·행동 확인의 사용자 연구, 자동화 편향과 이를 다루는 규제(EU AI Act 제14조, 한국 인공지능기본법 제34조)는 무엇을 말하는가? (25. 안전·위험 관리 연결, 한국 자료 우선 규칙)",
    "대화 시스템의 명시적·암시적 확인 방식은 확인 부담과 오류 교정 사이에서 어떻게 나뉘는가?"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "OWASP LLM 애플리케이션 Top 10(2025판)의 과도한 에이전시(Excessive Agency) 항목은 원인을 과도한 기능·과도한 권한·과도한 자율성 셋으로 나누고, 대응으로 영향이 큰 행동 전 사람 승인, 최소 권한, 완전한 중재(complete mediation)를 든다.",
      "tag": "사실",
      "source_ids": [
        "ref-713"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "LLM06 문서: 세 원인(excessive functionality·permissions·autonomy). 대응: \"require a human to approve high-impact actions before they are taken\", 확장 기능 최소화, 사용자 맥락 실행.",
      "as_of": "2024-11",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f2",
      "claim": "모델 컨텍스트 프로토콜(MCP) 명세(2025-06-18판) 도구 절은 도구 호출을 거부할 수 있는 사람이 항상 루프에 있어야 한다고(SHOULD) 적고, 클라이언트가 민감한 작업에 사용자 확인을 묻고 서버 호출 전 도구 입력을 사용자에게 보여 주며 도구 사용 감사 기록을 남기도록 권고한다.",
      "tag": "사실",
      "source_ids": [
        "ref-714"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "tools.mdx 보안 고려: 서버는 입력 검증·접근 통제·호출 속도 제한, 클라이언트는 민감 작업 확인 요청·호출 전 입력 표시·결과 검증·시간 초과·감사 기록.",
      "as_of": "2025-06-18",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f3",
      "claim": "LangChain 의 사람 참여(Human-in-the-Loop) 미들웨어는 설정한 도구 호출에서 에이전트 실행을 멈추고 사람이 승인(approve)·인자 수정(edit)·거부(reject, 피드백 포함)·직접 응답(respond) 가운데 하나로 결정하게 하며, 중단 상태를 보존하려면 체크포인터가 필요하다.",
      "tag": "사실",
      "source_ids": [
        "ref-715"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "interrupt_on 설정에 든 도구 호출(파일 쓰기·SQL 실행 등)만 중단. 네 결정 유형. 체크포인팅 필요 명시(docs 저장소 human-in-the-loop.mdx). (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f4",
      "claim": "Safety Chip(Yang 외, ICRA 2024)은 자연어로 준 금지 제약을 선형 시간 논리(LTL) 식으로 옮겨 오토마톤으로 두고, LLM 에이전트의 결정을 감시해 안전하지 않은 동작을 걸러내고 위반 이유를 설명·재프롬프트에 쓰는 질의 가능한 제약 모듈이며, VirtualHome 과 실제 로봇(Spot)에서 실험했다.",
      "tag": "사실",
      "source_ids": [
        "ref-716",
        "ref-717"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: constraint_module 이 자연어→LTL 변환(lang2ltl 확장), precond 모듈이 계획 생성 중 제약 검사, VirtualHome·Spot 실험. 논문 요약: 위반 추론·설명과 불안전 동작 가지치기.",
      "as_of": "2023-09",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f5",
      "claim": "RoboGuard 는 미리 정한 안전 규칙을 신뢰 기반(root-of-trust) LLM 이 로봇 환경의 의미 그래프에 접지해 시간 논리 안전 명세를 만들고, 후보 계획(사람 또는 LLM 이 만든)과 명세가 충돌하면 시간 논리 제어 합성으로 해소하는 2단계 가드레일이며, 저자들은 탈옥 공격 조건에서 안전하지 않은 계획 실행을 92% 넘음에서 3% 미만으로 줄였다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-718",
        "ref-719"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: 입력 의미 그래프·규칙집·후보 계획, 출력 안전 명세·계획 평가, Clearpath Jackal 실험. 수치(>92%→<3%)는 arXiv 초록 요약 기준 저자 보고값(RA-L 2026 채택 표기).",
      "as_of": "2025-03",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": false
    ...
    {
      "id": "f6",
      "claim": "SafePlan 은 LLM 이 여러 로봇·사람에게 작업 계획, 팀 구성, 작업 배정을 만드는 시스템에서 프롬프트 건전성 추론기와 불변·전제·사후 조건 추론기로 자연어 지시, 작업 계획, 배정 결과의 안전성을 각각 검사하며, 저자들은 전문가가 만든 621개 지시 벤치마크에서 유해 작업 수용을 90.5% 줄였다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-720"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "원문 미열람. 검색 요약: 형식 논리와 사고 연쇄 추론기 결합, 계획·팀 구성·배정 출력의 검토·수정·거부, 621 프롬프트–장면 쌍, 유해 작업 수용 90.5% 감소(저자 보고).",
      "as_of": "2025-03",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f7",
      "claim": "SafeGate 는 자연어 명령의 안전 속성을 뽑아 결정적 판정으로 실행을 승인·거부하고, 통과한 명령을 불변 조건·가드·중단 조건으로 된 작업 안전 계약으로 분해하는 실행 전 게이트다.",
      "tag": "사실",
      "source_ids": [
        "ref-417"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "ISO 13482 기반 결정적 판정(개인 돌봄 로봇 표준이라 물류 적용 미확인). (재인용: 2026-09-25-71)",
      "as_of": "2026-04",
      "flow_step": null,
      "flow_item": "제약",
      "source_unopened": true
    ...
    {
      "id": "f8",
      "claim": "Tang 외는 산업용 다중 로봇에서 에이전트의 제안을 결정적 검증과 원자적 반영(atomic commit)을 거쳐야만 작업 숲·관리형 블랙보드에 받아들이고 검증 기록을 남기는 구조를 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-711"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검증 게이트 뒤 원자적 반영, 블랙보드에 제안·검증 기록 보관. (재인용: 2026-09-25-71)",
      "as_of": "2026-06",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f9",
      "claim": "Rasa 폼은 필수 슬롯을 정해 두고 비어 있는 다음 필수 슬롯을 사용자에게 묻고, 추출한 값을 사용자 정의 검증 동작으로 검사한다.",
      "tag": "사실",
      "source_ids": [
        "ref-356"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "해석 단계 출력(슬롯)을 규칙으로 검사하는 결정적 구성 요소의 예. (재인용: 2026-09-25-71)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "시작 조건",
      "source_unopened": true
    ...
    {
      "id": "f10",
      "claim": "VDA 5050 3.0.0 은 관제가 로봇에 풀어 준 베이스는 바꿀 수 없어 관제가 이를 이미 실행된 것으로 가정해야 하고 주문 취소도 통신 한계로 신뢰할 수 없다고 보며, 로봇은 수행할 수 없는 동작이 든 주문을 내부 버퍼에 받지 않고 INVALID_ORDER_ACTION 경고로 거절한다.",
      "tag": "사실",
      "source_ids": [
        "ref-031"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "6.1.2: base cannot be changed, 관제는 베이스 실행을 가정, 취소 절차도 unreliable. 6.1.4.3: 인상 높이 초과 등 수행 불가 동작 주문은 버퍼에 넣지 않고 오류 보고(공식 저장소 main 3.0.0).",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f11",
      "claim": "Open-RMF 디스패처는 입찰 기간에 어떤 플릿 어댑터도 입찰하지 않으면 작업의 배정 상태를 FailedToAssign 으로 두고 오류를 기록하며, 그 작업은 수행되지 않는다.",
      "tag": "사실",
      "source_ids": [
        "ref-656"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "Dispatcher.cpp: 무입찰 시 FailedToAssign 과 'No fleet adapters offered a bid' 오류. (재인용: 2026-09-25-74)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f12",
      "claim": "He·Demartini·Gadiraju(CHI 2025)는 LLM 에이전트를 계획 후 실행 방식으로 쓰는 일상 비서 과제(위험도가 다른 6개 과제, 참가자 248명, 시뮬레이션 환경)에서 사용자 참여를 조사해, 계획 품질이 높고 실행 단계 사용자 참여가 있을 때는 잘 작동하지만 그럴듯해 보이는 계획을 사용자가 쉽게 잘못 신뢰했다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-721",
        "ref-722"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "저장소 README: 248명, 항공권 예약·카드 결제 등 6개 과제, 계획 단계·실행 단계 사용자 참여 비교, 그럴듯한 계획에 대한 잘못된 신뢰. 물류·로봇 조건 아님.",
      "as_of": "2025-04",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": false
    ...
    {
      "id": "f13",
      "claim": "컴퓨터 사용 에이전트의 사람 감독 전략(행동마다 확인, 위험 기반 선택적 승인, 계획 수준 감독)을 비교한 연구(arXiv 2604.04918)는 모든 전략에서 최종 공격 성공률이 상당히 남았고, 감독 전략은 문제 행동이 사용자에게 보이게 하는 데는 영향을 주었지만 보인 뒤 사용자가 멈추게 하는 데는 영향이 작았다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-723"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "원문 미열람. 검색 요약: Action Confirmation·Risk-Gated·Supervisory Co-Execution 비교, 개입 성공률은 낮았다고 요약되나 조건별 수치는 요약마다 달라 넣지 않음.",
      "as_of": "2026-04",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f14",
      "claim": "EU AI Act 제14조 제4항 (b)호는 고위험 AI 시스템을 감독하는 사람이 시스템 출력에 자동으로 의존하거나 과도하게 의존하는 경향(자동화 편향)을 계속 인식할 수 있게 설계하도록 요구한다.",
      "tag": "사실",
      "source_ids": [
        "ref-724",
        "ref-725"
      ...
      "cross_checked": true,
      "confidence": "medium",
      "evidence_excerpt": "원문 미열람. 조문 게재본 요약과 법학 논문(2502.10036) 요약이 모두 14(4)(b)의 자동화 편향 인식 요구를 적음. 물류 로봇 배정 AI 의 고위험 해당 여부는 미확인.",
      "as_of": "2024",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f15",
      "claim": "한국 인공지능 발전과 신뢰 기반 조성 등에 관한 기본법(법률 제20676호, 2025-01-21 제정) 제34조는 고영향 인공지능 사업자가 위험관리방안, 설명 방안, 이용자 보호 방안과 함께 사람의 관리·감독 조치를 이행하도록 정한다.",
      "tag": "사실",
      "source_ids": [
        "ref-620"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "원문 미열람. 검색 요약 기준 제34조 제1항에 '고영향 인공지능에 대한 사람의 관리·감독' 포함. 물류 로봇 배정 AI 의 고영향 해당 여부는 oq-105 로 미해결.",
      "as_of": "2025-01-21",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f16",
      "claim": "Sagawa(INTERSPEECH 2004)는 음성 대화 시스템의 오류 처리에서 명시적 확인, 최종 확인, 암시적 확인 세 방식을 비교해 사용자 만족과 효율을 평가했다.",
      "tag": "사실",
      "source_ids": [
        "ref-727"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "원문 미열람. 검색 요약: CAMMIA 대화 시스템에서 explicit·final·implicit 확인 비교. 결과 수치와 우열은 확인하지 못해 넣지 않음.",
      "as_of": "2004-10",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f17",
      "claim": "Mecalux 는 WMS 에 통합한 대화형 비서가 긴급 주문 출고 같은 WMS 작업을 채팅 요청으로 실행하되 실행 전에 동작과 영향 항목의 요약을 보여 주고 확인을 받는다고 밝힌다.",
      "tag": "추정",
      "source_ids": [
        "ref-418"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "벤더 주장: 실행 전 동작·영향 요약 확인. WMS 제품 기능이라 ROP 에게는 연계 대상 사례. (재인용: 2026-09-25-37)",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "시작 조건",
      "source_unopened": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "불확실성에 따라 확인을 발동하는 기준의 사례. (재인용: 2026-09-25-74)",
      "as_of": "2023-07",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f19",
      "claim": "q4-01 에 대해 확인한 자료를 이 위키가 묶으면, 확인 절차는 (1) 해석 게이트: 필수 슬롯·형식 검사와 불확실성 기준에 따른 되묻기, (2) 제약 게이트: 해석 결과·계획·배정을 안전 규칙·권한·능력 제약과 결정적으로 대조, (3) 사람 확인: 영향이 크거나 불확실할 때만 해석 요약을 승인·수정·거부로 받기, (4) 검증 뒤 기록과 함께 상태에 반영, (5) 디스패처·로봇 쪽의 마지막 거절(무입찰·수행 불가 동작 거절)의 다섯 겹으로 두는 구성이 근거가 가장 많은 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-356",
        "ref-350",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. 다섯 겹을 한 번에 제시한 단일 출처는 찾지 못함. 근거 조건은 가정·실험실 로봇, 소프트웨어 에이전트, 로봇 관제 규격.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": false
    ...
    {
      "id": "f20",
      "claim": "VDA 5050 에서 로봇에 풀어 준 베이스는 바꿀 수 없고 취소도 신뢰할 수 없으므로, 해석 확인은 배정 계산 전에, 배정 결과 확인은 배치(베이스 해제) 전에 끝나야 하며, 사람 확인이 필요한 작업은 확인이 날 때까지 배치를 보류하는 중단점(체크포인트)으로 두는 것이 선택지로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-031",
        "ref-715",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "베이스 불변·취소 불신(VDA 5050)과 중단·재개 체크포인트(LangChain), 검증 뒤 원자적 반영(Tang 외)을 대응시킨 추론.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약",
      "source_unopened": false
    ...
    {
      "id": "f21",
      "claim": "사람 승인만으로는 오해석을 걸러내기 어렵다는 보고(그럴듯한 계획에 대한 잘못된 신뢰, 문제 행동이 보여도 멈추지 못함)와 자동화 편향 인식을 요구하는 규정을 보면, 결정적 게이트를 먼저 두고 사람 확인은 영향이 큰 작업에 한정해 해석 결과와 결정적 검사 결과의 차이를 드러내는 형태로 두는 편이 맞는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-721",
        "ref-722",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "근거는 일상 비서·컴퓨터 사용 에이전트 조건. 물류 관제 요원의 승인 행동을 잰 자료는 찾지 못함.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "예외·성과",
      "source_unopened": false
    ...
    {
      "id": "f22",
      "claim": "확인 방식은 작업의 영향도와 해석 불확실성에 따라 나누어, 위험 구역 진입·적재 화물 취소·일괄 정지 같은 영향이 큰 작업은 명시적 확인(승인 전 대기)으로, 일상적 운반 지시는 응답에 해석 결과를 되풀이해 보여 주는 암시적 확인으로 두는 차등 구성이 선택지로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-727",
        "ref-713",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "확인 방식 비교(대화 시스템), 영향 큰 행동 승인(OWASP), 민감 작업 확인(MCP), 불확실성 기준(KnowNo)을 대응시킨 추론. 영향도 기준 목록은 출처 없음.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약",
      "source_unopened": false
    ...
    {
      "id": "f23",
      "claim": "분류 원문 질문(가장 가까운 로봇에 맡기는 것이 전체적으로도 유리한가)과 관련해, 확인 화면이 사용자에게 로봇 선택 자체를 고르게 하기보다 해석한 업무(대상·장소·기한)와 디스패처가 쓴 배정 기준(가장 빨리 끝남 등)을 보여 주면, 사람은 해석 오류를 확인하고 배정의 전체 기준 일관성은 결정적 배정기가 지키는 분담이 가능해 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-656",
        "ref-715",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "평가기 기반 자동 배정(Open-RMF)과 도구 입력 표시·수정 결정(MCP·LangChain)을 대응시킨 추론.",
      "as_of": "2026-09-25",
      "flow_step": "피킹",
      "flow_item": "수행 자원",
      "source_unopened": false
    ...
    {
      "id": "f24",
      "claim": "피킹 구역 관리자가 채팅으로 '피킹 끝난 토트를 10시 전까지 2번 도크로'라고 지시하면, 해석 게이트가 대상·장소·기한 슬롯을 검사해 빠진 값을 되묻고, 제약 게이트가 지시자의 구역·작업 권한과 도크 도달 가능성을 대조한 뒤, 일상 운반이면 해석 요약을 응답에 보여 주고 바로 반영하며 다른 사람의 진행 작업 취소를 포함하면 명시적 승인을 받을 때까지 배치를 보류하는 흐름이 가능해 보인다(설명용 가정 사례).",
      "tag": "추정",
      "source_ids": [
        "ref-356",
        "ref-714",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "설명용 가정 사례. 슬롯 검사(Rasa), 접근 통제·확인(MCP), 검증 뒤 반영(Tang 외), 베이스 해제 전 확인(VDA 5050)을 조합.",
      "as_of": "2026-09-25",
      "flow_step": "피킹",
      "flow_item": "시작 조건",
      "source_unopened": false
    ...
    {
      "id": "f25",
      "claim": "이번에 확인한 확인·가드레일 근거의 평가 환경은 가정 시뮬레이터·실험실 로봇, 일상 비서·컴퓨터 사용 에이전트, 음성 대화 시스템이었고, 물류 창고 로봇에 채팅으로 준 지시의 확인 절차를 평가한 연구와 국내 사례는 한국어 검색 3회를 포함한 검색 범위에서 찾지 못했다(부재의 확인은 아님).",
      "tag": "추정",
      "source_ids": [
        "ref-716",
        "ref-718",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "한국어 검색 결과는 해외 논문 번역 페이지와 법령 해설뿐이었음. 검색 범위 관찰.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
  ...
  "sources": [
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "과도한 에이전시의 세 원인(기능·권한·자율성)과 사람 승인·최소 권한·완전한 중재 같은 대응을 정리한 OWASP 프로젝트 문서.",
      "fetched": true,
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "MCP 도구 명세. 도구 호출을 거부할 수 있는 사람 참여, 민감 작업 확인, 호출 전 입력 표시, 입력 검증·접근 통제·감사 기록을 권고한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-715",
      "org": "LangChain (langchain-ai/docs GitHub)",
      "title": "Human-in-the-loop — LangChain docs (src/oss/langchain/human-in-the-loop.mdx)",
      "published": null,
      "url": "https://docs.langchain.com/oss/python/langchain/human-in-the-loop",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "설정한 도구 호출에서 실행을 멈추고 승인·수정·거부·응답 결정을 받는 미들웨어 문서. 체크포인터가 필요하다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/langchain-ai/docs/main/src/oss/langchain/human-in-the-loop.mdx",
      "source_unopened": false
    },
    {
      "id": "ref-716",
      "org": "Yang, Z. 외(Brown University H2R Lab)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자연어 제약을 LTL 오토마톤으로 바꿔 LLM 로봇 에이전트의 불안전 동작을 걸러내는 모듈(ICRA 2024).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-717",
      "org": "YzyLmc (Safety Chip 공식 저장소)",
      "title": "ltl_safety — README (Plug in the Safety Chip)",
      "published": null,
      "url": "https://github.com/YzyLmc/ltl_safety",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "Safety Chip 코드 저장소. 자연어→LTL 제약 모듈, 전제 조건 검사, VirtualHome·Spot 실험 구성.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/YzyLmc/ltl_safety/main/README.md",
      "source_unopened": false
    },
    {
      "id": "ref-718",
      "org": "RoboGuard 저자(KumarRobotics, arXiv 2503.07885)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 신뢰 기반 LLM 으로 안전 규칙을 환경에 접지해 시간 논리 명세를 만들고 제어 합성으로 불안전 계획을 해소하는 2단계 가드레일.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-719",
      "org": "KumarRobotics (RoboGuard 공식 저장소)",
      "title": "RoboGuard — Safety guardrails for LLM-enabled robots (GitHub README)",
      "published": null,
      "url": "https://github.com/KumarRobotics/RoboGuard",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "의미 그래프·규칙집·후보 계획을 받아 안전 명세를 만들고 계획을 평가하는 구현. SPINE 계획기·Jackal 로봇과 연동.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/KumarRobotics/RoboGuard/main/README.md",
      "source_unopened": false
    },
    {
      "id": "ref-720",
      "org": "SafePlan 저자(arXiv 2503.06892, 저자 미확인)",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 지시·계획·팀 구성·배정 출력을 불변·전제·사후 조건 추론기로 검사하는 틀과 621개 지시 벤치마크.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "title": "CHI2025_Plan-then-Execute_LLMAgent — README",
      "published": null,
      "url": "https://github.com/RichardHGL/CHI2025_Plan-then-Execute_LLMAgent",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "계획 후 실행 LLM 에이전트 사용자 연구(248명, 6개 과제)의 저장소. 그럴듯한 계획에 대한 잘못된 신뢰를 요약한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-722",
      "org": "He, G., Demartini, G., & Gadiraju, U.",
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. CHI 2025 논문. 계획·실행 단계 사용자 참여가 신뢰와 팀 성과에 주는 영향을 조사.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 행동 확인·위험 기반 승인·계획 수준 감독 전략을 비교한 사용자 연구. 모든 전략에서 공격 성공이 상당히 남음.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "title": "Article 14: Human Oversight | EU Artificial Intelligence Act",
      "published": "2024",
      "url": "https://artificialintelligenceact.eu/article/14/",
      "type": "정부·연구기관",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. EU AI Act 제14조(사람의 감독) 조문 게재본. 공식 관보(EUR-Lex) 판이 아님.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. EU AI Act 제14조의 자동화 편향 인식 요구의 법적 함의를 분석한 논문.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      "id": "ref-620",
      "org": "국가법령정보센터(법제처)",
      "title": "인공지능 발전과 신뢰 기반 조성 등에 관한 기본법 (법률 제20676호, 2025-01-21)",
      "published": "2025-01-21",
      "url": "https://www.law.go.kr/lsInfoP.do?lsiSeq=268543",
      "type": "정부·연구기관",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 한국 인공지능기본법. 제34조가 고영향 인공지능 사업자의 위험관리·설명·이용자 보호·사람의 관리·감독 조치를 정한다.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 음성 대화 시스템에서 명시적·최종·암시적 확인 방식을 비교한 연구.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "VDA 5050 3.0.0 명세(공식 저장소 main). 베이스·호라이즌, 주문 거절, 취소 규칙.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 등각 예측 기반으로 불확실할 때 사람에게 도움을 요청하는 LLM 계획기 프로젝트 페이지(이번 실행 재열람 안 함).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 필수 슬롯 질문과 추출 값 검증을 다루는 Rasa 폼 문서(이번 실행 재열람 안 함).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자연어 명령의 안전 속성을 결정적으로 판정하는 실행 전 게이트와 작업 안전 계약.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "low",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. WMS 대화형 비서가 실행 전 요약을 보여 주고 확인을 받는다는 벤더 발표.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. Open-RMF 디스패처 구현. 무입찰 시 FailedToAssign 기록(이번 실행 재열람 안 함).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 에이전트 제안을 결정적 검증과 원자적 반영을 거쳐 받아들이는 관리형 블랙보드 구조.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
  ...
  "page_proposals": [
    {
      ...
      "rationale": "q4-01 답: f1·f2·f3·f4·f5·f6·f7·f8·f9·f10·f11·f12·f13·f14·f15·f16·f17·f18·f19·f20·f21·f22·f23·f24·f25 (신뢰도 low) — 2절 q4-01 상태 답함, 3절 q4-01 소제목 신설({#q4-01}): 에이전트 설계 지침·도구 규격의 사람 확인(OWASP f1, MCP f2, LangChain f3), 로봇 가드레일(Safety Chip f4, RoboGuard f5, SafePlan f6 — 배정 출력 검사 포함, SafeGate f7), 검증 뒤 반영(f8), 해석 게이트(f9·f18), 로봇·디스패처 쪽 마지막 거절과 베이스 불변(f10·f11), 사람 승인의 한계(f12·f13)와 규제(EU AI Act f14, 한국 인공지능기본법 f15), 확인 방식(f16), 벤더 사례(f17 벤더 주장), 종합: 다섯 겹 확인 절차(f19, mermaid 흐름 권장)·확인 시점(f20)·사람 확인 한정(f21)·차등 확인 방식(f22)·SCM 질문 연결(f23)·피킹 시나리오(f24)·근거 공백(f25) / 4절 결론·불확실성 / 5절 후속 질문 / 6절 완료 조건 현황 / 8절 출처 / 9절 이력"
    },
    {
      "action": "update",
      ...
      "rationale": "아이디어 페이지 5절(트랙 산출물): '오해석 방지 확인 절차' 소절 신설 — 다섯 겹 확인 절차 f19, 확인 시점 f20, 사람 확인 한정과 자동화 편향 f21, 차등 확인 방식 f22(모두 추정), 근거 f1·f2·f3·f4·f5·f6·f10·f12·f13·f14. 명령 권한(q4-03)·제한 운영 기준(q4-04)은 미조사임을 명시"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신: track.ontology_changes(개념 '사용자 확인' 추가)가 승인되면 2절 반영과 초안 버전 인상(f2·f3·f16·f19·f22). 미승인 시 6절 '사용자 확인(승인)을 별도 개념으로 둘지, 배정의 속성(확인 여부)으로 둘지' 질문에 q4-01 답(f19·f20) 연결, 실행 2026-09-25-71 의 '검증 기록' 질문과의 경계 메모"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 4 반영 제안 (f6, f10, f11, f19, f23): LLM 배정 출력의 실행 전 검사(SafePlan), 디스패처 무입찰·로봇 수행 불가 거절을 마지막 거절 장치로, 확인 화면에 배정 기준을 보이는 분담과 분류 원문 질문 연결. 27. AI·학습·적응과 모델 운영과 양쪽 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 4 반영 제안 (f3, f12, f13, f16, f21, f22): 승인·수정·거부 확인 인터페이스, 계획 승인 사용자 연구와 감독 전략 비교에서 드러난 사람 승인의 한계, 명시적·암시적 확인의 차등 적용"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 4 반영 제안 (f4, f5, f6, f14, f15): LLM 로봇 계획의 형식 논리 가드레일(Safety Chip, RoboGuard, SafePlan), 자동화 편향 인식 요구(EU AI Act 제14조)와 인공지능기본법 제34조 사람의 관리·감독. 적용 대상 13. 작업 배정 — MRTA·18. 사람–로봇 협업·운영 인터페이스와 함께 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 nl-task-chatbot 단계 4 반영 제안 (f1, f2): 과도한 에이전시의 세 원인과 최소 권한·완전한 중재, MCP 도구 명세의 접근 통제·감사 기록 권고(q4-03 명령 권한 조사와 이어짐)"
    }
  ],
  "glossary_candidates": [
    {
...
### runs/2026-09-25-79/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: raw.githubusercontent.com 으로 LLM06_ExcessiveAgency.md 열람. 세 원인(excessive functionality·permissions·autonomy), 'require a human to approve high-impact actions before they are taken', 최소 권한, complete mediation 모두 원문에 있음. 발행일 2024-11 은 문서 내 표기 미확인(2025판 기준). 단일 출처."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: tools.mdx(2025-06-18) 열람. 'there SHOULD always be a human in the loop with the ability to deny tool invocations', 민감 작업 확인 프롬프트, 호출 전 입력 표시, 감사용 도구 사용 기록이 원문에 있음. 기준일 2025-06-18(명세 판)."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: human-in-the-loop.mdx 열람. approve·edit·reject·respond 네 결정 유형, interrupt_on 설정, 체크포인터 필수가 원문에 있음. 발행일 미확인(확인일 2026-09-25 기준). 용어는 용어집 '사람 참여 루프'로 맞출 것."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ltl_safety README 열람 — lang2ltl 기반 자연어→LTL, precond 모듈, VirtualHome·Spot 실험 확인. '위반 이유 설명·재프롬프트'는 README 에 없고 원문 미열람 논문(ref-716) 요약에 기댄다. ref-716·ref-717 은 같은 저자라 독립 교차 아님."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "README(ref-719) 열람: 의미 그래프·규칙집·후보 계획 입력, Clearpath Jackal 확인. 수치는 README 에 없고 arXiv 초록(원문 미열람, 검색 결과) 기준 '92% → 2.5% 미만'으로 표기되어 브리프의 '3% 미만'과 다름 — 초록 문구대로 고칠 것. 저자는 Ravichandran, Robey, Kumar, Pappas, Hassani(RA-L 2026-02 채택 표기)로 확인됨. ref-718·ref-719 는 같은 저자 쌍."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 일치). arXiv 2503.06892 실재, 프롬프트 건전성·불변·전제·사후 조건 추론기로 지시·계획·배정 출력 검사, 유해 작업 수용 90.5% 감소(저자 보고) 확인. '621개' 수치는 검증 검색 스니펫에서 재확인하지 못함 — 미확인 표시 필요. 저자 미확인."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용(실행 2026-09-25-71 승인·게시된 주장, ref-417). 원문 미열람. ISO 13482 는 개인 돌봄 로봇 표준이라 물류 적용 미확인 문구를 본문에 유지할 것."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용(실행 2026-09-25-71, ref-711). 원문 미열람. 기존 게시 문장과 같은 주장이므로 기존 각주 ref-711 재사용."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용(ref-356, 기존 게시). 원문 미열람 표시 유지. 확인일 2026-09-25 기준."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 입력 원문 텍스트 data/source_texts/ref-031.txt 의 6.1.2(base cannot be changed, 관제는 베이스 실행을 가정, 취소 절차도 unreliable)와 6.1.4.3(수행 불가 동작 주문은 버퍼에 넣지 않고 INVALID_ORDER_ACTION·WARNING 보고)과 일치. 로봇 쪽 거절은 연계 대상으로 서술할 것. 브리프 sources[] 는 fetched_via 를 github_raw 로, self_check 는 inbox 로 적어 불일치(verification_note 기록)."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용(실행 2026-09-25-74 에서 원문 열람·게시된 주장, ref-656). 이번 실행 재열람 안 함."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: CHI2025 저장소 README 열람 — 248명, 위험도가 다른 6개 과제, 계획 품질·실행 단계 참여, 'users can easily mistrust the LLM agents with plans that seem plausible' 확인. README 표현은 신뢰의 잘못된 보정(mistrust)이므로 '잘못 신뢰'를 '그럴듯한 계획에 대한 신뢰가 잘못 보정됨'으로 옮길 것. '시뮬레이션 환경'은 README 에서 확인되지 않음. ref-722 논문 원문 미열람. 물류·로봇 조건 아님."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "사실 → 추정: arXiv 2604.04918(2026-04-06 공개) 실재는 검색 결과로 확인했으나(원문 미열람) '최종 공격 성공률이 상당히 남음', '보인 뒤 멈추게 하는 데 영향이 작음'은 검증 검색 스니펫에 나타나지 않고 브리프도 요약마다 다르다고 적음. 저자 미확인."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": true,
      "tag_decision": "강등",
      "note": "사실 → 추정: 검색 결과(원문 미열람)로 제14조 제4항 (b)호의 자동화 편향 인식 요구가 조문 게재본과 법학 논문(2502.10036) 양쪽에서 확인되어 교차 확인은 인정하나, 근거가 발행 기관(EU 공식 관보 EUR-Lex) 원문이 아닌 제3자 게재본이므로 규제 주장 규칙(4절 항목 12)에 따라 강등. 물류 배정 AI 의 고위험 해당 여부 미확인."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(국가법령정보센터 검색 결과 일치, 발행 기관 자료). 제34조 고영향 인공지능 사업자 책무에 위험관리방안·설명 방안·이용자 보호 방안·사람의 관리·감독이 포함됨을 검색 결과로 확인. 시행일 2026-01-22, 시행령 세부는 미확인. 고영향 해당 여부는 oq-105."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(ISCA 아카이브 검색 결과 일치). 저자는 Sagawa, H., Mitamura, T., & Nyberg, E. 로 확인 — 출처 기관 칸 정정 필요. INTERSPEECH 2004-ICSLP(2004-10). 결과 우열은 브리프가 넣지 않았으므로 그대로 둔다."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용(ref-418, 기존 게시). 벤더 문서, vendor_claim: true, [추정] 유지. WMS 제품 기능이라 ROP 연계 대상 사례로만 쓸 것."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용(ref-350, 기존 게시 주장과 같은 문장). 기존 각주 재사용."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 종합([추정], low). 구성 요소 각각은 f1~f11·f18 로 뒷받침되나 다섯 겹을 제시한 단일 출처는 없음을 본문에 밝힐 것. f13 강등 반영(f19 근거에서 f13 은 쓰지 않음 확인)."
    },
    {
      "finding_id": "f20",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지. VDA 5050 베이스 불변·취소 불신(ref-031 원문 확인), LangChain 체크포인터(ref-715 확인), 원자적 반영(ref-711)을 대응시킨 추론."
    },
    {
      "finding_id": "f21",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지. 근거 가운데 f13·f14 가 강등되었으므로 '보고·규정'을 사실처럼 단정하지 말고 추정 근거임을 밝힐 것. 근거 조건은 일상 비서·컴퓨터 사용 에이전트."
    },
    {
      "finding_id": "f22",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지. 영향이 큰 작업 예시(위험 구역 진입·적재 화물 취소·일괄 정지)는 출처가 없는 설명용 예시이므로 '이 위키가 든 예시'로 밝힐 것. Sagawa(f16) 결과 우열은 미확인."
    },
    {
      "finding_id": "f23",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지. 분류 원문 SCM 질문을 다루는 finding."
    },
    {
      "finding_id": "f24",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "설명용 가정 사례로 표시되어 있음. 수치 없음. 피킹 단계 시나리오."
    },
    {
      "finding_id": "f25",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 범위 관찰(부재 확인 아님) 문구 유지. 한국어 검색 3회."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "f7·f8·f9·f11·f17·f18 은 이미 게시된 주장(아이디어 2 5절, 업무 분해·배정 설계 초안 6절)과 같다 — 기존 각주 ref-417·ref-711·ref-356·ref-656·ref-418·ref-350 재사용",
      "개념 '사용자 확인' 제안은 초안 6절 '사용자 확인을 별도 개념으로 둘지, 배정의 속성(확인 여부)으로 둘지' 질문, 배정 속성 '확인 여부', 거부된 '검증 기록' 제안(실행 2026-09-25-71)과 겹친다",
      "아이디어 2 5절 머리의 '확인 절차(단계 4)는 아직 조사되지 않았다' 문장은 이번 q4-01 답과 충돌하므로 실행 기준을 밝혀 갱신해야 한다"
    ]
  },
  "terminology": {
    "ok": true,
...
### docs/references/index.md (요약: 이번 대상 페이지가 인용한 159건 / 전체 703건. 목록에 없는 출처는 새 id 로 적는다 — 같은 URL 이 이미 있으면 퍼블리셔가 기존 id 로 합친다)
...
### docs/open-questions.md (요약: 대상 영역 [1, 2, 5, 6, 8, 12, 13, 14, 16, 18, 19, 20, 23, 25, 26, 27] 에 걸린 90건 / 전체 115건)
...
### docs/standards/index.md (요약: 193개 — 이름 · 종류 · 발행 기관)
...
### runs/2026-09-25-79/docs_tree.txt
...
| [단계 4. 오해석 방지와 확인 절차](stage-4-misinterpretation-safeguards.md) | 대기 | 12 | 미충족 |
| [단계 5. 검증 방법과 가설 판정](stage-5-verification-and-hypotheses.md) | 대기 | 9 | 미충족 |
...
<!-- auto:page-status:start -->
> 초안 버전: v0.8 · 페이지 상태: published · 신뢰도: low · 페이지 버전: 10 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
sources: [ref-054, ref-055, ref-057, ref-058, ref-059, ref-061, ref-089, ref-090, ref-091, ref-093, ref-094, ref-095, ref-087, ref-164, ref-166, ref-167, ref-168, ref-169, ref-170, ref-171, ref-172, ref-174, ref-175, ref-176, ref-177, ref-178, ref-179, ref-180, ref-181, ref-242, ref-272, ref-275, ref-276, ref-277, ref-278, ref-279, ref-280, ref-350, ref-351, ref-352, ref-353, ref-354, ref-355, ref-356, ref-357, ref-358, ref-359, ref-360, ref-362, ref-015, ref-031, ref-125, ref-130, ref-228, ref-411, ref-413, ref-418, ref-111, ref-495, ref-230, ref-496, ref-500, ref-501, ref-502, ref-116, ref-504, ref-539, ref-540, ref-541, ref-542, ref-543, ref-544, ref-545, ref-546, ref-547, ref-548, ref-056, ref-404, ref-377, ref-592, ref-593, ref-594, ref-595, ref-596, ref-598, ref-611, ref-612, ref-615, ref-616, ref-376, ref-236, ref-417, ref-586, ref-674, ref-675, ref-676, ref-711, ref-677, ref-712, ref-678, ref-656, ref-657, ref-039, ref-658, ref-659, ref-660, ref-661, ref-662, ref-663, ref-664, ref-041,
  ref-126, ref-127, ref-680, ref-537, ref-681, ref-682, ref-683, ref-684, ref-373, ref-685]
confidence: low
last_run: 2026-09-25
---
...
<!-- auto:page-status:start -->
> 페이지 상태: published · 신뢰도: low · 페이지 버전: 12 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
원천: [질문 백로그](../tracks/nl-task-chatbot/question-backlog.md)([자연어 업무 지시 챗봇](../tracks/nl-task-chatbot/index.md) 트랙) · 열림 38건 · 답함 11건
...
```json
{
  ...
  "outline": [
    {
      ...
      "section": "2. 질문 목록",
      "budget_chars": 900,
      "summary": "단계 4 질문 표를 백로그와 맞춰 q4-01을 답함으로 바꾸고 앞 단계에서 보낸 q4-05~q4-12와 새 질문 q4-13·q4-14를 더한다."
    },
    {
      "path": "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md",
      "section": "3. 조사 결과",
      "budget_chars": 5200,
      "summary": "확인한 자료를 이 위키가 묶으면 해석 게이트·제약 게이트·사람 확인·검증 뒤 반영·디스패처와 로봇 쪽 마지막 거절의 다섯 겹 확인 절차가 근거가 가장 많은 것으로 보인다. [추정][^ref-356][^ref-716][^ref-713][^ref-711][^ref-656]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 1100,
      "summary": "사람 확인을 요구하는 설계 지침·도구 규격과 로봇 가드레일 연구는 확인되지만, 다섯 겹 확인 절차는 이 위키의 종합이며 물류 조건 평가가 없다. [추정][^ref-713][^ref-714]"
    },
    {
      "path": "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md",
      "section": "5. 이 단계가 낳은 후속 질문",
      "budget_chars": 700,
      "summary": "확인 방식의 영향도 기준, 현장 규칙의 시간 논리 표현, 승인 피로 측정 실험의 세 질문을 올린다."
    },
    {
      "path": "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md",
      "section": "6. 완료 조건 충족 현황",
      "budget_chars": 600,
      "summary": "실행 전 검증 단계·명령 권한·제한 운영 기준 세 행 모두 미충족이며 다음 단계로 전환하지 않는다."
    },
    {
      "path": "docs/tracks/nl-task-chatbot/task-model-draft.md",
      "section": "6. 미해결 모델링 질문",
      "budget_chars": 900,
      "summary": "개념 '사용자 확인' 제안은 반영하지 않고 q4-01 답과 속성 후보를 질문으로 덧붙인다. [추정][^ref-714][^ref-715]"
    },
    {
      "path": "docs/ideas/nl-task-chatbot.md",
      "section": "5. 구현 가설",
      "budget_chars": 2200,
      "summary": "오해석 방지 확인 절차 소절을 추가해 다섯 겹 확인 절차 가설과 확인 시점·사람 확인 한정·차등 확인을 싣는다. [추정][^ref-713][^ref-031]"
    }
  ],
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "q4-01 답함(다섯 겹 확인 절차 가설, 신뢰도 low), 질문 표를 백로그와 맞춤, 후속 질문 3건, 완료 조건 미충족·전환 아니오, 상태 줄 진행 중(상태 줄이 H2 밖이라 전체 content 로 보냄)"
    },
    {
      "path": "docs/tracks/nl-task-chatbot/task-model-draft.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "6절에 사용자 확인 질문의 q4-01 답 링크와 속성 후보를 질문으로 덧붙임(개념 추가 미반영, 초안 v0.8 유지)",
      "patches": [
        {
          "section": "6. 미해결 모델링 질문",
          "action": "append",
          ...
          "content": "\n- 위 '사용자 확인(승인)을 별도 개념으로 둘지, 배정의 속성(확인 여부)으로 둘지' 항목에 대해 실행 2026-09-25-79 에서 [q4-01 답](stage-4-misinterpretation-safeguards.md#q4-01)이 나왔다. 확인한 자료를 이 위키가 묶으면 사람 확인은 결정적 게이트 뒤에 두되 영향이 크거나 불확실한 작업에 한정하고, 배치(베이스 해제) 전에 끝내며 확인이 날 때까지 배치를 보류하는 중단점으로 두는 구성이 선택지로 보인다. [추정][^ref-714][^ref-715][^ref-031][^ref-711] MCP 도구 명세(2025-06-18판)는 서버 호출 전 도구 입력을 사용자에게 보여 주고 도구 사용 감사 기록을 남기도록 권고하고 [사실][^ref-714] LangChain 사람 참여 미들웨어는 승인·인자 수정·거부·직접 응답 네 결정을 둔다(확인일 2026-09-25 기준). [사실][^ref-715] 음성 대화 시스템 연구는 명시적·최종·암시적 확인 방식을 비교했다(우열 미확인). [사실][^ref-727] 이를 근거로 개념 '사용자 확인(Confirmation)'을 속성 후보(확인 대상: 해석 결과 / 배정 / 지시 변경, 확인 방식: 명시적 / 암시적, 응답: 승인 / 수정 / 거부, 확인자, 확인 시각, 보여 준 입력 요약)와 함께 두자는 제안(근거 f2·f3·f16·f19, 실행 2026-09-25-79)은 검증이 반영하지 않았다. 배정 속성 '확인 여부', 반영되지 않은 '검증 기록'(결정적 검사 기록)과의 경계가 정해지지 않았고, 결정 근거인 다섯 겹 확인 절차가 이 위키의 종합([추정])이기 때문이다. 속성 후보를 별도 개념으로 묶을지, 배정·지시 변경의 속성으로 나눠 둘지, '검증 기록'은 결정적 검사만 담고 사람 확인은 따로 두는 식으로 경계를 나눌지를 이 질문으로 남긴다(초안 v0.8 유지). — 관련: q4-01, q4-04, q4-12, q4-13\n\n[^ref-714]: Model Context Protocol (modelcontextprotocol GitHub), Specification 2025-06-18 — Server Features: Tools (docs/specification/2025-06-18/server/tools.mdx), 2025-06-18, https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/docs/specification/2025-06-18/server/tools.mdx, 접근일 2026-09-25\n[^ref-715]: LangChain (langchain-ai/docs GitHub), Human-in-the-loop — LangChain docs (src/oss/langchain/human-in-the-loop.mdx), 미확인, https://docs.langchain.com/oss/python/langchain/human-in-the-loop, 접근일 2026-09-25\n[^ref-727]: Sagawa, H., Mitamura, T., & Nyberg, E. (INTERSPEECH 2004-ICSLP), A comparison of confirmation styles for error handling in a speech dialog system, 2004-10, https://www.isca-archive.org/interspeech_2004/sagawa04_interspeech.pdf, 접근일 2026-09-25 (원문 미열람)\n"
        }
      ]
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "5절에 '오해석 방지 확인 절차' 소절 추가(q4-01, 실행 2026-09-25-79), 머리 문장의 '확인 절차 미조사' 서술을 실행 기준과 함께 갱신, 명령 권한·제한 운영 기준 미조사 명시",
      "patches": [
        {
          "section": "5. 구현 가설",
          "action": "append",
          ...
          "content": "\n### 오해석 방지 확인 절차\n\n이 절 머리의 '확인 절차(단계 4)는 아직 조사되지 않았다'는 서술은 실행 2026-09-25-74 기준이며, 실행 2026-09-25-79 에서 [단계 4. 오해석 방지와 확인 절차](../tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md#q4-01)의 q4-01 에 답했다. 이 소절은 그 가운데 실행 전 검증 단계만 다루며, 명령 권한(q4-03)과 제한 운영 기준(q4-04)은 아직 조사하지 않았다.\n\n확인한 자료를 이 위키가 묶으면, 확인 절차는 해석 게이트, 제약 게이트, 사람 확인, 검증 뒤 반영, 디스패처·로봇 쪽 마지막 거절의 다섯 겹으로 두는 구성이 근거가 가장 많은 것으로 보인다. 다섯 겹을 한 번에 제시한 단일 출처는 찾지 못했고, 근거 조건이 가정·실험실 로봇, 소프트웨어 에이전트, 로봇 관제 규격이어서 신뢰도가 낮다. [추정][^ref-356][^ref-350][^ref-716][^ref-718][^ref-720][^ref-417][^ref-713][^ref-714][^ref-715][^ref-711][^ref-656][^ref-031]\n\n아래 표는 위 근거를 이 위키가 대응시켜 구성한 가설이다. [추정][^ref-356][^ref-716][^ref-713][^ref-711][^ref-656][^ref-031]\n\n| 겹 | 검사하는 것 | 맡는 쪽 | 근거 사례 |\n|---|---|---|---|\n| 해석 게이트 | 필수 슬롯·형식, 불확실하면 되묻기 | 결정적 규칙·불확실성 기준 | Rasa 폼, KnowNo |\n| 제약 게이트 | 해석 결과·계획·배정을 안전 규칙·권한·능력 제약과 대조 | 결정적 검사 | Safety Chip, RoboGuard, SafePlan, SafeGate |\n| 사람 확인 | 영향이 크거나 불확실할 때 해석 요약을 승인·수정·거부 | 사람 | OWASP 과도한 에이전시 항목, MCP 도구 명세, LangChain 사람 참여 미들웨어 |\n| 검증 뒤 반영 | 검증 기록과 함께 상태 반영 | 결정적 상태 관리 | Tang 외 관리형 블랙보드 |\n| 마지막 거절 | 무입찰·수행 불가 동작 | 디스패처, 로봇(로봇 쪽은 연계 대상) | Open-RMF 디스패처, VDA 5050 |\n\n- **사람 확인을 요구하는 규격**: OWASP LLM 애플리케이션 Top 10(2025판)의 과도한 에이전시 항목은 영향이 큰 행동 전 사람 승인, 최소 권한, 완전한 중재를 대응으로 든다. [사실][^ref-713] MCP 명세(2025-06-18판) 도구 절은 도구 호출을 거부할 수 있는 사람이 루프에 있어야 한다고 적고 호출 전 입력 표시·감사 기록을 권고한다. [사실][^ref-714] LangChain 의 [사람 참여 루프(Human-in-the-Loop, HITL)](../glossary/human-in-the-loop.md) 미들웨어는 승인·인자 수정·거부·직접 응답 결정을 둔다. [사실][^ref-715]\n- **로봇 가드레일**: Safety Chip 은 자연어 제약을 선형 시간 논리 오토마톤으로 두고 불안전 동작을 걸러내며 [사실][^ref-716][^ref-717] RoboGuard 저자들은 탈옥 공격 조건에서 불안전 계획 실행을 92% → 2.5% 미만(저자 보고값, 원문 미열람)으로 줄였다고 보고했다. [사실][^ref-718][^ref-719] SafePlan 은 배정 결과까지 검사하며 저자들은 유해 작업 수용을 90.5% 줄였다고 보고했다(저자 보고값, 원문 미열람). [사실][^ref-720] 로봇 쪽 안전 기능 자체는 연계 대상이고 ROP 는 작업·배정 수준의 제약 대조만 맡는 것으로 본다. [추정][^ref-716][^ref-718]\n- **확인 시점**: VDA 5050 에서 풀어 준 베이스는 바꿀 수 없고 취소도 신뢰할 수 없으므로, 해석 확인은 배정 계산 전에, 배정 결과 확인은 배치(베이스 해제) 전에 끝나야 하며, 사람 확인이 필요한 작업은 확인이 날 때까지 배치를 보류하는 중단점으로 두는 것이 선택지로 보인다. [추정][^ref-031][^ref-715][^ref-711]\n- **사람 확인의 범위**: 그럴듯해 보이는 계획에 대해 사용자의 신뢰가 잘못 보정되기 쉬웠다는 보고(일상 비서 조건) [사실][^ref-721][^ref-722]와, [추정] 수준인 감독 전략 비교 요약과 EU AI Act 의 자동화 편향 인식 요구(제3자 조문 게재본 기준)를 함께 보면, 결정적 게이트를 먼저 두고 사람 확인은 영향이 큰 작업에 한정하는 편이 선택지로 보인다. [추정][^ref-723][^ref-724][^ref-725][^ref-713]\n- **차등 확인**: 영향이 큰 작업은 명시적 확인으로, 일상 운반 지시는 해석 결과를 응답에 되풀이해 보여 주는 암시적 확인으로 두는 차등 구성이 선택지로 보인다. 영향이 큰 작업의 예(위험 구역 진입·적재 화물 취소·일괄 정지)는 이 위키가 든 설명용 예시(출처 없음)다. [추정][^ref-727][^ref-713][^ref-714][^ref-350]\n\n이번 실행에서 제안된 개념 '사용자 확인'은 검증이 반영하지 않아 [업무 분해·배정 설계 초안](../tracks/nl-task-chatbot/task-model-draft.md) 6절의 질문으로 남았다(초안 v0.8 유지).\n\n[^ref-713]: OWASP Top 10 for LLM Applications 프로젝트 (OWASP GitHub), LLM06:2025 Excessive Agency (2_0_vulns/LLM06_ExcessiveAgency.md), 2024-11, https://github.com/OWASP/www-project-top-10-for-large-language-model-applications/blob/main/2_0_vulns/LLM06_ExcessiveAgency.md, 접근일 2026-09-25\n[^ref-714]: Model Context Protocol (modelcontextprotocol GitHub), Specification 2025-06-18 — Server Features: Tools (docs/specification/2025-06-18/server/tools.mdx), 2025-06-18, https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/docs/specification/2025-06-18/server/tools.mdx, 접근일 2026-09-25\n[^ref-715]: LangChain (langchain-ai/docs GitHub), Human-in-the-loop — LangChain docs (src/oss/langchain/human-in-the-loop.mdx), 미확인, https://docs.langchain.com/oss/python/langchain/human-in-the-loop, 접근일 2026-09-25\n[^ref-716]: Yang, Z. 외(Brown University H2R Lab), Plug in the Safety Chip: Enforcing Constraints for LLM-driven Robot Agents, 2023-09, https://arxiv.org/abs/2309.09919, 접근일 2026-09-25 (원문 미열람)\n[^ref-717]: YzyLmc (Safety Chip 공식 저장소), ltl_safety — README (Plug in the Safety Chip), 미확인, https://github.com/YzyLmc/ltl_safety, 접근일 2026-09-25\n[^ref-718]: Ravichandran, Z., Robey, A., Kumar, V., Pappas, G. J., & Hassani, H.(IEEE RA-L 2026 채택 표기), Safety Guardrails for LLM-Enabled Robots, 2025-03, https://arxiv.org/abs/2503.07885, 접근일 2026-09-25 (원문 미열람)\n[^ref-719]: KumarRobotics (RoboGuard 공식 저장소), RoboGuard — Safety guardrails for LLM-enabled robots (GitHub README), 미확인, https://github.com/KumarRobotics/RoboGuard, 접근일 2026-09-25\n[^ref-720]: SafePlan 저자(arXiv 2503.06892, 저자 미확인), SafePlan: Leveraging Formal Logic and Chain-of-Thought Reasoning for Enhanced Safety in LLM-based Robotic Task Planning, 2025-03, https://arxiv.org/abs/2503.06892, 접근일 2026-09-25 (원문 미열람)\n[^ref-721]: RichardHGL (CHI 2025 Plan-then-Execute 공식 저장소), CHI2025_Plan-then-Execute_LLMAgent — README, 미확인, https://github.com/RichardHGL/CHI2025_Plan-then-Execute_LLMAgent, 접근일 2026-09-25\n[^ref-722]: He, G., Demartini, G., & Gadiraju, U., Plan-Then-Execute: An Empirical Study of User Trust and Team Performance When Using LLM Agents As A Daily Assistant, 2025-04, https://dl.acm.org/doi/10.1145/3706598.3713218, 접근일 2026-09-25 (원문 미열람)\n[^ref-723]: arXiv 2604.04918 저자(미확인), Comparing Human Oversight Strategies for Computer-Use Agents, 2026-04, https://arxiv.org/abs/2604.04918, 접근일 2026-09-25 (원문 미열람)\n[^ref-724]: Future of Life Institute (artificialintelligenceact.eu, EU 규정 2024/1689 조문 게재본), Article 14: Human Oversight — EU Artificial Intelligence Act, 2024, https://artificialintelligenceact.eu/article/14/, 접근일 2026-09-25 (원문 미열람)\n[^ref-725]: arXiv 2502.10036 저자(미확인), Automation Bias in the AI Act: On the Legal Implications of Attempting to De-Bias Human Oversight of AI, 2025-02, https://arxiv.org/abs/2502.10036, 접근일 2026-09-25 (원문 미열람)\n[^ref-727]: Sagawa, H., Mitamura, T., & Nyberg, E. (INTERSPEECH 2004-ICSLP), A comparison of confirmation styles for error handling in a speech dialog system, 2004-10, https://www.isca-archive.org/interspeech_2004/sagawa04_interspeech.pdf, 접근일 2026-09-25 (원문 미열람)\n"
        }
      ]
    }
  ],
  "changelog_entry": "2026-09-25 | 자연어 업무 지시 챗봇 단계 4 | q4-01 답함(다섯 겹 확인 절차 가설, 신뢰도 low), 후속 질문 3건, 초안 v0.8 유지(개념 '사용자 확인'은 6절 질문), 아이디어 2 5절에 오해석 방지 확인 절차 소절 추가 | run 2026-09-25-79",
  "index_updates": {
    "home_recent": "2026-09-25 — 자연어 업무 지시 챗봇 단계 4. 오해석 방지와 확인 절차: q4-01 답함(해석 게이트·제약 게이트·사람 확인·검증 뒤 반영·마지막 거절의 다섯 겹 확인 절차 가설, 신뢰도 low), 후속 질문 3건",
    "category_recent": "2026-09-25 — 자연어 업무 지시 챗봇 단계 4: 13. 작업 배정 — MRTA 에 LLM 배정 출력의 실행 전 검사와 디스패처·로봇 쪽 마지막 거절을 반영하도록 제안(q4-01)",
    "area_recent": "2026-09-25 — 자연어 업무 지시 챗봇 단계 4: 배정 출력의 실행 전 검사(SafePlan)·무입찰 FailedToAssign·수행 불가 동작 거절을 6. 대표 접근법과 기술에 반영하도록 제안(실행 2026-09-25-79)"
  },
  "glossary_updates": [
    {
      "action": "new",
      ...
      "term_en": "Excessive Agency",
      "definition": "LLM 기반 시스템이 필요 이상의 기능·권한·자율성을 가져 잘못되거나 조작된 출력이 해로운 행동으로 이어지는 위험으로, OWASP LLM Top 10(2025)의 한 항목이다.",
      "description": "OWASP 문서는 원인을 과도한 기능·과도한 권한·과도한 자율성으로 나누고, 영향이 큰 행동 전 사람 승인, 최소 권한, 완전한 중재를 대응으로 든다.",
      "related_areas": [
        27,
        26,
        18
    ...
    },
    {
      ...
      "term_en": "Automation Bias",
      "definition": "사람이 자동화 시스템의 출력이나 권고를 충분히 따져 보지 않고 과도하게 믿고 따르는 경향으로, 사람 승인 절차를 형식적 확인으로 만들 수 있다.",
      "description": "EU AI Act 제14조 제4항 (b)호가 고위험 AI 감독자가 이 경향을 인식할 수 있게 설계하도록 요구한다는 내용은 EU 공식 관보(EUR-Lex) 원문이 아닌 제3자 조문 게재본·법학 논문 기준이어서 [추정]으로 둔다.",
      "related_areas": [
        18,
        27,
        25
    ...
    {
      "action": "new",
      "slug": "explicit-implicit-confirmation",
      "term_ko": "명시적 확인·암시적 확인",
      "term_en": "Explicit / Implicit Confirmation",
      "definition": "대화 시스템이 이해한 내용을 사용자에게 직접 물어 승인받는 방식(명시적)과, 다음 응답 속에 이해한 내용을 되풀이해 보여 주고 사용자가 고치지 않으면 진행하는 방식(암시적)이다.",
      "description": "음성 대화 시스템 연구(Sagawa 외, INTERSPEECH 2004)는 명시적·최종·암시적 확인을 비교했으며, 비교 결과의 우열은 이 위키에서 확인하지 못했다.",
      "related_areas": [
        18,
        27
      ],
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 신뢰 기반 LLM 으로 안전 규칙을 환경에 접지해 시간 논리 명세를 만들고 제어 합성으로 불안전 계획을 해소하는 2단계 가드레일(RoboGuard). 초록 기준 불안전 계획 실행 92% → 2.5% 미만(저자 보고).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 지시·계획·팀 구성·배정 출력을 불변·전제·사후 조건 추론기로 검사하는 틀. 벤치마크 규모(621개)는 검증 미재확인.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "계획 후 실행 LLM 에이전트 사용자 연구(248명, 6개 과제)의 저장소. 그럴듯한 계획에 대한 신뢰의 잘못된 보정을 요약한다.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 행동 확인·위험 기반 승인·계획 수준 감독 전략을 비교한 사용자 연구. 주장 내용과 조건별 수치는 검증에서 확인되지 않아 [추정]으로 인용.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md",
        "docs/ideas/nl-task-chatbot.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 음성 대화 시스템에서 명시적·최종·암시적 확인 방식을 비교한 연구. 결과 우열 미확인.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md",
        "docs/ideas/nl-task-chatbot.md",
  ...
  "flow_matrix_updates": [
    {
      "step": "피킹",
      "item": "시작 조건",
      "link": "docs/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md#q4-01",
      "title": "단계 4. 오해석 방지와 확인 절차"
    },
    {
      "step": "피킹",
  ...
  "fixes_applied": [
    ...
    "f3·용어 — LangChain 문장을 [사람 참여 루프(Human-in-the-Loop, HITL)](../../glossary/human-in-the-loop.md) 링크로, MCP 를 용어집 '모델 컨텍스트 프로토콜' 링크로 맞췄다(아이디어 페이지는 상대 경로를 맞춤).",
  ...
  "standards_updates": [
    {
      "name": "OWASP Top 10 for LLM Applications 2025 (LLM06 Excessive Agency)",
      "kind": "프레임워크",
      "org": "OWASP",
      "url": "https://github.com/OWASP/www-project-top-10-for-large-language-model-applications/blob/main/2_0_vulns/LLM06_ExcessiveAgency.md",
      "related_areas": [
      ...
      "summary": "LLM 애플리케이션 보안 위험 목록. 과도한 에이전시 항목은 과도한 기능·권한·자율성을 원인으로 들고 영향이 큰 행동 전 사람 승인·최소 권한·완전한 중재를 대응으로 제시한다.",
      "ref_id": "ref-713"
    },
    {
      "name": "Model Context Protocol 명세 2025-06-18 (Server Features: Tools)",
      "kind": "표준",
      "org": "Model Context Protocol (modelcontextprotocol GitHub)",
      "url": "https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/docs/specification/2025-06-18/server/tools.mdx",
      "related_areas": [
      ...
      "summary": "LLM 이 도구를 호출하는 프로토콜의 도구 명세. 호출을 거부할 수 있는 사람 참여, 민감 작업 확인, 호출 전 입력 표시, 접근 통제·감사 기록을 권고한다.",
      "ref_id": "ref-714"
    },
    {
      "name": "LangChain Human-in-the-loop 미들웨어",
      "kind": "오픈소스",
      "org": "LangChain",
      "url": "https://docs.langchain.com/oss/python/langchain/human-in-the-loop",
      "related_areas": [
      ...
      "summary": "설정한 도구 호출에서 에이전트 실행을 멈추고 사람이 승인·인자 수정·거부·직접 응답으로 결정하게 하는 미들웨어. 체크포인터로 중단 상태를 보존한다.",
      "ref_id": "ref-715"
    }
  ],
  "track_updates": {
  ...
  "area_reflection_proposals": [
    ...
    },
    {
      "area_no": 18,
      "section": "6. 대표 접근법과 기술",
      "summary": "승인·인자 수정·거부·직접 응답 결정을 두는 사람 참여 미들웨어(LangChain)와 명시적·암시적 확인 방식(Sagawa 외 2004), 계획 승인 사용자 연구의 신뢰 보정 문제(CHI 2025)와 [추정] 수준의 감독 전략 비교를 근거로 사람 확인을 영향이 큰 작업에 한정하는 차등 구성([추정])을 소개한다."
    },
    {
      "area_no": 26,
      "section": "6. 대표 접근법과 기술",
      "summary": "OWASP LLM Top 10(2025)의 과도한 에이전시 원인(기능·권한·자율성)과 최소 권한·완전한 중재, MCP 도구 명세(2025-06-18)의 입력 검증·접근 통제·감사 기록 권고를 채팅 지시 명령 권한(q4-03)의 출발점으로 소개한다."
    },
    {
      "area_no": 27,
      "section": "6. 대표 접근법과 기술",
      "summary": "LLM 로봇 계획·배정 출력의 형식 논리 가드레일(Safety Chip, RoboGuard 92% → 2.5% 미만 저자 보고, SafePlan, SafeGate)을 소개하고 적용 대상 13. 작업 배정 — MRTA·18. 사람–로봇 협업·운영 인터페이스에 연결한다. 로봇 쪽 안전 기능은 연계 대상으로 짧게 둔다."
    },
    {
      "area_no": 27,
      "section": "8. 대표 연구와 자료",
      "summary": "사람의 관리·감독 규정으로 EU AI Act 제14조 제4항 (b)호의 자동화 편향 인식 요구([추정], 공식 관보 원문 미확인)와 한국 인공지능기본법 제34조(고영향 인공지능 사업자의 사람의 관리·감독 조치)를 자료로 추가한다. 물류 배정 AI 의 해당 여부는 oq-105."
    }
  ]
}
```

### runs/2026-09-25-79/pages/tracks/nl-task-chatbot/stage-4-misinterpretation-safeguards.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-350, ref-356, ref-417, ref-418, ref-656, ref-711, ref-713, ref-714, ref-715, ref-716, ref-717, ref-718, ref-719, ref-720, ref-721, ref-722, ref-723, ref-724, ref-725, ref-620, ref-727]
last_run: 2026-09-25
version: 2
---
...
이 단계의 시작 질문 4개(q4-01~q4-04)와 앞 단계 실행에서 이 단계로 보낸 질문(q4-05~q4-12), 이번 실행의 후속 질문(q4-13·q4-14)이다. q4-01은 사용자 요청의 시작 질문 문구 그대로이고, q4-02~q4-04는 구축자가 이 단계의 밝힐 것에서 정한 시작 질문이다. [가정] 상태와 답 위치는 [질문 백로그](question-backlog.md)와 일치시키며, 백로그의 "조사 중"은 이 표에서 "열림"으로 표시하고 "폐기"는 표에서 빼고 백로그에만 남긴다. q4-09와 q4-10은 같은 질문이 두 번 등록된 것이어서 백로그 정리가 필요하다.
...
| q4-03 | 채팅 사용자별 명령 권한(어느 로봇·구역·작업까지 지시할 수 있는가)과 지시·확인의 감사 기록은 어떻게 두는가? | 열림 | 사용자 | | |
| q4-04 | 해석의 불확실성이 클 때 되묻기·사람 승인·실행 보류 같은 제한 운영으로 넘기는 기준은 무엇인가? | 열림 | 사용자 | | |
| q4-05 | LLM 이 허용 동작 목록에 없는 동작이나 존재하지 않는 대상을 분해 결과에 넣을 때, 기존 장치(허용 동작 대응·assertion·계획기 검사)는 각각 어떤 오류를 걸러내고 무엇을 놓치는가? | 열림 | f14, 실행 2026-09-25-04 | | |
| q4-06 | 동작마다 현장에서 확인받는 방식과 해석 결과를 배정 전에 요약해 확인받는 방식을 함께 둘 때 각각 어떤 오류를 잡고 확인 부담은 얼마나 늘어나는가? | 열림 | f9, 실행 2026-09-25-26 | | |
| q4-07 | 필수 슬롯 누락은 규칙으로, 모호성은 불확실성 추정으로 나눠 판단할 때 모호성 탐지의 낮은 구분 성능이 물류 지시에서도 나타나는가? | 열림 | f11, 실행 2026-09-25-30 | | |
| q4-08 | LLM 이 루프 밖에서 만든 배정·스케줄 규칙을 시뮬레이션·샌드박스에서 검증한 뒤 운영 정책으로 반영할 때 어떤 검증 기준을 통과해야 하는가? | 열림 | f13, 실행 2026-09-25-66 | | |
| q4-09 | 채팅 LLM 에 노출할 도구를 상위 도구로 한정할 때 어떤 도구 목록과 사용자별 권한을 두어야 능력 질의·배정·검증 게이트를 우회하지 않는가? | 열림 | f20, 실행 2026-09-25-71 | | |
| q4-10 | (q4-09와 같은 질문의 중복 등록) | 열림 | f20, 실행 2026-09-25-71 | | |
| q4-11 | 배정 실패 원인마다 챗봇이 제시할 완화 선택지를 어떤 목록으로 두고, 사람 처리 전환이나 기한 완화는 누가 승인하는가? | 열림 | f22, 실행 2026-09-25-74 | | |
| q4-12 | 화물을 이미 실었거나 옮긴 작업을 채팅으로 취소할 때 되돌림 보상 작업의 생성·실행을 누가 승인하고, 지시 변경 요약 확인은 어떤 형식으로 보여 주는가? | 열림 | f23, 실행 2026-09-25-77 | | |
| q4-13 | 물류 지시에서 명시적 확인이 필요한 영향이 큰 작업과 암시적 확인으로 충분한 일상 작업을 어떤 기준으로 가르고, 그 기준 목록은 누가 정하고 갱신하는가? | 열림 | f22, 실행 2026-09-25-79 | | |
| q4-14 | 현장 안전·권한 규칙(금지 구역, 시간대 제한, 적재 제한)을 시간 논리 제약으로 옮겨 작업·배정 수준에서 대조하려면 규칙을 공간 그래프·로봇 기능 온톨로지의 어떤 개념으로 표현해야 하는가? | 열림 | f5, 실행 2026-09-25-79 | | |
...
- OWASP LLM 애플리케이션 Top 10(2025판, 문서 기준 2024-11)의 과도한 에이전시(Excessive Agency) 항목은 원인을 과도한 기능·과도한 권한·과도한 자율성 셋으로 나누고, 대응으로 영향이 큰 행동 전 사람 승인, 최소 권한, 완전한 중재(complete mediation)를 든다. [사실][^ref-713]
- [모델 컨텍스트 프로토콜](../../glossary/model-context-protocol.md)(Model Context Protocol, MCP) 명세(2025-06-18판) 도구 절은 도구 호출을 거부할 수 있는 사람이 항상 루프에 있어야 한다고(SHOULD) 적고, 클라이언트가 민감한 작업에 사용자 확인을 묻고 서버 호출 전 도구 입력을 사용자에게 보여 주며 도구 사용 감사 기록을 남기도록 권고한다. [사실][^ref-714]
- LangChain 의 [사람 참여 루프(Human-in-the-Loop, HITL)](../../glossary/human-in-the-loop.md) 미들웨어는 설정한 도구 호출에서 에이전트 실행을 멈추고 사람이 승인·인자 수정·거부(피드백 포함)·직접 응답 가운데 하나로 결정하게 하며, 중단 상태를 보존하려면 체크포인터가 필요하다(발행일 미확인, 확인일 2026-09-25 기준). [사실][^ref-715]

#### 로봇 계획·배정 출력의 가드레일

- Safety Chip(Yang 외, ICRA 2024)은 자연어로 준 금지 제약을 선형 시간 논리(LTL) 식으로 옮겨 오토마톤으로 두고, LLM 에이전트의 결정을 감시해 안전하지 않은 동작을 걸러내는 질의 가능한 제약 모듈이며 VirtualHome 과 실제 로봇(Spot)에서 실험했다. 위반 이유를 설명해 재프롬프트에 쓰는 부분은 원문 미열람 논문 요약 기준이고, 논문과 저장소는 같은 저자라 독립 교차가 아니다. [사실][^ref-716][^ref-717]
- RoboGuard 는 미리 정한 안전 규칙을 신뢰 기반 LLM 이 로봇 환경의 의미 그래프에 접지해 시간 논리 안전 명세를 만들고, 후보 계획이 명세와 충돌하면 시간 논리 제어 합성으로 해소하는 2단계 가드레일이다. 저자들은 탈옥 공격 조건에서 안전하지 않은 계획 실행을 92% → 2.5% 미만(저자 보고값, 원문 미열람)으로 줄였다고 보고했다. [사실][^ref-718][^ref-719]
- SafePlan 은 LLM 이 여러 로봇·사람에게 작업 계획·팀 구성·작업 배정을 만드는 시스템에서 프롬프트 건전성 추론기와 불변·전제·사후 조건 추론기로 지시·계획·배정 결과의 안전성을 검사하며, 저자들은 전문가가 만든 지시 벤치마크(621개, 검증 미재확인)에서 유해 작업 수용을 90.5% 줄였다고 보고했다(저자 보고값, 원문 미열람). [사실][^ref-720]
...
- 이 연구들은 가정·실험실 로봇 조건이다. 보호 정지 같은 로봇 쪽 안전 기능과 로봇 내부 안전 모듈은 분류 원문 9장의 로봇 자체 지능·제어 경계에 속하는 연계 대상이며, ROP 는 작업·배정 수준의 제약 대조만 맡는 것으로 본다. [추정][^ref-716][^ref-718][^ref-417]
...
- He·Demartini·Gadiraju(CHI 2025)는 LLM 에이전트를 계획 후 실행 방식으로 쓰는 일상 비서 과제(위험도가 다른 6개 과제, 참가자 248명)에서 사용자 참여를 조사해, 계획 품질이 높고 실행 단계 사용자 참여가 있을 때는 잘 작동하지만 그럴듯해 보이는 계획에 대해 사용자의 신뢰가 잘못 보정되기 쉬웠다고 보고했다. 물류·로봇 조건은 아니다. [사실][^ref-721][^ref-722]
- 컴퓨터 사용 에이전트의 사람 감독 전략(행동마다 확인, 위험 기반 선택적 승인, 계획 수준 감독)을 비교한 연구(arXiv 2604.04918)는 모든 전략에서 최종 공격 성공률이 상당히 남았고, 감독 전략이 문제 행동을 보이게 하는 데는 영향을 주었지만 보인 뒤 멈추게 하는 데는 영향이 작았다고 보고한 것으로 보인다. 저자 미확인, 원문 미열람, 조건별 수치 미확인이다. [추정][^ref-723]
- EU AI Act 제14조 제4항 (b)호는 고위험 AI 시스템을 감독하는 사람이 시스템 출력에 자동으로·과도하게 의존하는 경향(자동화 편향)을 계속 인식할 수 있게 설계하도록 요구하는 것으로 보인다. EU 공식 관보(EUR-Lex) 원문 미확인, 제3자 조문 게재본·법학 논문 기준이며, 물류 로봇 배정 AI 의 고위험 해당 여부도 미확인이다. [추정][^ref-724][^ref-725]
...
- Sagawa 외(INTERSPEECH 2004)는 음성 대화 시스템의 오류 처리에서 명시적 확인, 최종 확인, 암시적 확인 세 방식을 비교해 사용자 만족과 효율을 평가했다. 결과 우열과 수치는 미확인이다. [사실][^ref-727]

#### 확인 시점·사람 확인의 범위·차등 확인 (이 위키의 종합)

- VDA 5050 에서 풀어 준 베이스는 바꿀 수 없고 취소도 신뢰할 수 없으므로, 해석 확인은 배정 계산 전에, 배정 결과 확인은 배치(베이스 해제) 전에 끝나야 하며, 사람 확인이 필요한 작업은 확인이 날 때까지 배치를 보류하는 중단점으로 두는 것이 선택지로 보인다. [추정][^ref-031][^ref-715][^ref-711]
- 사람 승인만으로 오해석을 걸러내기 어려울 수 있다는 근거는 계획에 대한 신뢰 보정 문제(사실)와, [추정]으로 강등된 감독 전략 비교 요약·자동화 편향 인식 요구 조문 게재본이다. 이를 함께 보면 결정적 게이트를 먼저 두고 사람 확인은 영향이 큰 작업에 한정해 해석 결과와 결정적 검사 결과의 차이를 드러내는 형태로 두는 편이 선택지로 보이나, 근거가 일상 비서·컴퓨터 사용 에이전트 조건이고 물류 관제 요원의 승인 행동을 잰 자료는 찾지 못했다. [추정][^ref-721][^ref-722][^ref-723][^ref-724][^ref-725][^ref-713]
- 확인 방식은 작업의 영향도와 해석 불확실성에 따라 나누어, 영향이 큰 작업은 명시적 확인(승인 전 대기)으로, 일상적 운반 지시는 응답에 해석 결과를 되풀이해 보여 주는 암시적 확인으로 두는 차등 구성이 선택지로 보인다. 영향이 큰 작업의 예(위험 구역 진입·적재 화물 취소·일괄 정지)는 이 위키가 든 설명용 예시(출처 없음)다. [추정][^ref-727][^ref-713][^ref-714][^ref-350]

분류 원문의 이 영역 질문은 다음과 같다.

> 가장 가까운 로봇에 맡기는 것이 전체적으로도 유리한가? [분류원문]

이와 관련해, 확인 화면이 사용자에게 로봇 선택 자체를 고르게 하기보다 해석한 업무(대상·장소·기한)와 디스패처가 쓴 배정 기준(가장 빨리 끝남 등)을 보여 주면, 사람은 해석 오류를 확인하고 배정의 전체 기준 일관성은 결정적 배정기가 지키는 분담이 가능해 보인다. [추정][^ref-656][^ref-715][^ref-714]
...
이번에 확인한 확인·가드레일 근거의 평가 환경은 가정 시뮬레이터·실험실 로봇, 일상 비서·컴퓨터 사용 에이전트, 음성 대화 시스템이었고, 물류 창고 로봇에 채팅으로 준 지시의 확인 절차를 평가한 연구와 국내 사례는 한국어 검색을 포함한 검색 범위에서 찾지 못했다(부재의 확인은 아님). [추정][^ref-716][^ref-718][^ref-720][^ref-722][^ref-723]
...
| 항목 | 내용 |
|---|---|
| 시작 조건 | 관리자가 채팅으로 '피킹 끝난 토트를 10시 전까지 2번 도크로'라고 지시한다(설명용 가정 사례). 해석 게이트가 대상·장소·기한 슬롯을 검사해 빠진 값을 되묻는다. [추정][^ref-356][^ref-714] |
| 작업 대상 | 피킹이 끝난 토트(설명용 가정) |
| 수행 자원 | 챗봇은 해석을, 결정적 게이트와 디스패처는 검사와 배정을, 사람은 영향이 큰 작업의 승인을 맡는 분담이 가능해 보인다. [추정][^ref-656][^ref-715][^ref-714] |
| 제약 | 제약 게이트가 지시자의 구역·작업 권한과 도크 도달 가능성을 대조하고, 확인은 베이스 해제 전에 끝낸다. [추정][^ref-714][^ref-711][^ref-031] |
| 완료·인계 | 해당 없음 |
| 예외·성과 | 어떤 플릿도 입찰하지 않으면 배정 상태가 FailedToAssign 으로 기록되고 작업은 수행되지 않는다. [사실][^ref-656] 사람 승인만으로는 오해석을 걸러내기 어려울 수 있다. [추정][^ref-721][^ref-723] |

다음은 설명을 위한 가상의 시나리오이다. 일상 운반이면 챗봇은 해석 요약을 응답에 보여 주고 바로 반영하며, 지시가 다른 사람의 진행 작업 취소를 포함하면 명시적 승인을 받을 때까지 배치를 보류한다. [추정][^ref-356][^ref-714][^ref-711][^ref-031] 지어낸 현장 수치는 쓰지 않았다.

## 4. 결론과 남은 불확실성

**결론**
- 에이전트 설계 지침과 도구 규격(OWASP 과도한 에이전시 항목, MCP 도구 명세, LangChain 사람 참여 미들웨어)은 영향이 큰 행동 전 사람 승인·호출 전 입력 표시·승인·수정·거부 결정을 요구하거나 제공한다. [사실][^ref-713][^ref-714][^ref-715]
- LLM 로봇 계획·배정 출력을 형식 논리·결정적 판정으로 거르는 가드레일 연구(Safety Chip, RoboGuard, SafePlan, SafeGate)가 있으며, 그중 SafePlan 은 배정 결과까지 검사한다. [사실][^ref-716][^ref-718][^ref-720][^ref-417]
- 확인 절차는 다섯 겹(해석 게이트, 제약 게이트, 사람 확인, 검증 뒤 반영, 마지막 거절)으로 두고, 확인은 배치(베이스 해제) 전에 끝내며, 사람 확인은 영향이 크거나 불확실한 작업에 한정하는 구성이 선택지로 보인다(이 위키의 종합, 신뢰도 low). [추정][^ref-356][^ref-713][^ref-711][^ref-031][^ref-656]
...
### runs/2026-09-25-79/pages/tracks/nl-task-chatbot/task-model-draft.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-054, ref-055, ref-059, ref-089, ref-090, ref-091, ref-093, ref-125, ref-130, ref-166, ref-167, ref-169, ref-181, ref-242, ref-350, ref-352, ref-356, ref-357, ref-358, ref-359, ref-361, ref-411, ref-412, ref-413, ref-414, ref-111, ref-495, ref-230, ref-496, ref-501, ref-502, ref-504, ref-404, ref-377, ref-596, ref-597, ref-611, ref-612, ref-592, ref-594, ref-595, ref-598, ref-615, ref-616, ref-376, ref-236, ref-674, ref-711, ref-656, ref-039, ref-126, ref-681, ref-683, ref-684, ref-373, ref-685, ref-677, ref-714, ref-715, ref-727]
last_run: 2026-09-25
version: 11
---
...
### runs/2026-09-25-79/pages/ideas/nl-task-chatbot.md
...
version: 13
sources: [ref-054, ref-055, ref-057, ref-058, ref-059, ref-061, ref-089, ref-090, ref-091, ref-093, ref-094, ref-095, ref-087, ref-164, ref-166, ref-167, ref-168, ref-169, ref-170, ref-171, ref-172, ref-174, ref-175, ref-176, ref-177, ref-178, ref-179, ref-180, ref-181, ref-242, ref-272, ref-275, ref-276, ref-277, ref-278, ref-279, ref-280, ref-350, ref-351, ref-352, ref-353, ref-354, ref-355, ref-356, ref-357, ref-358, ref-359, ref-360, ref-362, ref-015, ref-031, ref-125, ref-130, ref-228, ref-411, ref-413, ref-418, ref-111, ref-495, ref-230, ref-496, ref-500, ref-501, ref-502, ref-116, ref-504, ref-539, ref-540, ref-541, ref-542, ref-543, ref-544, ref-545, ref-546, ref-547, ref-548, ref-056, ref-404, ref-377, ref-592, ref-593, ref-594, ref-595, ref-596, ref-598, ref-611, ref-612, ref-615, ref-616, ref-376, ref-236, ref-417, ref-586, ref-674, ref-675, ref-676, ref-711, ref-677, ref-712, ref-678, ref-656, ref-657, ref-039, ref-658, ref-659, ref-660, ref-661, ref-662, ref-663, ref-664, ref-041,
  ref-126, ref-127, ref-680, ref-537, ref-681, ref-682, ref-683, ref-684, ref-373, ref-685, ref-713, ref-714, ref-715, ref-716, ref-717, ref-718, ref-719, ref-720, ref-721, ref-722, ref-723, ref-724, ref-725, ref-727]
confidence: low
last_run: 2026-09-25
---
...
### runs/2026-09-25-79/verification2.json
...
```json
{
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "단계 4 페이지 2절 표의 q4-09·q4-10 은 백로그에 중복 등록된 기존 질문이다. 이번 실행의 새 질문(q4-13·q4-14·q5-10)과는 겹치지 않으며, 백로그 정리는 별도 요청 사항이다.",
      "업무 분해·배정 설계 초안 6절의 새 항목은 기존 '사용자 확인을 별도 개념으로 둘지' 질문과 '검증 기록' 질문에 q4-01 답을 연결한 것이다. 개념을 추가하지 않았으므로 충돌은 없다."
    ]
  },
  "terminology": {
    "ok": false,
    "conflicts": [
      "아이디어 2 5절 '오해석 방지 확인 절차' 소절과 업무 분해·배정 설계 초안 6절 새 항목에서 'MCP'를 풀어 쓰지 않았고 용어집 '모델 컨텍스트 프로토콜' 링크도 없다. 1차 수정 지시(f3·용어)가 두 페이지에서 이행되지 않았다."
    ]
  },
  "quotation_check": {
    "ok": true
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "단계 4 페이지 3절 OWASP 문장: '(2025판, 문서 기준 2024-11)'을 '(2025판, 발행일 2024-11 은 문서 안에서 확인하지 못함)'처럼 고친다 — 1차 검증 메모 f1 이 문서 안에서 2024-11 표기를 확인하지 못했다고 적었으므로 '문서 기준'은 브리프에 없는 주장(드리프트)이다.",
    "아이디어 2 5절 '오해석 방지 확인 절차' 소절에서 MCP 가 처음 나오는 곳을 '[모델 컨텍스트 프로토콜](../glossary/model-context-protocol.md)(Model Context Protocol, MCP)'로 고친다. 업무 분해·배정 설계 초안 6절 새 항목의 'MCP 도구 명세'도 '[모델 컨텍스트 프로토콜](../../glossary/model-context-protocol.md)(Model Context Protocol, MCP) 도구 명세'로 고친다 — fixes_applied 는 1차 지시 'f3·용어'를 이행했다고 보고했으나 두 페이지 모두 반영되지 않았다. 이행한 뒤 fixes_applied 항목도 사실대로 고친다.",
    "트랙 개요 docs/tracks/nl-task-chatbot/index.md 를 pages 에 update 로 추가하고 6. 살아있는 산출물 링크 절만 patch 한다. 추가할 내용: 실행 2026-09-25-79 에서 업무 분해·배정 설계 초안은 v0.8 그대로이고 개념 '사용자 확인' 제안은 6절 질문으로 남았다는 점, 아이디어 2 5절에 '오해석 방지 확인 절차' 소절(q4-01)이 추가됐다는 점. H1 아래 상태 줄의 '현재 단계: 단계 3. 구현 가설 설계'는 바꾸지 않는다 — 단계 3 완료와 단계 3 → 4 전환이 승인되지 않은 상태에서 CLI 지정으로 단계 4 를 다뤘기 때문이다. '마지막 트랙 실행: 2026-09-25'는 유지한다.",
    "단계 4 페이지 2절 표: q4-10 의 질문 칸을 '(q4-09와 같은 질문의 중복 등록)' 대신 백로그의 질문 문구 그대로 쓴다. 제기 근거 칸은 시드 규칙대로 finding id 또는 '사용자'만 쓴다('f14, 실행 2026-09-25-04' → 'f14'). 이렇게 하면 표가 질문 백로그와 일치한다. 중복 설명은 2절 도입 문장에만 둔다.",
    "track_updates.log_entry 의 '적용 대상 13·18'을 '적용 대상 13. 작업 배정 — MRTA·18. 사람–로봇 협업·운영 인터페이스'로 고친다 — 번호만으로 세부영역을 부르지 않는다는 규칙에 따른다."
  ],
  "confidence": "low",
  "verification_note": "판정: 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경(web_fetch_available: false, fetch_mode mirror_only)에서 검증됐다. 확인 24건, 미확인 1건(f13), 교차 확인 1건(f14). 강등: f13 사실 → 추정(주장 내용을 검색 결과로 확인하지 못함), f14 사실 → 추정(EU 공식 관보 원문이 아닌 제3자 게재본 근거). 원문 미열람 출처: ref-716, ref-718, ref-720, ref-722, ref-723, ref-724, ref-725, ref-620, ref-727, ref-350, ref-356, ref-417, ref-418, ref-656, ref-711. 주의: q4-01 의 답인 다섯 겹 확인 절차·확인 시점·사람 확인 한정·차등 확인(f19~f24)은 이 위키의 종합([추정])이다. 근거 조건은 가정·실험실 로봇, 일상 비서·컴퓨터 사용 에이전트, 로봇 관제 규격이며, 물류 창고 로봇 채팅 지시의 확인 절차를 평가한 연구와 국내 사례는 찾지 못했다. RoboGuard 수치는 초록 기준 '92% → 2.5% 미만'으로 반영됐다. 이번 실행은 단계 3 완료가 승인되지 않은 상태에서 CLI 지정으로 단계 4 질문을 다뤘다. 정정 요청 없음. 온톨로지 변경 승인: 없음 / 거부: 개념 '사용자 확인 (Confirmation)'(f2·f3·f16·f19) — 초안 6절 질문으로 반영됐고 v0.8 이 유지됐다. 단계 완료 조건: 미충족(부족: 명령 권한 q4-03·제한 운영 기준 q4-04 미조사). 단계 전환: 미승인(막힌 질문 q4-02~q4-14 열림, q4-09·q4-10 중복 정리 필요). / 2차 수정 후 재검증. 드리프트 1건 지적(OWASP 발행일의 '문서 기준' 표기). 1차 지시 1건 미이행(MCP 용어집 링크·약어 풀어쓰기). 트랙 개요 페이지 갱신 누락, 2절 표의 백로그 불일치(q4-10), 로그의 번호만 쓴 호칭을 지적했다. 그 밖의 강등·병기·범위 문구 지시는 이행됐다. [분류원문] 보존, 섹션 순서 준수, 링크 유효. 참고: 6절 첫 행(실행 전 검증 단계)은 이번 실행에서 초안 6절과 아이디어 5절에 반영됐으므로 다음 실행에서 충족 여부를 다시 판정한다.",
  "retry_reason": null,
  "track_checks": {
    "standard_sources_ok": false,
    "vendor_claims_tagged": true,
...
- 수정 지시(required_fixes):
    ...
    - 단계 4 페이지 2절 표: q4-10 의 질문 칸을 '(q4-09와 같은 질문의 중복 등록)' 대신 백로그의 질문 문구 그대로 쓴다. 제기 근거 칸은 시드 규칙대로 finding id 또는 '사용자'만 쓴다('f14, 실행 2026-09-25-04' → 'f14'). 이렇게 하면 표가 질문 백로그와 일치한다. 중복 설명은 2절 도입 문장에만 둔다.
    - track_updates.log_entry 의 '적용 대상 13·18'을 '적용 대상 13. 작업 배정 — MRTA·18. 사람–로봇 협업·운영 인터페이스'로 고친다 — 번호만으로 세부영역을 부르지 않는다는 규칙에 따른다.
- 검증 노트: 판정: 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경(web_fetch_available: false, fetch_mode mirror_only)에서 검증됐다. 확인 24건, 미확인 1건(f13), 교차 확인 1건(f14). 강등: f13 사실 → 추정(주장 내용을 검색 결과로 확인하지 못함), f14 사실 → 추정(EU 공식 관보 원문이 아닌 제3자 게재본 근거). 원문 미열람 출처: ref-716, ref-718, ref-720, ref-722, ref-723, ref-724, ref-725, ref-620, ref-727, ref-350, ref-356, ref-417, ref-418, ref-656, ref-711. 주의: q4-01 의 답인 다섯 겹 확인 절차·확인 시점·사람 확인 한정·차등 확인(f19~f24)은 이 위키의 종합([추정])이다. 근거 조건은 가정·실험실 로봇, 일상 비서·컴퓨터 사용 에이전트, 로봇 관제 규격이며, 물류 창고 로봇 채팅 지시의 확인 절차를 평가한 연구와 국내 사례는 찾지 못했다. RoboGuard 수치는 초록 기준 '92% → 2.5% 미만'으로 반영됐다. 이번 실행은 단계 3 완료가 승인되지 않은 상태에서 CLI 지정으로 단계 4 질문을 다뤘다. 정정 요청 없음. 온톨로지 변경 승인: 없음 / 거부: 개념 '사용자 확인 (Confirmation)'(f2·f3·f16·f19) — 초안 6절 질문으로 반영됐고 v0.8 이 유지됐다. 단계 완료 조건: 미충족(부족: 명령 권한 q4-03·제한 운영 기준 q4-04 미조사). 단계 전환: 미승인(막힌 질문 q4-02~q4-14 열림, q4-09·q4-10 중복 정리 필요). / 2차 수정 후 재검증. 드리프트 1건 지적(OWASP 발행일의 '문서 기준' 표기). 1차 지시 1건 미이행(MCP 용어집 링크·약어 풀어쓰기). 트랙 개요 페이지 갱신 누락, 2절 표의 백로그 불일치(q4-10), 로그의 번호만 쓴 호칭을 지적했다. 그 밖의 강등·병기·범위 문구 지시는 이행됐다. [분류원문] 보존, 섹션 순서 준수, 링크 유효. 참고: 6절 첫 행(실행 전 검증 단계)은 이번 실행에서 초안 6절과 아이디어 5절에 반영됐으므로 다음 실행에서 충족 여부를 다시 판정한다.
- 트랙 검사(track_checks): {"standard_sources_ok": false, "vendor_claims_tagged": true, "ontology_changes_grounded": false, "backlog_duplicates": [], "stage_tag_issues": [], "completeness_wording_ok": true, "stage_complete": false, "stage_transition_approved": false}
