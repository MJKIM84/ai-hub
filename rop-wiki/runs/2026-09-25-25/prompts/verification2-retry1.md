...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-283 원문(github_raw integration_doors.md)을 검증 단계에서 다시 열어 문 어댑터의 상태 감독자 역할, adapter_door_requests 토픽, 적절할 때만 전달한다는 문장을 확인했다. 단일 발행 주체(Open Robotics). 발행일 미확인."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-284 원문(github_raw integration_lifts.md)에서 'only relaying the instructions to the lift node if it is deemed appropriate', 직접 요청은 'negated'되고 승강기 노드는 OPC 같은 프로토콜로 제어기와 통신한다는 내용을 확인했다. 단일 출처."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-286 LiftState.msg 원문(github_raw)에 층·문 상태·운행 상태·모드(설정 가능한 HUMAN·AGV, 읽기만 가능한 FIRE·OFFLINE·EMERGENCY, UNKNOWN)·session_id 가 있다. 단일 출처. 같은 URL 이 실행 2026-09-25-24 에서 ref-382 로 제안됐다(중복 id 가능성, 검증 노트 참조)."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "ref-283·ref-284·ref-286 은 실행 2026-09-25-24(8. 실시간 세계 상태·데이터 일관성)가 ref-379·ref-380·ref-382 로 제안한 출처와 URL 이 같다. 둘 다 아직 참고문헌 목록에 없어 브리프 id 를 쓰지만, 퍼블리셔가 등록할 때 같은 URL 의 id 가 이중 등록되지 않게 확인해야 한다",
...
```json
{
  ...
  "outline": [
    ...
    },
    {
      ...
      "section": "4. 핵심 개념과 용어",
      "budget_chars": 900,
      "summary": "Open-RMF 의 문·승강기 어댑터, 승강기 세션·모드, 디스펜서·인제스터(예시 구현)와 VDA 5050 3.0.0 의 해제·동작 구역이 이 영역의 핵심 개념이다. [사실][^ref-284][^ref-031]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 800,
      "summary": "공개 자료의 접근법은 어댑터를 거친 요청·상태 감독, 요청–결과 방식의 작업대 인계, 관제의 구역 진입 허가 세 가지다. [사실][^ref-283][^ref-023][^ref-031]",
      "planned_findings": [
        "f1",
        "f3",
        "f5",
    ...
    },
    {
      ...
      "budget_chars": 900,
      "summary": "관련 공개 규격은 오픈소스 메시지 정의와 로봇 탑승 안전 표준이 중심이다. [사실][^ref-286][^ref-413] 설비–로봇 연동 자체를 정한 국제 표준은 이번 조사에서 확인하지 못했다.",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "section": "8. 대표 연구와 자료",
      "budget_chars": 650,
      "summary": "승강기를 포함한 다층 경로계획·병원 배송·지도 구축 연구와 국내 협의체·정부 과제 자료가 있다. [사실][^ref-060][^ref-103]",
      "planned_findings": [
        "f14",
        "f18",
        "f19",
    ...
    },
    {
      ...
      "budget_chars": 650,
      "summary": "ROP 는 어댑터를 통해 작업 요청·점유 예약·상태 확인·완료 확인을 맡고 설비 제어와 설비 안전 제어는 연계 대상으로 남는 것으로 보인다. [추정][^ref-283][^ref-284][^ref-031]",
      "planned_findings": [
        "f26",
        "f7",
        "f8",
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "섹션 3~11 신규 작성(Open-RMF 문·승강기 어댑터와 설비 메시지, 디스펜서·인제스터, VDA 5050 3.0.0 해제·동작 구역, KS B 7317·단체표준·제조사 API, 출하 시나리오), 페이지 상태 표식 추가. 2차 수정: 7절 요약의 '국제 표준 미확인' 태그·각주 분리, 5절 KS B 7317 문장 교체, 3·9절 [의견]에 구축자 의견 명시"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area10-s7.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 10. 설비·건물 시스템 연동 의 \"7. 관련 표준·프레임워크·오픈소스\" 절을 옮겼다. 2차 수정: 1절·3절 첫 문장의 '국제 표준 미확인'을 태그 없는 조사 한계 서술로 분리"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area10-s4.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 10. 설비·건물 시스템 연동 의 \"4. 핵심 개념과 용어\" 절을 옮겼다(2차 재실행에서 변경 없음)"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area10-s10.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 10. 설비·건물 시스템 연동 의 \"10. 다른 연구영역과의 연결\" 절을 옮겼다. 2차 수정: 9번 항목 사실·의견 분리, 17번 항목을 Open-RMF 예시 작업대로 한정, 28번 항목 사실·열린 질문 분리"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area10-s8.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 10. 설비·건물 시스템 연동 의 \"8. 대표 연구와 자료\" 절을 옮겼다(2차 재실행에서 변경 없음)"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area10-s11.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 10. 설비·건물 시스템 연동 의 \"11. 열린 질문\" 절을 옮겼다(2차 재실행에서 변경 없음)"
    }
  ],
  "changelog_entry": "2026-09-25 | 10. 설비·건물 시스템 연동 | 섹션 3~11 신규 작성(Open-RMF 문·승강기 어댑터·설비 메시지, 디스펜서·인제스터 예시 구현, VDA 5050 3.0.0 해제·동작 구역, KS B 7317·승강기협회 단체표준·제조사 API, 출하 시나리오), 2차 수정으로 태그 드리프트 6건 교정 | run 2026-09-25-25",
  "index_updates": {
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "summary": "원문 미열람. Open-RMF 디스펜서 요청 메시지 정의(재사용, 이번 실행 재열람 안 함).",
      "cited_by": [
        "docs/topics/2026/2026-09-25-area10-s4.md",
        "docs/topics/2026/2026-09-25-area10-s7.md"
      ],
      "source_unopened": true
    },
    {
    ...
    },
    {
      ...
      "cited_by": [
        "docs/categories/c-connectivity-and-execution-foundation/10-facility-and-building-system-integration.md",
        "docs/topics/2026/2026-09-25-area10-s8.md"
      ],
      "source_unopened": true
    },
    {
      ...
      "cited_by": [
        "docs/categories/c-connectivity-and-execution-foundation/10-facility-and-building-system-integration.md",
        "docs/topics/2026/2026-09-25-area10-s8.md",
        "docs/topics/2026/2026-09-25-area10-s10.md"
      ],
      "source_unopened": true
    },
    {
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "Open-RMF 의 문 연동 구조(문 노드, DoorState·DoorRequest, 상태 감독자 역할의 문 어댑터)를 설명하는 공식 문서(mdBook 원본). 같은 URL 이 참고문헌 ref-283 으로 이미 등록돼 있을 수 있어 이중 등록 확인이 필요하다.",
      "cited_by": [
        "docs/categories/c-connectivity-and-execution-foundation/10-facility-and-building-system-integration.md"
      ],
      "source_unopened": false
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "Open-RMF 의 승강기 연동 구조(승강기 노드, LiftState·LiftRequest, 요청을 걸러 전달하는 승강기 어댑터)를 설명하는 공식 문서(mdBook 원본). 같은 URL 이 참고문헌 ref-284 로 이미 등록돼 있을 수 있어 이중 등록 확인이 필요하다.",
      "cited_by": [
        "docs/categories/c-connectivity-and-execution-foundation/10-facility-and-building-system-integration.md"
      ],
      "source_unopened": false
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "승강기 층·문·운행 상태, 모드(사람·AGV·화재·오프라인·비상), 점유 세션을 보고하는 Open-RMF 승강기 상태 메시지 정의. 같은 URL 이 참고문헌 ref-286 으로 이미 등록돼 있을 수 있어 이중 등록 확인이 필요하다.",
      "cited_by": [
        "docs/categories/c-connectivity-and-execution-foundation/10-facility-and-building-system-integration.md"
      ],
      "source_unopened": false
...
### runs/2026-09-25-25/format_check.md
...
created: 2026-09-24
updated: 2026-09-25
sources: [ref-023, ref-031, ref-047, ref-049, ref-060, ref-103, ref-163, ref-283, ref-284, ref-286, ref-411, ref-412, ref-413, ref-414, ref-415, ref-416, ref-417, ref-418, ref-419, ref-420, ref-421]
last_run: 2026-09-25
version: 2
---
...
설비는 여러 로봇이 나눠 쓰는 자원이기도 하다. 로봇이 층간 운반에 승강기를 쓰면 한 로봇의 세션이 승강기를 점유하는 동안 다른 로봇이 기다려야 하므로, 승강기 대기가 처리량과 운반 시간에 영향을 줄 것으로 보인다. [추정][^ref-411][^ref-286][^ref-060][^ref-103] 다만 물류센터 화물용 승강기에 대한 정량 자료는 아직 확인되지 않았다(열린 질문 oq-010).
...
| 예외·성과 | 승강기가 화재·비상·오프라인 모드로 바뀌거나 문 상태가 오프라인·알 수 없음이면 해당 설비를 쓰는 작업을 멈추거나 다른 경로·사람 확인으로 넘기는 예외로 다뤄야 할 것으로 보인다. [추정][^ref-286][^ref-412] 승강기 세션 대기는 출하뿐 아니라 입고·적치의 층간 운반에서도 처리량과 운반 시간에 영향을 줄 것으로 보인다. [추정][^ref-411][^ref-286][^ref-060][^ref-103] |
...
Open-RMF 는 문과 승강기 앞에 어댑터를 두어 요청을 걸러 설비 노드에 전달하고, 승강기 노드는 OPC 같은 프로토콜로 승강기 제어기와 통신한다. [사실][^ref-283][^ref-284] 설비 상태는 문 모드와 승강기 상태 메시지로 되돌아온다. [사실][^ref-412][^ref-286] 한계는 이 구조가 Open-RMF 한 곳의 문서에 기댄다는 점이다.
...
이 영역과 관련된 공개 규격은 오픈소스 메시지 정의와 로봇 탑승 안전 표준이 중심이다. [사실][^ref-286][^ref-413] 설비–로봇 연동 자체를 정한 국제 표준은 이번 조사에서 확인하지 못했다.
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-023, ref-031, ref-047, ref-049, ref-283, ref-284, ref-286, ref-411, ref-412, ref-413, ref-414, ref-415, ref-417, ref-418, ref-419]
last_run: 2026-09-25
version: 1
...
- 이 영역과 관련된 공개 규격은 오픈소스 메시지 정의와 로봇 탑승 안전 표준이 중심이다. [사실][^ref-286][^ref-413]
...
| 이름 | 유형 | 이 영역과의 관계 | 출처 |
|---|---|---|---|
| Open-RMF 문·승강기 연동 문서 | 오픈소스 | 문·승강기 노드와 어댑터 구조를 설명한다. [사실] | [^ref-283][^ref-284] |
| Open-RMF 설비 메시지(rmf_lift_msgs·rmf_door_msgs) | 오픈소스 | 승강기 요청·상태, 문 모드 메시지를 정의한다. [사실] | [^ref-286][^ref-411][^ref-412] |
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-023, ref-031, ref-047, ref-049, ref-283, ref-284, ref-286, ref-411, ref-412]
last_run: 2026-09-25
version: 1
...
- **승강기 세션과 모드(Lift Session / Mode)** — 승강기 요청(LiftRequest)은 승강기 이름, 요청자별 세션 id, 요청 유형(세션 종료·AGV 모드·사람 모드), 목적 층, 문 상태를 담는다. AGV 모드에서는 정지 시 문이 항상 열려 있고, 사람 모드에서는 문이 시간 초과로 자동으로 닫힐 수 있다. [사실][^ref-411] 승강기 상태(LiftState)는 층·문·운행 상태와 모드(설정 가능한 사람·AGV, 읽기만 가능한 화재·오프라인·비상), 승강기를 점유한 세션 id 를 보고한다. [사실][^ref-286]
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-023, ref-031, ref-103, ref-163, ref-284, ref-286, ref-411, ref-412, ref-413, ref-415]
last_run: 2026-09-25
version: 1
...
- [9. 로봇·제조사 관제 연동](../../categories/c-connectivity-and-execution-foundation/09-robot-and-vendor-fleet-manager-integration.md) — VDA 5050 3.0.0 은 문·게이트·승강기 같은 주변 시스템과의 통신을 관제 최소 기능 목록에 넣는다. [사실][^ref-031] 그래서 로봇 연동과 설비 연동을 함께 설계해야 한다는 것이 이 위키 구축자의 의견이다. [의견]
- [12. 명령·작업 실행의 신뢰성](../../categories/c-connectivity-and-execution-foundation/12-command-and-task-execution-reliability.md) — 승강기 세션의 시작·종료와 어댑터를 거치지 않은 요청의 무효화는 제어권 관리 문제다. [사실][^ref-411][^ref-284]
...
- [16. 공용 자원·충전·에너지 최적화](../../categories/d-planning-and-optimization/16-shared-resource-charging-and-energy-optimization.md) — 승강기는 세션 단위로 점유되는 공용 자원이다. [사실][^ref-286]
- [17. 로봇 간 협업·물리적 인계](../../categories/e-collaboration-and-field-operations/17-robot-to-robot-collaboration-and-physical-handover.md) — Open-RMF 의 예시 작업대(디스펜서·인제스터)에서는 인계가 요청–결과 확인으로 이뤄진다. [사실][^ref-023]
- [20. 예외 복구·재계획·업무 연속성](../../categories/e-collaboration-and-field-operations/20-exception-recovery-replanning-and-business-continuity.md) — 승강기 화재·비상 모드와 문 오프라인 상태는 복구 규칙의 입력이 될 것으로 보인다. [추정][^ref-286][^ref-412]
...
- [28. 표준·상호운용성·다사업자 거버넌스](../../categories/g-safety-security-intelligence-and-governance/28-standards-interoperability-and-multi-vendor-governance.md) — 대한승강기협회의 승강기–로봇 연동 단체표준이 제정됐다고 보도됐다. [사실][^ref-415] 이 단체표준과 오픈소스 메시지의 대응은 열린 질문이다.
...
### runs/2026-09-25-25/verification2.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "1차 수정 지시 'f6 은 표준 규격이 아니라 예시 구현으로 서술'이 4·6·7절과 주제 페이지 s4·s7 에서는 이행됐다. 그러나 주제 페이지 s10 의 17. 로봇 간 협업·물리적 인계 항목은 '작업대와의 인계는 요청–결과 확인으로 이뤄진다. [사실][^ref-023]'이라고 써서 Open-RMF 예시 작업대의 방식을 작업대 인계 전반으로 일반화했다. 한정 표현을 넣어야 한다."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "ref-283·ref-284·ref-286 은 실행 2026-09-25-24 가 ref-379·ref-380·ref-382 로 제안한 출처와 URL 이 같다. 현재 참고문헌 목록(ref-281 까지)에는 둘 다 없으므로 퍼블리셔가 등록할 때 같은 URL 이 이중 등록되지 않게 확인해야 한다"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "세부영역 페이지 7절 요약 문장과 주제 페이지 docs/topics/2026/2026-09-25-area10-s7.md 의 1절·3절 첫 문장: '설비–로봇 연동 자체를 정한 국제 표준은 이번 조사에서 확인하지 못했다'에 붙은 [사실][^ref-286][^ref-413] 을 뗀다. ref-286·ref-413 은 표준이 없다는 사실을 뒷받침하지 않는다. 이 부분은 태그 없는 조사 한계 서술로 둔다. 앞부분 '관련 공개 규격은 오픈소스 메시지 정의와 로봇 탑승 안전 표준이 중심'만 남긴다면 각주를 유지해도 된다.",
    "세부영역 페이지 5절 시나리오 표 '제약' 칸: '로봇의 엘리베이터 탑승에는 국가표준 KS B 7317 의 안전 요구사항이 제약으로 걸린다. [사실][^ref-413]'을 고친다. f11 은 표준의 제정과 대상만 확인했고 적용 여부는 확인하지 않았다. 'KS B 7317 은 이동 로봇의 엘리베이터 탑승 안전 요구사항과 평가 방법을 정한다. [사실][^ref-413]'로 바꾸거나, 제약으로 걸린다는 서술을 [추정]으로 강등한다.",
    "주제 페이지 docs/topics/2026/2026-09-25-area10-s10.md 3절, 9. 로봇·제조사 관제 연동 항목: '로봇 연동과 설비 연동을 함께 설계해야 한다'는 브리프의 사실(f8)이 아니라 당위 판단이다. [사실] 을 [의견](구축자 의견)으로 바꾸거나, 사실 부분('VDA 5050 3.0.0 은 주변 시스템 통신을 관제 최소 기능 목록에 넣는다')과 판단 부분을 나눠 태그를 따로 붙인다.",
    "주제 페이지 docs/topics/2026/2026-09-25-area10-s10.md 3절, 17. 로봇 간 협업·물리적 인계 항목: '작업대와의 인계는 요청–결과 확인으로 이뤄진다'를 'Open-RMF 의 예시 작업대(디스펜서·인제스터)에서는 인계가 요청–결과 확인으로 이뤄진다'로 한정한다. 1차 지시(f6 은 예시 구현)에 따른 것이다.",
    "주제 페이지 docs/topics/2026/2026-09-25-area10-s10.md 3절, 28. 표준·상호운용성·다사업자 거버넌스 항목: 열린 질문을 소개하는 문장에 [사실][^ref-415] 이 붙어 있다. 태그를 떼거나, 사실 부분('대한승강기협회 단체표준이 제정됐다고 보도됐다. [사실][^ref-415]')과 열린 질문 소개를 나눈다.",
    "세부영역 페이지 3절 끝 문장('…설비 연동을 따로 설계해야 한다. [의견]')과 9절의 [의견] 두 곳(표 '로봇 자체 지능·제어' 행, 마지막 문장): 누구의 의견인지 밝혀 '[의견](구축자 의견)'처럼 적는다. 공통 규칙 2절과 verifier 3절 항목 5 에 따른 것이다. 판단의 근거가 된 f7·f8·f26 을 문장 안에서 가리켜도 된다."
  ],
  "confidence": "medium",
  "verification_note": "판정: 1차 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경(mirror_only)에서 검증됐다. 확인 22건, 미확인 4건, 교차 확인 0건. 강등: f14 사실 → 추정, f18 사실 → 추정, f22 사실 → 추정. f17 은 추정을 유지하고 '배송로봇 40여 대' 수치를 삭제했다. 원문 미열람 출처: ref-047, ref-049, ref-060, ref-103, ref-163, ref-413, ref-414, ref-415, ref-416, ref-417, ref-418, ref-419, ref-420, ref-421. github_raw 로 원문을 확인한 출처: ref-023, ref-031, ref-283, ref-284, ref-286, ref-411, ref-412. 주의: Open-RMF 설비 메시지와 VDA 5050 3.0.0 구역 규정은 각각 발행 주체 한 곳의 원문에만 기댄다. 국내 표준·제조사 API 는 기사·보도자료 스니펫 범위에서만 확인했다. 벤더 주장은 독립 확인이 없다. 분류 원문 질문(컨베이어 준비와 로봇 도착을 맞추는 방법)에는 추정 수준의 답(f23)만 있다. VDA 5050 명세는 운영 책임을 배분하지 않는다. 정정 요청 없음. / 2차 수정 후 재검증. 1차 수정 지시 14건은 대부분 이행됐다: f8·f14·f17·f18·f22 구절 삭제·강등, 벤더 주장 병기, 원문 미열람 표기, ref-103 발행일, 직접 인용 없음, oq-010 연결과 새 질문 3건. 드리프트 6건을 지적했다: 7절의 '국제 표준 미확인'에 근거 없는 [사실]·각주, 5절 KS B 7317 적용 서술의 [사실], 주제 페이지 s10 의 당위 문장·일반화 문장·열린 질문 문장에 붙은 [사실], 출처를 밝히지 않은 [의견] 세 곳. [분류원문] 보존, 범위 경계 준수(9절 '연계 대상' 표기), 섹션 순서와 링크는 형식 검증 통과 결과를 따른다. 참고 사항: 세부영역 페이지 프런트매터 sources 에는 분리된 주제 페이지에서만 인용하는 출처(ref-047·ref-049·ref-163·ref-415~ref-420)가 함께 들어 있다. reference_updates 는 기존 ref-047·ref-049 의 신뢰도를 high 에서 medium 으로 적었다. 퍼블리셔는 이번 실행에서 재열람하지 않았다는 이유만으로 기존 등록값을 낮추지 않는지 확인해야 한다. 분리 주제 페이지의 검증 노트 문구('1차·2차 검증을 거쳤다')는 코드가 생성한 것이며 게시 시점에만 참이 된다. 검증 검색 0회(2차, 도구 미사용).",
  "retry_reason": null
}
```
