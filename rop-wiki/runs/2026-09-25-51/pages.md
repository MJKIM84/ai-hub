...
| 동작 | 경로 | 상태 | 변경 요약 |
|---|---|---|---|
| update | docs/tracks/nl-task-chatbot/stage-2-data-and-standards.md | draft | q2-02 답함(3절 소제목 신설), 새 질문 q2-07·q3-08·q3-09, 4·6·7·8·9절과 상태 줄 갱신(상태 줄 수정 지시 때문에 전체 content 로 보냄) |
| update | docs/tracks/nl-task-chatbot/task-model-draft.md | draft | 초안 v0.4 → v0.5: 진행 상태 외부 표현 원천 메모(초안 → 확정), 배정 외부 표현 대응 메모(확정 유지), 6절 질문 2건 추가·선행 의존 질문 보강(H1 버전 변경 때문에 전체 content 로 보냄) |
| update | docs/ideas/nl-task-chatbot.md | draft | 4절에 '작업·배정 결과를 표현하는 표준·형식' 소절 신설(이 위키 구성 비교표, 초안 대비 빠진 항목, 임무 기술 형식 비교 연구), 평가 데이터(q2-03) 미조사 명시 |
| update | docs/tracks/nl-task-chatbot/index.md | draft | 6. 살아있는 산출물 링크: 초안 v0.5 와 아이디어 2 4절(q2-02 표준·형식 비교) 진행 상황 갱신 |
