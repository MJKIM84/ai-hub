...
```json
{
  ...
  "claim_checks": [
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "출처 중복: ref-328 은 참고문헌에 이미 있는 ref-163 과 같은 논문·같은 KCI URL 이다. ref-163 을 재사용하고 ref-328 은 만들지 않는다. 내용은 기존 6. 지도·공간·위치 모델 페이지 인용과 일치한다. 자율 탐사 SLAM·엘리베이터 버튼 조작은 연계 대상이다. 소요 시간 수치는 없다. 원문 미열람."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "설계 추론이며 [추정] 이 맞다. 각주는 ref-328 대신 ref-163 을 쓰고, f15 의 삭제된 오류 절에 기대지 않게 서술한다."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
  ...
  "duplication": {
    "ok": false,
    "overlaps": [
      "ref-328(노주형 외, 로봇학회 논문지 21(1))은 참고문헌에 이미 있는 ref-163 과 같은 논문·같은 URL 이다",
  ...
  "required_fixes": [
    ...
    "f16·f18: 출처 ref-328 을 만들지 않고 기존 ref-163 각주(참고문헌 페이지의 줄 그대로)를 쓴다 — 같은 논문·같은 URL 의 중복이다.",
  ...
  "confidence": "medium",
  "verification_note": "판정: 조건부 승인. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(fetch_mode mirror_only). 확인 13건, 미확인 6건(f4·f9·f10·f12·f13·f15), 교차 확인 0건. 강등: f2 사실 → 추정(핵심 수치 단일 출처·재게재본은 독립 아님), f4 비교 절 삭제, f12·f13 반복 추론 절을 추정으로 분리, f15 오류 절 삭제. 삭제: f10(BlueBotics 인용 구절 미확인, ref-325 미사용 출처). 원문 미열람 출처: ref-217, ref-318, ref-319, ref-320, ref-321, ref-322, ref-324, ref-326, ref-327, ref-163(ref-328 대체). 검증 에이전트가 GitHub 원본으로 ref-046·ref-323·ref-079·ref-105 를 열어 f7·f12·f13·f14 를 확인했다. 출처 중복: ref-328 은 ref-163 과 같은 논문이다. 용어: '지도 정합'은 기존 용어라 신규 등록하지 않는다. 주의: q1-04 의 답은 '현장 모델링 시간과 반복 작업은 정성 연구, 과제 측 보고값(PAN-Robots 6개월→2개월), 벤더 주장(OTTO 50%, ScaliRo 수 인일), 도구·형식 문서로만 확인된다'는 자료 유형 정리(추정)다. 단계별 소요 시간을 독립적으로 측정한 자료는 검색 범위(리서치 25회, 검증 11회)에서 찾지 못했고 부재가 확인된 것은 아니다. 수치는 가설 3 판정 근거로 쓰기 전에 재확인이 필요하다. 검색은 리서치 25회와 검증 11회를 합쳐 36회/40회를 썼다. 온톨로지 변경 승인: 없음 / 거부: 없음(제안 없음). 단계 완료 조건: 충족(아이디어 3절 비교와 공간 그래프 스키마 초안의 인식 대상 요소가 이전 실행에서 채워짐). 단계 전환: 미승인(막힌 질문 q1-05·q1-06, 이번 실행의 국내 소요 시간 신규 질문). 폐기 질문 q1-07·q2-05·q4-06 은 전환을 막지 않는다.",
  "retry_reason": null,
  "track_checks": {
