...
```json
{
  ...
  "outline": [
    ...
    },
    {
      ...
      "section": "4. 핵심 개념과 용어",
      "budget_chars": 900,
      "summary": "VDA 5050 주문 id·주문 갱신 id, ROS 2 목표 id, 멱등성 키, 마지막 유언, deadline·liveliness, 작업 백업, 사가를 정리한다. [사실][^ref-031][^ref-588]",
      "planned_findings": [
        "f1",
        "f8",
        "f15",
    ...
    },
    {
      ...
      "budget_chars": 1800,
      "summary": "식별자 기반 중복 무시, 요청 단위 멱등성 키, 상태 기계, 시간 초과 감지, 재시도·취소·보상, 상태 백업·복원의 여섯 기법을 정리한다. [추정][^ref-031][^ref-591]",
      "planned_findings": [
        "f1",
        "f2",
        "f5",
    ...
    },
    {
      ...
      "budget_chars": 700,
      "summary": "VDA 5050, ROS 2 액션·QoS·관리형 노드, Open-RMF, MQTT 5.0, IETF 초안, OPC UA Programs, PackML, ISA-95 Job Control, BehaviorTree.CPP 를 표로 정리한다. [사실][^ref-031]",
      "planned_findings": [
        "f1",
        "f8",
        "f9",
    ...
    },
    {
      ...
      "section": "8. 대표 연구와 자료",
      "budget_chars": 550,
      "summary": "행동 트리 입문서, 사가 논문, 브로커 신뢰성 비교 연구(저자 보고값), Open-RMF 재시작 이슈를 소개한다. [사실][^ref-596][^ref-599]",
      "planned_findings": [
        "f24",
        "f25",
        "f27",
    ...
    },
    {
      ...
      "budget_chars": 600,
      "summary": "ROP 는 명령 식별자·상태 기계·시간 초과·재시작 복원·상위 요청 중복 판별을, 로봇 내부 재시도·정지 방식은 제조사가 맡는 쪽으로 보인다. [추정][^ref-031][^ref-591]",
      "planned_findings": [
        "f28",
        "f29",
        "f17",
    ...
    },
    {
      ...
      "budget_chars": 550,
      "summary": "1. 주문·업무 시스템 연계, 2. 공정·워크플로 모델링, 9. 로봇·제조사 관제 연동, 11. 분산 시스템·통신·컴퓨팅 구조, 19. 모니터링·이상 탐지·원인 분석, 20. 예외 복구·재계획·업무 연속성과 연결한다. [추정][^ref-130]",
      "planned_findings": [
        "f22",
        "f31",
        "f16",
    ...
    },
    {
      ...
      "section": "11. 열린 질문",
      "budget_chars": 600,
      "summary": "기존 열린 질문 세 건과 멱등성 키 보존 기간, VDA 5050 재부팅 뒤 주문 처리, Open-RMF 작업 백업 반영 여부의 새 질문 세 건을 둔다.",
      "planned_findings": [
        "f4",
        "f14",
        "f15"
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "섹션 3~11 신규 작성(명령 식별자·중복 무시, 멱등성 키, 상태 기계, 시간 초과, 재시도·취소·보상, 재시작 복원, 적치 재전송 시나리오), 페이지 상태 표식 추가, 조건부 승인 수정 11건 이행"
    },
    {
  ...
  "fixes_applied": [
    ...
    "ref-306 — 새 참고문헌으로 등록하지 않고 f16·f17 문장의 각주를 기존 ref-306(OASIS, MQTT Version 5.0)으로 바꿨으며 reference_updates 에서 ref-306 을 뺐다.",
...
- 퍼블리셔 사전 검사: - topics/2026/2026-09-25-area12-s6.md: 깨진 링크 11-distributed-systems-communication-and-computing.md
