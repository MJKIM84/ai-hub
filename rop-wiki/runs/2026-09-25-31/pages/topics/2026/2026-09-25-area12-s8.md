...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-370, ref-373, ref-374, ref-375]
last_run: 2026-09-25
version: 1
...
- Colledanchise, M., & Ögren, P., Behavior Trees in Robotics and AI: An Introduction(2017) — 행동 트리를 자율 에이전트의 작업 전환을 구조화하는 방법으로 소개하고, 모듈성과 반응성을 함께 갖춘 시스템을 만드는 효율적 방법이라고 설명한다. [사실][^ref-370]
- Garcia-Molina, H., & Salem, K., Sagas(1987) — 오래 걸리는 트랜잭션을 다른 트랜잭션과 섞여 실행될 수 있는 작은 트랜잭션의 순서로 나누고 보상 트랜잭션으로 부분 실행을 보정하는 사가 개념을 제안했다. [사실][^ref-373]
- Paul, T. C. 외, Benchmarking Message Brokers for IoT Edge Computing(2026-03) — 네트워크 장애 조건에서 QoS 0 의 메시지 손실이 약 6.3~6.6% 였고 QoS 1·2 는 손실이 없었다고 보고했다. 저자 보고값(MQTT 브로커 5종, MTTF 30초·MTTR 5초 장애 모델)이며 원문 미열람이고, 이 페이지에서는 중복·재전송 판단의 입력으로만 참고한다. [추정][^ref-375]
- Open Robotics, rmf_ros2 이슈 #224 — 플릿 어댑터 재시작 시 배정 작업 유실 문제와 SQLite 기반 작업 백업 제안을 다룬다(배포판 반영 여부 미확인). [사실][^ref-374]
