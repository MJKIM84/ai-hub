...
```json
{
  ...
  "outline": [
    ...
    },
    {
      ...
      "budget_chars": 150,
      "summary": "분량 기준 초과로 절 본문을 주제 페이지 2026-09-26-area04-s10 으로 옮기고 요약 한 문장과 링크만 남긴다(내용 변경 없음).",
      "planned_findings": []
    },
    {
      "path": "docs/categories/g-safety-security-intelligence-and-governance/25-safety-and-risk-management.md",
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "월간 재검증: 7절 끝에 ISO 22400-2 판 상태 덧붙임, 10절 본문을 주제 페이지로 옮기고 요약·링크만 남김. 2차: 프런트매터 sources 에 ref-811·ref-812 추가, last_run 2026-09-26"
    },
    {
      "path": "docs/categories/g-safety-security-intelligence-and-governance/25-safety-and-risk-management.md",
      "action": "update",
      "status": "draft",
      "diff_summary": "월간 재검증: 7절 재검증 내용은 주제 페이지로 분리되어 요약·링크만 남김. 2차: 프런트매터 last_run 2026-09-26"
    },
    {
      "path": "docs/topics/2026/2026-09-26-area04-s10.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "분리: 4. 성과·경제성·프로세스 개선 의 \"10. 다른 연구영역과의 연결 (번호와 이름을 함께 표기)\" 절을 옮기고 링크를 주제 페이지 위치 기준 상대 경로로 고쳤다(2차 재실행에서 변경 없음)"
    },
    {
      "path": "docs/topics/2026/2026-09-26-area25-s7.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 25. 안전·위험 관리 의 \"7. 관련 표준·프레임워크·오픈소스\" 절을 옮겼다. 2차: ref-813 각주 정의 제목을 ISO 페이지 전체 제목으로 고침"
    }
  ],
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "source_unopened": true,
      "cited_by": [
        "docs/topics/2026/2026-09-26-area25-s7.md"
      ]
    },
    {
      "id": "ref-560",
  ...
  "additional_research_requests": [
    ...
    "pipeline 담당 요청: 자동 분리 뒤 reference_updates 의 cited_by 와 세부영역 프런트매터 sources·last_run 을 분리 결과에 맞춰 자동으로 갱신하는 처리가 필요하다(이번 2차 수정 지시 4건이 모두 이 불일치에서 나왔다)"
  ],
  "fixes_applied": [
    "갱신 범위 — 4. 성과·경제성·프로세스 개선과 25. 안전·위험 관리 두 페이지만 갱신했고, 주제 페이지 2026-09-25-area04-s4·s7 과 표준 목록은 건드리지 않았으며 주제 페이지 반영은 changelog_entry 의 다음 실행 후보에 적었다.",
    "append 로 보내기 — 두 페이지 모두 7. 관련 표준·프레임워크·오픈소스 절에 '재검증(2026-09-26, 실행 2026-09-26-02)' 소제목과 판 상태만 덧붙였고 기존 문장은 지우지 않았다(10절은 분량 분리로 기존 문장을 주제 페이지로 옮긴 것이며 삭제가 아니다).",
    "needs_update·deprecated 지시 없음 — 두 페이지 status 를 draft 로 두었다.",
    "f2·f3 — 4. 성과·경제성·프로세스 개선 7절 재검증 소제목 아래에 지시된 문장을 [사실][^ref-812][^ref-811][^ref-139]로 썼다.",
    "f4 — 'A3(미국 로봇 안전 표준 개발 단체) FAQ 설명에 따르면'으로 주체를 밝히고 용어집 '협동 적용(Collaborative Application)'을 링크해 [사실][^ref-471][^ref-560]로 썼다.",
    ...
    "형식 수정 — 자동 분리된 docs/topics/2026/2026-09-26-area04-s10.md 의 깨진 링크 7개를 ../../categories/<대분류 slug>/<파일>.md 로 고쳐 이 페이지를 content 로 직접 보내고, 4. 성과·경제성·프로세스 개선 10절은 같은 요약 문장과 주제 페이지 링크로 바꿔 코드가 다시 분리하지 않게 했다. 주장·태그·각주는 바꾸지 않았다.",
    ...
    "2차: 4. 성과·경제성·프로세스 개선 프런트매터 sources 에 ref-811·ref-812 를 더해 7절 각주·13절 정의와 맞췄다.",
    "2차: docs/topics/2026/2026-09-26-area25-s7.md 의 [^ref-813] 각주 정의 제목을 'ISO/AWI 15066-1 - Collaborative Safety – Physical contact with robots — Part 1: Biomechanical thresholds and data'로 고쳤다.",
    "2차: 04-performance-economics-and-process-improvement.md 와 25-safety-and-risk-management.md 프런트매터 last_run 을 2026-09-26 으로 고쳤다(version 3 유지).",
    "2차: reference_updates 의 ref-471·ref-560·ref-813·ref-814·ref-825·ref-568·ref-211·ref-470·ref-819·ref-566·ref-820·ref-821·ref-555 cited_by 에 docs/topics/2026/2026-09-26-area25-s7.md 를 넣었고, 기존 인용이 있는 ref-470·ref-560·ref-566 만 25. 안전·위험 관리 세부영역 경로를 함께 두었다."
  ]
}
```

### runs/2026-09-26-02/format_check.md
...
- [1. 주문·업무 시스템 연계](../../categories/a-business-supply-chain-design/01-order-and-business-system-integration.md) — 완전 주문 이행률 같은 주문 단위 지표는 WMS·ERP 주문 데이터와 로봇 작업 기록을 연결해야 계산될 것으로 보인다. [추정][^ref-140][^ref-111][^ref-148]
- [2. 공정·워크플로 모델링](../../categories/a-business-supply-chain-design/02-process-and-workflow-modeling.md) — 이벤트 로그로 실제 창고 흐름과 병목을 찾는 프로세스 마이닝이 공정 모델과 실제의 차이를 드러낸다. [사실][^ref-149]
- [3. 처리능력·거점·설비 계획](../../categories/a-business-supply-chain-design/03-capacity-site-and-facility-planning.md) — 작업대 위치와 피킹 방식이 처리량과 필요한 로봇 수를 좌우한다는 해석적 모델 결과를 공유한다. [사실][^ref-096][^ref-145]
- [8. 실시간 세계 상태·데이터 일관성](../../categories/b-common-information-and-environment-model/08-real-time-world-state-and-data-consistency.md) — 성과 계산의 원천은 로봇의 현재 상태를 표현하는 상태 기록(상태 값·배터리·작업 id·시각)이다. [사실][^ref-148]
- [16. 공용 자원·충전·에너지 최적화](../../categories/d-planning-and-optimization/16-shared-resource-charging-and-energy-optimization.md) — 충전 방식과 충전기 수 결정이 비용과 처리 시간의 절충으로 연구되어 있다. [사실][^ref-098][^ref-102]
- [19. 모니터링·이상 탐지·원인 분석](../../categories/e-collaboration-and-field-operations/19-monitoring-anomaly-detection-and-root-cause-analysis.md) — 활성 구간 기반 이동 병목 탐지를 로봇 상태 기록에 적용할 수 있을 것으로 보인다. [추정][^ref-115][^ref-148]
- [22. 시뮬레이션·예측용 디지털 트윈](../../categories/f-deployment-verification-and-maintenance/22-simulation-and-predictive-digital-twin.md) — 우선순위 정책과 충전 대안을 운영 전에 비교하는 일은 가정한 미래를 실험하는 쪽이다. [추정][^ref-146][^ref-102] 현재 상태를 표현하는 8. 실시간 세계 상태·데이터 일관성과는 역할을 나눠 연결한다.
...
원 페이지와 함께 실행 2026-09-26-02 의 1차·2차 검증을 거쳤다. 분리 과정에서 내용은 바꾸지 않았고 링크 경로만 이 페이지 위치 기준으로 고쳤다.
...
### runs/2026-09-26-02/verification2.json
...
```json
{
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "새 열린 질문(KS B ISO 10218-2·KS B ISO/TS 15066 부합 개정)은 oq-070·oq-092 와 인접하나 같은 질문이 아니며, 1차 지시대로 '(관련 기존 질문: oq-070, oq-092)'를 붙여 등록했다"
    ]
  },
  "terminology": {
    "ok": true,
