...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-096 README를 github_raw로 다시 열었다. LLM의 지시 해석·분해·배정, Fast Downward를 쓰는 PDDL 계획, ICRA 2025 게재를 확인했다. 여섯 모듈 이름은 README에 없고 arXiv 검색 요약(ref-097, 원문 미열람)에만 있다. ref-096과 ref-097은 같은 저자 계열이라 독립 교차 확인이 아니다."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 검증 검색(arXiv html)에서 선언 그래프(disjunctive graph) 구성 보조, 결정적 해법, 공정 트리(process tree), IMR-Bench 세 복잡도 수준을 확인했다. ref-104 README를 다시 열어 보니 제목·ICRA 2026·'Code coming soon'만 있고 방법 설명은 없다. 따라서 방법 내용은 원문 미열람 출처(ref-103)에 기대며, 브리프의 f6 source_unopened: false 표시는 실제와 맞지 않는다."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: PromptCraft README(github_raw 열람)와 논문 검색 요약(ref-108, 원문 미열람)을 대조했다. 두 출처 모두 Microsoft가 발행해 독립 교차 확인이 아니다."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
  ...
  "duplication": {
    "ok": false,
    "overlaps": [
      "신규 출처 ref-096~ref-114(19건)의 id가 docs/references/index.md에 이미 등록된 다른 출처와 겹친다. 예: ref-096·ref-097 Lamballais 외 RMFS, ref-098 Zou 외, ref-099 Le-Anh·de Koster, ref-100 Vis, ref-101 RAWSim-O, ref-102 FAIM 2025, ref-103 다층 호텔 배송, ref-104 rmf_demos, ref-105 fleet_adapter_template, ref-106 인증스마트물류센터, ref-107 물류시설법, ref-108 이문수·채준재, ref-109 Stark 외, ref-110~ref-114 Open-RMF task_new·task_state·OMG BPMN·Camunda·Corradini 외",
  ...
  "required_fixes": [
    "출처 id: 신규 출처 ref-096~ref-114(19건)는 참고문헌 목록에 이미 있는 다른 출처와 id가 겹친다. 기존 목록과 겹치지 않는 id(ref-125 이후의 빈 번호)로 일대일 재부여한다. 각주 정의, 본문 각주 참조, 프런트매터 sources, reference_updates에 같은 대응표를 일관되게 적용하고, 대응표를 changelog_entry에 남긴다. 이유: 그대로 게시하면 ref-096~ref-114 참고문헌 페이지와 이미 게시된 3. 처리능력·거점·설비 계획, 2. 공정·워크플로 모델링 페이지의 각주가 다른 문서를 가리키게 된다.",
    ...
    "f1: 여섯 모듈 이름은 arXiv 검색 요약(ref-097, 원문 미열람) 근거로만 각주를 달고, README(ref-096)로는 LLM 분해·배정과 Fast Downward 계획 구조만 뒷받침한다.",
    "f6: 방법 서술(선언 그래프·결정적 해법·공정 트리·IMR-Bench)에는 원문 미열람 출처 ref-103 각주를 단다. ref-104(README)는 제목·ICRA 2026 게재 표기의 근거로만 쓴다.",
