      ...
      "patches": [
        {
          ...
          "content": "(절 본문 생략 — runs/2026-09-25-12/pages/ideas/nl-task-chatbot.md 의 해당 절을 본다)"
        }
      ]
    },
    {
      ...
      "patches": [
        {
          ...
          "action": "replace",
          "content": "(절 본문 생략 — runs/2026-09-25-12/pages/tracks/nl-task-chatbot/index.md 의 해당 절을 본다)"
        }
      ]
    }
  ],
...
```json
{
  ...
  "duplication": {
    ...
    "overlaps": [
      ...
      "출처 id 재부여(ref-212~ref-162 → ref-220~ref-237): 입력의 docs/references/index.md는 ref-124에서 끝난다. 1차가 충돌 예로 든 출처(Lamballais 외, Zou 외 등)는 이 색인에서 ref-096~ref-114 자리에 있다. 따라서 1차의 충돌 전제는 이 입력으로 확인되지 않는다. 다만 재부여한 id ref-220~ref-143은 현재 색인의 어떤 id와도 겹치지 않고, 본문·각주 정의·프런트매터·reference_updates·changelog_entry에 일관되게 적용됐다."
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "단계 1 페이지 4절 결론의 bullet '사람이 정한 도구로 LLM 에이전트의 행동 범위를 정하는 공개 프레임워크(ROSA, RAI)와 국내 연구 사례(한국전자기술연구원)가 있다. [사실][^ref-229][^ref-138][^ref-237]'에서 RAI와 [^ref-138]을 뺀다. 그리고 문장을 ROSA와 한국전자기술연구원 사례로만 [사실]로 쓴다. RAI를 방식 3에 넣은 것은 3절처럼 f19의 [추정] 분류로만 남긴다. 이유: f13(ref-138)은 RAI가 도구로 행동 범위를 정한다는 내용을 뒷받침하지 않으며, 지금 문장은 태그를 올린 것이다.",
  ...
  "confidence": "medium",
  "verification_note": "판정: 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(fetch_mode mirror_only, GitHub 공식 저장소 README만 원문 열람). 확인 19건, 미확인 2건(f8, f17), 교차 확인 0건. 강등: f8 사실 → 추정(비교 우위 서술은 검증 검색에서 상충하는 요약을 확인해 삭제), f17 확인되지 않은 '로봇 직접 제어 응답' 구절 삭제(태그는 추정 유지). 원문 미열람 출처: ref-221, ref-222, ref-223, ref-224, ref-225, ref-227, ref-232, ref-233, ref-234, ref-235, ref-236, ref-237(재부여 후 id). 주의: 연구 수치(LaMMA-P 105%·36%, Peng 외 82%·90%, LTAA 77%)는 모두 저자 보고값이며 독립 재현이 확인되지 않았다. 제품 사례(InOrbit, Formant, 다임리서치)는 모두 보도자료·기사에 근거한 벤더 주장이다. 미션 실행 전 확인·권한 장치는 공개 자료에서 확인되지 않았다. 신규 출처 19건의 id는 ref-212~ref-162에서 ref-220~ref-143으로 일대일 재부여했다. 1차가 든 충돌 예는 입력 참고문헌 색인과 맞지 않아 충돌 전제는 확인되지 않았다. 재부여 id는 현재 색인과 겹치지 않는다. 퍼블리셔는 다음 실행의 next_ref_id가 ref-212 이후인지 확인해야 한다. 미사용 출처 ref-059는 이번 실행의 새 각주에 넣지 않았다. 기존 q1-01 인용은 유지했다. 백로그 q1-05와 q1-06은 사실상 같은 질문이라 정리가 필요하다. 온톨로지 변경 승인: 배정 (Assignment) 수정. 속성 '배정 산출 방식'(값: LLM 직접 추론 f7, 선형계획·MILP 같은 최적화 해법 f3·f5)을 추가하고 배정 행을 초안 → 확정, 초안 v0.1 → v0.2로 올렸다. 거부: 같은 변경의 '규칙' 값(근거 finding 없음, 6절 질문으로). 새 질문: q3-05(단계 3, f8) 등록. '운영 제품 미션 실행의 호출·분해·확인 장치' 질문은 q1-03과 중복이라 등록하지 않았다. 단계 완료 조건: 미충족(부족: 아이디어 2 3절 제품 사례의 확인·승인 비교(q1-03) 미조사, 지시 분해 접근 유형 목록이 업무 분해·배정 설계 초안에 미반영). 단계 전환: 미승인(막힌 질문 q1-03·q1-04·q1-05·q1-06). / 2차 수정 후 재검증. 드리프트 2건(단계 1 페이지 4절 결론에서 RAI를 [사실]로 올린 부분, LaMMA-P의 배정 주체를 일반화한 부분)에 수정 지시를 냈고, 초안의 MILP 첫 등장 표기 수정을 지시했다. 1차 수정 지시 14건은 모두 이행을 확인했다. [분류원문] 보존, 섹션 순서 준수, 링크 유효(형식 검증 통과).",
  "retry_reason": null,
  "track_checks": {
    "standard_sources_ok": true,
    "vendor_claims_tagged": true,
