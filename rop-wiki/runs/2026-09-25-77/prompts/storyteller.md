...
```json
{
  ...
  "duplication": {
    ...
    "overlaps": [
      ...
      "브리프의 신규 id ref-713~ref-723 은 실행 2026-09-25-74 브리프가 다른 문서에 쓴 번호(예: 당시 ref-716 = Rasa 폴백 문서, 게시 시 ref-656~ref-666 으로 재부여)와 겹친다 — 퍼블리셔의 URL 기준 병합·재부여 확인 필요"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "required_fixes": [
    ...
    "각주 정의: ref-717·ref-718·ref-719·ref-720·ref-721·ref-723·ref-677·ref-611 의 접근일 뒤에 ' (원문 미열람)'을 붙이고 reference_updates 해당 항목에 source_unopened: true 를 넣는다 — web_fetch_available: false 환경에서 원문을 열지 못했다.",
...
### docs/references/index.md (요약: 이번 대상 페이지가 인용한 148건 / 전체 680건. 목록에 없는 출처는 새 id 로 적는다 — 같은 URL 이 이미 있으면 퍼블리셔가 기존 id 로 합친다)
