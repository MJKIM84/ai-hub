...
- 대상: 트랙 floorplan-recognition (건축 도면 자동 인식) · 현재 단계: 단계 3. 구현 가설 설계 · 이번에 다룰 백로그 질문 id: q3-01 · 중심 세부영역: 6. 지도·공간·위치 모델 (B. 공통 정보·환경 모델)
- 예산:
...
### runs/2026-09-25-54/target.json
...
```json
{
  ...
  "selection_rationale": "CLI 지정 run_type=track, area=6; 트랙 실행일(Fri, track_days 앞 3개) → 트랙 floorplan-recognition 단계 3, 질문 q3-01 (CLI 지정 질문 id)"
}
```

### runs/2026-09-25-54/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q3-01 인식 → 벡터화 → 공간 그래프 생성 → 온톨로지 적재의 흐름에서 단계마다 입력·출력은 무엇이고 사람 검토는 어디에 두는가?",
    "공개 도구·연구(osmAG-from-cad, Raster-to-Graph, FloorplanVLM, Open-RMF traffic-editor, ifc2indoorgml)는 도면 처리 흐름을 어떤 단계와 중간 산출물 형식으로 나누는가? (단계 3 페이지 3절, 아이디어 페이지 5절 겨냥)",
    "평면도 인식·주석 흐름에서 사람 검토는 어디에 두는가(불확실성 기반 검토, 벡터 공간 전문가 보정, 반복 피드백)? (단계 3 페이지 3절 겨냥)",
    "BIM·공간 그래프를 RDF 온톨로지로 적재하는 도구와 적재 전 검증 수단(IFCtoLBD, SHACL, IDS)은 무엇인가? (공간 그래프 스키마 초안 6절, 28. 표준·상호운용성·다사업자 거버넌스 연결)",
    "로봇 쪽 온톨로지·지식 그래프에 건물·장면 정보를 적재한 연구(OBRNIT, 장면 그래프–로봇 온톨로지 결합)는 어떤 단계와 사람 주석을 두는가? (5. 로봇 능력·작업 온톨로지 연결)",
    "국내에 평면도를 벡터화해 BIM·3D 모델로 바꾸는 처리 흐름을 다룬 연구가 있는가? (한국 자료 우선 규칙)"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "osmAG-from-cad 공식 저장소 README 는 처리 흐름을 DXF → SVG·bounds.json → PNG → AreaGraph 분할 → osmAG.osm(OSM XML) → 선택적 문자 기반 방 이름 붙이기로 나누고, 사용자가 해상도(미터/픽셀)·문 폭·복도 폭·좌표 기준점(위도·경도·픽셀 좌표) 같은 파라미터를 설정하게 하며, 실행 입력·명령을 적은 실행 기록(manifest)을 함께 남긴다.",
      "tag": "사실",
      "source_ids": [
        "ref-084"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README 원문: \"DXF -> SVG + bounds.json -> PNG -> AreaGraph segmentation -> osmAG.osm -> optional text naming\". 문자 기반 이름 붙이기는 기본 꺼짐(--with-text), 최소 면적(min_area)·분할 파라미터 조정 가능.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f2",
      "claim": "Raster-to-Graph 공식 README 는 입력을 가운데 정렬한 512×512 래스터 평면도로, 출력을 벽 교차점(노드)과 벽 선분(엣지)에 평면도 의미를 붙인 구조 그래프로 두며, 다른 이미지 전처리를 쓰면 모델을 다시 학습해야 할 수 있다고 적는다.",
      "tag": "사실",
      "source_ids": [
        "ref-070"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: 출력은 \"structural graphs embedded with floorplan semantics\", 벽 교차점·선분을 그래프 순회 순서로 반복 예측. 전처리가 다르면 \"you might need to retrain the model\".",
      "as_of": "2024",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f3",
      "claim": "FloorplanVLM(arXiv 2602.06507)은 래스터 평면도에서 벽·문·창문·방을 구조화된 JSON 시퀀스로 바로 출력하는 시각-언어 모델 방식의 벡터화를 제안하고, 외벽 IoU 92.52% 를 보고했다(저자 보고 단일 출처).",
      "tag": "사실",
      "source_ids": [
        "ref-696"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: Qwen2.5-VL-3B 를 SFT·GRPO 로 미세조정해 raster → JSON(walls, doors, windows, rooms), Floorplan-2M·HQ-300K 데이터, external-wall IoU 92.52%. (원문 미열람)",
      "as_of": "2026-02",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f4",
      "claim": "ArchCAD-400K 프로젝트 페이지는 주석 과정을 레이어·블록 구조가 일관된 도면 선별, 레이어·블록 계층을 이용한 자동 라벨링, 전문가가 래스터로 바꾸지 않고 벡터 공간에서 직접 보정하는 단계로 나누고, 자동 라벨링으로 비용을 50배 넘게 줄였다고 적는다(저자 측 수치).",
      "tag": "사실",
      "source_ids": [
        "ref-434"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "프로젝트 페이지: 11,917건 중 5,538건 선별, 자동 라벨링 \"cost reduction by over 50x using CAD layers and blocks\", 전문가 보정은 벡터 공간에서 직접 수행. (재인용: 2026-09-25-36)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f5",
      "claim": "Jakubik 외(AAAI 2022)는 평면도 기호 검출 시스템이 검출한 기호마다 불확실성 척도를 계산해 분류하기 어려운 기호에만 전문가 판단을 받는 사람 참여 루프(human-in-the-loop) 설계를 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-691"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 건설 분야 평면도 물량 산출 사례에서 각 검출 기호의 불확실성을 계산해 어려운 기호에 대해 전문가 지식을 획득. AAAI 36(11), 12524–12530. (원문 미열람)",
      "as_of": "2022",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f6",
      "claim": "Sketch2BIM(arXiv 2510.20838)은 손으로 그린 축척 없는 평면도를 다중 모달 LLM 다중 에이전트가 사람 피드백과 스키마 검증을 거쳐 벽·문·창문의 구조화 JSON 레이아웃으로 반복 보정한 뒤 BIM 생성 스크립트로 바꾸는 흐름을 제안했고, 평면도 10장 실험에서 벽 검출은 첫 회 약 83% 에서 몇 번의 피드백 뒤 거의 완전히 맞았다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-690"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: perceptual extraction → human feedback → schema validation → automated BIM scripting. 정밀도·재현율·F1 모두 0.83 이상, RMSE·MAE 는 피드백으로 0까지 감소(저자 보고). (원문 미열람)",
      "as_of": "2025-10",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f7",
      "claim": "DoorDet(2025)은 객체 검출기로 문을 찾고 대규모 언어 모델(LLM)이 문 유형을 분류한 뒤 사람이 검수하는 반자동 데이터 구축 절차를 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-077"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검출기 → LLM 유형 분류 → 사람 검수의 반자동 절차. (재인용: 2026-09-25-05) (원문 미열람)",
      "as_of": "2025-08",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f8",
      "claim": "Open-RMF rmf_traffic_editor README 는 사람이 평면도 위에 주석한 결과를 .building.yaml 로 저장하고, building_map_generator 가 이 파일에서 nav 인자로 주행 그래프를, gazebo·ignition 인자로 시뮬레이터 월드(.world)를 만든다고 적어, 하나의 주석 파일에서 경로용 그래프와 시뮬레이션 초기값이 함께 나온다.",
      "tag": "사실",
      "source_ids": [
        "ref-441",
        "ref-079"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: traffic-editor 가 \".building.yaml\" 파일 생성, building_map_generator gazebo/ignition 은 .world, nav 는 주행 그래프 출력. 충전소 등 경유점 속성은 사람이 입력(ref-079, 같은 발행 주체라 독립 확인 아님).",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f9",
      "claim": "ifc2indoorgml(ISPRS Archives 2022)은 IFC 데이터에서 IndoorGML 모델을 자동 생성하는 오픈소스 도구로, BIM 입력은 이미지 인식·벡터화 단계를 거치지 않고 공간 그래프 표현으로 바로 변환하는 경로가 있다.",
      "tag": "사실",
      "source_ids": [
        "ref-225"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "IFC → IndoorGML 자동 생성 오픈소스 도구, IndoorGML 실용 도구 부족이 개발 동기. (재인용: 2026-09-25-11) (원문 미열람)",
      "as_of": "2022",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f10",
      "claim": "IFCtoLBD 공식 저장소 README 는 IFC STEP·IFC/XML·IFC/JSON 을 입력으로 받아 건물 위상 온톨로지(BOT) 등 링크드 빌딩 데이터 RDF 로 바꾸고 Turtle·JSON-LD·ICDD 패키지로 저장하며, 변환 결과를 SHACL 로 검증할 수 있다고 적는다(판 2.54.0, Apache 2.0).",
      "tag": "사실",
      "source_ids": [
        "ref-689"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: 출력은 \"Turtle, JSON-LD, or an ICDD package\", 변환 뒤 \"validate it with SHACL\". Building Topology Ontology 를 공유 어휘로 언급.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f11",
      "claim": "W3C SHACL 은 RDF 데이터 그래프를 형상(shapes) 그래프의 조건에 대해 검증하는 언어이며, 검증 결과로 sh:conforms(참·거짓)와 위반별 결과를 담은 검증 보고서를 낸다(2017 W3C 권고안).",
      "tag": "사실",
      "source_ids": [
        "ref-692"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "편집자 초안 원본: \"a language for validating RDF graphs against a set of conditions\". 검증 보고서에 sh:conforms 와 sh:result. 열람한 것은 w3c/data-shapes 저장소의 편집자 초안이며 권고안 본문과 문구가 다를 수 있음.",
      "as_of": "2017",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f12",
      "claim": "buildingSMART 의 IDS(Information Delivery Specification)는 IFC 기반 정보 요구사항을 컴퓨터가 해석할 수 있게 정의하는 XML 기반 표준으로, XSD 스키마와 XML 예시로 제공된다.",
      "tag": "사실",
      "source_ids": [
        "ref-697"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: \"computer interpretable XML based standard from buildingSMART to define IFC based Information Delivery Specifications\". 판 번호·검사 범위는 README 에 없어 미확인. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f13",
      "claim": "arXiv 2507.11770(IROS 2025)은 서로 다른 장면 기술 형식(MJCF·URDF·SDF)을 USD 장면 그래프로 통일하고, 웹 기반 도구에서 사람이 온톨로지 개념 클래스로 의미 라벨을 붙인 뒤 지식 그래프로 옮겨 역량 질문(competency question)에 답하게 하는 흐름을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-695"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: unified scene graph in USD, web-based visualization tool for annotating semantic labels with ontology concept classes, translated into a knowledge graph. (원문 미열람)",
      "as_of": "2025-07",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f14",
      "claim": "OBRNIT(Buildings 14(8), 2024)은 BIM 기반 로봇 주행·점검 작업을 위해 로봇, 건물, 주행 작업, 점검 작업의 네 개념 묶음을 두고 가구·HVAC 같은 건물 개념을 ifcOWL 에서 가져온 온톨로지다.",
      "tag": "사실",
      "source_ids": [
        "ref-694"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: four main groups of concepts (robot, building, navigation task, inspection task), ifcOWL 개념 재사용, 천장 누수 점검 사례로 평가. (원문 미열람)",
      "as_of": "2024",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f15",
      "claim": "대한건축학회 논문집 40(1)(2024)의 국내 연구는 기존 주택 평면도를 BIM 기반 3D 모델로 바꾸기 위해 인스턴스 정규화·화이트닝 기반 딥러닝 분할 뒤 경로 계획 기반 벡터 생성 알고리즘으로 벽선을 만드는 2단계 방법을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-693"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 정보 분할(IN·IW 로 일반화 성능 개선) → 경로계획 기반 벡터 생성으로 벽선 신속 생성, 레거시 2D 도면의 BIM 3D 변환 목적. (원문 미열람)",
      "as_of": "2024",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f16",
      "claim": "q3-01 에 대해 확인한 도구·연구를 이 위키가 묶으면 흐름은 (1) 입력 정리(래스터는 크기·여백 정규화와 축척, CAD 는 DXF 와 레이어, BIM 은 IFC) → (2) 인식·벡터화(요소 목록 JSON 이나 벽 구조 그래프) → (3) 공간 그래프 생성(방·구역 분할과 연결: osmAG, IndoorGML, building.yaml) → (4) 온톨로지 적재(BOT 등 RDF 와 SHACL 검증)로 나뉘고, BIM 입력은 (2)를 건너뛸 수 있으며, 충전소·스테이션 같은 운영 요소는 확인한 흐름 어디에서도 자동으로 채워지지 않는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-084",
        "ref-070",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f1(osmAG 단계), f2·f3(벡터화 출력), f8(building.yaml→주행 그래프·월드), f9(IFC→IndoorGML), f10·f11(RDF 적재·SHACL)을 이 위키가 대응시킨 종합. 이 네 단계 구분을 제시한 단일 출처는 확인하지 못함.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f17",
      "claim": "확인한 사례를 종합하면 사람 검토는 (a) 처리 전 입력 파라미터 확정(축척·좌표 기준점·레이어 대응), (b) 인식 뒤 불확실한 요소만 골라 벡터 공간에서 보정, (c) 공간 그래프에 운영 요소(충전소·스테이션·대기 지점)와 장소 이름을 주석, (d) 온톨로지 적재 전 검증 보고서 위반 확인의 네 지점에 둘 수 있을 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-084",
        "ref-691",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "근거: 입력 파라미터 설정(f1), 불확실성 기반 전문가 검토(f5), 벡터 공간 전문가 보정(f4), 반복 피드백·스키마 검증(f6), LLM 뒤 사람 검수(f7), 경유점 속성 수동 주석(f8), SHACL 보고서(f11), 온톨로지 라벨 사람 주석(f13). 검토 지점의 효과를 비교 측정한 자료는 찾지 못함.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f18",
      "claim": "분류 원문 질문(‘3층 출하 대기장’을 같은 장소로 인식)과 관련해, 확인한 흐름에서 방 이름은 CAD 문자 추출이 기본으로 꺼져 있거나 래스터 문자 인식에 기대므로 업무 장소 이름과 공간 노드를 잇는 일은 공간 그래프 생성 뒤 사람 확인 단계에 두어야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-084",
        "ref-079",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "osmAG 의 문자 기반 이름 붙이기는 선택 기능·기본 꺼짐(f1), Open-RMF 장소·경유점 이름은 사람 입력(f8), 장면 그래프 의미 라벨도 사람 주석(f13).",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "완료·인계"
    }
  ...
  "sources": [
    {
      ...
      "title": "IFCtoLBD — README (IFCtoLBD converts IFC (Industry Foundation Classes STEP formatted files into the Linked Building Data ontologies)",
      "published": null,
      "url": "https://github.com/jyrkioraskari/IFCtoLBD",
      "type": "오픈소스 문서",
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC(STEP·XML·JSON)를 BOT 등 링크드 빌딩 데이터 RDF(Turtle·JSON-LD·ICDD)로 변환하고 SHACL 검증을 지원하는 오픈소스 변환기의 README. 판 2.54.0, Apache 2.0.",
      "fetched": true,
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 다중 모달 LLM 다중 에이전트와 사람 피드백·스키마 검증으로 손그림 평면도를 JSON 레이아웃과 BIM 으로 바꾸는 흐름과 10장 실험 결과.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-691",
      "org": "Jakubik, J., Hemmer, P., Vössing, M., Blumenstiel, B., Bartos, A., & Mohr, K.",
      "title": "Designing a Human-in-the-Loop System for Object Detection in Floor Plans",
      "published": "2022",
      "url": "https://ojs.aaai.org/index.php/AAAI/article/view/21522",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 평면도 기호 검출에서 기호별 불확실성으로 전문가 판단을 받을 대상을 고르는 사람 참여 루프 시스템(AAAI 36(11)).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "RDF 데이터 그래프를 형상 그래프로 검증하고 sh:conforms 와 결과를 담은 검증 보고서를 내는 W3C 권고안. 열람은 w3c/data-shapes 저장소의 편집자 초안 원본(권고안 본문과 문구가 다를 수 있음).",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-693",
      "org": "대한건축학회 논문집 게재 논문 저자(미확인)",
      "title": "딥러닝과 경로계획 기반의 주택 평면도 3D 모델링 방법",
      "published": "2024",
      "url": "https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART003047128",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 레거시 주택 평면도를 딥러닝 분할과 경로계획 기반 벡터 생성으로 벽선을 만들어 BIM 3D 모델로 바꾸는 국내 연구(대한건축학회 논문집 40(1)).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-694",
      "org": "Buildings(MDPI) 게재 논문 저자(Concordia University, 목록 미확인)",
      "title": "Ontology for BIM-Based Robotic Navigation and Inspection Tasks",
      "published": "2024",
      "url": "https://www.mdpi.com/2075-5309/14/8/2274",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 로봇·건물·주행 작업·점검 작업 개념으로 된 BIM 기반 로봇 주행·점검 온톨로지 OBRNIT(ifcOWL 개념 재사용).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 장면 기술 형식을 USD 장면 그래프로 통일하고 사람이 온톨로지 개념으로 라벨을 붙여 지식 그래프로 옮기는 흐름(IROS 2025).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 래스터 평면도를 벽·문·창문·방의 구조화 JSON 으로 출력하는 시각-언어 모델 벡터화, 외벽 IoU 92.52% 보고.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "title": "IDS — README (Information Delivery Specification)",
      "published": null,
      "url": "https://github.com/buildingSMART/IDS",
      "type": "표준",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "IFC 기반 정보 요구사항을 정의하는 XML 기반 buildingSMART 표준 IDS 의 공식 저장소 README(XSD·XML 예시). 판·검사 범위는 README 에 없음.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "DXF 를 SVG·PNG 로 바꿔 AreaGraph 로 분할하고 osmAG(OSM XML)로 내보내는 파이프라인 README. 단계별 파라미터와 실행 기록.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "512×512 래스터 평면도에서 벽 교차점·선분 구조 그래프를 자기회귀로 예측하는 모델과 데이터 README.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "CAD 도면 선별, 레이어·블록 기반 자동 라벨링, 벡터 공간 전문가 보정의 주석 과정을 설명하는 프로젝트 페이지.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "traffic-editor 가 .building.yaml 을 저장하고 building_map_generator 가 주행 그래프와 Gazebo·Ignition 월드를 만드는 도구 README.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 평면도 배경 위 벽·문·차선 주석과 충전소 등 경유점 속성을 사람이 입력하는 traffic-editor 문서(이번 실행에서 다시 열지 않음).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. IFC 에서 IndoorGML 을 자동 생성하는 오픈소스 도구 논문.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 객체 검출·LLM 분류·사람 검수로 문 검출 데이터셋을 반자동 구축하는 절차.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
  ...
  "page_proposals": [
    {
      ...
      "rationale": "q3-01 답: f1·f2·f3·f4·f5·f6·f7·f8·f9·f10·f11·f12·f13·f14·f15·f16·f17·f18 (신뢰도 low) — 2절 q3-01 상태 답함, 3절 q3-01 소제목 신설({#q3-01}): 입력 정리·인식·벡터화 도구의 입출력(f1 osmAG, f2 Raster-to-Graph, f3 FloorplanVLM, f15 국내 연구), 공간 그래프 생성(f8 building.yaml, f9 IFC→IndoorGML), 온톨로지 적재와 검증(f10 IFCtoLBD, f11 SHACL, f12 IDS, f13 장면 그래프–온톨로지, f14 OBRNIT), 사람 검토 사례(f4·f5·f6·f7), 단계 종합(f16)·검토 지점 종합(f17)·분류 원문 질문(f18)은 추정으로 / 4절 결론·불확실성(검토 지점 효과 측정 자료 없음, 운영 요소 자동화 근거 없음) / 5절 후속 질문 / 6절 완료 조건 현황 / 8절 출처 / 9절 이력"
    },
    {
      "action": "update",
      ...
      "rationale": "아이디어 페이지 5절(트랙 산출물): '처리 흐름과 사람 검토 지점' 소절 신설 — 단계별 입력·출력 표(f1·f2·f3·f8·f9·f10, 종합 f16 추정), 사람 검토 네 지점(f17 추정, 근거 f4·f5·f6·f7·f11), 장소 이름 확인(f18). 공간 그래프 단위(q3-02)·능력 대조(q3-03)·시뮬레이션 초기값(q3-04)은 아직 없음을 명시"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 3 반영 제안 (f1, f8, f9, f16, f17, f18): 6절(주제 페이지 area06-s6)에 도면 처리 흐름의 단계 구분과 장소 이름·운영 요소를 사람이 확인하는 지점"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 3 반영 제안 (f3, f5, f6, f7): 교차 규칙(도면 해석은 6. 지도·공간·위치 모델에 적용)에 따라 시각-언어 모델 벡터화, 불확실성 기반 사람 참여 루프, LLM 다중 에이전트와 사람 피드백을 6. 지도·공간·위치 모델 페이지와 양쪽 연결"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 3 반영 제안 (f10, f11, f12): IFC→링크드 빌딩 데이터 변환(IFCtoLBD), 적재 전 검증 표준 SHACL, IFC 정보 요구 명세 IDS"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 3 반영 제안 (f1, f8, f17): 시운전 전 도면 처리에서 사람이 입력·확인하는 항목(축척·좌표 기준점, 운영 요소 주석, 검증 보고서 확인)"
    }
  ],
  "glossary_candidates": [
    {
  ...
  "track": {
    ...
    "new_questions": [
      ...
      {
        "question": "처리 단계마다 사람이 고친 요소 수와 검토 시간을 기록해 가설 1(인식만으로 대부분 추출)과 가설 3(현장 모델링 시간 단축)을 판정하는 지표로 쓸 수 있는가? (q3-01 에서 파생)",
        "stage": 5,
...
### runs/2026-09-25-54/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: raw README 재열람. 처리 흐름 문구, 파라미터(resolution m/px, door_width·corridor_width, root_node 위도·경도·픽셀 좌표), run_manifest.json(입력·출력·명령·메타데이터), --with-text 기본 꺼짐 모두 원문과 일치. 발행일 미확인, 확인일 2026-09-25. 기존 ref-084 재사용 적정(단계 1·2 페이지와 중복 서술하지 말고 처리 흐름 맥락으로만 인용)."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: raw README 재열람. 'centered 512*512 images', 'structural graphs embedded with floorplan semantics', 'you might need to retrain the model' 원문과 일치. 기준일 2024(README 판)."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(arXiv 정책 차단). 검증 검색 결과로 제목·arXiv 번호·2026-02-06 제출, 구조화 JSON 시퀀스 출력, Floorplan-2M·HQ-300K, SFT·GRPO, 외벽 IoU 92.52% 확인. 벽·문·창문·방 요소 목록과 Qwen2.5-VL-3B 는 검증 스니펫에 직접 나타나지 않음(리서치 스니펫 기준). 성능 수치는 저자 보고 단일 출처로 명시 유지."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 프로젝트 페이지 원본(raw) 재열람. 11,917건 중 5,538건 선별, 'reduces labeling cost by over 50x using CAD layers and blocks', 벡터 공간 전문가 보정 원문과 일치. 50배는 저자 측 수치. 같은 내용이 아이디어 페이지 4절(ref-434)에 이미 있으므로 기존 각주 재사용."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검증 검색 결과로 저자 6인, AAAI 36(11) 12524–12530, 기호별 불확실성 계산으로 어려운 분류에 전문가 지식 획득, 물량 산출 맥락 확인. 기준일 2022."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검증 검색 결과로 저자(Ratul·Acharjee·Park·Sakib), 2025-10-16 제출, 인지 추출→사람 피드백→스키마 검증→BIM 스크립트, 평면도 10장, 벽 검출 약 83%에서 피드백 뒤 거의 완전 정렬, P/R/F1 0.83 이상, RMSE·MAE 0 수렴 확인. 저자 보고 단일 출처."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(재인용). 검증 검색에서 arXiv 2508.07714 DoorDet 존재 확인. 절차 서술은 아이디어 페이지 3절의 기존 문장(ref-077)과 같으므로 새 서술 대신 기존 각주 재사용. 기존 페이지는 '공개 데이터셋이 드물다'는 저자 판단을 [의견]으로 두었으므로 이번 문장은 절차 부분만 [사실]로 쓴다."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: rmf_traffic_editor README(raw) 재열람 — .building.yaml 저장, building_map_generator nav(주행 그래프)·gazebo/ignition(월드) 원문과 일치. ref-079 는 같은 발행 주체라 독립 확인 아님(브리프도 명시). ref-079 는 이번 실행에서 원문 미열람. 단계 2 페이지 q2-03 의 같은 사실과 중복이므로 기존 각주 재사용."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(재인용, 기존 ref-225). 'IFC→IndoorGML 자동 생성 오픈소스 도구'는 기존 검증 문장과 같음. 뒷부분 'BIM 입력은 이미지 인식·벡터화 단계를 거치지 않고 … 바로 변환하는 경로가 있다'는 출처 진술이 아니라 이 위키의 해석이므로 [추정]으로 분리 표기 지시."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: IFCtoLBD README(raw) 열람 — IFC STEP·IFC/XML·IFC/JSON 입력, Turtle·JSON-LD·ICDD 패키지 출력, 'validate it with SHACL', Version 2.54.0, Apache License 2.0, BOT 언급 모두 원문과 일치. 발행일 미확인, 확인일 2026-09-25."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: w3c/data-shapes 저장소 편집자 초안(ED) 원본 열람 — 정의 문구, sh:conforms(결과 없을 때만 true)·sh:result 원문과 일치. 열람본은 권고안(/TR/shacl/, 2017)이 아니라 편집자 초안이므로 페이지·각주에 '편집자 초안으로 확인' 명시 필요. 기준일 2017(권고안)."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: buildingSMART/IDS README(development 브랜치, raw) 열람 — 'Computer interpretable XML based standard from buildingSMART to define IFC based Information Delivery Specifications', XSD(ids.xsd)와 XML 예시 언급 일치. 판 번호 없음(미확인). 발행일 미확인, 확인일 2026-09-25."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검증 검색 결과로 MJCF·URDF·SDF→USD 통합 장면 그래프, 웹 도구에서 온톨로지 개념 클래스로 의미 라벨 주석, 지식 그래프 변환과 역량 질문 응답 확인. 검색 결과는 'IROS 2025 에 제출'로 적으므로 '(IROS 2025)'를 '(IROS 2025 제출)'로 고치도록 지시."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검증 검색 결과로 MDPI Buildings 14(8) 2274, Concordia 대학 저장소 사본, 로봇·건물·주행 작업·점검 작업 네 개념 묶음, Protégé 개발·공개 저장소 확인. '가구·HVAC 같은 건물 개념을 ifcOWL 에서 가져옴'은 검증 스니펫에 없어 미확인 — 해당 구절 삭제 또는 '미확인' 표기 지시."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검증 검색 결과로 제목, 대한건축학회논문집 40(1) 297-303(2024), DOI 10.5659/JAIK.2024.40.1.297, 딥러닝과 경로계획 기반 주택 평면도 3D 모델 생성 확인. '인스턴스 정규화·화이트닝' 세부 기법은 검증 스니펫에 없어 미확인 — 세부 기법명 삭제 또는 '미확인' 표기 지시. 한국 자료 우선 규칙 충족."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "[추정] 유지: f1·f2·f3·f8·f9·f10·f11 을 이 위키가 대응시킨 종합이며 네 단계 구분을 제시한 단일 출처 없음(브리프 명시). 근거 finding 은 모두 살아남음. 신뢰도 low 적정. 근거에 원문 미열람 출처(ref-696·ref-225·ref-079) 포함."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "[추정] 유지: f1·f4·f5·f6·f7·f8·f11·f13 에서 도출한 검토 지점 네 곳은 이 위키의 종합이며 지점별 효과 측정 자료 없음. 근거 finding 모두 살아남음. 신뢰도 low 적정."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "[추정] 유지: 분류 원문 SCM 질문 대응. osmAG 문자 이름 기본 꺼짐(f1, 원문 확인)과 사람 주석 사례(f8·f13) 근거. '래스터 문자 인식에 기댄다'는 단계 2 q2-02 기존 추정과 일치. 흐름 단계 출하 / 완료·인계 적정."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "f1(osmAG 처리 흐름·문자 이름 기본 꺼짐)은 단계 1·2 페이지와 아이디어 페이지 3절의 ref-084 문장과 겹친다 — 기존 각주 재사용, 처리 흐름 맥락으로만 서술",
      "f2(Raster-to-Graph 512×512 정규화)는 단계 2 q2-02 와 스키마 초안 6절의 ref-070 문장과 겹친다 — 기존 각주 재사용",
      "f4(ArchCAD-400K 레이어·블록 자동 라벨링·벡터 공간 보정)는 아이디어 페이지 4절의 ref-434 문장과 겹친다 — 새 수치(50배, 5,538/11,917)만 추가",
      "f7(DoorDet 반자동 절차)은 아이디어 페이지 3절의 ref-077 문장과 겹친다 — 기존 문장 재사용",
      "f8(building.yaml→주행 그래프·월드)은 아이디어 페이지 4절 '관제·ROP 수용 형식'의 ref-441 문장과 겹친다",
      "f9(ifc2indoorgml)는 아이디어 페이지 3절 표의 ref-225 행과 겹친다",
      "새 참고문헌 ref-692(SHACL)는 용어집에 shacl 항목이 있어 기존 참고문헌과 URL 이 같을 수 있다 — 같은 URL 이면 퍼블리셔가 기존 id 로 합친다"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "track_checks": {
    ...
    "backlog_duplicates": [
      "처리 단계마다 사람이 고친 요소 수와 검토 시간을 기록해 가설 1(인식만으로 대부분 추출)과 가설 3(현장 모델링 시간 단축)을 판정하는 지표로 쓸 수 있는가? (q3-01 에서 파생) — q5-02 와 중복"
    ],
    "stage_tag_issues": [],
...
### docs/references/index.md (요약: 이번 대상 페이지가 인용한 110건 / 전체 481건. 목록에 없는 출처는 새 id 로 적는다 — 같은 URL 이 이미 있으면 퍼블리셔가 기존 id 로 합친다)
...
### docs/open-questions.md (요약: 대상 영역 [3, 5, 6, 8, 10, 15, 16, 21, 22, 23, 24, 27, 28] 에 걸린 45건 / 전체 75건)
...
### runs/2026-09-25-54/docs_tree.txt
...
track: floorplan-recognition
stage: 3
related_areas: [6, 5, 15, 16, 22]
tags: [처리 흐름, 공간 그래프, 온톨로지 적재, 시뮬레이션 초기값]
status: seed
created: 2026-09-25
updated: 2026-09-25
sources: []
...
이 단계의 시작 질문 4개다. 모두 구축자가 이 단계의 밝힐 것에서 정한 시작 질문이다. [가정] 상태와 답 위치는 [질문 백로그](question-backlog.md)와 일치시키며, 백로그의 "조사 중"은 이 표에서 "열림"으로 표시하고 "폐기"는 표에서 빼고 백로그에만 남긴다. 답은 3절의 질문 id로 시작하는 소제목에 실리고, 답 위치 칸에는 그 앵커 또는 주제 페이지 링크를 적는다. 제기 근거 칸에는 finding id 또는 "사용자"만 쓴다.

| id | 질문 | 상태 | 제기 근거 | 답한 실행 id | 답 위치 |
|---|---|---|---|---|---|
| q3-01 | 인식 → 벡터화 → 공간 그래프 생성 → 온톨로지 적재의 흐름에서 단계마다 입력·출력은 무엇이고 사람 검토는 어디에 두는가? | 열림 | 사용자 | | |
...
| 완료 조건 | 충족 여부 | 근거 | 검증 판정 |
|---|---|---|---|
| 처리 흐름·핵심 구성 요소·다른 아이디어와의 연결이 [아이디어 3. 건축 도면 자동 인식](../../ideas/floorplan-recognition.md)의 "5. 구현 가설" 절에 실림 | 미충족 | 이 단계를 다룬 트랙 실행이 아직 없다 | 없음(구축 시점, 판정 전) |
| [공간 그래프 스키마 초안](space-graph-schema-draft.md)이 근거 finding과 함께 v0.1 이상으로 갱신됨 | 미충족 | 이 단계를 다룬 트랙 실행이 아직 없다 | 없음(구축 시점, 판정 전) |
...
<!-- auto:page-status:start -->
> 페이지 상태: published · 신뢰도: medium · 페이지 버전: 8 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
원천: [질문 백로그](../tracks/floorplan-recognition/question-backlog.md)([건축 도면 자동 인식](../tracks/floorplan-recognition/index.md) 트랙) · 열림 24건 · 답함 7건 · 폐기 3건
...
````json
{
  ...
  "outline": [
    {
      ...
      "section": "2. 질문 목록",
      "budget_chars": 900,
      "summary": "q3-01을 답함(실행 2026-09-25-54, #q3-01)으로 바꾸고 백로그의 단계 3 질문 q3-05·q3-06과 새 질문 q3-07·q3-08을 표에 더한다."
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md",
      "section": "3. 조사 결과",
      "budget_chars": 3600,
      "summary": "확인한 도구·연구를 이 위키가 묶으면 처리 흐름은 입력 정리 → 인식·벡터화 → 공간 그래프 생성 → 온톨로지 적재로 나뉘고 사람 검토는 네 지점에 둘 수 있을 것으로 보인다. [추정][^ref-084][^ref-692]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 900,
      "summary": "결론은 모두 이 위키의 종합([추정])이며 검토 지점별 효과를 측정한 자료는 없다. 온톨로지 초안 변경 없음(v0.6 유지).",
      "planned_findings": [
        "f16",
        "f17",
        "f18"
    ...
    },
    {
      ...
      "budget_chars": 500,
      "summary": "후속 질문 q3-07(SHACL 검증 관문, f11)·q3-08(중간 산출물 형식, f16). 가설 판정 지표 질문은 q5-02와 중복이라 등록하지 않았다."
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md",
      "section": "6. 완료 조건 충족 현황",
      "budget_chars": 700,
      "summary": "처리 흐름만 아이디어 5절에 실렸고 나머지 조건은 미충족, 전환 미승인."
    },
    {
      "path": "docs/ideas/floorplan-recognition.md",
      "section": "5. 구현 가설",
      "budget_chars": 1800,
      "summary": "처리 흐름의 네 단계와 사람 검토 네 지점을 [추정]으로, 근거 사례를 각 태그대로 싣고 핵심 구성 요소·다른 아이디어와의 연결은 아직 조사되지 않음으로 둔다. [추정][^ref-084][^ref-689]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "q3-01 답함(처리 흐름 네 단계·사람 검토 네 지점, 결론은 추정), 2·3·4·5·6·7·8·9절 갱신, 후속 질문 2건(q3-07·q3-08). H1 아래 단계 상태 줄은 H2 절이 아니라 패치로 보낼 수 없어 '진행 중 · 열린 질문: 7건 · 답한 질문: 1건 · 완료 조건: 미충족 · 마지막 실행: 2026-09-25'로의 갱신을 코드 반영에 맡긴다",
      "patches": [
        {
          "section": "2. 질문 목록",
          "action": "replace",
        ...
        },
        {
          "section": "8. 출처",
          "action": "replace",
          "content": "[^ref-084]: Zhang, J. (jiajiezhang7 GitHub), osmAG-from-cad — README (CAD-to-osmAG pipeline), 미확인, https://github.com/jiajiezhang7/osmAG-from-cad, 접근일 2026-09-25\n[^ref-070]: Hu, S. 외, Raster-to-Graph — README (Raster-to-Graph: Floorplan Recognition via Autoregressive Graph Prediction with an Attention Transformer), 2024, https://github.com/SizheHu/Raster-to-Graph, 접근일 2026-09-25\n[^ref-696]: arXiv 2602.06507 저자(미확인), FloorplanVLM: A Vision-Language Model for Floorplan Vectorization, 2026-02, https://arxiv.org/abs/2602.06507, 접근일 2026-09-25 (원문 미열람)\n[^ref-693]: 대한건축학회논문집 40(1), 297-303(DOI 10.5659/JAIK.2024.40.1.297) 게재 논문 저자(미확인), 딥러닝과 경로계획 기반의 주택 평면도 3D 모델링 방법, 2024, https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART003047128, 접근일 2026-09-25 (원문 미열람)\n[^ref-441]: Open Robotics (open-rmf), rmf_traffic_editor — README, 미확인, https://github.com/open-rmf/rmf_traffic_editor, 접근일 2026-09-25\n[^ref-079]: Open Robotics, Traffic Editor - Programming Multiple Robots with ROS 2, 미확인, https://osrf.github.io/ros2multirobotbook/traffic-editor.html, 접근일 2026-09-25 (원문 미열람)\n[^ref-225]: Diakité, A. A., Díaz-Vilariño, L., Biljecki, F., Isikdag, Ü., Simmons, S., Li, K., & Zlatanova, S., IFC2INDOORGML: An Open-Source Tool for Generating IndoorGML from IFC, 2022, https://isprs-archives.copernicus.org/articles/XLIII-B4-2022/295/2022/, 접근일 2026-09-25 (원문 미열람)\n[^ref-689]: Oraskari, J. (jyrkioraskari GitHub), IFCtoLBD — README (IFCtoLBD converts IFC (Industry Foundation Classes STEP formatted files into the Linked Building Data ontologies), 미확인, https://github.com/jyrkioraskari/IFCtoLBD, 접근일 2026-09-25\n[^ref-692]: W3C RDF Data Shapes Working Group, Shapes Constraint Language (SHACL) (W3C data-shapes 저장소 편집자 초안으로 확인, 권고안(2017) 본문과 문구가 다를 수 있음), 2017, https://www.w3.org/TR/shacl/, 접근일 2026-09-25\n[^ref-697]: buildingSMART (buildingSMART/IDS GitHub), IDS — README (Information Delivery Specification), 미확인, https://github.com/buildingSMART/IDS, 접근일 2026-09-25\n[^ref-695]: arXiv 2507.11770 저자(미확인), Generating Actionable Robot Knowledge Bases by Combining 3D Scene Graphs with Robot Ontologies, 2025-07, https://arxiv.org/abs/2507.11770, 접근일 2026-09-25 (원문 미열람)\n[^ref-694]: Buildings(MDPI) 게재 논문 저자(Concordia University, 목록 미확인), Ontology for BIM-Based Robotic Navigation and Inspection Tasks, 2024, https://www.mdpi.com/2075-5309/14/8/2274, 접근일 2026-09-25 (원문 미열람)\n[^ref-434]: ArchiAI Lab (ArchCAD-400K 프로젝트), ArchCAD-400k: A Large-Scale CAD drawings Dataset and New Baseline for Panoptic Symbol Spotting — project page, 미확인, https://archiai-lab.github.io/ArchCAD.github.io/, 접근일 2026-09-25\n[^ref-691]: Jakubik, J., Hemmer, P., Vössing, M., Blumenstiel, B., Bartos, A., & Mohr, K., Designing a Human-in-the-Loop System for Object Detection in Floor Plans, 2022, https://ojs.aaai.org/index.php/AAAI/article/view/21522, 접근일 2026-09-25 (원문 미열람)\n[^ref-690]: Ratul, A. K., Acharjee, S., Park, S., & Sakib, M. N., Sketch2BIM: A Multi-Agent Human-AI Collaborative Pipeline to Convert Hand-Drawn Floor Plans to 3D BIM, 2025-10, https://arxiv.org/abs/2510.20838, 접근일 2026-09-25 (원문 미열람)\n[^ref-077]: DoorDet 저자(arXiv 2508.07714), DoorDet: Semi-Automated Multi-Class Door Detection Dataset via Object Detection and Large Language Models, 2025-08, https://arxiv.org/abs/2508.07714, 접근일 2026-09-25 (원문 미열람)"
        },
        {
          "section": "9. 이력",
          "action": "replace",
    ...
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "5. 구현 가설 절 첫 작성: 처리 흐름과 사람 검토 지점(q3-01, 실행 2026-09-25-54, 추정 중심), 핵심 구성 요소·다른 아이디어와의 연결은 아직 조사되지 않음",
      "patches": [
        {
          "section": "5. 구현 가설",
          "action": "replace",
  ...
  "glossary_updates": [
    {
      ...
      "term_en": "Human-in-the-Loop (HITL)",
      "definition": "자동 처리 결과 가운데 불확실하거나 중요한 부분을 사람이 확인·보정하고 그 판단을 다시 처리 흐름에 넣는 설계 방식이다.",
      "description": "평면도 기호 검출에서는 기호별 불확실성으로 전문가 판단을 받을 대상을 고르는 설계가 제안되었고, 손그림 평면도의 BIM 변환에서는 사람 피드백과 스키마 검증으로 결과를 반복 보정하는 흐름이 제안되었다.",
      "related_areas": [
        6,
        27,
        18
    ...
    {
      "action": "new",
      "slug": "information-delivery-specification",
      "term_ko": "정보 전달 명세",
      "term_en": "Information Delivery Specification (IDS)",
      "definition": "buildingSMART 가 정한, IFC 모델이 갖춰야 할 정보 요구사항을 컴퓨터가 해석할 수 있게 적는 XML 기반 표준이다.",
      "description": "XSD 스키마와 XML 예시로 제공된다. 판 번호와 검사 범위는 공식 저장소 README 에서 확인되지 않았다(2026-09-25 확인).",
      "related_areas": [
        28,
        6
      ],
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 다중 모달 LLM 다중 에이전트와 사람 피드백·스키마 검증으로 손그림 평면도를 JSON 레이아웃과 BIM 으로 바꾸는 흐름과 10장 실험 결과(저자 보고).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md",
        "docs/ideas/floorplan-recognition.md"
    ...
    },
    {
      "id": "ref-692",
      "org": "W3C RDF Data Shapes Working Group",
      "title": "Shapes Constraint Language (SHACL) (W3C data-shapes 저장소 편집자 초안으로 확인, 권고안(2017) 본문과 문구가 다를 수 있음)",
      "published": "2017",
      "url": "https://www.w3.org/TR/shacl/",
      "type": "표준",
      "reliability": "high",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 레거시 주택 평면도를 딥러닝 분할과 경로계획 기반 벡터 생성으로 벽선을 만들어 BIM 3D 모델로 바꾸는 국내 연구(세부 기법 미확인).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md"
      ]
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 로봇·건물·주행 작업·점검 작업 개념 묶음으로 된 BIM 기반 로봇 주행·점검 온톨로지 OBRNIT(ifcOWL 개념 재사용 여부 미확인).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md"
      ]
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 장면 기술 형식을 USD 장면 그래프로 통일하고 사람이 온톨로지 개념으로 라벨을 붙여 지식 그래프로 옮기는 흐름(IROS 2025 제출).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md",
        "docs/ideas/floorplan-recognition.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 래스터 평면도를 벽·문·창문·방의 구조화 JSON 으로 출력하는 시각-언어 모델 벡터화, 외벽 IoU 92.52% 보고(저자 보고, 단일 출처).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md",
        "docs/ideas/floorplan-recognition.md"
  ...
  "track_updates": {
    "stage_page": "docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md",
    ...
    "log_entry": "답한 질문: q3-01(근거 f1~f18, 결론 f16·f17·f18 은 [추정]) / 새 질문: q3-07(f11), q3-08(f16); 가설 판정 지표 질문은 q5-02 와 중복으로 등록하지 않음 / 온톨로지 변경: 없음(v0.6 유지 — q3-01 은 처리 흐름 질문이며 공간 그래프 개념·관계를 새로 뒷받침하는 finding 없음) / 완료 조건 평가: 미충족(부족: 아이디어 3 5절의 핵심 구성 요소·다른 아이디어와의 연결, 공간 그래프 스키마 초안의 단계 3 근거 갱신, 실험 계획; 막힌 질문 q3-02·q3-03·q3-04·q3-05·q3-06) / 세부영역 반영 제안: 6. 지도·공간·위치 모델 1건, 27. AI·학습·적응과 모델 운영 2건, 28. 표준·상호운용성·다사업자 거버넌스 1건, 21. 온보딩·설정·현장 시운전 1건, 5. 로봇 능력·작업 온톨로지 1건(총 6건) / 다음 실행 제안: q3-02(공간 그래프 단위), 이어서 q3-07·q3-08 / 비고: CLI 지정 질문 실행이라 트랙 현재 단계(단계 1)는 바꾸지 않음. 단계 3 페이지 H1 아래 상태 줄은 H2 절이 아니어서 패치로 보내지 못했으며 '단계 상태: 진행 중 · 열린 질문: 7건 · 답한 질문: 1건 · 완료 조건: 미충족 · 마지막 실행: 2026-09-25'로 갱신이 필요하다.",
    "overview_progress": "단계 3 진행 중(CLI 지정 질문, 트랙 현재 단계는 단계 1 유지) — 열린 질문 7, 답함 1, 완료 조건 미충족"
  },
  "area_reflection_proposals": [
    {
      "area_no": 6,
      "section": "6. 대표 접근법과 기술",
      "summary": "도면 처리 흐름을 입력 정리 → 인식·벡터화 → 공간 그래프 생성 → 온톨로지 적재로 나누는 이 위키의 종합([추정], f16)과 BIM 입력의 직접 변환 경로(ifc2indoorgml, f9), 사람 검토 네 지점([추정], f17), 업무 장소 이름과 공간 노드의 연결을 공간 그래프 생성 뒤 사람 확인에 두는 방향([추정], f18, osmAG 문자 이름 기본 꺼짐 f1, traffic-editor 경유점 주석 f8). 근거 실행 2026-09-25-54."
    },
    {
      "area_no": 27,
      "section": "6. 대표 접근법과 기술",
    ...
    },
    {
      "area_no": 27,
      "section": "8. 대표 연구와 자료",
      "summary": "FloorplanVLM(ref-696), Jakubik 외 AAAI 2022(ref-691), Sketch2BIM(ref-690), DoorDet(ref-077)를 도면 해석 연구로 등재(모두 원문 미열람, 수치는 저자 보고)."
    },
    {
      "area_no": 28,
    ...
    },
    {
      "area_no": 21,
      "section": "6. 대표 접근법과 기술",
      "summary": "시운전 전 도면 처리에서 사람이 입력·확인하는 항목: 축척·좌표 기준점 파라미터(osmAG-from-cad, f1), 운영 요소·경유점 주석(traffic-editor, f8), 적재 전 검증 보고서 확인(SHACL, f11) — 네 지점 구분은 [추정](f17)."
    },
    {
      "area_no": 5,
      "section": "8. 대표 연구와 자료",
      "summary": "건물·장면 정보를 로봇 온톨로지·지식 그래프에 적재한 연구: OBRNIT(로봇·건물·주행 작업·점검 작업 네 개념 묶음, ifcOWL 재사용 여부 미확인, f14), 장면 그래프를 USD로 통일하고 사람이 온톨로지 개념으로 라벨을 붙이는 흐름(arXiv 2507.11770, IROS 2025 제출, f13). 모두 원문 미열람."
    }
  ],
  "standards_updates": [
    {
      "name": "IFCtoLBD (IFC → 링크드 빌딩 데이터 변환기, 판 2.54.0)",
      "kind": "오픈소스",
      "org": "Oraskari, J. (jyrkioraskari GitHub)",
      "url": "https://github.com/jyrkioraskari/IFCtoLBD",
      "related_areas": [
      ...
      "summary": "IFC STEP·XML·JSON 을 BOT 등 링크드 빌딩 데이터 RDF(Turtle·JSON-LD·ICDD)로 변환하고 SHACL 검증을 지원하는 오픈소스 도구(Apache 2.0).",
      "ref_id": "ref-689"
    },
    {
      ...
      "summary": "RDF 데이터 그래프를 형상 그래프의 조건으로 검증해 sh:conforms 와 위반 결과를 담은 검증 보고서를 내는 W3C 권고안(2017). 이 위키는 편집자 초안으로 확인했다.",
      "ref_id": "ref-692"
    },
    {
      "name": "IDS (Information Delivery Specification)",
      "kind": "표준",
      "org": "buildingSMART",
      "url": "https://github.com/buildingSMART/IDS",
      "related_areas": [
      ...
      "summary": "IFC 기반 정보 요구사항을 컴퓨터가 해석할 수 있게 정의하는 XML 기반 표준(XSD·XML 예시 제공, 판 번호 미확인).",
      "ref_id": "ref-697"
    }
  ]
}
...
### runs/2026-09-25-54/pages/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-084, ref-070, ref-696, ref-693, ref-441, ref-079, ref-225, ref-689, ref-692, ref-697, ref-695, ref-694, ref-434, ref-691, ref-690, ref-077]
version: 2
confidence: low
last_run: 2026-09-25
---
...
### runs/2026-09-25-54/pages/ideas/floorplan-recognition.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-062, ref-063, ref-064, ref-065, ref-066, ref-067, ref-068, ref-069, ref-070, ref-071, ref-072, ref-073, ref-074, ref-075, ref-076, ref-077, ref-078, ref-079, ref-081, ref-082, ref-083, ref-084, ref-085, ref-086, ref-109, ref-120, ref-220, ref-221, ref-222, ref-223, ref-224, ref-225, ref-226, ref-227, ref-046, ref-212, ref-213, ref-214, ref-215, ref-216, ref-217, ref-218, ref-219, ref-241, ref-105, ref-163, ref-265, ref-267, ref-268, ref-271, ref-274, ref-156, ref-157, ref-158, ref-331, ref-332, ref-333, ref-334, ref-335, ref-336, ref-338, ref-339, ref-340, ref-341, ref-342, ref-345, ref-419, ref-420, ref-421, ref-422, ref-423, ref-424, ref-425, ref-426, ref-427, ref-428, ref-429, ref-430, ref-432, ref-433, ref-434, ref-435, ref-436, ref-440, ref-441, ref-442, ref-689, ref-690, ref-691, ref-692, ref-695, ref-696]
last_run: 2026-09-25
version: 9
---
...
도면 처리 흐름은 입력 정리 → 인식·벡터화 → 공간 그래프 생성 → 온톨로지 적재로 나눌 수 있고, BIM 입력은 인식·벡터화를 건너뛸 수 있으며, 충전소·스테이션 같은 운영 요소는 확인한 흐름 어디에서도 자동으로 채워지지 않는 것으로 보인다. 이 구분은 확인한 도구·연구를 이 위키가 묶은 것이다. [추정][^ref-084][^ref-070][^ref-696][^ref-441][^ref-225][^ref-689][^ref-692] 이 절은 [q3-01 답](../tracks/floorplan-recognition/stage-3-implementation-hypothesis.md#q3-01)(실행 2026-09-25-54)의 요약이며, 문장별 상세는 단계 페이지에 있다. 핵심 구성 요소와 다른 아이디어와의 연결은 [단계 3. 구현 가설 설계](../tracks/floorplan-recognition/stage-3-implementation-hypothesis.md)와 [단계 4. 지도 변환 보정과 현장 정합](../tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md) 실행이 채운다.
...
### runs/2026-09-25-54/pages/tracks/floorplan-recognition/index.md
...
### runs/2026-09-25-54/verification2.json
...
```json
{
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "f1·f2·f4·f7·f8·f9 는 1차 지시대로 기존 각주(ref-084·ref-070·ref-434·ref-077·ref-441·ref-225)를 재사용해 처리 흐름 맥락으로만 요약했다 — 중복 서술 문제 없음",
      "standards_updates 의 SHACL 항목은 용어집 shacl 과 같은 대상이다 — 표준 목록에 같은 항목이 이미 있으면 퍼블리셔가 기존 항목에 ref-692 만 더하도록 합친다"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "단계 3 페이지(docs/tracks/floorplan-recognition/stage-3-implementation-hypothesis.md) H1 아래 상태 줄을 '> 단계 상태: 진행 중 · 열린 질문: 7건 · 답한 질문: 1건 · 완료 조건: 미충족 · 마지막 실행: 2026-09-25'로 고친다. 이 줄은 H2 절이 아니므로 절 패치 대신 머리 영역 패치 또는 페이지 전문으로 보낸다. 지금 페이지에는 입력 시드의 '대기 · 열린 질문: 4건 · 답한 질문: 0건 · 마지막 실행: 없음'이 그대로 남아 있어 2절 표(열림 7, 답함 1)와 맞지 않는다. 이 줄을 채우는 퍼블리셔 단계도 없어서, 코드 반영에 맡기면 틀린 값이 그대로 게시된다.",
    "단계 3 페이지 6절 표 첫 행(처리 흐름이 아이디어 3. 건축 도면 자동 인식의 '5. 구현 가설' 절에 실림)의 검증 판정 칸을 '미충족 · 미승인'에서 '충족 · 전환 미승인'으로 고친다. 이번 실행에서 아이디어 페이지 5절에 처리 흐름 소절이 실제로 실렸으므로, 행을 나눈 이상 이 하위 항목은 충족으로 판정한다(단계 완료 조건 전체는 계속 미충족이다).",
    "단계 3 페이지 6절 아래 줄을 '다음 단계로 전환: 아니오(완료 조건 미충족: 아이디어 5절 핵심 구성 요소·다른 아이디어와의 연결, 스키마 단계 3 근거 갱신, 실험 계획 없음; 막힌 질문 q3-02·q3-03·q3-04·q3-05·q3-06)'로 고친다. 6절 표 둘째 행과 log_entry 는 핵심 구성 요소를 미충족으로 적었는데 전환 줄에는 빠져 있다. 1차 지시 문구가 이 항목을 빠뜨렸고, 이번에 바로잡는다."
  ],
  "confidence": "low",
  "verification_note": "판정: 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경(fetch_mode mirror_only)에서 검증됐다. 확인 18건, 미확인 0건, 교차 확인 0건. 강등: 없음(f9 뒷부분은 [추정]으로 분리, f14·f15 의 미확인 세부는 '미확인' 표기로 처리). 원문 미열람 출처: ref-079, ref-077, ref-225, ref-690, ref-691, ref-693, ref-694, ref-695, ref-696. 원문을 연 출처는 ref-084·ref-070·ref-434·ref-441·ref-689·ref-697 의 GitHub 원본과 ref-692 의 W3C 편집자 초안이다. 주의: q3-01 의 결론(처리 흐름의 네 단계 구분, 사람 검토 네 지점, 장소 이름의 사람 확인)은 확인한 도구·연구를 이 위키가 묶은 추정이다. 검토 지점별 효과를 측정한 자료는 찾지 못했다. FloorplanVLM·Sketch2BIM·ArchCAD-400K 의 성능·비용 수치는 저자 보고 단일 출처다. SHACL 은 권고안이 아니라 편집자 초안으로 확인했다. 새 질문 가운데 가설 판정 지표 질문은 q5-02 와 중복이어서 등록하지 않았다. 온톨로지 변경 승인: 없음(제안 없음, v0.6 유지) / 거부: 없음. 단계 완료 조건: 미충족(부족: 아이디어 3. 건축 도면 자동 인식 5절의 핵심 구성 요소와 다른 아이디어와의 연결, 공간 그래프 스키마 초안의 단계 3 근거 갱신, 실험 페이지의 실험 계획; 막힌 질문 q3-02·q3-03·q3-04·q3-05·q3-06). 처리 흐름 하위 항목은 이번에 충족됐다. 단계 전환: 미승인(완료 조건 미충족, 막힌 질문 있음). CLI 지정 실행이므로 트랙 현재 단계 표기(단계 1)는 바꾸지 않는다. / 2차 수정 후 재검증. 드리프트 없음, [분류원문] 보존, 섹션 순서 준수, 링크 유효. 1차 수정 지시 13건은 모두 이행됐다. 남은 수정은 세 가지다: 단계 3 페이지 H1 아래 상태 줄이 시드 값 그대로 남아 있음, 6절 첫 행 검증 판정, 전환 줄에서 빠진 핵심 구성 요소. 참고: 5. 로봇 능력·작업 온톨로지 반영 제안(f13·f14)은 브리프 page_proposals 에 없던 것을 스토리텔러가 더했으나, 검증된 finding 에 근거한 제안이라 허용했다. 단계 페이지에서 IoU·BIM·RDF 같은 약어를 첫 등장 때 풀어 쓰지 않은 점은 사소한 표기 의견이다.",
  "retry_reason": null,
  "track_checks": {
    "standard_sources_ok": true,
    "vendor_claims_tagged": true,
...
- 트랙 검사(track_checks): {"standard_sources_ok": true, "vendor_claims_tagged": true, "ontology_changes_grounded": true, "backlog_duplicates": ["처리 단계마다 사람이 고친 요소 수와 검토 시간을 기록해 가설 1(인식만으로 대부분 추출)과 가설 3(현장 모델링 시간 단축)을 판정하는 지표로 쓸 수 있는가? (q3-01 에서 파생) — q5-02 와 중복"], "stage_tag_issues": [], "completeness_wording_ok": true, "stage_complete": false, "stage_transition_approved": false}
