...
```json
{
  ...
  "outline": [
    ...
    },
    {
      ...
      "budget_chars": 600,
      "summary": "VDA 5050은 손실을 전제한 무선망 통신을[사실][^ref-031], KubeEdge는 단절 중 엣지 자율 동작을[사실][^ref-471] 정하며, 관련 표준·오픈소스를 표로 정리한다.",
      "planned_findings": [
        "f1",
        "f2",
        "f4",
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "섹션 3~11 신규 작성(외부망 단절 운영 범위, VDA 5050 무선망 전제·QoS·베이스/호라이즌, ROS 2 DDS·Zenoh, 엣지 단절 운영, 계산 배치 선례, 피킹 시나리오), 페이지 상태 표식 추가, 조건부 승인 수정 16건 이행, 4·6·7·8·10절 주제 페이지 분리 상태 유지. 2차 수정: 7절 첫 문장을 VDA 5050·KubeEdge 두 사례에 한정한 [사실] 문장으로 교체"
    },
    {
    ...
    },
    {
      ...
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 11. 분산 시스템·통신·컴퓨팅 구조 의 \"8. 대표 연구와 자료\" 절(855자)을 옮겼다. 2차 수정: NIST 항목의 쓰임새 평가를 [의견]으로, Gilbert·Lynch 항목의 적용 구절을 [추정]으로 분리"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area11-s7.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 11. 분산 시스템·통신·컴퓨팅 구조 의 \"7. 관련 표준·프레임워크·오픈소스\" 절(769자)을 옮겼다. 2차 수정: 세 줄 요약·본문 첫 문장의 '대부분' 일반화를 VDA 5050·KubeEdge 두 사례에 한정한 [사실] 문장으로 교체"
    },
    {
      "path": "docs/topics/2026/2026-09-25-area11-s10.md",
      "action": "create",
      "status": "draft",
      "diff_summary": "자동 분리: 11. 분산 시스템·통신·컴퓨팅 구조 의 \"10. 다른 연구영역과의 연결 (번호와 이름을 함께 표기)\" 절(737자)을 옮겼다. 형식 수정: 본문 세부영역 링크 8개를 주제 페이지 위치 기준(../../categories/…) 경로로 고쳤다. 2차 수정: 27. AI·학습·적응과 모델 운영 항목을 ref-476 사실 문장과 태그 없는 연결 문장으로 분리"
    }
  ],
  "changelog_entry": "2026-09-25 | 11. 분산 시스템·통신·컴퓨팅 구조 | 섹션 3~11 신규 작성(외부망 단절 운영 범위 추론, VDA 5050 무선망 전제·QoS·베이스/호라이즌, ROS 2 DDS·Zenoh, 엣지 단절 운영, 계산 배치 선례, 피킹 시나리오), 1차 조건부 승인 수정 16건·2차 수정 4건 이행 | run 2026-09-25-27",
  "index_updates": {
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "cited_by": [
        "docs/topics/2026/2026-09-25-area11-s6.md"
      ],
      "source_unopened": true
    },
    {
      ...
      "summary": "ROS 2 가 DDS 를 미들웨어로 채택한 이유를 설명하는 설계 문서. 중앙 마스터 없는 분산 발견과 QoS 조정으로 불안정한 무선망에 대응하는 점을 다룬다.",
      "cited_by": [
        "docs/topics/2026/2026-09-25-area11-s4.md"
      ],
      "source_unopened": false
    },
    {
    ...
    },
    {
      ...
      "summary": "원문 미열람. 클라우드가 로봇·자동화에 주는 네 가지 이점과 과제를 정리한 서베이(IEEE Transactions on Automation Science and Engineering).",
      "cited_by": [
        "docs/topics/2026/2026-09-25-area11-s8.md"
      ],
      "source_unopened": true
    },
    {
  ...
  "fixes_applied": [
    ...
    "2차: s8 NIST 항목 — '계층형 모델을 제시했다. [사실][^ref-474]' 문장과 '이 위키는 이 모델을 현장 서버의 역할을 설명하는 기준 틀로 쓴다. [의견]' 문장으로 나눴다.",
    "2차: s8 Gilbert·Lynch 항목 — CAP 증명을 [사실][^ref-481] 문장으로 두고 '단절 중 운영과 재연결 뒤 기록 맞춤이 왜 필요한지의 이론적 근거가 될 것으로 보인다'를 별도 [추정][^ref-481] 문장으로 분리했다.",
    "2차: 세부영역 페이지 7절 첫 문장과 s7 세 줄 요약·본문 첫 문장 — '대부분' 일반화를 빼고 'VDA 5050(3.0.0 기준)은 … 무선망을 전제한 통신 방식을 정한다. [사실][^ref-031]'와 'KubeEdge는 … 엣지가 자율 동작하는 구조를 둔다. [사실][^ref-471]' 두 사례 문장으로 고쳤다.",
    "2차: s10 27. AI·학습·적응과 모델 운영 항목 — 'Kehoe 외(2015)는 로봇 간 집단 학습을 클라우드가 로봇에 주는 이점의 하나로 든다. [사실][^ref-476]'와 태그 없는 연결 문장 '이 페이지에서는 연결만 두고 내용은 27. AI·학습·적응과 모델 운영 페이지에서 다룬다.'로 나누고 '계산 이전' 표현을 뺐다."
  ]
}
```

### runs/2026-09-25-27/format_check.md
...
VDA 5050(3.0.0 기준)은 연결 실패와 메시지 손실이 있는 무선망을 전제한 통신 방식을 정한다. [사실][^ref-031] KubeEdge는 클라우드–엣지 네트워크가 끊겨도 엣지 노드와 애플리케이션이 자율적으로 계속 동작하는 구조를 둔다. [사실][^ref-471]
...
- NIST, Fog Computing Conceptual Model(SP 500-325, 2018) — 클라우드와 말단 장치 사이에서 지연을 고려한 분산 애플리케이션을 지원하는 계층형 모델을 제시했다. [사실][^ref-474] 이 위키는 이 모델을 현장 서버의 역할을 설명하는 기준 틀로 쓴다. [의견]
- Gilbert·Lynch, Brewer's conjecture …(2002) — 비동기 네트워크 모델에서 일관성·가용성·분할 내성의 동시 달성 불가를 증명했다. [사실][^ref-481] 이 결과는 단절 중 운영과 재연결 뒤 기록 맞춤이 왜 필요한지의 이론적 근거가 될 것으로 보인다. [추정][^ref-481]
...
- [12. 명령·작업 실행의 신뢰성](../../categories/c-connectivity-and-execution-foundation/12-command-and-task-execution-reliability.md) — 단절 중 해제된 노드까지의 수행과 재연결 뒤 상태 재구성은 명령 재전송·중복 방지 설계와 이어진다. [추정][^ref-031][^ref-477]
- [8. 실시간 세계 상태·데이터 일관성](../../categories/b-common-information-and-environment-model/08-real-time-world-state-and-data-consistency.md) — 재연결 뒤 다음 상태 메시지로 로봇 상태를 다시 세우는 문제는 현재 상태를 표현하는 모델의 일관성 문제와 이어진다. [추정][^ref-031][^ref-477]
- [1. 주문·업무 시스템 연계](../../categories/a-business-supply-chain-design/01-order-and-business-system-integration.md) — 외부망 단절 중 클라우드 WMS의 새 주문 수신과 재고 확정이 멈추는 문제가 업무 시스템 연계의 경계가 된다. [추정][^ref-481][^ref-480]
- [20. 예외 복구·재계획·업무 연속성](../../categories/e-collaboration-and-field-operations/20-exception-recovery-replanning-and-business-continuity.md) — 단절 중 운영 범위와 재연결 뒤 기록 맞춤은 업무 연속성 계획의 한 경우다. [추정][^ref-471][^ref-472]
...
- [3. 처리능력·거점·설비 계획](../../categories/a-business-supply-chain-design/03-capacity-site-and-facility-planning.md) — 거점마다 현장 코어를 두는 다거점 구성은 거점·설비 계획과 이어진다. [추정][^ref-470][^ref-256][^ref-473]
- [27. AI·학습·적응과 모델 운영](../../categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md) — Kehoe 외(2015)는 로봇 간 집단 학습을 클라우드가 로봇에 주는 이점의 하나로 든다. [사실][^ref-476] 이 페이지에서는 연결만 두고 내용은 27. AI·학습·적응과 모델 운영 페이지에서 다룬다.
...
### runs/2026-09-25-27/verification2.json
...
```json
{
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "docs/topics/2026/2026-09-25-area11-s8.md 3. 본문 첫 항목(NIST SP 500-325): '현장 서버의 역할을 설명하는 기준 틀이 된다' 구절을 [사실] 문장에서 떼어 [의견](구축자 정리)으로 따로 쓰거나 삭제한다 — f11 은 포그 계층 모델의 내용만 뒷받침하고 이 위키에서의 쓰임새 평가는 뒷받침하지 않는다.",
    "docs/topics/2026/2026-09-25-area11-s8.md 3. 본문 둘째 항목(Gilbert·Lynch): '단절 중 운영과 재연결 뒤 기록 맞춤이 왜 필요한지의 이론적 근거가 된다' 구절을 [사실] 문장에서 떼어 [추정]으로 따로 쓴다 — 이 적용은 f22(추정)의 추론이며 f15 는 CAP 증명 자체만 뒷받침한다. [사실] 아래에 두면 추정을 사실로 올린 것이 된다.",
    "세부영역 페이지 7절 첫 문장과 docs/topics/2026/2026-09-25-area11-s7.md 1. 세 줄 요약·3. 본문 첫 문장('이 영역의 표준·오픈소스는 대부분 연결 끊김과 손실을 전제한 통신 방식 … [사실][^ref-031][^ref-471]'): 태그를 [의견](구축자 정리)으로 바꾸고 각주를 떼거나, '대부분'을 빼고 VDA 5050·KubeEdge 두 사례에 한정한 [사실] 문장으로 고친다 — 두 출처는 표의 아홉 항목 전체('대부분')에 대한 일반화를 뒷받침하지 않는다(NIST 포그 모델·3GPP MEC 은 단절 전제 설계가 아니다).",
    "docs/topics/2026/2026-09-25-area11-s10.md 3. 본문의 27. AI·학습·적응과 모델 운영 항목: '로봇 간 집단 학습과 계산 이전은 그 영역의 연구 방법이 적용되는 대상이며 … [사실][^ref-476]'을 'Kehoe 외(2015)는 로봇 간 집단 학습을 클라우드가 로봇에 주는 이점의 하나로 든다. [사실][^ref-476]' 와 '연결만 두고 내용은 27. AI·학습·적응과 모델 운영 페이지에서 다룬다'(태그 없는 연결 문장 또는 [의견])로 나눈다 — ref-476 은 영역 분류 판단을 뒷받침하지 않고, '계산 이전'(f13)은 강등된 [추정]이라 [사실] 문장에 섞을 수 없다."
  ],
  "confidence": "low",
  "verification_note": "판정: 1차 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(raw.githubusercontent.com 공식 저장소 원문과 입력 원문 텍스트만 열람). 확인 22건, 미확인 3건(f3·f13·f14), 교차 확인 0건. 강등: f13 사실 → 추정, f14 사실 → 추정, f3 은 [추정] 유지하되 '메시지 크기 상한' 구절 삭제(VDA 5050 팩트시트 protocolLimits 에 maximumMessageLength 가 있음). 원문 미열람 출처: ref-227, ref-474, ref-475, ref-476, ref-477, ref-478, ref-479, ref-480, ref-481, ref-482. 원문 확인 출처: ref-031(입력 원문), ref-468, ref-469, ref-470, ref-256, ref-471, ref-472, ref-473(공식 저장소 원문). 주의: 분류 원문 질문(인터넷이 끊겨도 어디까지 운영하는가)에 대한 답(f22)과 층별 역할 분담(f23)·다거점 구성(f25)은 모두 이 위키의 추론이며, 물류센터의 외부망 단절 운영 기준이나 다거점 공개 사례는 찾지 못했다. 3절의 중단 통계(f20)는 하이브리드 WMS 판매사의 조사 주장이고, 이음5G 속도(f19)와 관제 이중화(f21)는 벤더 주장이다. 핵심 주장의 절반 이상이 추정이라 신뢰도는 low 다. 정정 요청 없음. 미사용 출처 없음. / 2차 수정 후 재검증. 드리프트 4건 처리 지시(대표 연구 분리 페이지의 NIST·CAP 항목에 붙은 평가 구절이 [사실] 아래 있음, 7절 요약의 '대부분' 일반화를 [사실]로 표기, 27. AI·학습·적응과 모델 운영 연결 문장을 [사실][^ref-476]로 표기). 1차 수정 지시 16건은 모두 이행 확인(f3·f13·f14 강등·구절 삭제, 30초 표현, f20 비용 상한, f25 표현, 범위 표시, 원문 미열람·벤더 주장 병기, ref-031 직접 인용 1회, 기준일 명시). [분류원문] 보존, 섹션 순서 준수, 링크 유효. 참고: reference_updates 의 cited_by 가 분리 전 기준이라 분리 주제 페이지 인용이 일부 빠져 있고(예: ref-031·ref-475), 6·8·10절 요약의 [의견] 문장은 구축자 정리임을 밝히면 더 좋다 — 수정 지시 대상은 아니다.",
  "retry_reason": null
}
```
