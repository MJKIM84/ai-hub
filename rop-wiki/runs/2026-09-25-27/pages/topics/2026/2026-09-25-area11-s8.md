...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-303, ref-304, ref-305, ref-308, ref-310, ref-311]
last_run: 2026-09-25
version: 1
...
- NIST, Fog Computing Conceptual Model(SP 500-325, 2018) — 클라우드와 말단 장치 사이에서 지연을 고려한 분산 애플리케이션을 지원하는 계층형 모델을 제시했다. [사실][^ref-303] 이 위키는 이 모델을 현장 서버의 역할을 설명하는 기준 틀로 쓴다. [의견]
- Gilbert·Lynch, Brewer's conjecture …(2002) — 비동기 네트워크 모델에서 일관성·가용성·분할 내성의 동시 달성 불가를 증명했다. [사실][^ref-310] 이 결과는 단절 중 운영과 재연결 뒤 기록 맞춤이 왜 필요한지의 이론적 근거가 될 것으로 보인다. [추정][^ref-310]
- Kehoe 외, A Survey of Research on Cloud Robotics and Automation(2015) — 클라우드의 네 가지 이점을 정리한 서베이다. [사실][^ref-305]
- Ichnowski 외, FogROS2(2022, ICRA 2023) — 계산 이전의 지연 개선을 보고했으나 수치는 저자 보고값이고 실험 조건은 미확인이다. [추정][^ref-304]
- Brorsson 외, Infrastructure-based Autonomous Mobile Robots for Internal Logistics(2025) — 인프라 센서·클라우드 컴퓨팅·탑재 지능을 결합한 참조 구조를 제시했다. [추정][^ref-308]
- ETRI Journal 게재 논문, Ultra-low-latency services in 5G systems(2020) — 공장 자동화 같은 초저지연 서비스가 종단 간 10ms 미만 지연을 요구하며, 3GPP가 Release 15/16에서 MEC를, Release 16/17에서 IEEE 802.1 시간 민감 네트워킹(TSN)과 연동하는 시간 민감 통신을 표준화하고 있다고 정리했다(2020년 논문 기준). [사실][^ref-311]
