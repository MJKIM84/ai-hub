...
```json
{
  ...
  "claim_checks": [
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. ref-001·ref-111(브리프 id)은 모두 ASCM 자료라 독립 교차가 아니다. 성과 속성별 대표 지표의 분류는 검색 요약 범위에서 확인했다."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 협회 저널 글의 검색 요약이 WIP = TH × CT와 임계 재공품 설명을 담고 있다. 리틀의 법칙은 널리 확립된 관계이지만, 교차 출처는 이번 브리프에 등록돼 있지 않다. ref-114(브리프 id) 발행일 미확인."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문을 다시 열어 확인했다. 시작·종료 시각, original_estimate_millis·estimate_millis, 12개 상태 값, assigned_to, phases, interruptions·cancellation·killed가 모두 일치한다. 이 출처(task_state.json, 같은 URL)는 참고문헌에 이미 ref-111로 등록돼 있으므로 브리프의 새 id ref-120 대신 기존 ref-111을 재사용해야 한다."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 인증 운영 사이트의 검색 요약 범위다. '기반영역에서 성과관리 체계를 평가'는 기존 참고문헌 ref-124(국가물류통합정보센터 인증제 안내, 2. 공정·워크플로 모델링 페이지 근거)와도 맞는다. 그러나 두 출처 모두 같은 제도 운영 측 자료라 독립 교차로 보지 않는다. oq-011은 부분 보강일 뿐 해결이 아니다."
    },
    {
      "finding_id": "f23",
      "source_exists": true,
  ...
  "duplication": {
    "ok": false,
    "overlaps": [
      "참고문헌 id 충돌: 브리프의 신규 출처 ref-110~ref-124는 docs/references/index.md에 이미 다른 출처로 등록된 id다(ref-110 Open-RMF task_new, ref-111 task_state.json, ref-112 OMG BPMN, ref-113 Camunda, ref-114 Corradini 외 FaMe, ref-116 Filippone 외, ref-117·ref-118 B2MML, ref-119 IEC 62264-3, ref-121 workflow net, ref-122 OCEL 2.0, ref-123 SCOR F1.3, ref-124 국가물류통합정보센터 인증제 안내). 그대로 게시하면 게시된 2. 공정·워크플로 모델링 등의 각주가 덮어써진다",
      "브리프 ref-120(rmf_api_msgs task_state.json)은 기존 ref-111과 같은 URL·같은 문서다. 새 id를 만들지 말고 ref-111을 재사용해야 한다",
      "f22(스마트물류센터 인증 기반영역의 성과관리 평가)는 기존 ref-124(국가물류통합정보센터 인증제 안내)와 게시된 2. 공정·워크플로 모델링 페이지의 주장과 겹친다. 기존 각주를 함께 쓸 수 있다",
  ...
  "required_fixes": [
    "참고문헌 id 재부여: 브리프 신규 출처 ref-110~ref-124는 docs/references/index.md의 기존 출처와 id가 겹친다. 페이지 각주·프런트매터 sources·reference_updates에서 다음과 같이 바꾼다. ref-110→ref-125(ISO 22400-2), ref-111→ref-126(SCOR RL.1.1), ref-112→ref-127(WERC DC Measures 2025), ref-113→ref-128(CIE OEE 논문), ref-114→ref-129(PPI Little's Law), ref-115→ref-130(병목 탐지 리뷰), ref-116→ref-131(Azadeh 외), ref-117→ref-132(Ghelichi·Kilaru), ref-118→ref-133(Omega RMFS 에너지), ref-119→ref-134(PM4Py README), ref-121→ref-135(robot_state.json), ref-122→ref-136(창고 프로세스 마이닝 사례), ref-123→ref-137(CIO Korea 오토스토어 기사), ref-124→ref-138(박정수·안영효). 기존 ref-110~ref-124 페이지는 덮어쓰지 않는다 — 실행 컨텍스트의 next_ref_id(ref-110)가 이미 쓰인 범위를 가리켰기 때문이다.",
    "f17·f18의 브리프 ref-120(task_state.json)은 새로 등록하지 않는다. 기존 ref-111 각주 줄을 그대로 재사용한다 — 같은 URL·같은 문서다.",
    ...
    "원문 미열람 표기: f16·f17·f19의 근거(PM4Py README, robot_state.json, 기존 ref-111)를 뺀 모든 신규·재사용 출처의 각주 정의 접근일 뒤에 ' (원문 미열람)'을 붙이고, reference_updates의 해당 항목에 source_unopened: true를 넣는다 — 이번 실행은 fetch_mode mirror_only다.",
