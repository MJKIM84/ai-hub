...
- 언어: ko
- next_ref_id: ref-828
- 새 출처 id 구간: ref-828 ~ ref-857 — 이 실행 전용으로 예약한 번호다(동시에 도는 다른 실행과 겹치지 않는다). 새 출처는 ref-828 부터 순서대로 쓰고 ref-857 를 넘기지 않는다. 기존 출처는 참고문헌 목록의 id 를 그대로 쓴다
...
소속 대분류: D. 계획·최적화 · 상태: draft · 신뢰도: medium · 마지막 갱신: 2026-09-25 · 버전: 2
