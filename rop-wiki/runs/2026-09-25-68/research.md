...
- **ref-734**: 원문 미열람. NIST AI RMF 1.0(NIST AI 100-1) 발표문. 네 기능 Govern·Map·Measure·Manage.
- **ref-735**: 원문 미열람. AI 관리 시스템(AIMS)의 수립·실행·유지·개선 요구사항 표준 소개 페이지.
- **ref-736**: 원문 미열람. ISO 31000 을 AI 에 적용한 위험관리 지침 소개 페이지.
- **ref-737**: 원문 미열람. 한국 인공지능 기본법 법령 본문. 고영향 인공지능 정의와 사업자 책무, 2026-01-22 시행(검색 요약 기준).
- **ref-738**: 원문 미열람. Regulation (EU) 2024/1689(AI Act)의 위험 기반 규제와 고위험 AI 분류 안내 페이지.
- **ref-739**: 원문 미열람. SayCan: LLM 과제 접지와 기술 가치 함수의 세계 접지를 결합한 로봇 계획.
- **ref-740**: 원문 미열람. 자연어를 PDDL 로 바꿔 고전 계획기로 푸는 LLM+P 프레임워크.
- **ref-741**: 원문 미열람. KnowNo: 등각 예측으로 LLM 계획기의 불확실성을 보정해 필요할 때만 사람에게 묻는 방법.
- **ref-742**: 원문 미열람. 코드 생성 LLM 이 인식·제어 API 를 조합한 로봇 정책 코드를 쓰는 방법.
- **ref-743**: 원문 미열람. 창고 다중 로봇 작업 배정의 주의 기반 강화학습(ICRA 2023).
- **ref-744**: 원문 미열람. ML 시스템 고유의 기술 부채와 위험 요인(NeurIPS 2015).
- **ref-745**: 원문 미열람. 28개 시험·감시 항목으로 ML 운영 준비도를 점수화하는 기준표(IEEE Big Data 2017).
- **ref-746**: 원문 미열람. 모델 버전·별칭·태그·계보를 관리하는 MLflow 모델 레지스트리 공식 문서.
- **ref-747**: 원문 미열람. 국내 물류기업의 로봇·AI 적용 사례 기사(한진 적재량 예측, CJ대한통운 포장 로봇 등).
...
- **ref-575**: 원문 미열람. LLM 명령의 실행 전 안전 판정과 작업 안전 계약 SafeGate.
