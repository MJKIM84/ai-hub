# 스토리텔러 산출 2026-09-25-68

## 페이지

| 동작 | 경로 | 상태 | 변경 요약 |
|---|---|---|---|
| update | docs/categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md | draft | 영역 심화: 3~11절 신규 작성(LLM 계획 접지·검증·되묻기·안전 판정, 학습 기반 배차, 모델 운영, AI 위험관리 표준·법), 페이지 상태 자동 영역 추가, 각주 19건 |
| create | docs/topics/2026/2026-09-25-area27-s6.md | draft | 자동 분리: 27. AI·학습·적응과 모델 운영 의 "6. 대표 접근법과 기술" 절(1,218자)을 옮겼다 |
| create | docs/topics/2026/2026-09-25-area27-s7.md | draft | 자동 분리: 27. AI·학습·적응과 모델 운영 의 "7. 관련 표준·프레임워크·오픈소스" 절(1,001자)을 옮겼다 |
| create | docs/topics/2026/2026-09-25-area27-s8.md | draft | 자동 분리: 27. AI·학습·적응과 모델 운영 의 "8. 대표 연구와 자료" 절(969자)을 옮겼다 |
| create | docs/topics/2026/2026-09-25-area27-s10.md | draft | 자동 분리: 27. AI·학습·적응과 모델 운영 의 "10. 다른 연구영역과의 연결 (번호와 이름을 함께 표기)" 절(760자)을 옮겼다 |
| create | docs/topics/2026/2026-09-25-area27-s4.md | draft | 자동 분리: 27. AI·학습·적응과 모델 운영 의 "4. 핵심 개념과 용어" 절(754자)을 옮겼다 |

## 변경 이력·색인

- 변경 이력: 2026-09-25 | 27. AI·학습·적응과 모델 운영 | 영역 심화: 3~11절 신규 작성(LLM 계획 접지·검증·되묻기·안전 판정, 학습 기반 배차, 모델 운영, AI 위험관리 표준·법, 출처 19건 원문 미열람) | run 2026-09-25-68
- 홈 최근 업데이트: 2026-09-25 — 27. AI·학습·적응과 모델 운영: 3~11절 첫 작성(LLM 계획 접지·형식 검증·되묻기·실행 전 안전 판정, 학습 기반 배차, 모델 레지스트리, NIST AI RMF·ISO/IEC 42001·한국 인공지능 기본법)
- 대분류 최근 업데이트: 2026-09-25 — 27. AI·학습·적응과 모델 운영: 영역 심화 초안 작성, 교차 규칙에 따라 5·6·13·19·21 영역과 연결
- 세부영역 최근 업데이트: 2026-09-25 — 27. AI·학습·적응과 모델 운영: 3~11절 신규 작성(신뢰도 medium, 원문 미열람 출처 19건)

## 용어집 갱신

| 동작 | 용어(한글) | 용어(영문) | 한 줄 정의 | 관련 영역 | 출처 |
|---|---|---|---|---|---|
| new | 등각 예측 | Conformal Prediction | 보정 데이터로 예측 집합의 크기를 정해, 정답이 집합에 들어갈 확률을 사용자가 정한 수준 이상으로 통계적으로 보장하는 불확실성 정량화 방법이다. | 27, 18 | ref-741 |
| new | AI 관리 시스템 | Artificial Intelligence Management System (AIMS) | 조직이 AI 의 책임 있는 개발·제공·사용을 위한 정책·목표·프로세스를 세우고 운영하는 관리 체계로, ISO/IEC 42001 이 요구사항을 정한다. | 27, 28 | ref-735 |
| new | 고영향 인공지능 | High-impact AI (Korea AI Basic Act) | 한국 인공지능 기본법에서 사람의 생명·안전·기본권에 중대한 영향을 미칠 수 있어 별도 책무가 부과되는 영역의 AI 시스템이다. | 27, 25, 28 | ref-737 |
| new | 모델 레지스트리 | Model Registry | 학습된 모델의 버전·별칭·태그·계보를 한곳에서 관리해 어떤 버전을 운영에 쓰는지 정하고 교체·되돌림을 추적하게 하는 저장소다. | 27, 24 | ref-746 |

## 참고문헌 갱신

| id | 기관 | 제목 | 유형 | 신뢰도 | URL |
|---|---|---|---|---|---|
| ref-734 | NIST | NIST Risk Management Framework Aims to Improve Trustworthiness of Artificial Intelligence | 정부·연구기관 | medium | https://nist.gov/news-events/news/2023/01/nist-risk-management-framework-aims-improve-trustworthiness-artificial |
| ref-735 | ISO/IEC | ISO/IEC 42001:2023 - AI management systems | 표준 | medium | https://www.iso.org/standard/42001 |
| ref-736 | ISO/IEC | ISO/IEC 23894:2023 - AI — Guidance on risk management | 표준 | medium | https://www.iso.org/standard/77304.html |
| ref-737 | 국가법령정보센터(과학기술정보통신부) | 인공지능 발전과 신뢰 기반 조성 등에 관한 기본법 | 정부·연구기관 | medium | https://www.law.go.kr/lsInfoP.do?lsiSeq=268543 |
| ref-738 | European Commission | AI Act \| Shaping Europe's digital future | 정부·연구기관 | medium | https://digital-strategy.ec.europa.eu/en/policies/regulatory-framework-ai |
| ref-739 | Ahn, M. 외 | Do As I Can, Not As I Say: Grounding Language in Robotic Affordances | 논문 | medium | https://arxiv.org/abs/2204.01691 |
| ref-092 | Liu, B. 외 | LLM+P: Empowering Large Language Models with Optimal Planning Proficiency | 논문 | medium | https://arxiv.org/abs/2304.11477 |
| ref-741 | Ren, A. Z. 외 | Robots That Ask For Help: Uncertainty Alignment for Large Language Model Planners | 논문 | medium | https://arxiv.org/abs/2307.01928 |
| ref-742 | Liang, J. 외 | Code as Policies: Language Model Programs for Embodied Control | 논문 | medium | https://arxiv.org/abs/2209.07753 |
| ref-743 | Agrawal, A. 외 | RTAW: An Attention Inspired Reinforcement Learning Method for Multi-Robot Task Allocation in Warehouse Environments | 논문 | medium | https://arxiv.org/abs/2209.05738 |
| ref-744 | Sculley, D. 외 | Hidden Technical Debt in Machine Learning Systems | 논문 | medium | https://papers.nips.cc/paper/5656-hidden-technical-debt-in-machine-learning-systems |
| ref-745 | Breck, E. 외 (Google Research) | The ML Test Score: A Rubric for ML Production Readiness and Technical Debt Reduction | 논문 | medium | https://research.google/pubs/the-ml-test-score-a-rubric-for-ml-production-readiness-and-technical-debt-reduction/ |
| ref-746 | MLflow (Linux Foundation 오픈소스 프로젝트) | ML Model Registry \| MLflow AI Platform | 오픈소스 문서 | medium | https://mlflow.org/docs/latest/ml/model-registry/ |
| ref-747 | 머니투데이 | 포장은 로봇이, 간선운송은 무인차가…물류현장 스며든 '피지컬 AI' | 기사 | low | https://www.mt.co.kr/industry/2026/09/19/2026091818023697394 |
| ref-354 | cog-model (AmbiK 저자) | AmbiK-dataset — README (AmbiK: Dataset of Ambiguous Tasks in Kitchen Environment) | 오픈소스 문서 | medium | https://github.com/cog-model/AmbiK-dataset |
| ref-359 | Wang, W. 외 | Learning to Ask: When LLM Agents Meet Unclear Instruction | 논문 | medium | https://arxiv.org/abs/2409.00557 |
| ref-056 | Liu, J. X. 외 | Grounding Complex Natural Language Commands for Temporal Tasks in Unseen Environments | 논문 | medium | https://arxiv.org/abs/2302.11649 |
| ref-541 | lbaa2022 (LoTa-Bench 공식 저장소) | LLMTaskPlanning — LoTa-Bench: Benchmarking Language-oriented Task Planners for Embodied Agents (ICLR 2024) (GitHub README) | 오픈소스 문서 | medium | https://github.com/lbaa2022/LLMTaskPlanning |
| ref-575 | arXiv (SafeGate 저자, 저자명 미확인) | Pre-Execution Safety Gate & Task Safety Contracts for LLM-Controlled Robot Systems | 논문 | medium | https://arxiv.org/abs/2604.05427 |

## 열린 질문 갱신

| 동작 | id | 질문 | 영역 | 상태 | 링크 |
|---|---|---|---|---|---|
| new | — | 물류 현장 로봇의 작업 계획·배정에 쓰는 AI 가 한국 인공지능 기본법의 고영향 인공지능 영역에 해당하는가, 해당하면 ROP 사업자와 현장 운영사 중 누가 책무를 지는가? | 27, 28 | 열림 | — |
| new | — | LLM 이나 학습 모델이 ROP 의 정지·경로·구역 결정에 관여할 때 EU AI Act 가 말하는 제품 안전 구성요소로 볼 수 있는가? | 27, 25 | 열림 | — |
| new | — | KnowNo 의 등각 예측 보장은 보정 데이터와 운영 분포가 같다는 조건에 기대는데, 물류 지시 분포가 계절·고객에 따라 바뀔 때 보정을 얼마나 자주 다시 해야 하는가? | 27, 18 | 열림 | — |

## 흐름 매트릭스 갱신

| 단계 | 항목 | 링크 | 제목 |
|---|---|---|---|
| 피킹 | 시작 조건 | docs/categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md | 27. AI·학습·적응과 모델 운영 |
| 피킹 | 수행 자원 | docs/categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md | 27. AI·학습·적응과 모델 운영 |
| 피킹 | 제약 | docs/categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md | 27. AI·학습·적응과 모델 운영 |
| 피킹 | 예외·성과 | docs/categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md | 27. AI·학습·적응과 모델 운영 |
| 출하 | 시작 조건 | docs/categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md | 27. AI·학습·적응과 모델 운영 |
| 출하 | 수행 자원 | docs/categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md | 27. AI·학습·적응과 모델 운영 |
| 출하 | 예외·성과 | docs/categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md | 27. AI·학습·적응과 모델 운영 |

## 표준·프레임워크 갱신

| 이름 | 종류 | 기관 | 관련 영역 | 참고문헌 | URL |
|---|---|---|---|---|---|
| NIST AI RMF 1.0 (NIST AI 100-1) | 프레임워크 | NIST | 27, 25 | ref-734 | https://nist.gov/news-events/news/2023/01/nist-risk-management-framework-aims-improve-trustworthiness-artificial |
| ISO/IEC 42001:2023 AI 관리 시스템 | 표준 | ISO/IEC | 27, 28 | ref-735 | https://www.iso.org/standard/42001 |
| ISO/IEC 23894:2023 AI 위험관리 지침 | 표준 | ISO/IEC | 27, 25 | ref-736 | https://www.iso.org/standard/77304.html |
| MLflow 모델 레지스트리 | 오픈소스 | MLflow (Linux Foundation 오픈소스 프로젝트) | 27, 24 | ref-746 | https://mlflow.org/docs/latest/ml/model-registry/ |
| LoTa-Bench | 오픈소스 | lbaa2022 (LoTa-Bench 공식 저장소) | 27, 23 | ref-541 | https://github.com/lbaa2022/LLMTaskPlanning |
| AmbiK 데이터셋 | 오픈소스 | cog-model (AmbiK 저자) | 27, 18 | ref-354 | https://github.com/cog-model/AmbiK-dataset |

## 추가 조사 요청

- 11절: oq-030 LTAA(arXiv 2512.02810)의 LLM 배정 완료율 77% 주장과 동적 계획법 우위 요약 가운데 어느 쪽이 원문 결과인지 원문 확인이 필요하다(출처 충돌 해소).
- 3·7절: 한국 인공지능 기본법 개정 법률의 시행일(2026-07-21 언급)과 고영향 인공지능 영역 목록을 법령 원문으로 확인해야 기준일을 갱신할 수 있다.
- 3·7절: EU AI Act 고위험 의무의 적용 시점이 개정(AI Omnibus)으로 바뀌었는지 공식 자료로 확인해야 한다.
- 6·8절: 트랙 반영 제안 가운데 도면 해석(2026-09-25-05·36·54), 매뉴얼 추출(2026-09-25-57), LLM 다중 로봇 서베이·ROSA·RAI·ChatGPT for Robotics(2026-09-25-21), Cohen 외·Huang 외(-04), LMCR·구조화 출력(-30), DELIVER·SayPlan(-37)은 이번 브리프에 검증된 finding 이 없어 본문에 넣지 못했다. 다음 해당 영역 실행에서 재확인이 필요하다.
- 3·6절: 한 줄 정의의 수요·고장 예측(예: 로봇 고장 예측 사례)과 19. 모니터링·이상 탐지·원인 분석에 적용되는 장애 분석 AI 자료가 브리프에 없어 쓰지 못했다.
- 전 절: 사실 주장이 모두 단일 출처·원문 미열람이다. 핵심 주장(SayCan, KnowNo, RTAW 수치, ISO/IEC 42001)을 원문 열람 또는 독립 출처로 교차 확인해야 신뢰도를 올릴 수 있다.

## 이행한 수정 지시

- f7·f20·f21 각주 ref-740 → ref-092 — 3·4·5·6·8절의 LLM+P 인용을 모두 [^ref-092]로 바꾸고 각주 정의를 ref-092 로 두었으며, reference_updates 에 ref-740 을 등록하지 않았다(ref-092 는 cited_by 갱신용으로만 넣음).
- f1 '거버넌스가 나머지 세 기능을 가로지른다' 삭제 — 7절 표의 NIST AI RMF 행에 발표일·자율 적용·네 기능만 [사실]로 남겼다.
- f6 '곱해' → '결합해' — 4절 용어와 6절·8절 SayCan 서술을 '결합해'로 썼다.
- f8 '필요할 때'·'저자 보고' — 4·5·6·8절의 KnowNo 서술을 '필요할 때 사람에게 도움을 요청'으로 쓰고 저자 보고 한정을 붙였다.
- f14 '승인·거부·보류(defer)'와 물류 적용 [추정] — 6·8절에 판정 결과를 승인·거부·보류(defer)로 쓰고, 물류 적용 가능성은 별도 [추정] 문장으로 분리했다.
- f16 위험 요인 목록 축소, f22 문구 변경 — 3절에 '경계 침식, 얽힘, 숨은 피드백 루프, 선언되지 않은 소비자, 데이터 의존성 등'으로 쓰고, 5절 출하 시나리오의 '외부 세계 변화'를 '데이터 의존성·숨은 피드백 루프 같은 ML 고유 위험'으로 바꿨다.
- f17 '1급 관심사' 삭제 — 6·8절에 '28개 시험·감시 항목으로 운영 준비도를 점수화하는 기준표'로만 썼다.
- f4 기준일 2026-01-22와 미확인 표시 — 3절과 7절에 기준일 2026-01-22를 적고 개정 법률 시행일·고영향 영역 목록을 '미확인'으로 두었다.
- f5 적용 시점 미확정 문구와 요건 서술 — 3·7절에 '안전 구성요소이거나 제품 자체이고 제3자 적합성 평가 대상'으로 쓰고 '적용 시점은 개정(AI Omnibus) 논의로 이 페이지에서 확정하지 않는다'를 덧붙였다.
- f11 기준일 — 6·8절에 'arXiv 2024-09, 학회 게재판은 미확인'으로 적었다.
- f19 [추정]·벤더 주장, ROP 몫 한정 — 5절에서 '[추정] 벤더 주장'으로 표기하고 ROP 몫을 입출고 시간·접안·현장 상·하차 작업 동기화로, 간선 배차·운송계획을 분류 원문 9장 거점 간 운송의 연계 대상으로 적었으며 9절 표에도 같은 행을 두었다.
- f15 한정어와 13절 연결 — 6·8절 RTAW 수치에 '저자 보고, 시뮬레이션 창고 조건, 단일 출처'를 붙이고 10절에 13. 작업 배정 — MRTA 를 연결했다.
- f21·f22 가상 시나리오 명시 — 5절 첫머리에 설명용 가상 시나리오임과 수치를 넣지 않았음, 출처 환경(가정·주방·벤치마크, 시뮬레이션 창고)이 물류 현장과 다르다는 한계를 적었다.
- 10절 교차 규칙 연결 — 5. 로봇 능력·작업 온톨로지, 21. 온보딩·설정·현장 시운전, 6. 지도·공간·위치 모델, 13. 작업 배정 — MRTA, 19. 모니터링·이상 탐지·원인 분석을 번호와 이름으로 연결하고, 도면 해석·매뉴얼 추출은 '다음 실행에서 반영 예정인 트랙 제안'으로만 적었다.
- 11절 — oq-030 을 열림 상태로 유지하고(이번 실행 미조사 명시) open_questions_new 3건을 11절과 open_question_updates 에 new 로 등록했다.
- 모든 각주 정의 접근일 뒤에 ' (원문 미열람)'을 붙이고 reference_updates 의 모든 항목에 source_unopened: true 를 넣었다.
- 트랙 반영 제안 가운데 브리프가 다루지 않은 항목(2026-09-25-05·21·36·54·57, -04 의 Cohen 외·Huang 외, -30 의 LMCR·구조화 출력, -37 의 DELIVER·SayPlan)은 본문에 넣지 않고 additional_research_requests 에 재확인 요청으로만 남겼다.
- 분량 초과 자동 분리: 27. AI·학습·적응과 모델 운영 본문 8,004자 > 기준 4,000자 → 5개 절을 주제 페이지로 옮김, 남은 본문 3,936자
