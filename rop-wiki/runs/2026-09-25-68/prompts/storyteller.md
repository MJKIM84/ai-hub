...
```json
{
  ...
  "claim_checks": [
    ...
    },
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 종합 [추정]. 구성 출처는 모두 실재를 확인함(ref-740 은 ref-092 로 바꿈). 각 구성 요소의 성격은 해당 finding 들과 맞음. 원문 미열람."
    },
    {
      "finding_id": "f21",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "가상 시나리오 [추정]. 수치 없음. 출처가 가정·주방·벤치마크 환경임을 밝혔음. 피킹/시작 조건 배치 적절. ref-740 → ref-092."
    },
    {
      "finding_id": "f22",
      "source_exists": true,
  ...
  "duplication": {
    "ok": false,
    "overlaps": [
      "ref-740(LLM+P, https://arxiv.org/abs/2304.11477)은 기존 ref-092(2026-09-25-66 브리프 출처표, 같은 URL)와 같은 출처다. f7·f20·f21 의 각주는 ref-092 를 재사용해야 한다.",
      "ref-739(SayCan)·ref-741(KnowNo)은 트랙 반영 제안이 인용한 기존 id(SayCan ref-087·ref-088, KnowNo ref-350·ref-351)와 URL 이 같을 수 있다. 참고문헌 목록 요약이 입력에 없어 확인하지 못했다. 같은 URL 이면 퍼블리셔가 기존 id 로 합친다.",
  ...
  "required_fixes": [
    "f7·f20·f21: 각주 ref-740 을 기존 ref-092(같은 URL https://arxiv.org/abs/2304.11477)로 바꾸고, ref-740 은 reference_updates 에 새로 등록하지 않는다. 같은 출처가 이미 등록돼 있기 때문이다.",
  ...
  "confidence": "medium",
  "verification_note": "판정: 조건부 승인. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다. 확인 25건, 미확인 0건, 교차 확인 0건. 강등: 없음. 대신 f1·f6·f8·f14·f16·f17 은 검색 스니펫 범위로 문구를 줄이도록 지시했다. 원문 미열람 출처: ref-734, ref-735, ref-736, ref-737, ref-738, ref-739, ref-740, ref-741, ref-742, ref-743, ref-744, ref-745, ref-746, ref-747, ref-359, ref-056, ref-575. ref-354(AmbiK)와 ref-541(LoTa-Bench)은 검증 에이전트가 GitHub 공식 저장소 README 를 열어 확인했다. 주의: 사실로 둔 주장은 모두 단일 출처이고 논문 수치는 저자 보고다. 5절 시나리오와 9절 범위 구분은 이 위키의 추정이다. ref-738(EU AI Act 안내 페이지)은 정확한 URL 이 검색 결과에 나오지 않아 같은 포털 내용으로만 약하게 확인했다. ref-740 은 기존 ref-092 와 같은 출처라 재사용하도록 지시했다. 한국 인공지능 기본법의 개정 법률 내용과 EU AI Act 적용 시점은 미확인이다. oq-030 은 이번에 조사하지 않아 열림으로 남는다. 정정 요청 없음. 검색 사용: 검증 17회(리서치 12회와 합쳐 29/30).",
  "retry_reason": null
}
```
