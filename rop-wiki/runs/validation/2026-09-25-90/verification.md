...
|---|---|---|---|---|---|
| f1 | 예 | 예 | 아니오 | 유지 | 확인: raw.githubusercontent.com 3.0.0·2.1.0 태그 factsheet.schema 를 검증에서 직접 열어 최상위 properties 12개씩(agvGeometry·vehicleConfig → mobileRobotGeometry·mobileRobotConfiguration, 나머지 10개 동일)을 대조했다. 두 스키마는 같은 기관(VDA/VDMA)의 판별 산출물이라 독립 교차가 아니다. 발행일 미확인, 확인일 2026-09-25. |
| f2 | 예 | 예 | 아니오 | 유지 | 확인(ref-044 원문 열람): loadSets 필드, loadDimensions 길이·폭 필수·높이 선택, loadPositions 가 비면 모든 적재 장치에 적용, maximumWeight(kg), 취급 높이·깊이(m)·기울기(rad), pickTime·dropTime(s)이 원문과 같다. 단, 단위 기재가 원문과 다르다: 스키마는 loadSets.maximumSpeed 의 단위를 m/s 가 아니라 'm/s^2' 로 적고(스키마 표기 오류로 보이나 원문 그대로임), maximumDeceleration 에는 단위가 없다. 필수 필드는 setName·loadType 뿐이다. 단위 표기를 고치는 조건으로 유지. |
| f3 | 예 | 예 | 아니오 | 유지 | 확인(ref-045 원문 열람): 2.1.0 loadSets 필드 목록과 typeSpecification.maxLoadMass(kg)가 원문과 같다(스키마상 agvSpeedLimit 단위도 'm/s^2'). 다만 evidence_excerpt 의 '2.1.0 명세 본문도 6.15절에 maxWeight·agvSpeedLimit·pickTime 을 싣는다'는 검증 열람에서도 본문이 6.15.1 표 중간에서 잘려 확인하지 못했고 브리프의 ref-047 요약(잘림)과도 어긋난다 — 본문에 쓰지 않는다. |
| f4 | 예 | 예 | 아니오 | 유지 | [추정] 유지: 두 스키마 대조로 이름 변경 대응 관계는 확인했다. 설명문·단위의 의미 동일성은 미확인(단위 표기는 두 판 모두 속도 한계에 m/s^2 를 적어 서로는 같다). 후속 질문으로 넘긴 처리가 적절하다. |
| f5 | 예 | 예 | 아니오 | 유지 | 확인: 3.0.0 mobileRobotKinematics(DIFFERENTIAL·OMNIDIRECTIONAL·THREE_WHEEL), mobileRobotClass(FORKLIFT·CONVEYOR·TUGGER·CARRIER), maximumLoadMass 'Maximum loadable mass'(kg), supportedZones 존재 / 2.1.0 agvKinematic(DIFF·OMNI·THREEWHEEL), agvClass, maxLoadMass(kg), supportedZones 없음을 원문에서 확인. |
| f6 | 예 | 예 | 아니오 | 유지 | 확인: supportedZones 열거값 10개(BLOCKED, LINE_GUIDED, RELEASE, COORDINATED_REPLANNING, SPEED_LIMIT, ACTION, PRIORITY, PENALTY, DIRECTED, BIDIRECTED)가 원문과 같다. 2.1.0 에는 없음. |
| f7 | 예 | 예 | 아니오 | 유지 | 확인: 3.0.0 mobileRobotActions 의 actionScopes(INSTANT·NODE·EDGE·ZONE), blockingTypes(NONE·SOFT·SINGLE·HARD), actionResult('Free text' 결과 설명), pauseAllowed(startPause), cancelAllowed(cancelOrder) / 2.1.0 agvActions 의 resultDescription, actionScopes 3종, blockingTypes 3종, pauseAllowed·cancelAllowed 없음을 원문에서 확인. 3.0.0 명세(ref-046)의 startPause 설명에서 pauseAllowed=true 인 action 만 일시정지된다는 문장도 확인. |
| f8 | 예 | 예 | 아니오 | 유지 | 확인: 키 이름이 'batteryCharging '(끝 공백)이며 criticalLowChargingLevel(0–100%, 이하이면 관제가 충전소로 가는 주문만 보내야 함), 최저·최고 희망 충전량(%), minimumChargingTime(s)을 원문에서 확인. 2.1.0 vehicleConfig 는 versions·network 뿐이다. mobileRobotConfiguration 은 3.0.0 스키마 최상위 필수 항목이 아니다(선택). |
| f9 | 예 | 예 | 아니오 | 유지 | 확인(ref-046 원문 열람): 이동로봇 용어 사용, 5.2절의 'via the topic factsheet … defined in Section 7.10' 문장, 토픽 표에서 factsheet mandatory. 문서에 3.0.0 발행일 표기 없음 — oq-005 는 미해소. |
| f10 | 예 | 예 | 아니오 | 유지 | 확인: 2.1.0 명세 6.15절 Topic "factsheet", 2.0.0 태그 문서의 7장 AGV Factsheet 와 6.5절 토픽 표의 factsheet(AGV → master control, mandatory)를 확인. 2.0.0 태그 인용 구절 자체는 검증 열람(잘림)에서 재확인하지 못했다. 2.0.0 태그 문서 머리의 'RELEASE CANDIDATE, FOR REVIEW' 표기를 확인했으므로 VDA 게시 PDF(ref-022)와 동일성 미확인 단서를 본문에 반드시 둔다. |
| f11 | 예 | 예 | 아니오 | 유지 | [추정]·low 유지: 2.0.0 태그 문서에서 vehicleConfig 가 언급되지 않음을 확인. 다만 검증 열람은 2.0.0 팩트시트 블록에 localizationParameters 도 있다고 보고해 브리프의 블록 목록과 다르다 — 블록 목록을 완결 목록처럼 쓰지 않는다. RC 문서이고 본문이 잘려 2.0.0 적재 세트 필드는 미확인. |
| f12 | 예 | 예 | 아니오 | 유지 | [추정]·low 유지: f2 필드를 분류 원문 5. 로봇 능력·작업 온톨로지의 SCM 질문에 대응시킨 추론이며 근거 필드는 원문에서 확인됨. 값은 제조사 선언(광고 능력)이라는 단서를 유지한다. |
| f13 | 예 | 예 | 아니오 | 유지 | 사용자 실험(experiments/2026-09-25-vda5050-factsheet-field-diff/) 입력으로 존재 확인. 경로 수(132/144/104/92)는 재계산하지 않았다. 검증에서 2.0.0 태그 json_schemas/factsheet.schema 가 HTTP 404 임을 재확인했고, 실험이 보고한 최상위 필드·신규 필드(supportedZones, actionResult·pauseAllowed·cancelAllowed, batteryCharging 끝 공백)는 공식 스키마 원문과 일치했다. 페이지에서는 [추정]이 아니라 [사용자 실험]으로 표기하며 [사실] 승격은 하지 않는다. |
...
| 분류 적합성 | 예 | — |
| 범위 경계 | 예 | — |
| 중복·모순 | 아니오 | ref-046(3.0.0 태그 VDA5050_EN.md)은 기존 ref-031(main 브랜치 VDA5050_EN.md, 현재 main 은 3.0.0 판)과 같은 문서의 판 고정 URL이다 — 새 주장은 ref-046 으로 쓰되, ref-031 에 기댄 기존 '판 미확인' 문장을 대체·정리해야 한다, 단계 1 페이지 3절 q1-02 의 '팩트시트 블록 … 판 미확인 [추정][^ref-031]' 문장과 q1-03 부분 답·4절 남은 불확실성의 'VDA 5050 팩트시트 … 판 미확인' 문장이 이번 finding(f1·f5·f7·f11)과 겹친다, 모델·표준 비교표 VDA 5050 행의 파라미터 범위·적재·환경 제약 칸('판 미확인' [추정])과 5절 빠진 정보 요약이 f2·f6·f7 과 겹친다, 온톨로지 초안 6절의 '실행 조건과 제약의 경계(배터리 잔량 포함)' 미해결 질문이 실행 조건 변경 제안(f8)과 충돌한다, 온톨로지 초안의 기능(Capability) 정의(CSS 능력 = 구현 독립 명세)가 기능에 pauseAllowed·cancelAllowed·actionResult 를 붙이는 제안(f7)과 충돌한다 |
| 용어 일관성 | 아니오 | 용어집 'VDA 5050' 항목의 한 줄 정의('VDA 5050에서 차량이 자신의 기능 정보를 상위 관제에 미리 알리는 메시지(토픽)이다')가 표준 자체가 아니라 팩트시트를 설명한다 — 새 후보 'VDA 5050 팩트시트'와 뜻이 겹친다, 3.0.0 은 AGV 대신 이동로봇(mobile robot) 용어를 쓴다(f9) — 페이지에서 '차량'·'AGV'를 판에 따라 구분해 써야 한다 |
| 인용 길이·저작권 | 예 | ref-044 에서 여러 finding(f2·f5·f7·f8)이 원문 구절을 발췌했다 — 페이지 직접 인용은 ref-044 당 1회로 제한하고 나머지는 재서술해야 한다 |
| 정정 요청 반영 | — | — |
