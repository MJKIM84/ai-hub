...
- run_id: 2026-09-25-75
- date: 2026-09-25
- run_type: track (트랙 실행)
- 대상: 트랙 floorplan-recognition (건축 도면 자동 인식) · 현재 단계: 단계 4. 지도 변환 보정과 현장 정합 · 이번에 다룰 백로그 질문 id: q4-02 · 중심 세부영역: 6. 지도·공간·위치 모델 (B. 공통 정보·환경 모델)
- 예산:
...
### runs/2026-09-25-75/target.json
...
```json
{
  ...
  "selection_rationale": "CLI 지정 run_type=track, area=6; 트랙 실행일(Fri, track_days 앞 3개) → 트랙 floorplan-recognition 단계 4, 질문 q4-02 (CLI 지정 질문 id)"
}
```

### runs/2026-09-25-75/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q4-02 도면과 현장의 차이(개보수, 가구·랙 배치, 임시 장애물)를 어떻게 찾아 지도에 반영하는가?",
    "설계 도면·BIM(as-planned)과 현장(as-built)의 구조 편차를 레이저 스캔·라이다로 찾는 방법(scan-vs-BIM, 도면 기반 다중 세션 정렬)은 무엇을 자동화하는가? (단계 4 페이지 3절, 국내 연구 포함)",
    "팔레트·랙·가구처럼 옮겨지는 반정적(semi-static) 요소의 변화를 물류 현장 로봇 지도에서 탐지·갱신하는 연구와 오픈소스 도구는 무엇인가? (9절 경계 겨냥: 로봇 쪽 연계 대상 여부)",
    "임시 장애물과 일시적 통행 제한은 로봇 쪽 비용 지도와 관제 인터페이스(VDA 5050 구역 집합·지도 판, Open-RMF 차선 폐쇄)에서 어떻게 반영되는가? (15. 다중 로봇 경로·교통 관리 — MAPF, 24. 자산·소프트웨어 수명주기 관리 연결)",
    "변화의 지속성(구조 변경·반정적 배치·임시 장애물)에 따라 ROP 가 직접 맡을 반영 경로와 로봇·제조사에 맡길 부분은 어떻게 나뉘는가?"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "Bosché(2010)는 설계 3D CAD·BIM 모델(as-planned)을 현장 레이저 스캔 점군(as-built)에 정합한 뒤 모델 객체를 점군에서 자동 인식하고 시공된 치수를 계산해 치수 적합성을 관리하는 방법을 제안했으며, 이 개념은 scan-vs-BIM 으로 불린다(건설 현장 대상).",
      "tag": "사실",
      "source_ids": [
        "ref-745"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 설계 3D 모델을 스캔 점군과 ICP 기반으로 정합하고, 모델 객체의 as-built 자세·치수를 계산해 치수 적합성 관리에 쓴다. 건설 시공 품질 관리 대상 연구다.",
      "as_of": "2010",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f2",
      "claim": "국내 연구(설비공학 논문집 36(5), 2024)는 노후 건축물에서 모바일 기기로 Scan-to-BIM 역설계 도면을 만들어 기존 건축도면과 비교했고, 어린이집 3개소의 평균 오차율이 2.21%·5.99%·2.75%였으며 좁은 공간에서 빠르게 스캔하거나 표면 장애물을 치우지 않으면 오차가 크게 늘었다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-749"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "검색 요약: 어린이집 3개소 평균 오차율 2.21%, 5.99%, 2.75%. 좁은 공간 고속 촬영·장애물 미제거 시 오차 증가. 비교 기준(기존 도면 대 에너지 모델)의 세부는 원문 미열람으로 미확인.",
      "as_of": "2024",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f3",
      "claim": "연계 대상: BIM-SLAM(Vega Torres 외)은 BIM 에서 세션 데이터(포즈 그래프 지도·기술자)를 만들고 실제 3D 라이다 세션을 다중 세션 앵커링으로 정렬한 뒤, BIM 에 없는 새 요소를 양(+)의 변화로 탐지·분할해 BIM 옆에 재구성하는 3단계 틀을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-221"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 세 단계는 BIM 에서 세션 데이터 생성, 새 세션과 기준 BIM 정렬, 양의 변화 탐지와 새 요소 분할이다(ISARC 2023, arXiv 2024-08).",
      "as_of": "2024-08",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f4",
      "claim": "연계 대상: SLAM2REF 공식 저장소 README 는 포즈 그래프 다중 세션 앵커링으로 라이다 데이터를 기준 지도나 다른 세션에 정렬하고, 시설 디지털 트윈 갱신 등에 쓸 수 있도록 기준 지도와 정렬된 갱신 지도를 얻는다고 적는다.",
      "tag": "사실",
      "source_ids": [
        "ref-744"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: \"pose-graph multi-session anchoring to align your LiDAR data with a reference map or with another session\". 정밀 지상 레이저 스캔을 기준으로 하면 최대 3 cm 정확도라고 적음(저자 측 보고). Construction Robotics 8(2), 2024 논문 인용.",
      "as_of": "2024",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f5",
      "claim": "연계 대상: Shaheer 외는 건축 도면에서 만든 그래프와 라이다로 추정한 상황 그래프를 결합해 도면과 현장의 전역 정렬과 구조 편차를 실시간으로 추정하는 방법을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-224"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "도면(as-planned)–현장(as-built) 전역 정렬과 구조 편차의 실시간 추정(arXiv 2024-08 제출). (재인용: 2026-09-25-72)",
      "as_of": "2024-08",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f6",
      "claim": "연계 대상: Shaik 외(KI 2017)는 팔레트 등이 임시로 적치되어 물류 시설 환경이 정적이지 않다고 보고, 여러 로봇이 현재 지도와 비교해 변화를 감지해 임시 지도를 만들고 위치추정 정보와 지도의 선 특징으로 현재 지도에 병합하는 실시간 지도 갱신 방법을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-747"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: facility logistics environments are not static because pallets and other obstacles are stored temporarily. 각 로봇이 라이다로 변화를 감지해 임시 지도를 만들고 현재 지도에 병합.",
      "as_of": "2017",
      "flow_step": "적치",
      "flow_item": "예외·성과",
      "source_unopened": true
    ...
    {
      "id": "f7",
      "claim": "연계 대상: Stefanini 외(Sensors 23(13), 2023)는 로봇 자세 추정의 불확실성을 고려하고 사람·다른 로봇 같은 동적 장애물의 일시적 변화에는 강건한 라이다 점유 격자 지도 갱신 알고리즘을 제안하고, 창고의 물품 배치가 시간에 따라 바뀌는 상황을 모사해 시험했다.",
      "tag": "사실",
      "source_ids": [
        "ref-746"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: safe Lidar-based occupancy grid map-updating, robust to temporary changes from dynamic obstacles; 창고 물품 배치 변화 모사로 시험(2023-06-30 게재).",
      "as_of": "2023-06",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f8",
      "claim": "연계 대상: POV-SLAM(RSS 2023)은 반정적 객체의 객체 수준 변화를 추적·재구성하는 객체 인식 인자 그래프 SLAM 이며, 가동 중인 100m×80m 공장·창고에서 4개월 간격으로 수집해 팔레트·상자 위치가 바뀐 실제 창고 데이터셋으로 평가했다.",
      "tag": "사실",
      "source_ids": [
        "ref-748"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 같은 경로를 4개월 간격으로 수집한 두 궤적을 이어 장면 변화를 만들었고, 지게차 같은 동적 객체와 팔레트·상자 같은 반정적 객체가 포함된다.",
      "as_of": "2023-07",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f9",
      "claim": "연계 대상: Prakhya 외(2025-01)는 핸드헬드·로봇 탑재 라이다로 반복 수집한 3D 지도에서 시간에 따른 환경 변화를 탐지해 지도를 갱신하는 평생 3D 지도 작성 틀을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-160"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: lifelong mapping 은 시간에 따른 환경 변화를 탐지하고 지도를 갱신하며, 물품이 운반되는 창고 같은 환경에서 최신 지도 유지가 필요하다고 설명.",
      "as_of": "2025-01",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f10",
      "claim": "연계 대상: slam_toolbox 공식 README 는 평생 지도 작성으로 지도를 시간에 따라 정제·갱신할 수 있으나 노드 제거를 지원하는 '진정한 평생' 지도 작성은 매우 실험적인 구현이라고 적고, 위치추정 모드에서는 최근 스캔을 순환 버퍼에 두었다가 만료되면 지우며 바탕 지도는 바뀌지 않는다고 설명한다.",
      "tag": "사실",
      "source_ids": [
        "ref-270"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: 위치추정 모드는 \"maintains a rolling buffer of recent scans in the pose-graph\", 만료된 스캔은 제거되고 \"the underlying map is not affected\". 노드 제거는 'highly experimental'.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f11",
      "claim": "연계 대상: Nav2 비용 지도의 장애물 층(ObstacleLayer)은 레이저·점군 관측을 받아 2D 비용 지도에 장애물을 표시(marking)하고 광선 추적으로 빈 공간을 지우는(clearing) 관측 버퍼를 따로 두어, 임시 장애물을 정적 지도와 별도로 실행 중에 반영한다.",
      "tag": "사실",
      "source_ids": [
        "ref-743",
        "ref-644"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "헤더 주석: \"Takes in laser and pointcloud data to populate into 2D costmap\", marking_buffers_·clearing_buffers_ 를 따로 두고 광선 추적으로 빈 공간을 지움. README 는 비용 지도가 센서 관측으로 층을 갱신한다고 적음(같은 Nav2 프로젝트, 독립 확인 아님).",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f12",
      "claim": "VDA 5050 3.0.0 은 특정 구역 개방이나 최대 속도 변경 같은 환경의 (일시적) 변경을 관제 기능으로 두고, 진입 금지(BLOCKED) 구역을 정의하며, 구역 집합의 내용은 바꿀 수 없어 변경이 필요하면 새 zoneSetId 로 참조해야 하고 지도(mapId)마다 활성 구역 집합은 하나라고 규정한다.",
      "tag": "사실",
      "source_ids": [
        "ref-031"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "명세 5.3: \"(Temporary) changes in the environment, such as freeing certain areas or changing the maximum speed\". 6.4.2: 같은 zoneSetId 의 내용은 바뀌지 않으며 변경 시 새 zoneSetId 를 쓴다.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f13",
      "claim": "VDA 5050 3.0.0 은 지도를 mapId 와 mapVersion 의 조합으로 식별해 판 갱신을 mapVersion 으로 나타내고 새 판을 미리 내려받은 뒤 관제가 활성화하게 하며, 운용 모드 TEACH_IN 은 운영자가 지도 작성을 하는 동안 관제가 주문·동작을 보내지 않는 모드로 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-031"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "6.3.1: map version 은 이전 판의 갱신을 나타내며 지도는 미리 적재한 뒤 enableMap 으로 활성화. 6.6.6 TEACH_IN: 운영자가 매핑 등 교시를 하는 동안 관제는 주문·동작을 보내지 않음.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f14",
      "claim": "VDA 5050 3.0.0 에서 로봇이 주문 실행 중 노드에 도달할 수 없음을 알게 되면 NODE_UNREACHABLE 오류(CRITICAL)를 보고하고 다시 시도하지 않은 채 관제의 결정을 기다리므로, 현장의 예기치 않은 막힘이 관제 쪽 신호로 올라온다.",
      "tag": "사실",
      "source_ids": [
        "ref-031"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "6.4.5: 노드에 도달할 수 없으면 'NODE_UNREACHABLE' 오류를 level 'CRITICAL' 로 보고하고, 관제가 진행 방법을 결정하며 로봇은 다시 시도하지 않는다.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "예외·성과"
    },
    {
      "id": "f15",
      "claim": "Open-RMF 의 차선 요청 메시지(LaneRequest)는 플릿 이름과 열 차선·닫을 차선 번호 목록만 담아, 실행 중 차선 폐쇄·개방을 그래프 자체를 고치지 않고 요청으로 반영한다.",
      "tag": "사실",
      "source_ids": [
        "ref-569"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "메시지 필드: fleet_name(string), open_lanes(uint64[]), close_lanes(uint64[]).",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": "제약"
    },
    {
      "id": "f16",
      "claim": "확인한 자료를 이 위키가 묶으면, 도면–현장 차이는 지속성에 따라 (1) 개보수 같은 구조 변경은 재측량(scan-vs-BIM)이나 도면 기반 다중 세션 정렬로 찾아 도면·지도 판을 갱신하고, (2) 랙·팔레트·가구 같은 반정적 배치 변화는 반복 주행 데이터의 변화 탐지·지도 갱신과 관제의 구역·차선 규칙으로 반영하며, (3) 임시 장애물은 정적 지도에 넣지 않고 로봇 쪽 비용 지도가 실행 중에 처리하는 세 갈래로 나뉘는 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-745",
        "ref-221",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합이며 세 갈래 분류를 제시한 단일 출처는 찾지 못함. 근거: scan-vs-BIM(f1), BIM 기반 변화 탐지(f3·f5), 반정적 지도 갱신(f6~f8), 비용 지도 장애물 층(f11), 관제 구역·차선(f12·f15).",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f17",
      "claim": "차이를 찾는 경로는 시운전 전 재측량과 도면 대조, 시운전·운영 중 반복 주행 데이터의 다중 세션 정렬·변화 탐지, 운영 중 로봇이 보고하는 도달 불가 같은 예외 신호로 나뉘며, 이종 제조사를 연결하는 ROP 는 변화 탐지 계산은 로봇·제조사에 맡기고 탐지된 차이를 구역 집합·차선 폐쇄·지도 판으로 반영하고 도면 변경 이력을 관리하는 쪽을 맡는 경계가 될 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-745",
        "ref-744",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. SLAM·변화 탐지·비용 지도는 분류 원문 9장 '로봇 자체 지능·제어' 연계 대상이고, VDA 5050 구역 집합·지도 판과 Open-RMF 차선 폐쇄는 관제 쪽 인터페이스다.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": false
    ...
    {
      "id": "f18",
      "claim": "‘3층 출하 대기장’에 팔레트가 임시로 쌓여 로봇이 도달 불가를 보고하면, 짧은 막힘은 새 구역 집합의 진입 금지 구역이나 차선 폐쇄로 처리하고, 배치가 오래 유지되면 지도 판을 올리고 대기장 경유점과 제조사 지도 대응을 다시 확인하는 흐름이 필요할 것으로 보인다(설명용 가정 사례).",
      "tag": "추정",
      "source_ids": [
        "ref-031",
        "ref-569",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "근거: NODE_UNREACHABLE 보고(f14), 구역 집합 교체 규칙(f12), 지도 판(f13), 차선 폐쇄(f15), 물류 시설의 팔레트 임시 적치(f6). 판정 기준(지속 시간 등)은 근거 없음.",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "예외·성과",
      "source_unopened": false
    ...
    {
      "id": "f19",
      "claim": "이번에 확인한 도면–현장 차이 탐지 연구는 건설 품질 관리(scan-vs-BIM)와 단일 로봇·단일 플릿 SLAM 지도 갱신이 대부분이고, 여러 제조사 로봇 지도에 같은 현장 변화를 일관되게 반영하는 절차나 국내 물류센터 사례는 검색 범위에서 찾지 못했다(부재 확인 아님).",
      "tag": "추정",
      "source_ids": [
        "ref-745",
        "ref-749",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "검색 17회(한국어 4회 포함) 범위의 관찰. 국내 자료는 노후 건축물 Scan-to-BIM 연구(f2)만 확인.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
  ...
  "sources": [
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "VDA 5050 3.0.0 명세. 관제 기능(일시적 환경 변경), 구역 집합·진입 금지 구역, 지도 식별·판(mapId·mapVersion), 운용 모드, NODE_UNREACHABLE 오류 규정.",
      "fetched": true,
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 반복 수집한 라이다 3D 지도에서 변화를 탐지해 지도를 갱신하는 평생 지도 작성 틀.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. BIM 에서 세션 데이터를 만들고 실제 라이다 세션을 정렬한 뒤 BIM 에 없는 새 요소를 탐지·재구성하는 틀.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 건축 도면 그래프와 라이다 상황 그래프를 결합해 도면–현장 정렬과 구조 편차를 실시간 추정.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "평생 지도 작성·지도 병합·위치추정 모드(순환 스캔 버퍼, 바탕 지도 불변)와 실험적 노드 제거 기능 설명.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "플릿 이름과 열 차선·닫을 차선 번호 목록으로 실행 중 차선 폐쇄·개방을 요청하는 메시지 정의.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "title": "nav2_costmap_2d — README",
      "published": null,
      "url": "https://github.com/ros-navigation/navigation2/blob/main/nav2_costmap_2d/README.md",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "비용 지도가 여러 층으로 이루어지고 센서 관측으로 층을 갱신하며, 필터 마스크로 금지 구역·속도 제한을 표현한다고 설명.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      "id": "ref-743",
      "org": "ROS Navigation (ros-navigation/navigation2 GitHub)",
      "title": "nav2_costmap_2d — include/nav2_costmap_2d/obstacle_layer.hpp",
      "published": null,
      "url": "https://github.com/ros-navigation/navigation2/blob/main/nav2_costmap_2d/include/nav2_costmap_2d/obstacle_layer.hpp",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "Nav2 장애물 층 헤더. 레이저·점군 관측으로 2D 비용 지도에 장애물을 표시하고 광선 추적으로 빈 공간을 지우며, 표시·삭제용 관측 버퍼를 따로 둔다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/ros-navigation/navigation2/main/nav2_costmap_2d/include/nav2_costmap_2d/obstacle_layer.hpp",
      "source_unopened": false
    },
    {
      "id": "ref-744",
      "org": "Vega-Torres, M. A. (MigVega GitHub)",
      "title": "SLAM2REF — README",
      "published": null,
      "url": "https://github.com/MigVega/SLAM2REF",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "포즈 그래프 다중 세션 앵커링으로 라이다 데이터를 기준 지도(예: BIM·지상 레이저 스캔)에 정렬하고 갱신·정렬된 지도를 얻는 도구. Construction Robotics 8(2), 2024 논문 연계.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/MigVega/SLAM2REF/main/README.md",
      "source_unopened": false
    },
    {
      "id": "ref-745",
      "org": "Bosché, F. (Advanced Engineering Informatics)",
      ...
      "published": "2010",
      "url": "https://www.sciencedirect.com/science/article/abs/pii/S1474034609000482",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 설계 3D CAD 모델을 현장 레이저 스캔에 정합해 객체를 인식하고 시공 치수를 계산하는 scan-vs-BIM 방법(건설 품질 관리).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 자세 불확실성을 고려하고 일시적 동적 장애물에 강건한 라이다 점유 격자 지도 갱신 알고리즘, 창고 물품 배치 변화 모사로 시험(Sensors 23(13), 6066).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-747",
      "org": "Shaik, N. 외 (KI 2017, 저자 목록 미확인)",
      "title": "Dynamic Map Update of Non-static Facility Logistics Environment with a Multi-robot System",
      "published": "2017",
      "url": "https://link.springer.com/chapter/10.1007/978-3-319-67190-1_19",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 팔레트 임시 적치로 변하는 물류 시설에서 여러 로봇이 변화를 감지해 임시 지도를 만들고 현재 지도에 병합하는 실시간 지도 갱신.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-748",
      "org": "Qian, J. 외 (RSS 2023)",
      "title": "POV-SLAM: Probabilistic Object-Aware Variational SLAM in Semi-Static Environments",
      "published": "2023-07",
      "url": "https://arxiv.org/abs/2307.00488",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 반정적 객체의 객체 수준 변화를 추적하는 SLAM 과 4개월 간격 실제 창고 데이터셋.",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
    ...
    {
      "id": "ref-749",
      "org": "윤동희, 백주미, 심지수, 이동근, 송두삼(설비공학 논문집 36(5), 251-261)",
      "title": "노후건축물에서 모바일기기를 이용한 Scan-to-BIM 역설계 도면생성 방안",
      "published": "2024",
      "url": "https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART003077202",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 모바일 기기 스캔으로 노후 건축물 도면을 역설계해 기존 도면과 비교한 국내 연구(어린이집 3개소).",
      "source_unopened": true,
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null
  ...
  "page_proposals": [
    ...
    },
    {
      ...
      "rationale": "아이디어 페이지 5절(트랙 산출물): '도면–현장 차이 탐지와 반영' 소절 신설 — 근거 f1·f3·f6·f11·f12·f14·f15, 구현 가설 f16·f17(추정), 좌표 정렬과 층·목적지 이름 맞춤(q4-03)은 아직 조사되지 않음을 명시"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신(온톨로지 변경 없음): 6절 '도면–현장 차이·지도 버전을 층별 지도 속성으로 둘지 별도 개념으로 둘지' 항목의 근거 보강 — 지속성별 세 갈래(f16 추정), 구역 집합은 내용 불변·새 zoneSetId 로 교체(f12), 지도 판(f13), 차선 폐쇄는 그래프 수정이 아닌 요청(f15)"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 4 반영 제안 (f1, f3, f16, f17): 6절(주제 페이지 area06-s6)에 도면–현장 차이 탐지 방법(scan-vs-BIM, BIM 기반 다중 세션 변화 탐지)과 지속성별 반영 경로(추정), 9절에 변화 탐지 계산은 연계 대상이고 구역·차선·지도 판 반영은 ROP 쪽이라는 경계(추정)"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 4 반영 제안 (f12, f13, f18): 현장 변화에 따른 지도 판(mapVersion) 갱신과 구역 집합 교체(새 zoneSetId) 규칙"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 4 반영 제안 (f1, f2, f4): 시운전 전 재측량으로 도면과 현장을 대조하는 방법(scan-vs-BIM, 국내 노후 건축물 Scan-to-BIM 연구)과 기준 지도 정렬 도구"
    }
  ],
  "glossary_candidates": [
    {
...
### runs/2026-09-25-75/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검색 결과(Edinburgh Research Explorer·ScienceDirect)로 Bosché, Advanced Engineering Informatics 24(1), 107-118, 2010-01 확인. ICP 기반 정합, CAD 객체 인식, as-built 자세·치수 계산, 철골 구조 현장 실험은 스니펫과 일치. 다만 '이 개념은 scan-vs-BIM 으로 불린다'는 이 출처 스니펫에 나타나지 않아 별도 [추정] 문장으로 분리하도록 지시."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. KCI 검색 결과로 윤동희 외, 설비공학 논문집 36(5), 251-261 확인. 어린이집 3개소 평균 오차율 2.21%·5.99%·2.75%와 비교 기준(최종 BIM 결과의 실 폭·깊이 대 기존 건축도면)은 검증 검색 스니펫에서 재확인. '좁은 공간 고속 촬영·장애물 미제거 시 오차 증가' 부분은 검증 검색에서 재확인되지 않아 '저자 보고(검색 요약 기준)' 병기 지시. 단일 출처 수치이며 물류 시설이 아닌 노후 건축물 대상."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검색 결과(IAARC ISARC 2023 proceedings pp.521-528, arXiv 2408.15870)로 세 단계(BIM 기반 세션 데이터 생성, 다중 세션 앵커링, BIM 에 없는 새 요소 식별·재구성) 확인. f4 와 같은 TUM 저자 그룹이라 독립 확인 아님."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "raw.githubusercontent.com 으로 README 열람 확인: 'pose-graph multi-session anchoring to align your LiDAR data with a reference map', TLS 기준 최대 3 cm 6-DoF 자세, 'update a digital twin of a facility', Construction Robotics 8(2), 2024-07, DOI 10.1007/s41693-024-00126-w. 3 cm 는 저자 측 보고. README 의 '디지털 트윈 갱신'은 현재 상태 갱신 문맥이다."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 실행 2026-09-25-72 에서 검증된 finding 의 재인용이며 단계 4 페이지 3절(q4-01)에 같은 문장이 이미 있다. 기존 각주 ref-224 재사용, 본문 반복 대신 참조."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검색 결과(Springer, Semantic Scholar)로 KI 2017, 2017-09 게재, 저자 Shaik, N., Liebig, T., Kirsch, C., Müller, H. 확인. 팔레트 임시 적치로 시설 물류 환경이 정적이지 않음, 로봇별 변화 감지·임시 지도·선 특징으로 병합은 스니펫과 일치."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검색 결과(MDPI, PubMed)로 Stefanini·Ciancolini·Settimi·Pallottino, Sensors 23(13) 6066, 2023-06-30 확인. 자세 불확실성 고려, 사람·로봇 같은 동적 장애물의 일시 변화에 강건, 창고 물품 배치 변화를 모사한 100개 환경 변형 시험(시뮬레이션)은 스니펫과 일치."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 검색 결과(arXiv 2307.00488, RSS 2023)로 4개월 간격 두 궤적 연결, 팔레트·상자, 가동 중인 100m×80m Clearpath Robotics 시설(TorWIC 확장) 확인."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람. 출처는 참고문헌 색인에 이미 있는 ref-160(6. 지도·공간·위치 모델 페이지에서 재사용 중). 이번 검증에서 내용을 다시 검색하지 않았으며 리서치 스니펫 범위로 판단."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "raw README 열람 확인: 평생 지도 작성의 지도 정제·갱신, 'true lifelong' 노드 제거는 highly experimental, 위치추정 모드의 순환 스캔 버퍼와 'the underlying map is not affected'. 발행일 미확인, 확인일 2026-09-25."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "raw 헤더 열람 확인: 'Takes in laser and pointcloud data to populate into 2D costmap', marking_buffers_·clearing_buffers_, raytraceFreespace. ref-644 와 같은 Nav2 프로젝트라 독립 확인 아님. '정적 지도와 별도로 실행 중에 반영'은 층 구조 README(ref-644)에 기댄 해석이다."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "입력 원문 텍스트(data/source_texts/ref-031.txt)로 확인: 5.3 '(Temporary) changes in the environment, such as freeing certain areas or changing the maximum speed', BLOCKED 구역, 6.4.2 같은 zoneSetId 내용 불변·새 zoneSetId 사용, mapId 당 활성 구역 집합 하나. 명세 3.0.0, 발행일 미확인(oq-005)."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "입력 원문 텍스트로 확인: 6.3.1 mapId·mapVersion 식별, 미리 적재 후 enableMap 활성화, 표 10 TEACH_IN 은 운영자 매핑 등 교시 중이며 관제는 주문·동작을 보내지 않는다."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "입력 원문 텍스트 6.4.5 로 확인: NODE_UNREACHABLE, level CRITICAL, 관제가 진행 방법 결정, 로봇은 재시도하지 않음. 문장 끝 '현장의 예기치 않은 막힘이 관제 쪽 신호로 올라온다'는 명세에 없는 해석이라 [추정]으로 분리 지시."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "raw .msg 열람 확인: fleet_name(string), open_lanes(uint64[]), close_lanes(uint64[]). '그래프 자체를 고치지 않고'는 이 메시지 정의에 나타나지 않아 [추정]으로 분리하거나 삭제하도록 지시."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 종합 [추정]. 근거 finding 들은 모두 확인됨. 세 갈래 분류를 제시한 단일 출처 없음, 신뢰도 low 유지."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 종합 [추정]. 경계 서술(변화 탐지 계산은 로봇·제조사 연계 대상, 구역 집합·차선 폐쇄·지도 판 반영은 관제 쪽)은 분류 원문 9장과 맞는다."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "설명용 가정 사례 [추정]. 지속 시간 등 판정 기준은 근거 없음으로 명시되어 있어 수치를 넣지 않는 조건으로 유지."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 범위 관찰 [추정], 부재 확인 아님. 원문 미열람 출처에 기댄 종합이다."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "f5 는 단계 4 페이지 3절(q4-01)과 공간 그래프 스키마 초안 6절에 이미 실린 Shaheer 외(ref-224) 문장과 같다 — 기존 각주 재사용, q4-02 절에서는 반복 서술 대신 짧게 참조",
      "f10(slam_toolbox)·f11(Nav2 비용 지도)은 단계 4 페이지 q4-01 의 ref-270·ref-644 문장과 출처가 겹친다(다른 측면) — 기존 각주 재사용",
      "f12·f13 의 구역 집합·지도 판 규칙은 스키마 초안 6절(VDA 5050 구역 집합·지도 버전 항목)과 아이디어 페이지 4절에 일부 이미 있다 — 기존 ref-031 각주 재사용",
      "f9(ref-160)는 6. 지도·공간·위치 모델 페이지 8·9절에 이미 인용된 출처다"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "required_fixes": [
    "f1: 'Bosché(2010)는 … 제안했다' 부분은 [사실]로 두되 '이 개념은 scan-vs-BIM 으로 불린다'는 별도 문장으로 떼어 [추정]으로 표기한다 — ref-745 스니펫에 이 명칭이 나타나지 않는다. 연구 대상이 건설 시공 품질 관리이며 물류 적용은 미확인임을 함께 적는다.",
    "f2: 오차율 수치 문장에 '저자 보고, 단일 출처(원문 미열람)'와 비교 기준(BIM 결과의 실 폭·깊이 대 기존 건축도면)을 적고, '좁은 공간 고속 촬영·장애물 미제거 시 오차 증가' 부분에는 '검색 요약 기준' 을 병기한다 — 검증 검색에서 이 조건 부분은 재확인되지 않았다.",
    "f14: 문장 끝 '현장의 예기치 않은 막힘이 관제 쪽 신호로 올라온다'를 별도 문장으로 떼어 [추정]으로 표기한다 — 명세 6.4.5 에 없는 해석이다.",
    "f15: '그래프 자체를 고치지 않고 요청으로 반영한다'를 삭제하거나 별도 [추정] 문장으로 둔다 — LaneRequest.msg 는 fleet_name·open_lanes·close_lanes 세 필드만 정의하고 그래프 수정 여부를 말하지 않는다.",
    "f3·f4: 두 출처(ref-221, ref-744)가 같은 TUM 저자 그룹(Vega-Torres·Braun·Borrmann)임을 본문 또는 4절 불확실성에 적고 독립 교차 확인으로 쓰지 않는다. f11 의 ref-743·ref-644 도 같은 Nav2 프로젝트임을 적는다.",
    "f4: README 의 '시설 디지털 트윈 갱신'은 현재 상태를 표현하는 지도 갱신으로만 옮기고, 22. 시뮬레이션·예측용 디지털 트윈(가정한 미래를 실험)의 근거로 연결하지 않는다 — 분류 원문 7장의 8. 실시간 세계 상태·데이터 일관성과 22 구분.",
    "f3~f11: '연계 대상:' 표기를 유지하고 SLAM·다중 세션 정렬·변화 탐지·비용 지도를 ROP 직접 범위처럼 서술하지 않는다(분류 원문 9장 '로봇 자체 지능·제어').",
    "f16~f19: 모두 [추정]으로 두고 '이 위키의 종합이며 단일 출처 없음'을 밝히며, f18 은 '다음은 설명을 위한 가상의 시나리오이다'로 시작하고 지속 시간 같은 수치를 넣지 않는다. f16 의 세 갈래를 mermaid 로 그릴 때 '이 위키의 추정 구조'임을 도식 아래에 적는다.",
    "인용: 한 페이지 안에서 ref-270(f10)과 ref-031(f12) 각각 직접 인용은 1회만 쓰고 나머지는 재서술한다 — f10 excerpt 에는 같은 출처의 인용 구절이 두 개 있다.",
    "각주: 원문을 열지 않은 ref-745·ref-746·ref-747·ref-748·ref-749·ref-160·ref-221·ref-224 의 각주 정의 접근일 뒤에 ' (원문 미열람)'을 붙이고 reference_updates 의 해당 항목에 source_unopened: true 를 넣는다(web_fetch_available: false 환경). ref-031·ref-270·ref-569·ref-644·ref-743·ref-744 는 원문을 연 출처로 둔다.",
    "ref-747: 참고문헌 항목의 기관 칸 'Shaik, N. 외 (KI 2017, 저자 목록 미확인)'을 'Shaik, N., Liebig, T., Kirsch, C., & Müller, H. (KI 2017)'로, 발행일을 2017-09 로 고친다 — 검증 검색(Semantic Scholar·Springer) 결과.",
    "ref-745: 참고문헌 항목의 발행일을 2010-01 로, 제목 뒤 서지를 'Advanced Engineering Informatics 24(1), 107-118'로 보강한다 — 검증 검색 결과.",
    "ref-744: 참고문헌 항목에 연계 논문 'Construction Robotics 8(2), 2024-07, DOI 10.1007/s41693-024-00126-w'를 병기한다(발행일은 README 이므로 미확인 유지) — raw README 열람 결과.",
    "ref-224·ref-270·ref-644·ref-031·ref-160·ref-221·ref-569 는 기존 참고문헌 id 이므로 각주 정의를 기존 시드 문자열 그대로 재사용하고 새 id 를 만들지 않는다.",
    "단계 페이지 6절: 완료 조건 두 번째 행(도면–현장 정합 절차 초안)은 '미충족'으로 두고 아래 줄을 '다음 단계로 전환: 아니오(도면–현장 정합 절차 초안 없음 — q4-03 미답; 막힌 질문 q4-03·q4-04·q4-05·q4-07·q4-08·q4-09·q4-10)'로 쓴다. 트랙 개요 상태 줄의 현재 단계는 '단계 4. 지도 변환 보정과 현장 정합'을 유지한다.",
    "q4-02: 백로그·단계 페이지 2절에서 '답함'(답한 실행 2026-09-25-75, 답 위치 #q4-02)으로 바꾸고 3절에 '### q4-02 … {#q4-02}' 소제목을 둔다. 새 질문 두 건은 단계 4·단계 5 태그 그대로 백로그에 등록하고 origin 은 각각 f17·f8 로 둔다.",
    "oq-022 는 해결로 바꾸지 않는다 — 국내 물류센터 도면–현장 차이 확인 사례는 이번에도 찾지 못했다(f19).",
    "공간 그래프 스키마 초안: 온톨로지 변경이 없으므로 ontology_version '1.0' 을 유지하고, 6절 '정렬 정보·도면–현장 차이·지도 버전' 항목에 근거 보강으로 f12(새 zoneSetId 교체)·f13(mapVersion)·f15(차선 폐쇄 요청)·f16(추정)만 덧붙인다. 개념·관계 표는 바꾸지 않는다."
  ],
  "confidence": "low",
  "verification_note": "판정: 조건부 승인. 이번 실행은 원문 열람이 차단된 환경(web_fetch_available: false, fetch_mode mirror_only)에서 검증됐다. 확인 19건, 미확인 0건, 교차 확인 0건. 강등: 없음(f1 의 명칭 부분, f14·f15 의 해석 부분은 [추정] 문장으로 분리 지시). 원문 미열람 출처: ref-745, ref-746, ref-747, ref-748, ref-749, ref-160, ref-221, ref-224(검색 결과 일치로 실재 확인). 원문 확인 출처: ref-031(입력 원문 텍스트), ref-270·ref-569·ref-743·ref-744(raw.githubusercontent.com 검증 열람), ref-644(리서치 열람). 주의: q4-02 의 답은 도면–현장 차이를 지속성별 세 갈래(구조 변경·반정적 배치·임시 장애물)로 나눈 이 위키의 종합 [추정]이며 이를 제시한 단일 출처가 없다. scan-vs-BIM·국내 Scan-to-BIM 연구는 건설·시설 분야이고 물류 적용은 미확인이다. SLAM·변화 탐지·비용 지도는 로봇 쪽 연계 대상이다. 같은 저자 그룹(ref-221·ref-744)과 같은 프로젝트(ref-743·ref-644) 출처는 독립 확인으로 보지 않았다. 브리프 메타데이터 주의: ref-031 의 fetched_via 가 github_raw 로 적혔으나 self_check.limits 는 입력 원문 텍스트(inbox)라고 적어 서로 다르다(원문 열람 자체는 입력 원문으로 확인됨). 신규 출처 id ref-743~ref-749 는 과거 브리프(실행 2026-09-25-36)가 쓴 임시 id 구간과 겹치므로 퍼블리셔 병합 시 기존 URL 대조가 필요하다. 검증 검색 9회(리서치 17회와 합쳐 26/40), 열람 4회. 정정 요청 없음. 온톨로지 변경 승인: 없음(제안 없음) / 거부: 없음. 단계 완료 조건: 미충족(부족: 도면–현장 정합 절차 초안 — q4-02 는 이번에 답했으나 q4-03 미답). 단계 전환: 미승인(막힌 질문 q4-03·q4-04·q4-05·q4-07·q4-08·q4-09·q4-10).",
  "retry_reason": null,
  "track_checks": {
...
```json
{
  ...
  "outline": [
    {
      ...
      "section": "2. 질문 목록",
      "budget_chars": 900,
      "summary": "q4-02를 답함(실행 2026-09-25-75, 답 위치 #q4-02)으로 바꾸고 이 단계로 보내는 후속 질문 q4-11을 열림으로 더한다."
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
      "section": "3. 조사 결과",
      "budget_chars": 3600,
      "summary": "도면–현장 차이는 지속성에 따라 구조 변경·반정적 배치 변화·임시 장애물의 세 갈래로 반영 경로가 나뉘는 것으로 보인다(이 위키의 종합). [추정][^ref-745][^ref-221][^ref-747][^ref-743][^ref-031][^ref-569]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 1500,
      "summary": "보정 여섯 묶음(q4-01)과 도면–현장 차이의 세 갈래·ROP 경계(q4-02)가 결론이며 모두 추정 중심이라 신뢰도 low 이다. [추정][^ref-153][^ref-031][^ref-569]"
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
      "section": "5. 이 단계가 낳은 후속 질문",
      "budget_chars": 700,
      "summary": "q4-11(판정 기준, f17)과 q5-07(탐지 성능 지표, f8)을 더한다."
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
      "section": "6. 완료 조건 충족 현황",
      "budget_chars": 600,
      "summary": "정합 절차 초안 조건은 q4-03 미답으로 미충족이며 전환하지 않는다."
    },
    {
      "path": "docs/ideas/floorplan-recognition.md",
      "section": "5. 구현 가설",
      "budget_chars": 1600,
      "summary": "도면–현장 차이 탐지와 반영 소절을 더한다. [추정][^ref-745][^ref-221][^ref-747][^ref-743][^ref-031][^ref-569]",
      "planned_findings": [
        "f1",
        "f3",
        "f6",
    ...
    },
    {
      ...
      "budget_chars": 700,
      "summary": "정렬 정보·도면–현장 차이·지도 버전 항목에 f12·f13·f15·f16 근거를 보강한다(개념·관계 변경 없음).",
      "planned_findings": [
        "f12",
        "f13",
        "f15",
  ...
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "q4-02 답함(3절 소제목 신설), 후속 질문 q4-11·q5-07, 4·5·6·7·8절 갱신. H1 아래 상태 줄(패치 범위 밖)은 열린 질문 8건·답한 질문 2건으로 바뀌어야 한다",
      "patches": [
        {
          "section": "2. 질문 목록",
          "action": "replace",
          ...
          "content": "(절 본문 생략 — runs/2026-09-25-75/pages/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md 의 해당 절을 본다)"
        },
        {
          "section": "3. 조사 결과",
          "action": "append",
    ...
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "5절에 '도면–현장 차이 탐지와 반영' 소절 신설(q4-02, 실행 2026-09-25-75, 추정 중심)",
      "patches": [
        {
          "section": "5. 구현 가설",
          "action": "append",
          ...
          "content": "(절 본문 생략 — runs/2026-09-25-75/pages/ideas/floorplan-recognition.md 의 해당 절을 본다)"
        }
      ]
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "6절 정렬 정보·도면–현장 차이·지도 버전 항목에 근거 보강(f12·f13·f15·f16, 실행 2026-09-25-75). 개념·관계 변경 없음, 초안 버전 v1.0 유지",
      "patches": [
        {
          "section": "6. 미해결 모델링 질문",
          "action": "append",
          ...
          "content": "(절 본문 생략 — runs/2026-09-25-75/pages/tracks/floorplan-recognition/space-graph-schema-draft.md 의 해당 절을 본다)"
        }
      ]
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "6절 살아있는 산출물에 실행 2026-09-25-75(단계 4, q4-02) 반영 내용 한 단락 추가. 상태 줄(현재 단계 단계 4 유지, 마지막 트랙 실행 2026-09-25)은 변경 없음",
      "patches": [
        {
          "section": "6. 살아있는 산출물 링크",
          "action": "append",
          "content": "(절 본문 생략 — runs/2026-09-25-75/pages/tracks/floorplan-recognition/index.md 의 해당 절을 본다)"
        }
      ]
    }
  ],
  "changelog_entry": "2026-09-25 | 건축 도면 자동 인식 단계 4 | q4-02 답함(도면–현장 차이의 지속성별 탐지·반영 경로와 ROP 경계, 추정 중심), 후속 질문 q4-11·q5-07, 스키마 초안 변경 없음(v1.0) | run 2026-09-25-75",
  "index_updates": {
    "home_recent": "2026-09-25 — 건축 도면 자동 인식 단계 4. 지도 변환 보정과 현장 정합: q4-02 답함(도면–현장 차이를 구조 변경·반정적 배치·임시 장애물로 나눈 탐지·반영 경로, 추정), 후속 질문 2건",
    "category_recent": "2026-09-25 — 6. 지도·공간·위치 모델: 건축 도면 자동 인식 트랙 단계 4에서 도면–현장 차이 탐지와 반영 경로(q4-02)를 정리하고 6절·9절 반영을 제안",
    "area_recent": "2026-09-25 — 6. 지도·공간·위치 모델: 트랙 건축 도면 자동 인식 단계 4 q4-02(도면–현장 차이 탐지·반영) 결과를 6. 대표 접근법과 기술, 9. ROP가 직접 맡는 것과 외부와 연계하는 것 절에 반영 제안"
  },
  "glossary_updates": [
    {
      "action": "new",
      ...
      "term_en": "Scan-vs-BIM",
      "definition": "설계 BIM·3D 모델을 현장 레이저 스캔 점군에 정합해 모델 객체의 시공 상태와 설계 대비 편차를 찾는 방법이다.",
      "description": "Bosché(2010)의 설계 CAD 모델–레이저 스캔 정합과 객체 인식·시공 치수 계산 방법이 대표 예다. 이 명칭 자체는 해당 출처에서 확인하지 못했고, 연구 대상은 건설 시공 품질 관리이며 물류 시설 적용은 미확인이다.",
      "related_areas": [
        6,
        21
      ],
    ...
    {
      "action": "new",
      "slug": "semi-static-object",
      "term_ko": "반정적 객체",
      "term_en": "Semi-static Object",
      "definition": "팔레트·랙·가구처럼 로봇이 관측하는 동안은 움직이지 않지만 시간이 지나면 위치가 바뀌거나 나타나고 사라져 정적 지도를 낡게 만드는 물체다.",
      "description": "물류 시설에서는 팔레트 임시 적치가 대표 예이며, 반정적 객체의 변화를 추적·갱신하는 SLAM·지도 갱신 연구는 로봇 쪽 연계 대상이다.",
      "related_areas": [
        6,
        8
      ],
  ...
  "reference_updates": [
    ...
    },
    {
      "id": "ref-744",
      "org": "Vega-Torres, M. A. (MigVega GitHub)",
      "title": "SLAM2REF — README (연계 논문 Construction Robotics 8(2), 2024-07, DOI 10.1007/s41693-024-00126-w)",
      "published": null,
      "url": "https://github.com/MigVega/SLAM2REF",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "포즈 그래프 다중 세션 앵커링으로 라이다 데이터를 기준 지도(BIM·지상 레이저 스캔 등)에 정렬하고 갱신·정렬된 지도를 얻는 도구. 최대 3 cm 정확도는 저자 측 보고.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md"
      ]
    },
    {
      "id": "ref-745",
      "org": "Bosché, F. (Advanced Engineering Informatics)",
      "title": "Automated recognition of 3D CAD model objects in laser scans and calculation of as-built dimensions for dimensional compliance control in construction (Advanced Engineering Informatics 24(1), 107-118)",
      "published": "2010-01",
      "url": "https://www.sciencedirect.com/science/article/abs/pii/S1474034609000482",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 설계 3D CAD 모델을 현장 레이저 스캔에 정합해 객체를 인식하고 시공 치수를 계산하는 방법(건설 품질 관리).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
        "docs/ideas/floorplan-recognition.md",
    ...
    {
      "id": "ref-747",
      "org": "Shaik, N., Liebig, T., Kirsch, C., & Müller, H. (KI 2017)",
      "title": "Dynamic Map Update of Non-static Facility Logistics Environment with a Multi-robot System",
      "published": "2017-09",
      "url": "https://link.springer.com/chapter/10.1007/978-3-319-67190-1_19",
      "type": "논문",
  ...
  "flow_matrix_updates": [
    {
      "step": "출하",
      "item": "제약",
      "link": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md#q4-02",
  ...
  "standards_updates": [
    {
      "name": "SLAM2REF (라이다 데이터의 기준 지도 다중 세션 정렬 도구)",
      "kind": "오픈소스",
      "org": "Vega-Torres, M. A. (MigVega GitHub)",
      "url": "https://github.com/MigVega/SLAM2REF",
      "related_areas": [
      ...
      "summary": "포즈 그래프 다중 세션 앵커링으로 라이다 데이터를 BIM·지상 레이저 스캔 같은 기준 지도에 정렬하고 갱신된 지도를 얻는 도구다. 로봇 쪽 연계 대상 기술이다.",
      "ref_id": "ref-744"
    }
  ],
  ...
  "track_updates": {
    ...
    "log_entry": "답한 질문: q4-02(도면–현장 차이의 지속성별 탐지·반영 경로와 ROP 경계, 근거 f1~f19, 종합은 추정) / 새 질문: q4-11(단계 4, f17), q5-07(단계 5, f8) / 온톨로지 변경: 없음(공간 그래프 스키마 초안 v1.0 유지, 6절 정렬 정보·도면–현장 차이·지도 버전 항목에 f12·f13·f15·f16 근거 보강만) / 완료 조건 평가: 미충족(부족: 도면–현장 정합 절차 초안 — q4-03 미답) / 세부영역 반영 제안: 6. 지도·공간·위치 모델 2건, 21. 온보딩·설정·현장 시운전 1건, 24. 자산·소프트웨어 수명주기 관리 1건 / 다음 실행 제안: q4-03",
    "overview_progress": "단계 4 진행 중 — 열린 질문 8, 답함 2, 완료 조건 미충족"
  },
  "area_reflection_proposals": [
    {
      "area_no": 6,
      "section": "6. 대표 접근법과 기술",
      "summary": "도면–현장 차이 탐지 방법(scan-vs-BIM 재측량 대조, BIM 기반 다중 세션 정렬·변화 탐지 — 연계 대상)과 지속성별 반영 경로(구조 변경·반정적 배치·임시 장애물, 추정). 근거 ref-745·ref-221·ref-747·ref-743·ref-031·ref-569."
    },
    {
      "area_no": 6,
      "section": "9. ROP가 직접 맡는 것과 외부와 연계하는 것 (부록 A 9장 기준)",
      "summary": "변화 탐지 계산·SLAM·비용 지도는 로봇·제조사 쪽 연계 대상이고, 탐지된 차이를 구역 집합(새 zoneSetId)·차선 폐쇄·지도 판으로 반영하고 도면 변경 이력을 관리하는 것은 ROP 쪽이라는 경계(추정). 근거 ref-031·ref-569·ref-270."
    },
    {
      "area_no": 21,
      "section": "6. 대표 접근법과 기술",
      "summary": "시운전 전 재측량으로 도면과 현장을 대조하는 방법(Bosché 2010 scan-vs-BIM, 국내 노후 건축물 Scan-to-BIM 연구의 저자 보고 오차율)과 기준 지도 정렬 도구 SLAM2REF(연계 대상). 근거 ref-745·ref-749·ref-744."
    },
    {
      "area_no": 24,
      "section": "6. 대표 접근법과 기술",
      "summary": "현장 변화에 따른 지도 판(mapVersion) 갱신·사전 적재 후 활성화와 구역 집합 교체(내용 불변, 새 zoneSetId) 규칙, 임시 적치 대응을 판 갱신과 구역·차선 처리로 나누는 흐름(추정). 근거 ref-031."
    }
  ]
}
```

### runs/2026-09-25-75/format_check.md
...
- 차등 갱신 패치 적용:
    ...
    - docs/tracks/floorplan-recognition/space-graph-schema-draft.md (1개 절)
    - docs/tracks/floorplan-recognition/index.md (1개 절)
```

### runs/2026-09-25-75/pages/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-440, ref-079, ref-153, ref-080, ref-031, ref-082, ref-081, ref-628, ref-224, ref-270, ref-644, ref-645, ref-646, ref-647, ref-648, ref-745, ref-749, ref-221, ref-744, ref-747, ref-746, ref-748, ref-160, ref-743, ref-569]
last_run: 2026-09-25
version: 3
---
...
확인한 자료를 이 위키가 묶으면, 도면–현장 차이는 지속성에 따라 (1) 개보수 같은 구조 변경은 재측량이나 도면 기반 다중 세션 정렬로 찾아 도면·지도 판을 갱신하고, (2) 랙·팔레트·가구 같은 반정적 배치 변화는 반복 주행 데이터의 변화 탐지·지도 갱신과 관제의 구역·차선 규칙으로 반영하며, (3) 임시 장애물은 정적 지도에 넣지 않고 로봇 쪽 비용 지도가 실행 중에 처리하는 세 갈래로 나뉘는 것으로 보인다. 이는 이 위키의 종합이며 이 세 갈래를 제시한 단일 출처는 없다. [추정][^ref-745][^ref-221][^ref-224][^ref-747][^ref-746][^ref-748][^ref-743][^ref-031][^ref-569]
...
- Bosché(2010)는 설계 3D CAD·BIM 모델(as-planned)을 현장 레이저 스캔 점군(as-built)에 정합한 뒤 모델 객체를 점군에서 자동 인식하고 시공된 치수를 계산해 치수 적합성을 관리하는 방법을 제안했다(Advanced Engineering Informatics 24(1), 2010-01, 건설 시공 품질 관리 대상). [사실][^ref-745]
- 이런 방식은 스캔 대 BIM 비교(scan-vs-BIM)로 불리는 것으로 보이나, 이 명칭은 위 출처에서 확인하지 못했다. [추정][^ref-745] 물류 시설에 적용한 사례는 미확인이다.
- 국내 연구(설비공학 논문집 36(5), 2024)는 노후 건축물에서 모바일 기기로 Scan-to-BIM 역설계 도면을 만들어, BIM 결과의 실 폭·깊이를 기존 건축도면과 비교했을 때 어린이집 3개소의 평균 오차율이 2.21%·5.99%·2.75%였다고 보고했다(저자 보고, 단일 출처, 원문 미열람). [사실][^ref-749] 좁은 공간에서 빠르게 스캔하거나 표면 장애물을 치우지 않으면 오차가 크게 늘었다는 내용은 검색 요약 기준이다. [사실][^ref-749] 대상은 물류 시설이 아닌 노후 건축물이다.
...
- 연계 대상: SLAM2REF 공식 저장소 README 는 포즈 그래프 다중 세션 앵커링으로 라이다 데이터를 기준 지도나 다른 세션에 정렬하고, 기준 지도와 정렬된 갱신 지도를 시설의 현재 상태를 나타내는 지도 갱신에 쓸 수 있다고 적으며, 정밀 지상 레이저 스캔을 기준으로 하면 최대 3 cm 정확도라고 적는다(저자 측 보고, 2026-09-25 확인). [사실][^ref-744] 이 도구는 BIM-SLAM 과 같은 TUM 저자 그룹의 것이라 두 출처는 독립 교차 확인이 아니다.
- 도면 그래프와 라이다 상황 그래프를 결합해 도면–현장의 구조 편차를 실시간 추정하는 연구는 위 q4-01 절에 이미 실었다. [사실][^ref-224]

#### 반정적 배치 변화의 지도 갱신

- 연계 대상: Shaik 외(KI 2017, 2017-09)는 팔레트 등이 임시로 적치되어 물류 시설 환경이 정적이지 않다고 보고, 여러 로봇이 현재 지도와 비교해 변화를 감지해 임시 지도를 만들고 위치추정 정보와 지도의 선 특징으로 현재 지도에 병합하는 실시간 지도 갱신 방법을 제안했다. [사실][^ref-747]
- 연계 대상: Stefanini 외(Sensors 23(13), 2023-06-30)는 로봇 자세 추정의 불확실성을 고려하고 사람·다른 로봇 같은 동적 장애물의 일시적 변화에는 강건한 라이다 점유 격자 지도 갱신 알고리즘을 제안하고, 창고의 물품 배치가 시간에 따라 바뀌는 상황을 모사해 시험했다. [사실][^ref-746]
- 연계 대상: POV-SLAM(RSS 2023)은 반정적 객체의 객체 수준 변화를 추적·재구성하는 SLAM 이며, 가동 중인 100m×80m 공장·창고에서 4개월 간격으로 수집해 팔레트·상자 위치가 바뀐 실제 창고 데이터셋으로 평가했다. [사실][^ref-748]
...
- 연계 대상: Nav2 비용 지도의 장애물 층(ObstacleLayer)은 레이저·점군 관측을 받아 2D 비용 지도에 장애물을 표시하고 광선 추적으로 빈 공간을 지우는 관측 버퍼를 따로 두어, 임시 장애물을 정적 지도와 별도로 실행 중에 반영한다(발행일 미확인, 2026-09-25 확인). [사실][^ref-743][^ref-644] 두 출처는 같은 Nav2 프로젝트라 독립 교차 확인이 아니다.
...
- 차이를 찾는 경로는 시운전 전 재측량과 도면 대조, 시운전·운영 중 반복 주행 데이터의 다중 세션 정렬·변화 탐지, 운영 중 로봇이 보고하는 도달 불가 같은 예외 신호로 나뉘며, 이종 제조사를 연결하는 ROP 는 변화 탐지 계산은 로봇·제조사에 맡기고 탐지된 차이를 구역 집합·차선 폐쇄·지도 판으로 반영하고 도면 변경 이력을 관리하는 쪽을 맡는 경계가 될 것으로 보인다. 이는 이 위키의 종합이며 단일 출처는 없다. [추정][^ref-745][^ref-744][^ref-221][^ref-031][^ref-569][^ref-270]
- 이번에 확인한 도면–현장 차이 탐지 연구는 건설 품질 관리(scan-vs-BIM)와 단일 로봇·단일 플릿 SLAM 지도 갱신이 대부분이고, 여러 제조사 로봇 지도에 같은 현장 변화를 일관되게 반영하는 절차나 국내 물류센터 사례는 검색 범위에서 찾지 못했다(부재 확인 아님). 이 역시 이 위키의 종합이다. [추정][^ref-745][^ref-749][^ref-746][^ref-747][^ref-748]
...
| 완료·인계 | 해당 없음 |
| 예외·성과 | 로봇은 도달 불가를 보고하고 재시도 없이 관제의 결정을 기다린다. [사실][^ref-031] 짧은 막힘은 새 구역 집합의 진입 금지 구역이나 차선 폐쇄로 처리하고, 배치가 오래 유지되면 지도 판을 올리고 대기장 경유점과 제조사 지도 대응을 다시 확인하는 흐름이 필요할 것으로 보인다. 이는 이 위키의 종합이며, 두 처리를 가르는 기준(지속 시간 등)은 근거가 없어 후속 질문 q4-11 로 남긴다. [추정][^ref-031][^ref-569][^ref-747] 처리량·시간에 주는 영향은 미확인이다. |
...
- 도면–현장 차이는 지속성에 따라 구조 변경·반정적 배치 변화·임시 장애물의 세 갈래로 나누어 반영 경로를 달리해야 할 것으로 보인다. [추정][^ref-745][^ref-221][^ref-747][^ref-743][^ref-031][^ref-569]
- 차이를 찾는 경로는 재측량과 도면 대조, 반복 주행 데이터의 다중 세션 정렬·변화 탐지, 운영 중 도달 불가 같은 예외 신호로 나뉘고, ROP 는 탐지 계산을 로봇·제조사에 맡기고 탐지된 차이를 구역 집합·차선 폐쇄·지도 판으로 반영하며 도면 변경 이력을 관리하는 쪽을 맡을 것으로 보인다. [추정][^ref-745][^ref-744][^ref-031][^ref-569][^ref-270]
- 위 결론의 핵심은 모두 이 위키의 종합 [추정]이며, 이를 제시한 단일 출처가 없어 이 단계 페이지의 종합 신뢰도는 low 이다.

**남은 불확실성**
- 교차 확인된 주장이 없다. 층 정렬 근거 두 문장은 같은 traffic-editor 문서, 비용 지도 근거(ref-743·ref-644)는 같은 Nav2 프로젝트, BIM 기반 다중 세션 정렬 근거(ref-221·ref-744)는 같은 TUM 저자 그룹(Vega-Torres·Braun·Borrmann)에서 나왔다.
- 원문을 열지 못한 출처(ref-081, ref-628, ref-224, ref-645, ref-646, ref-647, ref-648, ref-745, ref-746, ref-747, ref-748, ref-749, ref-160, ref-221)는 검색 요약 범위에서만 썼다. Vega-Torres 외의 창문·문·가구 제외 여부와 유리 외 반사면(거울·금속)의 영향은 확인하지 못했다.
...
[^ref-749]: 윤동희, 백주미, 심지수, 이동근, 송두삼(설비공학 논문집 36(5), 251-261), 노후건축물에서 모바일기기를 이용한 Scan-to-BIM 역설계 도면생성 방안, 2024, https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART003077202, 접근일 2026-09-25 (원문 미열람)
...
[^ref-744]: Vega-Torres, M. A. (MigVega GitHub), SLAM2REF — README (연계 논문 Construction Robotics 8(2), 2024-07, DOI 10.1007/s41693-024-00126-w), 미확인, https://github.com/MigVega/SLAM2REF, 접근일 2026-09-25
[^ref-747]: Shaik, N., Liebig, T., Kirsch, C., & Müller, H. (KI 2017), Dynamic Map Update of Non-static Facility Logistics Environment with a Multi-robot System, 2017-09, https://link.springer.com/chapter/10.1007/978-3-319-67190-1_19, 접근일 2026-09-25 (원문 미열람)
[^ref-746]: Stefanini, E., Ciancolini, E., Settimi, A., & Pallottino, L., Safe and Robust Map Updating for Long-Term Operations in Dynamic Environments, 2023-06-30, https://www.mdpi.com/1424-8220/23/13/6066, 접근일 2026-09-25 (원문 미열람)
[^ref-748]: Qian, J. 외 (RSS 2023), POV-SLAM: Probabilistic Object-Aware Variational SLAM in Semi-Static Environments, 2023-07, https://arxiv.org/abs/2307.00488, 접근일 2026-09-25 (원문 미열람)
...
[^ref-743]: ROS Navigation (ros-navigation/navigation2 GitHub), nav2_costmap_2d — include/nav2_costmap_2d/obstacle_layer.hpp, 미확인, https://github.com/ros-navigation/navigation2/blob/main/nav2_costmap_2d/include/nav2_costmap_2d/obstacle_layer.hpp, 접근일 2026-09-25
...
### runs/2026-09-25-75/pages/ideas/floorplan-recognition.md
...
sources: [ref-031, ref-062, ref-063, ref-064, ref-065, ref-066, ref-067, ref-068, ref-069, ref-070, ref-071, ref-072, ref-073, ref-074, ref-075, ref-076, ref-077, ref-078, ref-079, ref-081, ref-082, ref-083, ref-084, ref-085, ref-086, ref-109, ref-120, ref-220, ref-221, ref-222, ref-223, ref-224, ref-225, ref-226, ref-227, ref-046, ref-212, ref-213, ref-214, ref-215, ref-216, ref-217, ref-218, ref-219, ref-241, ref-105, ref-163, ref-265, ref-267, ref-268, ref-271, ref-274, ref-156, ref-157, ref-158, ref-331, ref-332, ref-333, ref-334, ref-335, ref-336, ref-338, ref-339, ref-340, ref-341, ref-342, ref-345, ref-419, ref-420, ref-421, ref-422, ref-423, ref-424, ref-425, ref-426, ref-427, ref-428, ref-429, ref-430, ref-432, ref-433, ref-434, ref-435, ref-436, ref-440, ref-441, ref-442, ref-456, ref-457, ref-458, ref-459, ref-462, ref-463, ref-536, ref-413, ref-641, ref-642, ref-228, ref-572, ref-573, ref-574, ref-229, ref-038, ref-575, ref-576, ref-315, ref-314, ref-283, ref-348, ref-461, ref-406,
  ref-629, ref-632, ref-153, ref-644, ref-645, ref-648, ref-270, ref-745, ref-747, ref-743, ref-569]
last_run: 2026-09-25
version: 14
---
...
확인한 자료를 이 위키가 묶으면, 도면–현장 차이는 지속성에 따라 구조 변경(개보수)은 재측량이나 도면 기반 다중 세션 정렬로 찾아 도면·지도 판을 갱신하고, 랙·팔레트·가구 같은 반정적 배치 변화는 반복 주행 데이터의 변화 탐지와 관제의 구역·차선 규칙으로 반영하며, 임시 장애물은 정적 지도에 넣지 않고 로봇 쪽 비용 지도가 실행 중에 처리하는 세 갈래로 나뉘는 것으로 보인다. 이는 이 위키의 종합이며 이를 제시한 단일 출처는 없다. [추정][^ref-745][^ref-221][^ref-747][^ref-743][^ref-031][^ref-569] 이 소절은 [q4-02 답](../tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md#q4-02)(실행 2026-09-25-75)의 요약이며, 문장별 상세는 단계 페이지에 있다.

**근거**

- Bosché(2010)는 설계 3D CAD·BIM 모델을 현장 레이저 스캔 점군에 정합한 뒤 모델 객체를 자동 인식하고 시공 치수를 계산해 치수 적합성을 관리하는 방법을 제안했다(건설 시공 품질 관리 대상). [사실][^ref-745] 이 방식은 scan-vs-BIM 으로 불리는 것으로 보이나 이 명칭은 위 출처에서 확인하지 못했고, 물류 시설 적용은 미확인이다. [추정][^ref-745]
- 연계 대상: BIM-SLAM(2024-08)은 BIM 에서 만든 세션 데이터와 실제 3D 라이다 세션을 다중 세션 앵커링으로 정렬한 뒤 BIM 에 없는 새 요소를 탐지·재구성한다. [사실][^ref-221]
- 연계 대상: Shaik 외(KI 2017)는 팔레트 임시 적치로 정적이지 않은 물류 시설에서 여러 로봇이 변화를 감지해 임시 지도를 만들고 현재 지도에 병합하는 실시간 지도 갱신을 제안했다. [사실][^ref-747]
- 연계 대상: Nav2 비용 지도의 장애물 층은 레이저·점군 관측으로 장애물을 표시하고 광선 추적으로 빈 공간을 지워 임시 장애물을 정적 지도와 별도로 실행 중에 반영한다. [사실][^ref-743][^ref-644] 두 출처는 같은 Nav2 프로젝트라 독립 교차 확인이 아니다.
...
- 차이를 찾는 경로는 시운전 전 재측량과 도면 대조, 반복 주행 데이터의 다중 세션 정렬·변화 탐지, 운영 중 도달 불가 같은 예외 신호로 나뉘며, 이종 제조사를 연결하는 ROP 는 변화 탐지 계산은 로봇·제조사에 맡기고 탐지된 차이를 구역 집합·차선 폐쇄·지도 판으로 반영하고 도면 변경 이력을 관리하는 쪽을 맡는 경계가 될 것으로 보인다. 이는 이 위키의 종합이다. [추정][^ref-745][^ref-221][^ref-031][^ref-569][^ref-270]
...
원천: [질문 백로그](../tracks/floorplan-recognition/question-backlog.md)([건축 도면 자동 인식](../tracks/floorplan-recognition/index.md) 트랙) · 열림 29건 · 답함 12건 · 폐기 3건
...
updated: 2026-09-25
sources: [ref-031, ref-062, ref-063, ref-064, ref-065, ref-066, ref-067, ref-068, ref-069, ref-070, ref-071, ref-072, ref-073, ref-074, ref-075, ref-076, ref-077, ref-078, ref-079, ref-081, ref-082, ref-083, ref-084, ref-085, ref-086, ref-109, ref-120, ref-220, ref-221, ref-222, ref-223, ref-224, ref-225, ref-226, ref-227, ref-046, ref-212, ref-213, ref-214, ref-215, ref-216, ref-217, ref-218, ref-219, ref-241, ref-105, ref-163, ref-265, ref-267, ref-268, ref-271, ref-274, ref-156, ref-157, ref-158, ref-331, ref-332, ref-333, ref-334, ref-335, ref-336, ref-338, ref-339, ref-340, ref-341, ref-342, ref-345, ref-419, ref-420, ref-421, ref-422, ref-423, ref-424, ref-425, ref-426, ref-427, ref-428, ref-429, ref-430, ref-432, ref-433, ref-434, ref-435, ref-436, ref-440, ref-441, ref-442, ref-456, ref-457, ref-458, ref-459, ref-462, ref-463, ref-536, ref-413, ref-641, ref-642, ref-228, ref-572, ref-573, ref-574, ref-229, ref-038, ref-575, ref-576, ref-315, ref-314, ref-283, ref-348, ref-461, ref-406,
  ref-629, ref-632, ref-153, ref-644, ref-645, ref-648, ref-270]
last_run: 2026-09-25
version: 13
---
...
### runs/2026-09-25-75/pages/tracks/floorplan-recognition/space-graph-schema-draft.md
...
<!-- auto:page-status:start -->
> 초안 버전: v1.0 · 페이지 상태: published · 신뢰도: low · 페이지 버전: 11 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-063, ref-064, ref-065, ref-068, ref-069, ref-070, ref-071, ref-072, ref-078, ref-079, ref-080, ref-081, ref-082, ref-084, ref-085, ref-086, ref-223, ref-224, ref-046, ref-212, ref-213, ref-214, ref-215, ref-216, ref-156, ref-331, ref-332, ref-333, ref-334, ref-336, ref-338, ref-339, ref-343, ref-348, ref-419, ref-420, ref-421, ref-422, ref-426, ref-427, ref-428, ref-432, ref-435, ref-073, ref-346, ref-440, ref-441, ref-442, ref-536, ref-640, ref-414, ref-413, ref-642, ref-573, ref-574, ref-461, ref-229, ref-228, ref-283, ref-575, ref-315, ref-406, ref-629, ref-153, ref-644, ref-645, ref-648]
last_run: 2026-09-25
version: 11
---
...
### runs/2026-09-25-75/pages/tracks/floorplan-recognition/index.md
...
### runs/2026-09-25-75/docs_tree.txt
...
### docs/references/index.md (요약: 이번 대상 페이지가 인용한 154건 / 전체 662건. 목록에 없는 출처는 새 id 로 적는다 — 같은 URL 이 이미 있으면 퍼블리셔가 기존 id 로 합친다)
...
### docs/glossary/index.md (요약: 용어 171개, slug: 한국어 (영어). 정의는 docs/glossary/<slug>.md)
