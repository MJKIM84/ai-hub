...
- 대상: 트랙 floorplan-recognition (건축 도면 자동 인식) · 현재 단계: 단계 2. 필요한 데이터와 표준 조사 · 이번에 다룰 백로그 질문 id: q2-02 · 중심 세부영역: 6. 지도·공간·위치 모델 (B. 공통 정보·환경 모델)
- 예산:
...
### runs/2026-09-25-36/target.json
...
```json
{
  ...
  "selection_rationale": "CLI 지정 run_type=track, area=6; 트랙 실행일(Fri, track_days 앞 3개) → 트랙 floorplan-recognition 단계 2, 질문 q2-02 (CLI 지정 질문 id)"
}
```

### runs/2026-09-25-36/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q2-02 도면 입력 형식(벡터 CAD, BIM 모델, 래스터 스캔)마다 벽·문·엘리베이터·계단·충전 위치 정보가 어떻게 들어 있고 무엇이 빠지는가?",
    "BIM 모델(IFC 4.3)은 벽·문·계단·엘리베이터·층을 어떤 엔터티·속성·관계로 담고, 실무 IFC 모델에서는 무엇이 누락되거나 잘못 분류되는가? (단계 2 페이지 3절, 공간 그래프 스키마 초안 2절 겨냥)",
    "벡터 CAD(DXF·DWG)는 레이어·블록·텍스트·단위로 무엇을 담고, 레이어 명명 표준(ISO 13567, 미국 NCS, 국내 KS F 1542·건설CALS)은 의미 정보를 얼마나 보장하는가? (아이디어 페이지 4절 겨냥)",
    "래스터 스캔 평면도에서는 축척·요소 의미를 어디서 얻고, 공개 데이터셋은 엘리베이터·계단·충전 위치를 라벨로 담는가? (단계 4 q4-05 선행 근거)",
    "국내 CAD 레이어 표준과 공공 BIM 지침은 도면 입력의 정보 수준에 어떤 전제를 주는가? (한국 자료 우선 규칙)"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "IFC 4.3 문서는 IfcDoor 를 사람·물품·차량의 통제된 출입에 주로 쓰는 건축 요소로 정의하고, 전체 높이·폭(OverallHeight·OverallWidth)과 여닫는 방식(OperationType: 단일·양방향 스윙, 회전문 등) 속성을 두며, 문은 IfcRelFillsElement 로 벽의 개구부(IfcOpeningElement)를 채운다.",
      "tag": "사실",
      "source_ids": [
        "ref-738"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "개발 브랜치 원본(IfcDoor.md): 'predominately used to provide controlled access for people, goods, animals and vehicles'. OverallHeight·OverallWidth 가 없으면 관련 개구부에서 값을 얻는다. 게시판 IFC 4.3 ADD2 와 문구가 다를 수 있음. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f2",
      "claim": "IFC 4.3 문서는 IfcWall 을 건축물을 둘러싸거나 나누는 수직 구조로 정의하고, 문·창문 같은 개구부를 IfcRelVoidsElement 로 벽에 붙는 IfcOpeningElement 로 표현하며, 벽은 층·건물·대지에 공간적으로 포함된다.",
      "tag": "사실",
      "source_ids": [
        "ref-741"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "개발 브랜치 원본(IfcWall.md): 'vertical construction ... which bounds or subdivides a construction works'. 개구부는 IfcRelVoidsElement, 이미 형상에 포함된 문·창문은 IfcRelConnectsElements 로 연결. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f3",
      "claim": "IFC 4.3 문서는 IfcStair 를 다른 층으로 걸어서 오갈 수 있게 하는 수평 단(계단·참)의 연속으로 정의하고, 계단참(IfcStairFlight)·참 슬래브(IfcSlab LANDING)·난간(IfcRailing)으로 분해할 수 있으며, 기본 공간 컨테이너는 IfcBuildingStorey 이고 여러 층에 걸치면 IfcBuilding 에 포함한다.",
      "tag": "사실",
      "source_ids": [
        "ref-739"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "개발 브랜치 원본(IfcStair.md): 'a succession of horizontal stages (steps or landings) that make it possible to pass on foot to other levels'. 계단이 잇는 두 층을 가리키는 전용 속성은 이번 열람 범위에서 확인하지 못함. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f4",
      "claim": "IFC 4.3 의 운송 요소 유형 열거(IfcTransportElementTypeEnum)는 ELEVATOR(사람·물품을 수직으로 옮기는 승강기), ESCALATOR, MOVINGWALKWAY, CRANEWAY, HAULINGGEAR, LIFTINGGEAR 와 USERDEFINED·NOTDEFINED 값을 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-740"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "개발 브랜치 원본(IfcTransportElementTypeEnum.md): ELEVATOR 'Elevator or lift being a transport device to move people or goods vertically.' 이전 실행(2026-09-25-19)의 IfcTransportElement 정의와 같은 발행 주체. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f5",
      "claim": "IFC 4.3 문서는 IfcBuildingStorey 를 연속한 두 바닥(또는 바닥과 지붕) 사이 공간의 수평 집합으로 정의하며, 층 기준 고도 속성 Elevation 은 IFC4.3.0.0 에서 폐기 예정(deprecated)으로 두고 속성 세트의 ElevationOfSSLRelative 또는 ElevationOfFFLRelative 사용을 권한다.",
      "tag": "사실",
      "source_ids": [
        "ref-742"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "개발 브랜치 원본(IfcBuildingStorey.md): Elevation 은 건물 내부 기준 높이(0.00) 대비 층 바닥 고도이며 4.3 에서 deprecated, 'ElevationOfSSLRelative or ElevationOfFFLRelative instead'. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f6",
      "claim": "IFC 4.3 입력에는 벽·문·계단·엘리베이터·층이 유형이 붙은 객체와 층 포함 관계로 들어 있지만, 문이 어느 두 공간을 잇는지는 문→개구부→벽의 채움·보이드 관계와 공간 경계(IfcRelSpaceBoundary)를 거쳐 도출해야 하고 계단·엘리베이터가 잇는 층도 직접 속성이 아니라 포함 관계·형상에서 도출해야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-738",
        "ref-741",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f1(IfcRelFillsElement)·f2(IfcRelVoidsElement)·f3(층 컨테이너)과 이전 실행 2026-09-25-28 의 IfcRelSpaceBoundary·IfcSpace 열람 결과(공간–공간 직접 연결 관계 없음)를 결합한 추론. IFC 전체 관계 엔터티를 대조하지 않아 부재 확정 아님. (재인용: 2026-09-25-28)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f7",
      "claim": "실무에서 만든 IFC 모델을 표준 정의와 대조한 연구(Applied Sciences, 2021)는 IFC 가 예정하지 않은 요소를 담는 IfcBuildingElementProxy 가 유효한 IFC 엔터티가 있는 요소의 대체로 잘못 쓰일 수 있다고 보고 모델마다 그 사용을 점검·채점했다.",
      "tag": "사실",
      "source_ids": [
        "ref-751"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: IfcBuildingElementProxy 'is useful to include elements not foreseen by the IFC model, but can be wrongly used to substitute entities having a valid representation in IFC'. 실무자가 다른 목적으로 만든 모델 표본을 점검. 대체 비율 수치는 확인 못함. 원문 미열람.",
      "as_of": "2021",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f8",
      "claim": "BIM 입력에서 엘리베이터·문·계단이 IfcTransportElement(ELEVATOR)·IfcDoor·IfcStair 로 담긴다는 것은 스키마의 가능성이고, 실무 모델에서는 이들이 IfcBuildingElementProxy 로 내보내져 유형 정보가 빠질 수 있으므로 인식 전에 클래스 사용을 점검해야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-751",
        "ref-740",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f4·f1(스키마 정의)과 f7(실무 모델의 프록시 오용 점검)을 결합한 추론. 엘리베이터가 실제로 프록시로 내보내진 사례·비율은 이번에 확인하지 못함.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": false
    ...
    {
      "id": "f9",
      "claim": "IFC 4.3 개발 브랜치의 콘센트 유형 열거와 전기기기 유형 열거에는 차량·로봇 충전 설비를 뜻하는 값이 없고 USERDEFINED·NOTDEFINED 만 남아, BIM 입력의 충전 위치는 표준 유형 값으로 구분되지 않는다.",
      "tag": "사실",
      "source_ids": [
        "ref-214",
        "ref-215"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "IfcOutletTypeEnum: 음향영상·통신·전원·데이터·전화 콘센트, IfcElectricApplianceTypeEnum: 가전·사무기기 16종. 두 파일은 같은 발행 주체라 독립 교차 아님. 이번 실행에서 다시 열지 않음. (재인용: 2026-09-25-19)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f10",
      "claim": "DXF 형식에서 블록(block)은 여러 번 배치할 수 있는 엔터티 묶음이고 각 배치는 블록 참조(INSERT 엔터티)로 위치·회전·축척을 가지며, 블록 참조에는 태그가 붙은 속성 텍스트(ATTRIB)를 달아 메타데이터를 실을 수 있다.",
      "tag": "사실",
      "source_ids": [
        "ref-743"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "ezdxf 문서 원본(blocks.rst): 'collections of DXF entities which can be placed multiple times as block references'. ATTRIB 로 CAD 응용이 블록 참조에 메타데이터를 붙이고 내보낼 수 있음. Autodesk 공식 참조가 아닌 오픈소스 라이브러리 문서. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f11",
      "claim": "DXF 형식에서 레이어는 객체를 논리적 묶음으로 나누고 보이기·선택·색상·선 종류를 제어하는 수단이며, 벽·가구·주석 같은 레이어 구분은 사용 관례로 예시될 뿐 형식 자체가 정한 의미는 아니다.",
      "tag": "사실",
      "source_ids": [
        "ref-744"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "ezdxf 문서 원본(layers.rst): 'You use layers to organize objects into logical groups of things that belong together'. 레이어 이름의 표준 의미는 문서에 정의되어 있지 않음(열람 범위 기준). (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f12",
      "claim": "DXF 의 길이·좌표 값은 그 자체로 단위가 없고, 모델 공간의 단위는 선택 헤더 변수 $INSUNITS 로 주어지며 $MEASUREMENT 는 미터법·야드파운드법 선 종류·해치 패턴 선택에만 관계한다.",
      "tag": "사실",
      "source_ids": [
        "ref-745"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "ezdxf 문서 원본(units.rst): 'Any length or coordinate value in DXF is unitless in the first place, there is no unit information attached to the value.' 단위 지정은 선택이지만 단위가 다른 블록을 합칠 때 필요. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f13",
      "claim": "ISO 13567-1:2017 은 CAD 파일의 레이어 구조화 원칙을 정하고, 레이어 이름을 책임 주체(설계 분야)·요소(분류 체계 코드)·표현 등 고정 길이 필드로 이루어진 문자열로 구성하게 한다.",
      "tag": "사실",
      "source_ids": [
        "ref-746"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 레이어는 'used to control visibility and to manage and communicate CAD file data'; 필드는 Agent Responsible(2자), Element(6자, CI/SfB·Uniclass 등), Presentation 등. Part 2(2017)는 건설 문서용 개념·코드. 유료 표준, 원문 미열람.",
      "as_of": "2017",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f14",
      "claim": "미국 국가 CAD 표준(NCS)이 채택한 AIA CAD 레이어 형식은 하이픈으로 나눈 필드(분야 지정자·주 그룹 등)로 레이어 이름을 짓고, 건축 분야의 문 레이어를 A-DOOR, 벽 레이어를 A-WALL 같은 이름으로 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-747"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 'four separate fields separated by hyphens', 분야 지정자는 2자(첫 글자 분야, 둘째 선택 수식자), A-WALL-FULL·A-DOOR 예. 계단·승강기 레이어 코드는 확인 못함. NCS 5 문서, 원문 미열람. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f15",
      "claim": "국내에서는 국토교통부 건설CALS 체계의 '건설CALS 전자도면 작성표준'이 전자도면의 도면분류·파일명·선·색상·레이어·심벌을 정하며(V1.0 2004-08 공고, V1.1 2006-12 개정), 레이어 부문은 KS F 1542 'CAD 도면 작성을 위한 레이어 원칙과 기준'(2020 확인)으로 국가표준화되어 있다.",
      "tag": "사실",
      "source_ids": [
        "ref-749",
        "ref-748"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 전자도면 작성표준은 '도면분류, 파일명, 선, 색상, 레이어, 심벌 등을 정한 표준'. KSSN: KS F 1542(2020 확인), 관련 KS F 1540·1541. 문·계단·승강기 레이어 코드 내용은 원문 미열람으로 미확인. 두 출처는 서로 다른 문서를 말해 교차 확인 아님.",
      "as_of": "2020-12",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f16",
      "claim": "신동철(대한건축학회 논문집 계획계 25(11), 2009)은 국내 건축 표준 CAD 레이어의 실무 적용 실태를 분석했다.",
      "tag": "사실",
      "source_ids": [
        "ref-750"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "검색 요약: 제목 '건축 표준 캐드 레이어의 실무적용 실태 분석 연구', 2009-11 게재. 같은 저자의 정착 방안 연구(2010)도 있음. 분석 결과(표준 레이어 사용 비율 등)는 초록·본문 미열람으로 확인하지 못함.",
      "as_of": "2009-11",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f17",
      "claim": "ArchCAD-400K 프로젝트 페이지는 체계적으로 보관된 CAD 도면의 고유 속성인 레이어·블록 계층을 이용해 구조 인식형 자동 라벨링을 하고, 전문가 보정을 래스터가 아닌 벡터 공간에서 직접 한다고 설명한다.",
      "tag": "사실",
      "source_ids": [
        "ref-753"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "프로젝트 페이지 원본(index.html): 'leveraging inherent layer and block hierarchy', 'intrinsic attributes from systematically archived CAD drawings'. 27개 범주 목록은 페이지에 없음. (발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f18",
      "claim": "FloorPlanCAD 는 실제 CAD 도면을 SVG 벡터로 담고 선 요소마다 범주를 주석해, CAD 선 요소에서 문·창문 같은 기호의 인스턴스와 벽 같은 영역의 의미를 함께 판별하는 파놉틱 심볼 스포팅 과제를 정의했다.",
      "tag": "사실",
      "source_ids": [
        "ref-067",
        "ref-066"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "15,663장(초판 11,602장), 35개 범주, 선 단위 주석, 주석 라이선스 CC BY-NC 4.0. 두 출처 같은 저자라 독립 교차 아님. 이번 실행에서 다시 열지 않음. (재인용: 2026-09-25-05)",
      "as_of": "2021-05",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f19",
      "claim": "2026년 프리프린트(arXiv 2607.12678)는 CAD 평면도의 텍스트 주석이 유형과 등급·치수 같은 속성을 압축 코드(예: 'FM B 1321')로 담는 다층 의미 구조를 가져, 이를 선 요소 단위 심볼 스포팅에 결합하는 것이 과제라고 보고 텍스트를 함께 쓰는 다중 모달 방법을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-752"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: CAD annotations 'are neither generic natural language nor flat auxiliary labels but exhibit complex syntactic structures and multi-level semantics'; 'FM' 은 유형, 'B 1321' 은 등급·치수 속성. 성능 수치는 확인 못함. 원문 미열람.",
      "as_of": "2026-07",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f20",
      "claim": "osmAG-from-cad 공식 저장소는 DXF 를 기본 입력으로 받아 SVG·PNG 로 바꾼 뒤 AreaGraph 로 분할하며, DWG 는 외부 변환기(ODA File Converter)를 거쳐야 하고 문자 기반 방 이름 붙이기는 기본으로 꺼져 있다.",
      "tag": "사실",
      "source_ids": [
        "ref-084"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "README: DWG 변환은 'not part of the default reproducibility path', 출력은 방 기하·통로 위상·선택적 방 이름의 OSM XML. 이번 실행에서 다시 열지 않음. (재인용: 2026-09-25-11)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f21",
      "claim": "벡터 CAD 입력에서 벽·문·계단·엘리베이터는 선·폴리라인·블록 참조·텍스트로 들어 있고 그 의미는 형식이 아니라 프로젝트의 레이어·블록 이름 관례(ISO 13567, NCS, KS F 1542 같은 명명 표준을 따를 수도 있음)와 텍스트 주석에서 읽어야 하며, 길이 단위도 선택 헤더에 기대므로, 표준을 따르지 않은 도면에서는 요소 의미와 축척을 인식으로 복원해야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-743",
        "ref-744",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f10~f15·f17·f19 를 결합한 이 위키의 정리. 국내 실무 도면이 KS F 1542·건설CALS 레이어를 얼마나 따르는지는 f16 의 결과를 확인하지 못해 미확인.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": false
    ...
    {
      "id": "f22",
      "claim": "래스터 평면도 데이터셋 가운데 MLSTRUCT-FP 는 이미지별 축척(px/m) 메타데이터를 주지만 Raster-to-Graph 는 이미지를 512×512 로 정규화해 이미지 좌표만 다루는 등, 래스터 입력에는 축척이 이미지 자체에 들어 있지 않고 별도 메타데이터로만 주어진다.",
      "tag": "사실",
      "source_ids": [
        "ref-069",
        "ref-070"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "MLSTRUCT README: 954장, 'scale factors in px/m'. Raster-to-Graph README: 512×512 이미지. 두 출처는 서로 다른 데이터셋의 사례이며 교차 확인 아님. 이번 실행에서 다시 열지 않음. (재인용: 2026-09-25-05)",
      "as_of": "2024",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f23",
      "claim": "래스터 기반 주택 평면 인식·3D 재구성 연구(Buildings 15(7), 2025)는 YOLOv8 과 Shi–Tomasi 모서리 검출로 치수선 끝점을 찾고 사전학습 다중 모달 OCR(OFA-OCR)로 치수 숫자를 읽어 축척을 계산했으며, 축척 계산 정확도가 95%를 넘었다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-754"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 'Scale recognition combines YOLOv8 with Shi–Tomasi corner detection ... OFA-OCR ... scale calculation accuracy exceeding 95%'. 데이터셋·도면 조건은 미확인, 저자 보고 단일 출처. 원문 미열람.",
      "as_of": "2025",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f24",
      "claim": "공개 래스터 평면도 자료의 라벨은 벽·문·창문·방(CubiCasa5K 는 계단·난간 포함, Kratochvila 외는 계단·난간 포함)과 AI Hub 건축 도면 데이터의 벽체·창호·출입문·도면 문자 중심이며, 엘리베이터 라벨은 이들 자료에서 확인되지 않았다.",
      "tag": "사실",
      "source_ids": [
        "ref-063",
        "ref-078",
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "CubiCasa5K 구조 요소에 staircase, Kratochvila 외 대상 walls·windows·stairs·railings, AI Hub 는 출입문·창호·벽체 세그멘테이션과 YOLOv5+CRNN OCR. 전체 클래스 목록 원문 미열람이라 엘리베이터 부재는 확정 아님(q2-04). (재인용: 2026-09-25-05)",
      "as_of": "2024-08",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f25",
      "claim": "래스터 스캔 입력에는 레이어·객체·단위가 없어 벽·문·계단은 기호 모양에서, 축척은 치수 문자·축척 표기에서, 방·층 이름은 도면 문자 OCR 에서 복원해야 하고, 엘리베이터 라벨이 확인된 공개 래스터 데이터셋이 없어 엘리베이터 인식은 학습 자료부터 부족할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-063",
        "ref-069",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f22·f23·f24 를 결합한 이 위키의 정리. 스캔 품질(잡음·기울기)의 영향은 이번에 조사하지 않음.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f26",
      "claim": "세 입력 형식 모두에서 로봇 충전 위치는 표준 표현으로 확인되지 않았다 — IFC 4.3 유형 값에 충전 설비가 없고, 확인한 CAD 레이어 표준 자료에서 충전 위치 레이어 코드를 찾지 못했으며, 공개 평면도 데이터셋에 충전 위치 라벨이 없어, 충전 위치는 도면 밖 정보로 보완해야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-214",
        "ref-215",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f9·f13·f14·f24 와 이전 실행 2026-09-25-05 f18(물류·충전 라벨 데이터셋 부재), 2026-09-25-19(q1-03, 도면 밖 보완 사례)를 결합. 레이어 표준 원문은 미열람이라 충전 위치 코드 부재는 확정 아님.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f27",
      "claim": "Vega-Torres 외는 BIM(IFC)에서 자동 생성한 2D 점유 격자 지도가 구조 요소만 담으며, 가구·잡동사니와 설계–시공 편차 때문에 BIM 이 현실을 정확히 나타낸다는 가정이 성립하지 않는다고 지적했다.",
      "tag": "사실",
      "source_ids": [
        "ref-081"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 'only represent structural elements'; 'most of these studies assume that the BIM model precisely represents the real world, which is rarely true'. 원문 미열람. (재인용: 2026-09-25-11)",
      "as_of": "2023-08",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f28",
      "claim": "분류 원문 질문(‘3층 출하 대기장’을 같은 장소로 인식)에 대해, BIM 입력은 공간 이름과 층 소속을 객체 속성·관계로 주지만 벡터 CAD·래스터 입력에서는 장소 이름이 텍스트 주석으로만 있어 OCR·텍스트 해석으로 공간에 붙여야 하고, 어느 형식이든 제조사 로봇 지도 식별자와의 대응은 도면 밖의 ROP 쪽 대응 계층이 맡아야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-156",
        "ref-742",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f5(층), IfcSpace 정의(재인용: 2026-09-25-28), f19(CAD 텍스트 주석), AI Hub OCR 모델을 SCM 질문에 대응시킨 추론. 물류센터 도면에 구역 이름이 어떻게 적히는지는 확인하지 못함.",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "완료·인계",
      "source_unopened": false
    ...
    {
      "id": "f29",
      "claim": "국토교통부는 2022-07 BIM 성과품의 작성·납품과 활용의 방법·절차를 제시하는 '건설산업 BIM 시행지침'을 발표했다.",
      "tag": "사실",
      "source_ids": [
        "ref-755"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "검색 요약: 시행지침은 'BIM 적용 시 성과품의 작성/납품 및 활용에 대한 방법과 절차 등 세부 기준을 제시'. 공공공사 BIM 의무화 확대 일정은 벤더 요약에만 있어 finding 으로 내지 않음. 원문 미열람.",
      "as_of": "2022-07",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f30",
      "claim": "q2-02 에 대해 종합하면, BIM(IFC) 입력은 벽·문(폭·여닫는 방식)·계단·엘리베이터·층을 유형 객체로 담아 가장 정보가 많지만 연결은 도출해야 하고 실무 모델은 프록시 오분류가 있을 수 있으며, 벡터 CAD 는 기하는 정확하나 의미·단위가 레이어·블록·텍스트 관례에 기대고, 래스터 스캔은 의미·축척을 모두 인식으로 복원해야 하며, 충전 위치는 세 형식 모두 표준 표현이 확인되지 않은 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-738",
        "ref-739",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "f1~f26 을 입력 형식별로 묶은 이 위키의 정리. 이 3분 비교를 제시한 단일 출처는 확인하지 못함.",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": false
  ...
  "sources": [
    {
      ...
      "title": "IFC 4.3 — IfcDoor (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음)",
      "published": null,
      "url": "https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcDoor.md",
      "type": "표준",
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC 4.3 문 엔터티 정의 원본. 전체 높이·폭, 여닫는 방식 속성과 개구부를 채우는 관계(IfcRelFillsElement)를 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/buildingSMART/IFC4.3.x-development/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcDoor.md",
      "source_unopened": false
    },
    {
      "id": "ref-739",
      "org": "buildingSMART (IFC4.3.x-development GitHub)",
      "title": "IFC 4.3 — IfcStair (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음)",
      "published": null,
      "url": "https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcStair.md",
      "type": "표준",
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC 4.3 계단 엔터티 정의 원본. 계단참·참 슬래브·난간으로의 분해와 층·건물 공간 컨테이너를 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/buildingSMART/IFC4.3.x-development/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcStair.md",
      "source_unopened": false
    },
    {
      "id": "ref-740",
      "org": "buildingSMART (IFC4.3.x-development GitHub)",
      "title": "IFC 4.3 — IfcTransportElementTypeEnum (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음)",
      "published": null,
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC 4.3 운송 요소 유형 열거 원본. ELEVATOR·ESCALATOR·MOVINGWALKWAY·CRANEWAY·HAULINGGEAR·LIFTINGGEAR 값을 둔다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      "id": "ref-741",
      "org": "buildingSMART (IFC4.3.x-development GitHub)",
      "title": "IFC 4.3 — IfcWall (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음)",
      "published": null,
      "url": "https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcWall.md",
      "type": "표준",
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC 4.3 벽 엔터티 정의 원본. 개구부(IfcRelVoidsElement)와 공간 포함 관계를 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/buildingSMART/IFC4.3.x-development/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcWall.md",
      "source_unopened": false
    },
    {
      "id": "ref-742",
      "org": "buildingSMART (IFC4.3.x-development GitHub)",
      "title": "IFC 4.3 — IfcBuildingStorey (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음)",
      "published": null,
      "url": "https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/core/IfcProductExtension/Entities/IfcBuildingStorey.md",
      "type": "표준",
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC 4.3 건물 층 엔터티 정의 원본. Elevation 속성 폐기 예정과 속성 세트 기반 층 고도 사용 권고를 적는다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/buildingSMART/IFC4.3.x-development/ifc4.3-main/docs/schemas/core/IfcProductExtension/Entities/IfcBuildingStorey.md",
      "source_unopened": false
    },
    {
      "id": "ref-743",
      "org": "Moitzi, M. (mozman/ezdxf GitHub)",
      "title": "ezdxf documentation — Concepts: Blocks (docs/source/concepts/blocks.rst)",
      "published": null,
      "url": "https://github.com/mozman/ezdxf/blob/master/docs/source/concepts/blocks.rst",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "DXF 읽기·쓰기 오픈소스 라이브러리 문서. 블록·블록 참조(INSERT)·속성 텍스트(ATTRIB)의 개념을 설명한다. Autodesk 공식 참조가 아니다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/mozman/ezdxf/master/docs/source/concepts/blocks.rst",
      "source_unopened": false
    },
    {
      "id": "ref-744",
      "org": "Moitzi, M. (mozman/ezdxf GitHub)",
      "title": "ezdxf documentation — Concepts: Layers (docs/source/concepts/layers.rst)",
      "published": null,
      "url": "https://github.com/mozman/ezdxf/blob/master/docs/source/concepts/layers.rst",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "DXF 레이어가 객체를 논리적 묶음으로 나누고 보이기·색상·선 종류를 제어한다는 개념 설명. Autodesk 공식 참조가 아니다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/mozman/ezdxf/master/docs/source/concepts/layers.rst",
      "source_unopened": false
    },
    {
      "id": "ref-745",
      "org": "Moitzi, M. (mozman/ezdxf GitHub)",
      "title": "ezdxf documentation — Concepts: DXF Units (docs/source/concepts/units.rst)",
      "published": null,
      "url": "https://github.com/mozman/ezdxf/blob/master/docs/source/concepts/units.rst",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "DXF 좌표·길이 값에 단위가 없고 모델 공간 단위는 $INSUNITS 헤더 변수로 주어진다는 설명. Autodesk 공식 참조가 아니다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      "id": "ref-746",
      "org": "ISO",
      "title": "ISO 13567-1:2017 - Technical product documentation — Organization and naming of layers for CAD — Part 1: Overview and principles",
      "published": "2017",
      "url": "https://www.iso.org/standard/70181.html",
      "type": "표준",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. CAD 레이어의 구조화 원칙과 고정 길이 필드로 된 레이어 이름 체계를 정한 국제표준(유료).",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    ...
    {
      "id": "ref-747",
      "org": "National Institute of Building Sciences (United States National CAD Standard)",
      "title": "AIA CAD Layer Guidelines, Layer Name Format (NCS V5)",
      "published": null,
      "url": "https://www.nationalcadstandard.org/ncs5/pdfs/ncs5_clg_lnf.pdf",
      "type": "표준",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 미국 국가 CAD 표준이 채택한 AIA 레이어 이름 형식(하이픈으로 나눈 분야·주 그룹 등 필드)을 설명한 문서.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "title": "KS F 1542(2020 확인) CAD 도면 작성을 위한 레이어 원칙과 기준",
      "published": "2020-12",
      "url": "https://www.kssn.net/search/stddetail.do?itemNo=K001010129900",
      "type": "표준",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. CAD 도면의 레이어 원칙과 기준을 정한 국가표준(KS F 1540·1541 과 한 묶음, 유료).",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    ...
    {
      "id": "ref-749",
      "org": "국토교통부 건설사업정보시스템(CALS)",
      "title": "건설CALS 전자도면 작성표준",
      "published": null,
      "url": "https://www.calspia.go.kr/portal/intro/introStandard02.do",
      "type": "정부·연구기관",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 건설사업 전자도면의 도면분류·파일명·선·색상·레이어·심벌을 정한 표준의 소개 페이지(V1.0 2004, V1.1 2006).",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "title": "건축 표준 캐드 레이어의 실무적용 실태 분석 연구",
      "published": "2009-11",
      "url": "https://www.dbpia.co.kr/Journal/articleDetail?nodeId=NODE01288876",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 국내 건축 표준 CAD 레이어의 실무 적용 실태를 분석한 논문(25권 11호).",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "title": "An Inspection of IFC Models from Practice",
      "published": "2021",
      "url": "https://www.mdpi.com/2076-3417/11/5/2232",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 실무자가 만든 IFC 모델 표본을 표준 정의와 대조해 엔터티 사용 일관성과 IfcBuildingElementProxy 오용을 점검한 논문(11권 5호).",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. CAD 평면도 텍스트 주석의 압축 코드 의미를 선 요소 단위 심볼 스포팅에 결합하는 다중 모달 방법을 제안한 프리프린트.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "title": "ArchCAD-400k: A Large-Scale CAD drawings Dataset and New Baseline for Panoptic Symbol Spotting — project page",
      "published": null,
      "url": "https://archiai-lab.github.io/ArchCAD.github.io/",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "ArchCAD-400K 프로젝트 페이지 원본. CAD 레이어·블록 계층을 이용한 자동 라벨링과 벡터 공간 전문가 보정을 설명한다. 범주 목록은 없다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    {
      "id": "ref-754",
      "org": "Buildings(MDPI) 게재 논문 저자(미확인)",
      "title": "Raster Image-Based House-Type Recognition and Three-Dimensional Reconstruction Technology",
      "published": "2025",
      "url": "https://doi.org/10.3390/buildings15071178",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 래스터 주택 평면도에서 요소 인식·축척 인식(YOLOv8·OCR)·3D 재구성을 하는 방법을 제시한 논문(15권 7호).",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      "id": "ref-755",
      "org": "국토교통부",
      "title": "건설산업 BIM 시행지침 정책정보 상세보기",
      "published": "2022-07",
      "url": "https://www.molit.go.kr/USR/policyData/m_34681/dtl.jsp?srch_usr_titl=Y&psize=10&lcmspage=1&id=4634",
      "type": "정부·연구기관",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. BIM 성과품의 작성·납품·활용 방법과 절차를 제시한 국토교통부 시행지침의 게시 페이지.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않았다(2026-09-25-28 실행에서 원문 확인). IFC 4.3 공간 정의와 층 집합 관계.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않았다(2026-09-25-28 실행에서 원문 확인). 공간과 경계 요소를 잇는 관계 정의.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않았다. 콘센트 유형 열거에 로봇 충전 설비 값이 없음을 보인 근거.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않았다. 전기기기 유형 열거에 로봇 충전 설비 값이 없음을 보인 근거.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 핀란드 부동산 평면도 5,000장의 SVG 주석 데이터셋과 다중 작업 인식 모델.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않았다. FloorPlanCAD 도면 수, 주석 라이선스, 프로젝트 종료를 적은 공식 페이지.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. CAD 평면도를 선 단위로 주석하고 파놉틱 심볼 스포팅 과제를 제시한 ICCV 2021 논문.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않았다. 954장 평면도의 벽·슬래브·축척(px/m) JSON 데이터셋 README.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않았다. 512×512 주거 평면도의 구조 그래프 인식 방법 README.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 공공·상업 시설 중심 건축 CAD 도면 27개 범주 주석 데이터셋 논문.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 주택 유형별 도면의 객체·문자 주석 데이터와 객체 인식·세그멘테이션·OCR 학습 모델.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 다세대 래스터 평면도의 벽·창문·계단·난간 분할과 벡터화·3D 재구성.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. BIM 에서 구조 요소만 담은 점유 격자 지도를 자동 생성하고 설계–시공 편차에 견고한 위치추정을 다룬 논문.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 이번 실행에서 다시 열지 않았다(2026-09-25-11 실행에서 원문 확인). DXF→osmAG 파이프라인 README.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
  ...
  "page_proposals": [
    {
      ...
      "rationale": "q2-02 답: f1·f2·f3·f4·f5·f6·f7·f8·f9·f10·f11·f12·f13·f14·f15·f16·f17·f18·f19·f20·f21·f22·f23·f24·f25·f26·f27·f28·f29·f30 (신뢰도 medium) — 2절 q2-02 상태 답함, 3절 q2-02 소제목 신설({#q2-02}): BIM(IFC 4.3) 입력 f1·f2·f3·f4·f5·f6과 실무 모델 품질 f7·f8, 충전 위치 f9, BIM 기반 지도의 한계 f27, 국내 BIM 시행지침 f29 / 벡터 CAD 입력 f10·f11·f12, 레이어 명명 표준 f13·f14·f15·f16, CAD 인식 연구 f17·f18·f19·f20, 종합 f21 / 래스터 스캔 입력 f22·f23·f24·f25 / 충전 위치 종합 f26 / 분류 원문 질문 f28 / 입력 형식별 종합 f30 / 4절 결론·불확실성(레이어 표준 실무 준수율 미확인, 프록시 오분류 비율 미확인) / 5절 후속 질문 / 6절 완료 조건 현황 / 8절 출처 / 9절 이력"
    },
    {
      "action": "update",
      ...
      "rationale": "아이디어 페이지 4절(트랙 산출물): '입력 형식별 정보 항목' 소절 신설 — BIM(IFC 4.3) f1·f3·f4·f5·f6·f8, 벡터 CAD f10·f11·f12·f13·f14·f15·f21, 래스터 스캔 f22·f23·f24·f25, 충전 위치 부재 f9·f26, 종합 비교 f30(추정). 관제 수용 형식(q2-03)은 아직 없음을 명시"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신: track.ontology_changes 가 검증 승인되면 문(BIM 대응 클래스 IfcDoor, 폭·여닫는 방식의 IFC 원천 속성, f1), 계단(BIM 대응 클래스 IfcStair, f3), 엘리베이터(유형 값 ELEVATOR, f4), 평면도(길이 단위·축척 정보 속성, f12·f22) 반영. 미승인 제안과 f6(연결 도출)·f8(프록시 오분류)·f26(충전 위치 표준 표현 부재)은 6절 질문으로"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 2 반영 제안 (f1, f4, f10, f12, f15, f22, f27, f30): 7절(주제 페이지 area06-s7)에 도면 입력 형식(BIM·벡터 CAD·래스터)별로 담기는 공간 정보와 빠지는 정보(단위·축척·충전 위치), 국내 CAD 레이어 표준(KS F 1542·건설CALS) 추가"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 2 반영 제안 (f7, f13, f14, f15, f29): CAD 레이어 명명 표준(ISO 13567, 미국 NCS, KS F 1542·건설CALS 전자도면 작성표준), 실무 IFC 모델의 프록시 오용 점검, 국토교통부 건설산업 BIM 시행지침"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 2 반영 제안 (f17, f19, f23): 교차 규칙(도면 해석은 6. 지도·공간·위치 모델에 적용)에 따라 CAD 레이어·블록 기반 자동 라벨링, 텍스트 결합 심볼 스포팅, 래스터 축척 OCR 인식 연구를 6. 지도·공간·위치 모델 페이지와 양쪽 연결"
    }
  ],
  "glossary_candidates": [
    {
      ...
      "definition": "AutoCAD 도면의 엔터티·레이어·블록을 태그 붙은 데이터로 기록하는 CAD 교환 파일 형식으로, 좌표 값에 단위가 붙지 않고 모델 공간 단위는 선택 헤더 변수($INSUNITS)로 준다."
    },
    {
      "term_ko": "블록 참조",
      "term_en": "Block Reference (INSERT)",
      "definition": "CAD 도면에서 여러 번 재사용하는 엔터티 묶음(블록)을 위치·회전·축척을 주어 한 번 배치한 것으로, 문·설비 기호가 흔히 이 형태로 들어가며 속성 텍스트(ATTRIB)를 달 수 있다."
    }
  ],
  "open_questions_new": [],
  ...
  "track": {
    ...
    "new_questions": [
      ...
      {
        "question": "벡터 CAD 도면의 레이어·블록 이름과 텍스트 주석(문 기호 코드, 방·구역 이름)을 선 요소 인식과 결합해 공간 노드·문·구역 이름을 만드는 처리 흐름에서 사람 검토를 어디에 두는가? (q2-02 에서 파생)",
        "stage": 3,
    ...
    "ontology_changes": [
      {
        ...
        "description": "속성에 'BIM 대응 클래스(IFC 4.3 IfcDoor, 개발 브랜치 기준, IfcRelFillsElement 로 벽 개구부를 채움)'를 더하고, 기존 속성 '폭'·'여닫는 방식'의 IFC 원천으로 OverallWidth·OperationType 을 메모한다. v0.4 에서 보류된 '표준 대응 클래스'(IndoorGML 2.0 문 표현 미확인, q2-07)와는 별개로 IFC 한 표준에 한정한 제안이다."
      },
      {
        "op": "modify",
        "kind": "concept",
        ...
        "description": "속성에 'BIM 대응 클래스(IFC 4.3 IfcStair, 계단참·참 슬래브·난간으로 분해 가능, 기본 컨테이너 IfcBuildingStorey)'를 더한다. 계단이 잇는 층은 IFC 에서 직접 속성이 아니라 포함 관계·형상에서 도출해야 할 것으로 보인다(f6, 추정)."
      },
      {
        "op": "modify",
        "kind": "concept",
        ...
        "description": "기존 속성 'BIM 대응 클래스(IfcTransportElement)'에 유형 열거 값 ELEVATOR(IfcTransportElementTypeEnum)를 확정한다. v0.3 에서 '유형 열거 값 이름은 단계 2에서 확정'으로 남긴 항목이다."
      },
      {
        "op": "modify",
        "kind": "concept",
        ...
        "description": "속성에 '길이 단위·축척 정보(DXF 는 선택 헤더 $INSUNITS, 래스터는 별도 메타데이터나 치수 문자에서 복원)'를 더한다. 축척 복원 방법은 q4-05 에서 다룬다."
      }
    ],
...
### runs/2026-09-25-36/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: IfcDoor.md 원본(github_raw 재열람)에 정의 문장, OverallHeight·OverallWidth·PredefinedType·OperationType 속성, 생략 시 IfcOpeningElement 형상에서 값을 얻는다는 규정, IfcRelFillsElement 로 개구부를 채우는 관계가 있음. 개발 브랜치 기준, 단일 출처."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인(문구 수정 필요): 원본 정의는 'vertical construction that may bound or subdivide spaces'로 '건축물'이 아니라 '공간'을 둘러싸거나 나눈다. evidence_excerpt 인용('...construction works')은 원문 문구와 다름. IfcRelVoidsElement·IfcRelConnectsElements·공간 포함(층 기본, 건물, 외부는 대지) 확인."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "부분 확인(문구 수정 필요): 원본 정의는 'a vertical passageway allowing occupants to walk (step) from one floor level to another floor level at a different elevation'이며 evidence_excerpt 의 인용('a succession of horizontal stages ...')은 이번 원본에 없음. 분해(IfcStairFlight, IfcSlab LANDING, IfcRailing) 확인. 컨테이너는 IfcBuildingStorey 기본, 층에 할당할 수 없을 때 IfcBuilding, 외부는 IfcSite 이며 '여러 층에 걸치면 IfcBuilding'은 원문 표현이 아님. 잇는 층 전용 속성 없음은 열람 결과와 일치."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: IfcTransportElementTypeEnum 원본 값 CRANEWAY·ELEVATOR·ESCALATOR·HAULINGGEAR·LIFTINGGEAR·MOVINGWALKWAY·NOTDEFINED·USERDEFINED, ELEVATOR 설명 일치."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: Elevation 이 4.3 에서 deprecated, Pset_BuildingStoreyCommon 의 ElevationOfSSLRelative·ElevationOfFFLRelative 사용 권고 일치. 정의는 원본 기준 '수직으로 경계 지어진 공간들의 (거의) 수평 집합'으로 옮기는 편이 정확하다."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: f1·f2·f3(원본 확인)과 이전 실행 2026-09-25-28 의 IfcSpace·IfcRelSpaceBoundary 열람 결과를 결합한 추론. ref-156·ref-334 는 이번 실행 원문 미열람. 부재 확정 아님을 본문에 유지."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 결과로 실재 확인(원문 미열람): Noardo, F., Arroyo Ohori, K., Krijnen, T., & Stoter, J., Applied Sciences 11(5) 2232, 2021. 프록시 오용 문구 스니펫 일치. '채점했다'는 스니펫에서 확인되지 않음(모델을 수작업으로 점검) — '점검했다'로 한정. 저자 미확인 표기 정정 필요."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: f1·f4 와 f7 결합 추론. 엘리베이터·문·계단이 실제 프록시로 내보내진 사례·비율은 미확인."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용 확인: 실행 2026-09-25-19 검증에서 원문 확인된 내용, 이번 실행 원문 미열람. 기존 각주 ref-214·ref-215 재사용. 두 파일은 같은 발행 주체라 독립 교차 아님."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ezdxf blocks.rst 원본에 블록 정의, 블록 참조의 회전·축척·배치, ATTRIB(태그가 붙은 텍스트 주석) 설명 일치. Autodesk 공식 DXF 참조가 아닌 오픈소스 라이브러리 문서이므로 본문은 'ezdxf 문서는 …라고 설명한다'로 귀속해 서술해야 함."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: layers.rst 원본에 논리적 묶음, 보이기·색상·선 종류 제어, 벽·가구·텍스트 주석 예시 일치. '형식 자체가 정한 의미가 아니다'는 열람 범위 기준 관찰. ezdxf 문서 귀속 필요."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: units.rst 원본 인용 문장 일치, $INSUNITS(모델 공간 단위)·$MEASUREMENT(해치·선 종류 패턴 선택) 설명 일치. ezdxf 문서 귀속 필요."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 결과로 실재 확인(원문 미열람, 유료 표준): iso.org/standard/70181 제목 일치. 필수 필드 책임 주체(2자)·요소(6자, CI/SfB·Uniclass)·표현과 선택 필드 구조가 스니펫과 일치."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 결과로 실재 확인(원문 미열람): nationalcadstandard.org NCS5 문서 URL 일치. 분야 지정자·주 그룹(필수)·부 그룹·상태 필드, 하이픈 구분, A-WALL 예시 일치. 최신성: NCS V6 판 문서(ncs6_clg_lnf.pdf)가 검색됨 — 'NCS V5 문서 기준, V6 판 존재'를 명시해야 함."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 결과로 실재 확인(원문 미열람): 건설CALS/EC 전자도면 작성표준 V1.1(KCCS-0001-2006)은 2006-12-26 한국건설기술연구원장 공고, V1.0 은 2004-08, 도면분류·파일명·선·색상·레이어·심벌 규정 일치. KS F 1542(2020-12-21 확인) 실재 확인. 다만 '레이어 부문이 KS F 1542 로 국가표준화되었다'는 두 문서의 파생 관계는 확인되지 않음 — 두 사실을 따로 서술하고 파생 관계 표현은 뺀다. 공고 주체 정정 필요."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 결과로 실재 확인(원문 미열람): DBpia·AURIC 서지 일치(대한건축학회 논문집 계획계, 2009-11). 분석 결과는 미확인으로 두는 것이 맞다."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 프로젝트 페이지 원본(github_raw)에 레이어·블록 계층 활용 자동 주석과 벡터 공간 전문가 보정 문구 일치."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용 확인: 실행 2026-09-25-05 검증 내용, 이번 실행 원문 미열람. 기존 각주 ref-066·ref-067 재사용. 같은 저자라 독립 교차 아님."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 결과로 실재 확인(원문 미열람): arXiv 2607.12678(2026-07-14, TextCAD). 주석의 복잡한 구문·계층 의미, 유형·속성 결합 인코더, 다중 모달 결합은 스니펫과 일치. 예시 코드 'FM B 1321'은 이번 검증 검색 스니펫에서 확인되지 않음(스니펫 예시는 'Washroom', 'Staircase-11')."
    },
    {
      "finding_id": "f20",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용 확인: 실행 2026-09-25-11 에서 원문 확인된 README, 이번 실행 원문 미열람. 기존 각주 ref-084 재사용."
    },
    {
      "finding_id": "f21",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: f10~f15·f17·f19 결합 정리. 국내 실무 준수율 미확인 명시 유지."
    },
    {
      "finding_id": "f22",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "사실 → 추정: 두 데이터셋 사례(MLSTRUCT px/m 메타데이터, Raster-to-Graph 512×512 정규화)만 출처가 뒷받침하며 '래스터 입력에는 축척이 이미지 자체에 들어 있지 않고 별도 메타데이터로만 주어진다'는 일반화는 출처에 없다. f23 이 이미지 안 치수 문자로 축척을 계산하므로 일반화와도 어긋난다. 두 사례 서술은 사실로, 일반화는 추정으로 나눈다."
    },
    {
      "finding_id": "f23",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 결과로 확인(원문 미열람): Buildings 15(7) 1178, 2025-04-03 게재. YOLOv8·Shi–Tomasi·OFA-OCR 축척 인식과 축척 계산 정확도 95% 초과 스니펫 일치. 저자 보고 단일 출처 수치."
    },
    {
      "finding_id": "f24",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "사실 → 추정: 각 자료의 라벨 범주(계단·난간, 출입문·창호·벽체)는 이전 실행 확인 내용이지만 '엘리베이터 라벨은 이들 자료에서 확인되지 않았다'는 전체 클래스 목록 미열람 상태의 부재 관찰이며 이전 실행(2026-09-25-05 f17)에서도 추정이었다. 라벨 범주는 사실, 엘리베이터 부재는 추정으로 나눈다."
    },
    {
      "finding_id": "f25",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: f22~f24 결합. f22 강등을 반영해 '축척이 이미지에 없다'가 아니라 '축척 표기·치수 문자를 인식으로 읽어야 한다'로 서술해야 f23 과 맞는다."
    },
    {
      "finding_id": "f26",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: 레이어 표준 원문 미열람이라 충전 위치 코드 부재는 확정 아님을 유지."
    },
    {
      "finding_id": "f27",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "재인용 확인(원문 미열람, 실행 2026-09-25-11 검증 내용). BIM 기반 점유 격자 지도·위치추정은 분류 원문 9장 로봇 자체 지능·제어 연계 영역이므로 '연계 대상: '으로 표시해야 함. 기존 각주 ref-081 재사용."
    },
    {
      "finding_id": "f28",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: 분류 원문 SCM 질문 대응 추론. 물류센터 도면의 구역 이름 표기 방식 미확인 유지."
    },
    {
      "finding_id": "f29",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "검색 결과로 실재 확인(원문 미열람): 국토교통부 정책정보 페이지, 2022-07 발표, 성과품 작성·납품·활용 방법·절차 제시 스니펫 일치."
    },
    {
      "finding_id": "f30",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "추정 유지: f1~f26 의 입력 형식별 종합. f22·f24 강등과 f15 파생 관계 삭제를 반영한 문구로 서술."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "f6 은 단계 2 페이지 3절 q2-01 의 IFC 연결 도출 추정(ref-156·ref-334)과 같은 내용 — 기존 각주 재사용, 새 주장으로 반복하지 않고 q2-02 맥락의 연결만 쓴다",
      "f9·f26 의 IFC 충전 유형 부재는 아이디어 페이지 4절 BIM(IFC 4.3) 소절과 공간 그래프 스키마 초안 6절에 이미 있음 — 기존 각주 ref-214·ref-215 재사용",
      "f18·f20·f22·f24·f27 은 실행 2026-09-25-05·11 결과(아이디어 페이지 3절, 단계 1 페이지)의 재인용 — 기존 각주 재사용하고 새 조사처럼 서술하지 않는다",
      "f4 는 스키마 초안의 엘리베이터 속성(ref-213 IfcTransportElement)의 후속 확정 — ref-213 과 ref-740 을 함께 쓴다"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "quotation_check": {
    "ok": true,
    "issues": [
      "f2·f3 의 evidence_excerpt 인용 문구가 이번에 연 원본 문구와 다름 — 페이지에 직접 인용으로 옮기지 않는다(수정 지시 참조)"
    ]
  },
  "corrections_applied": [],
  "corrections_rejected": [],
  "required_fixes": [
    "f22: [사실] → [추정]으로 강등하고 두 부분으로 나눈다 — 'MLSTRUCT-FP 는 이미지별 축척(px/m) 메타데이터를 주고 Raster-to-Graph 는 512×512 로 정규화한 이미지 좌표를 쓴다'는 [사실](ref-069·ref-070), '래스터 입력에서는 축척을 별도 메타데이터나 도면 안 축척 표기·치수 문자 인식으로 얻어야 한다'는 [추정]으로 쓴다 — 출처는 두 데이터셋 사례만 뒷받침하고 일반화는 없다.",
    "f24: 라벨 범주(CubiCasa5K·Kratochvila 외의 계단·난간, AI Hub 의 출입문·창호·벽체·도면 문자)는 [사실]로, '엘리베이터 라벨은 확인되지 않았다'는 [추정](전체 클래스 목록 미열람, 부재 확정 아님, q2-04)으로 나눠 쓴다.",
    "f25: f22 강등에 맞춰 '축척이 이미지에 없다'가 아니라 '축척은 축척 표기·치수 문자에서 인식으로 복원해야 한다'로 서술하고 f23 과 모순되지 않게 한다(태그 [추정] 유지).",
    "f2: 정의를 '공간을 둘러싸거나 나누는 수직 구조'로 고친다(원본 'may bound or subdivide spaces'). evidence_excerpt 의 인용 문구는 원본과 달라 직접 인용으로 옮기지 않는다.",
    "f3: 정의를 '다른 높이의 층 사이를 걸어서 오가게 하는 수직 통로'로 고치고, 컨테이너 조건을 '기본은 IfcBuildingStorey, 층에 할당할 수 없으면 IfcBuilding, 외부는 IfcSite'로 고친다('여러 층에 걸치면 IfcBuilding' 표현 삭제). evidence_excerpt 인용 문구는 원본에 없으므로 직접 인용하지 않는다.",
    "f5: 층 정의를 원본 뜻대로 '수직으로 경계 지어진 공간들의 (거의) 수평 집합'으로 옮긴다.",
    "f10·f11·f12: DXF 형식 진술을 'ezdxf(오픈소스 DXF 라이브러리) 문서는 …라고 설명한다'로 귀속해 쓰고 'Autodesk 공식 DXF 참조가 아님'을 본문 또는 각주에 남긴다 — 형식 규격의 발행 주체 자료가 아니다.",
    "f7: 참고문헌 ref-751 의 기관을 'Noardo, F., Arroyo Ohori, K., Krijnen, T., & Stoter, J. (Applied Sciences 11(5), 2232)'로 고치고, '점검·채점했다'를 '점검했다'로 줄인다 — 저자는 검색 결과로 확인됐고 '채점'은 확인되지 않았다.",
    "f14: 'NCS V5 문서 기준이며 V6 판(ncs6_clg_lnf.pdf)이 있다'를 기준일과 함께 명시한다 — 더 새 판이 있다.",
    "f15: '레이어 부문은 KS F 1542 로 국가표준화되어 있다'의 파생 관계 표현을 삭제하고 두 사실을 따로 쓴다: 건설CALS/EC 전자도면 작성표준(V1.0 2004-08, V1.1 KCCS-0001-2006 은 2006-12-26 한국건설기술연구원장 공고)과 KS F 1542(2020-12-21 확인). 공고 주체를 '국토교통부'가 아니라 한국건설기술연구원(건설CALS 체계 안의 표준)으로 고친다.",
    "f19: 예시 코드 'FM B 1321'은 검증 검색에서 확인되지 않았으므로 본문에서 빼고, 주석의 유형·속성 결합 구조와 다중 모달 방법만 쓴다(발행일 2026-07-14).",
    "f27: 문장 앞에 '연계 대상: '을 붙이고 BIM 기반 지도·위치추정은 로봇 자체 지능·제어 쪽이며 도면 입력이 담지 못하는 정보의 근거로만 쓴다고 밝힌다 — 분류 원문 9장 경계.",
    "원문 미열람 표기: 이번 실행에서 원문을 열지 않은 출처(ref-746·ref-747·ref-748·ref-749·ref-750·ref-751·ref-752·ref-754·ref-755 와 재사용 ref-156·ref-334·ref-214·ref-215·ref-063·ref-066·ref-067·ref-069·ref-070·ref-073·ref-074·ref-078·ref-081·ref-084)의 페이지 각주 접근일 뒤에 ' (원문 미열람)'을 붙이고, reference_updates 의 신규 출처 ref-746~ref-752·ref-754·ref-755 에 source_unopened: true 를 넣는다. 단 기존 참고문헌 페이지의 각주 줄을 재사용할 때는 그 페이지 줄을 따른다.",
    "온톨로지 초안 반영(v0.4 → v0.5): 승인 변경만 반영한다 — (1) 문: 속성 'BIM 대응 클래스(IFC 4.3 IfcDoor, 개발 브랜치 기준, IfcRelFillsElement 로 벽 개구부를 채움)' 추가, 폭·여닫는 방식의 IFC 원천(OverallWidth·OperationType) 메모(f1). 6절의 '문 표준 대응 클래스(IndoorGML 2.0)' 질문은 그대로 둔다. (2) 계단: 속성 'BIM 대응 클래스(IFC 4.3 IfcStair, IfcStairFlight·LANDING 슬래브·IfcRailing 으로 분해 가능, 기본 컨테이너 IfcBuildingStorey)' 추가(f3, 위 f3 문구 수정 반영). '잇는 층은 포함 관계·형상에서 도출해야 할 것으로 보인다'(f6 추정)는 개념 표에 넣지 않고 6절 질문(q3-05 연결)으로 둔다. (3) 엘리베이터: 기존 BIM 대응 클래스 속성에 유형 열거 값 ELEVATOR(IfcTransportElementTypeEnum) 확정(f4), '(유형 열거 값 이름은 단계 2에서 확정)' 표시 제거. (4) 평면도: 속성 '길이 단위·축척 정보(DXF 는 선택 헤더 $INSUNITS, ezdxf 문서 기준)' 추가(f12). 래스터 축척 복원 방식(f22 강등)은 근거 칸에 넣지 않고 6절 질문(q4-05)으로 둔다.",
    "새 질문 '벡터 CAD 도면의 레이어·블록 이름과 텍스트 주석을 선 요소 인식과 결합해 … 사람 검토를 어디에 두는가?'(단계 3)는 q3-01(처리 흐름 단계별 입력·출력과 사람 검토 위치)과 중복 — backlog_updates 에 넣지 않고, 필요하면 단계 2 페이지 5절에 'q3-01 로 흡수'로만 적는다.",
    "단계 페이지 6절: 완료 조건은 두 항목 모두 미충족으로 적는다 — 입력 형식별 정보 항목(q2-02)은 이번 실행으로 채워지지만 관제 수용 형식(q2-03)이 없고, 관계(엣지) 쪽 표준 대응이 없다. '다음 단계로 전환: 아니오(q2-03 미작성, 엣지 표준 대응 없음; 열린 질문 q2-03·q2-04·q2-06·q2-07)'로 쓰고 검증 판정 칸은 '미충족 · 미승인'.",
    "중복 내용(f6·f9·f18·f20·f22·f24·f26·f27)은 기존 각주 id 를 재사용하고 이미 페이지에 실린 문장을 새 주장처럼 반복하지 않는다."
  ],
  "confidence": "medium",
  "verification_note": "판정: 조건부 승인. 이번 실행은 원문 열람이 차단된 환경에서 검증됐다(raw.githubusercontent.com 만 열림). 확인 28건, 미확인 2건, 교차 확인 0건. 강등: f22 사실 → 추정(두 데이터셋 사례를 래스터 입력 전체로 일반화), f24 사실 → 추정(엘리베이터 라벨 부재는 클래스 목록 미열람 상태의 관찰). 원문 미열람 출처: ref-746, ref-747, ref-748, ref-749, ref-750, ref-751, ref-752, ref-754, ref-755, 재사용 ref-156, ref-334, ref-214, ref-215, ref-063, ref-066, ref-067, ref-069, ref-070, ref-073, ref-074, ref-078, ref-081, ref-084. 원문 대조: IFC 4.3 개발 브랜치 5건(IfcDoor·IfcStair·IfcTransportElementTypeEnum·IfcWall·IfcBuildingStorey), ezdxf 문서 3건, ArchCAD-400K 프로젝트 페이지를 검증자가 다시 열어 확인했다. IfcWall·IfcStair 정의 인용은 원본 문구와 달라 정정을 지시했다. DXF 진술은 Autodesk 공식 참조가 아닌 ezdxf 문서 기준이다. 정정: ref-751 저자(Noardo 외), 건설CALS 전자도면 작성표준 V1.1 공고 주체(한국건설기술연구원), NCS V6 판 존재. f15 의 'KS F 1542 로 국가표준화' 파생 관계와 f19 의 예시 코드는 확인되지 않아 뺀다. 주의: 입력 형식별 비교(f30)와 충전 위치 표준 표현 부재(f26)는 이 위키의 추정이며, 모든 주장이 단일 발행 주체 자료에 기댄다. 온톨로지 변경 승인: 문 BIM 대응 클래스 IfcDoor(f1), 계단 BIM 대응 클래스 IfcStair(f3), 엘리베이터 유형 값 ELEVATOR 확정(f4), 평면도 길이 단위·축척 정보 속성(f12) / 거부: 계단이 잇는 층의 도출(f6 추정 → 6절 질문), 래스터 축척 복원 방식(f22 강등 → 6절 질문 q4-05). 새 질문 중복: 벡터 CAD 텍스트 결합 처리 흐름의 사람 검토 질문은 q3-01 과 중복. 단계 완료 조건: 미충족(부족: 관제 수용 형식 q2-03 미작성, 관계(엣지) 쪽 표준 대응 없음). 단계 전환: 미승인(막힌 질문 q2-03·q2-04·q2-06·q2-07 열림). 정정 요청 없음.",
  "retry_reason": null,
  "track_checks": {
    ...
    "backlog_duplicates": [
      "벡터 CAD 도면의 레이어·블록 이름과 텍스트 주석(문 기호 코드, 방·구역 이름)을 선 요소 인식과 결합해 공간 노드·문·구역 이름을 만드는 처리 흐름에서 사람 검토를 어디에 두는가? (q2-02 에서 파생) — q3-01 과 중복"
    ],
    "stage_tag_issues": [],
...
### runs/2026-09-25-36/docs_tree.txt
...
- [공간 그래프 스키마 초안](space-graph-schema-draft.md) — 현재 버전 v0.4. 실행 2026-09-25-05에서 창문·난간 개념과 문 없는 인접 관계를 더하고 공간 노드에 방 유형 속성을 넣었으며 벽·문·계단·공간 노드를 확정했다. 실행 2026-09-25-11에서 층간 정렬 기준점을 더하고 층별 지도(축척·도면 대비 변환)와 평면도(형식 값 후보)를 확정했으며, 문 속성 추가 제안은 미해결 질문으로 두었다. 실행 2026-09-25-19에서 작업 스테이션 개념을 더하고 충전 위치(접근 지점 속성)·엘리베이터(BIM 대응 클래스 속성)를 확정했으며, 정보 출처 속성과 작업 스테이션의 공용 자원 포함 여부는 미해결 질문으로 두었다. 실행 2026-09-25-22에서는 변경이 없었다. 실행 2026-09-25-28에서 공간 노드·층에 표준 대응 클래스(후보) 속성을 더하고 층을 확정했으며 주제 레이어 개념을 더했고, 문의 표준 대응 클래스와 주제 레이어의 로봇·사람 레이어 구분은 미해결 질문으로 두었다.
- [아이디어 3. 건축 도면 자동 인식](../../ideas/floorplan-recognition.md) — 확장 아이디어 페이지. 3절(선행 연구·제품 사례)에 공개 데이터셋 비교(실행 2026-09-25-05), 도면→로봇 지도 연구·오픈소스 도구 비교와 제품 사례(벤더 주장 병기, 실행 2026-09-25-11), 운영 시설(충전소·작업 스테이션) 보완 사례(실행 2026-09-25-19), 현장 모델링 부담의 근거(실행 2026-09-25-22)가 실렸다. 4절에는 IFC 4.3·VDMA LIF·VDA 5050의 시설 표현(실행 2026-09-25-19)과 공간 그래프를 표현하는 표준 비교(q2-01, 실행 2026-09-25-28)가 실렸으며, 입력 형식별 정보 항목과 관제 수용 형식은 아직 없다. 6절에는 단계 5 조사 전 선행 근거로 가설 3 판정의 비교 기준 후보와 측정 대상 반복 작업이 실렸다(실행 2026-09-25-22). 5절은 단계 3·4 실행이 채운다.
- [질문 백로그](question-backlog.md) — 시작 질문 18건과 후속 질문 9건(실행 2026-09-25-05의 3건, 실행 2026-09-25-11의 1건, 실행 2026-09-25-19의 1건, 실행 2026-09-25-22의 2건, 실행 2026-09-25-28의 2건). 실행 2026-09-25-11에서 중복 등록된 q1-07·q2-05·q4-06은 실행 2026-09-25-19에서 폐기했다. q1-01·q1-02·q1-03·q1-04·q2-01 답함. 최신 수치는 백로그 페이지의 자동 표를 따른다.
- [트랙 로그](log.md) — 실행별 기록
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-063, ref-064, ref-065, ref-068, ref-069, ref-070, ref-071, ref-072, ref-078, ref-079, ref-080, ref-081, ref-082, ref-084, ref-085, ref-086, ref-223, ref-224, ref-046, ref-212, ref-213, ref-214, ref-215, ref-216, ref-156, ref-331, ref-332, ref-333, ref-334, ref-336, ref-338, ref-339, ref-343, ref-348]
last_run: 2026-09-25
version: 5
---
...
<!-- auto:page-status:start -->
> 초안 버전: v0.4 · 페이지 상태: published · 신뢰도: medium · 페이지 버전: 5 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
<!-- auto:page-status:start -->
> 페이지 상태: published · 신뢰도: medium · 페이지 버전: 6 · 마지막 갱신: 2026-09-25 · 마지막 실행: 2026-09-25
<!-- auto:page-status:end -->
...
원천: [질문 백로그](../tracks/floorplan-recognition/question-backlog.md)([건축 도면 자동 인식](../tracks/floorplan-recognition/index.md) 트랙) · 열림 22건 · 답함 5건 · 폐기 3건
...
```json
{
  ...
  "outline": [
    {
      "path": "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
      "section": "3. 조사 결과 (q2-02 소제목 추가)",
      "budget_chars": 5200,
      "summary": "이 위키의 정리로는, BIM(IFC 4.3) 입력은 벽·문·계단·엘리베이터·층을 유형 객체로 담지만 연결은 도출해야 하고, 벡터 CAD는 의미·단위가 레이어·블록·텍스트 관례에 기대며, 래스터 스캔은 의미·축척을 인식으로 복원해야 하고, 충전 위치는 세 형식 모두 표준 표현이 확인되지 않은 것으로 보인다. [추정][^ref-738][^ref-744][^ref-754][^ref-214]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 1800,
      "summary": "q2-02 결론 3건과 스키마 v0.5 반영을 더하고, 원문 미열람 표준·ezdxf 기준·프록시 비율 미확인을 불확실성에 더한다. [추정][^ref-738][^ref-745]"
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
      "section": "6. 완료 조건 충족 현황",
      "budget_chars": 600,
      "summary": "두 조건 모두 미충족: q2-03 미작성, 엣지 표준 대응 없음."
    },
    {
      "path": "docs/tracks/floorplan-recognition/space-graph-schema-draft.md",
      "section": "2. 개념 목록 표",
      "budget_chars": 3500,
      "summary": "문·계단 BIM 대응 클래스, 엘리베이터 유형 값 ELEVATOR, 평면도 길이 단위·축척 정보 속성을 반영한다(v0.5). [사실][^ref-738][^ref-739][^ref-740][^ref-745]"
    },
    {
      "path": "docs/ideas/floorplan-recognition.md",
      "section": "4. 필요한 데이터와 표준",
      "budget_chars": 2500,
      "summary": "입력 형식별 정보 항목 소절을 더한다. [추정][^ref-738][^ref-744][^ref-754]"
    }
  ],
  "pages": [
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "q2-02 답함(3절 q2-02 소제목 신설: BIM·벡터 CAD·래스터 입력별 정보 항목과 빠지는 정보, 입력 형식별 비교표), 2절 표에 q2-08·q2-09 추가, 4·5·6·7·8·9절 갱신. H1 아래 단계 상태 줄(열린 질문 6건·답한 질문 2건)은 H2 밖이라 patches 로 바꾸지 못했다 — 퍼블리셔 쪽 갱신 필요",
      "patches": [
        {
          "section": "2. 질문 목록",
          "action": "replace",
    ...
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "초안 v0.4 → v0.5: 문 BIM 대응 클래스 IfcDoor(f1), 계단 BIM 대응 클래스 IfcStair(f3), 엘리베이터 유형 값 ELEVATOR 확정(f4), 평면도 길이 단위·축척 정보 속성(f12) 반영, 6절에 거부된 변경(계단이 잇는 층 도출, 래스터 축척 복원)과 프록시·충전 위치 질문 추가. H1 의 '(v0.4)' 표기는 H2 밖이라 patches 로 바꾸지 못했다 — 퍼블리셔가 프런트매터 ontology_version 으로 '(v0.5)'로 맞춰야 한다",
      "patches": [
        {
          "section": "2. 개념 목록 표",
          "action": "replace",
    ...
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "4절에 '입력 형식별 정보 항목' 소절(q2-02, 실행 2026-09-25-36) 신설, 4절 머리말과 q2-01 소절의 'q2-02 미조사' 문장을 갱신",
      "patches": [
        {
          "section": "4. 필요한 데이터와 표준",
          "action": "replace",
          ...
          "content": "BIM(IFC 4.3)은 엘리베이터를 표준 클래스로 담을 수 있지만 이번에 확인한 유형 값에는 로봇 충전 설비가 없고, VDA 5050과 LIF는 충전소·적재 스테이션을 스테이션 유형이 아니라 노드에 걸린 동작과 이름으로 드러내는 것으로 보인다. [추정][^ref-213][^ref-214][^ref-031][^ref-212] 이 절의 첫 세 소절은 [단계 2. 필요한 데이터와 표준 조사](../tracks/floorplan-recognition/stage-2-data-and-standards.md) 전에 [q1-03 답](../tracks/floorplan-recognition/stage-1-prior-work-and-products.md#q1-03)(실행 2026-09-25-19)에서 확인한 선행 근거이며, 운영 시설(엘리베이터·충전소·작업 스테이션)을 표준·교환 형식이 어떻게 담는지에 한정한다. 공간 그래프 표준 목록은 '공간 그래프를 표현하는 표준' 소절(q2-01), 입력 형식별 정보 항목은 '입력 형식별 정보 항목' 소절(q2-02)에 있고, 층별 지도·공용 자원 목록을 로봇 관제·ROP가 받아들이는 형식(q2-03)은 단계 2 실행이 채운다.\n\n### BIM(IFC 4.3)\n\n아래 근거는 buildingSMART 개발 저장소의 개발 브랜치(ifc4.3-main) 원본이며, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있다.\n\n- IfcTransportElement는 시설 안에서 사람·동물·물품을 옮기는 운송 관련 객체의 일반화로 정의되고 엘리베이터·에스컬레이터·무빙워크를 예로 들어, 엘리베이터는 표준 클래스로 담길 수 있다. [사실][^ref-213]\n- 콘센트 유형 열거(IfcOutletTypeEnum)와 전기기기 유형 열거(IfcElectricApplianceTypeEnum)에는 차량·로봇 충전 설비를 뜻하는 값이 없고 USERDEFINED·NOTDEFINED만 남는다. 두 파일은 같은 발행 주체라 독립 교차 확인이 아니다. [사실][^ref-214][^ref-215]\n- 따라서 로봇 충전소는 사용자 정의 유형·속성 세트로 따로 모델링되거나 설계 도면에 담기지 않을 가능성이 클 것으로 보인다. 다른 IFC 클래스·속성 세트와 작업대 표현은 미확인이다(후속 질문 q2-06). [추정][^ref-213][^ref-214][^ref-215]\n\n### 레이아웃 교환 형식(VDMA LIF)\n\n- VDMA의 LIF 공식 저장소 README는 LIF를 무인운반 차량 통합사업자가 주행 레이아웃(엣지·노드·스테이션)을 상위 관제에 넘기기 위한 구속력 없는 교환 형식으로 정의하고, 1.0.0 판을 2023-09로 적는다. [사실][^ref-046]\n- VDA 5050 3.0.0 명세는 LIF를 'VDMA 2024-03'으로 인용한다. [사실][^ref-031] 두 출처의 판·발행일이 달라 한쪽을 고르지 않고 [열린 질문](../open-questions.md)으로 올렸다.\n- VDMA 공식 산출물이 아닌 제3자(continua-systems) JSON 스키마에서는 스테이션이 식별자·상호작용 노드 목록·위치(x·y 미터, 선택 방향)·높이·이름·설명만 갖고 스테이션 유형 필드가 없으며, 레이아웃은 층·버전을 갖는다. 이를 LIF 표준 자체의 구조로 확정하지는 못했다. [사실][^ref-212]\n\n### VDA 5050 3.0.0\n\n- 충전은 즉시 동작(instantAction) 또는 노드 동작으로 쓰는 startCharging·stopCharging으로, 적재 스테이션은 pick·drop 동작의 선택 파라미터(stationType·stationName 등)로 표현되며, 구역 유형 10종에는 충전소·작업 스테이션 유형이 없다(공식 GitHub 저장소 main, 2026-09-25 확인). [사실][^ref-031]\n- 지도는 mapId·mapVersion으로 식별하고 관제가 downloadMap·enableMap 동작으로 배포·활성화하며, 도입 단계에서 LIF로 경로를 관제에 가져올 수 있다. [사실][^ref-031]\n\n### 시사점\n\n- LIF는 통합사업자가 관제에 레이아웃을 넘기는 교환 형식으로 정의되므로, 스테이션·충전소 정보는 건물 도면에서 인식한 공간 그래프와는 별도 출처의 시설 정보가 될 것으로 보인다. 두 정보의 식별자·좌표 대응은 단계 4의 q4-03에서 다룬다. [추정][^ref-031][^ref-046][^ref-212]\n\n[^ref-031]: VDA / VDMA (VDA5050 GitHub), VDA5050/VDA5050_EN.md — Official Specification document for the VDA 5050, 미확인, https://github.com/VDA5050/VDA5050/blob/main/VDA5050_EN.md, 접근일 2026-09-25\n[^ref-046]: VDMA (Intralogistics-2X-LIF GitHub), Layout-Interchange-Format — README (Repository for the Layout Interchange Format (LIF) developed by the VDMA), 2023-09, https://github.com/Intralogistics-2X-LIF/Layout-Interchange-Format, 접근일 2026-09-25\n[^ref-212]: continua-systems (GitHub), vdma-lif — schema/lif-schema.json (JSON parsers and models for the VDMA LIF, VDMA 공식 산출물이 아닌 제3자 스키마), 미확인, https://github.com/continua-systems/vdma-lif/blob/main/schema/lif-schema.json, 접근일 2026-09-25\n[^ref-213]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcTransportElement (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/core/IfcProductExtension/Entities/IfcTransportElement.md, 접근일 2026-09-25\n[^ref-214]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcOutletTypeEnum (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/domain/IfcElectricalDomain/Types/IfcOutletTypeEnum.md, 접근일 2026-09-25\n[^ref-215]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcElectricApplianceTypeEnum (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/domain/IfcElectricalDomain/Types/IfcElectricApplianceTypeEnum.md, 접근일 2026-09-25\n\n### 공간 그래프를 표현하는 표준 (2026-09-25 기준)\n\n확인한 표준 가운데 공간 연결을 노드–엣지 그래프로 명시하는 것은 IndoorGML이고, IFC 4.3·CityGML 3.0은 공간·층·경계를 담되 연결은 따로 도출해야 할 것으로 보이며, IndoorGML 2.0 인코딩은 아직 초안이다. 이 분류는 이 위키의 정리다. [추정][^ref-331][^ref-333][^ref-156][^ref-334][^ref-339] 이 소절은 [q2-01 답](../tracks/floorplan-recognition/stage-2-data-and-standards.md#q2-01)(실행 2026-09-25-28)의 요약이며, 문장별 상세는 단계 페이지에 있다. 입력 형식(벡터 CAD, BIM 모델, 래스터 스캔)별 정보 항목(q2-02)은 아래 '입력 형식별 정보 항목' 소절에 있고, 층별 지도·공용 자원 목록을 로봇 관제·ROP가 받아들이는 형식(q2-03)은 아직 조사하지 않아 이 절에 없다.\n\n아래 표는 검증된 발견 사항으로 직접 만든 것이며 출처의 표를 옮긴 것이 아니다. 각 칸은 근거 열 출처에서 확인한 사실이되, IFC의 '공간 사이 직접 연결 관계 없음'과 BOT의 '문 전용 클래스 없음'은 열람 범위 기준의 추정(부재 확정 아님)이다. 원문을 열지 못한 출처는 검색 요약 기준이며, \"미확인\"은 이번 실행에서 확인하지 못했다는 뜻이다.\n\n| 표준 | 발행 주체와 상태 | 공간 | 공간 사이 연결 | 층 | 문 | 근거 |\n|---|---|---|---|---|---|---|\n| IndoorGML 2.0 | OGC. Part 1 개념 모델(22-045r5) 2025-08 발행, Part 2 인코딩(XML·JSON·SQL)은 초안(JSON v0.5.0, 2026-02-28 제출) | 셀 공간(CellSpace) | 쌍대 공간의 노드(Node)·엣지(Edge), 여러 주제 레이어와 레이어 간 연결 | 미확인 | 미확인(경계와 공간 가운데 어느 쪽으로 표현하는지 미확인) | [^ref-331][^ref-332][^ref-333][^ref-157] |\n| IFC 4.3 | buildingSMART, ISO 16739-1:2024(기반시설 정보 추가). 근거는 개발 브랜치 원본 | IfcSpace | 공간–공간 직접 연결 관계 없음(추정). 공간 경계 관계 IfcRelSpaceBoundary(2차 A 유형은 반대편이 다른 공간) | IfcBuildingStorey(IfcRelAggregates로 묶음) | 개구부가 공간 경계 요소가 됨 | [^ref-156][^ref-334][^ref-335] |\n| CityGML 3.0 | OGC 20-010, 2021 | BuildingRoom(비점유 공간의 하위 클래스) | 공간 경계(AbstractSpaceBoundary)와 가상 경계(ClosureSurface). 연결 표현 방식은 미확인 | Storey | DoorSurface(채움 면) | [^ref-339][^ref-340] |\n| ISO 19164:2024 | ISO | 실내 지물의 핵심 의미 분류 | 기하·위상보다 의미에 초점 | 미확인 | 미확인 | [^ref-158] |\n| BOT v0.3.2 | W3C 링크드 빌딩 데이터 커뮤니티 그룹(W3C 권고안 아님), 2020-07-31 수정 | Space | adjacentZone·adjacentElement 관계와 Interface | Storey | 전용 클래스 없음(추정) | [^ref-336] |\n| Brick | Brick Consortium | brick:Location(방) | hasPart·isPartOf 계층, 정확한 기하는 담지 않음 | brick:Location(층) | 미확인 | [^ref-341] |\n| IMDF 1.0.0 | OGC 커뮤니티 표준(2021-02-23), Apple 개발, 사람 길안내용 | unit | 미확인 | level | opening(접근성·출입통제 속성) | [^ref-338] |\n\n- buildingSMART의 ifcOWL 저장소는 IFC 스키마를 RDF·TTL 온톨로지로 제공하며, README의 대상 판 목록은 IFC4_ADD2까지이고 IFC 4.3은 목록에 없다(2026-09-25 확인). [사실][^ref-342]\n- ISO 19164:2024가 정보성 부속서로 CityGML 3.0·IFC·IndoorGML과의 클래스 수준 대응을 제시한다는 내용은 검색 요약 기준(원문 미열람)이다. [사실][^ref-158]\n- 국토교통부 고시 '실내공간정보 구축 작업규정'(2018-03-05 제정판 기준)은 실내공간정보를 원칙적으로 CityGML 2.0 데이터 형식과 IndoorGML 공간 개념으로 제작하도록 한다. 현행 조문이 같은 원칙을 유지하는지는 미확인이다. [사실][^ref-345]\n- 이 위키의 정리로는, 공간 노드는 IndoorGML CellSpace·IFC IfcSpace·CityGML BuildingRoom·BOT Space·IMDF unit에, 층은 IfcBuildingStorey·BOT Storey·IMDF level에 대응할 수 있으나, 로봇 충전 위치·작업 스테이션 전용 클래스는 이번에 확인한 표준들에서 찾지 못한 것으로 보인다. [추정][^ref-333][^ref-156][^ref-334][^ref-339][^ref-336][^ref-338] 검증이 승인한 대응 후보는 [공간 그래프 스키마 초안](../tracks/floorplan-recognition/space-graph-schema-draft.md) v0.4에 반영했다.\n\n[^ref-156]: buildingSMART International, IFC 4.3 documentation — IfcSpace (IFC4.3.x-development), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/core/IfcProductExtension/Entities/IfcSpace.md, 접근일 2026-09-25\n[^ref-157]: OGC IndoorGML SWG (opengeospatial/IndoorGML-SWG GitHub), IndoorGML-SWG — README and OGC IndoorGML 2.0 Part 2a – XML Encoding (26-042, Candidate SWG Draft), 미확인, https://github.com/opengeospatial/IndoorGML-SWG, 접근일 2026-09-25\n[^ref-158]: ISO, ISO 19164:2024 - Geographic information — Indoor feature model, 2024, https://www.iso.org/standard/83153.html, 접근일 2026-09-25 (원문 미열람)\n[^ref-331]: OGC (Open Geospatial Consortium), OGC IndoorGML 2.0 Part 1 – Conceptual Model (22-045r5), 2025-08, https://docs.ogc.org/is/22-045r5/22-045r5.html, 접근일 2026-09-25 (원문 미열람)\n[^ref-332]: OGC (Open Geospatial Consortium), OGC Publishes IndoorGML 2.0 Part 1 Conceptual Model Standard, 2025-08-28, https://www.ogc.org/announcement/ogc-publishes-indoorgml-2-0-part-1-conceptual-model-standard/, 접근일 2026-09-25 (원문 미열람)\n[^ref-333]: OGC IndoorGML SWG (opengeospatial/IndoorGML-SWG GitHub), OGC IndoorGML 2.0 Part 2b – JSON Encoding (26-043, Candidate SWG Draft v0.5.0), 2026-02-28, https://github.com/opengeospatial/IndoorGML-SWG/blob/master/26-043.html, 접근일 2026-09-25\n[^ref-334]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcRelSpaceBoundary (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/core/IfcProductExtension/Entities/IfcRelSpaceBoundary.md, 접근일 2026-09-25\n[^ref-335]: ISO, ISO 16739-1:2024 - Industry Foundation Classes (IFC) for data sharing in the construction and facility management industries — Part 1: Data schema, 2024, https://www.iso.org/standard/84123.html, 접근일 2026-09-25 (원문 미열람)\n[^ref-336]: W3C Linked Building Data Community Group (w3c-lbd-cg GitHub), Building Topology Ontology (BOT) — bot.ttl (version 0.3.2), 2020-07-31, https://github.com/w3c-lbd-cg/bot/blob/master/bot.ttl, 접근일 2026-09-25\n[^ref-338]: OGC (Open Geospatial Consortium) / Apple, Indoor Mapping Data Format (1.0.0) — OGC Community Standard 20-094, 2021-02, https://docs.ogc.org/cs/20-094/, 접근일 2026-09-25 (원문 미열람)\n[^ref-339]: OGC (Open Geospatial Consortium), OGC City Geography Markup Language (CityGML) Part 1: Conceptual Model Standard (20-010), 2021, https://docs.ogc.org/is/20-010/20-010.html, 접근일 2026-09-25 (원문 미열람)\n[^ref-340]: PFG(Journal of Photogrammetry, Remote Sensing and Geoinformation Science) 게재 논문 저자(미확인), CityGML 3.0: New Functions Open Up New Applications, 2020, https://link.springer.com/article/10.1007/s41064-020-00095-z, 접근일 2026-09-25 (원문 미열람)\n[^ref-341]: Brick Consortium (Brick Schema), Relationships — Brick Ontology Documentation, 미확인, https://docs.brickschema.org/brick/relationships.html, 접근일 2026-09-25 (원문 미열람)\n[^ref-342]: buildingSMART (buildingsmart-community GitHub), ifcOWL — README (ifcOWL standard), 미확인, https://github.com/buildingsmart-community/ifcOWL, 접근일 2026-09-25\n[^ref-345]: 국토교통부(법제처 국가법령정보센터), 실내공간정보 구축 작업규정, 2018-03-05, https://law.go.kr/admRulLsInfoP.do?admRulSeq=2100000116559, 접근일 2026-09-25 (원문 미열람)\n\n### 입력 형식별 정보 항목 (2026-09-25 기준)\n\n이 위키의 정리로는, BIM(IFC 4.3) 입력은 벽·문·계단·엘리베이터·층을 유형 객체로 담아 세 입력 형식 가운데 정보가 가장 많지만 공간 사이 연결은 도출해야 하고 실무 모델에서는 프록시 오분류가 있을 수 있으며, 벡터 CAD는 요소 의미·단위가 레이어·블록·텍스트 관례에 기대고, 래스터 스캔은 의미·축척을 인식으로 복원해야 하며, 충전 위치는 세 형식 모두 표준 표현이 확인되지 않은 것으로 보인다. [추정][^ref-738][^ref-739][^ref-740][^ref-751][^ref-744][^ref-745][^ref-752][^ref-754][^ref-214] 이 소절은 [q2-02 답](../tracks/floorplan-recognition/stage-2-data-and-standards.md#q2-02)(실행 2026-09-25-36)의 요약이며, 문장별 상세는 단계 페이지에 있다.\n\n아래 표는 검증된 발견 사항으로 직접 만든 이 위키의 정리이며 출처의 표를 옮긴 것이 아니다. BIM 열의 클래스·속성 이름과 데이터셋 라벨 유무는 출처에서 확인한 사실이고, '(추정)'을 붙인 칸은 이 위키의 추정이다. \"미확인\"은 이번 실행에서 확인하지 못했다는 뜻이다. DXF 설명은 Autodesk 공식 DXF 참조가 아닌 오픈소스 라이브러리 ezdxf 문서 기준이다.\n\n| 요소 | BIM(IFC 4.3) | 벡터 CAD(DXF·DWG) | 래스터 스캔 |\n|---|---|---|---|\n| 벽 | IfcWall(공간을 둘러싸거나 나누는 수직 구조), 개구부는 IfcRelVoidsElement[^ref-741] | 선·폴리라인, 의미는 레이어 이름 관례에 기댐(추정)[^ref-744] | 기호 모양에서 인식(추정)[^ref-063] |\n| 문 | IfcDoor(전체 높이·폭, 여닫는 방식), IfcRelFillsElement 로 벽 개구부를 채움[^ref-738] | 블록 참조·속성 텍스트, A-DOOR 같은 레이어 이름 관례(추정)[^ref-743][^ref-747] | 공개 데이터셋에 출입문 라벨 있음[^ref-074] |\n| 엘리베이터 | 운송 요소 유형 값 ELEVATOR[^ref-740][^ref-213] | 레이어 코드 미확인 | 공개 데이터셋 라벨 미확인(추정)[^ref-063][^ref-074] |\n| 계단 | IfcStair(계단 구간·참 슬래브·난간으로 분해 가능), 잇는 층 전용 속성 미확인[^ref-739] | 레이어 코드 미확인 | CubiCasa5K·Kratochvila 외 라벨 있음[^ref-063][^ref-078] |\n| 층·축척 | IfcBuildingStorey, 층 고도는 속성 세트 사용 권고[^ref-742] | 좌표 값에 단위 없음, 모델 공간 단위는 선택 헤더 $INSUNITS[^ref-745] | 별도 메타데이터나 축척 표기·치수 문자 인식(추정)[^ref-069][^ref-754] |\n| 충전 위치 | 표준 유형 값 없음[^ref-214][^ref-215] | 레이어 코드 찾지 못함(추정)[^ref-746][^ref-747] | 공개 데이터셋 라벨 없음(추정)[^ref-063][^ref-073] |\n| 주의할 점 | 실무 모델의 프록시 오분류 가능성[^ref-751], 공간 연결은 도출 필요(추정) | 표준을 따르지 않은 도면은 의미·축척 복원 필요(추정)[^ref-752] | 의미·축척 모두 인식으로 복원(추정)[^ref-754] |\n\n- Noardo 외(Applied Sciences 11(5), 2021)는 실무 IFC 모델을 표준 정의와 대조해 점검하면서, 범용 요소 IfcBuildingElementProxy 가 유효한 IFC 엔터티가 있는 요소의 대체로 잘못 쓰일 수 있다고 보고 모델마다 그 사용을 점검했다. [사실][^ref-751]\n- CAD 레이어 이름 표준으로는 ISO 13567-1:2017(책임 주체·요소·표현 등 고정 길이 필드)과 미국 NCS가 채택한 AIA 레이어 형식(하이픈으로 나눈 필드, A-DOOR·A-WALL 같은 이름, NCS V5 문서 기준이며 V6 판이 있음)이 있다. [사실][^ref-746][^ref-747]\n- 국내에는 건설CALS/EC 전자도면 작성표준(V1.1 KCCS-0001-2006, 2006-12-26 한국건설기술연구원장 공고, 도면분류·파일명·선·색상·레이어·심벌 규정)과 국가표준 KS F 1542(CAD 도면 작성을 위한 레이어 원칙과 기준, 2020-12-21 확인)가 있다. 두 문서의 관계와 문·계단·승강기·충전 위치 레이어 코드 유무는 미확인이다. [사실][^ref-749][^ref-748]\n- ArchCAD-400K 는 CAD 도면의 레이어·블록 계층을 자동 라벨링에 이용하고, 2026-07-14 공개된 프리프린트는 CAD 텍스트 주석의 유형·속성을 선 요소 단위 심볼 스포팅에 결합하는 다중 모달 방법을 제안했다. [사실][^ref-753][^ref-752]\n- 래스터 평면도 연구(Buildings 15(7), 2025)는 치수선 검출과 문자 인식으로 축척을 계산해 정확도 95% 초과를 보고했다(저자 보고 단일 출처). [사실][^ref-754]\n- 국토교통부는 2022-07 BIM 성과품의 작성·납품·활용 방법과 절차를 제시하는 '건설산업 BIM 시행지침'을 발표했다. [사실][^ref-755]\n\n[^ref-738]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcDoor (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcDoor.md, 접근일 2026-09-25\n[^ref-739]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcStair (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcStair.md, 접근일 2026-09-25\n[^ref-740]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcTransportElementTypeEnum (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/core/IfcProductExtension/Types/IfcTransportElementTypeEnum.md, 접근일 2026-09-25\n[^ref-741]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcWall (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcWall.md, 접근일 2026-09-25\n[^ref-742]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcBuildingStorey (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/core/IfcProductExtension/Entities/IfcBuildingStorey.md, 접근일 2026-09-25\n[^ref-743]: Moitzi, M. (mozman/ezdxf GitHub), ezdxf documentation — Concepts: Blocks (docs/source/concepts/blocks.rst, Autodesk 공식 DXF 참조 아님), 미확인, https://github.com/mozman/ezdxf/blob/master/docs/source/concepts/blocks.rst, 접근일 2026-09-25\n[^ref-744]: Moitzi, M. (mozman/ezdxf GitHub), ezdxf documentation — Concepts: Layers (docs/source/concepts/layers.rst, Autodesk 공식 DXF 참조 아님), 미확인, https://github.com/mozman/ezdxf/blob/master/docs/source/concepts/layers.rst, 접근일 2026-09-25\n[^ref-745]: Moitzi, M. (mozman/ezdxf GitHub), ezdxf documentation — Concepts: DXF Units (docs/source/concepts/units.rst, Autodesk 공식 DXF 참조 아님), 미확인, https://github.com/mozman/ezdxf/blob/master/docs/source/concepts/units.rst, 접근일 2026-09-25\n[^ref-746]: ISO, ISO 13567-1:2017 - Technical product documentation — Organization and naming of layers for CAD — Part 1: Overview and principles, 2017, https://www.iso.org/standard/70181.html, 접근일 2026-09-25 (원문 미열람)\n[^ref-747]: National Institute of Building Sciences (United States National CAD Standard), AIA CAD Layer Guidelines, Layer Name Format (NCS V5), 미확인, https://www.nationalcadstandard.org/ncs5/pdfs/ncs5_clg_lnf.pdf, 접근일 2026-09-25 (원문 미열람)\n[^ref-748]: 국가표준인증통합정보시스템(KSSN), KS F 1542(2020 확인) CAD 도면 작성을 위한 레이어 원칙과 기준, 2020-12, https://www.kssn.net/search/stddetail.do?itemNo=K001010129900, 접근일 2026-09-25 (원문 미열람)\n[^ref-749]: 국토교통부 건설사업정보시스템(CALS), 건설CALS 전자도면 작성표준, 미확인, https://www.calspia.go.kr/portal/intro/introStandard02.do, 접근일 2026-09-25 (원문 미열람)\n[^ref-751]: Noardo, F., Arroyo Ohori, K., Krijnen, T., & Stoter, J. (Applied Sciences 11(5), 2232), An Inspection of IFC Models from Practice, 2021, https://www.mdpi.com/2076-3417/11/5/2232, 접근일 2026-09-25 (원문 미열람)\n[^ref-752]: arXiv 2607.12678 저자(미확인), Text-Aided Multi-Modal Panoptic Symbol Spotting for CAD Floor Plan Drawings, 2026-07-14, https://arxiv.org/abs/2607.12678, 접근일 2026-09-25 (원문 미열람)\n[^ref-753]: ArchiAI Lab (ArchCAD-400K 프로젝트), ArchCAD-400k: A Large-Scale CAD drawings Dataset and New Baseline for Panoptic Symbol Spotting — project page, 미확인, https://archiai-lab.github.io/ArchCAD.github.io/, 접근일 2026-09-25\n[^ref-754]: Buildings(MDPI) 게재 논문 저자(미확인), Raster Image-Based House-Type Recognition and Three-Dimensional Reconstruction Technology, 2025, https://doi.org/10.3390/buildings15071178, 접근일 2026-09-25 (원문 미열람)\n[^ref-755]: 국토교통부, 건설산업 BIM 시행지침 정책정보 상세보기, 2022-07, https://www.molit.go.kr/USR/policyData/m_34681/dtl.jsp?srch_usr_titl=Y&psize=10&lcmspage=1&id=4634, 접근일 2026-09-25 (원문 미열람)"
        }
      ]
    },
    {
  ...
  "glossary_updates": [
    ...
    },
    {
      ...
      "description": "ezdxf 문서 기준 설명이다. 도면 인식 연구(ArchCAD-400K)는 CAD 도면의 레이어·블록 계층을 자동 라벨링에 이용한다.",
      "related_areas": [
        6,
        27
      ],
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC 4.3 계단 엔터티 정의 원본. 다른 높이의 층 사이를 걸어서 오가게 하는 수직 통로로 정의하고, 계단 구간·참 슬래브·난간으로의 분해와 공간 컨테이너(기본 층, 불가 시 건물, 외부는 대지)를 설명한다.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
        "docs/tracks/floorplan-recognition/space-graph-schema-draft.md",
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC 4.3 벽 엔터티 정의 원본. 공간을 둘러싸거나 나누는 수직 구조로 정의하고 개구부(IfcRelVoidsElement)와 공간 포함 관계를 설명한다.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
        "docs/tracks/floorplan-recognition/space-graph-schema-draft.md",
    ...
    },
    {
      ...
      "reliability": "high",
      "accessed": "2026-09-25",
      "summary": "IFC 4.3 건물 층 엔터티 정의 원본. 수직으로 경계 지어진 공간들의 수평 집합으로 정의하고, Elevation 속성 폐기 예정과 속성 세트 기반 층 고도 사용 권고를 적는다.",
      "source_unopened": false,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
        "docs/ideas/floorplan-recognition.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 미국 국가 CAD 표준이 채택한 AIA 레이어 이름 형식(하이픈으로 나눈 분야·주 그룹 등 필드)을 설명한 V5 문서. 2026-09-25 확인 시점에 V6 판(ncs6_clg_lnf.pdf)이 있다.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
        "docs/tracks/floorplan-recognition/space-graph-schema-draft.md",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. CAD 도면의 레이어 원칙과 기준을 정한 국가표준(2020-12-21 확인, KS F 1540·1541 과 한 묶음, 유료).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
        "docs/ideas/floorplan-recognition.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 건설사업 전자도면의 도면분류·파일명·선·색상·레이어·심벌을 정한 표준의 소개 페이지(V1.0 2004-08, V1.1 KCCS-0001-2006 은 2006-12-26 한국건설기술연구원장 공고).",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
        "docs/ideas/floorplan-recognition.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. CAD 평면도 텍스트 주석의 유형·속성 의미를 선 요소 단위 심볼 스포팅에 결합하는 다중 모달 방법을 제안한 프리프린트.",
      "source_unopened": true,
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-2-data-and-standards.md",
        "docs/ideas/floorplan-recognition.md"
  ...
  "standards_updates": [
    {
      "name": "ISO 13567-1:2017 CAD 레이어 구성·명명 — Part 1: 개요와 원칙",
      "kind": "표준",
      "org": "ISO",
      "url": "https://www.iso.org/standard/70181.html",
      "related_areas": [
      ...
      "summary": "CAD 파일의 레이어 구조화 원칙과 책임 주체·요소·표현 등 고정 길이 필드로 된 레이어 이름 체계를 정한다(원문 미열람).",
      "ref_id": "ref-746"
    },
    {
      "name": "미국 국가 CAD 표준(NCS) AIA CAD 레이어 이름 형식(V5, V6 판 있음)",
      "kind": "표준",
      "org": "National Institute of Building Sciences",
      "url": "https://www.nationalcadstandard.org/ncs5/pdfs/ncs5_clg_lnf.pdf",
      "related_areas": [
        6,
        28
      ],
      "summary": "하이픈으로 나눈 분야 지정자·주 그룹 등 필드로 레이어 이름을 짓는 형식이며 A-DOOR·A-WALL 같은 이름을 둔다(V5 문서 기준, 원문 미열람).",
      "ref_id": "ref-747"
    },
    {
      "name": "KS F 1542 CAD 도면 작성을 위한 레이어 원칙과 기준",
      "kind": "표준",
      "org": "국가표준인증통합정보시스템(KSSN)",
      "url": "https://www.kssn.net/search/stddetail.do?itemNo=K001010129900",
      "related_areas": [
      ...
      "summary": "CAD 도면의 레이어 원칙과 기준을 정한 국가표준(2020-12-21 확인, 원문 미열람).",
      "ref_id": "ref-748"
    },
    {
      "name": "건설CALS/EC 전자도면 작성표준(V1.1, KCCS-0001-2006)",
      "kind": "표준",
      "org": "한국건설기술연구원(건설CALS 체계)",
      "url": "https://www.calspia.go.kr/portal/intro/introStandard02.do",
      "related_areas": [
        6,
        28
      ],
      "summary": "건설사업 전자도면의 도면분류·파일명·선·색상·레이어·심벌을 정한 표준(V1.0 2004-08, V1.1 2006-12-26 공고, 원문 미열람).",
      "ref_id": "ref-749"
    },
    {
      "name": "ezdxf (DXF 읽기·쓰기 라이브러리)",
      "kind": "오픈소스",
      "org": "Moitzi, M. (mozman/ezdxf GitHub)",
      "url": "https://github.com/mozman/ezdxf/blob/master/docs/source/concepts/blocks.rst",
      "related_areas": [
      ...
      "summary": "DXF 도면의 블록·레이어·단위를 다루는 오픈소스 라이브러리이며, 그 문서가 DXF 형식 개념을 설명한다(Autodesk 공식 참조 아님).",
      "ref_id": "ref-743"
    }
  ],
  ...
  "track_updates": {
    ...
    "log_entry": "답한 질문: q2-02(도면 입력 형식별 정보 항목, f1~f30) / 새 질문: q2-08(f15), q2-09(f7); 벡터 CAD 텍스트 결합 처리 흐름의 사람 검토 질문은 q3-01 과 중복이어서 등록하지 않음 / 온톨로지 변경: v0.4 → v0.5: 문 BIM 대응 클래스 IfcDoor 추가(f1), 계단 BIM 대응 클래스 IfcStair 추가(f3), 엘리베이터 유형 값 ELEVATOR 확정(f4), 평면도 길이 단위·축척 정보 속성 추가(f12); 거부: 계단이 잇는 층 도출(f6 추정 → 6절 질문 q3-05), 래스터 축척 복원 방식(f22 강등 → 6절 질문 q4-05); 근거 실행 2026-09-25-36 / 완료 조건 평가: 미충족(부족: 관제 수용 형식 q2-03 미작성, 관계(엣지) 쪽 표준 대응 없음) / 세부영역 반영 제안: 6. 지도·공간·위치 모델 2건, 28. 표준·상호운용성·다사업자 거버넌스 1건, 27. AI·학습·적응과 모델 운영 1건 / 다음 실행 제안: q2-03, q2-07 / 비고: 단계·스키마·개요 페이지의 H1 아래 상태 줄과 스키마 H1 버전 표기는 patches 범위 밖이라 갱신하지 못함",
    "overview_progress": "단계 2 진행 중 — 열린 질문 6, 답함 2, 완료 조건 미충족"
  },
  "area_reflection_proposals": [
    {
      "area_no": 6,
      "section": "7. 관련 표준·프레임워크·오픈소스",
      "summary": "도면 입력 형식별로 담기는 공간 정보와 빠지는 정보: IFC 4.3 의 IfcDoor·IfcStair·IfcTransportElement(ELEVATOR)·IfcBuildingStorey(ref-738~ref-742), DXF 좌표 무단위와 $INSUNITS(ezdxf 문서 기준, ref-745), 래스터 축척 메타데이터 사례(ref-069·ref-070, 일반화는 추정), 국내 CAD 레이어 표준(KS F 1542 ref-748, 건설CALS 전자도면 작성표준 ref-749), 충전 위치 표준 표현 부재(추정), BIM 기반 지도의 한계(연계 대상, ref-081)."
    },
    {
      "area_no": 6,
      "section": "8. 대표 연구와 자료",
      "summary": "교차 규칙(도면 해석은 6. 지도·공간·위치 모델에 적용)에 따라 ArchCAD-400K 레이어·블록 기반 자동 라벨링(ref-753), CAD 텍스트 결합 심볼 스포팅 프리프린트(ref-752, 2026-07-14), 래스터 치수 문자 인식 축척 계산(ref-754, 저자 보고 95% 초과)을 27. AI·학습·적응과 모델 운영 페이지와 함께 연결."
    },
    {
      "area_no": 28,
      "section": "7. 관련 표준·프레임워크·오픈소스",
      "summary": "CAD 레이어 명명 표준 ISO 13567-1:2017(ref-746), 미국 NCS AIA 레이어 형식(V5 기준, V6 판 있음, ref-747), KS F 1542(ref-748)와 건설CALS/EC 전자도면 작성표준(한국건설기술연구원 공고, ref-749)을 따로 기재(파생 관계 미확인), 실무 IFC 모델의 IfcBuildingElementProxy 오용 점검(Noardo 외 2021, ref-751), 국토교통부 건설산업 BIM 시행지침(2022-07, ref-755)."
    },
    {
      "area_no": 27,
      "section": "8. 대표 연구와 자료",
      "summary": "도면 해석 AI 방법으로 CAD 레이어·블록 계층을 이용한 자동 라벨링(ArchCAD-400K, ref-753), 텍스트 주석 유형·속성을 결합한 다중 모달 심볼 스포팅(arXiv 2607.12678, ref-752), YOLOv8·OFA-OCR 기반 래스터 축척 인식(ref-754)을 적용 대상 6. 지도·공간·위치 모델과 양쪽 연결."
    }
  ]
}
```

### runs/2026-09-25-36/pages/tracks/floorplan-recognition/stage-2-data-and-standards.md
...
### runs/2026-09-25-36/pages/tracks/floorplan-recognition/space-graph-schema-draft.md
...
- 계단·엘리베이터가 잇는 층을 계단·엘리베이터 개념의 속성 값으로 어떻게 채울지 정해지지 않았다. IFC 입력에서 계단·엘리베이터가 잇는 층은 직접 속성이 아니라 포함 관계·형상에서 도출해야 할 것으로 보여 v0.5의 개념 표에 넣지 않았다. [추정][^ref-738][^ref-741][^ref-739][^ref-334][^ref-156] — 관련: q3-05
- 래스터 평면도의 축척을 별도 메타데이터로 받을지, 도면 안 축척 표기·치수 문자 인식으로 복원할지 정해지지 않아 평면도의 길이 단위·축척 정보 속성 근거에는 DXF 헤더만 넣었다(v0.5). [추정][^ref-069][^ref-070][^ref-754] — 관련: q4-05
- 실무 IFC 모델에서 엘리베이터·문·계단이 범용 요소 IfcBuildingElementProxy 로 내보내지면 BIM 대응 클래스만으로는 해당 요소를 찾을 수 없을 것으로 보여, 입력 점검·보정 규칙을 어디에 둘지 정해야 한다. [추정][^ref-751][^ref-740][^ref-738] — 관련: q2-09
- 충전 위치의 표준 표현은 BIM뿐 아니라 확인한 CAD 레이어 표준 자료와 공개 평면도 데이터셋에서도 확인되지 않아, 세 입력 형식 모두에서 도면 밖 정보로 보완해야 할 것으로 보인다(레이어 표준 원문 미열람으로 부재 확정 아님). [추정][^ref-214][^ref-215][^ref-746][^ref-747][^ref-063][^ref-073] — 관련: q2-06, q2-08
...
[^ref-740]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcTransportElementTypeEnum (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/core/IfcProductExtension/Types/IfcTransportElementTypeEnum.md, 접근일 2026-09-25
[^ref-741]: buildingSMART (IFC4.3.x-development GitHub), IFC 4.3 — IfcWall (개발 브랜치 ifc4.3-main 원본, 게시판 IFC 4.3 ADD2와 문구가 다를 수 있음), 미확인, https://github.com/buildingSMART/IFC4.3.x-development/blob/ifc4.3-main/docs/schemas/shared/IfcSharedBldgElements/Entities/IfcWall.md, 접근일 2026-09-25
[^ref-745]: Moitzi, M. (mozman/ezdxf GitHub), ezdxf documentation — Concepts: DXF Units (docs/source/concepts/units.rst), 미확인, https://github.com/mozman/ezdxf/blob/master/docs/source/concepts/units.rst, 접근일 2026-09-25
[^ref-746]: ISO, ISO 13567-1:2017 - Technical product documentation — Organization and naming of layers for CAD — Part 1: Overview and principles, 2017, https://www.iso.org/standard/70181.html, 접근일 2026-09-25 (원문 미열람)
[^ref-747]: National Institute of Building Sciences (United States National CAD Standard), AIA CAD Layer Guidelines, Layer Name Format (NCS V5), 미확인, https://www.nationalcadstandard.org/ncs5/pdfs/ncs5_clg_lnf.pdf, 접근일 2026-09-25 (원문 미열람)
[^ref-751]: Applied Sciences(MDPI) 게재 논문 저자(미확인), An Inspection of IFC Models from Practice, 2021, https://www.mdpi.com/2076-3417/11/5/2232, 접근일 2026-09-25 (원문 미열람)
[^ref-754]: Buildings(MDPI) 게재 논문 저자(미확인), Raster Image-Based House-Type Recognition and Three-Dimensional Reconstruction Technology, 2025, https://doi.org/10.3390/buildings15071178, 접근일 2026-09-25 (원문 미열람)
````

### runs/2026-09-25-36/pages/ideas/floorplan-recognition.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-062, ref-063, ref-064, ref-065, ref-066, ref-067, ref-068, ref-069, ref-070, ref-071, ref-072, ref-073, ref-074, ref-075, ref-076, ref-077, ref-078, ref-079, ref-081, ref-082, ref-083, ref-084, ref-085, ref-086, ref-109, ref-120, ref-220, ref-221, ref-222, ref-223, ref-224, ref-225, ref-226, ref-227, ref-046, ref-212, ref-213, ref-214, ref-215, ref-216, ref-217, ref-218, ref-219, ref-241, ref-105, ref-163, ref-265, ref-267, ref-268, ref-271, ref-274, ref-156, ref-157, ref-158, ref-331, ref-332, ref-333, ref-334, ref-335, ref-336, ref-338, ref-339, ref-340, ref-341, ref-342, ref-345, ref-738, ref-739, ref-740, ref-741, ref-742, ref-743, ref-744, ref-745, ref-746, ref-747, ref-748, ref-749, ref-751, ref-752, ref-753, ref-754, ref-755]
last_run: 2026-09-25
version: 7
---
...
### runs/2026-09-25-36/pages/tracks/floorplan-recognition/index.md
...
### runs/2026-09-25-36/verification2.json
...
```json
{
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "단계 2 페이지 3절 q2-02 의 '입력 형식별 비교' 표와 아이디어 페이지 4절 '입력 형식별 정보 항목' 표가 같은 내용이다. 요약·상세 관계로 둔 것이며 기존 각주를 재사용했으므로 허용한다. 다만 두 표의 칸 문구가 서로 다르다(래스터 스캔 열의 문 칸)."
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "공간 그래프 스키마 초안(docs/tracks/floorplan-recognition/space-graph-schema-draft.md) 각주 [^ref-751]: 기관을 'Applied Sciences(MDPI) 게재 논문 저자(미확인)'에서 'Noardo, F., Arroyo Ohori, K., Krijnen, T., & Stoter, J. (Applied Sciences 11(5), 2232)'로 고친다. 이유: 1차 수정 지시 f7 이 이 페이지에는 반영되지 않았다. 단계 2 페이지·아이디어 페이지·reference_updates 와 각주 줄이 서로 다르고, fixes_applied 의 f7 보고와도 맞지 않는다.",
    "단계 2 페이지 3절 '입력 형식별 비교' 표, 문 행의 래스터 스캔 칸: '기호 모양에서 인식'에 '(추정)'을 붙인다(예: '기호 모양에서 인식(추정), 공개 데이터셋에 출입문 라벨 있음'). 이유: 이 내용은 f25(추정)에서 나왔다. 그런데 표 설명이 '(추정)을 붙인 칸만 추정'이라고 밝히고 있어, 표시가 없으면 사실처럼 읽힌다. 아이디어 페이지 표의 같은 칸은 이 구절이 없는 형태로 이미 되어 있다.",
    "단계 2 페이지 3절 표와 아이디어 페이지 4절 표, 충전 위치 행의 BIM(IFC 4.3) 칸: '표준 유형 값 없음'을 '확인한 콘센트·전기기기 유형 열거에 충전 설비 값 없음'처럼 범위를 한정한 문구로 고친다. 이유: f9 가 뒷받침하는 것은 IfcOutletTypeEnum·IfcElectricApplianceTypeEnum 두 열거뿐이다. 표 설명은 BIM 열을 '출처에서 확인한 사실'로 규정하므로, 지금 문구는 IFC 전체 유형 값에 대한 일반화가 된다."
  ],
  "confidence": "medium",
  "verification_note": "판정: 조건부 승인 / 2차 수정 후 재검증.\n이번 실행은 원문 열람이 차단된 환경에서 검증됐다(raw.githubusercontent.com 만 열림).\n\n**건수**\n- 확인 28건, 미확인 2건, 교차 확인 0건.\n- 강등: f22 사실 → 추정(두 데이터셋 사례를 래스터 입력 전체로 일반화), f24 사실 → 추정(엘리베이터 라벨 부재는 클래스 목록을 열람하지 못한 상태의 관찰).\n\n**원문 미열람 출처**\n- 신규: ref-746, ref-747, ref-748, ref-749, ref-750, ref-751, ref-752, ref-754, ref-755.\n- 재사용: ref-156, ref-334, ref-214, ref-215, ref-063, ref-066, ref-067, ref-069, ref-070, ref-073, ref-074, ref-078, ref-081, ref-084.\n\n**주의**\n- 입력 형식별 비교(f30)와 충전 위치 표준 표현 부재(f26)는 이 위키의 추정이다.\n- 모든 주장이 단일 발행 주체 자료에 기댄다.\n- DXF 진술은 Autodesk 공식 참조가 아닌 ezdxf 문서 기준이다.\n\n**온톨로지 변경**\n- 승인: 문 BIM 대응 클래스 IfcDoor(f1), 계단 BIM 대응 클래스 IfcStair(f3), 엘리베이터 유형 값 ELEVATOR 확정(f4), 평면도 길이 단위·축척 정보 속성(f12).\n- 거부: 계단이 잇는 층의 도출(f6 → 6절 질문 q3-05), 래스터 축척 복원 방식(f22 → 6절 질문 q4-05).\n\n**단계 판정**\n- 단계 완료 조건: 미충족(부족: 관제 수용 형식 q2-03 미작성, 관계(엣지) 쪽 표준 대응 없음).\n- 단계 전환: 미승인(막힌 질문 q2-03·q2-04·q2-06·q2-07·q2-08·q2-09 열림).\n- 정정 요청 없음.\n\n**2차 검증 결과**\n- 2차 수정 후 재검증: 드리프트 1건(단계 페이지 비교표의 태그 없는 추정 칸)과 범위 확대 1건(BIM 충전 칸)을 지적했다. 1차 수정 지시 17건 가운데 f7 이 스키마 초안 각주에 반영되지 않았다.\n- [분류원문] 보존, 섹션 순서 준수, 링크 유효.\n- 1차 수정 지시 가운데 f22·f24·f25·f2·f3·f5·f10~f12·f14·f15·f19·f27, 원문 미열람 표기, 온톨로지 v0.5 반영, 중복 질문 제외, 6절 완료 조건 표기는 이행을 확인했다.\n\n**퍼블리셔 처리 필요(스토리텔러 patches 범위 밖)**\n\n아래 항목이 반영되지 않으면 게시 페이지의 버전·상태 표기가 서로 어긋난다. 사용자 결정 항목으로 올린다.\n- 스키마 초안 H1 '(v0.4)'를 프런트매터 ontology_version '0.5'에 맞춰 '(v0.5)'로 고친다.\n- 단계 2 페이지 상태 줄을 '열린 질문 6건·답한 질문 2건'으로 고친다.\n- 트랙 개요 상태 줄과 track-progress 의 '현재 단계: 단계 1'을 단계 2 로 고친다. config/tracks/floorplan-recognition.yaml 의 current_stage 도 1 로 남아 있다.\n\n사소한 표기: 스키마 초안 6절 머리 문장에 v0.5 갱신(실행 2026-09-25-36)이 빠져 있다.",
  "retry_reason": null,
  "track_checks": {
