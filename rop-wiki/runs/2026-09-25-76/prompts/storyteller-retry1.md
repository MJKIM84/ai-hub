...
### runs/2026-09-25-76/research.json
...
```json
{
  ...
  "research_questions": [
    ...
    "q4-03 도면 좌표계와 로봇별 지도 좌표계를 정렬하고 층·목적지 이름을 맞추는 방법은 무엇인가?",
    "관제·상호운용 규격(Open-RMF 플릿 어댑터·traffic-editor, VDA 5050 3.0.0, MassRobotics AMR 상호운용 표준, ISO 21423)은 공통 좌표계의 원점·층 구분과 제조사 지도 좌표 변환을 무엇으로 정하는가? (단계 4 페이지 3절, oq-027 겨냥)",
    "대응점으로 두 좌표계의 변환을 추정하는 방법(유사 변환 최소제곱)과 도면–격자 지도 자동 정합 연구는 무엇을 가정하고 어떤 오차 지표를 주는가? (단계 4 페이지 3절, 21. 온보딩·설정·현장 시운전 oq-077 연결)",
    "층 이름(지도 층, 승강기 층, 실내 지도 표준의 층 순번·약칭)과 목적지 이름(경유점 이름, 스테이션 이름, GLN 하위 위치)은 형식마다 어떻게 표현되며 공통 키가 있는가? (oq-045·oq-029, 10. 설비·건물 시스템 연동·7. 화물·재고·자산 식별과 추적 연결)",
    "국내 실내공간정보 규정은 기준점 선정과 좌표 부여를 어떻게 정하는가? (한국 자료 우선 규칙, oq-044 관련)"
  ],
  "findings": [
    {
      "id": "f1",
      "claim": "Open-RMF 플릿 어댑터 튜토리얼은 로봇 지도와 RMF 좌표의 변환을 층(지도)마다 따로 구하며, 대응 경유점을 최소 4쌍 권장하고 nudged 라이브러리로 회전·축척·이동을 추정한 뒤 층별 평균제곱오차(MSE)를 기록하게 한다.",
      "tag": "사실",
      "source_ids": [
        "ref-153"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "\"A minimum of 4 matching waypoints is recommended.\" nudged.estimate() 로 rotation·scale·translation 을 구하고 nudged.estimate_error() 로 층별 MSE 를 로그에 남긴다(raw 원문 확인, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f2",
      "claim": "Open-RMF 공식 플릿 어댑터 템플릿 설정은 reference_coordinates 를 층 이름(예: L1)을 키로 하고 그 아래 RMF 좌표 목록과 로봇 좌표 목록을 같은 순서의 대응점 4쌍으로 적게 한다.",
      "tag": "사실",
      "source_ids": [
        "ref-105"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "reference_coordinates: L1: rmf: [[20.33, -3.156], …] robot: [[59, 399], …] — 층 키 외에 별도 map_name 필드는 없음(raw 원문 확인, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f3",
      "claim": "nudged 라이브러리 README 는 이 라이브러리를 반사 없는 유사 변환(이동·축척·회전)에 대한 최소제곱 추정기로 설명하고, 점 집합 크기에 선형인 시간으로 계산하며 평균제곱오차로 적합도를 분석하는 기능을 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-744"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "\"optimal least squares estimators for the group of nonreflective similarity transformation matrices\" — O(n), nudged.analysis.mse 제공. README 는 JavaScript 판(2.x) 기준이며 Open-RMF 가 쓰는 Python 판과의 동일성은 미확인(발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f4",
      "claim": "Umeyama(IEEE TPAMI 13(4), 1991)는 두 점 패턴 사이의 평균제곱오차를 최소화하는 유사 변환(회전·이동·축척)의 해를 제시했으며, 데이터가 크게 오염돼도 회전 대신 반사를 내는 기존 해의 문제를 피한다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-745"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "similarity transformation parameters (rotation, translation, and scaling) that give the least mean squared error between two point patterns; 기존 해는 심하게 오염된 데이터에서 반사를 내기도 했다(검색 요약 기준)",
      "as_of": "1991",
      "flow_step": null,
      "flow_item": null,
    ...
    {
      "id": "f5",
      "claim": "Open-RMF traffic-editor 는 층마다 이름과 고도(미터)를 두고, 실제 거리를 넣은 측정선으로 도면 축척을 정하며, 층 사이 수직으로 겹칠 기준점 2쌍 이상으로 이동·회전·축척 변환을 구하고, 로봇이 작업 목적지로 끝낼 경유점에는 이름을 붙여야 한다고 적는다.",
      "tag": "사실",
      "source_ids": [
        "ref-079"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "\"With two or more pairs of corresponding markers between a level and a reference level, a geometric transformation (translation, rotation and scale) may be derived.\" 목적지 경유점에는 name 이 필요(raw 원문 확인, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f6",
      "claim": "VDA 5050 3.0.0 은 위치를 관제와 로봇 사이에 정한 프로젝트 고유 좌표계로 주고 층·구역 구분에 고유 mapId 를 쓰며, initializePosition 동작은 x·y·theta·mapId·lastNodeId 로 자세를 재설정해 승강기 노드에서도 쓸 수 있고, pick·drop 동작은 선택 파라미터 stationName 으로 스테이션을 가리킨다.",
      "tag": "사실",
      "source_ids": [
        "ref-031"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "position is always specified in reference to the project-specific coordinate system; unique mapId for levels; initializePosition 은 node 범위 yes(Elevator); pick 파라미터 stationName(string, optional)(명세 3.0.0, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f7",
      "claim": "ISO/FDIS 21423 소개 자료는 공통 좌표계(CCS)의 원점을 시설 안에서 임의로 고른 한 점(벽·기둥·바닥 위의 점 등)으로 두고 이를 시설의 속성으로 보며, 공유되는 모든 위치 데이터를 그 원점에 대한 미터 단위 위치로 정한다고 전한다.",
      "tag": "사실",
      "source_ids": [
        "ref-746"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "CCS origin point is an arbitrarily selected point within a facility … designated as the (0,0,0) … property of the facility; locational data … relative to the CCS origin point, stated in meters(FDIS 미리보기 검색 요약 기준, 발행판 문구는 미확인)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f8",
      "claim": "MassRobotics AMR 상호운용 표준 JSON 스키마의 location 은 x·y·angle(쿼터니언)·planarDatum 을 필수로 두고 planarDatum 을 '로봇이 참조하는 planarDatum 의 id'(UUID)로 설명하며, 건물·층을 나타내는 별도 필드는 두지 않는다.",
      "tag": "사실",
      "source_ids": [
        "ref-230"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "location required: x, y, angle, planarDatum; planarDatum \"Id of planarDatum AMR is referencing\"(uuid), z 는 선택. 건물·층 필드 없음(raw 원문 확인, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f9",
      "claim": "Open-RMF 건물 지도 메시지에서 층(Level)은 이름·고도·이미지·장소·문·주행 그래프를 갖고, 승강기(Lift)는 운행 층을 '층 이름' 문자열 목록(levels)으로 두며 층 사이 정렬에 쓸 수 있는 칸 기준 방향(ref_x·ref_y·ref_yaw)을 둔다.",
      "tag": "사실",
      "source_ids": [
        "ref-346",
        "ref-743"
      ...
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "Lift.msg: string[] levels — floor names; ref_x, ref_y, ref_yaw — reference orientation of the lift cabin which can be used to align floors. Level.msg: name, elevation, images, places, doors, nav_graphs(raw 원문 확인, 같은 저장소라 독립 출처 아님)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f10",
      "claim": "Open-RMF 승강기 상태 메시지(LiftState)는 현재 층·목적 층·운행 가능 층을 주석 없는 문자열(current_floor, destination_floor, available_floors)로만 나타낸다.",
      "tag": "사실",
      "source_ids": [
        "ref-286"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "string current_floor / string destination_floor / string[] available_floors — 추가 주석 없음(raw 원문 확인, 발행일 미확인, 확인일 기준)",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f11",
      "claim": "IMDF 1.0.0 의 층(Level)은 지상 출입이 가능한 가장 낮은 층을 순번(ordinal) 0, 지하층을 음수로 두는 물리적 층 순번과 사람이 보는 약칭(short_name, 예: P1)을 따로 가지며, 층 표기 관례가 다른 층을 같은 순번의 여러 Level 로 모델링할 수 있다.",
      "tag": "사실",
      "source_ids": [
        "ref-338"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "the Level that models the lowest floor which supports ground-floor access … MUST have an ordinal equal to 0; short_name 은 물리적 층의 라벨; 표기 관례가 다르면 같은 ordinal 의 Level 여러 개 가능(검색 요약 기준)",
      "as_of": "2021-02",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f12",
      "claim": "GS1 GLN 은 확장 요소로 도크 문·보관 칸·판독 지점 같은 하위 위치를 식별할 수 있으나, 확장 요소는 조직 내부나 거래 당사자 간 합의로만 쓴다.",
      "tag": "사실",
      "source_ids": [
        "ref-162"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "extension component may identify sub-locations such as storage bins, dock doors … SHALL only be used internally by an organisation or through mutual agreement between partners(검색 요약 기준, 재인용: 2026-09-25-17)",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "완료·인계",
      "source_unopened": true
    ...
    {
      "id": "f13",
      "claim": "Carpin(Autonomous Robots, 2008)은 여러 로봇의 점유 격자 지도를 합치기 위해 허프 스펙트럼의 순환 상호상관으로 회전 후보를, 축별 투영 스펙트럼으로 이동을 구해 가중치가 붙은 변환 후보 여러 개를 결정적·비반복적으로 내는 방법을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-747"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "Hough spectra … circular cross-correlation … multiple local maxima, each associated with a possible rotation; returns n transformations, 가중치로 모호한 경우를 추적(검색 요약 기준)",
      "as_of": "2008",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f14",
      "claim": "Kakuma 외(2017)는 SLAM 으로 만든 점유 격자 지도와 건물 평면도를 그래프 매칭으로 대응시키고 정렬해, 로봇이 평면도가 가진 의미 정보(방 이름 등)에 접근하게 하는 방법을 제안했다.",
      "tag": "사실",
      "source_ids": [
        "ref-748"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "graph matching to align a floor map with an occupancy grid map generated by SLAM; floor map includes semantic information(검색 요약 기준, 학회명 표기는 요약마다 다름)",
      "as_of": "2017",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f15",
      "claim": "Hou·Kuang·Schwertfeger(ROBIO 2019)는 2D 점유 격자 지도를 방 분할 기반 영역 그래프(Area Graph)로 바꾼 뒤 그 공간에서 투표로 두 지도를 맞추는 방법을 제안하고, 대규모 지도에서 기존 방법보다 성능과 계산 속도가 낫다고 보고했다.",
      "tag": "사실",
      "source_ids": [
        "ref-749"
      ],
      "cross_checked": false,
      "confidence": "medium",
      "evidence_excerpt": "general 2D occupancy grid maps are transferred to an area graph representation, then computed through voting; better performance on large-scale maps and faster(검색 요약 기준, 저자 보고)",
      "as_of": "2019",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f16",
      "claim": "국토교통부 고시 '실내공간정보 구축 작업규정'은 기준점을 바닥 중심의 고정 시설물이나 선의 교차점에 두고 가상 표시는 피하게 하며, 절대좌표는 지상기준점 측량 성과나 수치지형도 가운데 활용 목적에 맞게 골라 부여하게 한다.",
      "tag": "사실",
      "source_ids": [
        "ref-345"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "기준점(선점)은 공간상의 고정되어 있는 바닥 중심의 시설물을 이용 … 가상적인 표시는 피하여야; 절대좌표 부여는 지상기준점 측량 성과물 또는 수치지형도 중 선택(검색 요약 기준, 해당 조문의 판 미확인)",
      "as_of": "2018-03-05",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f17",
      "claim": "국가법령정보센터에는 '실내공간정보구축작업규정'의 2021-12-24 판(고시 제2021-1445호) 항목이 있어, 2018-03-05 제정 이후 개정판이 있는 것으로 확인된다.",
      "tag": "사실",
      "source_ids": [
        "ref-750"
      ],
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "행정규칙 실내공간정보구축작업규정 (2021-1445, 20211224) 항목 URL — 개정 내용은 미확인(검색 결과 제목·URL 기준)",
      "as_of": "2021-12-24",
      "flow_step": null,
      "flow_item": null,
      "source_unopened": true
    ...
    {
      "id": "f18",
      "claim": "확인한 도구·규격을 이 위키가 묶으면 도면–로봇 지도 좌표 정렬 절차 초안은 (1) 시설 공통 좌표계의 원점과 층별 기준점을 도면 위 고정 지점에 정하고, (2) 측정선으로 도면 축척을, 층간 기준점으로 층 사이 변환을 정하며, (3) 제조사·플릿·층마다 대응점(최소 4쌍 권장)으로 유사 변환을 최소제곱 추정하고, (4) 잔차(MSE)를 확인한 뒤, (5) 층·장소 식별자 대응표를 등록하는 순서가 될 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-746",
        "ref-345",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. 원점(ISO 21423 FDIS 요약)·기준점 선정(국내 작업규정)·측정선·층간 기준점(traffic-editor)·층별 대응점 4쌍과 MSE(플릿 어댑터)를 한 절차로 묶은 단일 출처는 찾지 못함",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f19",
      "claim": "층은 형식마다 Open-RMF 층 이름·승강기 층 이름 문자열, VDA 5050 mapId, MassRobotics planarDatum UUID, IMDF 순번·약칭으로 따로 표현되고 공통 키가 없으므로, ROP 는 물리적 층 순번 같은 한 키에 시스템별 층 식별자를 별칭으로 매다는 층 대응표를 따로 두어야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-346",
        "ref-743",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. Lift.msg levels·LiftState 층은 주석 없는 문자열, MassRobotics 는 층 필드 없이 planarDatum 만, IMDF 는 ordinal 과 short_name 을 분리",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f20",
      "claim": "확인한 추정 방법은 층마다 균일 축척·반사 없는 유사 변환 하나를 가정하므로, 제조사 SLAM 지도에 국소 왜곡이 있으면 층 전체 잔차가 작아도 특정 목적지의 오차가 노드 허용 편차를 넘을 수 있어, 합격 판정은 층 평균 잔차가 아니라 목적지 대응점별 잔차로 해야 할 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-153",
        "ref-744",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 추론. 플릿 어댑터는 층별 MSE 만 기록하고, VDA 5050 은 노드마다 allowedDeviationXY 를 둔다. 도면–현장 편차 연구(ref-224)는 도면이 부정확할 수 있음을 전제로 함",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    },
    {
      "id": "f21",
      "claim": "목적지 이름은 공간 그래프의 구역 노드 이름을 기준 키로 두고, 그 아래에 제조사별 경유점 이름(Open-RMF)·노드 id 와 스테이션 이름(VDA 5050)·업무 위치 식별자(GLN 하위 위치 또는 WMS 로케이션 코드)를 대응시키는 대응표로 맞추는 방식이 될 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-079",
        "ref-031",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. traffic-editor 는 목적지 경유점에 이름이 필요하고, VDA 5050 은 stationName, GLN 확장 요소는 조직 내부 합의로만 쓴다. WMS 로케이션 코드와의 대응 사례는 찾지 못함",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "완료·인계",
      "vendor_claim": false
    ...
    {
      "id": "f22",
      "claim": "‘3층 출하 대기장’의 경우, 국내 층 표기상 ‘3층’이 지상 1층을 순번 0 으로 두는 IMDF 식 순번에서는 2 가 될 수 있으므로, 사람이 쓰는 층 이름과 순번·제조사 mapId 를 대응표로 명시하지 않으면 같은 이름이 다른 층으로 해석될 위험이 있어 보인다(설명용 가정 사례).",
      "tag": "추정",
      "source_ids": [
        "ref-338",
        "ref-031",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 추론. IMDF 는 지상 출입 최저층을 ordinal 0 으로 강제하고 short_name 을 따로 둔다. 국내 층 표기 관례와의 대응은 출처로 확인하지 않은 가정",
      "as_of": "2026-09-25",
      "flow_step": "출하",
      "flow_item": "완료·인계"
    },
    {
      "id": "f23",
      "claim": "이종 제조사를 연결하는 ROP 는 공통 좌표계 원점·층 대응표·목적지 대응표·제조사별 변환과 그 잔차 확인을 직접 맡고, 제조사 지도 작성과 위치추정은 로봇 쪽 연계 대상으로 두며, 격자 지도–도면 자동 정합 알고리즘은 대응점 입력을 줄이는 시운전 보조 도구 후보로 보는 경계가 될 것으로 보인다.",
      "tag": "추정",
      "source_ids": [
        "ref-153",
        "ref-031",
      ...
      "cross_checked": false,
      "confidence": "low",
      "evidence_excerpt": "이 위키의 종합. 플릿 어댑터·VDA 5050 은 관제 쪽에서 좌표 변환을 맡기고, 자동 정합 연구는 SLAM 격자 지도와 평면도 정렬을 다룸. 자동 정합을 물류 관제 시운전에 쓴 사례는 찾지 못함",
      "as_of": "2026-09-25",
      "flow_step": null,
      "flow_item": null
    }
  ...
  "sources": [
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "플릿 어댑터 통합 튜토리얼. 층별 대응 경유점으로 로봇–RMF 좌표 변환을 추정하고 오차를 확인하는 절차를 설명한다.",
      "fetched": true,
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "플릿 어댑터 템플릿 설정 파일. 층 이름을 키로 한 reference_coordinates(RMF·로봇 대응점)를 둔다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "traffic-editor 문서. 층 이름·고도, 측정선 축척, 층간 기준점 변환, 경유점 이름을 설명한다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "VDA 5050 3.0.0 명세. 프로젝트 고유 좌표계, 층별 mapId, initializePosition, pick·drop 의 stationName 을 정한다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": null,
      "source_unopened": false
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "MassRobotics AMR 상호운용 표준 JSON 스키마. location 은 x·y·angle·planarDatum(UUID)을 필수로 두고 층 필드는 없다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "Open-RMF 건물 지도 층 메시지. 이름·고도·이미지·장소·문·주행 그래프를 가진다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "Open-RMF 승강기 상태 메시지. 현재·목적·운행 가능 층을 문자열로 둔다.",
      "fetched": true,
      "fetched_via": "github_raw",
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 사람 길안내용 실내 지도 형식. Level 에 물리적 층 순번(ordinal)과 약칭(short_name)을 둔다.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. GLN 으로 물리적 위치와 하위 위치를 식별하는 방법. 확장 요소는 내부·합의 사용.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 실내공간정보 제작 기준 고시. 기준점 선정과 절대좌표 부여 방법을 정한다.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 부정확한 건축 도면과 라이다 상황 그래프를 결합해 도면–현장 정렬과 편차를 추정한다.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      "id": "ref-743",
      "org": "Open Robotics (open-rmf)",
      "title": "rmf_building_map_msgs — rmf_building_map_msgs/msg/Lift.msg",
      "published": null,
      "url": "https://github.com/open-rmf/rmf_building_map_msgs/blob/main/rmf_building_map_msgs/msg/Lift.msg",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "Open-RMF 건물 지도 승강기 메시지. 운행 층을 층 이름 문자열 목록으로, 층 정렬용 칸 기준 방향을 둔다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/open-rmf/rmf_building_map_msgs/main/rmf_building_map_msgs/msg/Lift.msg",
      "source_unopened": false
    },
    {
      "id": "ref-744",
      "org": "Palonen, A. (axelpale/nudged GitHub)",
      "title": "nudged — README (Affine transformation estimator e.g. for multi-touch gestures and calibration)",
      "published": null,
      "url": "https://github.com/axelpale/nudged",
      "type": "오픈소스 문서",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "2D 점 집합에서 이동·축척·회전(유사 변환)을 최소제곱으로 추정하는 라이브러리 README(JavaScript 판). Open-RMF 튜토리얼이 쓰는 Python 판과 같은 방법 계열이다.",
      "fetched": true,
      "fetched_via": "github_raw",
      "fetch_url": "https://raw.githubusercontent.com/axelpale/nudged/master/README.md",
      "source_unopened": false
    },
    {
      "id": "ref-745",
      "org": "Umeyama, S. (IEEE Transactions on Pattern Analysis and Machine Intelligence 13(4), 376-380)",
      "title": "Least-Squares Estimation of Transformation Parameters Between Two Point Patterns",
      "published": "1991",
      "url": "https://ieeexplore.ieee.org/document/88573/",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 두 점 패턴 사이 평균제곱오차를 최소화하는 유사 변환의 해를 제시한 논문.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "title": "ISO/FDIS 21423 - Robotics — Industrial mobile robots — Communications and interoperability",
      "published": null,
      "url": "https://standards.iteh.ai/catalog/standards/iso/f772b16e-582a-4ed4-9f79-0a80ad376f40/iso-fdis-21423",
      "type": "표준",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. ISO 21423 최종 국제표준안 미리보기. 공통 좌표계(CCS) 원점을 시설의 속성으로 정의한다고 검색 요약이 전한다.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    ...
    {
      "id": "ref-747",
      "org": "Carpin, S. (Autonomous Robots)",
      "title": "Fast and accurate map merging for multi-robot systems",
      "published": "2008",
      "url": "https://link.springer.com/article/10.1007/s10514-008-9097-4",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 허프 스펙트럼으로 점유 격자 지도 사이 변환 후보를 구하는 지도 병합 방법.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    ...
    {
      "id": "ref-748",
      "org": "Kakuma, D., Tsuichihara, S., Garcia Ricardez, G. A., Takamatsu, J., & Ogasawara, T.",
      "title": "Alignment of Occupancy Grid and Floor Maps Using Graph Matching",
      "published": "2017",
      "url": "https://ieeexplore.ieee.org/document/7889504/",
      "type": "논문",
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. SLAM 점유 격자 지도와 건물 평면도를 그래프 매칭으로 대응·정렬하는 방법.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 점유 격자 지도를 영역 그래프로 바꿔 투표로 두 지도를 맞추는 방법.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. 실내공간정보 구축 작업규정의 2021-12-24 판 항목. 개정 내용은 확인하지 못했다.",
      "fetched": false,
      "fetched_via": null,
      "fetch_url": null,
      "source_unopened": true
  ...
  "page_proposals": [
    ...
    },
    {
      ...
      "rationale": "아이디어 페이지 5절(트랙 산출물): '좌표 정렬과 층·목적지 이름 맞춤' 소절 신설 — 근거 f1·f5·f6·f7·f8·f9·f11·f12, 구현 가설 f18(정합 절차 초안)·f19·f21·f23(추정). q4-02 소절과 합쳐 도면–현장 정합 절차 초안이 됨을 명시"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 산출물 갱신: track.ontology_changes(개념 '좌표계 정렬' 추가, 층 '시스템별 층 식별자' 속성)가 승인되면 2절 반영과 v1.1 인상(f1·f2·f5·f6·f8·f9·f10·f11). 미승인 시 6절 '정렬 정보를 층별 지도 속성으로 둘지 별도 개념으로 둘지' 항목과 '형식별 층·장소 식별자 대응 규칙' 항목의 근거 보강(f18·f19·f21)"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 4 반영 제안 (f1, f7, f8, f11, f18, f19, f21, f23): 6절(주제 페이지 area06-s6)에 대응점 유사 변환과 잔차 확인·층/목적지 대응표(추정), 7절(주제 페이지 area06-s7)에 ISO 21423 CCS 원점 정의(FDIS 요약)·MassRobotics planarDatum·IMDF 층 순번, 9절에 대응표·변환은 ROP, 지도 작성·위치추정은 연계 대상이라는 경계(추정). oq-027·oq-045 근거 보강"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 4 반영 제안 (f1, f16, f20, f14, f15): 시운전에서 층별 대응점 4쌍 이상과 목적지별 잔차로 정렬을 합격 판정하는 방법(추정)과 도면–격자 지도 자동 정합 연구, 국내 작업규정의 기준점 선정. oq-077 근거 보강"
    },
    {
      "action": "update",
      ...
      "rationale": "트랙 floorplan-recognition 단계 4 반영 제안 (f9, f10, f19): 승강기 층 이름이 주석 없는 문자열이라 지도 층 이름과의 대응표가 필요하다는 점(oq-045 근거 보강)"
    }
  ],
  "glossary_candidates": [
    {
    ...
    "ontology_changes": [
      {
        "op": "add",
        "kind": "concept",
        "name": "좌표계 정렬 (Coordinate Alignment)",
      ...
      },
      {
        ...
        "description": "속성 '시스템별 층 식별자(별칭)'를 더한다: Open-RMF 건물 지도 층 이름·승강기 층 이름 문자열, VDA 5050 mapId, MassRobotics planarDatum UUID, IMDF 순번(ordinal)·약칭(short_name). 기존 속성 '층 이름'·'표준 대응 클래스(후보)'와 충돌하지 않으며, 기준 키를 무엇으로 둘지(f19 추정)는 정의에 넣지 않고 6절 질문으로 둔다."
      }
    ],
...
### runs/2026-09-25-76/verification.json
...
```json
{
  ...
  "claim_checks": [
    {
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-153 raw 원문(github_raw) 재열람. 층(지도)별 변환, 'A minimum of 4 matching waypoints is recommended', nudged 사용과 변환 오차 추정값 로그 기록이 원문에 있음. 'MSE' 표기는 nudged 의 오차 함수 이름에 기댄 것이므로 페이지에서는 '변환 오차 추정값(평균제곱오차)'로 쓴다. 발행일 미확인."
    },
    {
      "finding_id": "f2",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-105 raw 원문 재열람. reference_coordinates 아래 층 키 L1, rmf 좌표 4개·robot 좌표 4개(4쌍). 별도 map_name 필드 없음은 해당 절 범위 기준. f1 과 같은 Open Robotics 계열이라 독립 교차 확인 아님."
    },
    {
      "finding_id": "f3",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-744 raw README 재열람. 'optimal least squares estimators for the group of nonreflective similarity transformation matrices', O(n), nudged.analysis.mse 확인. README 는 JavaScript 2.x 기준이며 Python 판 존재만 언급 — Open-RMF 가 쓰는 Python 판과의 구현 동일성 미확인 표기 유지."
    },
    {
      "finding_id": "f4",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 일치). IEEE TPAMI 13(4) 376-380, 1991 확인(dblp·IEEE Xplore·ADS 검색 결과). 유사 변환 최소 평균제곱오차 해와 기존 해(Arun·Horn)의 반사 문제 회피가 검색 요약에 나타남. 발행 30년 이상의 방법론 논문으로 최신성 문제 없음."
    },
    {
      "finding_id": "f5",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-079 raw 원문 재열람. 층 이름·고도(미터), 측정선 distance 로 Scale 갱신, 'With two or more pairs of corresponding markers ... translation, rotation and scale' 인용 일치, 작업 목적지 경유점에 name 필요 확인. 층간 변환(기준점 2쌍 이상)과 f1 의 제조사 지도 변환(4쌍 권장)은 서로 다른 변환임을 페이지에서 구분해야 함."
    },
    {
      "finding_id": "f6",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: 입력 원문 텍스트 data/source_texts/ref-031.txt(6.3 Maps, 6.2.3.1 표). 프로젝트 고유 좌표계, 층 구분용 고유 mapId, initializePosition 파라미터 x·y·theta·mapId·lastNodeId 와 node 범위 'yes (Elevator)', pick·drop 의 stationName(string, optional) 모두 원문에 있음. 명세 3.0.0, 발행일 미확인(oq-005). 브리프의 fetched_via 가 github_raw 로 적혀 있으나 실제 열람 경로는 inbox(원문 텍스트)이고 fetch_url 이 null — 브리프 표기 오류(verification_note 기록)."
    },
    {
      "finding_id": "f7",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 일치). iTeh Standards 의 ISO/FDIS 21423 미리보기 URL·제목이 검색 결과와 일치하고, 검색 요약에 CCS 원점이 시설 안 임의 선택 지점(벽·기둥·바닥의 점)으로 (0,0,0) 이며 시설의 속성이라는 문구가 나타남. '미터 단위'는 브리프 스니펫 기준이며 이번 검색 요약에는 직접 나타나지 않음 — 페이지에서 'FDIS 미리보기 검색 요약 기준, 발행판 문구 미확인' 병기 필수. iTeh 는 ISO 발행 기관이 아닌 판매처 미리보기이므로 발행 기관 자료(ref-159)와 함께 표시. oq-027 해결 근거 아님."
    },
    {
      "finding_id": "f8",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-230 raw JSON 재열람. location 필수 x·y·angle·planarDatum, planarDatum 설명 'Id of planarDatum AMR is referencing'(uuid), z 선택, 건물·층 필드 없음 확인."
    },
    {
      "finding_id": "f9",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-346(Level.msg)·ref-743(Lift.msg) raw 재열람. Level: name·elevation·images·places·doors·nav_graphs(+wall_graph). Lift: string[] levels, ref_x·ref_y·ref_yaw(층 정렬용 칸 기준 방향), width·depth. 두 출처는 같은 저장소라 독립 교차 확인 아님(브리프 표기와 일치)."
    },
    {
      "finding_id": "f10",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "확인: ref-286 raw 재열람. current_floor·destination_floor 문자열, available_floors 문자열 배열. 층 필드에 별도 주석 없음은 열람 범위 기준."
    },
    {
      "finding_id": "f11",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "강등",
      "note": "원문 미열람(검색 결과 일치). 검색 요약으로 지상 출입 최저층 ordinal 0·지하층 음수·short_name(예: P1) 분리는 확인됨. 그러나 '층 표기 관례가 다른 층을 같은 순번의 여러 Level 로 모델링할 수 있다'는 이번 검증 검색 요약에 나타나지 않아 확인하지 못함 — 이 절만 [추정]으로 분리하거나 삭제(부분 강등). 앞 두 절은 [사실] 유지 가능. 발행일 2021-02(OGC 20-094)."
    },
    {
      "finding_id": "f12",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 일치). GS1 검색 결과(확장 요소 페이지·GS1 UK·Healthcare GLN 지침)에 확장 요소로 보관 칸·도크 문·판독 지점 등 하위 위치 식별, 거래 당사자 간에는 상호 합의로만 교환이 나타남. 기존 6. 지도·공간·위치 모델 5절의 ref-162 문장과 같은 주장이므로 기존 각주 재사용."
    },
    {
      "finding_id": "f13",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 일치). Autonomous Robots 25, 305-316(2008), Springer URL 일치. 허프 스펙트럼 상관으로 회전, x-y 투영으로 이동, 가중치 붙은 변환 여러 개, 결정적·비반복 확인. SLAM 격자 지도 병합 알고리즘이라 로봇 쪽 기술과 맞닿음 — 시운전 보조 도구 후보로만 서술."
    },
    {
      "finding_id": "f14",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 일치). IEEE Xplore 7889504, 2017 확인. 검색 요약상 게재처는 IEEE ICSC 2017(Semantic Computing), 결과는 '대략적(gross) 정렬' 수준 — 페이지에서 정렬 정밀도를 과장하지 않는다."
    },
    {
      "finding_id": "f15",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 일치). arXiv 1911.07432, ROBIO 2019 게재 확인. 영역 그래프 변환·투표, 대규모 지도 성능·속도 우위는 저자 보고. 검색 요약상 CAD 도면과 로봇 지도 정합에도 쓸 수 있다고 적음."
    },
    {
      "finding_id": "f16",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 일치). 국가법령정보센터 URL 일치, 검색 요약에 기준점 선점은 바닥 중심 고정 시설물·선상 교차점, 가상 표시 회피, 절대좌표는 지상기준점 측량 성과물 또는 수치지형도 선택 부여가 나타남. 해당 조문이 2018-03-05 제정판 기준인지 2021 개정판 기준인지 미확인 — 신뢰도 low 유지, 판 미확인 병기."
    },
    {
      "finding_id": "f17",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "원문 미열람(검색 결과 제목·URL 일치). 2021-1445, 2021-12-24 항목 존재 확인. 개정 내용 미확인."
    },
    {
      "finding_id": "f18",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "이 위키의 종합([추정], low). 각 단계의 근거(f1·f2·f3·f4·f5·f6·f7·f16)가 위에서 확인됨. 다섯 단계 절차를 제시한 단일 출처 없음을 명시하고, 도식은 직접 그린다. 원점 정의는 FDIS 요약, 기준점 선점은 판 미확인 규정에 기댐."
    },
    {
      "finding_id": "f19",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "[추정] 유지. 다만 MassRobotics planarDatum 은 '로봇이 참조하는 기준면 id'이고 층 필드가 아니므로 '층을 나타내는 식별자'로 단정하지 말고 '층 구분에 쓰일 수 있는 식별자로 보이는'으로 쓴다. Lift.msg levels 의 주석 표현과 LiftState 의 무주석 문자열 차이도 구분."
    },
    {
      "finding_id": "f20",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "[추정] 유지. VDA 5050 allowedDeviationXY 가 노드마다 있음은 ref-031 6.6.2 절로 확인, 플릿 어댑터의 층별 오차 기록은 f1 로 확인. 목적지별 잔차 합격 판정은 이 위키의 추론이며 근거 사례 없음."
    },
    {
      "finding_id": "f21",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "[추정] 유지. traffic-editor 목적지 경유점 이름(f5), VDA 5050 stationName(f6), GLN 확장 요소 합의 사용(f12) 확인. GLN·WMS 로케이션 코드의 부여·관리는 상위 업무 시스템 쪽 연계 대상으로 서술. WMS 대응 사례 없음 명시."
    },
    {
      "finding_id": "f22",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "[추정] 유지, 설명용 가정 사례임을 명시. IMDF 의 ordinal 0 규칙은 확인됐으나 국내 층 표기('1층'=지상 출입층) 관례는 출처 미확인 가정이다."
    },
    {
      "finding_id": "f23",
      "source_exists": true,
      ...
      "cross_checked": false,
      "tag_decision": "유지",
      "note": "[추정] 유지. 분류 원문 9장(로봇 자체 지능·제어 경계, 이종 제조사 ROP 는 인터페이스·실행 보장 담당)과 맞는 경계 서술. 제조사 지도 작성·위치추정은 연계 대상, 자동 정합 알고리즘은 시운전 보조 도구 후보로만."
    }
  ],
  "category_fit": {
    "ok": true,
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "f12 는 6. 지도·공간·위치 모델 5절의 GLN 문장([사실][^ref-162])과 같은 주장 — 기존 각주 ref-162 재사용",
      "f7(ISO 21423 CCS 원점, ref-746 FDIS 미리보기)은 6. 지도·공간·위치 모델 11절·oq-027 의 ISO 21423 문장(ref-159)과 겹침 — oq-027 은 해결로 바꾸지 않고 근거 보강만",
      "f1·f5·f6 은 단계 4 페이지 q4-01 절의 기존 문장(ref-153·ref-079·ref-031)과 같은 사실 — 기존 각주 재사용, 새 문장은 층별·플릿별 설정과 목적지 이름 부분만",
      "온톨로지 제안 '좌표계 정렬'은 실행 2026-09-25-72 에서 거부된 층별 지도 속성 '로봇 지도 좌표계 변환'과 같은 대상이고, 기존 개념 '층간 정렬 기준점'·용어집 '지도 정합(Map Alignment)'과 범위가 겹침"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "required_fixes": [
    "f11: '층 표기 관례가 다른 층을 같은 순번의 여러 Level 로 모델링할 수 있다'는 절을 [사실] 문장에서 떼어 [추정]으로 두거나 삭제한다 — 검증 검색 요약에서 확인되지 않았다. ordinal 0·지하층 음수·short_name 분리는 [사실][^ref-338] 유지(원문 미열람 표기).",
    "f7: ISO 21423 CCS 문장에는 'FDIS 미리보기(iTeh Standards) 검색 요약 기준, 발행판 문구·층별 원점 여부·기준점 개수 미확인'을 병기하고, 발행 기관 페이지 각주 ref-159 를 함께 둔다. oq-027 은 해결로 바꾸지 않고 근거 보강으로만 적는다.",
    "f19: MassRobotics planarDatum 을 '층 식별자'로 단정하지 말고 '층 구분에 쓰일 수 있는 기준면 식별자로 보이는'으로 쓴다 — 스키마에는 층 필드가 없고 planarDatum 은 로봇이 참조하는 기준면 id 로만 정의된다.",
    "f1·f5: 제조사 지도–공통 좌표 변환(대응 경유점 4쌍 권장, 플릿 어댑터)과 층–기준층 변환(기준점 2쌍 이상, traffic-editor)을 서로 다른 변환으로 구분해 서술한다.",
    "f3: nudged README 는 JavaScript 2.x 기준이며 Open-RMF 튜토리얼이 쓰는 Python 판과의 구현 동일성은 미확인임을 병기한다.",
    "f16·f17: 실내공간정보 구축 작업규정 문장은 '2018-03-05 제정판 기준으로 검색됨, 현행 조문 미확인, 2021-12-24 개정판 존재(개정 내용 미확인)'로 쓴다.",
    "f18: 도면–로봇 지도 좌표 정렬 절차 초안은 [추정](이 위키의 종합, 단일 출처 없음, 신뢰도 low)으로 표기하고 mermaid 도식은 직접 그린다. 공간 그래프 스키마 초안 6절과 아이디어 3. 건축 도면 자동 인식 5절에는 q4-02 답(도면–현장 차이 탐지·반영)과 합쳐 '도면–현장 정합 절차 초안(추정)'으로 싣는다.",
    "f22: '설명용 가정 사례'임을 밝히고, 국내 층 표기('1층'이 지상 출입층)와 IMDF ordinal 의 대응은 출처로 확인하지 않은 가정이라고 적는다.",
    "f13·f14·f15: 격자 지도 병합·도면 정합 알고리즘은 제조사 SLAM 지도를 입력으로 하는 시운전 보조 도구 후보(추정)로만 두고, 지도 작성·위치추정 자체는 분류 원문 9장 '로봇 자체 지능·제어'의 연계 대상임을 적는다. f14 는 '대략적 정렬' 수준으로 보고됐음을 과장하지 않는다.",
    "f21: GLN·WMS 로케이션 코드의 부여·관리는 상위 업무 시스템 쪽 연계 대상이며 ROP 는 대응표만 둔다는 경계를 유지하고, WMS 로케이션 코드–경유점 대응 사례는 찾지 못했음을(oq-029 미해결) 적는다.",
    "온톨로지 변경 '층 (Floor)' 속성 '시스템별 층 식별자(별칭)'는 승인한다: 값 후보는 Open-RMF 건물 지도 층 이름·승강기 층 이름 문자열, VDA 5050 mapId, IMDF 순번(ordinal)·약칭(short_name)으로 두고 행 상태는 '확정' 유지, ontology_version 은 '1.0' → '1.1'(H1·상태 표시·프런트매터·JSON 모두). MassRobotics planarDatum 은 층 필드가 아니므로 이 속성 값에 넣지 않고 6절 층 식별자 대응 항목의 메모로만 둔다.",
    "온톨로지 변경 '좌표계 정렬 (Coordinate Alignment)' 개념 추가는 반영하지 않고 6절 '정렬 정보를 층별 지도 속성으로 둘지 별도 개념으로 둘지' 항목의 근거 보강으로 둔다 — 근거: 플릿 어댑터 설정이 층 키 아래 플릿별 대응점을 두어 층당 여러 변환이 생길 수 있음(f1·f2)은 별도 개념 쪽 논거이나, VDA 5050 은 관제와 로봇이 공유하는 프로젝트 고유 좌표계를 쓰므로 로봇별 변환 기록 근거가 아니고(f6), 기존 개념 '층간 정렬 기준점'·용어집 '지도 정합'과 범위 관계가 정해지지 않았다.",
    "단계 4 페이지 6절: 완료 조건 2행(도면–현장 정합 절차 초안)은 두 페이지 반영 시 '충족(1차 예비, 2차 확인)'으로, 검증 판정 칸은 '충족 · 미승인'으로 적고, 아래 줄은 '다음 단계로 전환: 아니오(막힌 질문 q4-04·q4-05·q4-07·q4-08·q4-09·q4-10·q4-11 및 이번 새 질문)'로 둔다. 상태 줄은 '진행 중'. track_updates.stage_transition 은 넣지 않는다.",
    "q4-03: 백로그·단계 페이지 2절에서 '답함'(답한 실행 2026-09-25-76, 답 위치 #q4-03), 3절 소제목 '### q4-03 … {#q4-03}'을 둔다. 답의 핵심 종합(f18~f23)은 [추정]이므로 절 첫 문단에 종합 신뢰도 low 를 밝힌다.",
    "원문 미열람 출처(ref-745·ref-746·ref-747·ref-748·ref-749·ref-750·ref-338·ref-162·ref-345·ref-224)의 각주 정의에는 접근일 뒤 ' (원문 미열람)'을 붙이고 reference_updates 의 해당 항목에 source_unopened: true 를 넣는다. raw 로 연 출처(ref-153·ref-105·ref-079·ref-230·ref-346·ref-286·ref-743·ref-744)와 입력 원문 ref-031 에는 붙이지 않는다.",
    "세부영역 반영 제안(6. 지도·공간·위치 모델, 21. 온보딩·설정·현장 시운전, 10. 설비·건물 시스템 연동)은 세부영역 페이지를 직접 고치지 않고 트랙 로그의 반영 제안으로만 남긴다. oq-027·oq-045·oq-029 는 해결로 바꾸지 않는다."
  ],
  "confidence": "low",
  "verification_note": "판정: 1차 조건부 승인. 이번 실행은 원문 열람이 차단된 환경(fetch_mode mirror_only)에서 검증됐다. 확인 22건, 미확인 1건(f11 일부 절), 교차 확인 0건. 강등: f11 사실 → 일부 절(같은 순번 여러 Level)만 추정 분리. 원문 미열람 출처: ref-745, ref-746, ref-747, ref-748, ref-749, ref-750, ref-338, ref-162, ref-345, ref-224. 주의: q4-03 답의 핵심인 좌표 정렬 절차 초안·층/목적지 대응표·목적지별 잔차 합격 기준(f18~f23)은 이 위키의 종합 추정이며 단일 출처가 없다. ISO 21423 공통 좌표계 정의는 FDIS 미리보기 검색 요약 기준이고, 국내 작업규정 조문의 판은 미확인이다. 브리프 표기 오류: ref-031 은 입력 원문 텍스트로 열었으나 fetched_via 가 github_raw, fetch_url 이 null 로 적혔다(실제는 inbox). 검증 검색 8회(리서치 16회 포함 누계 24/40), raw 원문 재열람 8건. 온톨로지 변경 승인: 층(Floor) 속성 '시스템별 층 식별자(별칭)'(f6·f9·f10·f11, MassRobotics planarDatum 제외) / 거부: 개념 '좌표계 정렬'(f1·f2·f5·f6·f8 — 6절 정렬 정보 질문 근거 보강으로 이동). 단계 완료 조건: 충족(예비 — 보정 항목 목록 기존 충족, 도면–현장 정합 절차 초안은 q4-02·q4-03 답을 스키마 초안 6절과 아이디어 3. 건축 도면 자동 인식 5절에 반영하는 조건으로, 2차에서 확정). 단계 전환: 미승인(막힌 질문 q4-04·q4-05·q4-07·q4-08·q4-09·q4-10·q4-11 과 이번 새 질문 2건이 열림).",
  "retry_reason": null,
  "track_checks": {
...
### docs/references/index.md (요약: 이번 대상 페이지가 인용한 161건 / 전체 680건. 목록에 없는 출처는 새 id 로 적는다 — 같은 URL 이 이미 있으면 퍼블리셔가 기존 id 로 합친다)
...
### docs/glossary/index.md (요약: 용어 175개, slug: 한국어 (영어). 정의는 docs/glossary/<slug>.md)
...
| SLAM2REF (라이다 데이터의 기준 지도 다중 세션 정렬 도구) | Vega-Torres, M. A. (MigVega GitHub) | 오픈소스 | [6. 지도·공간·위치 모델](../categories/b-common-information-and-environment-model/06-map-space-and-location-model.md)<br>[21. 온보딩·설정·현장 시운전](../categories/f-deployment-verification-and-maintenance/21-onboarding-configuration-and-commissioning.md) | [ref-650](../references/ref-650.md) | <https://github.com/MigVega/SLAM2REF> |
| Open-RMF rmf_task_ros2 디스패처·입찰 경매자(평가기) | Open Robotics (open-rmf) | 오픈소스 | [13. 작업 배정 — MRTA](../categories/d-planning-and-optimization/13-task-allocation-mrta.md)<br>[9. 로봇·제조사 관제 연동](../categories/c-connectivity-and-execution-foundation/09-robot-and-vendor-fleet-manager-integration.md) | [ref-656](../references/ref-656.md) | <https://github.com/open-rmf/rmf_ros2/blob/main/rmf_task_ros2/src/rmf_task_ros2/Dispatcher.cpp> |
| Rasa 폴백·사람 인계(Fallback and Human Handoff, Rasa 3.x) | Rasa Technologies | 오픈소스 | [18. 사람–로봇 협업·운영 인터페이스](../categories/e-collaboration-and-field-operations/18-human-robot-collaboration-and-operator-interface.md)<br>[27. AI·학습·적응과 모델 운영](../categories/g-safety-security-intelligence-and-governance/27-ai-learning-adaptation-and-model-operations.md) | [ref-658](../references/ref-658.md) | <https://github.com/RasaHQ/rasa/blob/main/docs/docs/fallback-handoff.mdx> |
...
### runs/2026-09-25-76/docs_tree.txt
...
````json
{
  ...
  "outline": [
    {
      ...
      "section": "2. 질문 목록",
      "budget_chars": 1800,
      "summary": "q4-03 을 답함(실행 2026-09-25-76, #q4-03)으로 바꾸고 후속 질문 q4-12·q4-13 을 열림으로 더한다.",
      "planned_findings": [
        "f19",
        "f20"
      ]
    },
    {
      ...
      "section": "3. 조사 결과",
      "budget_chars": 6500,
      "summary": "도면 좌표계와 로봇별 지도 좌표계의 정렬은 원점·기준점 지정 → 축척·층간 변환 → 제조사·플릿·층별 유사 변환 → 잔차 확인 → 층·장소 식별자 대응표 등록 순서가 될 것으로 보인다. [추정][^ref-746][^ref-079][^ref-153]",
      "planned_findings": [
        "f1",
        "f2",
        "f3",
    ...
    },
    {
      ...
      "budget_chars": 2600,
      "summary": "좌표 정렬 절차·층/목적지 대응표·목적지별 잔차 판정은 추정이며, ISO 21423 발행판과 국내 사례가 미확인이다. [추정][^ref-746][^ref-153]",
      "planned_findings": [
        "f18",
        "f19",
        "f20",
    ...
    },
    {
      ...
      "budget_chars": 1200,
      "summary": "국소 왜곡과 목적지별 잔차(q4-12), 층 대응표의 기준 키와 겹치는 층 표현(q4-13)을 더한다.",
      "planned_findings": [
        "f19",
        "f20"
      ]
    },
    {
      ...
      "budget_chars": 700,
      "summary": "도면–현장 정합 절차 초안 행은 충족(1차 예비, 2차 확인), 검증 판정 충족 · 미승인, 단계 전환 아니오.",
      "planned_findings": [
        "f18"
      ]
    },
    {
      ...
      "section": "7. 관련 세부영역",
      "budget_chars": 500,
      "summary": "q4-03 반영 제안: 6. 지도·공간·위치 모델 6·7·9·11절, 21. 온보딩·설정·현장 시운전 6절, 10. 설비·건물 시스템 연동 6절.",
      "planned_findings": []
    },
    {
      "path": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
    ...
    },
    {
      ...
      "section": "2. 개념 목록 표",
      "budget_chars": 600,
      "summary": "층에 속성 '시스템별 층 식별자(별칭)'를 더해 v1.1 로 올린다. [사실][^ref-031][^ref-346]",
      "planned_findings": [
        "f6",
        "f9",
        "f10",
    ...
    },
    {
      ...
      "budget_chars": 2200,
      "summary": "정렬 정보·층/장소 식별자 대응 항목 근거 보강과 도면–현장 정합 절차 초안(추정). [추정][^ref-153][^ref-031]",
      "planned_findings": [
        "f1",
        "f2",
        "f5",
    ...
    },
    {
      ...
      "section": "5. 구현 가설",
      "budget_chars": 2800,
      "summary": "좌표 정렬과 층·목적지 이름 맞춤 소절과 q4-02·q4-03 을 합친 도면–현장 정합 절차 초안(추정)을 더한다. [추정][^ref-153][^ref-079]",
      "planned_findings": [
        "f1",
        "f5",
        "f6",
    ...
    },
    {
      ...
      "budget_chars": 500,
      "summary": "실행 2026-09-25-76 의 산출물 변화(v1.1, 정합 절차 초안) 한 단락.",
      "planned_findings": []
    }
  ],
  "pages": [
    {
      ...
      "patches": [
        ...
        },
        {
          "section": "8. 출처",
          "action": "append",
          "content": "[^ref-105]: Open Robotics (open-rmf), fleet_adapter_template — fleet_adapter_template/config.yaml, 미확인, https://github.com/open-rmf/fleet_adapter_template/blob/main/fleet_adapter_template/config.yaml, 접근일 2026-09-25\n[^ref-744]: Palonen, A. (axelpale/nudged GitHub), nudged — README (Affine transformation estimator e.g. for multi-touch gestures and calibration), 미확인, https://github.com/axelpale/nudged, 접근일 2026-09-25\n[^ref-745]: Umeyama, S. (IEEE Transactions on Pattern Analysis and Machine Intelligence 13(4), 376-380), Least-Squares Estimation of Transformation Parameters Between Two Point Patterns, 1991, https://ieeexplore.ieee.org/document/88573/, 접근일 2026-09-25 (원문 미열람)\n[^ref-746]: ISO (iTeh Standards 미리보기), ISO/FDIS 21423 - Robotics — Industrial mobile robots — Communications and interoperability, 미확인, https://standards.iteh.ai/catalog/standards/iso/f772b16e-582a-4ed4-9f79-0a80ad376f40/iso-fdis-21423, 접근일 2026-09-25 (원문 미열람)\n[^ref-159]: ISO, ISO 21423 - Robotics — Industrial mobile robots — Communications and interoperability, 미확인, https://www.iso.org/standard/86749.html, 접근일 2026-09-25 (원문 미열람)\n[^ref-345]: 국토교통부(법제처 국가법령정보센터), 실내공간정보 구축 작업규정, 2018-03-05, https://law.go.kr/admRulLsInfoP.do?admRulSeq=2100000116559, 접근일 2026-09-25 (원문 미열람)\n[^ref-750]: 국토교통부(법제처 국가법령정보센터), 실내공간정보구축작업규정 (고시 제2021-1445호, 2021-12-24), 2021-12-24, https://www.law.go.kr/%ED%96%89%EC%A0%95%EA%B7%9C%EC%B9%99/%EC%8B%A4%EB%82%B4%EA%B3%B5%EA%B0%84%EC%A0%95%EB%B3%B4%EA%B5%AC%EC%B6%95%EC%9E%91%EC%97%85%EA%B7%9C%EC%A0%95/(2021-1445,20211224), 접근일 2026-09-25 (원문 미열람)\n[^ref-230]: MassRobotics, MassRobotics-AMR/AMR_Interop_Standard — AMR_Interop_Standard.json, 미확인, https://github.com/MassRobotics-AMR/AMR_Interop_Standard/blob/main/AMR_Interop_Standard.json, 접근일 2026-09-25\n[^ref-346]: Open Robotics (open-rmf), rmf_building_map_msgs — rmf_building_map_msgs/msg/Level.msg, 미확인, https://github.com/open-rmf/rmf_building_map_msgs/blob/main/rmf_building_map_msgs/msg/Level.msg, 접근일 2026-09-25\n[^ref-743]: Open Robotics (open-rmf), rmf_building_map_msgs — rmf_building_map_msgs/msg/Lift.msg, 미확인, https://github.com/open-rmf/rmf_building_map_msgs/blob/main/rmf_building_map_msgs/msg/Lift.msg, 접근일 2026-09-25\n[^ref-286]: Open Robotics (open-rmf), rmf_internal_msgs — rmf_lift_msgs/msg/LiftState.msg, 미확인, https://github.com/open-rmf/rmf_internal_msgs/blob/main/rmf_lift_msgs/msg/LiftState.msg, 접근일 2026-09-25\n[^ref-338]: OGC (Open Geospatial Consortium) / Apple, Indoor Mapping Data Format (1.0.0) — OGC Community Standard 20-094, 2021-02, https://docs.ogc.org/cs/20-094/, 접근일 2026-09-25 (원문 미열람)\n[^ref-162]: GS1, Identifying a physical location - GLN, 미확인, https://www.gs1.org/standards/id-keys/gln/physical-location, 접근일 2026-09-25 (원문 미열람)\n[^ref-747]: Carpin, S. (Autonomous Robots), Fast and accurate map merging for multi-robot systems, 2008, https://link.springer.com/article/10.1007/s10514-008-9097-4, 접근일 2026-09-25 (원문 미열람)\n[^ref-748]: Kakuma, D., Tsuichihara, S., Garcia Ricardez, G. A., Takamatsu, J., & Ogasawara, T., Alignment of Occupancy Grid and Floor Maps Using Graph Matching, 2017, https://ieeexplore.ieee.org/document/7889504/, 접근일 2026-09-25 (원문 미열람)\n[^ref-749]: Hou, J., Kuang, H., & Schwertfeger, S. (ROBIO 2019), Fast 2D Map Matching Based on Area Graphs, 2019, https://arxiv.org/abs/1911.07432, 접근일 2026-09-25 (원문 미열람)",
          "frontmatter": {
            "related_areas": [
              6,
              21,
    ...
    },
    {
      ...
      "action": "update",
      "status": "draft",
      "diff_summary": "6절에 실행 2026-09-25-76 산출물 변화(스키마 초안 v1.1, 도면–현장 정합 절차 초안, q4-03 답함·q4-12·q4-13) 단락 추가",
      "patches": [
        {
          "section": "6. 살아있는 산출물 링크",
          "action": "append",
          "content": "\n실행 2026-09-25-76(단계 4, q4-03)에서는 [공간 그래프 스키마 초안](space-graph-schema-draft.md)의 층에 속성 '시스템별 층 식별자(별칭)'를 더해 v1.1 로 올렸고(위 목록의 '현재 버전 v1.0'은 v1.1 로 읽는다), 개념 '좌표계 정렬' 추가 제안은 반영하지 않고 6절 정렬 정보 항목의 근거 보강으로 두었다. 스키마 초안 6절과 [아이디어 3. 건축 도면 자동 인식](../../ideas/floorplan-recognition.md) 5절에는 q4-02·q4-03 답을 합친 도면–현장 정합 절차 초안(추정)을 실었다. [질문 백로그](question-backlog.md)에는 q4-03 답함과 후속 질문 q4-12·q4-13 이 반영된다. 단계 전환은 승인되지 않았다."
        }
      ]
    }
  ],
  "changelog_entry": "2026-09-25 | 건축 도면 자동 인식 단계 4 | q4-03 답함(좌표 정렬 절차·층/목적지 대응표, 추정 중심), 공간 그래프 스키마 초안 v1.1(층 속성 '시스템별 층 식별자' 추가), 도면–현장 정합 절차 초안(추정) 반영 | run 2026-09-25-76",
  "index_updates": {
    "home_recent": "2026-09-25 — 건축 도면 자동 인식 단계 4: q4-03 답함(도면–로봇 지도 좌표 정렬과 층·목적지 대응표, 추정 중심), 공간 그래프 스키마 초안 v1.1, 도면–현장 정합 절차 초안(추정)",
    "category_recent": "2026-09-25 — 6. 지도·공간·위치 모델(건축 도면 자동 인식 트랙 단계 4): 좌표 정렬과 층·목적지 이름 맞춤 조사, 세부영역 반영 제안 4건",
    "area_recent": "2026-09-25 — 6. 지도·공간·위치 모델: 트랙 건축 도면 자동 인식 q4-03 에서 좌표 정렬·층/목적지 대응표 조사(추정 중심), 6·7·9·11절 반영 제안(실행 2026-09-25-76)"
  },
  "glossary_updates": [
    {
      "action": "new",
      "slug": "similarity-transformation",
      "term_ko": "유사 변환",
      "term_en": "Similarity Transformation",
      "definition": "회전·이동·균일 축척만으로 한 좌표계의 점을 다른 좌표계로 옮기는 변환으로, 대응점 쌍에서 최소제곱으로 추정해 도면·관제 지도와 제조사 로봇 지도를 맞추는 데 쓴다.",
      "description": "Open-RMF 플릿 어댑터 튜토리얼은 층마다 대응 경유점(최소 4쌍 권장)으로 이 변환을 추정하고 변환 오차 추정값을 기록하게 한다. 반사 없는 유사 변환의 최소제곱 해는 Umeyama(1991)가 제시했다.",
      "related_areas": [
        6,
        21,
        9
    ...
    {
      "action": "new",
      "slug": "common-coordinate-system",
      "term_ko": "공통 좌표계",
      "term_en": "Common Coordinate System (CCS, ISO 21423)",
      "definition": "ISO 21423 이 시설 안의 한 점을 원점으로 정해 여러 제조사 이동로봇이 위치를 미터 단위로 함께 표현하게 하는 시설 공통 좌표계다(최종안 요약 기준).",
      "description": "FDIS 미리보기 검색 요약 기준이며 발행판 문구·층별 원점 여부·기준점 개수는 미확인이다(열린 질문 oq-027).",
      "related_areas": [
        6,
        28,
        9
  ...
  "reference_updates": [
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "2D 점 집합에서 이동·축척·회전(유사 변환)을 최소제곱으로 추정하는 라이브러리 README(JavaScript 판). Open-RMF 튜토리얼이 쓰는 Python 판과의 구현 동일성은 미확인이다.",
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
        "docs/tracks/floorplan-recognition/space-graph-schema-draft.md",
        "docs/ideas/floorplan-recognition.md"
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. ISO 21423 발행 기관 페이지. 산업용 이동로봇의 통신·상호운용성 표준이다.",
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
        "docs/ideas/floorplan-recognition.md"
      ],
    ...
    },
    {
      ...
      "reliability": "medium",
      "accessed": "2026-09-25",
      "summary": "원문 미열람. SLAM 점유 격자 지도와 건물 평면도를 그래프 매칭으로 대응·정렬하는 방법(대략적 정렬 수준 보고).",
      "cited_by": [
        "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
        "docs/ideas/floorplan-recognition.md"
      ],
  ...
  "flow_matrix_updates": [
    {
      "step": "출하",
      "item": "수행 자원",
      "link": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md#q4-03",
      "title": "단계 4. 지도 변환 보정과 현장 정합"
    },
    {
      "step": "출하",
  ...
  "standards_updates": [
    {
      ...
      "summary": "2D 점 집합에서 반사 없는 유사 변환(이동·축척·회전)을 최소제곱으로 추정하는 라이브러리. Open-RMF 플릿 어댑터 튜토리얼이 로봇–RMF 좌표 변환 추정에 쓰며, README 는 JavaScript 판 기준이라 Python 판과의 구현 동일성은 미확인이다.",
      "ref_id": "ref-744"
    }
  ],
  ...
  "fixes_applied": [
    ...
    "docs/ideas/floorplan-recognition.md: 각주 정의 11개를 참고문헌에서 만들어 붙임: ref-159, ref-162, ref-230, ref-286, ref-346, ref-743, ref-744, ref-746, ref-747, ref-748, ref-749"
  ],
  "track_updates": {
    "stage_page": "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md",
    "ontology_draft_version": "1.1",
    "backlog_updates": [
      {
        "id": "q4-03",
        "status": "답함",
    ...
    "log_entry": "답한 질문: q4-03(좌표 정렬 절차와 층·목적지 대응표, 근거 f1~f23, 핵심 종합 f18~f23 은 추정, 종합 신뢰도 low) / 새 질문: q4-12(f20, 국소 왜곡과 목적지별 잔차 판정), q4-13(f19, 층 대응표 기준 키와 겹치는 층) / 온톨로지 변경: v1.0 → v1.1: 층 속성 '시스템별 층 식별자(별칭)' 추가(f6·f9·f10·f11, MassRobotics planarDatum 제외 — 6절 메모); 거부: 개념 '좌표계 정렬'(f1·f2·f5·f6·f8 → 6절 정렬 정보 항목 근거 보강). 버전 이력 행: 1.1 | 2026-09-25 | 층 속성 '시스템별 층 식별자(별칭)' 추가(f6·f9·f10·f11), 거부: 개념 '좌표계 정렬'(6절 근거 보강) | 2026-09-25-76 / 완료 조건 평가: 보정 항목 목록 충족(2차 확인), 도면–현장 정합 절차 초안 충족(1차 예비, 2차 확인 — q4-02·q4-03 답을 스키마 초안 6절과 아이디어 3. 건축 도면 자동 인식 5절에 반영), 단계 전환 미승인(열린 질문 q4-04·q4-05·q4-07·q4-08·q4-09·q4-10·q4-11·q4-12·q4-13) / 세부영역 반영 제안: 6. 지도·공간·위치 모델 4건(6·7·9·11절), 21. 온보딩·설정·현장 시운전 1건(6절), 10. 설비·건물 시스템 연동 1건(6절) / 다음 실행 제안: q4-04(도면·지도 버전 관리와 재검증), 이어서 q4-07(지도 파일 배포와 mapVersion)",
    "overview_progress": "단계 4 진행 중 — 열린 질문 9, 답함 3(q4-01·q4-02·q4-03), 완료 조건 충족(1차 예비 판정, 2차 확정 대기), 단계 전환 미승인"
  },
  "area_reflection_proposals": [
    {
      "area_no": 6,
      "section": "6. 대표 접근법과 기술",
      "summary": "층별·플릿별 대응 경유점(최소 4쌍 권장)으로 유사 변환을 최소제곱 추정하고 변환 오차 추정값을 확인하는 방법(사실, ref-153·ref-105·ref-744·ref-745)과 traffic-editor 의 층–기준층 변환(기준점 2쌍 이상, ref-079)이 서로 다른 변환이라는 점, 정합 절차 초안과 목적지 대응점별 잔차 판정(추정)."
    },
    {
      "area_no": 6,
      "section": "7. 관련 표준·프레임워크·오픈소스",
      "summary": "ISO/FDIS 21423 공통 좌표계 원점 정의(FDIS 미리보기 검색 요약 기준, 발행판 미확인, ref-746·ref-159), MassRobotics location 의 planarDatum(층 필드 없음, ref-230), IMDF 층 순번·약칭(ref-338), Open-RMF Level·Lift 층 이름(ref-346·ref-743), nudged 라이브러리(ref-744)."
    },
    {
      "area_no": 6,
      "section": "9. ROP가 직접 맡는 것과 외부와 연계하는 것 (부록 A 9장 기준)",
      "summary": "공통 좌표계 원점·층 대응표·목적지 대응표·제조사별 변환과 잔차 확인은 ROP, 제조사 지도 작성·위치추정과 격자 지도–도면 자동 정합 알고리즘은 로봇 쪽 연계 대상(시운전 보조 도구 후보), GLN·WMS 로케이션 코드 부여는 상위 업무 시스템 연계 대상이라는 경계(추정)."
    },
    {
      "area_no": 6,
      "section": "11. 열린 질문",
      "summary": "oq-027(ISO 21423 CCS)·oq-045(지도 층 이름–승강기 층 이름)·oq-029(GLN·WMS 로케이션–경유점)의 근거 보강. 모두 해결 아님."
    },
    {
      "area_no": 21,
      "section": "6. 대표 접근법과 기술",
      "summary": "시운전에서 층별 대응점 4쌍 이상과 목적지 대응점별 잔차로 정렬을 합격 판정하는 방법(추정), 국내 실내공간정보 구축 작업규정의 기준점 선정(2018 제정판 기준 검색, 현행 미확인), 격자 지도–도면 자동 정합 연구(Carpin 2008, Kakuma 외 2017 대략적 정렬, Hou 외 2019). oq-077 근거 보강."
    },
    {
      "area_no": 10,
      "section": "6. 대표 접근법과 기술",
      "summary": "Open-RMF 승강기 상태의 층이 주석 없는 문자열(ref-286)이고 승강기 메시지는 운행 층을 층 이름 목록으로 두므로(ref-743), 지도 층 이름과의 층 대응표가 필요할 것으로 보인다(추정, oq-045)."
    }
  ]
}
````

### runs/2026-09-25-76/pages/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md
...
### runs/2026-09-25-76/pages/tracks/floorplan-recognition/space-graph-schema-draft.md
...
created: 2026-09-25
updated: 2026-09-25
sources: [ref-031, ref-063, ref-064, ref-065, ref-068, ref-069, ref-070, ref-071, ref-072, ref-078, ref-079, ref-080, ref-081, ref-082, ref-084, ref-085, ref-086, ref-223, ref-224, ref-046, ref-212, ref-213, ref-214, ref-215, ref-216, ref-156, ref-331, ref-332, ref-333, ref-334, ref-336, ref-338, ref-339, ref-343, ref-348, ref-419, ref-420, ref-421, ref-422, ref-426, ref-427, ref-428, ref-432, ref-435, ref-073, ref-346, ref-440, ref-441, ref-442, ref-536, ref-640, ref-414, ref-413, ref-642, ref-573, ref-574, ref-461, ref-229, ref-228, ref-283, ref-575, ref-315, ref-406, ref-629, ref-153, ref-644, ref-645, ref-648, ref-569, ref-651, ref-221, ref-653, ref-652, ref-654, ref-649, ref-105, ref-230, ref-743, ref-286, ref-746, ref-159, ref-345, ref-750, ref-744, ref-745, ref-162]
last_run: 2026-09-25
version: 13
---
...
### runs/2026-09-25-76/pages/ideas/floorplan-recognition.md
...
updated: 2026-09-25
sources: [ref-031, ref-062, ref-063, ref-064, ref-065, ref-066, ref-067, ref-068, ref-069, ref-070, ref-071, ref-072, ref-073, ref-074, ref-075, ref-076, ref-077, ref-078, ref-079, ref-081, ref-082, ref-083, ref-084, ref-085, ref-086, ref-109, ref-120, ref-220, ref-221, ref-222, ref-223, ref-224, ref-225, ref-226, ref-227, ref-046, ref-212, ref-213, ref-214, ref-215, ref-216, ref-217, ref-218, ref-219, ref-241, ref-105, ref-163, ref-265, ref-267, ref-268, ref-271, ref-274, ref-156, ref-157, ref-158, ref-331, ref-332, ref-333, ref-334, ref-335, ref-336, ref-338, ref-339, ref-340, ref-341, ref-342, ref-345, ref-419, ref-420, ref-421, ref-422, ref-423, ref-424, ref-425, ref-426, ref-427, ref-428, ref-429, ref-430, ref-432, ref-433, ref-434, ref-435, ref-436, ref-440, ref-441, ref-442, ref-456, ref-457, ref-458, ref-459, ref-462, ref-463, ref-536, ref-413, ref-641, ref-642, ref-228, ref-572, ref-573, ref-574, ref-229, ref-038, ref-575, ref-576, ref-315, ref-314, ref-283, ref-348, ref-461, ref-406,
  ref-629, ref-632, ref-153, ref-644, ref-645, ref-648, ref-270, ref-651, ref-653, ref-649, ref-569, ref-230, ref-346, ref-743, ref-286, ref-744, ref-746, ref-159, ref-162, ref-747, ref-748, ref-749]
last_run: 2026-09-25
version: 15
---
...
### runs/2026-09-25-76/pages/tracks/floorplan-recognition/index.md
...
### runs/2026-09-25-76/verification2.json
...
```json
{
  ...
  "duplication": {
    "ok": true,
    "overlaps": [
      "docs/ideas/floorplan-recognition.md 5절: 기존 '내비게이션 지도 변환 보정' 소절 끝 문장(도면–현장 정합 절차 q4-03 아직 조사되지 않음)과 '도면–현장 차이 탐지와 반영' 소절 끝 문장(q4-03 미조사, 정합 절차 초안 미완성)이 이번에 추가한 '좌표 정렬과 층·목적지 이름 맞춤'·'도면–현장 정합 절차 초안(추정)' 소절과 모순된 채 남아 있음"
    ]
  },
  "terminology": {
    "ok": true,
  ...
  "corrections_rejected": [],
  "required_fixes": [
    "docs/ideas/floorplan-recognition.md 5. 구현 가설: 패치 action 을 append 가 아니라 replace 로 보내, '내비게이션 지도 변환 보정' 소절 끝 문장 '도면–현장 정합 절차(q4-02 …, q4-03 …)는 아직 조사되지 않았으며 단계 4 후속 실행이 채운다'와 '도면–현장 차이 탐지와 반영' 소절 끝 문장 '좌표 정렬과 층·목적지 이름 맞춤(q4-03)은 아직 조사되지 않았으므로 도면–현장 정합 절차 초안은 아직 완성되지 않았다'를 이번 실행 결과(q4-02는 실행 2026-09-25-75, q4-03은 실행 2026-09-25-76에서 답했고 정합 절차 초안(추정)은 아래 소절에 있다)로 고치고, 새 소절의 메타 문장 '위 소절 끝의 … 문장은 이 초안으로 갱신된다'를 지운다 — 게시 페이지에 사실과 어긋나는 미조사 서술이 남는다.",
    "docs/tracks/floorplan-recognition/space-graph-schema-draft.md: H1 을 '공간 그래프 스키마 초안 (v1.1)'로 고친다 — 프런트매터 ontology_version·JSON ontology_draft_version 이 '1.1'인데 H1 이 '(v1.0)'으로 남아 네 곳이 일치하지 않는다(패치로 H1 을 바꿀 수 없으면 퍼블리셔 패치 적용 코드가 프런트매터 값으로 H1 을 맞추는지 확인한 뒤 재제출한다).",
    "docs/tracks/floorplan-recognition/space-graph-schema-draft.md 프런트매터 sources 에서 본문에 인용되지 않고 각주 정의도 없는 ref-159·ref-745·ref-750 을 뺀다. reference_updates 의 ref-745·ref-750 cited_by 에서도 space-graph-schema-draft.md 를 뺀다 — 프런트매터 sources·cited_by 가 실제 각주와 일치해야 한다.",
    "docs/tracks/floorplan-recognition/stage-4-map-conversion-and-site-alignment.md H1 아래 상태 줄을 '> 단계 상태: 진행 중 · 열린 질문: 9건 · 답한 질문: 3건 · 완료 조건: 충족 · 마지막 실행: 2026-09-25'로 고친다 — 2절 표(답함 q4-01·q4-02·q4-03, 열림 q4-04·q4-05·q4-07~q4-13)와 6절 표(두 행 모두 충족, 2차에서 확정)와 숫자·값이 맞지 않는다(패치로 고칠 수 없으면 패치 적용 코드 처리를 요청하고 fixes_applied 에 적는다).",
    "단계 4 페이지 7. 관련 세부영역: 프런트매터 related_areas 에 10이 추가됐으나 절 목록에는 10. 설비·건물 시스템 연동 항목이 없다 — '프런트매터 related_areas는 아래 목록과 같다' 문장과 맞도록 [10. 설비·건물 시스템 연동](../../categories/c-connectivity-and-execution-foundation/10-facility-and-building-system-integration.md) 항목(승강기 층 이름이 주석 없는 문자열이라 지도 층 이름과의 층 대응표가 필요하다는 연결, 추정)을 목록에 더한다.",
    "docs/tracks/floorplan-recognition/index.md 6. 살아있는 산출물 링크: 패치를 replace 로 보내 공간 그래프 스키마 초안 항목의 '현재 버전 v1.0'을 'v1.1'로 고치고, 덧붙인 단락의 괄호 '(위 목록의 '현재 버전 v1.0'은 v1.1 로 읽는다)'를 지운다 — 독자에게 본문을 고쳐 읽으라는 문장은 쓰지 않는다.",
    "index_updates.category_recent 의 '세부영역 반영 제안 4건'을 area_reflection_proposals 와 맞게 '세부영역 반영 제안 6건(6. 지도·공간·위치 모델 4건, 21. 온보딩·설정·현장 시운전 1건, 10. 설비·건물 시스템 연동 1건)'으로 고친다."
  ],
  "confidence": "low",
  "verification_note": "판정: 1차 조건부 승인 / 2차 수정 후 재검증. 이번 실행은 원문 열람이 차단된 환경(fetch_mode mirror_only)에서 검증됐다. 확인 22건, 미확인 1건(f11 일부 절), 교차 확인 0건. 강등: f11 사실 → 일부 절(같은 순번 여러 Level)은 본문에서 삭제. 원문 미열람 출처: ref-745, ref-746, ref-747, ref-748, ref-749, ref-750, ref-338, ref-162, ref-345, ref-224, ref-159. 주의: q4-03 답의 핵심인 좌표 정렬 절차 초안·층/목적지 대응표·목적지별 잔차 합격 기준은 이 위키의 종합 추정이며 단일 출처가 없다. ISO 21423 공통 좌표계 정의는 FDIS 미리보기 검색 요약 기준이고, 국내 작업규정 조문의 판은 미확인이다. 브리프 표기 오류: ref-031 은 입력 원문 텍스트로 열었으나 fetched_via 가 github_raw 로 적혔다. 온톨로지 변경 승인: 층(Floor) 속성 '시스템별 층 식별자(별칭)'(f6·f9·f10·f11, MassRobotics planarDatum 제외) / 거부: 개념 '좌표계 정렬'(f1·f2·f5·f6·f8 — 6절 정렬 정보 항목 근거 보강). 단계 완료 조건: 충족(보정 항목 목록, 도면–현장 정합 절차 초안이 공간 그래프 스키마 초안 6절과 아이디어 3. 건축 도면 자동 인식 5절에 실린 것을 2차에서 확인). 단계 전환: 미승인(막힌 질문 q4-04·q4-05·q4-07·q4-08·q4-09·q4-10·q4-11·q4-12·q4-13 열림). / 2차 수정 후 재검증. 드리프트 없음, [분류원문] 보존, 1차 수정 지시 16건 이행 확인. 남은 지적 7건: 아이디어 페이지 5절의 'q4-03 미조사·정합 절차 미완성' 문장이 남아 새 소절과 모순됨, 스키마 초안 H1 버전 표기 v1.0(프런트매터 1.1), 스키마 초안 프런트매터의 미인용 출처(ref-159·ref-745·ref-750), 단계 4 페이지 상태 줄 수치·완료 조건 값 불일치, 7절 목록과 related_areas 불일치, 트랙 개요 6절의 버전 표기, category_recent 건수.",
  "retry_reason": null,
  "track_checks": {
    "standard_sources_ok": true,
    "vendor_claims_tagged": true,
...
- 트랙 검사(track_checks): {"standard_sources_ok": true, "vendor_claims_tagged": true, "ontology_changes_grounded": false, "backlog_duplicates": [], "stage_tag_issues": [], "completeness_wording_ok": true, "stage_complete": true, "stage_transition_approved": false}
